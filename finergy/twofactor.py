...
from finergy.utils import cint, get_datetime, get_url, time_diff_in_seconds
from finergy.utils.background_jobs import enqueue
from finergy.utils.password import decrypt, encrypt

PARENT_FOR_DEFAULTS = "__2fa"


def get_default(key):
	return finergy.db.get_default(key, parent=PARENT_FOR_DEFAULTS)


def set_default(key, value):
	finergy.db.set_default(key, value, parent=PARENT_FOR_DEFAULTS)


def clear_default(key):
	finergy.defaults.clear_default(key, parent=PARENT_FOR_DEFAULTS)
...
def toggle_two_factor_auth(state, roles=None):
	...
	for role in roles or []:
		role = finergy.get_doc("Role", {"role_name": role})
...
def two_factor_is_enabled(user=None):
	...
	enabled = int(finergy.db.get_value("System Settings", None, "enable_two_factor_auth") or 0)
	if enabled:
		bypass_two_factor_auth = int(
			finergy.db.get_value("System Settings", None, "bypass_2fa_for_retricted_ip_users") or 0
		)
		if bypass_two_factor_auth and user:
			...
			if restrict_ip_list and finergy.local.request_ip:
				for ip in restrict_ip_list:
					if finergy.local.request_ip.startswith(ip):
						enabled = False
						break
...
def get_cached_user_pass():
	...
	user = pwd = None
	tmp_id = finergy.form_dict.get("tmp_id")
	if tmp_id:
		user = finergy.safe_decode(finergy.cache().get(tmp_id + "_usr"))
		pwd = finergy.safe_decode(finergy.cache().get(tmp_id + "_pwd"))
	return (user, pwd)


def authenticate_for_2factor(user):
	"""Authenticate two factor for enabled user before login."""
	if finergy.form_dict.get("otp"):
		return
	otp_secret = get_otpsecret_for_(user)
	token = int(pyotp.TOTP(otp_secret).now())
	tmp_id = finergy.generate_hash(length=8)
	cache_2fa_data(user, token, otp_secret, tmp_id)
	verification_obj = get_verification_obj(user, token, otp_secret)
	# Save data in local
	finergy.local.response["verification"] = verification_obj
	finergy.local.response["tmp_id"] = tmp_id


def cache_2fa_data(user, token, otp_secret, tmp_id):
	"""Cache and set expiry for data."""
	pwd = finergy.form_dict.get("pwd")
	...
	if verification_method in ["SMS", "Email"]:
		expiry_time = finergy.flags.token_expiry or 300
		finergy.cache().set(tmp_id + "_token", token)
		finergy.cache().expire(tmp_id + "_token", expiry_time)
	else:
		expiry_time = finergy.flags.otp_expiry or 180
	for k, v in iteritems({"_usr": user, "_pwd": pwd, "_otp_secret": otp_secret}):
		finergy.cache().set("{0}{1}".format(tmp_id, k), v)
		finergy.cache().expire("{0}{1}".format(tmp_id, k), expiry_time)
...
def two_factor_is_enabled_for_(user):
	...
	roles = [finergy.db.escape(d.role) for d in user.roles or []]
	roles.append("'All'")
...
def get_verification_method():
	return finergy.db.get_value("System Settings", None, "two_factor_method")


def confirm_otp_token(login_manager, otp=None, tmp_id=None):
	"""Confirm otp matches."""
	from finergy.auth import get_login_attempt_tracker

	if not otp:
		otp = finergy.form_dict.get("otp")
	if not otp:
	...
	if not tmp_id:
		tmp_id = finergy.form_dict.get("tmp_id")
	hotp_token = finergy.cache().get(tmp_id + "_token")
	otp_secret = finergy.cache().get(tmp_id + "_otp_secret")
	if not otp_secret:
	...
	if hotp_token:
		if hotp.verify(otp, int(hotp_token)):
			finergy.cache().delete(tmp_id + "_token")
...
def get_verification_obj(user, token, otp_secret):
	otp_issuer = finergy.db.get_value("System Settings", "System Settings", "otp_issuer_name")
	verification_method = get_verification_method()
...
def process_2fa_for_sms(user, token, otp_secret):
	...
	phone = finergy.db.get_value("User", user, ["phone", "mobile_no"], as_dict=1)
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _("Login Verification Code from {}").format(
		finergy.db.get_value("System Settings", "System Settings", "otp_issuer_name")
	)
	subject = finergy.render_template(subject_template, kwargs_dict)
	return subject
...
def get_email_body_for_2fa(kwargs_dict):
	...
	"""
	body = finergy.render_template(body_template, kwargs_dict)
	return body
...
def get_link_for_qrcode(user, totp_uri):
	...
	lifespan = (
		int(finergy.db.get_value("System Settings", "System Settings", "lifespan_qrcode_image")) or 240
	)
	finergy.cache().set_value(key_uri, totp_uri, expires_in_sec=lifespan)
	finergy.cache().set_value(key_user, user, expires_in_sec=lifespan)
...
def send_token_via_sms(otpsecret, token=None, phone_no=None):
	...
	try:
		from finergy.core.doctype.sms_settings.sms_settings import send_request
	except Exception:
		return False
	...
	ss = finergy.get_doc("SMS Settings", "SMS Settings")
...
def send_token_via_email(user, token, otp_secret, otp_issuer, subject=None, message=None):
	...
	user_email = finergy.db.get_value("User", user, "email")
	if not user_email:
		return False
	hotp = pyotp.HOTP(otp_secret)
...
def qrcode_as_png(user, totp_uri):
	...
	png_file_name = "{}.png".format(finergy.generate_hash(length=20))
	...
	file_path = os.path.join(finergy.get_site_path("public", "files"), _file.file_name)
	url = qrcreate(totp_uri)
...
def create_barcode_folder():
	...
	folder = finergy.db.exists("File", {"file_name": folder_name})
	if folder:
		return folder
	folder = finergy.get_doc(
...
def delete_qrimage(user, check_expiry=False):
	...
	for barcode in user_barcodes:
		if check_expiry and not should_remove_barcode_image(barcode):
			continue
		barcode = finergy.get_doc("File", barcode.name)
		finergy.delete_doc("File", barcode.name, ignore_permissions=True)


def delete_all_barcodes_for_users():
	"""Task to delete all barcodes for user."""

	users = finergy.get_all("User", {"enabled": 1})
	for user in users:
...
def should_remove_barcode_image(barcode):
	...
	if isinstance(barcode, string_types):
		barcode = finergy.get_doc("File", barcode)
	lifespan = (
		finergy.db.get_value("System Settings", "System Settings", "lifespan_qrcode_image") or 240
	)
...
def disable():
	finergy.db.set_value("System Settings", None, "enable_two_factor_auth", 0)
...
def reset_otp_secret(user):
	otp_issuer = finergy.db.get_value("System Settings", "System Settings", "otp_issuer_name")
	user_email = finergy.db.get_value("User", user, "email")
	if finergy.session.user in ["Administrator", user]:
		clear_default(user + "_otplogin")
		clear_default(user + "_otpsecret")
		email_args = {
			"recipients": user_email,
			"sender": None,
			"subject": _("OTP Secret Reset - {0}").format(otp_issuer or "Finergy Framework"),
			"message": _(
			...
			).format(otp_issuer or "Finergy Framework"),
			"delayed": False,
			"retry": 3,
		}
		enqueue(
		...
		return finergy.throw(_("OTP secret can only be reset by the Administrator."))
