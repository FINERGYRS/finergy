...
import finergy
from finergy.defaults import _clear_cache
...
def _new_site(
	...
	from finergy.commands.scheduler import _is_scheduler_enabled
	from finergy.utils import get_site_path, scheduler, touch_file
	...
	apps_to_install = (
		["finergy"] + (finergy.conf.get("install_apps") or []) + (list(install_apps) or [])
	)
	...
	scheduler_status = "disabled" if finergy.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from finergy.database import setup_database

	if not db_type:
		db_type = finergy.conf.db_type or "mariadb"
	...
	finergy.flags.in_install_db = True

	finergy.flags.root_login = root_login
	finergy.flags.root_password = root_password
	setup_database(force, source_sql, verbose, no_mariadb_socket)

	finergy.conf.admin_password = finergy.conf.admin_password or admin_password

	remove_missing_apps()

	finergy.db.create_auth_table()
	finergy.db.create_global_search_table()
	finergy.db.create_user_settings_table()

	finergy.flags.in_install_db = False


def install_app(name, verbose=False, set_as_patched=True):
	from finergy.core.doctype.scheduled_job_type.scheduled_job_type import sync_jobs
	from finergy.model.sync import sync_for
	from finergy.modules.utils import sync_customizations
	from finergy.utils.fixtures import sync_fixtures

	finergy.flags.in_install = name
	finergy.flags.ignore_in_install = False

	finergy.clear_cache()
	app_hooks = finergy.get_hooks(app_name=name)
	installed_apps = finergy.get_installed_apps()
	...
	if name not in finergy.get_all_apps():
		raise Exception("App not in apps.txt")

	if name in installed_apps:
		finergy.msgprint(finergy._("App {0} already installed").format(name))
		return
	...
	for before_install in app_hooks.before_install or []:
		out = finergy.get_attr(before_install)()
		if out == False:
			return
	...
	for after_install in app_hooks.after_install or []:
		finergy.get_attr(after_install)()
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if not app_name in installed_apps:
		...
		finergy.db.set_global("installed_apps", json.dumps(installed_apps))
		finergy.db.commit()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = finergy.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in finergy.get_installed_apps():
	...
	if not (dry_run or no_backup):
		from finergy.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	finergy.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		finergy.get_attr(before_uninstall)()

	modules = finergy.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		finergy.get_attr(after_uninstall)()

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	finergy.flags.in_uninstall = False
...
def _delete_modules(modules: List[str], dry_run: bool) -> List[str]:
	...
	for module_name in modules:
		...
		for doctype in finergy.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					finergy.delete_doc("DocType", doctype.name, ignore_on_trash=True)
				else:
		...
		if not dry_run:
			finergy.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in finergy.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				finergy.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> Dict[str, str]:
	...
	linked_doctypes = finergy.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if finergy.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: List[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			finergy.delete_doc("DocType", doctype, ignore_on_trash=True)
			finergy.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	patch_path = os.path.join(finergy.get_pymodule_path(app), "patches.txt")
	if os.path.exists(patch_path):
		for patch in finergy.get_file_items(patch_path):
			finergy.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
		finergy.db.commit()


def init_singles():
	singles = [single["name"] for single in finergy.get_all("DocType", filters={"issingle": True})]
	for single in singles:
		if not finergy.db.get_singles_dict(single):
			doc = finergy.new_doc(single)
...
def make_conf(
	...
	sites_path = finergy.local.sites_path
	finergy.destroy()
	finergy.init(site, sites_path=sites_path)


def make_site_config(
	db_name=None, db_password=None, site_config=None, db_type=None, db_host=None, db_port=None
):
	finergy.create_folder(os.path.join(finergy.local.site_path))
...
def update_site_config(key, value, validate=True, site_config_path=None):
	...
	if hasattr(finergy.local, "conf"):
		finergy.local.conf[key] = value


def get_site_config_path():
	return os.path.join(finergy.local.site_path, "site_config.json")
...
def make_site_dirs():
	for dir_path in [
		...
		path = finergy.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app):
	modules = finergy.get_module_list(app)
	for module in modules:
		d = finergy.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("finergy_subscription", "shopping_cart")
	installed_apps = json.loads(finergy.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_sql_from_archive(sql_file_path):
	...
	"""
	from finergy.utils import get_bench_relative_path
...
def extract_files(site_name, file_path):
	...
	# Need to do finergy.init to maintain the site locals
	finergy.init(site=site_name)
	abs_site_path = os.path.abspath(finergy.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	...
	if finergy.conf.db_type not in (None, "mariadb"):
		return False
	...
	with open(sql_file_path) as f:
		for line in f:
			if head in line:
				# 'line' (str) format: ('2056588823','2020-05-11 18:21:31.488367','2020-06-12 11:49:31.079506','Administrator','Administrator',0,'Installed Applications','installed_applications','Installed Applications',1,'finergy','v10.1.71-74 (3c50d5e) (v10.x.x)','v10.x.x'),('855c640b8e','2020-05-11 18:21:31.488367','2020-06-12 11:49:31.079506','Administrator','Administrator',0,'Installed Applications','installed_applications','Installed Applications',2,'your_custom_app','0.0.1','master')
				line = line.strip().lstrip(head).rstrip(";").strip()
				app_rows = finergy.safe_eval(line)
				...
				# 'all_apps' (list) format: [('finergy', '12.x.x-develop ()', 'develop'), ('your_custom_app', '0.0.1', 'master')]
				...
				for app in all_apps:
					...
					if app_name == "finergy":
						try:
							current_version = Version(finergy.__version__)
						...
						if verbose and downgrade:
							print(f"Your site will be downgraded from Finergy {backup_version} to {current_version}")
...
def partial_restore(sql_file_path, verbose=False):
	...
	if finergy.conf.db_type in (None, "mariadb"):
		from finergy.database.mariadb.setup_db import import_db_from_sql
	elif finergy.conf.db_type == "postgres":
		import warnings

		from click import style

		from finergy.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path, _raise=True):
	...
	if _raise and (missing_table or empty_file):
		raise finergy.InvalidDatabaseFile
