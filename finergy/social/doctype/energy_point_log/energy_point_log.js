...
finergy.ui.form.on('Energy Point Log', {
		...
		const revert_dialog = new finergy.ui.Dialog({
			...
			primary_action: (values) => {
				return frm.call('revert', {
					...
					revert_dialog.hide();
					revert_dialog.clear();
					finergy.model.docinfo[frm.doc.reference_doctype][frm.doc.reference_name].energy_point_logs.unshift(revert_log);
					frm.refresh();
				});
			},
	...
	make_reference_name_link(frm) {
		...
		frm.fields_dict.reference_name.$input_wrapper.find('.control-value').wrapInner(`<a href='/app/${finergy.router.slug(dt)}/${dn}'></a>`);
	}
});
