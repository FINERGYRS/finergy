...
import finergy
from finergy.desk.form.assign_to import add as assign_to
...
class TestEnergyPointLog(unittest.TestCase):
	def setUp(self):
		finergy.cache().delete_value("energy_point_rule_map")

	def tearDown(self):
		finergy.set_user("Administrator")
		finergy.db.sql("DELETE FROM `tabEnergy Point Log`")
		finergy.db.sql("DELETE FROM `tabEnergy Point Rule`")
		finergy.cache().delete_value("energy_point_rule_map")
	...
	def test_disabled_energy_points(self):
		settings = finergy.get_single("Energy Point Settings")
		settings.enabled = 0
		settings.save()
	...
	def test_revert_points_on_cancelled_doc(self):
		...
		energy_point_logs = finergy.get_all("Energy Point Log")
	...
	def test_allow_creation_of_new_log_if_the_previous_log_was_reverted(self):
		...
		log_name = finergy.db.exists("Energy Point Log", {"reference_name": created_todo.name})
		finergy.get_doc("Energy Point Log", log_name).revert("Just for test")
...
def create_energy_point_rule_for_todo(
	...
	name = "ToDo Closed"
	point_rule_exists = finergy.db.exists("Energy Point Rule", name)

	if point_rule_exists:
		return finergy.get_doc("Energy Point Rule", name)
