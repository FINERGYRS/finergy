...
from finergy.core.doctype.user.user import get_enabled_users
from finergy.model import log_types
from finergy.model.document import Document
from finergy.social.doctype.energy_point_log.energy_point_log import create_energy_points_log
...
class EnergyPointRule(Document):
	def on_update(self):
		finergy.cache_manager.clear_doctype_map("Energy Point Rule", self.reference_doctype)
	...
	def apply(self, doc):
		if self.rule_condition_satisfied(doc):
			...
			try:
				...
				finergy.log_error(finergy.get_traceback(), "apply_energy_point")
	...
	def eval_condition(self, doc):
		return self.condition and finergy.safe_eval(self.condition, None, {"doc": doc.as_dict()})
...
def get_energy_point_doctypes():
	return [
		d.reference_doctype
		for d in finergy.get_all("Energy Point Rule", ["reference_doctype"], {"enabled": 1})
	]
