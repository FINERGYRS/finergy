...
from finergy.modules import get_module_name, load_doctype_module
...
def main(
	...
	if doctype_list_path:
		...
		with open(finergy.get_app_path(app, doctype_list_path), "r") as f:
	...
	try:
		finergy.flags.print_messages = verbose
		finergy.flags.in_test = True

		if not finergy.db:
			finergy.connect()

		# if not finergy.conf.get("db_name").startswith("test_"):
		...
		if not finergy.flags.skip_before_tests:
			if verbose:
				print('Running "before_tests" hooks')
			for fn in finergy.get_hooks("before_tests", app_name=app):
				finergy.get_attr(fn)()
...
def run_all_tests(
	...
	apps = [app] if app else finergy.get_installed_apps()

	test_suite = unittest.TestSuite()
	for app in apps:
		for path, folders, files in os.walk(finergy.get_pymodule_path(app)):
...
def run_tests_for_doctype(
	...
	for doctype in doctypes:
		module = finergy.db.get_value("DocType", doctype, "module")
		if not module:
		...
		if force:
			for name in finergy.db.sql_list("select name from `tab%s`" % doctype):
				finergy.delete_doc(doctype, name, force=True)
...
def _run_unittest(
	...
	finergy.flags.tests_verbose = verbose
...
def _add_test(app, path, filename, verbose, test_suite=None, ui_tests=False):
	...
	app_path = finergy.get_pymodule_path(app)
...
def make_test_records(doctype, verbose=0, force=False):
	...
	if finergy.flags.skip_test_records:
		return
	...
	for options in get_dependencies(doctype):
		...
		if not options in finergy.local.test_objects:
			finergy.local.test_objects[options] = []
...
def get_dependencies(doctype):
	...
	for df in meta.get_table_fields():
		link_fields.extend(finergy.get_meta(df.options).get_link_fields())
...
def make_test_records_for_doctype(doctype, verbose=0, force=False):
	...
	if hasattr(test_module, "_make_test_records"):
		finergy.local.test_objects[doctype] += test_module._make_test_records(verbose)

	elif hasattr(test_module, "test_records"):
		if doctype in finergy.local.test_objects:
			finergy.local.test_objects[doctype] += make_test_objects(
		...
		if test_records:
			finergy.local.test_objects[doctype] += make_test_objects(doctype, test_records, verbose, force)
...
def make_test_objects(doctype, test_records=None, verbose=None, reset=False):
	...
	for doc in test_records:
		...
		if finergy.db.exists(d.doctype, d.name) and not reset:
			finergy.db.rollback()
...
def add_to_test_record_log(doctype):
	...
	if not doctype in test_record_log:
		finergy.flags.test_record_log.append(doctype)
		with open(finergy.get_site_path(".test_log"), "w") as f:
			f.write("\n".join(filter(None, finergy.flags.test_record_log)))


def get_test_record_log():
	"""Return the list of doctypes for which test records have been created"""
	if "test_record_log" not in finergy.flags:
		if os.path.exists(finergy.get_site_path(".test_log")):
			with open(finergy.get_site_path(".test_log"), "r") as f:
				finergy.flags.test_record_log = f.read().splitlines()
		else:
			finergy.flags.test_record_log = []

	return finergy.flags.test_record_log
