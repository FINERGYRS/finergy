...
"""
Finergy - Low Code Open Source Framework in Python and JS

Finergy, pronounced fra-pay, is a full stack, batteries-included, web
framework written in Python and Javascript with MariaDB as the database.
It is the framework which powers CapKPI. It is pretty generic and can
be used to build database driven apps.

Read the documentation: https://finergy-rs.fr/docs
"""
import importlib
import inspect
import json
...
from finergy.query_builder import get_query_builder, patch_query_execute
from finergy.utils.data import sbool
...
def _(msg, lang=None, context=None) -> str:
	...
	"""
	from finergy.translate import get_full_dict
	from finergy.utils import is_html, strip_html_tags
...
def get_lang_dict(fortype, name=None):
	...
	from finergy.translate import get_dict

	return get_dict(fortype, name)


def set_user_lang(user, user_language=None):
	"""Guess and set user language for the session. `finergy.local.lang`"""
	from finergy.translate import get_user_lang
...
if typing.TYPE_CHECKING:
	from finergy.database.mariadb.database import MariaDBDatabase
	from finergy.database.postgres.database import PostgresDatabase
	from finergy.query_builder.builder import MariaDB, Postgres
	from finergy.utils.redis_wrapper import RedisWrapper
...
def init(site, sites_path=None, new_site=False):
	"""Initialize finergy for the current site. Reset thread locals `finergy.local`"""
...
def connect(site=None, db_name=None, set_admin_as_user=True):
	...
	:param site: If site is given, calls `finergy.init`.
	:param db_name: Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from finergy.database import get_db
...
def get_traceback():
	...
	from finergy.utils import get_traceback
...
def throw(msg, exc=ValidationError, title=None, is_minimizable=None, wide=None, as_list=False):
	...
	:param msg: Message.
	:param exc: Exception class. Default `finergy.ValidationError`"""
	msgprint(
		msg,
		raise_exception=exc,
		title=title,
...
def create_folder(path, with_init=False):
	...
	from finergy.utils import touch_file
...
def get_roles(username=None):
	...
	return finergy.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from finergy.utils import md_to_html
	...
	from finergy.email import queue
...
def is_whitelisted(method):
	from finergy.utils import sanitize_html
...
def only_for(roles, message=False):
	"""Raise `finergy.PermissionError` if the user does not have any of the given **Roles**.
...
def clear_cache(user=None, doctype=None):
	...
	if doctype:
		finergy.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		finergy.cache_manager.clear_user_cache(user)
	else:  # everything
		from finergy import translate

		finergy.cache_manager.clear_user_cache()
		finergy.cache_manager.clear_domain_cache()
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	permissions = finergy.permissions.get_role_permissions(doctype, user=user)
...
def has_permission(doctype=None, ptype="read", doc=None, user=None, verbose=False, throw=False):
	"""Raises `finergy.PermissionError` if not permitted.
	...
	out = finergy.permissions.has_permission(
		doctype, ptype, doc=doc, verbose=verbose, user=user, raise_exception=throw
	)
	if throw and not out:
		if doc:
			finergy.throw(_("No permission for {0}").format(_(doc.doctype) + " " + doc.name))
		else:
			finergy.throw(_("No permission for {0}").format(_(doctype)))
...
def new_doc(doctype, parent_doc=None, parentfield=None, as_dict=False):
	...
	from finergy.model.create_new import get_new_doc

	return get_new_doc(doctype, parent_doc, parentfield, as_dict=as_dict)


def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `finergy.client.set_value`"""
	import finergy.client

	return finergy.client.set_value(doctype, docname, fieldname, value)
...
def get_doc(*args, **kwargs):
	"""Return a `finergy.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = finergy.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = finergy.get_doc("ToDo", "TD0001")

	"""
	import finergy.model.document

	doc = finergy.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `finergy.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)


def get_meta(doctype, cached=True):
	"""Get `finergy.model.meta.Meta` instance of given doctype name."""
	import finergy.model.meta

	return finergy.model.meta.get_meta(doctype, cached=cached)


def get_meta_module(doctype):
	import finergy.modules

	return finergy.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `finergy.model.delete_doc.delete_doc`.
	...
	import finergy.model.delete_doc

	finergy.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(module, dt=None, dn=None, force=False, reset_permissions=False):
	...
	return finergy.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(*args, **kwargs):
	...
	Calls `finergy.model.rename_doc.rename_doc`
	"""
...
def get_all_apps(with_internal_apps=True, sites_path=None):
	...
	if "finergy" in apps:
		apps.remove("finergy")
	apps.insert(0, "finergy")

	return apps


def get_installed_apps(sort=False, finergy_last=False):
	...
	if finergy_last:
		if "finergy" in installed:
			installed.remove("finergy")
		installed.append("finergy")
...
def get_hooks(hook=None, default=None, app_name=None):
	...
	def load_app_hooks(app_name=None):
		...
		for app in [app_name] if app_name else get_installed_apps(sort=True):
			app = "finergy" if app == "webnotes" else app
			try:
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = finergy.utils.strip(content)
...
def compare(val1, condition, val2):
	"""Compare two values using `finergy.utils.compare`
	...
	return finergy.utils.compare(val1, condition, val2)
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        finergy.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import finergy.desk.reportview

	return finergy.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `finergy.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        finergy.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        finergy.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])

	        # filter as a list of dicts
	        finergy.get_list("ToDo", fields="*", filters = {"description": ("like", "test%")})
	"""
	import finergy.model.db_query

	return finergy.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `finergy.model.db_query`. Will **not** check for permissions.
	Parameters are same as `finergy.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        finergy.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        finergy.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])

	        # filter as a list of dicts
	        finergy.get_all("ToDo", fields=["*"], filters = {"description": ("like", "test%")})
	"""
...
def get_value(*args, **kwargs):
	...
	Alias for `finergy.db.get_value`
...
def get_test_records(doctype):
	...
	from finergy.modules import get_doctype_module, get_module_path
...
def format_value(*args, **kwargs):
	...
	import finergy.utils.formatters

	return finergy.utils.formatters.format_value(*args, **kwargs)
...
def attach_print(
	...
	from finergy.utils import scrub_urls
...
def publish_progress(*args, **kwargs):
	...
	"""
	import finergy.realtime

	return finergy.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return finergy.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: finergy.local.cache[namespace]
	:param key: finergy.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import finergy.utils.background_jobs

	return finergy.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return finergy.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(
	...
	from finergy.utils.logger import get_logger
...
def get_active_domains():
	from finergy.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	Example
	>>> finergy.get_version('User', 'foobar@gmail.com')
	>>>
	[
	        {
	...
	if meta.track_changes:
		...
		from finergy.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from finergy.utils import parse_json
...
def mock(type, size=1, locale="en"):
	...
	from finergy.utils import squashify

	return squashify(results)


def validate_and_sanitize_search_inputs(fn):
	from finergy.desk.search import validate_and_sanitize_search_inputs as func
