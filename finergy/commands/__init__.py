...
def pass_context(f):
	...
	def _func(ctx, *args, **kwargs):
		...
		try:
			ret = f(finergy._dict(ctx.obj), *args, **kwargs)
		except finergy.exceptions.SiteNotSpecifiedError as e:
			click.secho(str(e), fg="yellow")
			sys.exit(1)
		except finergy.exceptions.IncorrectSitePath:
...
def get_site(context, raise_err=True):
	try:
		...
		if raise_err:
			raise finergy.SiteNotSpecifiedError
		return None
