...
def new_site(
	...
	"Create a new site"
	from finergy.installer import _new_site

	finergy.init(site=site, new_site=True)
...
def restore(
	...
	if is_partial(decompressed_file_name):
		click.secho(
			"Partial Backup file detected. You cannot use a partial file to restore a Finergy Site.",
			fg="red",
		)
		click.secho(
	...
	# dont allow downgrading to older versions of finergy without force
...
def partial_restore(context, sql_file_path, verbose):
	from finergy.installer import partial_restore

	verbose = context.verbose or verbose

	site = get_site(context)
	finergy.init(site=site)
	finergy.connect(site=site)
...
def _reinstall(
	...
	try:
		...
		finergy.connect()
		finergy.clear_cache()
		installed = finergy.get_installed_apps()
		finergy.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context, apps):
	...
	from finergy.installer import install_app as _install_app
...
def list_apps(context, format):
	...
	for site in context.sites:
		...
		apps = finergy.get_single("Installed Applications").installed_applications
		...
		if apps:
			...
			installed_applications = finergy.get_installed_apps()
			applications_summary = "\n".join(installed_applications)
			summary = f"{site_title}\n{applications_summary}\n"
	...
	if format == "json":
		click.echo(finergy.as_json(summary_dict))
...
def add_system_manager(context, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		finergy.connect(site=site)
		try:
			finergy.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			finergy.db.commit()
		finally:
			finergy.destroy()
	if not context.sites:
...
def disable_user(context, email):
	...
	with finergy.init_site(site):
		finergy.connect()
		user = finergy.get_doc("User", email)
		user.enabled = 0
...
def migrate(context, skip_failing=False, skip_search_index=False):
	...
	from finergy.migrate import migrate
...
@click.argument("finergy_provider")
@pass_context
def migrate_to(context, finergy_provider):
	"Migrates site to the specified provider"
	from finergy.integrations.finergy_providers import migrate_to

	for site in context.sites:
		finergy.init(site=site)
		finergy.connect()
		migrate_to(site, finergy_provider)
		finergy.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def run_patch(context, module, force):
	...
	for site in context.sites:
		...
		try:
			finergy.connect()
			finergy.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			finergy.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context, module, doctype, docname):
	...
	for site in context.sites:
		try:
			finergy.init(site=site)
			finergy.connect()
			finergy.reload_doc(module, doctype, docname, force=context.force)
			finergy.db.commit()
		finally:
			finergy.destroy()
	if not context.sites:
...
def reload_doctype(context, doctype):
	...
	for site in context.sites:
		try:
			finergy.init(site=site)
			finergy.connect()
			finergy.reload_doctype(doctype, force=context.force)
			finergy.db.commit()
		finally:
			finergy.destroy()
	if not context.sites:
...
def add_to_hosts(context):
	"Add site to hosts"
	for site in context.sites:
		finergy.commands.popen("echo 127.0.0.1\t{0} | sudo tee -a /etc/hosts".format(site))
	if not context.sites:
		raise SiteNotSpecifiedError
...
def uninstall(context, app, dry_run, yes, no_backup, force):
	...
	from finergy.installer import remove_app
...
def _drop_site(
	...
	from finergy.database import drop_user_and_database
	from finergy.utils.backups import scheduled_backup
	...
	drop_user_and_database(finergy.conf.db_name, db_root_username, db_root_password)

	if not archived_sites_path:
		archived_sites_path = os.path.join(
			finergy.get_app_path("finergy"), "..", "..", "..", "archived_sites"
		)
...
def move(dest_dir, site):
	...
	finergy.init(site)
	old_path = finergy.utils.get_site_path()
...
def set_last_active_for_user(context, user=None):
	...
	from finergy.utils.user import set_last_active_to_now
...
def publish_realtime(context, event, message, room, user, doctype, docname, after_commit):
	...
	from finergy import publish_realtime
...
def browse(context, site):
	...
	if site in finergy.utils.get_sites():
		webbrowser.open(finergy.utils.get_site_url(site), new=2)
	else:
