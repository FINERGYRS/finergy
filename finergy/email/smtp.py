...
from finergy.utils import cint, cstr, parse_addr
...
def send(email, append_to=None, retry=1):
	...
	def _send(retry):
		try:
			...
			finergy.throw(_("Invalid login or password"), title="Email Failed")
			raise
		except smtplib.SMTPRecipientsRefused:
			finergy.msgprint(_("Invalid recipient address"), title="Email Failed")
			raise
...
def get_outgoing_email_account(raise_exception_not_set=True, append_to=None, sender=None):
	...
	if not getattr(finergy.local, "outgoing_email_account", None):
		finergy.local.outgoing_email_account = {}

	if not (
		finergy.local.outgoing_email_account.get(append_to)
		or finergy.local.outgoing_email_account.get(sender_email_id)
		or finergy.local.outgoing_email_account.get("default")
	):
		email_account = None
		...
		if not email_account and append_to:
			...
			email_accounts = finergy.db.get_values(
				"Email Account",
				{
					"enable_outgoing": 1,
					"enable_incoming": 1,
		...
		finergy.local.outgoing_email_account[append_to or sender_email_id or "default"] = email_account
...
def get_default_outgoing_email_account(raise_exception_not_set=True):
	...
	if not email_account and finergy.conf.get("mail_server"):
		# from site_config.json
		email_account = finergy.new_doc("Email Account")
		email_account.update(
			{
				"smtp_server": finergy.conf.get("mail_server"),
				"smtp_port": finergy.conf.get("mail_port"),
				# legacy: use_ssl was used in site_config instead of use_tls, but meant the same thing
				"use_tls": cint(finergy.conf.get("use_tls") or 0) or cint(finergy.conf.get("use_ssl") or 0),
				"login_id": finergy.conf.get("mail_login"),
				"email_id": finergy.conf.get("auto_email_id")
				or finergy.conf.get("mail_login")
				or "notifications@example.com",
				"password": finergy.conf.get("mail_password"),
				"always_use_account_email_id_as_sender": finergy.conf.get(
					"always_use_account_email_id_as_sender", 0
				),
				"always_use_account_name_as_sender_name": finergy.conf.get(
					"always_use_account_name_as_sender_name", 0
				),
			}
		)
		email_account.from_site_config = True
		email_account.name = finergy.conf.get("email_sender_name") or "Finergy"

	if not email_account and not raise_exception_not_set:
		return None

	if finergy.are_emails_muted():
		# create a stub
		email_account = finergy.new_doc("Email Account")
...
def _get_email_account(filters):
	name = finergy.db.get_value("Email Account", filters)
	return finergy.get_doc("Email Account", name) if name else None
...
class SMTPServer:
		...
		if self.email_account:
			...
			if not self.email_account.no_smtp_authentication:
				if self.email_account.ascii_encode_password:
					self.password = finergy.safe_encode(self.email_account.password, "ascii")
				else:
	...
	def sess(self):
		...
		if not getattr(self, "server"):
			...
			finergy.msgprint(err_msg)
			raise finergy.OutgoingEmailError(err_msg)
		...
		try:
			...
			if self.login and self.password:
				...
				if ret[0] != 235:
					finergy.msgprint(ret[1])
					raise finergy.OutgoingEmailError(ret[1])

			return self._sess

		except smtplib.SMTPAuthenticationError as e:
			from finergy.email.doctype.email_account.email_account import EmailAccount
			...
			finergy.throw(
				...
				exc=finergy.ValidationError,
				title=_("Incorrect Configuration"),
			)

		except smtplib.SMTPException:
			finergy.msgprint(_("Unable to send emails at this time"))
			raise
