...
class TestSMTP(unittest.TestCase):
	...
	def test_get_email_account(self):
		existing_email_accounts = finergy.get_all(
		...
		for email_account in existing_email_accounts:
			finergy.db.set_value("Email Account", email_account["name"], unset_details)

		# remove mail_server config so that test@example.com is not created
		mail_server = finergy.conf.get("mail_server")
		del finergy.conf["mail_server"]
		...
		finergy.conf["mail_server"] = mail_server
		for email_account in existing_email_accounts:
			set_details = {
			...
			finergy.db.set_value("Email Account", email_account["name"], set_details)
