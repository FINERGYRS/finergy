...
import finergy
from finergy import _, enqueue, msgprint, safe_decode, safe_encode
from finergy.email.email_body import add_attachment, get_email, get_formatted_html
from finergy.email.smtp import SMTPServer, get_outgoing_email_account
...
class EmailLimitCrossedError(finergy.ValidationError):
	pass
...
def send(
	...
	:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/finergy.email.queue.unsubscribe`.
	...
	if not unsubscribe_method:
		unsubscribe_method = "/api/method/finergy.email.queue.unsubscribe"
	...
	unsubscribed = finergy.db.sql_list(
		"""
		SELECT
			distinct email
		from
...
def get_email_queue(recipients, sender, subject, **kwargs):
	...
	except finergy.InvalidEmailAddressError:
...
def get_unsubscribe_message(unsubscribe_message: str, expose_recipients: str):
	...
	return finergy._dict(html=html, text=text)
...
def get_unsubcribed_url(
	...
	# for test
	finergy.local.flags.signed_query_string = query_string
...
def flush(from_test=False):
	...
	smtpserver_dict = finergy._dict()

	for email in get_queue():

		if cint(finergy.db.get_default("suspend_email_queue")) == 1:
			break
		...
		if email.name:
			...
			if from_test:
				...
				enqueue(method="finergy.email.queue.send_one", queue="short", **send_one_args)
...
def send_one(email, smtpserver=None, auto_commit=True, now=False):
	...
	recipients_list = finergy.db.sql(
	...
	if finergy.are_emails_muted():
		finergy.msgprint(_("Emails are muted"))
		return
	...
	if email.communication:
		finergy.get_doc("Communication", email.communication).set_delivery_status(commit=auto_commit)
	...
	try:
		...
		if not finergy.flags.in_test:
			...
			if getattr(finergy.local, "outgoing_email_account", None):
		...
		for recipient in recipients_list:
			...
			if not finergy.flags.in_test:
				...
				if method:
					queue = finergy.get_doc("Email Queue", email.name)
		...
		if finergy.flags.in_test:
			finergy.flags.sent_mail = message
			return
		if email.communication:
			finergy.get_doc("Communication", email.communication).set_delivery_status(commit=auto_commit)
		...
		if now:
			...
			# log to Error Log
			finergy.log_error("finergy.email.queue.flush")
...
def prepare_message(email, recipient, recipients_list):
	...
	if finergy.conf.use_ssl and email_account.track_email_status:
		# Using SSL => Publically available domain => Email Read Reciept Possible
		message = message.replace(
			"<!--email_open_check-->",
			quopri.encodestring(
				'<img src="https://{}/api/method/finergy.core.doctype.communication.email.mark_email_as_seen?name={}"/>'.format(
					finergy.local.site, email.communication
				).encode()
			).decode(),
		)
	else:
	...
	if email.attachments:
		...
		for attachment in attachments:
			...
			if fid:
				_file = finergy.get_doc("File", fid)
				...
				print_format_file = finergy.attach_print(**attachment)
...
def clear_outbox(days=None):
	...
	email_queues = finergy.db.sql_list(
...
def get_email_retry_limit():
	return cint(finergy.db.get_system_setting("email_retry_limit")) or 3
