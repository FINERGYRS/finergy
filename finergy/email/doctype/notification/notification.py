...
from finergy.core.doctype.role.role import get_info_based_on_role, get_user_info
from finergy.core.doctype.sms_settings.sms_settings import send_sms
from finergy.desk.doctype.notification_log.notification_log import enqueue_create_notification
from finergy.integrations.doctype.slack_webhook_url.slack_webhook_url import send_slack_message
from finergy.model.document import Document
from finergy.modules.utils import export_module_json, get_doc_module
from finergy.utils import add_to_date, cast, is_html, nowdate, validate_email_address
...
class Notification(Document):
	...
	def validate(self):
		...
		if self.event in ("Days Before", "Days After") and not self.date_changed:
			finergy.throw(_("Please specify which date field must be checked"))

		if self.event == "Value Change" and not self.value_changed:
			finergy.throw(_("Please specify which value field must be checked"))

		self.validate_forbidden_types()
		self.validate_condition()
		self.validate_standard()
		finergy.cache().hdel("notifications", self.document_type)
...
"""
	...
	def validate_standard(self):
		if self.is_standard and self.enabled and not finergy.conf.developer_mode:
			finergy.throw(
				_("Cannot edit Standard Notification. To edit, please disable this and duplicate it")
			)

	def validate_condition(self):
		temp_doc = finergy.new_doc(self.document_type)
		if self.condition:
			try:
				finergy.safe_eval(self.condition, None, get_context(temp_doc.as_dict()))
			except Exception:
				finergy.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def validate_forbidden_types(self):
		forbidden_document_types = ("Email Queue",)
		if self.document_type in forbidden_document_types or finergy.get_meta(self.document_type).istable:
			# currently notifications don't work on child tables as events are not fired for each record of child table

			finergy.throw(_("Cannot set Notification on Document Type {0}").format(self.document_type))
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = finergy.get_doc(self.document_type, d.name)

			if self.condition and not finergy.safe_eval(self.condition, None, get_context(doc)):
				continue
	...
	def send(self, doc):
		...
		try:
			...
			finergy.log_error(title="Failed to send notification", message=finergy.get_traceback())
		...
		if self.set_property_after_alert:
			...
			try:
				if allow_update and not doc.flags.in_notification_update:
					...
					if doc.meta.get_field(fieldname).fieldtype in finergy.model.numeric_fieldtypes:
						value = finergy.utils.cint(value)
				...
				finergy.log_error(title="Document update failed", message=finergy.get_traceback())

	def create_system_notification(self, doc, context):
		subject = self.subject
		if "{" in subject:
			subject = finergy.render_template(self.subject, context)
		...
		notification_doc = {
			...
			"email_content": finergy.render_template(self.message, context),
	...
	def send_an_email(self, doc, context):
		...
		from finergy.core.doctype.communication.email import _make as make_communication
		...
		sender = None
		message = finergy.render_template(self.message, context)
	...
	def send_sms(self, doc, context):
		send_sms(
			...
			msg=finergy.render_template(self.message, context),
		)
	...
	def get_list_of_recipients(self, doc, context):
		...
		for recipient in self.recipients:
			if recipient.condition:
				if not finergy.safe_eval(recipient.condition, None, context):
					continue
			...
			if recipient.cc and "{" in recipient.cc:
				recipient.cc = finergy.render_template(recipient.cc, context)
			...
			if recipient.bcc and "{" in recipient.bcc:
				recipient.bcc = finergy.render_template(recipient.bcc, context)
	...
	def get_attachment(self, doc):
		...
		print_settings = finergy.get_doc("Print Settings", "Print Settings")
			...
			return [
				{
					...
					"lang": finergy.db.get_value("Print Format", self.print_format, "default_print_language")
	...
	def get_template(self):
		...
		def load_template(extn):
			template = ""
			template_path = os.path.join(os.path.dirname(module.__file__), finergy.scrub(self.name) + extn)
	...
	def load_standard_properties(self, context):
		...
		if not is_html(self.message):
			self.message = finergy.utils.md_to_html(self.message)


@finergy.whitelist()
def get_documents_for_today(notification):
	notification = finergy.get_doc("Notification", notification)
...
def trigger_notifications(doc, method=None):
	if finergy.flags.in_import or finergy.flags.in_patch:
	...
	if method == "daily":
		...
		for d in doc_list:
			alert = finergy.get_doc("Notification", d.name)
...
def evaluate_alert(doc: Document, alert, event):
	...
	try:
		if isinstance(alert, string_types):
			alert = finergy.get_doc("Notification", alert)

		context = get_context(doc)

		if alert.condition:
			if not finergy.safe_eval(alert.condition, None, context):
				return

		if event == "Value Change" and not doc.is_new():
			if not finergy.db.has_column(doc.doctype, alert.value_changed):
				alert.db_set("enabled", 0)
				finergy.log_error("Notification {0} has been disabled due to missing field".format(alert.name))
				return
		...
		error_log = finergy.log_error(message=finergy.get_traceback(), title=str(e))
		finergy.throw(
			_("Error in Notification: {}").format(
				finergy.utils.get_link_to_form("Error Log", error_log.name)
			)
		)
...
def get_context(doc):
	return {
		"doc": doc,
		"nowdate": nowdate,
		"finergy": finergy._dict(utils=get_safe_globals().get("finergy").get("utils")),
	}


def get_assignees(doc):
	assignees = []
	assignees = finergy.get_all(
		"ToDo",
