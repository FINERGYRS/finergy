...
finergy.notification = {
	setup_fieldname_select: function(frm) {
		...
		finergy.model.with_doctype(frm.doc.document_type, function() {
			...
			let fields = finergy.get_doc('DocType', frm.doc.document_type).fields;
			let options = $.map(fields, function(d) {
				return in_list(finergy.model.no_value_type, d.fieldtype)
			...
			if (frm.doc.channel === 'Email') {
				receiver_fields = $.map(fields, function(d) {
					...
					if (d.fieldtype == 'Table') {
						let child_fields = finergy.get_doc('DocType', d.options).fields;
...
finergy.ui.form.on('Notification', {
	onload: function(frm) {
	...
	refresh: function(frm) {
		finergy.notification.setup_fieldname_select(frm);
		finergy.notification.setup_example_message(frm);
	...
	view_properties: function(frm) {
		finergy.route_options = { doc_type: frm.doc.document_type };
	...
	event: function(frm) {
		if (in_list(['Days Before', 'Days After'], frm.doc.event)) {
			frm.add_custom_button(__('Get Alerts for Today'), function() {
				finergy.call({
					method:
						'finergy.email.doctype.notification.notification.get_documents_for_today',
					args: {
					...
					callback: function(r) {
						if (r.message) {
							...
							finergy.msgprint(__('No alerts for today'));
						}
					}
				});
			});
