...
from finergy.utils import cint, cstr, validate_email_address
...
class EmailDomain(Document):
	...
	def validate(self):
		...
		if not finergy.local.flags.in_install and not finergy.local.flags.in_patch:
			try:
				if self.use_imap:
					logger.info(
	...
	def on_update(self):
		...
		for email_account in finergy.get_all("Email Account", filters={"domain": self.name}):
			try:
