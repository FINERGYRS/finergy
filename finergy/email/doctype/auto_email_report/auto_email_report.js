...
finergy.ui.form.on('Auto Email Report', {
	refresh: function(frm) {
		...
		if(!frm.is_new()) {
			frm.add_custom_button(__('Download'), function() {
				var w = window.open(
					finergy.urllib.get_full_url(
						"/api/method/finergy.email.doctype.auto_email_report.auto_email_report.download?"
			...
			frm.add_custom_button(__('Send Now'), function() {
				finergy.call({
					method: 'finergy.email.doctype.auto_email_report.auto_email_report.send_now',
					args: {name: frm.doc.name},
					callback: function() {
						finergy.msgprint(__('Scheduled to send'));
					}
				});
			});
		} else {
			if(!frm.doc.user) {
				frm.set_value('user', finergy.session.user);
			}
			if(!frm.doc.email_to) {
				frm.set_value('email_to', finergy.session.user);
			}
		}
	},
	report: function(frm) {
	...
	fetch_report_filters(frm) {
		if (frm.doc.report
			...
			finergy.call({
				method: "finergy.desk.query_report.get_script",
				args: {
					report_name: frm.doc.report
				},
				callback: function(r) {
					finergy.dom.eval(r.message.script || "");
					frm.script_setup_for = frm.doc.report;
	...
	show_filters: function(frm) {
		...
		if(frm.doc.report_type === 'Custom Report' || (frm.doc.report_type !== 'Report Builder'
			&& finergy.query_reports[frm.doc.report]
			&& finergy.query_reports[frm.doc.report].filters)) {
			...
			if (frm.doc.report_type === 'Custom Report'
				&& finergy.query_reports[frm.doc.reference_report]
				&& finergy.query_reports[frm.doc.reference_report].filters) {
				report_filters = finergy.query_reports[frm.doc.reference_report].filters;
			} else {
				report_filters = finergy.query_reports[frm.doc.report].filters;
			}
			...
			report_filters.forEach(f => {
				...
				$("<td>" + finergy.format(filters[f.fieldname], f) +"</td>")
					.css(css)
					.appendTo(row);
			});
