...
from finergy.desk.query_report import build_xlsx_data
...
from finergy.utils.csvutils import to_csv
...
class AutoEmailReport(Document):
	def autoname(self):
		...
		if finergy.db.exists("Auto Email Report", self.name):
	...
	def validate_report_count(self):
		count = finergy.db.count("Auto Email Report", {"user": self.user, "enabled": 1})

		max_reports_per_user = (
			cint(finergy.local.conf.max_reports_per_user)  # kept for backward compatibilty
			or cint(finergy.db.get_single_value("System Settings", "max_auto_email_report_per_user"))
			or 20
		)
		...
		if count > max_reports_per_user + (-1 if self.flags.in_insert else 0):
			...
			finergy.throw(msg, title=_("Report limit reached"))
		...
		if self.format not in valid_report_formats:
			finergy.throw(
				_("{0} is not a valid report format. Report format should one of the following {1}").format(
					finergy.bold(self.format), finergy.bold(", ".join(valid_report_formats))
				)
			)

	def validate_mandatory_fields(self):
		# Check if all Mandatory Report Filters are filled by the User
		filters = finergy.parse_json(self.filters) if self.filters else {}
		filter_meta = finergy.parse_json(self.filter_meta) if self.filter_meta else {}
		throw_list = []
	...
	def get_report_content(self):
		...
		report = finergy.get_doc("Report", self.report)

		self.filters = finergy.parse_json(self.filters) if self.filters else {}
		...
		# add serial numbers
		columns.insert(0, finergy._dict(fieldname="idx", label="", width="30px"))
		...
		if self.format == "HTML":
			...
			finergy.throw(_("Invalid Output Format"))

	def get_html_table(self, columns=None, data=None):

		date_time = global_date_format(now()) + " " + format_time(now())
		report_doctype = finergy.db.get_value("Report", self.report, "ref_doctype")

		return finergy.render_template(
			"finergy/templates/emails/auto_email_report.html",
			{
				"title": self.name,
	...
	def prepare_dynamic_filters(self):
		self.filters = finergy.parse_json(self.filters)
	...
	def send(self):
		if self.filter_meta and not self.filters:
			finergy.throw(_("Please set filters value in Report Filter table."))
...
def download(name):
	...
	auto_email_report = finergy.get_doc("Auto Email Report", name)
	auto_email_report.check_permission()
	data = auto_email_report.get_report_content()

	if not data:
		finergy.msgprint(_("No Data"))
		return

	finergy.local.response.filecontent = data
	finergy.local.response.type = "download"
	finergy.local.response.filename = auto_email_report.get_file_name()
...
def send_daily():
	...
	enabled_reports = finergy.get_all(
		"Auto Email Report", filters={"enabled": 1, "frequency": ("in", ("Daily", "Weekdays", "Weekly"))}
	)

	for report in enabled_reports:
		auto_email_report = finergy.get_doc("Auto Email Report", report.name)
		...
		try:
			...
			finergy.log_error(e, _("Failed to send {0} Auto Email Report").format(auto_email_report.name))


def send_monthly():
	"""Check reports to be sent monthly"""
	for report in finergy.get_all("Auto Email Report", {"enabled": 1, "frequency": "Monthly"}):
		finergy.get_doc("Auto Email Report", report.name).send()
	...
	for row in data:
		...
		for col in columns:
			...
			if col.fieldtype == "Link":
				...
				doc = finergy.get_doc(col.parent, doc_name) if doc_name and col.parent else None
				# Pass the Document to get the currency based on docfield option
				row[col.fieldname] = finergy.format_value(row[col.fieldname], col, doc=doc)
	return columns, data
