...
from finergy.email.doctype.email_account.email_account import notify_unreplied


class TestEmailAccount(unittest.TestCase):
	def setUp(self):
		finergy.flags.mute_emails = False
		finergy.flags.sent_mail = None

		email_account = finergy.get_doc("Email Account", "_Test Email Account 1")
	...
	def test_incoming(self):
		...
		comm = finergy.get_doc("Communication", {"sender": "test_sender@example.com"})
		self.assertTrue("test_receiver@example.com" in comm.recipients)

		# check if todo is created
		self.assertTrue(finergy.db.get_value(comm.reference_doctype, comm.reference_name, "name"))
	...
	def test_incoming_with_attach(self):
		...
		existing_file = finergy.get_doc({"doctype": "File", "file_name": "capkpi-conf-14.png"})
		finergy.delete_doc("File", existing_file.name)
		...
		self.assertTrue("capkpi-conf-14.png" in [f.file_name for f in attachments])
	...
	def test_outgoing(self):
		...
		mail = email.message_from_string(finergy.get_last_doc("Email Queue").message)
	...
	def test_sendmail(self):
		...
		sent_mail = email.message_from_string(finergy.safe_decode(finergy.flags.sent_mail))
	...
	def test_print_format(self):
		...
		sent_mail = email.message_from_string(finergy.get_last_doc("Email Queue").message)
	...
	def test_threading(self):
		...
		sent_name = make(
			...
			doctype="ToDo",
			name=finergy.get_last_doc("ToDo").name,
			send_email=True,
		)["name"]
		...
		sent = finergy.get_doc("Communication", sent_name)
	...
	def test_threading_by_message_id(self):
		...
		event = finergy.get_doc(dict(doctype="Event", subject="test-message")).insert()
		...
		last_mail = finergy.get_doc("Email Queue", dict(reference_name=event.name))
...
def cleanup(sender=None):
	...
	names = finergy.get_list("Communication", filters=filters, fields=["name"])
	for name in names:
		finergy.delete_doc_if_exists("Communication", name.name)
		finergy.delete_doc_if_exists("Communication Link", {"parent": name.name})
