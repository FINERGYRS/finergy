...
import finergy
from finergy import _, safe_encode
from finergy.core.doctype.communication.email import set_incoming_outgoing_accounts
from finergy.desk.form import assign_to
from finergy.email.receive import Email, EmailServer
from finergy.email.smtp import SMTPServer
from finergy.email.utils import get_port
...
from finergy.utils.html_utils import clean_email_html
from finergy.utils.jinja import render_template
from finergy.utils.user import get_system_managers, is_system_user
...
class InvalidEmailCredentials(finergy.ValidationError):
	pass
...
class EmailAccount(Document):
	...
	def validate(self):
		...
		if self.login_id_is_different:
			if not self.login_id:
				finergy.throw(_("Login Id is required"))
		else:
			self.login_id = None

		duplicate_email_account = finergy.get_all(
		...
		if duplicate_email_account:
			finergy.throw(
				_("Email ID must be unique, Email Account already exists for {0}").format(
					finergy.bold(self.email_id)
				)
			)

		if finergy.local.flags.in_patch or finergy.local.flags.in_test:
			return

		# if self.enable_incoming and not self.append_to:
		# 	finergy.throw(_("Append To is mandatory for incoming mails"))

		self.use_starttls = cint(self.use_imap and self.use_starttls and not self.use_ssl)

		if (
			not self.awaiting_password
			and not finergy.local.flags.in_install
			and not finergy.local.flags.in_patch
		):
			if self.password or self.smtp_server in ("127.0.0.1", "localhost"):
				...
				if self.enable_incoming or (self.enable_outgoing and not self.no_smtp_authentication):
					finergy.throw(_("Password is required or select Awaiting Password"))

		if self.notify_if_unreplied:
			if not self.send_notification_to:
				finergy.throw(_("{0} is mandatory").format(self.meta.get_label("send_notification_to")))
		...
		if self.enable_incoming and self.append_to:
			...
			if self.append_to not in valid_doctypes:
				finergy.throw(_("Append To can be one of {0}").format(comma_or(valid_doctypes)))
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					finergy.bold(_("Default Incoming")), finergy.bold(_("Enable Incoming"))
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					finergy.bold(_("Default Outgoing")), finergy.bold(_("Enable Outgoing"))
				)
			)
		if messages:
			if len(messages) == 1:
				(as_list, messages) = (0, messages[0])
			finergy.msgprint(messages, as_list=as_list, indicator="orange", title=_("Defaults Updated"))
	...
	def there_must_be_only_one_default(self):
		...
		for field in ("default_incoming", "default_outgoing"):
			...
			for email_account in finergy.get_all("Email Account", filters={field: 1}):
				if email_account.name == self.name:
					continue

				email_account = finergy.get_doc("Email Account", email_account.name)
	...
	def get_domain(self, email_id):
		...
		try:
			...
			return finergy.db.get_value("Email Domain", domain[1], fields, as_dict=True)
		except Exception:
			pass
	...
	def check_smtp(self):
		...
		if self.enable_outgoing:
			if not self.smtp_server:
				finergy.throw(_("{0} is required").format("SMTP Server"))
	...
	def get_incoming_server(self, in_receive=False, email_sync_rule="UNSEEN"):
		...
		if finergy.cache().get_value("workers:no-internet") == True:
			return None
		...
		if not args.get("host"):
			finergy.throw(_("{0} is required").format("Email Server"))

		email_server = EmailServer(finergy._dict(args))
	...
	def set_failed_attempts_count(self, value):
		finergy.cache().set("{0}:email-account-failed-attempts".format(self.name), value)

	def get_failed_attempts_count(self):
		return cint(finergy.cache().get("{0}:email-account-failed-attempts".format(self.name)))
	...
	def receive(self, test_mails=None):
		...
		if self.enable_incoming:
			...
			if finergy.local.flags.in_test:
				...
				try:
					...
					finergy.log_error(title=_("Error while connecting to email account {0}").format(self.name))
			...
			for idx, msg in enumerate(incoming_mails):
				...
				try:
					...
					finergy.db.rollback()
					finergy.log_error("email_account.receive")
					if self.use_imap:
						self.handle_bad_emails(email_server, uid, msg, finergy.get_traceback())
					exceptions.append(finergy.get_traceback())
			...
			if len(incoming_mails) > 0 and not finergy.local.flags.in_test:
				finergy.publish_realtime(
					"new_email", {"account": self.email_account_name, "number": len(incoming_mails)}
				)

			if exceptions:
				raise Exception(finergy.as_json(exceptions))
	...
	def handle_bad_emails(self, email_server, uid, raw, reason):
		if email_server and cint(email_server.settings.use_imap):
			...
			unhandled_email = finergy.get_doc(
				{
					"raw": raw,
					"uid": uid,
					"reason": reason,
	...
	def insert_communication(self, msg, args=None):
		...
		if email.message_id:
			...
			if names:
				...
				finergy.db.set_value(
					...
					name,
					"uid",
					finergy.safe_decode(uid),
					update_modified=False,
				)

				self.flags.notify = False

				return finergy.get_doc("Communication", name)

		if email.content_type == "text/html":
			email.content = clean_email_html(email.content)

		communication = finergy.get_doc(
			{
		...
		if communication.seen:
			...
			users = finergy.get_all("User Email", filters={"email_account": self.name}, fields=["parent"])
	...
	def set_sender_field_and_subject_field(self):
		...
		meta = finergy.get_meta(self.append_to)
		...
		if self.append_to and self.sender_field:
			if self.subject_field:
				if "#" in email.subject:
					...
					if parent_id:
						parent = finergy.db.get_all(self.append_to, filters=dict(name=parent_id), fields="name")

				if not parent:
					# try and match by subject and sender
					# if sent by same sender with same subject,
					# append it to old coversation
					subject = finergy.as_unicode(
						strip(
							re.sub(
			...
			if parent:
				parent = finergy._dict(doctype=self.append_to, name=parent[0].name)
				return parent
	...
	def create_new_parent(self, communication, email):
		...
		parent = finergy.new_doc(self.append_to)

		if self.subject_field:
			parent.set(self.subject_field, finergy.as_unicode(email.subject)[:140])

		if self.sender_field:
			parent.set(self.sender_field, finergy.as_unicode(email.from_email))
		...
		try:
			...
			parent_name = finergy.db.get_value(self.append_to, {self.sender_field: email.from_email})
			if parent_name:
	...
	def find_parent_from_in_reply_to(self, communication, email):
		...
		if in_reply_to:
			if "@{0}".format(finergy.local.site) in in_reply_to:
				# reply to a communication sent from the system
				email_queue = finergy.db.get_value(
					"Email Queue",
				...
				if finergy.db.exists(parent_doctype, parent_name):
					parent = finergy._dict(doctype=parent_doctype, name=parent_name)
					...
					if parent_doctype == "Communication":
						...
						if parent.reference_name:
							...
							parent = finergy.get_doc(parent.reference_doctype, parent.reference_name)
			else:
				comm = finergy.db.get_value(
					"Communication",
				...
				if comm and comm.reference_doctype and comm.reference_name:
					parent = finergy._dict(doctype=comm.reference_doctype, name=comm.reference_name)
	...
	def send_auto_reply(self, communication, email):
		...
		if self.enable_auto_reply:
			...
			finergy.sendmail(
				...
				or finergy.get_template("templates/emails/auto_reply.html").render(communication.as_dict()),
	...
	def on_trash(self):
		...
		finergy.db.sql("update `tabCommunication` set email_account='' where email_account=%s", self.name)
		remove_user_email_inbox(email_account=self.name)

	def after_rename(self, old, new, merge=False):
		finergy.db.set_value("Email Account", new, "email_account_name", new)
	...
	def check_automatic_linking_email_account(self):
		if self.enable_automatic_linking:
			if not self.enable_incoming:
				finergy.throw(_("Automatic Linking can be activated only if Incoming is enabled."))

			if finergy.db.exists(
				"Email Account", {"enable_automatic_linking": 1, "name": ("!=", self.name)}
			):
				finergy.throw(_("Automatic Linking can be activated only for one Email Account."))
...
def get_append_to(
	...
	for dt in finergy.get_all("DocType", filters=filters, fields=["name", "email_append_to"]):
		email_append_to_list.append(dt.name)

	# Set Email Append To DocTypes set via Customize Form
	for dt in finergy.get_list(
...
def notify_unreplied():
	...
	for email_account in finergy.get_all(
		...
		if email_account.append_to:
			...
			for comm in finergy.get_all(
				...
				comm = finergy.get_doc("Communication", comm.name)

				if finergy.db.get_value(comm.reference_doctype, comm.reference_name, "status") == "Open":
...
def pull(now=False):
	...
	if finergy.cache().get_value("workers:no-internet") == True:
		if test_internet():
			finergy.cache().set_value("workers:no-internet", False)
		else:
			return
	queued_jobs = get_jobs(site=finergy.local.site, key="job_name")[finergy.local.site]
	for email_account in finergy.get_list(
...
def pull_from_email_account(email_account):
	...
	email_account = finergy.get_doc("Email Account", email_account)
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing):
	...
	from finergy.core.doctype.user.user import ask_pass_update
	...
	user_names = finergy.db.get_values("User", {"email": email_id}, as_dict=True)
	if not user_names:
		return
	...
	if update_user_email_settings:
		...
		users = " and ".join([finergy.bold(user.get("name")) for user in user_names])
		finergy.msgprint(_("Enabled email inbox for user {0}").format(users))
	ask_pass_update()
...
def set_email_password(email_account, user, password):
	account = finergy.get_doc("Email Account", email_account)
