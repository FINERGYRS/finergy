...
finergy.ui.form.on("Email Account", {
	service: function(frm) {
		$.each(finergy.email_defaults[frm.doc.service], function(key, value) {
			frm.set_value(key, value);
		});
		if (!frm.doc.use_imap) {
			$.each(finergy.email_defaults_pop[frm.doc.service], function(key, value) {
				frm.set_value(key, value);
			});
		}
	...
	onload: function(frm) {
		...
		frm.set_query("append_to", "finergy.email.doctype.email_account.email_account.get_append_to");
	},
	...
	refresh: function(frm) {
		...
		if(finergy.route_flags.delete_user_from_locals && finergy.route_flags.linked_user) {
			delete finergy.route_flags.delete_user_from_locals;
			delete locals['User'][finergy.route_flags.linked_user];
		}
	},
	...
	set_domain_fields: function(frm, args) {
		if(!args){
			args = finergy.route_flags.set_domain_values? finergy.route_options: {};
		}
		...
		delete finergy.route_flags.set_domain_values;
	...
	email_sync_option: function(frm) {
		...
		if(frm.doc.email_sync_option == "ALL"){
			...
			finergy.confirm(msg, null, function() {
	...
	warn_autoreply_on_incoming: function(frm) {
		if (frm.doc.enable_incoming && frm.doc.enable_auto_reply && frm.doc.__islocal) {
			...
			finergy.confirm(msg, null, function() {
				...
				finergy.show_alert({message: __("Disabled Auto Reply"), indicator: "blue"});
			});
		}
	}
});
