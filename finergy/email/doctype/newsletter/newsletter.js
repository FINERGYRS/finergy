...
finergy.ui.form.on('Newsletter', {
	refresh(frm) {
		let doc = frm.doc;
		if (!doc.__islocal && !cint(doc.email_sent) && !doc.__unsaved
				&& in_list(finergy.boot.user.can_write, doc.doctype)) {
			frm.add_custom_button(__('Send Now'), function() {
				finergy.confirm(__("Do you really want to send this email newsletter?"), function() {
		...
		if (doc.__islocal && !doc.send_from) {
			let { fullname, email } = finergy.user_info(doc.owner);
