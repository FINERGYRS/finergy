...
from finergy.email.doctype.email_group.email_group import add_subscribers
from finergy.utils.verified_command import get_signed_params, verify_request
...
class Newsletter(WebsiteGenerator):
	...
	@finergy.whitelist()
	def test_send(self):
		test_emails = finergy.utils.split_emails(self.test_email_id)
		self.queue_all(test_emails=test_emails)
		finergy.msgprint(_("Test email sent to {0}").format(self.test_email_id))

	@finergy.whitelist()
	def send_emails(self):
		"""send emails to leads and customers"""
		self.queue_all()
		finergy.msgprint(_("Email queued to {0} recipients").format(len(self.newsletter_recipients)))

	def setup_newsletter_status(self):
		"""Setup analytical status for current Newsletter. Can be accessible from desk."""
		if self.email_sent:
			status_count = finergy.get_all(
				"Email Queue",
	...
	def validate_newsletter_status(self):
		if self.email_sent:
			finergy.throw(_("Newsletter has already been sent"), exc=NewsletterAlreadySentError)

		if self.get("__islocal"):
			finergy.throw(_("Please save the Newsletter before sending"), exc=NewsletterNotSavedError)

	def validate_newsletter_recipients(self):
		if not self.newsletter_recipients:
			finergy.throw(_("Newsletter should have atleast one recipient"), exc=NoRecipientFoundError)
		self.validate_recipient_address()

	def validate_sender_address(self):
		"""Validate self.send_from is a valid email address or not."""
		if self.send_from:
			finergy.utils.validate_email_address(self.send_from, throw=True)
	...
	def validate_recipient_address(self):
		...
		for recipient in self.newsletter_recipients:
			finergy.utils.validate_email_address(recipient, throw=True)
	...
	def queue_all(self, test_emails: List[str] = None):
		...
		if test_emails:
			for test_email in test_emails:
				finergy.utils.validate_email_address(test_email, throw=True)
		else:
			self.validate()
			self.validate_send()
		...
		if not test_emails:
			...
			self.schedule_send = finergy.utils.now_datetime()
	...
	def send_newsletter(self, emails: List[str]):
		...
		sender = self.send_from or finergy.utils.get_formatted_email(self.owner)
		args = self.as_dict()
		args["message"] = self.get_message()

		is_auto_commit_set = bool(finergy.db.auto_commit_on_many_writes)
		finergy.db.auto_commit_on_many_writes = not finergy.flags.in_test
		...
		finergy.db.auto_commit_on_many_writes = is_auto_commit_set

	def get_message(self) -> str:
		if self.content_type == "HTML":
			return finergy.render_template(self.message_html, {"doc": self.as_dict()})
		if self.content_type == "Markdown":
			return finergy.utils.markdown(self.message_md)
		# fallback to Rich Text
		return self.message

	def get_recipients(self) -> List[str]:
		"""Get recipients from Email Group"""
		emails = finergy.get_all(
			"Email Group Member",
	...
	def get_email_groups(self) -> List[str]:
		...
		return [x.email_group for x in self.email_group] or finergy.get_all(
...
def confirmed_unsubscribe(email, group):
	...
	finergy.flags.ignore_permissions = True
	doc = finergy.get_doc("Email Group Member", {"email": email, "email_group": group})
...
def subscribe(email, email_group=_("Website")):
	...
	api_endpoint = finergy.utils.get_url(
		"/api/method/finergy.email.doctype.newsletter.newsletter.confirm_subscription"
	)
	...
	email_confirmation_template = finergy.db.get_value(
	...
	if email_confirmation_template:
		...
		email_template = finergy.get_doc("Email Template", email_confirmation_template)
		email_subject = email_template.subject
		content = finergy.render_template(email_template.response, args)
	else:
...
def confirm_subscription(email, email_group=_("Website")):
	...
	if not finergy.db.exists("Email Group", email_group):
		finergy.get_doc({"doctype": "Email Group", "title": email_group}).insert(ignore_permissions=True)
...
def get_newsletter_list(
	...
	email_group_list = finergy.db.sql(
		"""SELECT eg.name
...
def send_scheduled_email():
	...
	scheduled_newsletter = finergy.get_all(
		"Newsletter",
		filters={
			"schedule_send": ("<=", finergy.utils.now_datetime()),
			"email_sent": False,
	...
	for newsletter in scheduled_newsletter:
		try:
			finergy.get_doc("Newsletter", newsletter).queue_all()

		except Exception:
			finergy.db.rollback()

			# wasn't able to send emails :(
			finergy.db.set_value("Newsletter", newsletter, "email_sent", 0)
			message = (
				f"Newsletter {newsletter} failed to send" "\n\n" f"Traceback: {finergy.get_traceback()}"
			)
			finergy.log_error(title="Send Newsletter", message=message)
