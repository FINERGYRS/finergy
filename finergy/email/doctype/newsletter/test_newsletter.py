...
import finergy
from finergy.desk.form.load import run_onload
from finergy.email.doctype.newsletter.exceptions import (
	NewsletterAlreadySentError,
	NoRecipientFoundError,
)
from finergy.email.doctype.newsletter.newsletter import (
	Newsletter,
	confirmed_unsubscribe,
	get_newsletter_list,
	send_scheduled_email,
)
from finergy.email.queue import flush
...
class TestNewsletterMixin:
	...
	def tearDown(self):
		...
		for newsletter in newsletters:
			...
			finergy.delete_doc("Newsletter", newsletter)
			finergy.db.delete("Newsletter Email Group", newsletter)
			newsletters.remove(newsletter)

	def setup_email_group(self):
		if not finergy.db.exists("Email Group", "_Test Email Group"):
			finergy.get_doc({"doctype": "Email Group", "title": "_Test Email Group"}).insert()
		...
		for email in emails:
			...
			try:
				...
				finergy.db.update(doctype, email_filters, "unsubscribed", 0)

	def send_newsletter(self, published=0, schedule_send=None) -> Union[str, None]:
		finergy.db.delete("Email Queue")
		finergy.db.delete("Email Queue Recipient")
		finergy.db.delete("Newsletter")
	...
	def get_newsletter(**kwargs) -> "Newsletter":
		...
		similar_newsletters = finergy.db.get_all(doctype, newsletter_content, pluck="name")

		for similar_newsletter in similar_newsletters:
			finergy.delete_doc(doctype, similar_newsletter)

		newsletter = finergy.get_doc({"doctype": doctype, **newsletter_content, **kwargs})
		...
		attached_files = finergy.get_all(
			"File",
			{
...
class TestNewsletter(TestNewsletterMixin, unittest.TestCase):
	def test_send(self):
		...
		email_queue_list = [finergy.get_doc("Email Queue", e.name) for e in finergy.get_all("Email Queue")]
	...
	def test_send_newsletter_with_attachments(self):
		newsletter = self.get_newsletter()
		newsletter.reload()
		file_attachment = finergy.get_doc(
			{
				"doctype": "File",
				"file_name": "test1.txt",
				"attached_to_doctype": newsletter.doctype,
				"attached_to_name": newsletter.name,
				"content": finergy.mock("paragraph"),
			}
		)
	...
	def test_send_scheduled_email_error_handling(self):
		...
		job_path = "finergy.email.doctype.newsletter.newsletter.Newsletter.queue_all"
		m = MagicMock(side_effect=finergy.OutgoingEmailError)

		with self.assertRaises(finergy.OutgoingEmailError):
