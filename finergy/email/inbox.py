...
def create_email_flag_queue(names, action):
	...
	def mark_as_seen_unseen(name, action):
		doc = finergy.get_doc("Communication", name)
		if action == "Read":
			doc.add_seen()
		else:
			_seen = json.loads(doc._seen or "[]")
			_seen = [user for user in _seen if finergy.session.user != user]
	...
	for name in json.loads(names or []):
		uid, seen_status, email_account = finergy.db.get_value(
		...
		if (action == "Read" and seen_status == 0) or (action == "Unread" and seen_status == 1):
			create_new = True
			email_flag_queue = finergy.db.sql(
			...
			for queue in email_flag_queue:
				if queue.action != action:
					finergy.delete_doc("Email Flag Queue", queue.name, ignore_permissions=True)
				elif queue.action == action:
					# Read or Unread request for email is already available
					create_new = False

			if create_new:
				flag_queue = finergy.get_doc(
					{
						"uid": uid,
						"action": action,
						"communication": name,
				...
				finergy.db.set_value("Communication", name, "seen", seen, update_modified=False)
...
def mark_as_closed_open(communication, status):
	...
	finergy.db.set_value("Communication", communication, "status", status)


@finergy.whitelist()
def move_email(communication, email_account):
	"""Move email to another email account."""
	finergy.db.set_value("Communication", communication, "email_account", email_account)


@finergy.whitelist()
def mark_as_trash(communication):
	"""Set email status to trash."""
	finergy.db.set_value("Communication", communication, "email_status", "Trash")


@finergy.whitelist()
def mark_as_spam(communication, sender):
	"""Set email status to spam."""
	email_rule = finergy.db.get_value("Email Rule", {"email_id": sender})
	if not email_rule:
		finergy.get_doc({"doctype": "Email Rule", "email_id": sender, "is_spam": 1}).insert(
			ignore_permissions=True
		)
	finergy.db.set_value("Communication", communication, "email_status", "Spam")
