...
import finergy
from finergy import _, safe_decode, safe_encode
from finergy.core.doctype.file.file import MaxFileSizeReachedError, get_random_filename
...
class EmailSizeExceededError(finergy.ValidationError):
	pass


class EmailTimeoutError(finergy.ValidationError):
	pass


class TotalSizeExceededError(finergy.ValidationError):
	pass


class LoginLimitExceeded(finergy.ValidationError):
	pass
...
class EmailServer:
	...
	def setup(self, args=None):
		# overrride
		self.settings = args or finergy._dict()
		...
		try:
			if cint(self.settings.use_ssl):
				self.imap = Timed_IMAP4_SSL(
					self.settings.host, self.settings.incoming_port, timeout=finergy.conf.get("pop_timeout")
				)
			else:
			...
			finergy.msgprint(_("Invalid Mail Server. Please rectify and try again."))
			raise
	...
	def connect_pop(self):
		...
		try:
			...
			finergy.log_error("receive.connect_pop")
			...
			if self.is_temporary_system_problem(e):
				...
				finergy.msgprint(_("Invalid User Name or Support Password. Please rectify and try again."))
				raise
	...
	def get_messages(self):
		...
		try:
			...
			# size limits
			self.total_size = 0
			self.max_email_size = cint(finergy.local.conf.get("max_email_size"))
	...
	def check_imap_uidvalidity(self):
		...
		finergy.db.set_value("Email Account", self.settings.email_account, "uidnext", uidnext)
	...
	def retrieve_message(self, message_meta, msg_num=None):
		...
		try:
			...
			if self.has_login_limit_exceeded(e):
				...
				# log performs rollback and logs error in Error Log
				finergy.log_error("receive.get_messages", self.make_error_msg(msg_num, incoming_mail))
				self.errors = True
				finergy.db.rollback()
	...
	def get_email_seen_status(self, uid, flag_string):
		...
		for flag in imaplib.ParseFlags(flag_string) or []:
			...
			match = re.search(pattern, finergy.as_unicode(flag))
...
class Email:
	...
	def set_from(self):
		...
		if _reply_to and not finergy.db.get_value("Email Account", {"email_id": _reply_to}, "email_id"):
	...
	def decode_email(self, email):
		...
		for part, encoding in decode_header(
			finergy.as_unicode(email).replace('"', " ").replace("'", " ")
		):
			if encoding:
	...
	def save_attachments_in_doc(self, doc):
		...
		for attachment in self.attachments:
			try:
				...
				pass
			except finergy.FileAlreadyAttachedException:
				pass
