...
import finergy
from finergy.desk.reportview import build_match_conditions


def sendmail_to_system_managers(subject, content):
	finergy.sendmail(recipients=get_system_managers(), subject=subject, content=content)
...
def get_communication_doctype(doctype, txt, searchfield, start, page_len, filters):
	user_perms = finergy.utils.user.UserPermissions(finergy.session.user)
	...
	if len(txt) < 2:
		...
		for name in finergy.get_hooks("communication_doctypes"):
			try:
		...
		com_doctypes = [
			d[0] for d in finergy.db.get_values("DocType", {"issingle": 0, "istable": 0, "hide_toolbar": 0})
		]
...
def get_cached_contacts(txt):
	contacts = finergy.cache().hget("contacts", finergy.session.user) or []
...
def update_contact_cache(contacts):
	cached_contacts = finergy.cache().hget("contacts", finergy.session.user) or []

	uncached_contacts = [d for d in contacts if d not in cached_contacts]
	cached_contacts.extend(uncached_contacts)

	finergy.cache().hset("contacts", finergy.session.user, cached_contacts)
