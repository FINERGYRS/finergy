...
import finergy
from finergy.email.smtp import get_outgoing_email_account
...
class EMail:
	...
	def attach_file(self, n):
		...
		_file = finergy.get_doc("File", {"file_name": n})
	...
	def make(self):
		...
		for hook in finergy.get_hooks("make_email_body_message"):
			finergy.get_attr(hook)(self)
...
def get_formatted_html(
	...
	rendered_email = finergy.get_template("templates/emails/standard.html").render(
		{
...
def get_email_html(template, args, subject, header=None, with_container=False):
	...
	email = finergy.utils.jinja.get_email_from_template(template, args)
...
def get_footer(email_account, footer=None):
	...
	sender_address = finergy.db.get_default("email_footer_address")

	if sender_address:
		args.update({"sender_address": sender_address})

	if not cint(finergy.db.get_default("disable_standard_email_footer")):
		args.update({"default_mail_footer": finergy.get_hooks("default_mail_footer")})

	footer += finergy.utils.jinja.get_email_from_template("email_footer", args)[0]

	return footer


def replace_filename_with_cid(message):
	"""Replaces <img embed="assets/finergyrs/images/filename.jpg" ...> with
...
def get_filecontent_from_path(path):
	...
	if path.startswith("assets/"):
		...
		# public file
		full_path = finergy.get_site_path("public", path)
	elif path.startswith("private/files/"):
		# private file
		full_path = finergy.get_site_path(path)
	else:
		full_path = path
...
def get_header(header=None):
	...
	from finergy.utils.jinja import get_email_from_template
	...
	if not title:
		title = finergy.get_hooks("app_title")[-1]
