...
import finergy
from finergy.query_builder.terms import NamedParameterWrapper
...
class ImportMapper:
	...
	def __call__(self, *args: Any, **kwds: Any) -> Callable:
		db = db_type_is(finergy.conf.db_type or "mariadb")
...
def DocType(*args, **kwargs):
	return finergy.qb.DocType(*args, **kwargs)


def patch_query_execute():
	"""Patch the Query Builder with helper execute method
	This excludes the use of `finergy.db.sql` method while
	executing the query object
	"""

	def execute_query(query, *args, **kwargs):
		query, params = prepare_query(query)
		return finergy.db.sql(query, params, *args, **kwargs)  # nosemgrep
	...
	def prepare_query(query):
		...
		if finergy.flags.in_safe_exec and not query.lower().strip().startswith("select"):
	...
	query_class = get_attr(str(finergy.qb).split("'")[1])
