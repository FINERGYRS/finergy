...
def add(
	...
	if share_name:
		doc = finergy.get_doc("DocShare", share_name)
	else:
		doc = finergy.new_doc("DocShare")
		doc.update(
...
def remove(doctype, name, user, flags=None):
	share_name = finergy.db.get_value(
		"DocShare", {"user": user, "share_name": name, "share_doctype": doctype}
	)

	if share_name:
		finergy.delete_doc("DocShare", share_name, flags=flags)
...
def set_permission(doctype, name, user, permission_to, value=1, everyone=0):
	...
	if not share_name:
		...
		share = finergy.get_doc("DocShare", share_name)
...
def get_shared(doctype, user=None, rights=None):
	...
	shared_docs = finergy.db.get_all(
...
def check_share_permission(doctype, name):
	...
	if not finergy.has_permission(doctype, ptype="share", doc=name):
		finergy.throw(
			_("No permission to {0} {1} {2}").format("share", _(doctype), name), finergy.PermissionError
		)
...
def notify_assignment(shared_by, doctype, doc_name, everyone, notify=0):
	...
	reference_user = get_fullname(finergy.session.user)
	notification_message = _("{0} shared a document {1} {2} with you").format(
		finergy.bold(reference_user), finergy.bold(_(doctype)), get_title_html(title)
	)
