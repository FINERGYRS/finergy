...
from finergy import _, msgprint
from finergy.query_builder import DocType
...
def check_admin_or_system_manager(user=None):
	...
	if ("System Manager" not in finergy.get_roles(user)) and (user != "Administrator"):
		finergy.throw(_("Not permitted"), finergy.PermissionError)


def print_has_permission_check_logs(func):
	def inner(*args, **kwargs):
		finergy.flags["has_permission_check_logs"] = []
		result = func(*args, **kwargs)
		self_perm_check = True if not kwargs.get("user") else kwargs.get("user") == finergy.session.user
		...
		if not result and self_perm_check and raise_exception:
			msgprint(("<br>").join(finergy.flags.get("has_permission_check_logs", [])))
		finergy.flags.pop("has_permission_check_logs", None)
		return result
...
def has_permission(
	...
	if doc:
		if isinstance(doc, string_types):
			doc = finergy.get_doc(meta.name, doc)
		perm = get_doc_permissions(doc, user=user, ptype=ptype).get(ptype)
		if not perm:
			push_perm_check_log(
				_("User {0} does not have access to this document").format(finergy.bold(user))
			)
	else:
		...
		if not perm:
			push_perm_check_log(
				_("User {0} does not have doctype access via role permission for document {1}").format(
					finergy.bold(user), finergy.bold(doctype)
				)
			)

	def false_if_not_shared():
		if ptype in ("read", "write", "share", "submit", "email", "print"):
			shared = finergy.share.get_shared(
...
def get_doc_permissions(doc, user=None, ptype=None):
	...
	if finergy.is_table(doc.doctype):
		return {"read": 1, "write": 1}

	meta = finergy.get_meta(doc.doctype)

	def is_user_owner():
		return (doc.get("owner") or "").lower() == finergy.session.user.lower()
...
def get_role_permissions(doctype_meta, user=None, is_owner=None):
	...
	if isinstance(doctype_meta, string_types):
		doctype_meta = finergy.get_meta(doctype_meta)  # assuming doctype name was passed
	...
	if not finergy.local.role_permissions.get(cache_key):
		perms = finergy._dict(if_owner={})
		...
		finergy.local.role_permissions[cache_key] = perms

	return finergy.local.role_permissions[cache_key]


def get_user_permissions(user):
	from finergy.core.doctype.user_permission.user_permission import get_user_permissions
...
def has_user_permission(doc, user=None):
	...
	apply_strict_user_permissions = finergy.get_system_settings("apply_strict_user_permissions")
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = finergy.get_meta(d.get("doctype"))
...
def has_controller_permissions(doc, ptype, user=None):
	...
	methods = finergy.get_hooks("has_permission").get(doc.doctype, [])

	if not methods:
		return None

	for method in reversed(methods):
		controller_permission = finergy.call(finergy.get_attr(method), doc=doc, ptype=ptype, user=user)
...
def get_all_perms(role):
	...
	perms = finergy.get_all("DocPerm", fields="*", filters=dict(role=role))
	custom_perms = finergy.get_all("Custom DocPerm", fields="*", filters=dict(role=role))
	doctypes_with_custom_perms = finergy.db.sql_list(
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == "Administrator":
			return [r[0] for r in finergy.db.sql("select name from `tabRole`")]  # return all available roles
		else:
			return (
				[
					r[0]
					for r in finergy.db.sql(
	...
	roles = finergy.cache().hget("roles", user, get)
...
def get_perms_for(roles, perm_doctype="DocPerm"):
	...
	return finergy.db.get_all(perm_doctype, fields=["*"], filters=filters)


def get_doctypes_with_custom_docperms():
	"""Returns all the doctypes with Custom Docperms"""

	doctypes = finergy.db.get_all("Custom DocPerm", fields=["parent"], distinct=1)
...
def set_user_permission_if_allowed(doctype, name, user, with_message=False):
	if get_role_permissions(finergy.get_meta(doctype), user).set_user_permissions != 1:
...
def add_user_permission(
	...
	"""Add user permission"""
	from finergy.core.doctype.user_permission.user_permission import user_permission_exists

	if not user_permission_exists(user, doctype, name, applicable_for):
		if not finergy.db.exists(doctype, name):
			finergy.throw(_("{0} {1} not found").format(_(doctype), name), finergy.DoesNotExistError)
...
def remove_user_permission(doctype, name, user):
	user_permission_name = finergy.db.get_value(
		"User Permission", dict(user=user, allow=doctype, for_value=name)
	)
	finergy.delete_doc("User Permission", user_permission_name)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	if user:
		filters["user"] = user
	user_permissions_for_doctype = finergy.db.get_all("User Permission", filters=filters)
	for d in user_permissions_for_doctype:
		finergy.delete_doc("User Permission", d.name)


def can_import(doctype, raise_exception=False):
	if not ("System Manager" in finergy.get_roles() or has_permission(doctype, "import")):
		if raise_exception:
			raise finergy.PermissionError("You are not allowed to import: {doctype}".format(doctype=doctype))
		else:
			return False
	return True
...
def can_export(doctype, raise_exception=False):
	if "System Manager" in finergy.get_roles():
		return True
	else:
		role_permissions = finergy.permissions.get_role_permissions(doctype)
		has_access = role_permissions.get("export") or role_permissions.get("if_owner").get("export")
		if not has_access and raise_exception:
			raise finergy.PermissionError(_("You are not allowed to export {} doctype").format(doctype))
		return has_access


def update_permission_property(doctype, role, permlevel, ptype, value=None, validate=True):
	"""Update a property in Custom Perm"""
	from finergy.core.doctype.doctype.doctype import validate_permissions_for_doctype
	...
	finergy.qb.update(table).set(ptype, value).where(table.name == name).run()
...
def setup_custom_perms(parent):
	...
	if not finergy.db.exists("Custom DocPerm", dict(parent=parent)):
		copy_perms(parent)
		return True
...
def add_permission(doctype, role, permlevel=0, ptype=None):
	...
	custom_docperm = finergy.get_doc(
		{
...
def copy_perms(parent):
	...
	for d in finergy.get_all("DocPerm", fields="*", filters=dict(parent=parent)):
		custom_perm = finergy.new_doc("Custom DocPerm")
...
def reset_perms(doctype):
	...
	from finergy.desk.notifications import delete_notification_count_for

	delete_notification_count_for(doctype)

	finergy.db.sql("""delete from `tabCustom DocPerm` where parent=%s""", doctype)
...
def get_linked_doctypes(dt):
	return list(
		set(
			...
			+ [
				d.options
				for d in finergy.get_meta(dt).get(
					"fields",
...
def push_perm_check_log(log):
	if finergy.flags.get("has_permission_check_logs") == None:
		return
	finergy.flags.get("has_permission_check_logs").append(_(log))
