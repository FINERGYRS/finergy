...
class PrintFormat(Document):
	def validate(self):
		if (
			...
			finergy.throw(finergy._("Standard Print Format cannot be updated"))

		# old_doc_type is required for clearing item cache
		self.old_doc_type = finergy.db.get_value("Print Format", self.name, "doc_type")
		...
		if self.custom_format and self.raw_printing and not self.raw_commands:
			finergy.throw(
				_("{0} are required").format(finergy.bold(_("Raw Commands"))), finergy.MandatoryError
			)

		if self.custom_format and not self.html and not self.raw_printing:
			finergy.throw(_("{0} is required").format(finergy.bold(_("HTML"))), finergy.MandatoryError)
	...
	def on_update(self):
		if hasattr(self, "old_doc_type") and self.old_doc_type:
			finergy.clear_cache(doctype=self.old_doc_type)
		if self.doc_type:
...
def make_default(name):
	...
	finergy.has_permission("Print Format", "write")

	print_format = finergy.get_doc("Print Format", name)

	if (finergy.conf.get("developer_mode") or 0) == 1:
		# developer mode, set it default in doctype
		doctype = finergy.get_doc("DocType", print_format.doc_type)
	...
	finergy.msgprint(
		finergy._("{0} is now default print format for {1} doctype").format(
			finergy.bold(name), finergy.bold(print_format.doc_type)
		)
	)
