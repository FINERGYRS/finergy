finergy.pages['print'].on_page_load = function(wrapper) {
	finergy.ui.make_app_page({
		parent: wrapper,
	});

	let print_view = new finergy.ui.form.PrintView(wrapper);

	$(wrapper).bind('show', () => {
		const route = finergy.get_route();
		const doctype = route[1];
		const docname = route.slice(2).join("/");
		if (!finergy.route_options || !finergy.route_options.frm) {
			finergy.model.with_doc(doctype, docname, () => {
				let frm = { doctype: doctype, docname: docname };
				frm.doc = finergy.get_doc(doctype, docname);
				finergy.model.with_doctype(doctype, () => {
					frm.meta = finergy.get_meta(route[1]);
					print_view.show(frm);
				});
			});
		} else {
			print_view.frm = finergy.route_options.frm.doctype ?
				finergy.route_options.frm : finergy.route_options.frm.frm;
			finergy.route_options.frm = null;
			print_view.show(print_view.frm);
		}
	});
};

finergy.ui.form.PrintView = class {
	constructor(wrapper) {
	...
	make() {
		this.print_wrapper = this.page.main.empty().html(
			`<div class="print-preview-wrapper"><div class="print-preview">
				${finergy.render_template('print_skeleton_loading')}
		...
		this.print_settings = finergy.model.get_doc(
			':Print Settings',
			'Print Settings'
		);
		this.setup_menu();
	...
	setup_toolbar() {
		...
		this.page.add_button(
			__('PDF'),
			() => this.render_page('/api/method/finergy.utils.print_format.download_pdf?'),
			{ icon: 'small-file' }
		);
	...
	add_sidebar_item(df, is_dynamic) {
		...
		let field = finergy.ui.form.make_control({
			df: df,
	...
	setup_menu() {
		...
		if (finergy.model.can_create('Print Format')) {
	...
	set_breadcrumbs() {
		finergy.breadcrumbs.add(this.frm.meta.module, this.frm.doctype);
	}
	...
	setup_additional_settings() {
		...
		finergy
			.xcall('finergy.printing.page.print.print.get_print_settings_to_show', {
	...
	edit_print_format() {
		...
		if (is_standard_but_editable) {
			finergy.set_route('Form', 'Print Format', print_format.name);
			return;
		}
		if (is_custom_format) {
			finergy.set_route('print-format-builder', print_format.name);
			return;
		}
		...
		finergy.prompt(
			...
			(data) => {
				...
				finergy.set_route('print-format-builder');
	...
	setup_keyboard_shortcuts() {
		this.wrapper.find('.print-toolbar a.btn-default').each((i, el) => {
			finergy.ui.keys.get_shortcut_group(this.frm.page).add($(el));
		});
	}
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = finergy.dom.pixel_to_inches(print_height);
	...
	setup_print_format_dom(out, $print_format) {
		...
		let base_url = finergy.urllib.get_base_url();
		let print_css_url = `${base_url}/assets/${finergy.utils.is_rtl(this.lang_code) ? 'css-rtl' : 'css'}/printview.css`;
		this.$print_format_body.find('html').attr('dir', finergy.utils.is_rtl(this.lang_code) ? 'rtl': 'ltr');
	...
	go_to_form_view() {
		...
		finergy.set_route('Form', this.frm.doctype, this.frm.docname);
	}
			...
			if (me.is_raw_printing()) {
				me.get_raw_commands(function(out) {
					finergy.ui.form
						...
						.then(finergy.ui.form.qz_success)
						.catch((err) => {
							finergy.ui.form.qz_fail(err);
						});
				});
			} else {
				finergy.show_alert(
	...
	print_by_server() {
		...
		if (localStorage.getItem('network_printer')) {
			finergy.call({
				method: 'finergy.utils.print_format.print_by_server',
				args: {
	...
	network_printer_setting_dialog(callback) {
		finergy.call({
			method: 'finergy.printing.doctype.network_printer_settings.network_printer_settings.get_network_printer_settings',
			callback: function(r) {
				if (r.message) {
	...
	render_page(method, printit = false) {
		...
		if (!w) {
			finergy.msgprint(__('Please enable pop-ups'));
			return;
		}
	}
	...
	get_print_html(callback) {
		...
		this._req = finergy.call({
			method: 'finergy.www.printview.get_html_and_style',
			args: {
				doc: this.frm.doc,
	...
	get_raw_commands(callback) {
		...
		finergy.call({
			method: 'finergy.www.printview.get_rendered_raw_commands',
			args: {
				doc: this.frm.doc,
				print_format: this.selected_format(),
				_lang: this.lang_code,
	...
	refresh_print_options() {
		this.print_formats = finergy.meta.get_print_formats(this.frm.doctype);
	...
	set_style(style) {
		finergy.dom.set_style(style || finergy.boot.print_css, 'print-style');
	}
	...
	printer_setting_dialog() {
		...
		this.printer_list = [];
		finergy.ui.form.qz_get_printer_list().then((data) => {
			...
			if (!(this.printer_list && this.printer_list.length)) {
				finergy.throw(__('No Printer is Available.'));
			}
		});
	}
};
