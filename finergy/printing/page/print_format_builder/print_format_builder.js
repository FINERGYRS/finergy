finergy.pages['print-format-builder'].on_page_load = function(wrapper) {
	finergy.print_format_builder = new finergy.PrintFormatBuilder(wrapper);
	finergy.breadcrumbs.add("Setup", "Print Format");
}

finergy.pages['print-format-builder'].on_page_show = function(wrapper) {
	var route = finergy.get_route();
	if(route.length>1) {
		finergy.model.with_doc('Print Format', route[1], function() {
			finergy.print_format_builder.print_format = finergy.get_doc('Print Format', route[1]);
			finergy.print_format_builder.refresh();
		});
	} else if(finergy.route_options) {
		if(finergy.route_options.make_new) {
			let { doctype, name, based_on } = finergy.route_options;
			finergy.route_options = null;
			finergy.print_format_builder.setup_new_print_format(doctype, name, based_on);
		} else {
			finergy.print_format_builder.print_format = finergy.route_options.doc;
			finergy.route_options = null;
			finergy.print_format_builder.refresh();
		}
	}
...
finergy.PrintFormatBuilder = Class.extend({
	...
	make: function() {
		...
		this.page.main = $('<div class="col-md-12 border print-format-builder-main finergy-card"></div>').appendTo(this.page.main);
	...
	show_start: function() {
		this.page.main.html(finergy.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format: function() {
		...
		var me = this;
		this.print_format_input = finergy.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function() {
			...
			if(!name) return;
			finergy.model.with_doc("Print Format", name, function(doc) {
				finergy.set_route('print-format-builder', name);
			});
		});
	},
	start_new_print_format: function() {
		var me = this;
		this.doctype_input = finergy.ui.form.make_control({
		...
		this.name_input = finergy.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function() {
			...
			if(!(doctype && name)) {
				finergy.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format: function(doctype, name, based_on) {
		finergy.call({
			method: 'finergy.printing.page.print_format_builder.print_format_builder.create_custom_format',
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on
	...
	setup_print_format: function() {
		var me = this;
		finergy.model.with_doctype(this.print_format.doc_type, function(doctype) {
			me.meta = finergy.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function() {
				finergy.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	},
	setup_sidebar: function() {
		...
		this.page.sidebar.html(
			$(finergy.render_template("print_format_builder_sidebar", {fields: fields}))
		);
	...
	render_layout: function() {
		...
		this.prepare_data();
		$(finergy.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function(i, f) {
			...
			if(!f.name && f.fieldname) {
				...
				if(f.fieldname==="_custom_html") {
					...
					f = $.extend(finergy.meta.get_docfield(me.print_format.doc_type,
						f.fieldname) || {}, f);
				}
			}
	...
	init_visible_columns: function(f) {
		f.visible_columns = []
		$.each(finergy.get_meta(f.options).fields, function(i, _f) {
			...
			onAdd: function(evt) {
				...
				if(!$item.hasClass("print-format-builder-field")) {
					...
					if(fieldname==="_custom_html") {
						...
						var field = finergy.meta.get_docfield(me.print_format.doc_type,
							fieldname);
					}

					var html = finergy.render_template("print_format_builder_field",
						{field: field, me:me});
	...
	setup_add_section: function() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function() {
			...
			var $section = $(finergy.render_template("print_format_builder_section",
	...
	setup_column_selector: function() {
		...
		this.page.main.on("click", ".select-columns", function() {
			...
			var doc_fields = finergy.get_meta(doctype).fields;
			...
			// render checkboxes
			$(finergy.render_template("print_format_builder_column_selector", {
				fields: fields,
	...
	save_print_format: function() {
		...
		finergy.call({
			...
			callback: function(r) {
				...
				finergy.show_alert({message: __("Saved"), indicator: 'green'});
			}
		});
	}
});
