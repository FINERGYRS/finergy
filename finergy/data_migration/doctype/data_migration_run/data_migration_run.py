...
from finergy.data_migration.doctype.data_migration_mapping.data_migration_mapping import (
	get_source_value,
)
...
class DataMigrationRun(Document):
	...
	def enqueue_next_mapping(self):
		...
		if next_mapping_name:
			...
			finergy.enqueue_doc(self.doctype, self.name, "run_current_mapping", now=finergy.flags.in_test)
		else:
			self.complete()
	...
	def enqueue_next_page(self):
		...
		if percent_complete < 100:
			finergy.publish_realtime(
				self.trigger_name, {"progress_percent": percent_complete}, user=finergy.session.user
			)
	...
	def get_last_modified_condition(self):
		last_run_timestamp = finergy.db.get_value(
			"Data Migration Run",
			dict(
	...
	def begin(self):
		...
		self.mappings = [
			finergy.get_doc("Data Migration Mapping", m.mapping) for m in plan_active_mappings
		]
	...
	def complete(self):
		...
		finergy.publish_realtime(self.trigger_name, {"progress_percent": 100}, user=finergy.session.user)
	...
	def execute_postprocess(self, status):
		...
		if postprocess_method_path:
			finergy.get_attr(postprocess_method_path)(
				{
					"status": status,
					"stats": {
	...
	def get_plan(self):
		if not hasattr(self, "plan"):
			self.plan = finergy.get_doc("Data Migration Plan", self.data_migration_plan)
		return self.plan
		...
		if hasattr(self, "mappings"):
			for m in self.mappings:
				if m.name == mapping_name:
					return m
		return finergy.get_doc("Data Migration Mapping", mapping_name)
	...
	def get_next_mapping_name(self):
		...
		raise finergy.ValidationError("Mapping Broken")
	...
	def get_data(self, filters):
		...
		for d in doclist:
			doc = finergy.get_doc(mapping.local_doctype, d["name"])
			data.append(doc)
		return data

	def get_new_local_data(self):
		"""Fetch newly inserted local data using `finergy.get_all`. Used during Push"""
		mapping = self.get_mapping(self.current_mapping)
	...
	def get_updated_local_data(self):
		"""Fetch local updated data using `finergy.get_all`. Used during Push"""
		mapping = self.get_mapping(self.current_mapping)
		filters = mapping.get_filters() or {}
	...
	def get_deleted_local_data(self):
		"""Fetch local deleted data using `finergy.get_all`. Used during Push"""
		mapping = self.get_mapping(self.current_mapping)
		filters = self.get_last_modified_condition()
		filters.update({"deleted_doctype": mapping.local_doctype})

		data = finergy.get_all("Deleted Document", fields=["name", "data"], filters=filters)
	...
	def get_connection(self):
		if not hasattr(self, "connection"):
			self.connection = finergy.get_doc(
...
def update_local_doc(mapping, remote_doc, migration_id_value):
	try:
		...
		docname = finergy.db.get_value(
			mapping.local_doctype, filters={mapping.migration_id_field: migration_id_value}
		)

		doc = finergy.get_doc(mapping.local_doctype, docname)
...
def local_doc_exists(mapping, migration_id_value):
	return finergy.db.exists(mapping.local_doctype, {mapping.migration_id_field: migration_id_value})
