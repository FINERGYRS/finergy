...
from finergy.modules.export_file import create_init_py

from .connectors.base import BaseConnection
from .connectors.finergy_connection import FinergyConnection


class DataMigrationConnector(Document):
	def validate(self):
		if not (self.python_module or self.connector_type):
			finergy.throw(_("Enter python module or select connector type"))

		if self.python_module:
			try:
				get_connection_class(self.python_module)
			except Exception:
				finergy.throw(finergy._("Invalid module path"))
	...
	def get_connection(self):
		if self.python_module:
			...
			self.connection = FinergyConnection(self)
...
def create_new_connection(module, connection_name):
	if not finergy.conf.get("developer_mode"):
		finergy.msgprint(_("Please enable developer mode to create new connection"))
		return
	# create folder
	module_path = finergy.get_module_path(module)
	connectors_folder = os.path.join(module_path, "connectors")
	finergy.create_folder(connectors_folder)

	# create init py
	create_init_py(module_path, "connectors", "")

	connection_class = connection_name.replace(" ", "")
	file_name = finergy.scrub(connection_name) + ".py"
...
def get_connection_class(python_module):
	...
	classname = finergy.unscrub(filename).replace(" ", "")
	module = finergy.get_module(python_module)
...
from finergy.data_migration.doctype.data_migration_connector.connectors.base import BaseConnection
