...
class DataMigrationMapping(Document):
	def get_filters(self):
		if self.condition:
			return finergy.safe_eval(self.condition, get_safe_globals())
	...
	def get_fields(self):
		...
		if finergy.db.has_column(self.local_doctype, self.migration_id_field):
...
def get_mapped_child_records(mapping_name, child_docs):
	mapped_child_docs = []
	mapping = finergy.get_doc("Data Migration Mapping", mapping_name)
...
def get_value_from_fieldname(field_map, fieldname_field, doc):
	...
	if field_name.startswith("eval:"):
		value = finergy.safe_eval(field_name[5:], get_safe_globals())
