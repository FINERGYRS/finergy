...
def run_all(skip_failing=False):
	...
	executed = [p[0] for p in finergy.db.sql("""select patch from `tabPatch Log`""")]
	...
	for patch in finergy.flags.final_patches:
...
def get_all_patches():
	...
	for app in finergy.get_installed_apps():
		...
		if app == "webnotes":
			app = "finergy"
		patches.extend(finergy.get_file_items(finergy.get_pymodule_path(app, "patches.txt")))
...
def reload_doc(args):
	...
	run_single(method=finergy.modules.reload_doc, methodargs=args)
	...
	try:
		log(
			"Executing {patch} in {site} ({db})".format(
				patch=patchmodule or str(methodargs), site=finergy.local.site, db=finergy.db.cur_db_name
			)
		)
		if patchmodule:
			if patchmodule.startswith("finally:"):
				# run run patch at the end
				finergy.flags.final_patches.append(patchmodule)
			else:
				if patchmodule.startswith("execute:"):
					exec(patchmodule.split("execute:")[1], globals())
				else:
					finergy.get_attr(patchmodule.split()[0] + ".execute")()
...
def update_patch_log(patchmodule):
	...
	finergy.get_doc({"doctype": "Patch Log", "patch": patchmodule}).insert(ignore_permissions=True)
...
def executed(patchmodule):
	...
	done = finergy.db.get_value("Patch Log", {"patch": patchmodule})
	# if done:
	# 	print "Patch %s already executed in %s" % (patchmodule, finergy.db.cur_db_name)
	return done


def block_user(block, msg=None):
	"""stop/start execution till patch is run"""
	finergy.local.flags.in_patch = block
	finergy.db.begin()
	if not msg:
		msg = "Patches are being executed in the system. Please try again in a few moments."
	finergy.db.set_global("__session_status", block and "stop" or None)
	finergy.db.set_global("__session_status_message", block and msg or None)
	finergy.db.commit()


def check_session_stopped():
	if finergy.db.get_global("__session_status") == "stop":
		finergy.msgprint(finergy.db.get_global("__session_status_message"))
