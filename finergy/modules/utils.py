...
def export_module_json(doc, is_standard, module):
	...
	if not finergy.flags.in_import and getattr(finergy.get_conf(), "developer_mode", 0) and is_standard:
		...
		path = os.path.join(
			finergy.get_module_path(module), scrub(doc.doctype), scrub(doc.name), scrub(doc.name)
		)
...
def get_doc_module(module, doctype, name):
	...
	module_name = "{app}.{module}.{doctype}.{name}.{name}".format(
		app=finergy.local.module_app[scrub(module)],
		doctype=scrub(doctype),
		module=scrub(module),
		name=scrub(name),
	)
	return finergy.get_module(module_name)
...
def export_customizations(module, doctype, sync_on_migrate=0, with_permissions=0):
	...
	if not finergy.get_conf().developer_mode:
	...
	def add(_doctype):
		custom["custom_fields"] += finergy.get_all("Custom Field", fields="*", filters={"dt": _doctype})
		custom["property_setters"] += finergy.get_all(
	...
	if custom["custom_fields"] or custom["property_setters"] or custom["custom_perms"]:
		...
		with open(path, "w") as f:
			f.write(finergy.as_json(custom))

		finergy.msgprint(_("Customizations for <b>{0}</b> exported to:<br>{1}").format(doctype, path))
...
def sync_customizations(app=None):
	...
	for app_name in apps:
		for module_name in finergy.local.app_modules.get(app_name) or []:
			folder = finergy.get_app_path(app_name, module_name, "custom")
...
def sync_customizations_for_doctype(data, folder):
	...
	from finergy.core.doctype.doctype.doctype import validate_fields_for_doctype
	...
	def sync(key, custom_doctype, doctype_fieldname):
		...
		def sync_single_doctype(doc_type):
			...
			if custom_doctype != "Custom Field":
				...
				for d in data[key]:
					field = finergy.db.get_value("Custom Field", {"dt": doc_type, "fieldname": d["fieldname"]})
					if not field:
						d["owner"] = "Administrator"
						_insert(d)
					else:
						custom_field = finergy.get_doc("Custom Field", field)
		...
		for doc_type in doctypes:
			...
			if doc_type == doctype or not os.path.exists(
				os.path.join(folder, finergy.scrub(doc_type) + ".json")
			):
	...
	if update_schema and not finergy.db.get_value("DocType", doctype, "issingle"):
		finergy.db.updatedb(doctype)


def scrub(txt):
	return finergy.scrub(txt)
...
def get_module_path(module):
	...
	return finergy.get_module_path(module)
...
def reload_doc(module, dt=None, dn=None, force=False, reset_permissions=False):
	from finergy.modules.import_file import import_files
...
def export_doc(doctype, name, module=None):
	...
	from finergy.modules.export_file import write_document_file

	print(doctype, name)

	if not module:
		module = finergy.db.get_value("DocType", name, "module")
	write_document_file(finergy.get_doc(doctype, name), module)
...
def get_doctype_module(doctype):
	...
	def make_modules_dict():
		return dict(finergy.db.sql("select name, module from tabDocType"))

	return finergy.cache().get_value("doctype_modules", make_modules_dict)[doctype]
...
def load_doctype_module(doctype, module=None, prefix="", suffix=""):
	...
	try:
		if key not in doctype_python_modules:
			doctype_python_modules[key] = finergy.get_module(module_name)
...
def get_module_app(module):
	return finergy.local.module_app[scrub(module)]


def get_app_publisher(module):
	app = finergy.local.module_app[scrub(module)]
	if not app:
		finergy.throw(_("App not found"))
	app_publisher = finergy.get_hooks(hook="app_publisher", app_name=app)[0]
...
def make_boilerplate(template, doc, opts=None):
	...
	if not os.path.exists(target_file_path):
		...
		base_class = "Document"
		base_class_import = "from finergy.model.document import Document"
		if doc.get("is_tree"):
			base_class = "NestedSet"
			base_class_import = "from finergy.utils.nestedset import NestedSet"
	...
	def get_stats(self, args):
		...
		with open(target_file_path, "w") as target:
			with open(
				...
				target.write(
					finergy.as_unicode(
						finergy.utils.cstr(source.read()).format(
							app_publisher=app_publisher,
							year=finergy.utils.nowdate()[:4],
