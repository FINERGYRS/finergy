...
from finergy.utils import get_datetime, now
...
def import_file_by_path(
	...
	"""
	finergy.flags.dt = finergy.flags.dt or []
	try:
	...
	if docs:
		...
		for doc in docs:
			...
			db_modified_timestamp = finergy.db.get_value(doc["doctype"], doc["name"], "modified")
			...
			if not force or db_modified_timestamp:
				try:
					stored_hash = finergy.db.get_value(doc["doctype"], doc["name"], "migration_hash")
				except Exception:
					finergy.flags.dt += [doc["doctype"]]
					stored_hash = None
			...
			if doc["doctype"] == "DocType":
				...
				finergy.qb.update(doctype_table).set(doctype_table.migration_hash, calculated_hash).where(
...
def is_timestamp_changed(doc):
	...
	db_modified = finergy.db.get_value(doc["doctype"], doc["name"], "modified")
...
def update_modified(original_modified, doc):
	...
	if doc["doctype"] == doc["name"] and doc["name"] != "DocType":
		...
		finergy.qb.update(singles_table).set(singles_table.value, original_modified).where(
		...
		finergy.qb.update(doctype_table).set(doctype_table.modified, original_modified).where(
			doctype_table.name == doc["name"]
		).run()
...
def import_doc(
	...
	if finergy.db.exists(doc.doctype, doc.name):

		old_doc = finergy.get_doc(doc.doctype, doc.name)
		...
		# delete old
		finergy.delete_doc(doc.doctype, doc.name, force=1, ignore_doctypes=ignore, for_reload=True)
