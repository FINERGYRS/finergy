...
finergy.ui.form.on('Web Page', {
	layout: function(frm) {
		if (frm.is_new()) {
			if (frm.doc.insert_code) {
				if (!frm.doc.javascript) {
					frm.set_value('javascript', `finergy.ready(() => {\n\t\n});`);
				}
			}
		}
	},
	...
	validate: function(frm) {
		if (frm.doc.title && !frm.doc.route) {
			frm.set_value('route', finergy.scrub(frm.doc.title, '-'));
		}
	},
	...
	published: function(frm) {
		...
		if (!frm.doc.published && frm.doc.end_date) {
			var end_date = finergy.datetime.str_to_obj(finergy.datetime.now_datetime());
...
finergy.ui.form.on("Web Page Block", {
	edit_values(frm, cdt, cdn) {
		...
		open_web_template_values_editor(row.web_template, values)
			.then(new_values => {
				finergy.model.set_value(cdt, cdn, "web_template_values", JSON.stringify(new_values));
			});
	},
});

finergy.tour['Web Page'] = [
	{
		fieldname: "title",
	...
	},
	{
		...
		description: `${__('You can select one from the following,')} <br>
					<ul>
						...
						<li><b>${__('Page Builder')}</b>: ${__('Finergy page builder using components')}</li>
					</ul>
					`
	},
	{
