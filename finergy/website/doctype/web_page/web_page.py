...
from finergy.utils import get_datetime, now, quoted, strip_html
from finergy.utils.jinja import render_template
from finergy.utils.safe_exec import safe_exec
from finergy.website.doctype.website_slideshow.website_slideshow import get_slideshow
...
class WebPage(WebsiteGenerator):
	...
	def get_context(self, context):
		...
		if self.context_script:
			_locals = dict(context=finergy._dict())
	...
	def validate_dates(self):
		if self.end_date:
			...
			if self.published and now() > self.end_date:
				...
				finergy.msgprint(_("Clearing end date, as it cannot be in the past for published pages."))


def check_publish_status():
	# called via daily scheduler
	web_pages = finergy.get_all("Web Page", fields=["name", "published", "start_date", "end_date"])
	...
	for page in web_pages:
		...
		if page.published:
			...
			if (start_date and now_date < start_date) or (end_date and now_date > end_date):
				finergy.db.set_value("Web Page", page.name, "published", 0)
		else:
			# Publish pages that are inside the set date ranges
			if start_date:
				if not end_date or (end_date and now_date < end_date):
					finergy.db.set_value("Web Page", page.name, "published", 1)


def check_broken_links():
	cnt = 0
	for p in finergy.db.sql("select name, main_section from `tabWeb Page`", as_dict=True):
		for link in re.findall("href=[\"']([^\"']*)[\"']", p.main_section):
			if link.startswith("http"):
				try:
					res = requests.get(link)
				except requests.exceptions.SSLError:
					res = finergy._dict({"status_code": "SSL Error"})
				except requests.exceptions.ConnectionError:
					res = finergy._dict({"status_code": "Connection Error"})
...
def get_web_blocks_html(blocks):
	...
	out = finergy._dict(html="", scripts=[], styles=[])
	extracted_scripts = []
	extracted_styles = []
	for block in blocks:
		web_template = finergy.get_cached_doc("Web Template", block.web_template)
		rendered_html = finergy.render_template(
