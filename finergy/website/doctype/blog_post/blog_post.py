...
from finergy.website.utils import find_first_image, get_comment_list, get_html_content_based_on_type
from finergy.website.website_generator import WebsiteGenerator


class BlogPost(WebsiteGenerator):
	website = finergy._dict(route="blog", order_by="published_on desc")

	@finergy.whitelist()
	def make_route(self):
		if not self.route:
			return (
				finergy.db.get_value("Blog Category", self.blog_category, "route")
				+ "/"
	...
	def validate(self):
		...
		if self.featured:
			if not self.meta_image:
				finergy.throw(_("A featured post must have a cover image"))
			self.reset_featured_for_other_blogs()

		self.set_read_time()

	def reset_featured_for_other_blogs(self):
		all_posts = finergy.get_all("Blog Post", {"featured": 1})
		for post in all_posts:
			finergy.db.set_value("Blog Post", post.name, "featured", 0)
	...
	def get_context(self, context):
		...
		context.enable_cta = not self.hide_cta and finergy.db.get_single_value(
			"Blog Settings", "show_cta_in_blog", cache=True
		)

		if self.blogger:
			context.blogger_info = finergy.get_doc("Blogger", self.blogger).as_dict()
		...
		context.category = finergy.db.get_value(
	...
	def fetch_cta(self):
		if finergy.db.get_single_value("Blog Settings", "show_cta_in_blog", cache=True):
			blog_settings = finergy.get_cached_doc("Blog Settings")
	...
	def fetch_social_links_info(self):
		if not finergy.db.get_single_value("Blog Settings", "enable_social_sharing", cache=True):
			return []

		url = finergy.local.site + "/" + self.route
...
def get_list_context(context=None):
	...
	category = finergy.utils.escape_html(
		finergy.local.form_dict.blog_category or finergy.local.form_dict.category
	)
	if category:
	...
	elif finergy.local.form_dict.blogger:
		blogger = finergy.db.get_value("Blogger", {"name": finergy.local.form_dict.blogger}, "full_name")
		list_context.sub_title = _("Posts by {0}").format(blogger)
		list_context.title = blogger

	elif finergy.local.form_dict.txt:
		list_context.sub_title = _('Filtered by "{0}"').format(sanitize_html(finergy.local.form_dict.txt))
	...
	list_context.update(finergy.get_doc("Blog Settings").as_dict(no_default_fields=True))
...
def clear_blog_cache():
	for blog in finergy.db.sql_list(
		"""select route from
...
def get_blog_category(route):
	return finergy.db.get_value("Blog Category", {"name": route}, "title") or route
...
def get_blog_list(
	...
	conditions = []
	category = filters.blog_category or finergy.utils.escape_html(
		finergy.local.form_dict.blog_category or finergy.local.form_dict.category
	)
	if filters:
		if filters.blogger:
			conditions.append("t1.blogger=%s" % finergy.db.escape(filters.blogger))
	if category:
		conditions.append("t1.blog_category=%s" % finergy.db.escape(category))

	if txt:
		conditions.append(
			'(t1.content like {0} or t1.title like {0}")'.format(finergy.db.escape("%" + txt + "%"))
		)

	if conditions:
		finergy.local.no_cache = 1
	...
	posts = finergy.db.sql(query, as_dict=1)
	...
	for post in posts:
		...
		post.category = finergy.db.get_value(
