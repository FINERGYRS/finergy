...
from finergy.utils import random_string, set_request
from finergy.website.doctype.blog_post.blog_post import get_blog_list
...
class TestBlogPost(unittest.TestCase):
	...
	def test_generator_not_found(self):
		pages = finergy.get_all("Blog Post", fields=["name", "route"], filters={"published": 0}, limit=1)

		finergy.db.set_value("Blog Post", pages[0].name, "route", "test-route-000")
	...
	def test_category_link(self):
		...
		blog_page_html = finergy.safe_decode(blog_page_response.get_data())
		...
		category_page_html = finergy.safe_decode(category_page_response.get_data())

		# Category page should contain the blog post title
		self.assertIn(blog.title, category_page_html)

		# Cleanup afterwords
		finergy.delete_doc("Blog Post", blog.name)
		finergy.delete_doc("Blog Category", blog.blog_category)
	...
	def test_blog_pagination(self):
		...
		filters = finergy._dict({"blog_category": scrub(category_title)})
		...
		for blog in blogs:
			finergy.delete_doc(blog.doctype, blog.name)
		finergy.delete_doc("Blog Category", blogs[0].blog_category)
...
def make_test_blog(category_title="Test Blog Category"):
	...
	if not finergy.db.exists("Blog Category", category_name):
		finergy.get_doc(dict(doctype="Blog Category", title=category_title)).insert()
	if not finergy.db.exists("Blogger", "test-blogger"):
		finergy.get_doc(
			dict(doctype="Blogger", short_name="test-blogger", full_name="Test Blogger")
		).insert()

	test_blog = finergy.get_doc(
		dict(
			doctype="Blog Post",
