...
class PortalSettings(Document):
	...
	def sync_menu(self):
		...
		dirty = False
		for item in finergy.get_hooks("standard_portal_menu_items"):
			if item.get("role") and not finergy.db.exists("Role", item.get("role")):
				finergy.get_doc({"doctype": "Role", "role_name": item.get("role"), "desk_access": 0}).insert()
