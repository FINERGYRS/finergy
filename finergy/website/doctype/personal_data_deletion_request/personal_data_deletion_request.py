...
class PersonalDataDeletionRequest(Document):
	def __init__(self, *args, **kwargs):
		...
		self.user_data_fields = finergy.get_hooks("user_data_fields")
	...
	def autoname(self):
		...
		domain = finergy.local.site.replace("_", "-")
		...
		finergy.utils.validate_email_address(self.email, throw=True)

	def after_insert(self):
		self.send_verification_mail()

	def generate_url_for_confirmation(self):
		params = {"email": self.email, "name": self.name, "host_name": finergy.local.site}
		api = finergy.utils.get_url(
			"/api/method/finergy.website.doctype.personal_data_deletion_request"
	...
	def disable_user(self):
		user = finergy.get_doc("User", self.email)
		user.enabled = False
		user.save()
	...
	def notify_system_managers(self):
		...
		finergy.sendmail(
			...
			args={"user": self.email, "url": finergy.utils.get_url(self.get_url())},
	...
	def validate_data_anonymization(self):
		...
		if self.status != "Pending Approval":
			finergy.throw(_("This request has not yet been approved by the user."))
	...
	def generate_anonymization_dict(self, ref):
		...
		meta = finergy.get_meta(ref["doctype"])
	...
	def redact_doc(self, doc, ref):
		...
		if ref.get("rename") and doc["name"] != self.anon:
			finergy.rename_doc(ref["doctype"], doc["name"], self.anon, force=True, show_alert=False)
	...
	def _anonymize_data(self, email=None, anon=None, set_data=True, commit=False):
		...
		finergy.rename_doc("User", email, anon, force=True, show_alert=False)
	...
	def __redact_partial_match_data(self, doctype):
		...
		for df in finergy.get_meta(doctype["doctype"]).fields:
...
def confirm_deletion(email, name, host_name):
	...
	doc = finergy.get_doc("Personal Data Deletion Request", name)
	host_name = finergy.local.site
