...
import finergy
from finergy.website.doctype.personal_data_deletion_request.personal_data_deletion_request import (
	remove_unverified_record,
)
from finergy.website.doctype.personal_data_download_request.test_personal_data_download_request import (
...
class TestPersonalDataDeletionRequest(unittest.TestCase):
	def setUp(self):
		...
		self.delete_request = finergy.get_doc(
	...
	def test_delete_request(self):
		email_queue = finergy.get_all("Email Queue", fields=["*"], order_by="creation desc", limit=1)
	...
	def test_unverified_record_removal(self):
		...
		self.delete_request.save()
		remove_unverified_record()
		self.assertFalse(finergy.db.exists("Personal Data Deletion Request", self.delete_request.name))
