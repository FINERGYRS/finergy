...
def authorize_access(reauthorize=None):
	...
	redirect_uri = (
		...
		+ "?cmd=finergy.website.doctype.website_settings.google_indexing.google_callback"
	)
	...
	if not website_settings.indexing_authorization_code or reauthorize:
		...
		try:
			...
			finergy.local.response["location"] = "/app/Form/{0}".format(quote("Website Settings"))

			finergy.msgprint(_("Google Indexing has been configured."))
...
def google_callback(code=None):
	...
	finergy.db.set_value("Website Settings", None, "indexing_authorization_code", code)
	finergy.db.commit()
...
def get_google_indexing_object():
	...
	google_settings = finergy.get_doc("Google Settings")
	account = finergy.get_doc("Website Settings")
...
def publish_site(url, operation_type="URL_UPDATED"):
	...
	try:
		...
		finergy.log_error(message=e, title="API Indexing Issue")
