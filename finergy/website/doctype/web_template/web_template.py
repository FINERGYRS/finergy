...
from finergy.modules.export_file import get_module_path, scrub_dt_dn, write_document_file
from finergy.website.render import clear_cache


class WebTemplate(Document):
	def validate(self):
		if self.standard and not (finergy.conf.developer_mode or finergy.flags.in_patch):
			finergy.throw(_("Enable developer mode to create a standard Web Template"))

		for field in self.fields:
			if not field.fieldname:
				field.fieldname = finergy.scrub(field.label)
	...
	def on_trash(self):
		if finergy.conf.developer_mode and self.standard:
	...
	def get_template_path(self):
		...
		file_name = finergy.scrub(self.name) + ".html"
	...
	def render(self, values=None):
		if not values:
			values = {}
		values = finergy.parse_json(values)
		values.update({"values": values})
		template = self.get_template(self.standard)

		return finergy.render_template(template, values)
