...
from finergy.core.doctype.file.file import get_max_file_size, remove_file_by_url
from finergy.custom.doctype.customize_form.customize_form import docfield_properties
from finergy.desk.form.meta import get_code_files_via_hooks
from finergy.integrations.utils import get_payment_gateway_controller
from finergy.modules.utils import export_module_json, get_doc_module
from finergy.rate_limiter import rate_limit
from finergy.utils import cstr
from finergy.website.utils import get_comment_list
from finergy.website.website_generator import WebsiteGenerator


class WebForm(WebsiteGenerator):
	website = finergy._dict(no_cache=1)

	def onload(self):
		super(WebForm, self).onload()
		if self.is_standard and not finergy.conf.developer_mode:
	...
	def validate(self):
		...
		if not self.module:
			self.module = finergy.db.get_value("DocType", self.doc_type, "module")

		if (
			not (
				finergy.flags.in_install
				or finergy.flags.in_patch
				or finergy.flags.in_test
				or finergy.flags.in_fixtures
			)
			and self.is_standard
			and not finergy.conf.developer_mode
		):
			finergy.throw(_("You need to be in developer mode to edit a Standard Web Form"))

		if not finergy.flags.in_import:
	...
	def validate_fields(self):
		...
		missing = []
		meta = finergy.get_meta(self.doc_type)
		...
		if missing:
			finergy.throw(_("Following fields are missing:") + "<br>" + "<br>".join(missing))

	def validate_payment_amount(self):
		if self.amount_based_on_field and not self.amount_field:
			finergy.throw(_("Please select a Amount Field."))
		elif not self.amount_based_on_field and not self.amount > 0:
			finergy.throw(_("Amount must be greater than 0."))
...
"""
	...
	def get_context(self, context):
		...
		doc, delimeter = make_route_string(finergy.form_dict)
		context.doc = doc
		context.delimeter = delimeter

		# check permissions
		if finergy.session.user == "Guest" and finergy.form_dict.name:
			finergy.throw(
				_("You need to be logged in to access this {0}.").format(self.doc_type), finergy.PermissionError
			)

		if finergy.form_dict.name and not self.has_web_form_permission(
			self.doc_type, finergy.form_dict.name
		):
			finergy.throw(
				_("You don't have the permissions to access this document"), finergy.PermissionError
			)
		...
		if not finergy.session.user == "Guest":
			if self.allow_edit:
				if self.allow_multiple:
					if not finergy.form_dict.name and not finergy.form_dict.new:
						# list data is queried via JS
						context.is_list = True
				else:
					if finergy.session.user != "Guest" and not finergy.form_dict.name:
						finergy.form_dict.name = finergy.db.get_value(
							self.doc_type, {"owner": finergy.session.user}, "name"
						)

					if not finergy.form_dict.name:
						# only a single doc allowed and no existing doc, hence new
						finergy.form_dict.new = 1

		if finergy.form_dict.is_list:
			context.is_list = True
		...
		if self.breadcrumbs:
			context.parents = finergy.safe_eval(self.breadcrumbs, {"_": _})

		context.has_header = (finergy.form_dict.name or finergy.form_dict.new) and (
			finergy.session.user != "Guest" or not self.login_required
		)

		if context.success_message:
			context.success_message = finergy.db.escape(context.success_message.replace("\n", "<br>")).strip(
				"'"
			)
	...
	def load_translations(self, context):
		translated_messages = finergy.translate.get_dict("doctype", self.doc_type)
		# Sr is not added by default, had to be added manually
		translated_messages["Sr"] = _("Sr")
		context.translated_messages = finergy.as_json(translated_messages)

	def load_document(self, context):
		"""Load document `doc` and `layout` properties for template"""
		if finergy.form_dict.name or finergy.form_dict.new:
			context.layout = self.get_layout()
			context.parents = [{"route": self.route, "label": _(self.title)}]

		if finergy.form_dict.name:
			context.doc = finergy.get_doc(self.doc_type, finergy.form_dict.name)
			...
			if self.show_attachments:
				context.attachments = finergy.get_all(
					"File",
					filters={
	...
	def get_payment_gateway_url(self, doc):
		if self.accept_payment:
			...
			if amount is None or Decimal(amount) <= 0:
				return finergy.utils.get_url(self.success_url or self.route)
			...
			payment_details = {
				...
				"payer_email": finergy.session.user,
				"payer_name": finergy.utils.get_fullname(finergy.session.user),
				"order_id": doc.name,
				"currency": self.currency,
				"redirect_to": finergy.utils.get_url(self.success_url or self.route),
			}
	...
	def add_custom_context_and_script(self, context):
		...
		if self.web_form_module:
			...
			if os.path.exists(js_path):
				script = finergy.render_template(open(js_path, "r").read(), context)

				for path in get_code_files_via_hooks("webform_include_js", context.doc_type):
					custom_js = finergy.render_template(open(path, "r").read(), context)
	...
	def has_web_form_permission(self, doctype, name, ptype="read"):
		...
		if self.apply_document_permissions:
			return finergy.get_doc(doctype, name).has_permission()

		# owner matches
		elif finergy.db.get_value(doctype, name, "owner") == finergy.session.user:
			return True

		elif finergy.has_website_permission(name, ptype=ptype, doctype=doctype):
			return True
...
def accept(web_form, data, docname=None, for_payment=False):
	"""Save the web form"""
	data = finergy._dict(json.loads(data))
	for_payment = finergy.parse_json(for_payment)

	files = []
	files_to_delete = []

	web_form = finergy.get_doc("Web Form", web_form)

	if data.name and not web_form.allow_edit:
		finergy.throw(_("You are not allowed to update this Web Form Document"))

	finergy.flags.in_web_form = True
	meta = finergy.get_meta(data.doctype)

	if docname:
		# update
		doc = finergy.get_doc(data.doctype, docname)
	else:
		# insert
		doc = finergy.new_doc(data.doctype)
	...
	if doc.name:
		...
		# insert
		if web_form.login_required and finergy.session.user == "Guest":
			finergy.throw(_("You must login to submit this form"))
	...
	finergy.flags.web_form_doc = doc
...
def delete(web_form_name, docname):
	web_form = finergy.get_doc("Web Form", web_form_name)

	owner = finergy.db.get_value(web_form.doc_type, docname, "owner")
	if finergy.session.user == owner and web_form.allow_delete:
		finergy.delete_doc(web_form.doc_type, docname, ignore_permissions=True)
	else:
		raise finergy.PermissionError("Not Allowed")
...
def get_form_data(doctype, docname=None, web_form_name=None):
	...
	if finergy.session.user != "Guest" and not docname and not web_form.allow_multiple:
		docname = finergy.db.get_value(doctype, {"owner": finergy.session.user}, "name")
...
def get_link_options(web_form_name, doctype, allow_read_on_all_link_options=False):
	web_form_doc = finergy.get_doc("Web Form", web_form_name)
	doctype_validated = False
	limited_to_user = False
	if web_form_doc.login_required:
		# check if finergy session user is not guest or admin
		if finergy.session.user != "Guest":
	...
	if doctype_validated:
		...
		if limited_to_user:
			link_options = "\n".join(
				[doc.name for doc in finergy.get_all(doctype, filters={"owner": finergy.session.user})]
			)
		else:
			link_options = "\n".join([doc.name for doc in finergy.get_all(doctype)])

		return link_options

	else:
		raise finergy.PermissionError("Not Allowed, {0}".format(doctype))
