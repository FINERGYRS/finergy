from __future__ import unicode_literals

import re

import finergy


def resolve_redirect(path):
	"""
	Resolve redirects from hooks

	Example:

	        website_redirect = [
	                # absolute location
	                {"source": "/from", "target": "https://mysite/from"},

	                # relative location
	                {"source": "/from", "target": "/main"},

	                # use regex
	                {"source": r"/from/(.*)", "target": r"/main/\1"}
	                # use r as a string prefix if you use regex groups or want to escape any string literal
	        ]
	"""
	redirects = finergy.get_hooks("website_redirects")
	redirects += finergy.db.get_all("Website Route Redirect", ["source", "target"])

	if not redirects:
		return

	redirect_to = finergy.cache().hget("website_redirects", path)

	if redirect_to:
		finergy.flags.redirect_location = redirect_to
		raise finergy.Redirect

	for rule in redirects:
		pattern = rule["source"].strip("/ ") + "$"
		if re.match(pattern, path):
			redirect_to = re.sub(pattern, rule["target"], path)
			finergy.flags.redirect_location = redirect_to
			finergy.cache().hset("website_redirects", path, redirect_to)
			raise finergy.Redirect
