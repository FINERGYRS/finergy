...
from finergy.website.doctype.website_settings.website_settings import get_website_settings
from finergy.website.router import get_page_context
...
def get_context(path, args=None):
	...
	if hasattr(finergy.local, "request"):
		# for <body data-path=""> (remove leading slash)
		# path could be overriden in render.resolve_from_map
		context["path"] = finergy.local.request.path.strip("/ ")
	else:
		context["path"] = path

	context.canonical = finergy.utils.get_url(finergy.utils.escape_html(context.path))
	context.route = context.path
	context = build_context(context)

	# set using finergy.respond_as_web_page
	if hasattr(finergy.local, "response") and finergy.local.response.get("context"):
		context.update(finergy.local.response.context)

	# to be able to inspect the context dict
	# Use the macro "inspect" from macros.html
	context._context_dict = context

	context.developer_mode = finergy.conf.developer_mode

	return context


def update_controller_context(context, controller):
	module = finergy.get_module(controller)
	...
	if module:
		...
		if hasattr(module, "get_context"):
			...
			try:
				...
				if ret:
					context.update(ret)
			except (finergy.PermissionError, finergy.PageDoesNotExistError, finergy.Redirect):
				raise
			except Exception:
				if not any([finergy.flags.in_migrate, finergy.flags.in_website_search_build]):
...
def build_context(context):
	...
	context.update(finergy.local.conf.get("website_context") or {})
	...
	if context.doc:
		...
		context_controller_hooks = finergy.get_hooks("extend_website_page_controller_context") or {}
	...
	if not context.base_template_path:
		app_base = finergy.get_hooks("base_template")
	...
	update_website_context = finergy.get_hooks("update_website_context")
	for method in update_website_context:
		values = finergy.get_attr(method)(context)
		if values:
...
def load_sidebar(context, sidebar_json_path):
	with open(sidebar_json_path, "r") as sidebarfile:
		try:
			...
			finergy.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def add_sidebar_and_breadcrumbs(context):
	...
	from finergy.website.router import get_page_info_from_template
	...
	if context.show_sidebar:
		...
		if context.basepath:
			hooks = finergy.get_hooks("look_for_sidebar_json")
...
def add_sidebar_data(context):
	...
	from finergy.utils.user import get_fullname_and_avatar

	if context.show_sidebar and context.website_sidebar:
		context.sidebar_items = finergy.get_all(
	...
	if not context.sidebar_items:
		sidebar_items = finergy.cache().hget("portal_menu_items", finergy.session.user)
		if sidebar_items == None:
			sidebar_items = []
			roles = finergy.get_roles()
			portal_settings = finergy.get_doc("Portal Settings", "Portal Settings")
			...
			items_via_hooks = finergy.get_hooks("portal_menu_items")
			if items_via_hooks:
				for i in items_via_hooks:
					i["enabled"] = 1
				add_items(sidebar_items, items_via_hooks)

			finergy.cache().hset("portal_menu_items", finergy.session.user, sidebar_items)

		context.sidebar_items = sidebar_items

	info = get_fullname_and_avatar(finergy.session.user)
...
def add_metatags(context):
	tags = finergy._dict(context.get("metatags") or {})
	...
	if image:
		tags["og:image"] = tags["twitter:image"] = tags["image"] = finergy.utils.get_url(image)
		tags["twitter:card"] = "summary_large_image"

	if "author" not in tags and context.author:
		tags["author"] = context.author

	tags["og:url"] = tags["url"] = finergy.utils.get_url(context.path)
	...
	tags["language"] = finergy.local.lang or "en"
	...
	route_exists = (
		route and not route.endswith((".js", ".css")) and finergy.db.exists("Website Route Meta", route)
	)

	if route_exists:
		website_route_meta = finergy.get_doc("Website Route Meta", route)
