...
def can_cache(no_cache=False):
	if finergy.conf.disable_website_cache or finergy.conf.developer_mode:
		return False
	if getattr(finergy.local, "no_cache", False):
		return False
	return not no_cache
...
def get_comment_list(doctype, name):
	comments = finergy.get_all(
		...
		or_filters=[["owner", "=", finergy.session.user], ["published", "=", 1]],
	)

	communications = finergy.get_all(
		"Communication",
		fields=[
			"name",
			"creation",
		...
		or_filters=[
			["recipients", "like", "%{0}%".format(finergy.session.user)],
			["cc", "like", "%{0}%".format(finergy.session.user)],
			["bcc", "like", "%{0}%".format(finergy.session.user)],
		],
	)
...
def get_home_page():
	if finergy.local.flags.home_page and not finergy.flags.in_test:
		return finergy.local.flags.home_page
	...
	def _get_home_page():
		...
		if finergy.session.user != "Guest":
			# by role
			for role in finergy.get_roles():
				home_page = finergy.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = finergy.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = finergy.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if finergy.session.user == "Guest" else "me"
	...
	return finergy.cache().hget("home_page", finergy.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = finergy.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = finergy.get_attr(home_page_method[-1])(finergy.session.user)
	elif finergy.get_hooks("website_user_home_page"):
		home_page = finergy.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = finergy.get_hooks("role_home_page")
		if role_home_page:
			for role in finergy.get_roles():
	...
	if not home_page:
		home_page = finergy.get_hooks("home_page")
		if home_page:
...
def is_signup_enabled():
	if getattr(finergy.local, "is_signup_enabled", None) is None:
		finergy.local.is_signup_enabled = True
		if finergy.utils.cint(
			finergy.db.get_value("Website Settings", "Website Settings", "disable_signup")
		):
			finergy.local.is_signup_enabled = False

	return finergy.local.is_signup_enabled


def is_signup_disabled():
	return finergy.db.get_single_value("Website Settings", "disable_signup", True)
...
def get_toc(route, url_prefix=None, app=None):
	...
	return finergy.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from finergy.website.router import get_pages

	if not finergy.local.flags.children_map:
		...
		children_map = finergy.cache().get_value("website_full_index", _build)

		finergy.local.flags.children_map = children_map

	return finergy.local.flags.children_map
...
def add_missing_headers():
	...
	path = finergy.get_app_path("capkpi", "docs")
	for basepath, folders, files in os.walk(path):
		for fname in files:
			if fname.endswith(".md"):
				with open(os.path.join(basepath, fname), "r") as f:
					content = finergy.as_unicode(f.read())
