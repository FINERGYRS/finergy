...
from finergy.website.utils import can_cache, delete_page_cache, extract_comment_tag, extract_title
...
def get_page_context(path):
	page_context = None
	if can_cache():
		page_context_cache = finergy.cache().hget("page_context", path) or {}
		page_context = page_context_cache.get(finergy.local.lang, None)

	if not page_context:
		page_context = make_page_context(path)
		if can_cache(page_context.no_cache):
			page_context_cache[finergy.local.lang] = page_context
			finergy.cache().hset("page_context", path, page_context_cache)
...
def make_page_context(path):
	...
	if not context:
		raise finergy.PageDoesNotExistError
	...
	context.pathname = finergy.local.path
...
def get_page_info_from_template(path):
	...
	for app in finergy.get_installed_apps(finergy_last=True):
		app_path = finergy.get_app_path(app)
...
def get_page_context_from_doctype(path):
	...
	if page_info:
		return finergy.get_doc(page_info.get("doctype"), page_info.get("name")).get_page_info()
...
def get_all_page_context_from_doctypes():
	...
	"""
	routes = finergy.cache().get_value("website_generator_routes")
	if not routes:
		routes = get_page_info_from_doctypes()
		finergy.cache().set_value("website_generator_routes", routes)
...
def evaluate_dynamic_routes(rules, path):
	...
	if hasattr(finergy.local, "request") and finergy.local.request.environ:
		urls = route_map.bind_to_environ(finergy.local.request.environ)
		try:
			...
			if args:
				...
				finergy.local.form_dict.update(args)
...
def get_pages(app=None):
	...
	def _build(app):
		...
		if app:
			...
			apps = finergy.local.flags.web_pages_apps or finergy.get_installed_apps()
	...
	return finergy.cache().get_value("website_pages", lambda: _build(app))
...
def get_pages_from_path(start, app, app_path):
	...
	if os.path.exists(start_path):
		for basepath, folders, files in os.walk(start_path):
			...
			for fname in files:
				fname = finergy.utils.cstr(fname)
				if not "." in fname:
					continue
...
def get_page_info(path, app, start, basepath=None, app_path=None, fname=None):
	...
	page_info.build_version = finergy.utils.get_build_version()
...
def setup_source(page_info):
	...
	if page_info.template.endswith((".md", ".html")):
		...
		if page_info.template.endswith(".md"):
			source = finergy.utils.md_to_html(source)
...
def get_base_template(path=None):
	...
	base_template_map = finergy.get_hooks("base_template_map") or {}
...
def load_properties_from_controller(page_info):
	...
	module = finergy.get_module(page_info.controller)
	if not module:
		return
...
def get_doctypes_with_web_view():
	...
	def _get():
		...
		doctypes = finergy.get_hooks("website_generators")
		doctypes += [
			d.name
			for d in finergy.get_all("DocType", "name, module", dict(has_web_view=1))
			if finergy.local.module_app[finergy.scrub(d.module)] in installed_apps
		]
		return doctypes

	return finergy.cache().get_value("doctypes_with_web_view", _get)


def get_start_folders():
	return finergy.local.flags.web_pages_folders or ("www", "templates/pages")
