# Copyright (c) 2013, Finergy Technologies and contributors
...
import finergy
from finergy.query_builder.functions import Coalesce, Count
...
class WebsiteAnalytics(object):
	def __init__(self, filters=None):
		self.filters = finergy._dict(filters or {})

		if not self.filters.to_date:
			self.filters.to_date = datetime.now()

		if not self.filters.from_date:
			self.filters.from_date = finergy.utils.add_days(self.filters.to_date, -7)

		if not self.filters.range:
			self.filters.range = "Daily"

		self.filters.to_date = finergy.utils.add_days(self.filters.to_date, 1)
	...
	def get_data(self):
		WebPageView = finergy.qb.DocType("Web Page View")
		count_all = Count("*").as_("count")
		case = finergy.qb.terms.Case().when(WebPageView.is_unique == "1", "1")
		count_is_unique = Count(case).as_("unique_count")

		return (
			finergy.qb.from_(WebPageView)
			...
			.orderby("count", Order=finergy.qb.desc)
		).run()
	...
	def get_chart_data(self):
		...
		self.chart_data = finergy.db.sql(query, values=values, as_dict=1)
	...
	def prepare_chart_data(self, data):
		...
		if self.filters.range == "Monthly":
			date_range = [finergy.utils.add_days(dd, 1) for dd in date_range]
