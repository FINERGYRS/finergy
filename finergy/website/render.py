...
from finergy.website.context import get_context
from finergy.website.redirect import resolve_redirect
from finergy.website.router import clear_sitemap, evaluate_dynamic_routes
from finergy.website.utils import (
	can_cache,
	delete_page_cache,
	get_home_page,
	get_next_link,
...
def render(path=None, http_status_code=None):
	"""render html page"""
	if not path:
		path = finergy.local.request.path
	...
	try:
		...
		if can_cache() and finergy.cache().hget("website_404", finergy.request.url):
			...
			try:
			...
			except finergy.PageDoesNotExistError:
				doctype, name = get_doctype_from_path(path)
				if doctype and name:
					path = "printview"
					finergy.local.form_dict.doctype = doctype
					finergy.local.form_dict.name = name
				elif doctype:
					path = "list"
					finergy.local.form_dict.doctype = doctype
				else:
					# 404s are expensive, cache them!
					finergy.cache().hset("website_404", finergy.request.url, True)
					data = render_page("404")
					http_status_code = 404
				...
				if not data:
					try:
						...
						http_status_code = http_status_code or finergy.flags.response_status_code
					except finergy.PermissionError as e:
						data, http_status_code = render_403(e, path)

			except finergy.PermissionError as e:
				data, http_status_code = render_403(e, path)

			except finergy.Redirect as e:
				raise e
	...
	except finergy.Redirect:
		return build_response(
			path,
			"",
			301,
			{
				"Location": finergy.flags.redirect_location or (finergy.local.response or {}).get("location"),
...
def is_static_file(path):
	...
	for app in finergy.get_installed_apps():
		file_path = finergy.get_app_path(app, "www") + "/" + path
		if os.path.isfile(file_path) and (extn or is_binary_file(file_path)):
			finergy.flags.file_path = file_path
			return True
...
def is_web_form(path):
	return bool(finergy.get_all("Web Form", filters={"route": path}))
...
def get_static_file_response():
	try:
		f = open(finergy.flags.file_path, "rb")
	except IOError:
		raise NotFound

	response = Response(wrap_file(finergy.local.request.environ, f), direct_passthrough=True)
	response.mimetype = mimetypes.guess_type(finergy.flags.file_path)[0] or "application/octet-stream"
	return response
...
def build_response(path, data, http_status_code, headers=None):
	...
	response.headers["X-From-Cache"] = finergy.local.response.from_cache or False
...
def render_page_by_language(path):
	translated_languages = finergy.get_hooks("translated_languages_for_website")
...
def render_page(path):
	...
	if can_cache():
		...
		page_cache = finergy.cache().hget("website_page", path)
		if page_cache and finergy.local.lang in page_cache:
			out = page_cache[finergy.local.lang]

	if out:
		finergy.local.response.from_cache = True
		return out
...
def build_page(path):
	if not getattr(finergy.local, "path", None):
		finergy.local.path = path

	context = get_context(path)

	finergy.flags.response_status_code = context.get("http_status_code") or 200

	if context.source:
		html = finergy.render_template(context.source, context)
	elif context.template:
		if path.endswith("min.js"):
			html = finergy.get_jloader().get_source(finergy.get_jenv(), context.template)[0]
		else:
			html = finergy.get_template(context.template).render(context)
	...
	# html = finergy.get_template(context.base_template_path).render(context)

	if can_cache(context.no_cache):
		page_cache = finergy.cache().hget("website_page", path) or {}
		page_cache[finergy.local.lang] = html
		finergy.cache().hset("website_page", path, page_cache)
...
def get_website_rules():
	...
	def _get():
		rules = finergy.get_hooks("website_route_rules")
		for d in finergy.get_all("DocType", "name, route", dict(has_web_view=1)):
			if d.route:
	...
	return finergy.cache().get_value("website_route_rules", _get)
...
def clear_cache(path=None):
	...
	for key in ("website_generator_routes", "website_pages", "website_full_index", "sitemap_routes"):
		finergy.cache().delete_value(key)

	finergy.cache().delete_value("website_404")
	if path:
		finergy.cache().hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		clear_sitemap()
		finergy.clear_cache("Guest")
		for key in (
			"portal_menu_items",
			"home_page",
			"website_route_rules",
	...
	for method in finergy.get_hooks("website_clear_cache"):
		finergy.get_attr(method)(path)


def render_403(e, pathname):
	finergy.local.message = cstr(e.message if six.PY2 else e)
	finergy.local.message_title = _("Not Permitted")
	finergy.local.response["context"] = dict(
...
def get_doctype_from_path(path):
	doctypes = finergy.db.sql_list("select name from tabDocType")
...
def add_csrf_token(data):
	if finergy.local.session:
		return data.replace(
			"<!-- csrf_token -->",
			'<script>finergy.csrf_token = "{0}";</script>'.format(finergy.local.session.data.csrf_token),
		)
	else:
		return data
