...
finergy.provide("website");
finergy.provide("finergy.awesome_bar_path");
window.cur_frm = null;
...
$.extend(finergy, {
	...
	add_asset_to_head(link) {
		return new Promise(resolve => {
			if (finergy._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				finergy._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function(opts) {
		...
		return $.ajax({
			...
			data: opts.args,
			dataType: "json",
			headers: { "X-Finergy-CSRF-Token": finergy.csrf_token, "X-Finergy-CMD": (opts.args && opts.args.cmd  || '') || '' },
			statusCode: opts.statusCode || {
				404: function() {
					finergy.msgprint(__("Not found"));
				},
				403: function() {
					finergy.msgprint(__("Not permitted"));
				},
				200: function(data) {
					if(opts.callback)
						opts.callback(data);
			...
			finergy.process_response(opts, data);
		});
	},
	...
	process_response: function(opts, data) {
		...
		if (data._server_messages) {
			...
			if(opts.error_msg) {
				...
				finergy.msgprint(server_messages);
			}
		}
	...
	send_message: function(opts, btn) {
		return finergy.call({
			type: "POST",
			method: "finergy.www.contact.send_message",
			btn: btn,
			args: opts,
	...
	has_permission: function(doctype, docname, perm_type, callback) {
		return finergy.call({
			type: "GET",
			method: "finergy.client.has_permission",
			no_spinner: true,
	...
	render_user: function() {
		if (finergy.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", finergy.get_cookie("user_image"));

			$('.user-image-wrapper').html(finergy.avatar(null, 'avatar-medium', null, null, null, true));
			$('.user-image-sidebar').html(finergy.avatar(null, 'avatar-medium', null, null, null, true));
			$('.user-image-myaccount').html(finergy.avatar(null, 'avatar-large', null, null, null, true));
		}
	},
	freeze_count: 0,
	freeze: function(msg) {
		...
		finergy.freeze_count++;
	},
	unfreeze: function() {
		if(!finergy.freeze_count) return; // anything open?
		finergy.freeze_count--;
		if(!finergy.freeze_count) {
	...
	trigger_ready: function() {
		finergy.ready_events.forEach(function(fn) {
			fn();
		});
	},
	...
	bind_filters: function() {
		...
		$(".filter").each(function() {
			...
			var val = finergy.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function() {
		finergy.get_navbar_search().on("keypress", function(e) {
			var val = $(this).val();
			if(e.which===13 && val) {
				$(this).val("").blur();
				finergy.do_search(val);
				return false;
			}
		});
	},
	do_search: function(val) {
		var path = (finergy.awesome_bar_path && finergy.awesome_bar_path[location.pathname]
			|| window.search_path || location.pathname);

		window.location.href = path + "?txt=" + encodeURIComponent(val);
	},
	set_search_path: function(path) {
		finergy.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function() {
		return finergy.get_cookie("user_id") !== "Guest" && finergy.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (finergy.session.user === 'Guest' && window.show_language_picker) {
			finergy.call("finergy.translate.get_all_languages", {
				with_language_name: true
			}).then(res => {
				let language_list = res.message;
				let language = finergy.get_cookie('preferred_language');
				let language_codes = [];
				...
				language_switcher.change(() => {
					...
					finergy.call("finergy.translate.set_preferred_language_cookie", {
...
finergy.setup_search = function (target, search_scope) {
	...
	$input.on('input', finergy.utils.debounce(() => {
		if (!$input.val()) {
			clear_dropdown();
			return;
		}
		...
		finergy.call({
			method: 'finergy.search.web_search',
			args: {
...
window.valid_email = function(id) {
	...
	// copied regex from finergy/utils.js validate_type
...
$(document).ready(function() {
	window.full_name = finergy.get_cookie("full_name");
	var logged_in = finergy.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	finergy.bind_navbar_search();

	// switch to app link
	if(finergy.get_cookie("system_user")==="yes" && logged_in) {
		finergy.add_switch_to_desk();
	}

	finergy.render_user();
	finergy.setup_lazy_images();
...
$(document).on("page-change", function() {
	...
	$.extend(finergy, finergy.get_cookies());
	finergy.session = {'user': finergy.user_id};

	finergy.datetime.refresh_when();
	finergy.trigger_ready();
	finergy.bind_filters();
	finergy.highlight_code_blocks();
	finergy.add_link_to_headings();
	finergy.make_navbar_active();
	// scroll to hash
...
finergy.ready(function() {
	finergy.show_language_picker();
	finergy.socketio.init(window.socketio_port);
});
