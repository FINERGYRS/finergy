...
from finergy.modules import get_module_name
from finergy.search.website_search import remove_document_from_index, update_index_for_path
from finergy.website.render import clear_cache
from finergy.website.utils import cleanup_page_name


class WebsiteGenerator(Document):
	website = finergy._dict()
	...
	def send_indexing_request(self, operation_type="URL_UPDATED"):
		...
		if (
			finergy.db.get_single_value("Website Settings", "enable_google_indexing")
			and self.is_website_published()
			and self.meta.allow_guest_to_view
		):

			url = finergy.utils.get_url(self.route)
			finergy.enqueue(
				"finergy.website.doctype.website_settings.google_indexing.publish_site",
				url=url,
	...
	def remove_old_route_from_index(self):
		...
		if self.allow_website_search_indexing() or finergy.flags.in_test:
			return
	...
	def update_website_search_index(self):
		...
		"""
		if not self.allow_website_search_indexing() or finergy.flags.in_test:
			return

		if self.is_website_published():
			finergy.enqueue(update_index_for_path, path=self.route)
		elif self.route:
