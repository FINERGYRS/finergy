...
from finergy.desk.moduleview import (
	config_exists,
	get_data,
...
def get_modules_from_all_apps_for_user(user=None):
	...
	global_blocked_modules = finergy.get_doc("User", "Administrator").get_blocked_modules()
	...
	for module in allowed_modules_list:
		...
		# Set defaults links
		module["links"] = get_onboard_items(module["app"], finergy.scrub(module_name))[:5]
...
def get_all_empty_tables_by_module():
	table_rows = finergy.qb.Field("table_rows")
	table_name = finergy.qb.Field("table_name")
	information_schema = finergy.qb.Schema("information_schema")

	empty_tables = (
		finergy.qb.from_(information_schema.tables).select(table_name).where(table_rows == 0)
	).run()

	empty_tables = {r[0] for r in empty_tables}

	results = finergy.get_all("DocType", fields=["name", "module"])
