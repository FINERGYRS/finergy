...
class OAuthWebRequestValidator(RequestValidator):
	...
	def validate_client_id(self, client_id, request, *args, **kwargs):
		...
		cli_id = finergy.db.get_value("OAuth Client", {"name": client_id})
		if cli_id:
			request.client = finergy.get_doc("OAuth Client", client_id).as_dict()
			return True
		else:
			return False
	...
	def validate_redirect_uri(self, client_id, redirect_uri, request, *args, **kwargs):
		...
		redirect_uris = finergy.db.get_value("OAuth Client", client_id, "redirect_uris").split(
			get_url_delimiter()
		)
	...
	def get_default_redirect_uri(self, client_id, request, *args, **kwargs):
		...
		redirect_uri = finergy.db.get_value("OAuth Client", client_id, "default_redirect_uri")
		return redirect_uri
	...
	def save_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		oac = finergy.new_doc("OAuth Authorization Code")
	...
	def authenticate_client(self, request, *args, **kwargs):
		...
		if request.client_id:
			oc = finergy.get_doc("OAuth Client", request.client_id)
		else:
			# Extract token, instantiate OAuth Bearer Token and use clientid from there.
			if "refresh_token" in finergy.form_dict:
				oc = finergy.get_doc(
					"OAuth Client",
					finergy.db.get_value(
						"OAuth Bearer Token",
						{"refresh_token": finergy.form_dict["refresh_token"]},
						"client",
					),
				)
			elif "token" in finergy.form_dict:
				oc = finergy.get_doc(
					"OAuth Client",
					finergy.db.get_value("OAuth Bearer Token", finergy.form_dict["token"], "client"),
				)
			else:
				oc = finergy.get_doc(
					"OAuth Client",
					finergy.db.get_value(
						"OAuth Bearer Token",
						finergy.get_request_header("Authorization").split(" ")[1],
						"client",
					),
				)
		try:
		...
		return finergy.session.user == user_id

	def authenticate_client_id(self, client_id, request, *args, **kwargs):
		cli_id = finergy.db.get_value("OAuth Client", client_id, "name")
		if not cli_id:
			# Don't allow public (non-authenticated) clients
			return False
		else:
			request["client"] = finergy.get_doc("OAuth Client", cli_id)
			return True
	...
	def validate_code(self, client_id, code, client, request, *args, **kwargs):
		...
		if code in checkcodes:
			request.scopes = finergy.db.get_value("OAuth Authorization Code", code, "scopes").split(
				get_url_delimiter()
			)
			request.user = finergy.db.get_value("OAuth Authorization Code", code, "user")
			code_challenge_method = finergy.db.get_value(
				"OAuth Authorization Code", code, "code_challenge_method"
			)
			code_challenge = finergy.db.get_value("OAuth Authorization Code", code, "code_challenge")

			if code_challenge and not request.code_verifier:
				if finergy.db.exists("OAuth Authorization Code", code):
					finergy.delete_doc("OAuth Authorization Code", code, ignore_permissions=True)
					finergy.db.commit()
				return False
	...
	def confirm_redirect_uri(self, client_id, code, redirect_uri, client, *args, **kwargs):
		saved_redirect_uri = finergy.db.get_value("OAuth Client", client_id, "default_redirect_uri")

		redirect_uris = finergy.db.get_value("OAuth Client", client_id, "redirect_uris")
	...
	def save_bearer_token(self, token, request, *args, **kwargs):
		...
		otoken = finergy.new_doc("OAuth Bearer Token")
		otoken.client = request.client["name"]
		try:
			otoken.user = (
				request.user
				if request.user
				else finergy.db.get_value(
					"OAuth Bearer Token",
			...
			otoken.user = finergy.session.user
		...
		default_redirect_uri = finergy.db.get_value(
	...
	def invalidate_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		finergy.db.set_value("OAuth Authorization Code", code, "validity", "Invalid")
		finergy.db.commit()
	...
	def validate_bearer_token(self, token, scopes, request):
		...
		otoken = finergy.get_doc("OAuth Bearer Token", token)
		token_expiration_local = otoken.expiration_time.replace(
			tzinfo=pytz.timezone(finergy.utils.get_time_zone())
		)
		token_expiration_utc = token_expiration_local.astimezone(pytz.utc)
		is_token_valid = (
			finergy.utils.datetime.datetime.utcnow().replace(tzinfo=pytz.utc) < token_expiration_utc
		) and otoken.status != "Revoked"
		client_scopes = finergy.db.get_value("OAuth Client", otoken.client, "scopes").split(
			get_url_delimiter()
		)
	...
	def get_original_scopes(self, refresh_token, request, *args, **kwargs):
		...
		# request.
		obearer_token = finergy.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token})
	...
	def revoke_token(self, token, token_type_hint, request, *args, **kwargs):
		...
		if token_type_hint == "access_token":
			finergy.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		elif token_type_hint == "refresh_token":
			finergy.db.set_value("OAuth Bearer Token", {"refresh_token": token}, "status", "Revoked")
		else:
			finergy.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		finergy.db.commit()
	...
	def finalize_id_token(self, id_token, token, token_handler, request):
		# Check whether finergy server URL is set
		...
		return finergy.safe_decode(id_token_encoded)

	def get_authorization_code_nonce(self, client_id, code, redirect_uri, request):
		if finergy.get_value("OAuth Authorization Code", code, "validity") == "Valid":
			return finergy.get_value("OAuth Authorization Code", code, "nonce")

		return None

	def get_authorization_code_scopes(self, client_id, code, redirect_uri, request):
		scope = finergy.get_value("OAuth Client", client_id, "scopes")
		if not scope:
			scope = []
		else:
	...
	def validate_id_token(self, token, scopes, request):
		try:
			id_token = finergy.get_doc("OAuth Bearer Token", token)
	...
	def validate_jwt_bearer_token(self, token, scopes, request):
		try:
			jwt = finergy.get_doc("OAuth Bearer Token", token)
	...
	def validate_silent_login(self, request):
		...
		- OpenIDConnectHybrid
		"""
		if finergy.session.user == "Guest" or request.prompt.lower() == "login":
			return False
		else:
			return True
		...
		if id_token_hint:
			try:
				...
				if payload.get("sub") and client_id and client_secret:
					user = finergy.db.get_value(
						"User Social Login",
						{"userid": payload.get("sub"), "provider": "finergy"},
						"parent",
					)
					...
					if verified_payload:
						return user.name == finergy.session.user

			except Exception:
				return False

		elif finergy.session.user != "Guest":
			return True
...
def delete_oauth2_data():
	...
	code_list = finergy.get_all("OAuth Authorization Code", filters={"validity": "Invalid"})
	token_list = finergy.get_all("OAuth Bearer Token", filters={"status": "Revoked"})
	if len(code_list) > 0:
		commit_code = True
	if len(token_list) > 0:
		commit_token = True
	for code in code_list:
		finergy.delete_doc("OAuth Authorization Code", code["name"])
	for token in token_list:
		finergy.delete_doc("OAuth Bearer Token", token["name"])
	if commit_code or commit_token:
		finergy.db.commit()


def get_client_scopes(client_id):
	scopes_string = finergy.db.get_value("OAuth Client", client_id, "scopes")
...
def get_userinfo(user):
	...
	if user.user_image:
		if finergy.utils.validate_url(user.user_image, valid_schemes=valid_url_schemes):
			picture = user.user_image
		else:
			picture = finergy_server_url + "/" + user.user_image

	userinfo = finergy._dict(
		{
			"sub": finergy.db.get_value(
				"User Social Login",
				{"parent": user.name, "provider": "finergy"},
				"userid",
			),
			...
			"email": user.email,
			"picture": picture,
			"roles": finergy.get_roles(user.name),
			"iss": finergy_server_url,
		}
	)
...
def generate_json_error_response(e):
	...
	finergy.local.response["http_status_code"] = getattr(e, "status_code", 500)
	return


def get_server_url():
	request_url = urlparse(finergy.request.url)
	request_url = f"{request_url.scheme}://{request_url.netloc}"
	return finergy.get_value("Social Login Key", "finergy", "base_url") or request_url
