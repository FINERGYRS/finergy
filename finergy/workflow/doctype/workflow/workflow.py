...
class Workflow(Document):
	...
	def on_update(self):
		...
		finergy.clear_cache(doctype=self.document_type)
		finergy.cache().delete_key("workflow_" + self.name)  # clear cache created in model/workflow.py

	def create_custom_field_for_workflow_state(self):
		finergy.clear_cache(doctype=self.document_type)
		meta = finergy.get_meta(self.document_type)
	...
	def validate_docstatus(self):
		def get_state(state):
			...
			finergy.throw(finergy._("{0} not a valid State").format(state))
		...
		for t in self.transitions:
			...
			if state.doc_status == "2":
				finergy.throw(
					finergy._("Cannot change state of Cancelled Document. Transition row {0}").format(t.idx)
				)

			if state.doc_status == "1" and next_state.doc_status == "0":
				finergy.throw(
					finergy._("Submitted Document cannot be converted back to draft. Transition row {0}").format(
						t.idx
					)
				)

			if state.doc_status == "0" and next_state.doc_status == "2":
				finergy.throw(finergy._("Cannot cancel before submitting. See Transition {0}").format(t.idx))
...
def get_fieldnames_for(doctype):
	return [
		f.fieldname for f in finergy.get_meta(doctype).fields if f.fieldname not in no_value_fields
	]


@finergy.whitelist()
def get_workflow_state_count(doctype, workflow_state_field, states):
	states = finergy.parse_json(states)
