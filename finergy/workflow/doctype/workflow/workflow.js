finergy.provide("finergy.core")

finergy.ui.form.on("Workflow", {
	onload: function(frm) {
	...
	refresh: function(frm) {
		if (frm.doc.document_type) {
			frm.add_custom_button(__('Go to {0} List', [frm.doc.document_type]), () => {
				finergy.set_route('List', frm.doc.document_type);
			});
		}
	...
	update_field_options: function(frm) {
		...
		if (doc.document_type) {
			const get_field_method = 'finergy.workflow.doctype.workflow.workflow.get_fieldnames_for';
			finergy.xcall(get_field_method, { doctype: doc.document_type })
				.then(resp => {
	...
	set_table_html: function(frm) {
		...
		const promises = frm.states.map(r => {
			...
			return finergy.utils.get_indicator_color(state).then(color => {
				return `<tr>
				<td>
	...
	get_orphaned_states_and_count: function(frm) {
		...
		return finergy.xcall('finergy.workflow.doctype.workflow.workflow.get_workflow_state_count', {
	...
	render_state_table: function(frm) {
		if (frm.states && frm.states.length) {
			...
			$(frm.state_table).find('a.orphaned-state').on('click', (e) => {
				...
				finergy.set_route('List', frm.doc.document_type, filters);
			});
		}
	}
...
finergy.ui.form.on("Workflow Document State", {
