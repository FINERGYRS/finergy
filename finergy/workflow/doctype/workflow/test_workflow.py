...
class TestWorkflow(unittest.TestCase):
	...
	def tearDown(self):
		finergy.delete_doc("Workflow", "Test ToDo")
	...
	def test_if_workflow_actions_were_processed(self):
		finergy.db.sql("delete from `tabWorkflow Action`")
		...
		workflow_actions = finergy.get_all("Workflow Action", fields=["*"])
		...
		workflow_actions = finergy.get_all("Workflow Action", fields=["status"])
		self.assertEqual(len(workflow_actions), 1)
...
def create_todo_workflow():
	if finergy.db.exists("Workflow", "Test ToDo"):
		finergy.delete_doc("Workflow", "Test ToDo")

	if not finergy.db.exists("Role", "Test Approver"):
		finergy.get_doc(dict(doctype="Role", role_name="Test Approver")).insert(ignore_if_duplicate=True)
	workflow = finergy.new_doc("Workflow")
...
def create_new_todo():
	return finergy.get_doc(dict(doctype="ToDo", description="workflow " + random_string(10))).insert()
