...
from finergy.desk.form.utils import get_pdf_link
from finergy.desk.notifications import clear_doctype_notifications
from finergy.model.document import Document
from finergy.model.workflow import (
	apply_workflow,
	get_workflow_name,
...
from finergy.utils import get_datetime, get_url
from finergy.utils.background_jobs import enqueue
from finergy.utils.data import get_link_to_form
...
def on_doctype_update():
	finergy.db.add_index("Workflow Action", ["status", "user"])
...
def get_permission_query_conditions(user):
	...
	return "(`tabWorkflow Action`.`user`={user})".format(user=finergy.db.escape(user))
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = finergy.session.user
	...
	if logged_in_user == "Guest":
		finergy.set_user(logged_in_user)
...
def return_success_page(doc):
	finergy.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), finergy.bold(doc.get("name")), finergy.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	finergy.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			finergy.bold(doc.get("name")),
			finergy.bold(doc_workflow_state),
			finergy.bold(finergy.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def clear_old_workflow_actions(doc, user=None):
	user = user if user else finergy.session.user
	finergy.db.sql(
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = finergy._dict(
					{
						"possible_actions": [],
						"email": finergy.db.get_value("User", user, "email"),
					}
				)
...
def send_workflow_action_email(users_data, doc):
	...
	for d in users_data:
		...
		enqueue(method=finergy.sendmail, queue="short", **email_args)
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = (
		"/api/method/finergy.workflow.doctype.workflow_action.workflow_action.apply_action"
	)
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/finergy.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def filter_allowed_users(users, doc, transition):
	...
	"""
	from finergy.permissions import has_permission
...
def get_common_email_args(doc):
	...
	if email_template:
		subject = finergy.render_template(email_template.subject, vars(doc))
		response = finergy.render_template(email_template.response, vars(doc))
	else:
	...
	common_args = {
		...
		"attachments": [finergy.attach_print(doctype, docname, file_name=docname, doc=doc)],
		"subject": subject,
		"message": response,
	}
	return common_args
...
def get_email_template(doc):
	...
	template_name = finergy.db.get_value(
	...
	if not template_name:
		return
	return finergy.get_doc("Email Template", template_name)


def get_state_optional_field_value(workflow_name, state):
	return finergy.get_cached_value(
