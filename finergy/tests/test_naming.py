...
from finergy.model.naming import (
...
from finergy.utils import now_datetime, nowdate, nowtime


class TestNaming(unittest.TestCase):
	def setUp(self):
		finergy.db.delete("Note")
	...
	def test_field_autoname_name_sync(self):
		...
		country = finergy.get_last_doc("Country")
	...
	def test_child_table_naming(self):
		...
		name = finergy.generate_hash(length=10)

		doc = finergy.new_doc("dt_with_childtable_naming")
	...
	def test_format_autoname(self):
		...
		todo_doctype = finergy.get_doc("DocType", doctype)
		todo_doctype.autoname = "format:TODO-{MM}-{status}-{##}"
		todo_doctype.save()

		description = "Format"

		todo = finergy.new_doc(doctype)
	...
	def test_format_autoname_for_datetime_field(self):
		...
		for field in [now_datetime(), nowdate(), nowtime()]:
			doc = finergy.new_doc(doctype.name)
			doc.field = field
			doc.insert()
	...
	def test_revert_series(self):
		...
		finergy.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 1)""", (series,))
		...
		finergy.db.sql("""delete from `tabSeries` where name = %s""", series)

		series = "TEST-{}-".format(year)
		key = "TEST-.YYYY.-.#####"
		name = "TEST-{}-00002".format(year)
		finergy.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 2)""", (series,))
		revert_series_if_last(key, name)
		count = finergy.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
		...
		name = "TEST-00003"
		finergy.db.sql("DELETE FROM `tabSeries` WHERE `name`=%s", series)
		finergy.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 3)""", (series,))
		revert_series_if_last(key, name)
		count = finergy.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
	...
	def test_naming_for_cancelled_and_amended_doc(self):
		submittable_doctype = finergy.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"custom": 1,
		...
		doc = finergy.new_doc("Submittable Doctype")
		doc.save()
		...
		amended_doc = finergy.copy_doc(doc)
	...
	def test_naming_validations(self):
		...
		# set name via prompt
		tag = finergy.get_doc({"doctype": "Tag", "__newname": "Tag"})
		self.assertRaises(finergy.NameError, tag.insert)

		# set by passing set_name as ToDo
		self.assertRaises(finergy.NameError, make_invalid_todo)

		# set new name - Note
		note = finergy.get_doc({"doctype": "Note", "title": "Note"})
		self.assertRaises(finergy.NameError, note.insert)

		# case 2: set name with "New ---"
		tag = finergy.get_doc({"doctype": "Tag", "__newname": "New Tag"})
		self.assertRaises(finergy.NameError, tag.insert)
		...
		tag = finergy.get_doc({"doctype": "Tag", "__newname": ""})
		self.assertRaises(finergy.ValidationError, tag.insert)

	def test_naming_with_empty_part(self):
		# check naming with empty part (duplicate dots)

		webhook = finergy.new_doc("Webhook")
...
def make_invalid_todo():
	finergy.get_doc({"doctype": "ToDo", "description": "Test"}).insert(set_name="ToDo")
