...
import finergy
from finergy.auth import HTTPRequest, get_login_attempt_tracker, validate_ip_address
...
class TestTwoFactor(unittest.TestCase):
	...
	def setUp(self):
		...
		self.login_manager = finergy.local.login_manager
		self.user = self.login_manager.user
		update_system_settings({"allow_consecutive_login_attempts": 2})

	def tearDown(self):
		finergy.local.response["verification"] = None
		finergy.local.response["tmp_id"] = None
		disable_2fa()
		finergy.clear_cache(user=self.user)
	...
	def test_authenticate_for_2factor(self):
		"""Verification obj and tmp_id should be set in finergy.local."""
		authenticate_for_2factor(self.user)
		verification_obj = finergy.local.response["verification"]
		tmp_id = finergy.local.response["tmp_id"]
		self.assertTrue(verification_obj)
		self.assertTrue(tmp_id)
		for k in ["_usr", "_pwd", "_otp_secret"]:
			self.assertTrue(finergy.cache().get("{0}{1}".format(tmp_id, k)), "{} not available".format(k))
	...
	def test_two_factor_is_enabled(self):
		...
		# Scenario 3
		enable_2fa()
		user = finergy.get_doc("User", self.user)
		user.restrict_ip = finergy.local.request_ip
		user.save()
		self.assertTrue(should_run_2fa(self.user))
	...
	def test_confirm_otp_token(self):
		...
		finergy.flags.otp_expiry = 2
		authenticate_for_2factor(self.user)
		tmp_id = finergy.local.response["tmp_id"]
		otp = "wrongotp"
		with self.assertRaises(finergy.AuthenticationError):
		...
		finergy.flags.otp_expiry = None
		if finergy.flags.tests_verbose:
	...
	def test_render_string_template(self):
		...
		args = {"issuer_name": "Finergy Technologies"}
		_str = "Verification Code from {{issuer_name}}"
		_str = finergy.render_template(_str, args)
		self.assertEqual(_str, "Verification Code from Finergy Technologies")
	...
	def test_otp_attempt_tracker(self):
		...
		with self.assertRaises(finergy.SecurityException):
			confirm_otp_token(self.login_manager, otp=otp, tmp_id=tmp_id)
...
def create_http_request():
	...
	enable_2fa()
	finergy.form_dict["usr"] = "test@example.com"
	finergy.form_dict["pwd"] = "Eastern_43A1W"
	finergy.local.form_dict["cmd"] = "login"
...
def enable_2fa(bypass_two_factor_auth=0, bypass_restrict_ip_check=0):
	...
	system_settings = finergy.get_doc("System Settings")
...
def toggle_2fa_all_role(state=None):
	...
	all_role = finergy.get_doc("Role", "All")
	if state == None:
