...
from finergy.permissions import add_user_permission, clear_user_permissions_for_doctype
...
class TestReportview(unittest.TestCase):
	...
	def test_build_match_conditions(self):
		...
		test2user = finergy.get_doc("User", "test2@example.com")
	...
	def test_none_filter(self):
		query = finergy.db.query.get_sql("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_between_filters(self):
		...
		finergy.db.sql("delete from tabEvent")
	...
	def test_ignore_permissions_for_get_filters_cond(self):
		finergy.set_user("test2@example.com")
		self.assertRaises(finergy.PermissionError, get_filters_cond, "DocType", dict(istable=1), [])
	...
	def test_nested_permission(self):
		...
		from finergy.core.page.permission_manager.permission_manager import update
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = finergy.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = finergy.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = finergy.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = finergy.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = finergy.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = finergy.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = finergy.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = finergy.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = finergy.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(finergy.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(finergy.get_all("Nested DocType"))
		)
	...
	def test_reportview_get(self):
		...
		response = execute_cmd("finergy.desk.reportview.get")
	...
	def test_fieldname_starting_with_int(self):
		...
		finergy.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": "10"}).insert(
	...
	def test_permission_query_condition(self):
		from finergy.desk.doctype.dashboard_settings.dashboard_settings import create_dashboard_settings
		...
		dashboard_settings = finergy.db.sql(
			"""
				SELECT name
...
def create_nested_doctype():
	if finergy.db.exists("DocType", "Nested DocType"):
		return
...
def create_nested_doctype_records():
	...
	for r in records:
		d = finergy.new_doc("Nested DocType")
		d.update(r)
