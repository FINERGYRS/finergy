...
import finergy
from finergy.integrations.oauth2 import encode_params
...
class TestOAuth20(unittest.TestCase):
	def setUp(self):
		...
		self.client_id = finergy.get_all("OAuth Client", fields=["*"])[0].get("client_id")
		self.form_header = {"content-type": "application/x-www-form-urlencoded"}
		self.scope = "all openid"
		self.redirect_uri = "http://localhost"

		# Set Finergy server URL reqired for id_token generation
		try:
			finergy_login_key = finergy.get_doc("Social Login Key", "finergy")
		...
		finergy_login_key.base_url = finergy.utils.get_url()
	...
	def test_login_using_authorization_code(self):
		...
		try:
			session.get(
				get_full_url("/api/method/finergy.integrations.oauth2.authorize"),
		...
		token_response = requests.post(
			get_full_url("/api/method/finergy.integrations.oauth2.get_token"),
	...
	def test_revoke_token(self):
		client = finergy.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = requests.post(
			get_full_url("/api/method/finergy.integrations.oauth2.revoke_token"),
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = finergy.get_doc("OAuth Client", self.client_id)
...
def check_valid_openid_response(access_token=None):
	...
	openid_response = requests.get(
		get_full_url("/api/method/finergy.integrations.oauth2.openid_profile"), headers=headers
	)
...
def get_full_url(endpoint):
	...
	return urljoin(finergy.utils.get_url(), endpoint)


def update_client_for_auth_code_grant(client_id):
	client = finergy.get_doc("OAuth Client", client_id)
	client.grant_type = "Authorization Code"
	client.response_type = "Code"
	client.save()
	finergy.db.commit()
