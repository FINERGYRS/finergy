...
import finergy
from finergy.desk.listview import get_group_by_count, get_list_settings, set_list_settings


class TestListView(unittest.TestCase):
	def setUp(self):
		if finergy.db.exists("List View Settings", "DocType"):
			finergy.delete_doc("List View Settings", "DocType")
	...
	def test_get_list_settings_with_default_settings(self):
		finergy.get_doc({"doctype": "List View Settings", "name": "DocType"}).insert()
	...
	def test_get_list_settings_with_non_default_settings(self):
		finergy.get_doc({"doctype": "List View Settings", "name": "DocType", "disable_count": 1}).insert()
		settings = get_list_settings("DocType")
		self.assertIsNotNone(settings)
	...
	def test_set_list_settings_without_settings(self):
		...
		settings = finergy.get_doc("List View Settings", "DocType")
	...
	def test_list_view_child_table_filter_with_created_by_filter(self):
		if finergy.db.exists("Note", "Test created by filter with child table filter"):
			finergy.delete_doc("Note", "Test created by filter with child table filter")
