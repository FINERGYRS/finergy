...
from finergy.utils import get_site_url, get_test_client

try:
	_site = finergy.local.site
except Exception:
	_site = None
...
class TestResourceAPI(unittest.TestCase):
	SITE_URL = get_site_url(finergy.local.site)
	...
	def setUpClass(self):
		for _ in range(10):
			doc = finergy.get_doc({"doctype": "ToDo", "description": finergy.mock("paragraph")}).insert()
	...
	def tearDownClass(self):
		for name in self.GENERATED_DOCUMENTS:
			finergy.delete_doc_if_exists(self.DOCTYPE, name)
	...
	def post(self, path, data):
		return requests.post(f"{self.RESOURCE_URL}/{path}?sid={self.sid}", data=finergy.as_json(data))

	def put(self, path, data):
		return requests.put(f"{self.RESOURCE_URL}/{path}?sid={self.sid}", data=finergy.as_json(data))
	...
	def test_get_list_dict(self):
		...
		json = finergy._dict(response.json())
		self.assertEqual(response.status_code, 200)
	...
	def test_create_document(self):
		...
		data = {"description": finergy.mock("paragraph")}
	...
	def test_update_document(self):
		...
		generated_desc = finergy.mock("paragraph")
		data = {"description": generated_desc}
		random_doc = choice(self.GENERATED_DOCUMENTS)
		desc_before_update = finergy.db.get_value(self.DOCTYPE, random_doc, "description")
	...
	def test_delete_document(self):
		...
		non_existent_doc = finergy.generate_hash(length=12)
...
class TestMethodAPI(unittest.TestCase):
	METHOD_URL = f"{get_site_url(finergy.local.site)}/api/method"
	...
	def test_version(self):
		...
		self.assertEqual(Version(json.message), Version(finergy.__version__))
...
class FinergyAPITestCase(FinergyTestCase):
	...
	def sid(self):
		if not getattr(self, "_sid", None):
			...
			finergy.local.login_manager.login_as("Administrator")
			self._sid = finergy.session.sid
...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("finergy.app.get_site_name", return_value=self.site):
				header_patch = patch("finergy.get_request_header", new=patch_request_header)
