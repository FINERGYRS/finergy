...
from finergy.utils import global_search
...
class TestGlobalSearch(unittest.TestCase):
	def setUp(self):
		...
		self.assertTrue("__global_search" in finergy.db.get_tables())
		doctype = "Event"
		global_search.reset()
	...
	def tearDown(self):
		finergy.db.sql("DELETE FROM `tabProperty Setter` WHERE `doc_type`='Event'")
	...
	def insert_test_events(self):
		...
		for text in phrases:
			finergy.get_doc(
				dict(doctype="Event", subject=text, repeat_on="Monthly", starts_on=finergy.utils.now_datetime())
			).insert()
	...
	def test_update_doc(self):
		...
		event = finergy.get_doc("Event", finergy.get_all("Event")[0].name)
	...
	def test_delete_doc(self):
		...
		event_name = finergy.get_all("Event")[0].name
		event = finergy.get_doc("Event", event_name)
		...
		finergy.delete_doc("Event", event_name)
		global_search.sync_global_search()
	...
	def test_insert_child_table(self):
		...
		for text in phrases:
			doc = finergy.get_doc(
				{"doctype": "Event", "subject": text, "starts_on": finergy.utils.now_datetime()}
			)
			doc.insert()
