...
from finergy.finergyclient import FinergyClient, FinergyException
from finergy.utils.data import get_url


class TestFinergyClient(unittest.TestCase):
	def test_insert_many(self):
		server = FinergyClient(get_url(), "Administrator", "admin", verify=False)
		finergy.db.sql("delete from `tabNote` where title in ('Sing','a','song','of','sixpence')")
		finergy.db.commit()
		...
		self.assertTrue(finergy.db.get_value("Note", {"title": "Sing"}))
		self.assertTrue(finergy.db.get_value("Note", {"title": "a"}))
		self.assertTrue(finergy.db.get_value("Note", {"title": "song"}))
		self.assertTrue(finergy.db.get_value("Note", {"title": "of"}))
		self.assertTrue(finergy.db.get_value("Note", {"title": "sixpence"}))

	def test_create_doc(self):
		server = FinergyClient(get_url(), "Administrator", "admin", verify=False)
		finergy.db.sql("delete from `tabNote` where title = 'test_create'")
		finergy.db.commit()

		server.insert({"doctype": "Note", "public": True, "title": "test_create"})

		self.assertTrue(finergy.db.get_value("Note", {"title": "test_create"}))
	...
	def test_get_doc(self):
		server = FinergyClient(get_url(), "Administrator", "admin", verify=False)
		finergy.db.sql("delete from `tabNote` where title = 'get_this'")
		finergy.db.commit()
	...
	def test_get_value(self):
		server = FinergyClient(get_url(), "Administrator", "admin", verify=False)
		finergy.db.sql("delete from `tabNote` where title = 'get_value'")
		finergy.db.commit()
	...
	def test_get_single(self):
		...
		finergy.db.set_value("Website Settings", None, "title_prefix", "")

	def test_update_doc(self):
		server = FinergyClient(get_url(), "Administrator", "admin", verify=False)
		finergy.db.sql("delete from `tabNote` where title in ('Sing','sing')")
		finergy.db.commit()
	...
	def test_update_child_doc(self):
		server = FinergyClient(get_url(), "Administrator", "admin", verify=False)
		finergy.db.sql("delete from `tabContact` where first_name = 'George' and last_name = 'Steevens'")
		finergy.db.sql(
			"delete from `tabContact` where first_name = 'William' and last_name = 'Shakespeare'"
		)
		finergy.db.sql("delete from `tabCommunication` where reference_doctype = 'Event'")
		finergy.db.sql("delete from `tabCommunication Link` where link_doctype = 'Contact'")
		finergy.db.sql("delete from `tabEvent` where subject = 'Sing a song of sixpence'")
		finergy.db.sql("delete from `tabEvent Participants` where reference_doctype = 'Contact'")
		finergy.db.commit()
		...
		self.assertTrue(finergy.db.exists("Communication Link", {"link_name": "William Shakespeare"}))

	def test_delete_doc(self):
		server = FinergyClient(get_url(), "Administrator", "admin", verify=False)
		finergy.db.sql("delete from `tabNote` where title = 'delete'")
		finergy.db.commit()
		...
		self.assertFalse(finergy.db.get_value("Note", {"title": "delete"}))

	def test_auth_via_api_key_secret(self):
		# generate API key and API secret for administrator
		keys = generate_keys("Administrator")
		finergy.db.commit()
		generated_secret = finergy.utils.password.get_decrypted_password(
			"User", "Administrator", fieldname="api_secret"
		)

		api_key = finergy.db.get_value("User", "Administrator", "api_key")
		header = {"Authorization": "token {}:{}".format(api_key, generated_secret)}
		res = requests.post(get_url() + "/api/method/finergy.auth.get_logged_user", headers=header)
		...
		header = {
			"Authorization": "Basic {}".format(
				base64.b64encode(finergy.safe_encode("{}:{}".format(api_key, generated_secret))).decode()
			)
		}
		res = requests.post(get_url() + "/api/method/finergy.auth.get_logged_user", headers=header)
		self.assertEqual(res.status_code, 200)
