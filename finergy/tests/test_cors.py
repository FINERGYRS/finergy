...
import finergy
from finergy.app import process_response
...
class TestCORS(unittest.TestCase):
	def make_request_and_test(self, origin="http://example.com", absent=False):
		...
		finergy.utils.set_request(headers=headers)
	...
	def test_valid_origin(self):
		...
		finergy.conf.allow_cors = "*"
		self.make_request_and_test()

		finergy.conf.allow_cors = ["http://example.com", "https://example.com"]
		self.make_request_and_test()

	def test_invalid_origin(self):
		finergy.conf.allow_cors = "http://example1.com"
		self.make_request_and_test(absent=True)

		finergy.conf.allow_cors = ["http://example1.com", "https://example.com"]
		self.make_request_and_test(absent=True)
