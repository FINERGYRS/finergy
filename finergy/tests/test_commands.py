...
from finergy.installer import add_to_installed_apps, remove_app
from finergy.utils import add_to_date, get_bench_relative_path, now
from finergy.utils.backups import fetch_latest_backups


# TODO: check finergy.cli.coloured_output to set coloured output!
def supports_color():
	"""
...
def exists_in_backup(doctypes, file):
	...
	"""
	predicate = 'COPY public."tab{}"' if finergy.conf.db_type == "postgres" else "CREATE TABLE `tab{}`"
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("bench --site {site} execute finergy.db.get_database_size")
		self.assertEquals(self.returncode, 0)
		self.assertIsInstance(float(self.stdout), float)

		# test 2: execute a command expecting an errored output as local won't exist
		self.execute("bench --site {site} execute finergy.local.site")
		...
		self.execute("""bench --site {site} execute finergy.bold --kwargs '{{"text": "DocType"}}'""")
		self.assertEquals(self.returncode, 0)
		self.assertEquals(self.stdout[1:-1], finergy.bold(text="DocType"))
	...
	def test_backup(self):
		...
		site_backup_path = finergy.utils.get_site_path("private", "backups")
		...
		# test 7: take a backup with finergy.conf.backup.includes
		self.execute(
		...
		# test 8: take a backup with finergy.conf.backup.excludes
		self.execute(
		...
		# test 9: take a backup with --include (with finergy.conf.excludes still set)
		self.execute(
	...
	def test_restore(self):
		...
		global_config = {
			"admin_password": finergy.conf.admin_password,
			"root_login": finergy.conf.root_login,
			"root_password": finergy.conf.root_password,
			"db_type": finergy.conf.db_type,
		}
		site_data = {"another_site": f"{finergy.local.site}-restore.test", **global_config}
		...
		self.execute(
			"bench --site {another_site} execute finergy.utils.backups.fetch_latest_backups",
			site_data,
		)
		...
		self.execute(
			...
			" finergy.utils.backups.fetch_latest_backups --kwargs {kw}",
			site_data,
		)
		site_data.update({"database": json.loads(self.stdout)["database"]})
		self.execute("bench --site {another_site} restore {database}", site_data)
	...
	def test_partial_restore(self):
			...
			finergy.get_doc(
				{
					...
					"description": finergy.mock("paragraph"),
				}
			).insert()
		finergy.db.commit()
		todo_count = finergy.db.count("ToDo")
		...
		finergy.db.sql_ddl("DROP TABLE IF EXISTS `tabToDo`")
		finergy.db.commit()

		self.execute("bench --site {site} partial-restore {path}", {"path": db_path})
		self.assertEquals(self.returncode, 0)
		self.assertEquals(finergy.db.count("ToDo"), todo_count)
	...
	def test_recorder(self):
		...
		finergy.local.cache = {}
		self.assertEqual(finergy.recorder.status(), True)

		self.execute("bench --site {site} stop-recording")
		finergy.local.cache = {}
		self.assertEqual(finergy.recorder.status(), False)
	...
	def test_list_apps(self):
		...
		doctype = finergy.get_single("Installed Applications").installed_applications
		if doctype:
			installed_apps = set([x.app_name for x in doctype])
		else:
			installed_apps = set(finergy.get_installed_apps())
	...
	def test_finergy_site_env(self):
		os.putenv("FINERGY_SITE", finergy.local.site)
		self.execute("bench execute finergy.ping")
		self.assertEquals(self.returncode, 0)
...
class RemoveAppUnitTests(unittest.TestCase):
	def test_delete_modules(self):
		from finergy.installer import (
			_delete_doctypes,
			_delete_modules,
			_get_module_linked_doctype_field_map,
		)

		test_module = finergy.new_doc("Module Def")

		test_module.update({"module_name": "RemoveThis", "app_name": "finergy"})
		test_module.save()

		module_def_linked_doctype = finergy.get_doc(
			{
				"doctype": "DocType",
		...
		self.assertFalse(finergy.db.exists("Module Def", test_module.module_name))
		self.assertFalse(finergy.db.exists("DocType", module_def_linked_doctype.name))

	def test_dry_run(self):
		"""Check if dry run in not destructive."""

		# nothing to assert, if this fails rest of the test suite will crumble.
		remove_app("finergy", dry_run=True, yes=True, no_backup=True)
