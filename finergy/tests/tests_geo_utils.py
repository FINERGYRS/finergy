...
import finergy
from finergy.geo.utils import get_coords


class TestGeoUtils(unittest.TestCase):
	def setUp(self):
		self.todo = finergy.get_doc(
		...
		self.test_location = finergy.get_doc(
	...
	def test_get_coords_from_not_existable_location(self):
		self.assertRaises(
			finergy.ValidationError, get_coords, "ToDo", self.test_filter_todo, "location_field"
		)

	def test_get_coords_from_not_existable_coords(self):
		self.assertRaises(
			finergy.ValidationError, get_coords, "ToDo", self.test_filter_todo, "coordinates"
		)
