...
class TestDefaults(unittest.TestCase):
	...
	def test_user_permission_on_defaults(self):
		...
		old_user = finergy.session.user
		user = "test@example.com"
		finergy.set_user(user)

		perm_doc = finergy.get_doc(
			dict(
		...
		finergy.delete_doc("User Permission", perm_doc.name)
		finergy.set_user(old_user)
