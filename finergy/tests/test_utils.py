...
from finergy.utils.dateutils import get_dates_from_timegrain
	...
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/finergyrs/test.jpg">
			</div>
			<div style="background-image: url('/assets/finergyrs/bg.jpg')">
		...
		self.assertTrue('<img src="{0}/assets/finergyrs/test.jpg">'.format(url) in html)
		self.assertTrue(
			"style=\"background-image: url('{0}/assets/finergyrs/bg.jpg') !important\"".format(url) in html
		)
...
class TestValidationUtils(unittest.TestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://finergy-rs.fr", throw=True))
		...
		self.assertRaises(finergy.ValidationError, validate_url, "finergy", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://finergy.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(
			validate_url("bolo://finergy-rs.fr", valid_schemes=("http", "https", "ftp", "ftps"))
		)
		self.assertRaises(
			finergy.ValidationError, validate_url, "gopher://finergy-rs.fr", valid_schemes="https", throw=True
		)
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@finergy.com"))
		self.assertTrue(validate_email_address("someone@finergy.com, anyone@finergy-rs.fr"))
		...
		# Invalid with throw
		self.assertRaises(
			finergy.InvalidEmailAddressError, validate_email_address, "someone.com", throw=True
		)
...
class TestPythonExpressions(unittest.TestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(finergy.ValidationError, validate_python_code, expr)
...
class TestDateUtils(unittest.TestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(finergy.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				finergy.utils.get_first_day_of_week("2020-12-25"), finergy.utils.getdate("2020-12-21")
			)
			self.assertEqual(
				finergy.utils.get_first_day_of_week("2020-12-20"), finergy.utils.getdate("2020-12-14")
			)

		# Sunday as start of the week
		self.assertEqual(
			finergy.utils.get_first_day_of_week("2020-12-25"), finergy.utils.getdate("2020-12-20")
		)
		self.assertEqual(
			finergy.utils.get_first_day_of_week("2020-12-21"), finergy.utils.getdate("2020-12-20")
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			finergy.utils.get_last_day_of_week("2020-12-24"), finergy.utils.getdate("2020-12-26")
		)
		self.assertEqual(
			finergy.utils.get_last_day_of_week("2020-12-28"), finergy.utils.getdate("2021-01-02")
		)
...
class TestResponse(unittest.TestCase):
	def test_json_handler(self):
		...
		GOOD_OBJECT = {
			...
			"doc": [
				finergy.get_doc("System Settings"),
			],
			"iter": [
				{1, 2, 3},
				(1, 2, 3),
