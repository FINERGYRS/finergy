...
from finergy.translate import get_language, get_parent_language, get_translation_dict_from_file
...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	finergy.get_all("Language", pluck="name", filters=[["name", "not like", "en%"]]),
	k=5,
)
...
class TestTranslate(unittest.TestCase):
	...
	def setUp(self):
		...
		finergy.local.lang_full_dict = None  # reset cached translations

	def tearDown(self):
		finergy.form_dict.pop("_lang", None)
		if self._testMethodName in self.guest_sessions_required:
	...
	def test_request_language_resolution_with_form_dict(self):
		"""Test for finergy.translate.get_language

		Case 1: finergy.form_dict._lang is set
		"""

		finergy.form_dict._lang = first_lang

		with patch.object(finergy.translate, "get_preferred_language_cookie", return_value=second_lang):
	...
	def test_request_language_resolution_with_cookie(self):
		...
		Case 2: finergy.form_dict._lang is not set, but preferred_language cookie is
		"""

		with patch.object(finergy.translate, "get_preferred_language_cookie", return_value="fr"):
	...
	def test_guest_request_language_resolution_with_cookie(self):
		...
		Case 3: finergy.form_dict._lang is not set, but preferred_language cookie is [Guest User]
		"""
	...
	def test_guest_request_language_resolution_with_request_header(self):
		...
		Case 4: finergy.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is [Guest User]
		"""
	...
	def test_request_language_resolution_with_request_header(self):
		...
		Case 5: finergy.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is
		"""
	...
	def test_load_all_translate_files(self):
		...
		verify_translation_files("finergy")
...
def verify_translation_files(app):
	...
	translations_dir = Path(finergy.get_app_path(app)) / "translations"
