...
import finergy
from finergy.auth import HTTPRequest, LoginAttemptTracker
from finergy.finergyclient import AuthError, FinergyClient
...
class TestAuth(unittest.TestCase):
	...
	def tearDown(self):
		finergy.delete_doc("User", self.test_user_email, force=True)
	...
	def set_system_settings(self, k, v):
		finergy.db.set_value("System Settings", "System Settings", k, v)
		finergy.db.commit()
	...
	def test_allow_login_using_mobile(self):
		...
		FinergyClient(finergy.get_site_config().host_name, self.test_user_mobile, self.test_user_password)
		FinergyClient(finergy.get_site_config().host_name, self.test_user_email, self.test_user_password)

		# login by username should fail
		with self.assertRaises(AuthError):
			FinergyClient(finergy.get_site_config().host_name, self.test_user_name, self.test_user_password)
