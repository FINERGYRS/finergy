...
import finergy
from finergy.query_builder import Case
from finergy.query_builder.custom import ConstantColumn
from finergy.query_builder.functions import Coalesce, CombineDatetime, GroupConcat, Match
from finergy.query_builder.utils import db_type_is


def run_only_if(dbtype: db_type_is) -> Callable:
	return unittest.skipIf(db_type_is(finergy.conf.db_type) != dbtype, f"Only runs for {dbtype.value}")
...
class TestCustomFunctionsMariaDB(unittest.TestCase):
	...
	def test_constant_column(self):
		query = finergy.qb.from_("DocType").select("name", ConstantColumn("John").as_("User"))
		self.assertEqual(query.get_sql(), "SELECT `name`,'John' `User` FROM `tabDocType`")

	def test_timestamp(self):
		note = finergy.qb.DocType("Note")
		self.assertEqual(
...
class TestBuilderBase(object):
	def test_adding_tabs(self):
		self.assertEqual("tabNotes", finergy.qb.DocType("Notes").get_sql())
		self.assertEqual("__Auth", finergy.qb.DocType("__Auth").get_sql())
		self.assertEqual("Notes", finergy.qb.Table("Notes").get_sql())

	def test_run_patcher(self):
		query = finergy.qb.from_("ToDo").select("*").limit(1)
...
class TestParameterization(unittest.TestCase):
	def test_where_conditions(self):
		...
		query = (
			finergy.qb.from_(DocType).select(DocType.name).where((DocType.owner == "Administrator' --"))
		)
	...
	def test_set_cnoditions(self):
		DocType = finergy.qb.DocType("DocType")
		query = finergy.qb.update(DocType).set(DocType.value, "some_value")
	...
	def test_case(self):
		DocType = finergy.qb.DocType("DocType")
		query = finergy.qb.from_(DocType).select(
			Case()
	...
	def test_case_in_update(self):
		DocType = finergy.qb.DocType("DocType")
		query = finergy.qb.update(DocType).set(
			"parent",
			Case()
			.when(DocType.search_fields == "value", "other_value")
			.when(Coalesce(DocType.search_fields == "subject_in_function"), "true_value")
...
class TestBuilderMaria(unittest.TestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual("SELECT * FROM `tabNotes`", finergy.qb.from_("Notes").select("*").get_sql())
		self.assertEqual("SELECT * FROM `__Auth`", finergy.qb.from_("__Auth").select("*").get_sql())
...
class TestBuilderPostgres(unittest.TestCase, TestBuilderBase):
	...
	def test_replace_tables(self):
		info_schema = finergy.qb.Schema("information_schema")
		self.assertEqual(
			'SELECT * FROM "pg_stat_all_tables"',
			finergy.qb.from_(info_schema.tables).select("*").get_sql(),
		)

	def test_replace_fields_post(self):
		self.assertEqual("relname", finergy.qb.Field("table_name").get_sql())
