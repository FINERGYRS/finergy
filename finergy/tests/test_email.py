...
class TestEmail(unittest.TestCase):
	...
	def test_flush(self):
		...
		self.assertTrue("test1@example.com" in queue_recipients)
		self.assertEqual(len(queue_recipients), 2)
		self.assertTrue("Unsubscribe" in finergy.safe_decode(finergy.flags.sent_mail))
	...
	def test_cc_footer(self):
		...
		self.assertTrue(
			...
			in finergy.safe_decode(finergy.flags.sent_mail)
		)
	...
	def test_expose(self):
		...
		email_obj = email.message_from_string(finergy.safe_decode(finergy.flags.sent_mail))
		for part in email_obj.walk():
			...
			if content:
				...
				finergy.local.flags.signed_query_string = re.search(
					r"(?<=/api/method/finergy.email.queue.unsubscribe\?).*(?=" + eol + ")", content.decode()
				).group(0)
	...
	def test_expired(self):
		self.test_email_queue()
		finergy.db.sql("UPDATE `tabEmail Queue` SET `modified`=(NOW() - INTERVAL '8' day)")

		from finergy.email.queue import set_expiry_for_email_queue
	...
	def test_unsubscribe(self):
		from finergy.email.queue import send, unsubscribe
		...
		before = finergy.db.sql("""select count(name) from `tabEmail Queue` where status='Not Sent'""")[
			0
		][0]
	...
	def test_image_parsing(self):
		...
		finergy.db.sql("""delete from `tabCommunication` where sender = 'sukh@yyy.com' """)

		with open(finergy.get_app_path("finergy", "tests", "data", "email_with_image.txt"), "r") as raw:
