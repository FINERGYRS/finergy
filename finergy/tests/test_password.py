...
import finergy
from finergy.utils.password import check_password, passlibctx, update_password


class TestPassword(unittest.TestCase):
	def setUp(self):
		finergy.delete_doc("Email Account", "Test Email Account Password")
		finergy.delete_doc("Email Account", "Test Email Account Password-new")
	...
	def make_email_account(self, name="Test Email Account Password"):
		if not finergy.db.exists("Email Account", name):
			...
			return finergy.get_doc("Email Account", name)
	...
	def test_hashed_password(self, user="test@example.com"):
		...
		self.assertRaises(finergy.AuthenticationError, check_password, user, new_password)
	...
	def test_password_on_rename_user(self):
		...
		finergy.rename_doc(doc.doctype, old_name, new_name)

		new_doc = finergy.get_doc(doc.doctype, new_name)
		self.assertEqual(new_doc.get_password(), password)
		self.assertTrue(not get_password_list(doc))

		finergy.rename_doc(doc.doctype, new_name, old_name)
