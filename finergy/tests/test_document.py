...
import finergy
from finergy.model.naming import make_autoname, parse_naming_series, revert_series_if_last
from finergy.utils import cint, set_request
...
class TestDocument(unittest.TestCase):
	def test_get_return_empty_list_for_table_field_if_none(self):
		d = finergy.get_doc({"doctype": "User"})
	...
	def test_load_single(self):
		d = finergy.get_doc("Website Settings", "Website Settings")
	...
	def test_insert(self):
		...
		self.assertEqual(finergy.db.get_value("Event", d.name, "subject"), "test-doc-test-event 1")
	...
	def test_insert_with_child(self):
		...
		d.insert()
		self.assertTrue(d.name.startswith("EV"))
		self.assertEqual(finergy.db.get_value("Event", d.name, "subject"), "test-doc-test-event 2")
	...
	def test_update(self):
		...
		self.assertEqual(finergy.db.get_value(d.doctype, d.name, "subject"), "subject changed")
	...
	def test_mandatory(self):
		...
		finergy.delete_doc_if_exists("User", "test_mandatory@example.com", 1)
		...
		self.assertRaises(finergy.MandatoryError, d.insert)

		d.set("first_name", "Test Mandatory")
		d.insert()
		self.assertEqual(finergy.db.get_value("User", d.name), d.name)

	def test_conflict_validation(self):
		d1 = self.test_insert()
		d2 = finergy.get_doc(d1.doctype, d1.name)
		d1.save()
		self.assertRaises(finergy.TimestampMismatchError, d2.save)

	def test_conflict_validation_single(self):
		d1 = finergy.get_doc("Website Settings", "Website Settings")
		d1.home_page = "test-web-page-1"

		d2 = finergy.get_doc("Website Settings", "Website Settings")
	...
	def test_permission(self):
		...
		self.assertRaises(finergy.PermissionError, self.test_insert)
		finergy.set_user("Administrator")

	def test_permission_single(self):
		finergy.set_user("Guest")
		d = finergy.get_doc("Website Settings", "Website Settings")
		self.assertRaises(finergy.PermissionError, d.save)
		finergy.set_user("Administrator")

	def test_link_validation(self):
		finergy.delete_doc_if_exists("User", "test_link_validation@example.com", 1)
		...
		self.assertRaises(finergy.LinkValidationError, d.insert)
	...
	def test_validate(self):
		...
		self.assertRaises(finergy.ValidationError, d.validate)
		self.assertRaises(finergy.ValidationError, d.run_method, "validate")
		self.assertRaises(finergy.ValidationError, d.save)

	def test_update_after_submit(self):
		d = self.test_insert()
		d.starts_on = "2014-09-09"
		self.assertRaises(finergy.UpdateAfterSubmitError, d.validate_update_after_submit)
	...
	def test_varchar_length(self):
		...
		self.assertRaises(finergy.CharacterLengthExceededError, d.save)
	...
	def test_naming_series(self):
		...
		for series in data:
			...
			old_current = finergy.db.get_value("Series", prefix, "current", order_by="name")

			revert_series_if_last(series, name)
			new_current = cint(finergy.db.get_value("Series", prefix, "current", order_by="name"))

			self.assertEqual(cint(old_current) - 1, new_current)

	def test_non_negative_check(self):
		finergy.delete_doc_if_exists("Currency", "Finergy Coin", 1)

		d = finergy.get_doc(
			{"doctype": "Currency", "currency_name": "Finergy Coin", "smallest_currency_fraction_value": -1}
		)

		self.assertRaises(finergy.NonNegativeError, d.insert)

		d.set("smallest_currency_fraction_value", 1)
		d.insert()
		self.assertEqual(finergy.db.get_value("Currency", d.name), d.name)
	...
	def test_get_formatted(self):
		...
		finergy.delete_doc_if_exists("Currency", "INR", 1)
		...
		d = finergy.get_doc({"doctype": "Test Formatted", "currency": 100000})
...
class TestDocumentWebView(unittest.TestCase):
	def get(self, path):
		from finergy.app import make_form_dict

		finergy.set_user("Guest")
		set_request(method="GET", path=path)
		make_form_dict(finergy.local.request)
	...
	def test_web_view_link_authentication(self):
		...
		# expire the key
		document_key_doc = finergy.get_doc("Document Share Key", {"key": document_key})
