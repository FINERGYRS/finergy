...
import finergy
from finergy.desk.search import get_names_for_mentions, search_link, search_widget
...
class TestSearch(unittest.TestCase):
	...
	def test_search_field_sanitizer(self):
		...
		result = finergy.response["results"][0]
	...
	def test_only_enabled_in_mention(self):
		...
		finergy.delete_doc("User", email)
		if not finergy.db.exists("User", email):
	...
	def test_link_field_order(self):
		...
		result = finergy.response["results"]
	...
	def test_link_search_in_foreign_language(self):
		try:
			...
			finergy.local.lang_full_dict = None  # discard translation cache
			search_widget(doctype="DocType", txt="pay", page_length=20)
			output = finergy.response["values"]
	...
	def test_validate_and_sanitize_search_inputs(self):
		...
		# should not fail if function is called via finergy.call with extra arguments
		args = ("Random DocType", "Random", "email", "2", "10", dict())
		kwargs = {"as_dict": False}
		self.assertListEqual(finergy.call("finergy.tests.test_search.get_data", *args, **kwargs), [])

		# should not fail if query has @ symbol in it
		search_link("User", "user@random", searchfield="name")
		self.assertListEqual(finergy.response["results"], [])
...
def setup_test_link_field_order(TestCase):
	...
	# Create Tree doctype
	TestCase.tree_doc = finergy.get_doc(
		{
			"doctype": "DocType",
