...
import finergy
from finergy.utils.minify import JavascriptMinify
...
def build_missing_files():
	...
	missing_assets = []
	current_asset_files = []
	finergy_build = os.path.join("..", "apps", "finergy", "finergy", "public", "build.json")
	...
	if missing_assets:
		...
		check_call(command, cwd=os.path.join("..", "apps", "finergy"))


def get_assets_link(finergy_head) -> str:
	tag = getoutput(
		r"cd ../apps/finergy && git show-ref --tags -d | grep %s | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{}//'" % finergy_head
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/finergyrs/finergy/releases/download/{tag}/assets.tar.gz"
	else:
		url = f"http://assets.finergyframework.com/{finergy_head}.tar.gz"

	if not head(url):
		reference = f"Release {tag}" if tag else f"Commit {finergy_head}"
		raise AssetsDontExistError(f"Assets for {reference} don't exist")

	return url


def fetch_assets(url, finergy_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="finergy-assets-", suffix=finergy_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	print(f"\n{green('✔')} Downloaded Finergy assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./finergy-bench/sites", "")
...
def download_finergy_assets(verbose=True):
	"""Downloads and sets up Finergy assets if they exist based on the current
	commit HEAD.
	Returns True if correctly setup else returns False.
	"""
	finergy_head = getoutput("cd ../apps/finergy && git rev-parse HEAD")

	if not finergy_head:
		return False

	try:
		url = get_assets_link(finergy_head)
		assets_archive = fetch_assets(url, finergy_head)
...
def setup():
	...
	for app in finergy.get_all_apps(True):
		try:
			pymodules.append(finergy.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(finergy.local.sites_path, "assets")
...
def bundle(no_compress, app=None, hard_link=False, verbose=False, skip_finergy=False):
	...
	if skip_finergy:
		command += " --skip_finergy"

	finergy_app_path = os.path.abspath(os.path.join(app_paths[0], ".."))
	check_yarn()
	finergy.commands.popen(command, cwd=finergy_app_path, env=get_node_env(), raise_err=True)
...
def watch(no_compress):
	...
	finergy_app_path = os.path.abspath(os.path.join(app_paths[0], ".."))
	check_yarn()
	finergy_app_path = finergy.get_app_path("finergy", "..")
	finergy.commands.popen(
		"{pacman} run watch".format(pacman=pacman), cwd=finergy_app_path, env=get_node_env()
	)
...
def generate_assets_map():
	...
	for app_name in finergy.get_all_apps():
		app_doc_path = None

		pymodule = finergy.get_module(app_name)
					...
					for target, sources in (json.loads(f.read() or "{}")).items():
						...
						for source in sources:
							if isinstance(source, list):
								s = finergy.get_pymodule_path(source[0], *source[1].split("/"))
							else:
...
def html_to_js_template(path, content):
	"""returns HTML template content as Javascript code, adding it to `finergy.templates`"""
	return """finergy.templates["{key}"] = '{content}';\n""".format(
