finergy.pages['permission-manager'].on_page_load = (wrapper) => {
	let page = finergy.ui.make_app_page({
		parent: wrapper,
	...
	$(finergy.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new finergy.PermissionEngine(wrapper);

};

finergy.pages['permission-manager'].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

finergy.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select
			= this.wrapper.page.add_select(__("Document Type"),
				...
				.change(function () {
					finergy.set_route("permission-manager", $(this).val());
				});
		...
		this.page.add_inner_button(__('Set User Permissions'), () => {
			return finergy.set_route('List', 'User Permission');
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (finergy.get_route()[1]) {
			this.doctype_select.val(finergy.get_route()[1]);
		} else if (finergy.route_options) {
			if (finergy.route_options.doctype) {
				this.doctype_select.val(finergy.route_options.doctype);
			}
			if (finergy.route_options.role) {
				this.role_select.val(finergy.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = finergy.confirm(__("Reset Permissions for {0}?", [doctype]), () => {
			return finergy.call({
				module: "finergy.core",
				page: "permission_manager",
				method: "reset",
		...
		let $d = $(d.wrapper).find(".finergy-confirm-message").append("<hr><h5>Standard Permissions:</h5><br>");
		let $wrapper = $("<p></p>").appendTo($d);
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(finergy.unscrub(r)));
				});
			...
			.click(function () {
				...
				finergy.call({
					...
					callback: function (r) {
						...
						finergy.msgprint(__("Users with role {0}:", [__(role)])
	...
	add_delete_button(row, d) {
		$(`<button class='btn btn-danger btn-remove-perm btn-xs'>${finergy.utils.icon('delete')}</button>`)
			...
			.on("click", () => {
				return finergy.call({
					...
					callback: (r) => {
						if (r.exc) {
							finergy.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					}
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			finergy.route_options = { allow: this.get_doctype() || "" };
			...
			() => {
				...
				d.set_primary_action(__('Add'), () => {
					...
					finergy.call({
						...
						callback: (r) => {
							if (r.exc) {
								finergy.msgprint(__("Did not add"));
							} else {
								this.refresh();
							}
						}
	...
	get_link_fields(doctype) {
		return finergy.get_children("DocType", doctype, "fields",
