finergy.pages["background_jobs"].on_page_load = (wrapper) => {
...
class BackgroundJobs {
	constructor(wrapper) {
		...
		this.remove_failed_button = this.page.add_inner_button(__("Remove Failed Jobs"), () => {
			finergy.call({
				method: 'finergy.core.page.background_jobs.background_jobs.remove_failed_jobs',
				callback: () => {
					this.refresh_jobs();
				}
			});
		...
		$(finergy.render_template('background_jobs_outer')).appendTo(this.page.body);
	...
	refresh_jobs() {
		...
		finergy.call({
			method: 'finergy.core.page.background_jobs.background_jobs.get_info',
			args: {
			...
			callback: (res) => {
				...
				$(finergy.render_template('background_jobs', { jobs: res.message || [] })).appendTo(this.content);

				if (finergy.get_route()[0] === 'background_jobs') {
