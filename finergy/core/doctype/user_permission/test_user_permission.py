...
from finergy.core.doctype.user_permission.user_permission import (
	add_user_permissions,
	remove_applicable,
)
from finergy.permissions import has_user_permission
...
class TestUserPermission(unittest.TestCase):
	def setUp(self):
		...
		finergy.delete_doc_if_exists("DocType", "Person")
		finergy.db.sql_ddl("DROP TABLE IF EXISTS `tabPerson`")
		finergy.delete_doc_if_exists("DocType", "Doc A")
		finergy.db.sql_ddl("DROP TABLE IF EXISTS `tabDoc A`")
	...
	def test_default_user_permission_validation(self):
		...
		self.assertRaises(finergy.ValidationError, add_user_permissions, param)
	...
	def test_default_user_permission_corectness(self):
		...
		add_user_permissions(param)
		finergy.db.delete("User Permission", filters={"for_value": test_blog.name})
		finergy.delete_doc("Blog Post", test_blog.name)
	...
	def test_default_user_permission(self):
		...
		for category in ["general", "public"]:
			if not finergy.db.exists("Blog Category", category):
				finergy.get_doc({"doctype": "Blog Category", "title": category}).insert()
		...
		finergy.set_user("test_user_perm1@example.com")
	...
	def test_for_applicable_on_update_from_apply_to_all(self):
		...
		removed_apply_to_all = finergy.db.exists("User Permission", get_exists_param(user))
		is_created_applicable_first = finergy.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		is_created_applicable_second = finergy.db.exists(
	...
	def test_for_apply_to_all_on_update_from_applicable(self):
		...
		is_created = add_user_permissions(param)
		is_created_apply_to_all = finergy.db.exists("User Permission", get_exists_param(user))
		removed_applicable_first = finergy.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		removed_applicable_second = finergy.db.exists(
			"User Permission", get_exists_param(user, applicable="Contact")
		)
	...
	def test_user_perm_for_nested_doctype(self):
		...
		if not finergy.db.exists("DocType", "Person"):
			doc = new_doctype(
				"Person",
		...
		parent_record = finergy.get_doc(
			{"doctype": "Person", "person_name": "Parent", "is_group": 1}
		).insert()

		child_record = finergy.get_doc(
			{
				"doctype": "Person",
				"person_name": "Child",
				"is_group": 0,
		...
		self.assertTrue(has_user_permission(finergy.get_doc("Person", parent_record.name), user.name))
		self.assertTrue(has_user_permission(finergy.get_doc("Person", child_record.name), user.name))

		finergy.db.set_value(
			"User Permission", {"allow": "Person", "for_value": parent_record.name}, "hide_descendants", 1
		)
		finergy.cache().delete_value("user_permissions")

		# check if adding perm on a group record with hide_descendants enabled,
		# hides child records
		self.assertTrue(has_user_permission(finergy.get_doc("Person", parent_record.name), user.name))
		self.assertFalse(has_user_permission(finergy.get_doc("Person", child_record.name), user.name))

	def test_user_perm_on_new_doc_with_field_default(self):
		"""Test User Perm impact on finergy.new_doc. with *field* default value"""
		finergy.set_user("Administrator")
		user = create_user("new_doc_test@example.com", "Blogger")

		# make a doctype "Doc A" with 'doctype' link field and default value ToDo
		if not finergy.db.exists("DocType", "Doc A"):
			doc = new_doctype(
				"Doc A",
				fields=[
					{
		...
		finergy.set_user("new_doc_test@example.com")

		new_doc = finergy.new_doc("Doc A")
	...
	def test_user_perm_on_new_doc_with_user_default(self):
		"""Test User Perm impact on finergy.new_doc. with *user* default value"""
		...
		if not finergy.db.exists("Session Default", {"ref_doctype": "DocType"}):
		...
		finergy.set_user("user_default_test@example.com")
...
def create_user(email, *roles):
	...
	if finergy.db.exists("User", email):
		return finergy.get_doc("User", email)
