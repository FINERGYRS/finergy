finergy.listview_settings['User Permission'] = {
		...
		list_view.page.add_inner_button( __("Add / Update"), function() {
			let dialog =new finergy.ui.Dialog({
				...
				primary_action: (data) => {
					...
					finergy.call({
						async: false,
						method: "finergy.core.doctype.user_permission.user_permission.add_user_permissions",
						args: {
							data : data
						},
						callback: function(r) {
							if(r.message === 1) {
								finergy.show_alert({message:__("User Permissions created sucessfully"), indicator:'blue'});
							} else {
								finergy.show_alert({message:__("Nothing to update"), indicator:'red'});
		...
		list_view.page.add_inner_button( __("Bulk Delete"), function() {
			const dialog = new finergy.ui.Dialog({
				...
				primary_action: (data) => {
					...
					finergy.confirm(__('Are you sure?'), () => {
						finergy
							.xcall('finergy.core.doctype.user_permission.user_permission.clear_user_permissions', data)
							.then(data => {
								dialog.hide();
								let message = '';
								if (data === 0) {
	...
	validate: function(dialog, data) {
		...
		if(data.apply_to_all_doctypes == 0 && !("applicable_doctypes" in data)) {
			finergy.throw(__("Please select applicable Doctypes"));
		}
		return data;
	},
	...
	get_applicable_doctype: function(dialog) {
		return new Promise(resolve => {
			finergy.call({
				method: 'finergy.core.doctype.user_permission.user_permission.check_applicable_doc_perm',
				async: false,
				args:{
	...
	get_multi_select_options: function(dialog, applicable){
		return new Promise(resolve => {
			finergy.call({
				method: 'finergy.desk.form.linked_with.get_linked_doctypes',
				async: false,
				args:{
					user: dialog.fields_dict.user.value,
					doctype: dialog.fields_dict.doctype.value,
	...
	on_doctype_change: function(dialog) {
		...
		let show = finergy.boot.nested_set_doctypes.includes(dialog.get_value("doctype"));
