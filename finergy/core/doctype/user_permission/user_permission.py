...
from finergy.desk.form.linked_with import get_linked_doctypes
from finergy.model.document import Document
from finergy.permissions import get_valid_perms, update_permission_property
...
class UserPermission(Document):
	...
	def on_update(self):
		finergy.cache().hdel("user_permissions", self.user)
		finergy.publish_realtime("update_user_permissions")
	...
	def validate_user_permission(self):
		...
		duplicate_exists = finergy.db.get_all(
			self.doctype,
			filters={
				"allow": self.allow,
	...
	def validate_default_permission(self):
		...
		if overlap_exists:
			ref_link = finergy.get_desk_link(self.doctype, overlap_exists[0].name)
			finergy.throw(_("{0} has already assigned default value for {1}.").format(ref_link, self.allow))
...
def get_user_permissions(user=None):
	...
	if finergy.request and finergy.local.form_dict.cmd == "get_user_permissions":
	...
	cached_user_permissions = finergy.cache().hget("user_permissions", user)
	...
	try:
		for perm in finergy.get_all(
			...
			meta = finergy.get_meta(perm.allow)
			add_doc_to_perm(perm, perm.for_value, perm.is_default)

			if meta.is_nested_set() and not perm.hide_descendants:
				decendants = finergy.db.get_descendants(perm.allow, perm.for_value)
				for doc in decendants:
					add_doc_to_perm(perm, doc, False)

		out = finergy._dict(out)
		finergy.cache().hset("user_permissions", user, out)
	except finergy.db.SQLError as e:
...
def clear_user_permissions(user, for_doctype):
	finergy.only_for("System Manager")
	total = finergy.db.count("User Permission", filters=dict(user=user, allow=for_doctype))
	if total:
		finergy.db.sql(
...
def add_user_permissions(data):
	...
	finergy.only_for("System Manager")
	if isinstance(data, finergy.string_types):
...
def insert_user_perm(
	...
	user_perm = finergy.new_doc("User Permission")
	user_perm.user = user
