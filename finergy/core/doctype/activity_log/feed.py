...
def update_feed(doc, method=None):
	if finergy.flags.in_patch or finergy.flags.in_install or finergy.flags.in_import:
		return
			...
			finergy.get_doc(
				{
					...
					"reference_owner": finergy.db.get_value(doctype, name, "owner"),
...
def logout_feed(user, reason):
	if user and user != "Guest":
		subject = _("{0} logged out: {1}").format(get_fullname(user), finergy.bold(reason))
...
def get_feed_match_conditions(user=None, doctype="Comment"):
	...
	conditions = [
		"`tab{doctype}`.owner={user} or `tab{doctype}`.reference_owner={user}".format(
			user=finergy.db.escape(user), doctype=doctype
		)
	]

	user_permissions = finergy.permissions.get_user_permissions(user)
	...
	if can_read_doctypes:
		...
		if user_permissions:
			...
			for dt, obj in user_permissions.items():
				for n in obj:
					can_read_docs.append("{}|{}".format(finergy.db.escape(dt), finergy.db.escape(n.get("doc", ""))))
