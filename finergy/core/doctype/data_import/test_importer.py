...
import finergy
from finergy.core.doctype.data_import.importer import Importer
from finergy.tests.test_query_builder import db_type_is, run_only_if
from finergy.utils import format_duration, getdate
...
class TestImporter(unittest.TestCase):
	...
	def test_data_import_from_file(self):
		...
		doc1 = finergy.get_doc(doctype_name, "Test")
		doc2 = finergy.get_doc(doctype_name, "Test 2")
		doc3 = finergy.get_doc(doctype_name, "Test 3")
	...
	def test_data_import_without_mandatory_values(self):
		...
		data_import.start_import()
		data_import.reload()
		import_log = finergy.parse_json(data_import.import_log)
		self.assertEqual(import_log[0]["row_indexes"], [2, 3])
		expected_error = (
			"Error: <b>Child 1 of DocType for Import</b> Row #1: Value missing for: Child Title"
		)
		self.assertEqual(finergy.parse_json(import_log[0]["messages"][0])["message"], expected_error)
		expected_error = (
			"Error: <b>Child 1 of DocType for Import</b> Row #2: Value missing for: Child Title"
		)
		self.assertEqual(finergy.parse_json(import_log[0]["messages"][1])["message"], expected_error)

		self.assertEqual(import_log[1]["row_indexes"], [4])
		self.assertEqual(finergy.parse_json(import_log[1]["messages"][0])["message"], "Title is required")

	def test_data_import_update(self):
		existing_doc = finergy.get_doc(
			doctype=doctype_name,
			title=finergy.generate_hash(doctype_name, 8),
		...
		updated_doc = finergy.get_doc(doctype_name, existing_doc.name)
	...
	def get_importer(self, doctype, import_file, update=False):
		data_import = finergy.new_doc("Data Import")
...
def create_doctype_if_not_exists(doctype_name, force=False):
	if force:
		finergy.delete_doc_if_exists("DocType", doctype_name)
		finergy.delete_doc_if_exists("DocType", "Child 1 of " + doctype_name)
		finergy.delete_doc_if_exists("DocType", "Child 2 of " + doctype_name)

	if finergy.db.exists("DocType", doctype_name):
		return
...
def get_import_file(csv_file_name, force=False):
	...
	_file = finergy.db.exists("File", {"file_name": file_name})
	if force and _file:
		finergy.delete_doc_if_exists("File", _file)

	if finergy.db.exists("File", {"file_name": file_name}):
		f = finergy.get_doc("File", {"file_name": file_name})
	else:
		full_path = get_csv_file_path(file_name)
		f = finergy.get_doc(
			doctype="File", content=finergy.read_file(full_path), file_name=file_name, is_private=1
		)
...
def get_csv_file_path(file_name):
	return finergy.get_app_path("finergy", "core", "doctype", "data_import", "fixtures", file_name)
