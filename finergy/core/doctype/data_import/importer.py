...
from finergy.core.doctype.version.version import get_diff
from finergy.model import no_value_fields
from finergy.model import table_fields as table_fieldtypes
from finergy.utils import cint, cstr, duration_to_seconds, flt, update_progress_bar
from finergy.utils.csvutils import get_csv_content_from_google_sheets, read_csv_content
from finergy.utils.xlsxutils import (
...
class Importer:
	def __init__(self, doctype, data_import=None, file_path=None, import_type=None, console=False):
		...
		if not self.data_import:
			self.data_import = finergy.get_doc(doctype="Data Import")
			if import_type:
				self.data_import.import_type = import_type

		self.template_options = finergy.parse_json(self.data_import.template_options or "{}")
	...
	def before_import(self):
		...
		finergy.cache().hdel("lang", finergy.session.user)
		finergy.set_user_lang(finergy.session.user)

		# set flags
		finergy.flags.in_import = True
		finergy.flags.mute_emails = self.data_import.mute_emails
	...
	def import_data(self):
		...
		if self.data_import.import_log:
			import_log = finergy.parse_json(self.data_import.import_log)
		else:
			import_log = []
		...
		for log in import_log:
			log = finergy._dict(log)
			if log.success:
				imported_rows += log.row_indexes

		# start import
		total_payload_count = len(payloads)
		batch_size = finergy.conf.data_import_batch_size or 1000

		for batch_index, batched_payloads in enumerate(finergy.utils.create_batch(payloads, batch_size)):
			for i, payload in enumerate(batched_payloads):
				...
				try:
					...
					import_log.append(finergy._dict(success=True, docname=doc.name, row_indexes=row_indexes))
					...
					import_log.append(
						finergy._dict(
							success=False,
							exception=finergy.get_traceback(),
							messages=finergy.local.message_log,
							row_indexes=row_indexes,
						)
					)
	...
	def insert_record(self, doc):
		meta = finergy.get_meta(self.doctype)
		new_doc = finergy.new_doc(self.doctype)
		new_doc.update(doc)
	...
	def update_record(self, doc):
		...
		existing_doc = finergy.get_doc(self.doctype, doc.get(id_field.fieldname))

		updated_doc = finergy.get_doc(self.doctype, doc.get(id_field.fieldname))
		...
		if get_diff(existing_doc, updated_doc):
			...
			# throw if no changes
			finergy.throw("No changes to update")
	...
	def export_errored_rows(self):
		...
		import_log = finergy.parse_json(self.data_import.import_log or "[]")
		failures = [log for log in import_log if not log.get("success")]
		row_indexes = []
		for f in failures:
	...
	def print_import_log(self, import_log):
		...
		if failed_records:
			...
			file_name = "{0}_import_on_{1}.txt".format(self.doctype, finergy.utils.now())
...
class ImportFile:
	def __init__(self, doctype, file, template_options=None, import_type=None):
		self.doctype = doctype
		self.template_options = template_options or finergy._dict(column_to_field_map=finergy._dict())
		...
		if isinstance(file, finergy.string_types):
			if finergy.db.exists("File", {"file_url": file}):
				self.file_doc = finergy.get_doc("File", {"file_url": file})
		...
		if not self.file_doc and not self.file_path and not self.google_sheets_url:
			finergy.throw(_("Invalid template file for import"))
	...
	def get_data_from_template_file(self):
		...
		if not content:
			finergy.throw(_("Invalid or corrupted content for import"))
	...
	def get_data_for_import_preview(self):
		...
		columns = [finergy._dict({"header_title": "Sr. No", "skip_import": True})]
	...
	def get_payloads_for_import(self):
		...
		while data:
			...
			payloads.append(finergy._dict(doc=doc, rows=rows))
		return payloads
	...
	def read_content(self, content, extension):
		...
		if extension not in ("csv", "xlsx", "xls"):
			finergy.throw(_("Import template should be of type .csv, .xlsx or .xls"), title=error_title)
...
class Row:
	...
	def _parse_doc(self, doctype, columns, values, parent_doc=None, table_df=None):
		...
		for key in finergy.model.default_fields + ("__islocal",):
			doc.pop(key, None)
		...
		is_table = finergy.get_meta(doctype).istable
		...
		if is_table and is_update:
			...
			if id_value and finergy.db.exists(doctype, id_value):
				existing_doc = finergy.get_doc(doctype, id_value)
				existing_doc.update(doc)
				doc = existing_doc
			else:
				# for table rows being inserted in update
				# create a new doc with defaults set
				new_doc = finergy.new_doc(doctype, as_dict=True)
				new_doc.update(doc)
				doc = new_doc
	...
	def validate_value(self, value, col):
		...
		if df.fieldtype == "Select":
			...
			if select_options and value not in select_options:
				options_string = ", ".join([finergy.bold(d) for d in select_options])
			...
			if not exists:
				msg = _("Value {0} missing for {1}").format(finergy.bold(value), finergy.bold(df.options))
				self.warnings.append(
					{
						"row": self.row_number,
						"field": df_as_json(df),
			...
			if isinstance(value, finergy.string_types):
				...
				self.warnings.append(
					{
						...
						"message": _("Value {0} must in {1} format").format(
							finergy.bold(value), finergy.bold(get_user_format(col.date_format))
						),
					}
				)
				return
	...
	def link_exists(self, value, df):
		...
		if Row.link_values_exist_map.get(key) is None:
			Row.link_values_exist_map[key] = finergy.db.exists(df.options, value)
...
class Header(Row):
	def __init__(self, index, row, doctype, raw_data, column_to_field_map=None):
		...
		self.data = row
		self.doctype = doctype
		column_to_field_map = column_to_field_map or finergy._dict()
...
class Column:
	...
	def __init__(self, index, header, doctype, column_values, map_to_field=None, seen=None):
		...
		self.meta = finergy.get_meta(doctype)
		self.parse()
	...
	def parse(self):
				...
				self.warnings.append(
					{
						"message": _("Mapping column {0} to field {1}").format(
							finergy.bold(header_title or "<i>Untitled Column</i>"), finergy.bold(df.label)
						),
						"type": "info",
					}
				)
		...
		if header_title in self.seen:
			self.warnings.append(
				{
					"col": column_number,
					"message": _("Skipping Duplicate Column {0}").format(finergy.bold(header_title)),
					"type": "info",
				}
			)
			df = None
			...
			self.warnings.append(
				{
					"col": column_number,
					"message": _("Skipping column {0}").format(finergy.bold(header_title)),
					"type": "info",
				}
			)
			...
			self.warnings.append(
				{
					"col": column_number,
					"message": _("Cannot match column {0} with any field").format(finergy.bold(header_title)),
					"type": "info",
				}
			)
		...
		def guess_date_format(d):
			if isinstance(d, (datetime, date, time)):
				...
				if self.df.fieldtype == "Time":
					return "%H:%M:%S"
			if isinstance(d, str):
				return finergy.utils.guess_date_format(d)
			...
			self.warnings.append(
				{
					...
					"message": message.format(
						finergy.bold(self.header_title),
						len(unique_date_formats),
						finergy.bold(user_date_format),
					),
					"type": "info",
				}
			)
	...
	def validate_values(self):
		...
		if self.df.fieldtype == "Link":
			...
			exists = [d.name for d in finergy.db.get_all(self.df.options, filters={"name": ("in", values)})]
			...
			if options:
				...
				if invalid:
					valid_values = ", ".join([finergy.bold(o) for o in options])
					invalid_values = ", ".join([finergy.bold(i) for i in invalid])
					self.warnings.append(
						{
							"col": self.column_number,
							"message": (
...
def build_fields_dict_for_column_matching(parent_doctype):
	...
	parent_meta = finergy.get_meta(parent_doctype)
	out = {}
	...
	for doctype, table_df in doctypes:
		...
		fields = get_standard_fields(doctype) + finergy.get_meta(doctype).fields
		for df in fields:
...
def get_df_for_column_header(doctype, header):
	...
	df_by_labels_and_fieldname = finergy.cache().hget(
...
def get_id_field(doctype):
	...
	return finergy._dict({"label": "ID", "fieldname": "name", "fieldtype": "Data"})
