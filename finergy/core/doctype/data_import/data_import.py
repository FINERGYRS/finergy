...
from finergy.core.doctype.data_import.exporter import Exporter
...
from finergy.utils.csvutils import validate_google_sheets_url
...
def get_preview_from_template(data_import, import_file=None, google_sheets_url=None):
	return finergy.get_doc("Data Import", data_import).get_preview_from_template(
...
def form_start_import(data_import):
	return finergy.get_doc("Data Import", data_import).start_import()


def start_import(data_import):
	"""This method runs in background job"""
	data_import = finergy.get_doc("Data Import", data_import)
	try:
		...
		finergy.log_error(title=data_import.name)
	finally:
		finergy.flags.in_import = False

	finergy.publish_realtime("data_import_refresh", {"data_import": data_import.name})
...
def download_template(
	...
	export_fields = finergy.parse_json(export_fields)
	export_filters = finergy.parse_json(export_filters)
...
def download_errored_template(data_import_name):
	data_import = finergy.get_doc("Data Import", data_import_name)
...
def validate_csv_import_file(path):
	if path.endswith(".csv"):
		...
		print("Or use the method finergy.core.doctype.data_import.data_import.import_file")
		print()
