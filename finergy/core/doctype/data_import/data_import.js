...
finergy.ui.form.on('Data Import', {
	setup(frm) {
		finergy.realtime.on('data_import_refresh', ({ data_import }) => {
			frm.import_in_progress = false;
			if (data_import !== frm.doc.name) return;
			finergy.model.clear_doc('Data Import', frm.doc.name);
			finergy.model.with_doc('Data Import', frm.doc.name).then(() => {
				frm.refresh();
			});
		});
		finergy.realtime.on('data_import_progress', data => {
		...
		frm.set_query('reference_doctype', () => {
			return {
				filters: {
					name: ['in', finergy.boot.user.can_import]
				}
			};
		});
	...
	refresh(frm) {
		...
		if (frm.doc.status.includes('Success')) {
			frm.add_custom_button(
				...
				() => finergy.set_route('List', frm.doc.reference_doctype)
			);
		}
	},
	...
	update_indicators(frm) {
		const indicator = finergy.get_indicator(frm.doc);
		if (indicator) {
				...
				.then(result => {
					if (result.length > 0) {
						frm.add_custom_button('Report Error', () => {
							...
							finergy.request.report_error(fake_xhr, {});
						});
					}
				});
		}
	...
	download_template(frm) {
		finergy.require('/assets/js/data_import_tools.min.js', () => {
			frm.data_exporter = new finergy.data_import.DataExporter(
	...
	toggle_submit_after_import(frm) {
		...
		if (doctype) {
			finergy.model.with_doctype(doctype, () => {
				let meta = finergy.get_meta(doctype);
	...
	show_import_preview(frm, preview_data) {
		...
		finergy.require('/assets/js/data_import_tools.min.js', () => {
			frm.import_preview = new finergy.data_import.ImportPreview({
	...
	export_errored_rows(frm) {
		open_url_post(
			'/api/method/finergy.core.doctype.data_import.data_import.download_errored_template',
			{
				...
				if (log.success) {
					if (frm.doc.import_type === 'Insert New Records') {
						html = __('Successfully imported {0}', [
							`<span class="underline">${finergy.utils.get_form_link(
								frm.doc.reference_doctype,
								log.docname,
								true
							)}<span>`
					...
					let messages = log.messages
						...
						.join('');
					let id = finergy.dom.get_unique_id();
					html = `${messages}
