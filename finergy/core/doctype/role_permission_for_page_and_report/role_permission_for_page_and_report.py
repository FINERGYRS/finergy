...
import finergy
from finergy.core.doctype.report.report import is_prepared_report_disabled
...
class RolePermissionforPageandReport(Document):
	...
	def set_custom_roles(self):
		...
		name = finergy.db.get_value("Custom Role", args, "name")
		if name:
			doc = finergy.get_doc("Custom Role", name)
			roles = doc.roles
		else:
	...
	def update_custom_roles(self):
		...
		if self.report:
			args.update({"ref_doctype": finergy.db.get_value("Report", self.report, "ref_doctype")})

		if name:
			custom_role = finergy.get_doc("Custom Role", name)
	...
	def update_disable_prepared_report(self):
		if self.report:
			# intentionally written update query in finergy.db.sql instead of finergy.db.set_value
			finergy.db.sql(
