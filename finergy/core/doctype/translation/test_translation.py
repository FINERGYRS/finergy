...
class TestTranslation(unittest.TestCase):
	def setUp(self):
		finergy.db.sql("delete from tabTranslation")

	def tearDown(self):
		finergy.local.lang = "en"
		finergy.local.lang_full_dict = None

	def test_doctype(self):
		translation_data = get_translation_data()
		for key, val in translation_data.items():
			finergy.local.lang = key
			finergy.local.lang_full_dict = None
			translation = create_translation(key, val)
			self.assertEqual(_(val[0]), val[1])

			finergy.delete_doc("Translation", translation.name)
			finergy.local.lang_full_dict = None
...
def create_translation(key, val):
	translation = finergy.new_doc("Translation")
