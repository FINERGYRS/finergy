...
from finergy.translate import get_translator_url
...
def create_translations(translation_map, language):
	...
	translation_map_to_send = finergy._dict({})
	# first create / update local user translations
	for source_id, translation_dict in translation_map.items():
		translation_dict = finergy._dict(translation_dict)
		existing_doc_name = finergy.db.get_all(
			"Translation",
			{
	...
	params = {
		"language": language,
		"contributor_email": finergy.session.user,
		"contributor_name": finergy.utils.get_fullname(finergy.session.user),
	...
	for local_docname, remote_docname in added_translations.items():
		finergy.db.set_value("Translation", local_docname, "contribution_docname", remote_docname)


def clear_user_translation_cache(lang):
	finergy.cache().hdel("lang_user_translations", lang)
