...
from finergy.automation.doctype.assignment_rule.assignment_rule import (
	apply as apply_assignment_rule,
)
from finergy.contacts.doctype.contact.contact import get_contact_name
from finergy.core.doctype.comment.comment import update_comment_in_doc
from finergy.core.doctype.communication.email import _notify, notify, validate_email
from finergy.core.utils import get_parent_doc
from finergy.model.document import Document
from finergy.utils import cstr, parse_addr, strip_html, time_diff_in_seconds, validate_email_address
from finergy.utils.bot import BotReply
from finergy.utils.user import is_system_user
...
class Communication(Document):
	...
	def onload(self):
		...
		if (
			...
			email_flag_queue = finergy.db.get_value(
	...
	def validate_reference(self):
		if self.reference_doctype and self.reference_name:
			if not self.reference_owner:
				self.reference_owner = finergy.db.get_value(
					self.reference_doctype, self.reference_name, "owner"
				)

			# prevent communication against a child table
			if finergy.get_meta(self.reference_doctype).istable:
				finergy.throw(
	...
	def after_insert(self):
		...
		if self.reference_doctype == "Communication" and self.sent_or_received == "Sent":
			finergy.db.set_value("Communication", self.reference_name, "status", "Replied")

		if self.communication_type == "Communication":
			self.notify_change("add")

		elif self.communication_type in ("Chat", "Notification", "Bot"):
			if self.reference_name == finergy.session.user:
				message = self.as_dict()
				message["broadcast"] = True
				finergy.publish_realtime("new_message", message, after_commit=True)
			else:
	...
	def set_signature_in_email_content(self):
		...
		signature = user_email_signature or finergy.db.get_value(
			"Email Account",
	...
	def set_status(self):
		...
		email_rule = finergy.db.get_value("Email Rule", {"email_id": self.sender, "is_spam": 1})
		if (
			self.communication_type == "Communication"
			and self.communication_medium == "Email"
	...
	def set_sender_full_name(self):
		if not self.sender_full_name and self.sender:
			if self.sender == "Administrator":
				self.sender_full_name = finergy.db.get_value("User", "Administrator", "full_name")
				self.sender = finergy.db.get_value("User", "Administrator", "email")
				...
				if not self.sender_full_name:
					self.sender_full_name = finergy.db.get_value("User", self.sender, "full_name")

				if not self.sender_full_name:
					first_name, last_name = finergy.db.get_value(
	...
	def set_delivery_status(self, commit=False):
		...
		status_counts = Counter(
			finergy.db.sql_list("""select status from `tabEmail Queue` where communication=%s""", self.name)
		)
	...
	def set_timeline_links(self):
		contacts = []
		create_contact_enabled = self.email_account and finergy.db.get_value(
...
def on_doctype_update():
	...
	finergy.db.add_index("Communication", ["reference_doctype", "reference_name"])
	finergy.db.add_index("Communication", ["status", "communication_type"])
...
def has_permission(doc, ptype, user):
	if ptype == "read":
		...
		if doc.reference_doctype and doc.reference_name:
			if finergy.has_permission(doc.reference_doctype, ptype="read", doc=doc.reference_name):
				return True
...
def get_contacts(email_strings: List[str], auto_create_contact=False) -> List[str]:
	...
	for email in email_addrs:
		...
		if not contact_name and email and auto_create_contact:
			...
			first_name = finergy.unscrub(email_parts[0])
...
def add_contact_links_to_communication(communication, contact_name):
	contact_links = finergy.get_list(
		"Dynamic Link",
...
def parse_email(communication, email_strings):
	...
	"""
	if not finergy.get_all("Email Account", filters={"enable_automatic_linking": 1}):
		return
		...
		if email_string:
			for email in email_string.split(","):
				if delimiter in email:
					...
					if doctype and docname and finergy.db.exists(doctype, docname):
...
def set_avg_response_time(parent, communication):
	if parent.meta.has_field("avg_response_time") and communication.sent_or_received == "Sent":
		...
		communications = finergy.get_list(
			"Communication",
