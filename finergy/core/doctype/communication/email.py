...
import finergy
import finergy.email.smtp
from finergy import _
from finergy.core.utils import get_parent_doc
from finergy.email.email_body import get_message_id
...
def make(
	...
	**kwargs,
	...
	if kwargs:
		...
		warn(
			f"Options {kwargs} used in finergy.core.doctype.communication.email.make "
			"are deprecated or unsupported",
			category=DeprecationWarning,
		)

	if doctype and name and not finergy.has_permission(doctype=doctype, ptype="email", doc=name):
		raise finergy.PermissionError(f"You are not allowed to send emails related to: {doctype} {name}")
...
def _make(
	...
	sender = sender or get_formatted_email(finergy.session.user)
	...
	if cint(send_email):
		...
		_ = finergy.email.smtp.get_outgoing_email_account(append_to=comm.doctype, sender=comm.sender)
		finergy.flags.print_letterhead = cint(print_letterhead)
...
def notify(
	...
	if finergy.flags.in_test:
		...
		enqueue(
			...
			session=finergy.local.session,
			print_letterhead=finergy.flags.print_letterhead,
		)
...
def prepare_to_notify(doc, print_html=None, print_format=None, attachments=None):
	...
	view_link = finergy.utils.cint(
		finergy.db.get_value("System Settings", "System Settings", "attach_view_link")
	)
		...
		for a in attachments:
			if isinstance(a, string_types):
				...
				try:
					...
					file_id = finergy.db.get_all("File", or_filters={"file_name": a, "name": a}, limit=1)
					if not file_id:
						finergy.throw(_("Unable to find attachment {0}").format(a))
					file_id = file_id[0]["name"]
					_file = finergy.get_doc("File", file_id)
					_file.get_content()
...
def set_incoming_outgoing_accounts(doc):
	...
	if not doc.incoming_email_account and doc.sender:
		doc.incoming_email_account = finergy.db.get_value(
	...
	doc.outgoing_email_account = finergy.email.smtp.get_outgoing_email_account(
...
def get_recipients(doc, fetched_from_email_account=False):
	...
	# doc.previous_email_sender = finergy.db.get_value("Communication", doc.in_reply_to, "sender")
	# recipients.append(doc.previous_email_sender)
...
def get_cc(doc, recipients=None, fetched_from_email_account=False):
	...
	if cc:
		...
		exclude += [
			d[0] for d in finergy.db.get_all("User", ["email"], {"thread_notify": 0}, as_list=True)
		]
...
def filter_email_list(doc, email_list, exclude, is_cc=False, is_bcc=False):
	...
	for email in list(set(email_list)):
		...
		if is_cc:
			is_user_enabled = finergy.db.get_value("User", email_address, "enabled")
...
def get_attach_link(doc, print_format):
	...
	return finergy.get_template("templates/emails/print_link.html").render(
		{
			"url": get_url(),
			"doctype": doc.reference_doctype,
			"name": doc.reference_name,
...
def sendmail(
	...
	try:
		...
		if session:
			...
			session["data"] = finergy._dict(session["data"])
			finergy.local.session.update(session)

		if print_letterhead:
			finergy.flags.print_letterhead = print_letterhead

		# upto 3 retries
		for i in range(3):
			try:
				communication = finergy.get_doc("Communication", communication_name)
		...
		traceback = finergy.log_error("finergy.core.doctype.communication.email.sendmail")
		raise


@finergy.whitelist(allow_guest=True, methods=("GET",))
def mark_email_as_seen(name: str = None):
	try:
		update_communication_as_read(name)
		finergy.db.commit()  # nosemgrep: this will be called in a GET request

	except Exception:
		finergy.log_error(finergy.get_traceback())

	finally:
		finergy.response.update(
			{
				"type": "binary",
...
def update_communication_as_read(name):
	...
	communication = finergy.db.get_value("Communication", name, "read_by_recipient", as_dict=True)
