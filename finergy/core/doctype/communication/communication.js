finergy.ui.form.on("Communication", {
	onload: function(frm) {
		if(frm.doc.content) {
			frm.doc.content = finergy.dom.remove_script_and_style(frm.doc.content);
		}
	...
	refresh: function(frm) {
		...
		if(frm.doc.reference_doctype && frm.doc.reference_name) {
			frm.add_custom_button(__(frm.doc.reference_name), function() {
				finergy.set_route("Form", frm.doc.reference_doctype, frm.doc.reference_name);
			});
		} else {
	...
	show_relink_dialog: function(frm) {
		var d = new finergy.ui.Dialog ({
			...
			fields: [{
				...
				"get_query": function() {return {"query": "finergy.email.get_communication_doctype"}}
			},
			{
	...
	show_move_dialog: function(frm) {
		var d = new finergy.ui.Dialog ({
			...
			primary_action(values) {
				d.hide();
				finergy.call({
					method: "finergy.email.inbox.move_email",
					args: {
	...
	mark_as_read_unread: function(frm) {
		...
		return finergy.call({
			method: "finergy.email.inbox.create_email_flag_queue",
			args: {
				'names': [frm.doc.name],
				'action': action,
				'flag': flag
	...
	mark_as_closed_open: function(frm) {
		...
		return finergy.call({
			method: "finergy.email.inbox.mark_as_closed_open",
			args: {
				communication: frm.doc.name,
				status: status
			},
	...
	get_mail_args: function(frm) {
		var sender_email_id = ""
		$.each(finergy.boot.email_accounts, function(idx, account) {
	...
	add_to_contact: function(frm) {
		...
		finergy.new_doc("Contact")
	},

	mark_as_spam: function(frm) {
		finergy.call({
			method: "finergy.email.inbox.mark_as_spam",
			args: {
				communication: frm.doc.name,
			...
			freeze: true,
			callback: function(r) {
				finergy.msgprint(__("Email has been marked as spam"))
			}
		})
	},
	...
	move_to_trash: function(frm) {
		finergy.call({
			method: "finergy.email.inbox.mark_as_trash",
			args: {
				communication: frm.doc.name
			},
			freeze: true,
			callback: function(r) {
				finergy.msgprint(__("Email has been moved to trash"))
			}
		})
	}
});
