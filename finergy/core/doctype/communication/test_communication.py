...
import finergy
from finergy.core.doctype.communication.communication import get_emails
from finergy.email.doctype.email_queue.email_queue import EmailQueue

test_records = finergy.get_test_records("Communication")
...
class TestCommunication(unittest.TestCase):
	def test_email(self):
		...
		for x in valid_email_list:
			self.assertTrue(finergy.utils.parse_addr(x)[1])

		for x in invalid_email_list:
			self.assertFalse(finergy.utils.parse_addr(x)[0])
	...
	def test_name(self):
		...
		for x in valid_email_list:
			self.assertTrue(finergy.utils.parse_addr(x)[0])
	...
	def test_circular_linking(self):
		...
		a = finergy.get_doc("Communication", a.name)
		a.reference_doctype = "Communication"
		a.reference_name = c.name

		self.assertRaises(finergy.CircularLinkingError, a.save)

	def test_deduplication_timeline_links(self):
		finergy.delete_doc_if_exists("Note", "deduplication timeline links")

		note = finergy.get_doc(
			{
				"doctype": "Note",
	...
	def test_contacts_attached(self):
		contact_sender = finergy.get_doc(
			{
				"doctype": "Contact",
		...
		contact_recipient = finergy.get_doc(
			{
				"doctype": "Contact",
	...
	def test_get_communication_data(self):
		from finergy.desk.form.load import get_communication_data

		finergy.delete_doc_if_exists("Note", "get communication data")

		note = finergy.get_doc(
			{"doctype": "Note", "title": "get communication data", "content": "get communication data"}
		).insert(ignore_permissions=True)

		comm_note_1 = finergy.get_doc(
			{
				"doctype": "Communication",
				"communication_type": "Communication",
	...
	def test_link_in_email(self):
		finergy.delete_doc_if_exists("Note", "test document link in email")
...
def create_email_account():
	finergy.delete_doc_if_exists("Email Account", "_Test Comm Account 1")

	finergy.flags.mute_emails = False
	finergy.flags.sent_mail = None

	email_account = finergy.get_doc(
		{
			"is_default": 1,
			"is_global": 1,
