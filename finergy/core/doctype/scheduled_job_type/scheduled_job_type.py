...
from finergy.utils import get_datetime, now_datetime
...
class ScheduledJobType(Document):
	...
	def enqueue(self, force=False):
		...
		if self.is_event_due() or force:
			if finergy.flags.enqueued_jobs:
				finergy.flags.enqueued_jobs.append(self.method)

			if finergy.flags.execute_job:
				self.execute()
			else:
				if not self.is_job_in_queue():
					enqueue(
						"finergy.core.doctype.scheduled_job_type.scheduled_job_type.run_scheduled_job",
	...
	def is_job_in_queue(self):
		queued_jobs = get_jobs(site=finergy.local.site, key="job_type")[finergy.local.site]
	...
	def get_next_execution(self):
		CRON_MAP = {
			...
			"Hourly": "0 * * * *",
			"Hourly Long": "0 * * * *",
			"All": "0/" + str((finergy.get_conf().scheduler_interval or 240) // 60) + " * * * *",
		}
	...
	def execute(self):
		...
		try:
			...
			if self.server_script:
				script_name = finergy.db.get_value("Server Script", self.server_script)
				if script_name:
					finergy.get_doc("Server Script", script_name).execute_scheduled_method()
			else:
				finergy.get_attr(self.method)()
			finergy.db.commit()
	...
	def log_status(self, status):
		# log file
		finergy.logger("scheduler").info(f"Scheduled Job {status}: {self.method} for {finergy.local.site}")
	...
	def on_trash(self):
		finergy.db.sql("delete from `tabScheduled Job Log` where scheduled_job_type=%s", self.name)
...
def execute_event(doc: str):
	...
	doc = json.loads(doc)
	finergy.get_doc("Scheduled Job Type", doc.get("name")).enqueue(force=True)
	return doc
...
def run_scheduled_job(job_type: str):
	...
	try:
		finergy.get_doc("Scheduled Job Type", dict(method=job_type)).execute()
	except Exception:
...
def sync_jobs(hooks: Dict = None):
	...
	scheduler_events = hooks or finergy.get_hooks("scheduler_events")
...
def clear_events(all_events: List):
	for event in finergy.get_all("Scheduled Job Type", fields=["name", "method", "server_script"]):
		...
		if not (is_defined_in_hooks or is_server_script):
			finergy.delete_doc("Scheduled Job Type", event.name)
