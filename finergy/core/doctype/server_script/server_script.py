...
from finergy.utils.safe_exec import NamespaceDict, get_safe_globals, safe_exec
...
class ServerScript(Document):
	...
	def on_trash(self):
		if self.script_type == "Scheduler Event":
			for job in self.scheduled_jobs:
				finergy.delete_doc("Scheduled Job Type", job.name)
	...
	def sync_scheduled_jobs(self):
		...
		for scheduled_job in self.scheduled_jobs:
			if bool(scheduled_job.stopped) != bool(self.disabled):
				job = finergy.get_doc("Scheduled Job Type", scheduled_job.name)
	...
	def clear_scheduled_events(self):
		...
		if self.script_type == "Scheduler Event" and self.has_value_changed("event_frequency"):
			for scheduled_job in self.scheduled_jobs:
				finergy.delete_doc("Scheduled Job Type", scheduled_job.name)

	def execute_method(self) -> Dict:
		"""Specific to API endpoint Server Scripts

		Raises:
		        finergy.DoesNotExistError: If self.script_type is not API
		        finergy.PermissionError: If self.allow_guest is unset for API accessed by Guest user

		Returns:
		        dict: Evaluates self.script with finergy.utils.safe_exec.safe_exec and returns the flags set in it's safe globals
		"""
		# wrong report type!
	...
	def execute_scheduled_method(self):
		...
		Raises:
		        finergy.DoesNotExistError: If script type is not a scheduler event
		"""
		if self.script_type != "Scheduler Event":
			raise finergy.DoesNotExistError
	...
	def get_autocompletion_items(self):
		...
		Returns:
		        ...
		        For e.g., ["finergy.utils.cint", "finergy.db.get_all", ...]
		"""
		...
		items = finergy.cache().get_value("server_script_autocompletion_items")
		if not items:
			items = get_keys(get_safe_globals())
			items = [{"value": d[0], "score": d[1]} for d in items]
			finergy.cache().set_value("server_script_autocompletion_items", items)
		return items
...
def setup_scheduler_events(script_name, frequency):
	...
	Args:
	        ...
	        frequency (str): Event label compatible with the Finergy scheduler
	"""
	method = finergy.scrub(f"{script_name}-{frequency}")
	scheduled_script = finergy.db.get_value("Scheduled Job Type", {"method": method})
	...
	if not scheduled_script:
		...
		finergy.msgprint(_("Enabled scheduled execution for script {0}").format(script_name))

	else:
		doc = finergy.get_doc("Scheduled Job Type", scheduled_script)
		...
		finergy.msgprint(_("Scheduled execution for script {0} has updated").format(script_name))
