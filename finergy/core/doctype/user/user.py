...
import finergy.permissions
import finergy.share
from finergy import _, msgprint, throw
from finergy.core.doctype.user_type.user_type import user_linked_with_permission_on_doctype
...
from finergy.utils.password import check_password, get_password_reset_limit
from finergy.utils.password import update_password as _update_password
from finergy.utils.user import get_system_managers
from finergy.website.utils import is_signup_disabled
...
class User(Document):
	...
	def after_insert(self):
		...
		finergy.cache().delete_key("users_for_mentions")
		finergy.cache().delete_key("enabled_users")
	...
	def validate(self):
		...
		if (self.name not in ["Administrator", "Guest"]) and (
			not self.get_social_login_userid("finergy")
		):
			self.set_social_login_userid("finergy", finergy.generate_hash(length=39))

	def validate_roles(self):
		if self.role_profile_name:
			role_profile = finergy.get_doc("Role Profile", self.role_profile_name)
			self.set("roles", [])
			self.append_roles(*[role.role for role in role_profile.roles])

	def validate_allowed_modules(self):
		if self.module_profile:
			module_profile = finergy.get_doc("Module Profile", self.module_profile)
	...
	def validate_user_image(self):
		if self.user_image and len(self.user_image) > 2000:
			finergy.throw(_("Not a valid User Image."))

	def on_update(self):
		# clear new password
		self.share_with_self()
		clear_notifications(user=self.name)
		finergy.clear_cache(user=self.name)
		now = finergy.flags.in_test or finergy.flags.in_install
		self.send_password_notification(self.__new_password)
		finergy.enqueue(
			"finergy.core.doctype.user.user.create_contact", user=self, ignore_mandatory=True, now=now
		)
		if self.name not in ("Administrator", "Guest") and not self.user_image:
			finergy.enqueue("finergy.core.doctype.user.user.update_gravatar", name=self.name, now=now)

		# Set user selected timezone
		if self.time_zone:
			finergy.defaults.set_default("time_zone", self.time_zone, self.name)
	...
	def has_website_permission(self, ptype, user, verbose=False):
		...
		return self.name == finergy.session.user
	...
	def check_enable_disable(self):
		...
		if not cint(self.enabled) and self.name in STANDARD_USERS:
			finergy.throw(_("User {0} cannot be disabled").format(self.name))

		if not cint(self.enabled):
			self.a_system_manager_should_exist()

		# clear sessions if disabled
		if not cint(self.enabled) and getattr(finergy.local, "login_manager", None):
			finergy.local.login_manager.logout(user=self.name)
	...
	def add_system_manager_role(self):
		...
		if (
			...
			and cint(finergy.db.get_single_value("System Settings", "setup_complete"))
		):
	...
	def set_system_user(self):
		...
		if self.user_type and not finergy.get_cached_value("User Type", self.user_type, "is_standard"):
	...
	def set_roles_and_modules_based_on_user_type(self):
		user_type_doc = finergy.get_cached_doc("User Type", self.user_type)
	...
	def validate_share(self, docshare):
		...
		# 		if docshare.share != 1:
		# 			finergy.throw(_("Sorry! User should have complete access to their own record."))
		# 	else:
		# 		finergy.throw(_("Sorry! Sharing with Website User is prohibited."))
	...
	def send_password_notification(self, new_password):
		...
		except finergy.OutgoingEmailError:
	...
	def reset_password(self, send_email=False, password_expired=False):
		from finergy.utils import get_url, random_string
	...
	def send_welcome_mail_to_user(self):
		...
		subject = None
		method = finergy.get_hooks("welcome_email")
		if method:
			subject = finergy.get_attr(method[-1])()
		if not subject:
			site_name = finergy.db.get_default("site_name") or finergy.get_conf().get("site_name")
			if site_name:
	...
	def send_login_mail(self, subject, template, add_args, now=None):
		...
		from finergy.utils import get_url
		from finergy.utils.user import get_user_fullname

		created_by = get_user_fullname(finergy.session["user"])
		if created_by == "Guest":
		...
		sender = (
			finergy.session.user not in STANDARD_USERS and get_formatted_email(finergy.session.user) or None
		)
	...
	def on_trash(self):
		...
		self.enabled = 0
		if getattr(finergy.local, "login_manager", None):
			finergy.local.login_manager.logout(user=self.name)

		# delete todos
		finergy.db.sql("""DELETE FROM `tabToDo` WHERE `owner`=%s""", (self.name,))
		finergy.db.sql("""UPDATE `tabToDo` SET `assigned_by`=NULL WHERE `assigned_by`=%s""", (self.name,))
		...
		# delete shares
		finergy.db.sql("""delete from `tabDocShare` where user=%s""", self.name)
		...
		finergy.delete_doc("Notification Settings", self.name, ignore_permissions=True)
		...
		finergy.db.delete("User Permission", {"user": self.name})

	def before_rename(self, old_name, new_name, merge=False):
		finergy.clear_cache(user=old_name)
	...
	def after_rename(self, old_name, new_name, merge=False):
		tables = finergy.db.get_tables()
		for tab in tables:
			desc = finergy.db.get_table_columns_description(tab)
			has_fields = []
			for d in desc:
		...
		if finergy.db.exists("Notification Settings", old_name):
			finergy.rename_doc("Notification Settings", old_name, new_name, force=True, show_alert=False)
	...
	def remove_disabled_roles(self):
		disabled_roles = [d.name for d in finergy.get_all("Role", filters={"disabled": 1})]
	...
	def validate_username(self):
		if not self.username and self.is_new() and self.first_name:
			self.username = finergy.scrub(self.first_name)
		...
		if self.username_exists():
			if self.user_type == "System User":
				finergy.msgprint(_("Username {0} already exists").format(self.username))
	...
	def suggest_username(self):
		...
		# @firstname
		username = _check_suggestion(finergy.scrub(self.first_name))

		if not username:
			# @firstname_last_name
			username = _check_suggestion(
				finergy.scrub("{0} {1}".format(self.first_name, self.last_name or ""))
			)

		if username:
			finergy.msgprint(_("Suggested Username: {0}").format(username))
	...
	def validate_user_email_inbox(self):
		...
		if len(email_accounts) != len(set(email_accounts)):
			finergy.throw(_("Email Account added multiple times"))
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(
			finergy.db.get_value("System Settings", "System Settings", "allow_login_using_mobile_number")
		)
		login_with_username = cint(
			finergy.db.get_value("System Settings", "System Settings", "allow_login_using_user_name")
		)
		...
		users = finergy.db.get_all("User", fields=["name", "enabled"], or_filters=or_filters, limit=1)
		if not users:
			return
		...
		if validate_password:
			...
			except finergy.AuthenticationError:
...
def get_roles(arg=None):
	...
	return finergy.get_roles(finergy.form_dict["uid"])


@finergy.whitelist()
def get_perm_info(role):
	"""get permission info"""
	from finergy.permissions import get_all_perms
...
def update_password(new_password, logout_all_sessions=0, key=None, old_password=None):
	...
	if key and not isinstance(key, str):
		finergy.throw(_("Invalid key type"))
	...
	if res.get("message"):
		finergy.local.response.http_status_code = 410
		return res["message"]
	else:
		user = res["user"]

	logout_all_sessions = cint(logout_all_sessions) or finergy.db.get_single_value(
	...
	redirect_to = finergy.cache().hget("redirect_after_login", user)
	if redirect_to:
		redirect_url = redirect_to
		finergy.cache().hdel("redirect_after_login", user)

	finergy.local.login_manager.login_as(user)

	finergy.db.set_value("User", user, "last_password_reset_date", today())
	finergy.db.set_value("User", user, "reset_password_key", "")
...
def test_password_strength(new_password, key=None, old_password=None, user_data=None):
	from finergy.utils.password_strength import test_password_strength as _test_password_strength
	...
	if not user_data:
		user_data = finergy.db.get_value(
			"User", finergy.session.user, ["first_name", "middle_name", "last_name", "email", "birth_date"]
		)
...
def has_email_account(email):
	return finergy.get_list("Email Account", filters={"email_id": email})


@finergy.whitelist(allow_guest=False)
def get_email_awaiting(user):
	waiting = finergy.db.sql(
...
def ask_pass_update():
	...
	from finergy.utils import set_default
...
def _get_user_for_update_password(key, old_password):
	# verify old password
	result = finergy._dict()
	if key:
		result.user = finergy.db.get_value("User", {"reset_password_key": key})
		if not result.user:
			result.message = _("The Link specified has either been used before or Invalid")

	elif old_password:
		# verify old password
		finergy.local.login_manager.check_password(finergy.session.user, old_password)
...
def reset_user_data(user):
	user_doc = finergy.get_doc("User", user)
...
def verify_password(password):
	finergy.local.login_manager.check_password(finergy.session.user, password)


@finergy.whitelist(allow_guest=True)
def sign_up(email, full_name, redirect_to):
	if is_signup_disabled():
		finergy.throw(_("Sign Up is disabled"), title=_("Not Allowed"))

	user = finergy.db.get("User", {"email": email})
	if user:
		if user.enabled:
		...
		if finergy.db.get_creation_count("User", 60) > 300:
		...
		default_role = finergy.db.get_value("Portal Settings", None, "default_role")
		if default_role:
			user.add_roles(default_role)

		if redirect_to:
			finergy.cache().hset("redirect_after_login", user.name, redirect_to)
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	from finergy.desk.reportview import get_filters_cond, get_match_cond
	...
	""".format(
			...
			standard_users=", ".join([finergy.db.escape(u) for u in STANDARD_USERS]),
			key=searchfield,
...
def get_system_users(exclude_users=None, limit=None):
	...
	system_users = finergy.db.sql_list(
...
def get_permission_query_conditions(user):
	if user == "Administrator":
		...
		return """(`tabUser`.name not in ({standard_users}))""".format(
			standard_users=", ".join(finergy.db.escape(user) for user in STANDARD_USERS)
		)
...
def notify_admin_access_to_system_manager(login_manager=None):
	if (
		...
		and finergy.local.conf.notify_admin_access_to_system_manager
	):

		site = '<a href="{0}" target="_blank">{0}</a>'.format(finergy.local.request.host_url)
		date_and_time = "<b>{0}</b>".format(format_datetime(now_datetime(), format_string="medium"))
		ip_address = finergy.local.request_ip
...
def extract_mentions(txt):
	...
	for mention in soup.find_all(class_="mention"):
		if mention.get("data-is-group") == "true":
			try:
				user_group = finergy.get_cached_doc("User Group", mention["data-id"])
...
def handle_password_test_fail(result):
	...
	finergy.throw(" ".join([_("Invalid Password:"), warning, suggestions]))


def update_gravatar(name):
	gravatar = has_gravatar(name)
	if gravatar:
		finergy.db.set_value("User", name, "user_image", gravatar)
...
def throttle_user_creation():
	...
	if finergy.db.get_creation_count("User", 60) > finergy.local.conf.get("throttle_user_limit", 60):
		finergy.throw(_("Throttled"))


@finergy.whitelist()
def get_role_profile(role_profile):
	roles = finergy.get_doc("Role Profile", {"role_profile": role_profile})
	return roles.roles


@finergy.whitelist()
def get_module_profile(module_profile):
	module_profile = finergy.get_doc("Module Profile", {"module_profile_name": module_profile})
	return module_profile.get("block_modules")


def create_contact(user, ignore_links=False, ignore_mandatory=False):
	from finergy.contacts.doctype.contact.contact import get_contact_name
	...
	if not contact_name:
		...
		contact = finergy.get_doc("Contact", contact_name)
...
def generate_keys(user):
	...
	user_details = finergy.get_doc("User", user)
	api_secret = finergy.generate_hash(length=15)
	# if api key is not set generate api key
	if not user_details.api_key:
		api_key = finergy.generate_hash(length=15)
...
def switch_theme(theme):
	if theme in ["Dark", "Light"]:
		finergy.db.set_value("User", finergy.session.user, "desk_theme", theme)


def get_enabled_users():
	def _get_enabled_users():
		enabled_users = finergy.get_all("User", filters={"enabled": "1"}, pluck="name")
		return enabled_users

	return finergy.cache().get_value("enabled_users", _get_enabled_users)
