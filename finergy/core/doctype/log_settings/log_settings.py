...
class LogSettings(Document):
	...
	def clear_activity_logs(self):
		from finergy.core.doctype.activity_log.activity_log import clear_activity_logs

		clear_activity_logs(days=self.clear_activity_log_after)

	def clear_email_queue(self):
		from finergy.email.queue import clear_outbox

		clear_outbox(days=self.clear_email_queue_after)


def run_log_clean_up():
	doc = finergy.get_doc("Log Settings")
	doc.clear_logs()
...
def has_unseen_error_log(user):
	...
	if finergy.db.sql_list("select name from `tabError Log` where seen = 0 limit 1"):
		log_settings = finergy.get_cached_doc("Log Settings")
