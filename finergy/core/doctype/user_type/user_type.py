...
from finergy.config import get_modules_from_app
from finergy.model.document import Document
from finergy.permissions import add_permission, add_user_permission
...
class UserType(Document):
	...
	def on_trash(self):
		if self.is_standard:
			finergy.throw(_("Standard user type {0} can not be deleted.").format(finergy.bold(self.name)))

	def set_modules(self):
		if not self.user_doctypes:
			return

		modules = finergy.get_all(
			"DocType",
	...
	def validate_document_type_limit(self):
		limit = finergy.conf.get("user_type_doctype_limit", {}).get(finergy.scrub(self.name))

		if not limit and finergy.session.user != "Administrator":
			finergy.throw(
				_("User does not have permission to create the new {0}").format(finergy.bold(_("User Type"))),
	...
	def validate_role(self):
		if not self.role:
			finergy.throw(_("The field {0} is mandatory").format(finergy.bold(_("Role"))))

		if not finergy.db.get_value("Role", self.role, "is_custom"):
			finergy.throw(
				_("The role {0} should be a custom role.").format(
					finergy.bold(get_link_to_form("Role", self.role))
				)
			)

	def update_users(self):
		for row in finergy.get_all("User", filters={"user_type": self.name}):
			user = finergy.get_cached_doc("User", row.name)
	...
	def add_role_permissions_for_user_doctypes(self):
		...
		for row in self.user_doctypes:
			...
			finergy.db.set_value("Custom DocPerm", docperm, values)

	def add_select_perm_doctypes(self):
		if finergy.flags.ignore_select_perm:
			return
	...
	def add_role_permissions_for_file(self):
		...
		finergy.db.set_value("Custom DocPerm", docperm, {"read": 1, "create": 1, "write": 1})
	...
	def remove_permission_for_deleted_doctypes(self):
		...
		for perm in finergy.get_all(
			"Custom DocPerm", filters={"role": self.role, "parent": ["not in", doctypes]}
		):
			finergy.delete_doc("Custom DocPerm", perm.name)


def add_role_permissions(doctype, role):
	name = finergy.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=0))
...
def get_non_standard_user_type_details():
	...
	if user_types:
		...
		finergy.cache().set_value("non_standard_user_types", user_type_details)
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
	modules = [d.get("module_name") for d in get_modules_from_app("finergy")]
	...
	custom_doctypes = finergy.get_all(
...
def user_linked_with_permission_on_doctype(doc, user):
	...
	if not doc.user_id_field:
		finergy.throw(_("User Id Field is mandatory in the user type {0}").format(finergy.bold(doc.name)))

	if finergy.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "name"):
		return True
	else:
		label = finergy.get_meta(doc.apply_user_permission_on).get_field(doc.user_id_field).label

		finergy.msgprint(
			_(
				"To set the role {0} in the user {1}, kindly set the {2} field as {3} in one of the {4} record."
			).format(
				finergy.bold(doc.role),
				finergy.bold(user),
				finergy.bold(label),
				finergy.bold(user),
				finergy.bold(doc.apply_user_permission_on),
			)
		)
...
def apply_permissions_for_non_standard_user_type(doc, method=None):
	...
	if not finergy.db.table_exists("User Type"):
		return

	user_types = finergy.cache().get_value("non_standard_user_types")
	...
	for user_type, data in iteritems(user_types):
		...
		if doc.get(data[1]) and (
			...
			if not perm_data:
				...
				finergy.db.set_value("User Permission", perm_data[0], "user", doc.get(data[1]))
