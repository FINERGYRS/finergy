...
finergy.ui.form.on('User Type', {
	refresh: function(frm) {
		frm.toggle_display('is_standard', finergy.boot.developer_mode);
		frm.set_df_property('is_standard', 'read_only', !finergy.boot.developer_mode);
		...
		frm.set_query('apply_user_permission_on', function() {
			return {
				query: "finergy.core.doctype.user_type.user_type.get_user_linked_doctypes"
			};
		});
	},
	...
	get_user_id_fields: function(frm) {
		if (frm.doc.apply_user_permission_on) {
			finergy.call({
				method: 'finergy.core.doctype.user_type.user_type.get_user_id',
				args: {
