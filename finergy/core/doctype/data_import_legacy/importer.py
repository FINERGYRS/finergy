...
def upload(
	...
	# for translations
	if user:
		finergy.cache().hdel("lang", user)
		finergy.set_user_lang(user)

	if data_import_doc and isinstance(data_import_doc, string_types):
		data_import_doc = finergy.get_doc("Data Import Legacy", data_import_doc)
	if data_import_doc and from_data_import == "Yes":
		...
		# extra input params
		params = json.loads(finergy.form_dict.get("params") or "{}")
	...
	finergy.flags.mute_emails = no_email
	...
	def check_data_length():
		if not data:
			finergy.throw(_("No data found in the file. Please reattach the new file with data."))
	...
	def get_doc(start_idx):
		if doctypes:
			...
			doc = finergy._dict(zip(columns, rows[start_idx][1:]))
			doc["doctype"] = doctype
			return doc, [], None
	...
	def validate_naming(doc):
		autoname = finergy.get_meta(doctype).autoname
		if autoname:
			...
			if (autoname not in doc) or (not doc[autoname]):
				...
				if not hasattr(get_controller(doctype), "autoname"):
					finergy.throw(_("{0} is a mandatory field").format(autoname))
		return True

	users = finergy.db.sql_list("select name from tabUser")

	def prepare_for_insert(doc):
		# don't block data import if user is not set
		# migrating from another system
		if not doc.owner in users:
			doc.owner = finergy.session.user
		if not doc.modified_by in users:
			doc.modified_by = finergy.session.user
	...
	if not rows:
		_file = finergy.get_doc("File", {"file_url": data_import_doc.import_file})
		...
		if file_extension == ".xlsx" and from_data_import == "Yes":
			...
			finergy.throw(_("Unsupported File Format"))
	...
	try:
		...
		finergy.throw(_("Cannot change header content"))
	doctypes = []
	...
	if submit_after_import and not cint(finergy.db.get_value("DocType", doctype, "is_submittable")):
	...
	# check permissions
	if not finergy.permissions.can_import(parenttype or doctype):
	...
	for batch_start in range(0, total, batch_size):
		...
		for i, row in enumerate(batch):
			...
			try:
				...
				original = None
				if parentfield:
					parent = finergy.get_doc(parenttype, doc["parent"])
					doc = parent.append(parentfield, doc)
					parent.save()
				else:
					if overwrite and doc.get("name") and finergy.db.exists(doctype, doc["name"]):
						original = finergy.get_doc(doctype, doc["name"])
				...
				if finergy.local.message_log:
					err_msg = "\n".join(
						[
							...
							for msg in finergy.local.message_log
						]
					)
				else:
					err_msg = '<p class="border-bottom small">{}</p>'.format(cstr(e))

				error_trace = finergy.get_traceback()
				if error_trace:
					error_log_doc = finergy.log_error(error_trace)
				...
				log(
					**{
						"row": row_idx + 1,
						"title": "Error for row %s" % (len(row) > 1 and finergy.safe_decode(row[1]) or ""),
						"message": err_msg,
						"indicator": "red",
						"link": error_link,
					}
...
def get_parent_field(doctype, parenttype):
	...
	# get parentfield
	if parenttype:
		for d in finergy.get_meta(parenttype).get_table_fields():
			if d.options == doctype:
				parentfield = d.fieldname
				break

		if not parentfield:
			finergy.msgprint(_("Did not find {0} for {0} ({1})").format("parentfield", parenttype, doctype))
			raise Exception
...
def delete_child_rows(rows, doctype):
	...
	for p in list(set([r[1] for r in rows])):
		if p:
			finergy.db.sql("""delete from `tab{0}` where parent=%s""".format(doctype), p)
