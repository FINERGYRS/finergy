...
finergy.ui.form.on('Data Import Legacy', {
	onload: function(frm) {
		...
		finergy.call({
			method: "finergy.core.doctype.data_import_legacy.data_import_legacy.get_importable_doctypes",
			callback: function (r) {
		...
		finergy.realtime.on("data_import_progress", function(data) {
	...
	reference_doctype: function(frm){
		if (frm.doc.reference_doctype) {
			finergy.model.with_doctype(frm.doc.reference_doctype);
		}
	},
	...
	refresh: function(frm) {
		...
		if (frm.doc.reference_doctype && !frm.doc.import_file) {
			...
			if (frm.doc.import_status) {
				const listview_settings = finergy.listview_settings['Data Import Legacy'];
		...
		frm.add_custom_button(__("Help"), function() {
			finergy.help.show_video("6wiriRKPhmg");
		});

		if (frm.doc.reference_doctype && frm.doc.docstatus === 0) {
			frm.add_custom_button(__("Download template"), function() {
				finergy.data_import.download_dialog(frm).show();
			});
		}
	...
	action: function(frm) {
		...
		if(!frm.doc.reference_doctype) {
			finergy.msgprint(__("Please select document type first."));
			frm.set_value("action", "");
			return;
		}
	...
	create_log_table: function(frm) {
		...
		$(finergy.render_template("log_details", {
			data: msg.messages,
...
finergy.provide('finergy.data_import');
finergy.data_import.download_dialog = function(frm) {
	var dialog;
	const filter_fields = df => finergy.model.is_value_type(df) && !df.hidden;
	const get_fields = dt => finergy.meta.get_docfields(dt).filter(filter_fields);
	...
	const child_table_fields = finergy.meta.get_table_fields(frm.doc.reference_doctype)
		.map(df => {
			return {
				"label": df.options,
				"fieldname": df.fieldname + '_fields',
				"fieldtype": "MultiCheck",
				"options": finergy.meta.get_docfields(df.options)
	...
	dialog = new finergy.ui.Dialog({
		...
		primary_action: function(values) {
			...
			if (frm.doc.reference_doctype) {
				...
				let get_template_url = '/api/method/finergy.core.doctype.data_export.exporter.export_data';
				open_url_post(get_template_url, export_params());
			} else {
				finergy.msgprint(__("Please select the Document Type."));
			}
			dialog.hide();
		},
	...
	$(button_container).find('.finergy-control').map((index, button) => {
