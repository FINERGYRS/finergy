finergy.listview_settings["Deleted Document"] = {
	onload: function (doclist) {
		const action = () => {
			...
			if (selected_docs.length > 0) {
				...
				finergy.call({
					method: "finergy.core.doctype.deleted_document.deleted_document.bulk_restore",
					args: { docnames },
					callback: function (r) {
						if (r.message) {
							let body = (docnames) => {
							...
							finergy.msgprint(summary, __("Document Restoration Summary"), true);
