...
def send_sms(receiver_list, msg, sender_name="", success_msg=True):
	...
	arg = {
		"receiver_list": receiver_list,
		"message": finergy.safe_decode(msg).encode("utf-8"),
		"success_msg": success_msg,
	}

	if finergy.db.get_value("SMS Settings", None, "sms_gateway_url"):
...
def send_via_gateway(arg):
	...
	message = finergy.safe_decode(arg.get("message"))
	...
	if len(success_list) > 0:
		...
		if arg.get("success_msg"):
			finergy.msgprint(_("SMS sent to following numbers: {0}").format("\n" + "\n".join(success_list)))


def get_headers(sms_settings=None):
	if not sms_settings:
		sms_settings = finergy.get_doc("SMS Settings", "SMS Settings")
