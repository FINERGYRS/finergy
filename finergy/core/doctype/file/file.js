finergy.ui.form.on("File", "refresh", function(frm) {
	...
	var is_viewable = finergy.utils.is_image_file(frm.doc.file_url);
	...
	if(frm.doc.file_name && frm.doc.file_name.split('.').splice(-1)[0]==='zip') {
		frm.add_custom_button(__('Unzip'), function() {
			finergy.call({
				method: "finergy.core.doctype.file.file.unzip_file",
				args: {
					name: frm.doc.name,
				},
				callback: function() {
					finergy.set_route('List', 'File');
				}
			});
		});
	}
