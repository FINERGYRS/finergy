...
import finergy
from finergy import _, conf, safe_decode
...
from finergy.utils.image import strip_exif_data
...
class MaxFileSizeReachedError(finergy.ValidationError):
	pass


class FolderNotEmpty(finergy.ValidationError):
	pass
...
class File(Document):
	...
	def before_insert(self):
		finergy.local.rollback_observers.append(self)
	...
	def autoname(self):
		...
		if self.is_folder:
			...
			self.name = finergy.generate_hash("", 10)
				...
				_("Added {0}").format(
					"<a href='{file_url}' target='_blank'>{file_name}</a>{icon}".format(
						**{
							...
							"file_url": quote(finergy.safe_encode(self.file_url)) if self.file_url else self.file_name,
	...
	def get_successor(self):
		return finergy.db.get_values(doctype="File", filters={"folder": self.name}, fieldname="name")
	...
	def validate(self):
		...
		self.file_size = finergy.form_dict.file_size or self.file_size
	...
	def validate_url(self):
		...
		if not self.file_url.startswith(("/files/", "/private/files/")):
			finergy.throw(_("URL must start with http:// or https://"), title=_("Invalid URL"))
		...
		if not os.path.realpath(self.get_full_path()).startswith(base_path):
			finergy.throw(_("The File URL you've entered is incorrect"), title=_("Invalid File URL"))

	def handle_is_private_changed(self):
		if not finergy.db.exists("File", {"name": self.name, "is_private": cint(not self.is_private)}):
			return

		old_file_url = self.file_url

		file_name = self.file_url.split("/")[-1]
		private_file_path = finergy.get_site_path("private", "files", file_name)
		public_file_path = finergy.get_site_path("public", "files", file_name)
	...
	def fetch_attached_to_field(self, old_file_url):
		...
		reference_dict = finergy.get_doc(self.attached_to_doctype, self.attached_to_name).as_dict()
	...
	def validate_attachment_limit(self):
		...
		if self.attached_to_doctype and self.attached_to_name:
			attachment_limit = cint(finergy.get_meta(self.attached_to_doctype).max_attachments)
			...
			if current_attachment_count >= attachment_limit:
				finergy.throw(
					_("Maximum Attachment Limit of {0} has been reached for {1} {2}.").format(
						finergy.bold(attachment_limit), self.attached_to_doctype, self.attached_to_name
					),
					exc=finergy.exceptions.AttachmentLimitReached,
	...
	def set_folder_name(self):
		...
		if self.attached_to_doctype and not self.folder:
			self.folder = finergy.db.get_value("File", {"is_attachments_folder": 1})
	...
	def validate_file(self):
		...
		if not os.path.exists(full_path):
			finergy.throw(_("File {0} does not exist").format(self.file_url), IOError)
	...
	def validate_duplicate_entry(self):
		if not self.flags.ignore_duplicate_entry_error and not self.is_folder:
			...
			duplicate_file = finergy.db.get_value("File", filters, ["name", "file_url"], as_dict=1)

			if duplicate_file:
				duplicate_file_doc = finergy.get_cached_doc("File", duplicate_file.name)
	...
	def generate_content_hash(self):
		...
		try:
			...
			finergy.throw(_("File {0} does not exist").format(self.file_url))

	def on_trash(self):
		if self.is_home_folder or self.is_attachments_folder:
			finergy.throw(_("Cannot delete Home and Attachments folders"))
	...
	def make_thumbnail(
		...
		if self.file_url:
			...
			path = os.path.abspath(finergy.get_site_path("public", thumbnail_url.lstrip("/")))
			...
			try:
				...
				finergy.msgprint(_("Unable to write file format for {0}").format(path))
				return
	...
	def check_folder_is_empty(self):
		...
		files = finergy.get_all("File", filters={"folder": self.name}, fields=("name", "file_name"))

		if self.is_folder and files:
			finergy.throw(_("Folder {0} is not empty").format(self.name), FolderNotEmpty)
	...
	def call_delete_file(self):
		...
		if (
			...
			and (
				not finergy.db.count("File", {"content_hash": self.content_hash, "name": ["!=", self.name]})
			)
		):
	...
	def unzip(self):
		...
		if not self.file_url.endswith(".zip"):
			finergy.throw(_("{0} is not a zip file").format(self.file_name))
		...
		finergy.delete_doc("File", self.name)
		return files
	...
	def get_content(self):
		...
		if self.is_folder:
			finergy.throw(_("Cannot get file contents of a Folder"))
	...
	def get_full_path(self):
		...
		if file_path.startswith("/private/files/"):
			...
			finergy.throw(_("There is some problem with the file url: {0}").format(file_path))
	...
	def write_file(self):
		...
		if os.path.sep in self.file_name:
			finergy.throw(_("File name cannot have {0}").format(os.path.sep))

		# create directory (if not exists)
		finergy.create_folder(file_path)
		# write the file
	...
	def save_file(self, content=None, decode=False, ignore_existing_file_check=False):
		...
		if (
			...
			and finergy.get_system_settings("strip_exif_metadata_from_uploaded_images")
		):
		...
		if not ignore_existing_file_check:
			duplicate_file = finergy.get_value(
				"File",
		...
		if duplicate_file:
			file_doc = finergy.get_cached_doc("File", duplicate_file.name)
	...
	def add_comment_in_reference_doc(self, comment_type, text):
		if self.attached_to_doctype and self.attached_to_name:
			try:
				doc = finergy.get_doc(self.attached_to_doctype, self.attached_to_name)
...
def on_doctype_update():
	finergy.db.add_index("File", ["attached_to_doctype", "attached_to_name"])


def make_home_folder():
	home = finergy.get_doc(
...
def move_file(file_list, new_parent, old_parent):
	...
	# recalculate sizes
	finergy.get_doc("File", old_parent).save()
	finergy.get_doc("File", new_parent).save()


def setup_folder_path(filename, new_parent):
	file = finergy.get_doc("File", filename)
...
def get_local_image(file_url):
	...
	file_path = finergy.get_site_path(*file_url_path)

	try:
		image = Image.open(file_path)
	except IOError:
		finergy.throw(_("Unable to read file format for {0}").format(file_url))
...
def get_web_image(file_url: str) -> Tuple["ImageFile", str, str]:
	# download
	file_url = finergy.utils.get_url(file_url)
	r = requests.get(file_url, stream=True)
	try:
		r.raise_for_status()
	except HTTPError:
		if r.status_code == 404:
			finergy.msgprint(_("File '{0}' not found").format(file_url))
		else:
			finergy.msgprint(_("Unable to read file format for {0}").format(file_url))
		raise

	try:
		image = Image.open(BytesIO(r.content))
	except Exception as e:
		finergy.msgprint(_("Image link '{0}' is not valid").format(file_url), raise_exception=e)
...
def delete_file(path):
	...
	if path:
		...
		if parts[0] == "files":
			path = finergy.utils.get_site_path("public", "files", parts[-1])

		else:
			path = finergy.utils.get_site_path("private", "files", parts[-1])
...
def has_permission(doc, ptype=None, user=None):
	...
	if ptype == "create":
		has_access = finergy.has_permission("File", "create", user=user)
	...
	if doc.attached_to_doctype and doc.attached_to_name:
		...
		try:
			ref_doc = finergy.get_doc(attached_to_doctype, attached_to_name)
...
def remove_file_by_url(file_url, doctype=None, name=None):
	if doctype and name:
		fid = finergy.db.get_value(
			"File", {"file_url": file_url, "attached_to_doctype": doctype, "attached_to_name": name}
		)
	else:
		fid = finergy.db.get_value("File", {"file_url": file_url})

	if fid:
		from finergy.utils.file_manager import remove_file
...
def download_file(file_url):
	...
	Method : GET
	Endpoint : finergy.core.doctype.file.file.download_file
	URL Params : file_name = /path/to/file relative to site path
	"""
	file_doc = finergy.get_doc("File", {"file_url": file_url})
	file_doc.check_permission("read")

	finergy.local.response.filename = os.path.basename(file_url)
	finergy.local.response.filecontent = file_doc.get_content()
	finergy.local.response.type = "download"


def extract_images_from_doc(doc, fieldname):
	content = doc.get(fieldname)
	content = extract_images_from_html(doc, content)
	if finergy.flags.has_dataurl:
		doc.set(fieldname, content)


def extract_images_from_html(doc, content, is_private=False):
	finergy.flags.has_dataurl = False
	...
	def _save_file(match):
		...
		finergy.flags.has_dataurl = True
...
def unzip_file(name):
	...
	file_obj = finergy.get_doc("File", name)
...
def attach_files_to_document(doc, event):
	...
	for df in attach_fields:
		...
		try:
			...
			finergy.log_error(title=_("Error Attaching File"))
