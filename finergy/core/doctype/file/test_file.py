...
from finergy.core.doctype.file.file import (
	File,
	get_attached_images,
	get_files_in_folder,
	move_file,
...
from finergy.utils import get_files_path
...
class TestSimpleFile(unittest.TestCase):
	...
	def test_save(self):
		_file = finergy.get_doc("File", {"file_url": self.saved_file_url})
...
class TestSameFileName(unittest.TestCase):
	def test_saved_content(self):
		...
		_file = finergy.get_doc("File", {"file_url": self.saved_file_url1})
		content1 = _file.get_content()
		self.assertEqual(content1, self.test_content1)
		_file = finergy.get_doc("File", {"file_url": self.saved_file_url2})
	...
	def test_saved_content_private(self):
		...
		_file = finergy.get_doc("File", {"file_url": _file1.file_url})
		self.assertEqual(_file.get_content(), test_content1)

		_file = finergy.get_doc("File", {"file_url": _file2.file_url})
...
class TestSameContent(unittest.TestCase):
	...
	def test_attachment_limit(self):
		...
		self.assertRaises(finergy.exceptions.AttachmentLimitReached, file2.insert)
		limit_property.delete()
		finergy.clear_cache(doctype="ToDo")
...
class TestFile(unittest.TestCase):
	...
	def tests_after_upload(self):
		...
		file_folder = finergy.db.get_value("File", self.saved_name, "folder")
		self.assertEqual(file_folder, _("Home/Test Folder 1"))

	def test_file_copy(self):
		folder = self.get_folder("Test Folder 2", "Home")

		file = finergy.get_doc("File", {"file_name": "file_copy.txt"})
	...
	def test_folder_copy(self):
		...
		file = finergy.get_doc("File", {"file_name": "folder_copy.txt"})
		file_copy_txt = finergy.get_value("File", {"file_name": "file_copy.txt"})
		if file_copy_txt:
			finergy.get_doc("File", file_copy_txt).delete()

		self.assertEqual(_("Home/Test Folder 1/Test Folder 3"), file.folder)

	def test_default_folder(self):
		d = finergy.get_doc({"doctype": "File", "file_name": _("Test_Folder"), "is_folder": 1})
		d.save()
	...
	def test_on_delete(self):
		...
		self.assertEqual(finergy.db.get_value("File", _("Home/Test Folder 1"), "file_size"), 0)
		...
		folder = finergy.get_doc("File", "Home/Test Folder 1/Test Folder 3")
		self.assertRaises(finergy.ValidationError, folder.delete)
	...
	def test_same_file_url_update(self):
		...
		file2 = finergy.get_doc("File", file2.name)
	...
	def test_parent_directory_validation_in_file_url(self):
		...
		self.assertRaises(finergy.exceptions.ValidationError, file1.save)
	...
	def test_file_url_validation(self):
		test_file = finergy.get_doc(
			{"doctype": "File", "file_name": "logo", "file_url": "https://finergy-rs.fr/files/finergy.png"}
		)
		...
		self.assertRaisesRegex(
			finergy.exceptions.ValidationError, "URL must start with http:// or https://", test_file.validate
		)

		test_file.file_url = None
		test_file.file_name = "/usr/bin/man"
		self.assertRaisesRegex(
			finergy.exceptions.ValidationError, "There is some problem with the file url", test_file.validate
		)
	...
	def test_make_thumbnail(self):
		# test web image
		test_file: File = finergy.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": finergy.utils.get_url("/_test/assets/image.jpg"),
			}
		...
		test_file = finergy.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": finergy.utils.get_url("/_test/assets/image"),
			}
		).insert(ignore_permissions=True)
		...
		test_file.db_set("thumbnail_url", None)
		test_file.reload()
		test_file.file_url = finergy.utils.get_url("unknown.jpg")
		test_file.make_thumbnail(suffix="xs")
		self.assertEqual(
			json.loads(finergy.message_log[0]).get("message"),
			f"File '{finergy.utils.get_url('unknown.jpg')}' not found",
		)
		self.assertEquals(test_file.thumbnail_url, None)

	def test_file_unzip(self):
		file_path = finergy.get_app_path("finergy", "www/_test/assets/file.zip")
		public_file_path = finergy.get_site_path("public", "files")
		try:
			import shutil
		...
		self.assertRaisesRegex(finergy.exceptions.ValidationError, "not a zip file", test_file.unzip)
...
class TestAttachment(unittest.TestCase):
	...
	def setUp(self):
		if finergy.db.exists("DocType", self.test_doctype):
			return
	...
	def tearDown(self):
		finergy.delete_doc("DocType", self.test_doctype)

	def test_file_attachment_on_update(self):
		doc = finergy.get_doc(doctype=self.test_doctype, title="test for attachment on update").insert()

		file = finergy.get_doc(
...
class TestAttachmentsAccess(unittest.TestCase):
	def test_attachments_access(self):
		...
		finergy.set_user("test4@example.com")
		self.attached_to_doctype, self.attached_to_docname = make_test_doc()
...
class TestFileUtils(unittest.TestCase):
	def test_extract_images_from_doc(self):
		...
		self.assertTrue(finergy.db.exists("File", {"attached_to_name": todo.name}))
		...
		filename = finergy.db.exists("File", {"attached_to_name": todo.name})
		self.assertIn(f'<img src="{finergy.get_doc("File", filename).file_url}', todo.description)

	def test_create_new_folder(self):
		from finergy.core.doctype.file.file import create_new_folder
