finergy.ui.form.on('Report', {
	refresh: function(frm) {
		if (frm.doc.is_standard === "Yes" && !finergy.boot.developer_mode) {
		...
		frm.add_custom_button(__("Show Report"), function() {
			switch(doc.report_type) {
				case "Report Builder":
					finergy.set_route('List', doc.ref_doctype, 'Report', doc.name);
					break;
				case "Query Report":
					finergy.set_route("query-report", doc.name);
					break;
				case "Script Report":
					finergy.set_route("query-report", doc.name);
					break;
