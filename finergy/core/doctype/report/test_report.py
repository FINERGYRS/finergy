...
from finergy.custom.doctype.customize_form.customize_form import reset_customization
from finergy.desk.query_report import add_total_row, run, save_report
from finergy.desk.reportview import delete_report
from finergy.desk.reportview import save_report as _save_report

test_records = finergy.get_test_records("Report")
test_dependencies = ["User"]


class TestReport(unittest.TestCase):
	def test_report_builder(self):
		if finergy.db.exists("Report", "User Activity Report"):
			finergy.delete_doc("Report", "User Activity Report")

		with open(os.path.join(os.path.dirname(__file__), "user_activity_report.json"), "r") as f:
			finergy.get_doc(json.loads(f.read())).insert()

		report = finergy.get_doc("Report", "User Activity Report")
	...
	def test_query_report(self):
		report = finergy.get_doc("Report", "Permitted Documents For User")
	...
	def test_save_or_delete_report(self):
		...
		try:
			...
			finergy.set_user("test_report_owner@example.com")
			self.assertRaises(finergy.PermissionError, delete_report, report.name)
	...
	def test_custom_report(self):
		...
		custom_report = finergy.get_doc("Report", custom_report_name)
		columns, result = custom_report.run_query_report(
			filters={"user": "Administrator", "doctype": "User"}, user=finergy.session.user
		)

		self.assertListEqual(["email"], [column.get("fieldname") for column in columns])
		admin_dict = finergy.core.utils.find(result, lambda d: d["name"] == "Administrator")
	...
	def test_report_permissions(self):
		...
		if not finergy.db.exists("Role", "Test Has Role"):
			role = finergy.get_doc({"doctype": "Role", "role_name": "Test Has Role"}).insert(
				ignore_permissions=True
			)

		if not finergy.db.exists("Report", "Test Report"):
			report = finergy.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = finergy.get_doc("Report", "Test Report")
	...
	def test_report_custom_permissions(self):
		finergy.set_user("test@example.com")
		finergy.db.delete("Custom Role", {"report": "Test Custom Role Report"})
		finergy.db.commit()  # nosemgrep
		if not finergy.db.exists("Report", "Test Custom Role Report"):
			report = finergy.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = finergy.get_doc("Report", "Test Custom Role Report")
	...
	def test_format_method(self):
		if finergy.db.exists("Report", "User Activity Report Without Sort"):
			finergy.delete_doc("Report", "User Activity Report Without Sort")
		with open(
			os.path.join(os.path.dirname(__file__), "user_activity_report_without_sort.json"), "r"
		) as f:
			finergy.get_doc(json.loads(f.read())).insert()

		report = finergy.get_doc("Report", "User Activity Report Without Sort")
		columns, data = report.get_data()
	...
	def test_non_standard_script_report(self):
		...
		if not finergy.db.exists("Report", report_name):
			report = finergy.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = finergy.get_doc("Report", report_name)

		report.report_script = """
totals = {}
for user in finergy.get_all('User', fields = ['name', 'user_type', 'creation']):
...
"""
	...
	def test_script_report_with_columns(self):
		...
		if finergy.db.exists("Report", report_name):
			finergy.delete_doc("Report", report_name)
...
"""
	...
	def test_toggle_disabled(self):
		...
		reports = finergy.get_all(doctype="Report", limit=1)
		...
		doc = finergy.get_doc("Report", report_name)
		with self.assertRaises(finergy.exceptions.ValidationError):
