...
import finergy
import finergy.desk.query_report
from finergy import _, scrub
from finergy.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from finergy.core.doctype.page.page import delete_custom_role
from finergy.desk.reportview import append_totals_row
from finergy.model.document import Document
from finergy.modules import make_boilerplate
...
class Report(Document):
	def validate(self):
		...
		if not self.module:
			self.module = finergy.db.get_value("DocType", self.ref_doctype, "module")

		if not self.is_standard:
			self.is_standard = "No"
			if (
				finergy.session.user == "Administrator" and getattr(finergy.local.conf, "developer_mode", 0) == 1
			):
		...
		if self.is_standard == "No":
			...
			if self.report_type != "Report Builder":
				finergy.only_for("Script Manager", True)

			if finergy.db.get_value("Report", self.name, "is_standard") == "Yes":
				finergy.throw(_("Cannot edit a standard report. Please duplicate and create a new report"))

		if self.is_standard == "Yes" and finergy.session.user != "Administrator":
			finergy.throw(_("Only Administrator can save a standard report. Please rename and save."))
	...
	def on_trash(self):
		if (
			self.is_standard == "Yes"
			and not cint(getattr(finergy.local.conf, "developer_mode", 0))
			and not finergy.flags.in_patch
		):
			finergy.throw(_("You are not allowed to delete Standard Report"))
		delete_custom_role("report", self.name)
		self.delete_prepared_reports()

	def delete_prepared_reports(self):
		prepared_reports = finergy.get_all(
	...
	def set_doctype_roles(self):
		if not self.get("roles") and self.is_standard == "No":
			meta = finergy.get_meta(self.ref_doctype)
			if not meta.istable:
	...
	def is_permitted(self):
		...
		from finergy.utils import has_common

		allowed = [
			d.role for d in finergy.get_all("Has Role", fields=["role"], filters={"parent": self.name})
		]
		...
		if has_common(finergy.get_roles(), allowed):
			return True
	...
	def export_doc(self):
		...
		if self.is_standard == "Yes" and (finergy.local.conf.get("developer_mode") or 0) == 1:
			export_to_files(
	...
	def execute_query_report(self, filters):
		if not self.query:
			finergy.throw(_("Must specify a Query to run"), title=_("Report Document Error"))

		if not self.query.lower().startswith("select"):
			finergy.throw(_("Query must be a SELECT"), title=_("Report Document Error"))

		result = [list(t) for t in finergy.db.sql(self.query, filters)]
		columns = self.get_columns() or [cstr(c[0]) for c in finergy.db.get_description()]
	...
	def execute_script_report(self, filters):
		...
		finergy.cache().hset("report_execution_time", self.name, execution_time)

		return res

	def execute_module(self, filters):
		# report in python module
		module = self.module or finergy.db.get_value("DocType", self.ref_doctype, "module")
		method_name = get_report_module_dotted_path(module, self.name) + ".execute"
		return finergy.get_attr(method_name)(finergy._dict(filters))

	def execute_script(self, filters):
		# server script
		loc = {"filters": finergy._dict(filters), "data": None, "result": None}
	...
	def run_query_report(self, filters, user, ignore_prepared_report=False):
		columns, result = [], []
		data = finergy.desk.query_report.run(
		...
		for d in data.get("columns"):
			if isinstance(d, dict):
				...
				columns.append(
					finergy._dict(label=parts[0], fieldtype=fieldtype, fieldname=parts[0], options=options)
				)
	...
	def get_standard_report_columns(self, params):
		if params.get("fields"):
			...
			for df in finergy.get_meta(self.ref_doctype).fields:
				if df.in_list_view:
	...
	def get_standard_report_order_by(self, params):
		...
		if params.get("group_by"):
			group_by_args = finergy._dict(params["group_by"])
	...
	def build_standard_report_columns(self, columns, group_by_args):
		...
		for (fieldname, doctype) in columns:
			...
			if meta.get_field(fieldname):
				...
				field = finergy._dict(fieldname=fieldname, label=label)
	...
	def toggle_disable(self, disable):
		if not self.has_permission("write"):
			finergy.throw(_("You are not allowed to edit the report."))
...
def is_prepared_report_disabled(report):
	return finergy.db.get_value("Report", report, "disable_prepared_report") or 0


def get_report_module_dotted_path(module, report_name):
	return (
		finergy.local.module_app[scrub(module)]
		+ "."
		+ scrub(module)
		+ ".report."
		+ scrub(report_name)
