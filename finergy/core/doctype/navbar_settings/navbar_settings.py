...
class NavbarSettings(Document):
	...
	def validate_standard_navbar_items(self):
		...
		if not finergy.flags.in_patch and (len(before_save_items) > len(after_save_items)):
			finergy.throw(_("Please hide the standard navbar items instead of deleting them"))


def get_app_logo():
	app_logo = finergy.db.get_single_value("Navbar Settings", "app_logo", cache=True)
	if not app_logo:
		app_logo = finergy.get_hooks("app_logo_url")[-1]
