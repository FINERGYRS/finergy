...
from finergy.build import html_to_js_template
from finergy.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from finergy.desk.form.meta import get_code_files_via_hooks, get_js
...
class Page(Document):
	def autoname(self):
		...
		if (self.name and self.name.startswith("New Page")) or not self.name:
			...
			if finergy.db.exists("Page", self.name):
				cnt = finergy.db.sql(
	...
	def validate(self):
		...
		if self.is_new() and not getattr(conf, "developer_mode", 0):
			finergy.throw(_("Not in Developer Mode"))

		# setting ignore_permissions via update_setup_wizard_access (setup_wizard.py)
		if finergy.session.user != "Administrator" and not self.flags.ignore_permissions:
			finergy.throw(_("Only Administrator can edit"))
	...
	def on_update(self):
		...
		from finergy.core.doctype.doctype.doctype import make_module_and_roles
		...
		if path:
			...
			if not os.path.exists(path + ".js"):
				with open(path + ".js", "w") as f:
					f.write(
						"""finergy.pages['%s'].on_page_load = function(wrapper) {
	...
	def load_assets(self):
			...
			if fname.endswith(".html"):
				with open(os.path.join(path, fname), "r") as f:
					...
					if "<!-- jinja -->" in template:
						...
						try:
							out = finergy.get_attr(
								"{app}.{module}.page.{page}.{page}.get_context".format(
									app=finergy.local.module_app[scrub(self.module)], module=scrub(self.module), page=page_name
								)
							)(context)
						...
						template = finergy.render_template(template, context)
		...
		if finergy.lang != "en":
			from finergy.translate import get_lang_js
...
def delete_custom_role(field, docname):
	name = finergy.db.get_value("Custom Role", {field: docname}, "name")
	if name:
		finergy.delete_doc("Custom Role", name)
