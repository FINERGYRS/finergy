...
from finergy.cache_manager import clear_controller_cache, clear_user_cache
...
from finergy.database.schema import validate_column_length, validate_column_name
from finergy.desk.notifications import delete_notification_count_for, get_filters_for
from finergy.desk.utils import validate_route_conflict
from finergy.model import (
	data_field_options,
	data_fieldtypes,
	default_fields,
	no_value_fields,
...
from finergy.model.docfield import supports_translation
from finergy.model.document import Document
from finergy.model.meta import Meta
from finergy.modules import get_doc_path, make_boilerplate
from finergy.modules.import_file import get_file_path
from finergy.utils import cint, now


class InvalidFieldNameError(finergy.ValidationError):
	pass


class UniqueFieldnameError(finergy.ValidationError):
	pass


class IllegalMandatoryError(finergy.ValidationError):
	pass


class DoctypeLinkError(finergy.ValidationError):
	pass


class WrongOptionsDoctypeLinkError(finergy.ValidationError):
	pass


class HiddenAndMandatoryWithoutDefaultError(finergy.ValidationError):
	pass


class NonUniqueError(finergy.ValidationError):
	pass


class CannotIndexedError(finergy.ValidationError):
	pass


class CannotCreateStandardDoctypeError(finergy.ValidationError):
	pass
...
class DocType(Document):
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = finergy.get_doc("DocType", self.name)
		...
		if self.default_print_format and not self.custom:
			finergy.throw(_("Standard DocType cannot have default print format, use Customize Form"))

		if finergy.conf.get("developer_mode"):
	...
	def check_developer_mode(self):
		...
		if finergy.flags.in_patch or finergy.flags.in_test:
			return

		if not finergy.conf.get("developer_mode") and not self.custom:
			finergy.throw(
	...
	def setup_fields_to_fetch(self):
		...
		try:
			old_meta = finergy.get_meta(finergy.get_doc("DocType", self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except finergy.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = finergy.get_meta(self, cached=False)
	...
	def validate_website(self):
		...
		if self.has_web_view:
			...
			if not "route" in [d.fieldname for d in self.fields]:
				finergy.throw(_('Field "route" is mandatory for Web Views'), title="Missing Field")
	...
	def change_modified_of_parent(self):
		...
		if finergy.flags.in_import:
			return
		parent_list = finergy.db.get_all(
			"DocField", "parent", dict(fieldtype=["in", finergy.model.table_fields], options=self.name)
		)
		for p in parent_list:
			finergy.db.sql("UPDATE `tabDocType` SET modified=%s WHERE `name`=%s", (now(), p.parent))
		...
		for d in self.get("fields"):
			if d.fieldtype:
				if not getattr(d, "fieldname", None):
					...
					if d.fieldname in restricted:
						finergy.throw(_("Fieldname {0} is restricted").format(d.fieldname), InvalidFieldNameError)
	...
	def on_update(self):
		...
		try:
			finergy.db.updatedb(self.name, Meta(self))
		...
		allow_doctype_export = (
			not self.custom
			and not finergy.flags.in_import
			and (finergy.conf.developer_mode or finergy.flags.allow_doctype_export)
		)
		...
		finergy.clear_cache(doctype=self.name)

		if not finergy.flags.in_install and hasattr(self, "before_update"):
			self.sync_global_search()

		# clear from local cache
		if self.name in finergy.local.meta_cache:
			del finergy.local.meta_cache[self.name]
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not finergy.request) or finergy.flags.in_test or finergy.flags.in_install
			finergy.enqueue("finergy.utils.global_search.rebuild_for_doctype", now=now, doctype=self.name)
	...
	def set_base_class_for_controller(self):
		...
		controller_path = finergy.get_module_path(
			finergy.scrub(self.module), "doctype", finergy.scrub(self.name), finergy.scrub(self.name) + ".py"
		)
		...
		if "\nfrom finergy.model.document import Document" in code and class_string in code:
			code = code.replace(
				"from finergy.model.document import Document",
	...
	def run_module_method(self, method):
		from finergy.modules import load_doctype_module
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and finergy.session.user != "Administrator":
			finergy.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			finergy.throw(_("DocType can not be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			finergy.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			finergy.db.sql(
			...
			finergy.db.rename_table(old, new)
			finergy.db.commit()
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if finergy.scrub(old) in fname:
				old_file_name = os.path.join(new_path, fname)
				new_file_name = os.path.join(new_path, fname.replace(finergy.scrub(old), finergy.scrub(new)))
				shutil.move(old_file_name, new_file_name)

		self.rename_inside_controller(new, old, new_path)
		finergy.msgprint(_("Renamed files and replaced code in controllers, please check!"))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ("{}.js", "{}.py", "{}_list.js", "{}_calendar.js", "test_{}.py", "test_{}.js"):
			fname = os.path.join(new_path, fname.format(finergy.scrub(new)))
				...
				with open(fname, "w") as f:
					if fname.endswith(".js"):
						...
						file_content = code.replace(
							finergy.scrub(old), finergy.scrub(new)
		...
		doctype_json_path = os.path.join(new_path, "{}.json".format(finergy.scrub(new)))
		current_data = finergy.get_file_json(doctype_json_path)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (
			finergy.db.table_exists(self.name)
			and finergy.db.sql("select name from `tab{}` limit 1".format(self.name))
		):
			return

		existing_property_setter = finergy.db.get_value(
	...
	def import_doc(self):
		...
		from finergy.modules.import_module import import_from_files
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = finergy.get_module_path(
				finergy.scrub(self.module), "doctype", finergy.scrub(self.name), "templates"
			)
	...
	def validate_nestedset(self):
		...
		if not self.nsm_parent_field:
			field_label = finergy.bold(_("Parent Field (Tree)"))
			finergy.throw(_("{0} is a mandatory field").format(field_label), finergy.MandatoryError)

		# check if field is valid
		fieldnames = [df.fieldname for df in self.fields]
		if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
			finergy.throw(_("Parent Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def add_nestedset_fields(self):
		...
		parent_field_name = finergy.scrub(parent_field_label)
		self.append(
			"fields",
			{
	...
	def get_max_idx(self):
		...
		max_idx = finergy.db.sql("""select max(idx) from `tabDocField` where parent = %s""", self.name)
	...
	def validate_name(self, name=None):
		...
		max_length = finergy.db.MAX_COLUMN_LENGTH - 3
		if len(name) > max_length:
			# length(tab + <Doctype Name>) should be equal to 64 characters hence doctype should be 61 characters
			finergy.throw(
				_("Doctype name is limited to {0} characters ({1})").format(max_length, name), finergy.NameError
			)
		...
		if re.search(r"^[ \t\n\r]+|[ \t\n\r]+$", name, **flags):
			finergy.throw(_("DocType's name should not start or end with whitespace"), finergy.NameError)
...
def validate_series(dt, autoname=None, name=None):
	...
	if not autoname and dt.get("fields", {"fieldname": "naming_series"}):
		...
		finergy.throw(_("Invalid fieldname '{0}' in autoname").format(dt.autoname))
	...
	if autoname and autoname.startswith("field:"):
		...
		if not field or field not in [df.fieldname for df in dt.fields]:
			finergy.throw(_("Invalid fieldname '{0}' in autoname").format(field))
		else:
			for df in dt.fields:
	...
	if (
		...
		if used_in:
			finergy.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))


def validate_links_table_fieldnames(meta):
	"""Validate fieldnames in Links table"""
	if not meta.links or finergy.flags.in_patch or finergy.flags.in_fixtures:
		return
	...
	for index, link in enumerate(meta.links, 1):
		...
		if not link.parent_doctype:
			...
			finergy.throw(message, finergy.ValidationError, _("Parent Missing"))

		if not link.table_fieldname:
			message = _("Document Links Row #{0}: Table Fieldname is mandatory for internal links").format(
				index
			)
			finergy.throw(message, finergy.ValidationError, _("Table Fieldname Missing"))

		if meta.name == link.parent_doctype:
			field_exists = link.table_fieldname in fieldnames
		else:
			field_exists = finergy.get_meta(link.parent_doctype).has_field(link.table_fieldname)

		if not field_exists:
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, finergy.bold(link.table_fieldname), finergy.bold(meta.name)
			)
			finergy.throw(message, finergy.ValidationError, _("Invalid Table Fieldname"))
...
def _test_connection_query(doctype, field, idx):
	...
	try:
		finergy.get_all(doctype, filters=filters, limit=1, distinct=True, ignore_ifnull=True)
	except Exception as e:
		finergy.clear_last_message()
		msg = _("Document Links Row #{0}: Invalid doctype or fieldname.").format(idx)
		msg += "<br>" + str(e)
		finergy.throw(msg, InvalidFieldNameError)
...
def validate_fields(meta):
	...
	:param meta: `finergy.model.meta.Meta` object to check."""
		...
		if fieldname in invalid_fields:
			finergy.throw(
				_("{0}: Fieldname cannot be one of {1}").format(
					docname, ", ".join([finergy.bold(d) for d in invalid_fields])
				)
			)
	...
	def check_link_table_options(docname, d):
		if finergy.flags.in_patch:
			return

		if finergy.flags.in_fixtures:
			return
		...
		if d.fieldtype in ("Link",) + table_fields:
			...
			if d.options != d.parent:
				options = finergy.db.get_value("DocType", d.options, "name")
				if not options:
					finergy.throw(
	...
	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			finergy.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))
	...
	def check_illegal_default(d):
		...
		if d.fieldtype == "Select" and d.default:
			if not d.options:
				...
				finergy.throw(
					_("Default value for {0} must be in the list of options.").format(finergy.bold(d.fieldname))
				)
	...
	def check_precision(d):
		if (
			...
			finergy.throw(_("Precision should be between 1 and 6"))
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			...
			if not d.get("__islocal") and finergy.db.has_column(d.parent, d.fieldname):
				has_non_unique_values = finergy.db.sql(
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype == "Fold":
				if fold_exists:
					finergy.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields) - 1:
					nxt = fields[i + 1]
					if nxt.fieldtype != "Section Break":
						finergy.throw(_("Fold must come before a Section Break"))
				else:
					finergy.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or (fieldname not in fieldname_list):
				finergy.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			finergy.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_image_field(meta):
		...
		if not df:
			finergy.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != "Attach Image":
			finergy.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			finergy.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			finergy.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			finergy.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if (
				...
				finergy.throw(_("Invalid {0} condition").format(finergy.unscrub(field)), finergy.ValidationError)
	...
	def validate_data_field_type(docfield):
		if docfield.fieldtype == "Data" and not (
			...
			if docfield.options and (docfield.options not in data_field_options):
				df_str = finergy.bold(_(docfield.label))
				text_str = (
				...
				finergy.msgprint(text_str + df_options_str, title="Invalid Data Field", raise_exception=True)
		...
		if not meta.istable:
			finergy.throw(
				_("Option {0} for field {1} is not a child table").format(
					finergy.bold(doctype), finergy.bold(docfield.fieldname)
				),
...
def validate_permissions_for_doctype(doctype, for_remove=False, alert=False):
	...
	doctype = finergy.get_doc("DocType", doctype)
...
def clear_permissions_cache(doctype):
	...
	for user in finergy.db.sql_list(
		"""
		SELECT
...
def validate_permissions(doctype, for_remove=False, alert=False):
	...
	if not permissions and alert:
		finergy.msgprint(_("No Permissions Specified"), alert=True, indicator="orange")
	...
	def check_atleast_one_set(d):
		if (
			...
			finergy.throw(_("{0}: No basic permissions set").format(get_txt(d)))
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			finergy.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			finergy.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			finergy.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			finergy.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			finergy.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
		...
		for ptype, label in [["set_user_permissions", _("Set User Permissions")]]:
			if d.get(ptype):
				d.set(ptype, 0)
				finergy.msgprint(_("{0} cannot be set for Single types").format(label))

	def check_if_submittable(d):
		if d.submit and not issubmittable:
			finergy.throw(_("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d)))
		elif d.amend and not issubmittable:
			finergy.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			finergy.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))
			...
			if d.role == "All":
				finergy.throw(
					_("Row # {0}: Non administrator user can not set the role {1} to the custom doctype").format(
						d.idx, finergy.bold(_("All"))
					),
					title=_("Permissions Error"),
				)

			roles = [row.name for row in finergy.get_all("Role", filters={"is_custom": 1})]

			if d.role in roles:
				finergy.throw(
					_("Row # {0}: Non administrator user can not set the role {1} to the custom doctype").format(
						d.idx, finergy.bold(_(d.role))
					),
					title=_("Permissions Error"),
				)
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			and not finergy.db.exists("Domain", doc.restrict_to_domain)
		):
			finergy.get_doc(dict(doctype="Domain", domain=doc.restrict_to_domain)).insert()

		if "tabModule Def" in finergy.db.get_tables() and not finergy.db.exists("Module Def", doc.module):
			m = finergy.get_doc({"doctype": "Module Def", "module_name": doc.module})
			m.app_name = finergy.local.module_app[finergy.scrub(doc.module)]
		...
		for role in list(set(roles)):
			if not finergy.db.exists("Role", role):
				r = finergy.get_doc(dict(doctype="Role", role_name=role, desk_access=1))
				r.flags.ignore_mandatory = r.flags.ignore_permissions = True
				r.insert()
	except finergy.DoesNotExistError as e:
		pass
	except finergy.db.ProgrammingError as e:
		if finergy.db.is_table_missing(e):
			pass
		else:
			raise
...
def check_fieldname_conflicts(doctype, fieldname):
	...
	if fieldname in method_list + property_list:
		finergy.throw(_("Fieldname {0} conflicting with meta object").format(fieldname))


def clear_linked_doctype_cache():
	finergy.cache().delete_value("linked_doctypes_without_ignore_user_permissions_enabled")
...
def check_email_append_to(doc):
	...
	if doc.subject_field and not subject_field:
		finergy.throw(_("Select a valid Subject field for creating documents from Email"))
	...
	if subject_field and subject_field.fieldtype not in [
		...
		finergy.throw(_("Subject Field type should be Data, Text, Long Text, Small Text, Text Editor"))
	...
	if doc.sender_field and not sender_field:
		finergy.throw(_("Select a valid Sender Field for creating documents from Email"))

	if not sender_field.options == "Email":
		finergy.throw(_("Sender Field should have Email in options"))
