...
finergy.ui.form.on('DocType', {
	refresh: function(frm) {
		frm.set_query('role', 'permissions', function(doc) {
			if (doc.custom && finergy.session.user != 'Administrator') {
				return {
					query: "finergy.core.doctype.role.role.role_query",
					filters: [['Role', 'name', '!=', 'All']]
				};
			}
		});

		if(finergy.session.user !== "Administrator" || !finergy.boot.developer_mode) {
			if(frm.is_new()) {
		...
		if (!frm.is_new() && !frm.doc.istable) {
			if (frm.doc.issingle) {
				frm.add_custom_button(__('Go to {0}', [__(frm.doc.name)]), () => {
					window.open(`/app/${finergy.router.slug(frm.doc.name)}`);
				});
			} else {
