...
# test_records = finergy.get_test_records('DocType')


class TestDocType(unittest.TestCase):
	def test_validate_name(self):
		self.assertRaises(finergy.NameError, new_doctype("_Some DocType").insert)
		self.assertRaises(finergy.NameError, new_doctype("8Some DocType").insert)
		self.assertRaises(finergy.NameError, new_doctype("Some (DocType)").insert)
		self.assertRaises(
			finergy.NameError,
		...
		for name in ("Some DocType", "Some_DocType"):
			if finergy.db.exists("DocType", name):
				finergy.delete_doc("DocType", name)

			doc = new_doctype(name).insert()
			doc.delete()

	def test_doctype_unique_constraint_dropped(self):
		if finergy.db.exists("DocType", "With_Unique"):
			finergy.delete_doc("DocType", "With_Unique")

		dt = new_doctype("With_Unique", unique=1)
		dt.insert()

		doc1 = finergy.new_doc("With_Unique")
		doc2 = finergy.new_doc("With_Unique")
		...
		doc1.insert()
		self.assertRaises(finergy.UniqueValidationError, doc2.insert)
	...
	def test_validate_search_fields(self):
		...
		self.assertRaises(finergy.ValidationError, doc.save)
	...
	def test_data_field_options(self):
		...
		valid_data_field_options = finergy.model.data_field_options + ("",)
		invalid_data_field_options = ("Invalid Option 1", finergy.utils.random_string(5))

		for field_option in valid_data_field_options + invalid_data_field_options:
			test_doctype = finergy.get_doc(
				{
					"doctype": "DocType",
					"name": doctype_name,
					"module": "Core",
			...
			if field_option in invalid_data_field_options:
				# assert that only data options in finergy.model.data_field_options are valid
				self.assertRaises(finergy.ValidationError, test_doctype.insert)
			else:
				test_doctype.insert()
	...
	def test_sync_field_order(self):
		...
		finergy.delete_doc_if_exists("DocType", "Test Field Order DocType")
		if os.path.isfile(path):
			os.remove(path)

		try:
			finergy.flags.allow_doctype_export = 1
			test_doctype.save()

			# assert that field_order list is being created with the default order
			test_doctype_json = finergy.get_file_json(path)
			...
			with open(path, "w+") as txtfile:
				txtfile.write(finergy.as_json(test_doctype_json))
			...
			finergy.reload_doctype(test_doctype.name, force=True)
			test_doctype.reload()
			...
			finergy.flags.allow_doctype_export = 0
	...
	def test_cancel_link_doctype(self):
		...
		from finergy.desk.form.linked_with import cancel_all_linked_docs, get_submitted_linked_docs
		...
		# create doctype data
		data_link_doc = finergy.new_doc("Test Linked Doctype")
		data_link_doc.some_fieldname = "Data1"
		data_link_doc.insert()
		data_link_doc.save()
		data_link_doc.submit()

		data_doc = finergy.new_doc("Test Doctype")
	...
	def test_ignore_cancelation_of_linked_doctype_during_cancel(self):
		...
		data_doc = finergy.new_doc("Test Doctype 2")
		data_doc.some_fieldname = "Data1"
		...
		self.assertRaises(finergy.LinkExistsError, data_link_doc_1.cancel)
	...
	def test_create_virtual_doctype(self):
		...
		virtual_doc.insert()
		virtual_doc.save()
		doc = finergy.get_doc("DocType", "Test Virtual Doctype")

		self.assertEqual(doc.is_virtual, 1)
		self.assertFalse(finergy.db.table_exists("Test Virtual Doctype"))
