...
from finergy.core.doctype.data_import_legacy.importer import get_data_keys
from finergy.utils import cint, cstr, format_datetime, format_duration, formatdate, parse_json
from finergy.utils.csvutils import UnicodeWriter
...
class DataExporter:
	...
	def prepare_args(self):
		...
		if self.all_doctypes:
			self.child_doctypes = []
			for df in finergy.get_meta(self.doctype).get_table_fields():
	...
	def build_response(self):
		...
		if self.file_type == "Excel":
			...
			finergy.response["result"] = cstr(self.writer.getvalue())
			finergy.response["type"] = "csv"
			finergy.response["doctype"] = self.doctype
	...
	def build_field_columns(self, dt, parentfield=None):
		meta = finergy.get_meta(dt)

		# build list of valid docfields
		tablecolumns = []
		table_name = "tab" + dt
		for f in finergy.db.get_table_columns_description(table_name):
		...
		_column_start_end = finergy._dict(start=0)
		...
		if dt == self.doctype:
			...
			_column_start_end = finergy._dict(start=len(self.columns))
	...
	def getinforow(docfield):
		...
		if docfield.fieldtype == "Select":
			...
			return cstr(finergy.defaults.get_defaults().date_format)
	...
	def add_data(self):
		...
		finergy.permissions.can_export(self.parent_doctype, raise_exception=True)

		# sort nested set doctypes by `lft asc`
		order_by = None
		table_columns = finergy.db.get_table_columns(self.parent_doctype)
		if "lft" in table_columns and "rgt" in table_columns:
			order_by = "`tab{doctype}`.`lft` asc".format(doctype=self.parent_doctype)
		# get permitted data only
		self.data = finergy.get_list(
		...
		for doc in self.data:
			...
			if self.all_doctypes:
				...
				for c in self.child_doctypes:
					...
					data_row = (
						finergy.qb.from_(child_doctype_table)
						.select("*")
	...
	def build_response_as_excel(self):
		filename = finergy.generate_hash("", 10)
		...
		finergy.response["filename"] = self.doctype + ".xlsx"
