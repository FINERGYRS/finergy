...
finergy.ui.form.on('Data Export', {
	refresh: frm => {
		frm.disable_save();
		...
		frm.set_query("reference_doctype", () => {
			return {
				"filters": {
					"issingle": 0,
					"istable": 0,
					"name": ['in', finergy.boot.user.can_export]
				}
			};
		});
	},
	reference_doctype: frm => {
		...
		if (doctype) {
			finergy.model.with_doctype(doctype, () => set_field_options(frm));
		} else {
...
const can_export = frm => {
	...
	if (!doctype) {
		...
		finergy.msgprint(__('Atleast one field of Parent Document Type is mandatory'));
	} else {
		is_valid_form = true;
	}
...
const set_field_options = (frm) => {
	...
	frm.filter_list = new finergy.ui.FilterGroup({
...
const make_multiselect_buttons = parent_wrapper => {
	...
	["Select All", "Unselect All"].map(d => {
		finergy.ui.form.make_control({
			parent: $(button_container),
			df: {
				label: __(d),
				fieldname: finergy.scrub(d),
				fieldtype: "Button",
				click: () => {
...
const get_doctypes = parentdt => {
	return [parentdt].concat(
		finergy.meta.get_table_fields(parentdt).map(df => df.options)
	);
};
...
const add_doctype_field_multicheck_control = (doctype, parent_wrapper) => {
	...
	const multicheck_control = finergy.ui.form.make_control({
