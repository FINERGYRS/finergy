...
import finergy
from finergy.core.doctype.data_export.exporter import DataExporter
...
class TestDataExporter(unittest.TestCase):
	...
	def create_test_data(self, force=False):
		...
		"""
		if force:
			finergy.delete_doc(self.doctype_name, self.doc_name)

		if not finergy.db.exists(self.doctype_name, self.doc_name):
			self.doc = finergy.get_doc(
			...
			self.doc = finergy.get_doc(self.doctype_name, self.doc_name)

	def test_export_content(self):
		exp = DataExporter(doctype=self.doctype_name, file_type="CSV")
		exp.build_response()

		self.assertEqual(finergy.response["type"], "csv")
		self.assertEqual(finergy.response["doctype"], self.doctype_name)
		self.assertTrue(finergy.response["result"])
		self.assertIn('Child Title 1",50', finergy.response["result"])
		self.assertIn('Child Title 2",51', finergy.response["result"])
		...
		for type in ["csv", "Excel"]:
			with self.subTest(type=type):
				...
				if type == "csv":
					...
					self.assertEqual(finergy.response["type"], "binary")
					self.assertEqual(
						finergy.response["filename"], self.doctype_name + ".xlsx"
					)  # 'Test DocType for Export Tool.xlsx')
					self.assertTrue(finergy.response["filecontent"])
