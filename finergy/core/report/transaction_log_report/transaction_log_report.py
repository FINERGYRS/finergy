...
from finergy.utils import format_datetime
...
def get_data(filters=None):
	...
	logs = finergy.db.sql("SELECT * FROM `tabTransaction Log` order by creation desc ", as_dict=1)
	result = []
	for l in logs:
		row_index = int(l.row_index)
		if row_index > 1:
			previous_hash = finergy.db.sql(
