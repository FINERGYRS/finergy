...
finergy.ui.form.on('Google Drive', {
	refresh: function(frm) {
		...
		if (frm.doc.enable && frm.doc.refresh_token) {
			let sync_button = frm.add_custom_button(__("Take Backup"), function () {
				...
				finergy.call({
					method: "finergy.integrations.doctype.google_drive.google_drive.take_backup",
					btn: sync_button
				}).then((r) => {
	...
	authorize_google_drive_access: function(frm) {
		...
		finergy.call({
			method: "finergy.integrations.doctype.google_drive.google_drive.authorize_access",
			args: {
