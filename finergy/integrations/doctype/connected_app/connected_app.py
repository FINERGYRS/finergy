...
if any((os.getenv("CI"), finergy.conf.developer_mode, finergy.conf.allow_tests)):
...
class ConnectedApp(Document):
	...
	def validate(self):
		base_url = finergy.utils.get_url()
		callback_path = (
			"/api/method/finergy.integrations.doctype.connected_app.connected_app.callback/" + self.name
		)
	...
	def initiate_web_application_flow(self, user=None, success_uri=None):
		...
		if not token_cache:
			token_cache = finergy.new_doc("Token Cache")
	...
	def get_user_token(self, user=None, success_uri=None):
		...
		finergy.local.response["location"] = redirect
		return redirect
	...
	def get_token_cache(self, user):
		...
		if finergy.db.exists("Token Cache", token_cache_name):
			token_cache = finergy.get_doc("Token Cache", token_cache_name)
...
def callback(code=None, state=None):
	...
	if finergy.request.method != "GET":
		finergy.throw(_("Invalid request method: {}").format(finergy.request.method))

	if finergy.session.user == "Guest":
		finergy.local.response["type"] = "redirect"
		finergy.local.response["location"] = "/login?" + urlencode({"redirect-to": finergy.request.url})
		return

	path = finergy.request.path[1:].split("/")
	if len(path) != 4 or not path[3]:
		finergy.throw(_("Invalid Parameters."))

	connected_app = finergy.get_doc("Connected App", path[3])
	token_cache = finergy.get_doc("Token Cache", connected_app.name + "-" + finergy.session.user)

	if state != token_cache.state:
		finergy.throw(_("Invalid state."))
	...
	finergy.local.response["type"] = "redirect"
	finergy.local.response["location"] = token_cache.get("success_uri") or connected_app.get_url()
