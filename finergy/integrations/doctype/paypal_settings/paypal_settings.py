...
from finergy.integrations.utils import create_payment_gateway, create_request_log, make_post_request
from finergy.model.document import Document
from finergy.utils import call_hook_method, cint, get_datetime, get_url

api_path = "/api/method/finergy.integrations.doctype.paypal_settings.paypal_settings"
...
class PayPalSettings(Document):
	...
	def setup_sandbox_env(self, token):
		data = json.loads(finergy.db.get_value("Integration Request", token, "data"))
		setattr(self, "use_sandbox", cint(finergy._dict(data).use_sandbox) or 0)
	...
	def get_paypal_params_and_url(self):
		...
		if hasattr(self, "use_sandbox") and self.use_sandbox:
			params.update(
				{
					"USER": finergy.conf.sandbox_api_username,
					"PWD": finergy.conf.sandbox_api_password,
					"SIGNATURE": finergy.conf.sandbox_signature,
				}
			)
	...
	def validate_paypal_credentails(self):
		...
		try:
			...
			finergy.throw(_("Invalid payment gateway credentials"))
	...
	def execute_set_express_checkout(self, **kwargs):
		...
		if response.get("ACK")[0] != "Success":
			finergy.throw(_("Looks like something is wrong with this site's Paypal configuration."))
...
def get_paypal_and_transaction_details(token):
	doc = finergy.get_doc("PayPal Settings")
	doc.setup_sandbox_env(token)
	params, url = doc.get_paypal_params_and_url()

	integration_request = finergy.get_doc("Integration Request", token)
...
def setup_redirect(data, redirect_url, custom_redirect_to=None, redirect=True):
	...
	if redirect:
		...
		finergy.local.response["location"] = get_url(redirect_url)
...
def get_express_checkout_details(token):
	try:
		...
		doc = finergy.get_doc("Integration Request", token)
		...
		finergy.local.response["type"] = "redirect"
		finergy.local.response["location"] = get_redirect_uri(doc, token, response.get("PAYERID")[0])
...
def create_recurring_profile(token, payerid):
	try:
		...
		starts_at = get_datetime(subscription_details.get("start_date")) or finergy.utils.now_datetime()
		starts_at = starts_at.replace(tzinfo=pytz.timezone(finergy.utils.get_time_zone())).astimezone(
			pytz.utc
		)
...
def manage_recurring_payment_profile_status(profile_id, action, args, url):
	...
	if response.get("ACK")[0] != "Success" and response.get("L_ERRORCODE0", [])[0] != "11556":
		finergy.throw(_("Failed while amending subscription"))
...
def ipn_handler():
	try:
		...
		finergy.enqueue(
			method="finergy.integrations.doctype.paypal_settings.paypal_settings.handle_subscription_notification",
			queue="long",
			timeout=600,
			is_async=True,
...
def validate_ipn_request(data):
	def _throw():
		finergy.throw(_("In Valid Request"), exc=finergy.InvalidStatusError)
