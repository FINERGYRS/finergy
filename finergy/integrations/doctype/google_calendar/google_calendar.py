...
from finergy.integrations.doctype.google_settings.google_settings import get_auth_url
...
from finergy.utils.password import set_encrypted_password
...
class GoogleCalendar(Document):
	def validate(self):
		google_settings = finergy.get_single("Google Settings")
		if not google_settings.enable:
			finergy.throw(_("Enable Google API in Google Settings."))

		if not google_settings.client_id or not google_settings.client_secret:
			finergy.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			button_label = finergy.bold(_("Allow Google Calendar Access"))
			raise finergy.ValidationError(_("Click on {0} to generate Refresh Token.").format(button_label))
...
def authorize_access(g_calendar, reauthorize=None):
	...
	"""
	google_settings = finergy.get_doc("Google Settings")
	google_calendar = finergy.get_doc("Google Calendar", g_calendar)

	redirect_uri = (
		get_request_site_address(True)
		+ "?cmd=finergy.integrations.doctype.google_calendar.google_calendar.google_callback"
	)

	if not google_calendar.authorization_code or reauthorize:
		finergy.cache().hset("google_calendar", "google_calendar", google_calendar.name)
		...
		try:
			...
			finergy.local.response["location"] = "/app/Form/{0}/{1}".format(
				quote("Google Calendar"), quote(google_calendar.name)
			)

			finergy.msgprint(_("Google Calendar has been configured."))
...
def google_callback(code=None):
	...
	"""
	google_calendar = finergy.cache().hget("google_calendar", "google_calendar")
	finergy.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	finergy.db.commit()
...
def sync(g_calendar=None):
	...
	google_calendars = finergy.get_list("Google Calendar", filters=filters)
...
def get_google_calendar_object(g_calendar):
	...
	"""
	google_settings = finergy.get_doc("Google Settings")
	account = finergy.get_doc("Google Calendar", g_calendar)
	...
	try:
		if account.google_calendar_id:
			...
			calendar = {
				...
				"timeZone": finergy.db.get_single_value("System Settings", "time_zone"),
			}
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				finergy.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
				finergy.throw(msg)
	...
	for idx, event in enumerate(results):
		finergy.publish_realtime(
			"import_google_calendar", dict(progress=idx + 1, total=len(results)), user=finergy.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			if not finergy.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
			...
			finergy.get_doc(
				{
					...
					"reference_name": finergy.db.get_value(
						"Event",
						{
							"google_calendar_id": account.google_calendar_id,
							"google_calendar_event_id": event.get("id"),
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
	Inserts event in Finergy Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	finergy.get_doc(calendar_event).insert(ignore_permissions=True)


def update_event_in_calendar(account, event, recurrence=None):
	"""
	Updates Event in Finergy Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = finergy.get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	"""
	if (
		not finergy.db.exists("Google Calendar", {"name": doc.google_calendar})
	...
	try:
		...
		finergy.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		finergy.throw(
...
def update_event_in_google_calendar(doc, method=None):
	"""
	Updates Events in Google Calendar if any existing event is modified in Finergy Calendar
	"""
...
def delete_event_from_google_calendar(doc, method=None):
	"""
	Delete Events from Google Calendar if Finergy Event is deleted.
	"""

	if not finergy.db.exists("Google Calendar", {"name": doc.google_calendar}):
		return
