...
finergy.ui.form.on("Google Calendar", {
	refresh: function(frm) {
		...
		if (frm.doc.refresh_token) {
			frm.add_custom_button(__("Sync Calendar"), function () {
				...
				finergy.call({
					method: "finergy.integrations.doctype.google_calendar.google_calendar.sync",
					args: {
	...
	authorize_google_calendar_access: function(frm) {
		...
		finergy.call({
			method: "finergy.integrations.doctype.google_calendar.google_calendar.authorize_access",
			args: {
				"g_calendar": frm.doc.name,
