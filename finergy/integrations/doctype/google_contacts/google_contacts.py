...
from finergy.utils import get_request_site_address
...
class GoogleContacts(Document):
	def validate(self):
		if not finergy.db.get_single_value("Google Settings", "enable"):
	...
	def get_access_token(self):
		...
		if not google_settings.enable:
			finergy.throw(_("Google Contacts Integration is disabled."))

		if not self.refresh_token:
			button_label = finergy.bold(_("Allow Google Contacts Access"))
...
def authorize_access(g_contact, reauthorize=None):
	...
	google_settings = finergy.get_doc("Google Settings")
	google_contact = finergy.get_doc("Google Contacts", g_contact)

	redirect_uri = (
		get_request_site_address(True)
		+ "?cmd=finergy.integrations.doctype.google_contacts.google_contacts.google_callback"
	)

	if not google_contact.authorization_code or reauthorize:
		finergy.cache().hset("google_contacts", "google_contact", google_contact.name)
		...
		try:
			...
			finergy.local.response["location"] = "/app/Form/Google%20Contacts/{}".format(google_contact.name)

			finergy.msgprint(_("Google Contacts has been configured."))
...
def google_callback(code=None):
	...
	"""
	google_contact = finergy.cache().hget("google_contacts", "google_contact")
	finergy.db.set_value("Google Contacts", google_contact, "authorization_code", code)
	finergy.db.commit()
...
def get_google_contacts_object(g_contact):
	...
	"""
	google_settings = finergy.get_doc("Google Settings")
	account = finergy.get_doc("Google Contacts", g_contact)
...
def sync(g_contact=None):
	...
	google_contacts = finergy.get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	finergy.db.set_value("Google Contacts", account.name, "last_sync_on", finergy.utils.now_datetime())

	for idx, connection in enumerate(results):
		finergy.publish_realtime(
			"import_google_contacts", dict(progress=idx + 1, total=len(results)), user=finergy.session.user
		)
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not finergy.db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		finergy.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
		finergy.msgprint(
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		finergy.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		finergy.msgprint(
