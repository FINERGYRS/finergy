...
finergy.ui.form.on('Google Contacts', {
	refresh: function(frm) {
		...
		if (frm.doc.refresh_token) {
			let sync_button = frm.add_custom_button(__('Sync Contacts'), function () {
				...
				finergy.call({
					method: "finergy.integrations.doctype.google_contacts.google_contacts.sync",
					args: {
	...
	authorize_google_contacts_access: function(frm) {
		...
		finergy.call({
			method: "finergy.integrations.doctype.google_contacts.google_contacts.authorize_access",
			args: {
				"g_contact": frm.doc.name,
