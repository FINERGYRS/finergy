...
from finergy.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			finergy.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			finergy.throw(_("Invalid username or password"))
		except Exception as ex:
			finergy.throw(_(str(ex)))

	@staticmethod
	def get_ldap_client_settings() -> dict:
		# return the settings to be used on the client side.
		result = {"enabled": False}
		ldap = finergy.get_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "finergy.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def sync_roles(self, user: "User", additional_groups: list = None):
		...
		all_mapped_roles = {r.capkpi_role for r in self.ldap_groups}
		matched_roles = {
			r.capkpi_role for r in self.ldap_groups if r.ldap_group.lower() in lower_groups
		}
	...
	def create_or_update_user(self, user_data: dict, groups: list = None):
		...
		if finergy.db.exists("User", user_data["email"]):
			user = finergy.get_doc("User", user_data["email"])
			...
			user = finergy.get_doc(doc)
			user.insert(ignore_permissions=True)

		if self.default_user_type == "System User":
			role = self.default_role
		else:
			role = finergy.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			finergy.throw(_("LDAP is not enabled."))
		...
		try:
			...
			finergy.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					if logout_sessions:
						from finergy.sessions import clear_sessions

						clear_sessions(user=user, force=True)
					finergy.msgprint(_("Password changed successfully."))
				else:
					finergy.throw(_("Failed to change password."))
			else:
				finergy.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			finergy.throw(_("No LDAP User found for email: {0}").format(user))
...
def login():
	...
	ldap: LDAPSettings = finergy.get_doc("LDAP Settings")

	user = ldap.authenticate(finergy.as_unicode(args.usr), finergy.as_unicode(args.pop("pwd", None)))

	finergy.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(finergy.local.login_manager):
			return False
