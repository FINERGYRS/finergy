...
import finergy
from finergy.exceptions import MandatoryError, ValidationError
...
class LDAP_TestCase:
	...
	def mock_ldap_connection(f):
		...
		def wrapped(self, *args, **kwargs):
			...
			with mock.patch(
				"finergy.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.connect_to_ldap",
	...
	def clean_test_users():
		with contextlib.suppress(Exception):
			finergy.get_doc("User", "posix.user1@unit.testing").delete()
		with contextlib.suppress(Exception):
			finergy.get_doc("User", "posix.user2@unit.testing").delete()
		with contextlib.suppress(Exception):
			finergy.get_doc("User", "website_ldap_user@test.com").delete()

	@classmethod
	def setUpClass(cls):
		cls.clean_test_users()
		# Save user data for restoration in tearDownClass()
		cls.user_ldap_settings = finergy.get_doc("LDAP Settings")
		...
		user = finergy.get_doc(cls.user1doc)
		...
		user = finergy.get_doc(cls.user2doc)
		user.insert(ignore_permissions=True)
	...
	def tearDownClass(cls):
		with contextlib.suppress(Exception):
			finergy.get_doc("LDAP Settings").delete()
	...
	def test_connect_to_ldap(self):
		...
		prevent_connection_parameters = {
			"mode": {
				"IP_V4_ONLY": "Locks the user to IPv4 without finergy providing a way to configure",
				"IP_V6_ONLY": "Locks the user to IPv6 without finergy providing a way to configure",
			},
			"auto_bind": {
	...
	def test_update_user_fields(self):
		...
		test_user = finergy.get_doc("User", test_user_data["email"])
		self.test_class.update_user_fields(test_user, test_user_data)
		updated_user = finergy.get_doc("User", test_user_data["email"])
		...
		self.assertIn(self.test_class.default_role, finergy.get_roles(updated_user.name))
	...
	def test_create_website_user(self):
		...
		new_user = finergy.get_doc("User", new_test_user_data["email"])
	...
	def test_sync_roles(self):
		...
		role_to_group_map = {
			self.doc["ldap_groups"][0]["capkpi_role"]: self.doc["ldap_groups"][0]["ldap_group"],
			self.doc["ldap_groups"][1]["capkpi_role"]: self.doc["ldap_groups"][1]["ldap_group"],
			self.doc["ldap_groups"][2]["capkpi_role"]: self.doc["ldap_groups"][2]["ldap_group"],
			"Newsletter Manager": "default_role",
			"All": "finergy_default_all",
			"Guest": "finergy_default_guest",
		}

		# re-create user1 to ensure clean
		finergy.get_doc("User", "posix.user1@unit.testing").delete()
		user = finergy.get_doc(self.user1doc)
		user.insert(ignore_permissions=True)

		for test_user in test_user_data:
			test_user_doc = finergy.get_doc("User", f"{test_user}@unit.testing")
			test_user_roles = finergy.get_roles(f"{test_user}@unit.testing")

			self.assertTrue(
				len(test_user_roles) == 2, "User should only be a part of the All and Guest roles"
			)  # check default finergy roles

			self.test_class.sync_roles(test_user_doc, test_user_data[test_user])  # update user roles

			finergy.get_doc("User", f"{test_user}@unit.testing")
			updated_user_roles = finergy.get_roles(f"{test_user}@unit.testing")
	...
	def test_create_or_update_user(self):
		...
		finergy.get_doc("User", f"{test_user}@unit.testing").delete()

		with self.assertRaises(
			finergy.exceptions.DoesNotExistError
		):  # ensure user deleted so function can be tested
			finergy.get_doc("User", f"{test_user}@unit.testing")

		with mock.patch(
			"finergy.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.update_user_fields"
		) as update_user_fields_method:
			with mock.patch(
				"finergy.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.sync_roles"
	...
	def test_authenticate(self):
		with mock.patch(
			"finergy.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.fetch_ldap_groups"
		...
		for username, password in enumerate(invalid_users):
			with self.assertRaises(finergy.exceptions.ValidationError) as display_massage:
	...
	DOCUMENT_GROUP_MAPPINGS = [
		{
			...
			"capkpi_role": "System Manager",
		},
		{"doctype": "LDAP Group Mapping", "ldap_group": "Users", "capkpi_role": "Blogger"},
		{"doctype": "LDAP Group Mapping", "ldap_group": "Group3", "capkpi_role": "Accounts User"},
	]
...
class Test_ActiveDirectory(LDAP_TestCase, TestCase):
	...
	DOCUMENT_GROUP_MAPPINGS = [
		...
		{"doctype": "LDAP Group Mapping", "ldap_group": "Domain Users", "capkpi_role": "Blogger"},
		{
			"doctype": "LDAP Group Mapping",
			"ldap_group": "Enterprise Administrators",
			"capkpi_role": "Accounts User",
		},
	]
