...
from finergy.integrations.offsite_backup_utils import (
	get_chunk_site,
	get_latest_backup_file,
	send_email,
	validate_file_size,
)
from finergy.integrations.utils import make_post_request
...
from finergy.utils.backups import new_backup
...
class DropboxSettings(Document):
	def onload(self):
		if not self.app_access_key and finergy.conf.dropbox_access_key:
			self.set_onload("dropbox_setup_via_site_config", 1)

	def validate(self):
		if self.enabled and self.limit_no_of_backups and self.no_of_backups < 1:
			finergy.throw(_("Number of DB backups cannot be less than 1"))
...
def take_backup():
	...
	enqueue(
		"finergy.integrations.doctype.dropbox_settings.dropbox_settings.take_backup_to_dropbox",
		queue="long",
		timeout=1500,
	)
	finergy.msgprint(_("Queued for backup. It may take a few minutes to an hour."))
...
def take_backups_if(freq):
	if finergy.db.get_value("Dropbox Settings", None, "backup_frequency") == freq:
...
def take_backup_to_dropbox(retry_count=0, upload_db_backup=True):
	...
	try:
		if cint(finergy.db.get_value("Dropbox Settings", None, "enabled")):
			validate_file_size()
			...
			if cint(finergy.db.get_value("Dropbox Settings", None, "send_email_for_successful_backup")):
...
def backup_to_dropbox(upload_db_backup=True):
	...
	if upload_db_backup:
		if finergy.flags.create_new_backup:
...
def upload_from_folder(
	...
	if is_fresh_upload():
		...
		response = finergy._dict({"entries": []})

	path = str(path)

	for f in finergy.get_all(
		"File",
		...
		for file_metadata in response.entries:
			try:
				...
				error_log.append(finergy.get_traceback())
...
def upload_file_to_dropbox(filename, folder, dropbox_client):
	...
	try:
		...
		if isinstance(e.error, dropbox.files.UploadError):
			...
			error += finergy.get_traceback()
			finergy.log_error(error)
		else:
			raise
...
def update_file_dropbox_status(file_name):
	finergy.db.set_value("File", file_name, "uploaded_to_dropbox", 1, update_modified=False)


def is_fresh_upload():
	file_name = finergy.db.get_value("File", {"uploaded_to_dropbox": 1}, "name")
...
def get_uploaded_files_meta(dropbox_folder, dropbox_client):
	try:
		...
		if isinstance(e.error, dropbox.files.ListFolderError):
			return finergy._dict({"entries": []})
		else:
			raise


def get_dropbox_settings(redirect_uri=False):
	if not finergy.conf.dropbox_broker_site:
		finergy.conf.dropbox_broker_site = "https://dropbox.capkpi.com"
	settings = finergy.get_doc("Dropbox Settings")
	app_details = {
		"app_key": settings.app_access_key or finergy.conf.dropbox_access_key,
		"app_secret": settings.get_password(fieldname="app_secret_key", raise_exception=False)
		if settings.app_secret_key
		else finergy.conf.dropbox_secret_key,
		...
		app_details.update(
			{
				...
				+ "/api/method/finergy.integrations.doctype.dropbox_settings.dropbox_settings.dropbox_auth_finish"
				if settings.app_secret_key
				else finergy.conf.dropbox_broker_site
				+ "/api/method/dropbox_capkpi_broker.www.setup_dropbox.generate_dropbox_access_token",
			}
		)
...
def get_redirect_url():
	if not finergy.conf.dropbox_broker_site:
		finergy.conf.dropbox_broker_site = "https://dropbox.capkpi.com"
	url = "{0}/api/method/dropbox_capkpi_broker.www.setup_dropbox.get_authotize_url".format(
		finergy.conf.dropbox_broker_site
	)
...
def dropbox_auth_finish(return_access_token=False):
	...
	if callback.state or callback.code:
		...
		finergy.respond_as_web_page(
			...
			indicator_color="red",
			http_status_code=finergy.AuthenticationError.http_status_code,
		)
...
def set_dropbox_access_token(access_token):
	finergy.db.set_value("Dropbox Settings", None, "dropbox_access_token", access_token)
	finergy.db.commit()
