...
import finergy
from finergy.integrations.doctype.social_login_key.social_login_key import BaseUrlNotSetError


class TestSocialLoginKey(unittest.TestCase):
	def test_adding_finergy_social_login_provider(self):
...
def create_or_update_social_login_key():
	...
	try:
		social_login_key = finergy.get_doc("Social Login Key", "finergy")
	except finergy.DoesNotExistError:
		social_login_key = finergy.new_doc("Social Login Key")
	social_login_key.get_social_login_provider("Finergy", initialize=True)
	social_login_key.base_url = finergy.utils.get_url()
