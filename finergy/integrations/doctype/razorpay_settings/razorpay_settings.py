...
from finergy.integrations.utils import (
...
from finergy.utils import call_hook_method, cint, get_timestamp, get_url
...
class RazorpaySettings(Document):
	...
	def validate_razorpay_credentails(self):
		if self.api_key and self.api_secret:
			try:
				...
				finergy.throw(_("Seems API Key or API Secret is wrong !!!"))
	...
	def setup_addon(self, settings, **kwargs):
		...
		try:
			if not finergy.conf.converted_rupee_to_paisa:
			...
			for addon in kwargs.get("addons"):
				...
				if not resp.get("id"):
					finergy.log_error(str(resp), "Razorpay Failed while creating subscription")
		except Exception:
	...
	def prepare_subscription_details(self, settings, **kwargs):
		...
		if finergy.flags.status != "created":
	...
	def create_order(self, **kwargs):
		...
		if self.api_key and self.api_secret:
			try:
				...
				finergy.log(finergy.get_traceback())
				finergy.throw(_("Could not create razorpay order"))

	def create_request(self, data):
		self.data = finergy._dict(data)

		try:
			self.integration_request = finergy.get_doc("Integration Request", self.data.token)
			...
			finergy.log_error(finergy.get_traceback())
			return {
				"redirect_to": finergy.redirect_to_message(
					_("Server Error"),
					_(
	...
	def authorize_payment(self):
		...
		try:
			...
			if resp.get("status") == "captured":
				...
				finergy.log_error(str(resp), "Razorpay Payment not authorized")
		...
		status = finergy.flags.integration_request.status_code
		...
		if self.flags.status_changed_to in ("Authorized", "Verified", "Completed"):
			if self.data.reference_doctype and self.data.reference_docname:
				...
				try:
					finergy.flags.data = data
					custom_redirect_to = finergy.get_doc(
			...
			settings.update(
				{
					...
					"api_secret": finergy.conf.sandbox_api_secret,
				}
			)
	...
	def verify_signature(self, body, signature, key):
		...
		if not result:
			finergy.throw(_("Razorpay Signature Verification Failed"), exc=finergy.PermissionError)
...
def capture_payment(is_sandbox=False, sanbox_response=None):
	...
	for doc in finergy.get_all(
		...
		try:
			...
			if resp.get("status") == "captured":
				finergy.db.set_value("Integration Request", doc.name, "status", "Completed")

		except Exception:
			doc = finergy.get_doc("Integration Request", doc.name)
			doc.status = "Failed"
			doc.error = finergy.get_traceback()
			finergy.log_error(doc.error, "{0} Failed".format(doc.name))
...
def get_order(doctype, docname):
	...
	try:
		...
		finergy.log_error(finergy.get_traceback(), _("Controller method get_razorpay_order missing"))
		finergy.throw(_("Could not create Razorpay order. Please contact Administrator"))
...
def order_payment_success(integration_request, params):
	...
	integration = finergy.get_doc("Integration Request", integration_request)
	...
	controller.data = finergy._dict(data)
...
def order_payment_failure(integration_request, params):
	...
	"""
	finergy.log_error(params, "Razorpay Payment Failure")
	params = json.loads(params)
	integration = finergy.get_doc("Integration Request", integration_request)
	integration.update_status(params, integration.status)
...
def convert_rupee_to_paisa(**kwargs):
	...
	finergy.conf.converted_rupee_to_paisa = True


@finergy.whitelist(allow_guest=True)
def razorpay_subscription_callback():
	try:
		data = finergy.local.form_dict
		...
		doc = finergy.get_doc(
			{
				"data": json.dumps(finergy.local.form_dict),
		...
		finergy.enqueue(
			method="finergy.integrations.doctype.razorpay_settings.razorpay_settings.handle_subscription_notification",
			queue="long",
			timeout=600,
			is_async=True,
	...
	except finergy.InvalidStatusError:
		pass
	except Exception as e:
		finergy.log(finergy.log_error(title=e))


def validate_payment_callback(data):
	def _throw():
		finergy.throw(_("Invalid Subscription"), exc=finergy.InvalidStatusError)
