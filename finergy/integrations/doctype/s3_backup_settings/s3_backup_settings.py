...
class S3BackupSettings(Document):
	def validate(self):
		...
		try:
			...
			bucket_name = finergy.bold(self.bucket)
			if error_code == "403":
...
def take_backup():
	...
	enqueue(
		"finergy.integrations.doctype.s3_backup_settings.s3_backup_settings.take_backups_s3",
		queue="long",
		timeout=1500,
	)
...
def take_backups_if(freq):
	if cint(finergy.db.get_value("S3 Backup Settings", None, "enabled")):
		if finergy.db.get_value("S3 Backup Settings", None, "frequency") == freq:
			take_backups_s3()
...
def notify():
	error_message = finergy.get_traceback()
	send_email(False, "Amazon S3", "S3 Backup Settings", "notify_email", error_message)


def backup_to_s3():
	from finergy.utils import get_backups_path
	from finergy.utils.backups import new_backup

	doc = finergy.get_single("S3 Backup Settings")
	bucket = doc.bucket
