...
class TokenCache(Document):
	def get_auth_header(self):
		...
		raise finergy.exceptions.DoesNotExistError
	...
	def update_data(self, data):
		...
		if token_type not in ["bearer", "mac"]:
			finergy.throw(_("Received an invalid token type."))
		# 'Bearer' or 'MAC'
	...
	def get_expires_in(self):
		expiry_time = finergy.utils.get_datetime(self.modified) + timedelta(seconds=self.expires_in)
		return (expiry_time - finergy.utils.now_datetime()).total_seconds()
