...
from finergy.integrations.utils import create_payment_gateway, create_request_log
from finergy.model.document import Document
from finergy.utils import call_hook_method, cint, cstr, flt, get_request_site_address, get_url
...
def get_paytm_config():
	...
	paytm_config = finergy.db.get_singles_dict("Paytm Settings")
	paytm_config.update(
...
def get_paytm_params(payment_details, order_id, paytm_config):
	...
	redirect_uri = (
		...
		+ "/api/method/finergy.integrations.doctype.paytm_settings.paytm_settings.verify_transaction"
	)
...
def finalize_request(order_id, transaction_response):
	request = finergy.get_doc("Integration Request", order_id)
	transaction_data = finergy._dict(json.loads(request.data))
	...
	finergy.local.response["location"] = redirect_url


def get_gateway_controller(doctype, docname):
	reference_doc = finergy.get_doc(doctype, docname)
	gateway_controller = finergy.db.get_value(
