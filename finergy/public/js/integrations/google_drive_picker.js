...
export default class GoogleDrivePicker {
	...
	loadGapi() {
		...
		if (!finergy.boot.user.google_drive_token) {
	...
	handleAuthResult(authResult) {
		...
		if (authResult && !authResult.error) {
			finergy.boot.user.google_drive_token = authResult.access_token;
			this.createPicker();
		} else {
	...
	createPicker() {
		...
		if (this.pickerApiLoaded && finergy.boot.user.google_drive_token) {
			...
			this.picker = new google.picker.PickerBuilder()
				...
				.setOAuthToken(finergy.boot.user.google_drive_token)
				.addView(this.view)
				.setLocale(finergy.boot.lang)
