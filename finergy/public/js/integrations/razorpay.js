...
3. 	Inititate the payment in client using checkout API
	function make_payment(ticket) {
		...
		razorpay = new finergy.checkout.razorpay(options)
		razorpay.on_open = () => {
...
finergy.provide("finergy.checkout");

finergy.require('https://checkout.razorpay.com/v1/checkout.js').then(() => {
	finergy.checkout.razorpay = class RazorpayCheckout {
		constructor(opts) {
		...
		get_key() {
			return new Promise(resolve => {
				finergy.call("finergy.integrations.doctype.razorpay_settings.razorpay_settings.get_api_key").then(res => {
		...
		make_order() {
			return new Promise(resolve => {
				finergy.call("finergy.integrations.doctype.razorpay_settings.razorpay_settings.get_order", {
		...
		order_success(response) {
			finergy.call("finergy.integrations.doctype.razorpay_settings.razorpay_settings.order_payment_success", {
		...
		order_fail(response) {
			finergy.call( "finergy.integrations.doctype.razorpay_settings.razorpay_settings.order_payment_failure", {
				integration_request: this.order.integration_request,
				params: response
			})
		}
