finergy.RoleEditor = class {
	constructor(wrapper, frm, disable) {
		...
		this.multicheck = finergy.ui.form.make_control({
			...
			df: {
				...
				columns: 3,
				get_data: () => {
					return finergy.xcall('finergy.core.doctype.user.user.get_all_roles').then(roles => {
	...
	show_permissions(role) {
		...
		return finergy.xcall('finergy.core.doctype.user.user.get_perm_info', { role })
			.then(permissions => {
						...
						<table class="user-perm">
							<thead>
								<tr>
									...
									<th> ${__('Level')} </th>
									${finergy.perm.rights.map(p => `<th> ${finergy.unscrub(p)}</th>`).join("")}
								</tr>
							</thead>
							<tbody></tbody>
						</table>
					...
					permissions.forEach(perm => {
						$body.find('tbody').append(`
							<tr>
								...
								${finergy.perm.rights.map(p => `<td class="text-muted bold">${perm[p] ? finergy.utils.icon('check', 'xs') : '-'}</td>`).join("")}
							</tr>
						`);
					});
				}
	...
	make_perm_dialog() {
		this.perm_dialog = new finergy.ui.Dialog({
	...
	set_roles_in_table() {
		...
		roles.map(role_doc => {
			if (!checked_options.includes(role_doc.role)) {
				finergy.model.clear_doc(role_doc.doctype, role_doc.name);
			}
		});
		checked_options.map(role => {
			if (!roles.find(d => d.role === role)) {
				let role_doc = finergy.model.add_child(this.frm.doc, "Has Role", "roles");
				role_doc.role = role;
			}
		});
	}
