...
finergy.data_import.ImportPreview = class ImportPreview {
	constructor({
		wrapper,
		doctype,
		preview_data,
	...
	setup_styles() {
		...
		this.datatable.style.setStyle(`svg.import-success`, {
			width: '16px',
			fill: finergy.ui.color.get_color_shade('green', 'dark')
		});
		...
		this.datatable.style.setStyle(row_classes, {
			pointerEvents: 'none',
			backgroundColor: finergy.ui.color.get_color_shade('gray', 'extra-light'),
			color: finergy.ui.color.get_color_shade('gray', 'dark')
		});
	}
	...
	show_column_warning(_, $target) {
		...
		finergy.utils.scroll_to($warning, true, 30);
	}
...
function get_fields_as_options(doctype, column_map) {
	let keys = [doctype];
	finergy.meta.get_table_fields(doctype).forEach(df => {
	...
	return [].concat(
		...keys.map(key => {
			return column_map[key].map(df => {
				...
				if (doctype !== key) {
					let table_field = finergy.meta.get_docfield(doctype, key);
