...
finergy.data_import.DataExporter = class DataExporter {
	...
	make_dialog() {
		this.dialog = new finergy.ui.Dialog({
			...
			fields: [
				...
				...finergy.meta.get_table_fields(this.doctype).map(df => {
	...
	export_records() {
		let method =
			'/api/method/finergy.core.doctype.data_import.data_import.download_template';
	...
	make_select_all_buttons() {
		...
		`);
		finergy.utils.bind_actions_with_object($select_all_buttons, this);
		this.dialog
	...
	select_mandatory() {
		let mandatory_table_fields = finergy.meta
	...
	update_record_count_message() {
		...
		let count_method = {
			all: () => finergy.db.count(this.doctype),
			by_filter: () =>
				finergy.db.count(this.doctype, {
					filters: this.get_filters()
				}),
	...
	get_multicheck_options(doctype, child_fieldname = null) {
		...
		if (meta.autoname && meta.autoname.startsWith('field:')) {
			...
			autoname_field = finergy.meta.get_field(doctype, fieldname);
		}
...
export function get_columns_for_picker(doctype) {
	...
	const exportable_fields = df => {
		let keep = true;
		if (finergy.model.no_value_type.includes(df.fieldtype)) {
			keep = false;
		}
	...
	// parent
	let doctype_fields = finergy.meta
	...
	// children
	const table_fields = finergy.meta.get_table_fields(doctype);
	table_fields.forEach(df => {
		const cdt = df.options;
		const child_table_fields = finergy.meta
			.get_docfields(cdt)
			.filter(exportable_fields);
