...
finergy.ui.form.Dashboard = class FormDashboard {
	constructor(opts) {
		$.extend(this, opts);
	...
	setup_dashboard_sections() {
		...
		this.heatmap_area = new Section(this.parent, {
			...
			collapsible: 1,
			body_html: `
				<div id="heatmap-${finergy.model.scrub(this.frm.doctype)}" class="heatmap"></div>
	...
	refresh() {
		this.reset();
		if (this.frm.doc.__islocal || !finergy.boot.desk_settings.dashboard) {
			return;
		}
	...
	render_links() {
		...
		$(finergy.render_template('form_links', this.data))
	...
	render_report_links() {
		...
		if (this.data.reports && this.data.reports.length) {
			$(finergy.render_template('report_links', this.data))
				.appendTo(parent);
			// bind reports
	...
	open_report($link) {
		...
		finergy.provide('finergy.route_options');
		finergy.route_options[fieldname] = this.frm.doc.name;
		finergy.set_route("query-report", report);
	}
	...
	open_document_list($link, show_open) {
		...
		if (this.data.internal_links[doctype]) {
			if (names.length) {
				finergy.route_options = {'name': ['in', names]};
			} else {
				return false;
			}
		} else if (this.data.fieldname) {
			finergy.route_options = this.get_document_filter(doctype);
			if (show_open && finergy.ui.notifications) {
				finergy.ui.notifications.show_open_count_list(doctype);
			}
		}
	...
	set_open_count() {
		...
		var method = this.data.method || 'finergy.desk.notifications.get_open_count';
		finergy.call({
			type: "GET",
			method: method,
			args: {
	...
	render_heatmap() {
		if (!this.heatmap) {
			this.heatmap = new finergy.Chart("#heatmap-" + finergy.model.scrub(this.frm.doctype), {
				type: 'heatmap',
	...
	render_graph(args) {
		...
		this.chart = new finergy.Chart('.form-graph', args);
		if (!this.chart) {
			this.hide();
		}
	}
