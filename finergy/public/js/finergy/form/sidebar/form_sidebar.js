...
finergy.ui.form.Sidebar = class {
	constructor(opts) {
		$.extend(this, opts);
	}

	make () {
		var sidebar_content = finergy.render_template("form_sidebar", {doctype: this.frm.doctype, frm:this.frm});
		...
		finergy.ui.form.setup_user_image_event(this.frm);
	...
	bind_events () {
		...
		this.comments.on("click", function() {
			finergy.utils.scroll_to(me.frm.footer.wrapper.find(".comment-box"), true);
		});

		this.like_icon.on("click", function() {
			finergy.ui.toggle_like(me.like_wrapper, me.frm.doctype, me.frm.doc.name, function() {
				me.refresh_like();
			});
		});
	}
	...
	refresh () {
		if (this.frm.doc.__islocal) {
			...
			this.sidebar
				...
				.html(
					__("{0} edited this {1}", [
						finergy.user.full_name(this.frm.doc.modified_by).bold(),
			...
			this.sidebar
				...
				.html(
					__("{0} created this {1}", [
						finergy.user.full_name(this.frm.doc.owner).bold(),
			...
			finergy.ui.form.set_user_image(this.frm);
		}
	}
			...
			finergy.call({
				...
				callback: function(res) {
					...
					me.sidebar.find(".auto-repeat-status").on("click", function(){
						finergy.set_route("Form", "Auto Repeat", me.frm.doc.auto_repeat);
					});
				}
			});
		}
	...
	make_tags() {
		...
		this.frm.tags = new finergy.ui.TagEditor({
			parent: tags_parent,
	...
	make_attachments() {
		var me = this;
		this.frm.attachments = new finergy.ui.form.Attachments({
	...
	make_assignments() {
		this.frm.assign_to = new finergy.ui.form.AssignTo({
	...
	make_shared() {
		this.frm.shared = new finergy.ui.form.Share({
			frm: this.frm,
	...
	make_like() {
		...
		finergy.ui.setup_like_popover(this.sidebar.find(".form-stats-likes"), ".like-icon");
	}
	...
	make_follow() {
		...
		this.follow_button.on('click', () => {
			...
			finergy.call('finergy.desk.form.document_follow.update_follow', {
				'doctype': this.frm.doctype,
				'doc_name': this.frm.doc.name,
				'following': !is_followed
			}).then(() => {
				finergy.model.set_docinfo(this.frm.doctype, this.frm.doc.name, "is_document_followed", !is_followed);
	...
	refresh_like() {
		...
		const liked = finergy.ui.is_liked(this.frm.doc);
		this.like_wrapper
	...
	make_review() {
		...
		if (finergy.boot.energy_points_enabled && !this.frm.is_new()) {
			this.frm.reviews = new finergy.ui.form.Review({
	...
	reload_docinfo(callback) {
		finergy.call({
			method: "finergy.desk.form.load.get_docinfo",
			args: {
				doctype: this.frm.doctype,
