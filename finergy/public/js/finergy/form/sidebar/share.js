...
finergy.ui.form.Share = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	...
	render_sidebar: function() {
		...
		this.shares.append(finergy.avatar_group(shared_users, 5, {'align': 'left', 'overlap': true}));
	},
	show: function() {
		var me = this;
		...
		finergy.call({
			method: "finergy.share.get_users",
			args: {
	...
	render_shared: function(shared) {
		...
		$(finergy.render_template("set_sharing", {frm: this.frm, shared: this.shared, everyone: everyone}))
			.appendTo(d.body);

		if(finergy.model.can_share(null, this.frm)) {
	...
	make_user_input: function() {
		// make add-user input
		this.dialog.share_with = finergy.ui.form.make_control({
	...
	set_edit_share_events: function() {
		...
		$(d.body).find(".edit-share").on("click", function() {
			...
			finergy.call({
				method: "finergy.share.set_permission",
				args: {
					doctype: me.frm.doctype,
					name: me.frm.doc.name,
					user: user,
