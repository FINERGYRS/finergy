...
finergy.ui.form.AssignTo = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	...
	render: function(assignments) {
		...
		let avatar_group = finergy.avatar_group(assigned_users, 5, {'align': 'left', 'overlap': true});

		assignments_wrapper.show();
		assignments_wrapper.append(avatar_group);
		avatar_group.click(() => {
			new finergy.ui.form.AssignmentDialog({
	...
	add: function() {
		...
		if (this.frm.is_new()) {
			finergy.throw(__("Please save the document before assignment"));
			return;
		}

		if (!me.assign_to) {
			me.assign_to = new finergy.ui.form.AssignToDialog({
	...
	remove: function(owner) {
		if (this.frm.is_new()) {
			finergy.throw(__("Please save the document before removing assignment"));
			return;
		}

		return finergy.xcall('finergy.desk.form.assign_to.remove', {
...
finergy.ui.form.AssignToDialog = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	...
	make: function() {
		...
		me.dialog = new finergy.ui.Dialog({
	...
	assign_to_me: function() {
		...
		if (me.dialog.get_value("assign_to_me")) {
			assign_to.push(finergy.session.user);
		}
		...
		return [
			...
			},
			{
				...
				get_data: function(txt) {
					return finergy.db.get_link_options("User", txt, {user_type: "System User", enabled: 1});
				}
			},
			{
...
finergy.ui.form.AssignmentDialog = class {
	constructor(opts) {
		this.frm = opts.frm;
	...
	make() {
		...
		this.assignment_list.removeClass('finergy-control');
	...
	add_assignment(assignment) {
		return finergy.xcall('finergy.desk.form.assign_to.add', {
			doctype: this.frm.doctype,
			name: this.frm.docname,
	...
	get_assignment_row(assignment) {
		let row = $(`
			<div class="dialog-assignment-row" data-user="${assignment}">
				<span>
					${finergy.avatar(assignment)}
					${finergy.user.full_name(assignment)}
				</span>
			</div>
		`);

		if (assignment === finergy.session.user || this.frm.perm[0].write) {
			row.append(`
				<span class="remove-btn cursor-pointer">
					${finergy.utils.icon('close')}
				</span>
			`);
