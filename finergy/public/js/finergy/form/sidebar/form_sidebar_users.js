finergy.ui.form.SidebarUsers = class {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	show_in_sidebar(users, type, show_alert) {
		...
		users.current.forEach(username => {
			if (username === finergy.session.user) {
				// current user
				return;
			}

			var user_info = finergy.user_info(username);
			sidebar_users.push({
		...
		if (sidebar_users.length) {
			...
			this.parent.append(finergy.render_template('users_in_sidebar', {'users': sidebar_users}));
		} else {
	...
	show_alert(users, message) {
		if (users.length) {
			if (users.length===1) {
				finergy.show_alert(__('{0} is currently {1}', [users[0], message]));
			} else {
				finergy.show_alert(__('{0} are currently {1}', [finergy.utils.comma_and(users), message]));
			}
		}
	}
};
