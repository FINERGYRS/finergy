...
finergy.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = finergy.boot.points;
		this.reviews = this.parent.find('.reviews');
		this.setup_add_review_button();
		this.update_reviewers();
	}
	update_points() {
		return finergy.xcall('finergy.social.doctype.energy_point_log.energy_point_log.get_energy_points', {
			user: finergy.session.user
		}).then(data => {
			finergy.boot.points = data;
			this.points = data;
		});
	}
	...
	get_involved_users() {
		...
		return involved_users
			.uniqBy(u => u)
			.filter(user => !['Administrator', finergy.session.user].includes(user))
			.filter(Boolean);
	}
	show_review_dialog() {
		const user_options = this.get_involved_users();
		const review_dialog = new finergy.ui.Dialog({
			'title': __('Add Review'),
			'fields': [{
				fieldname: 'to_user',
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return finergy.msgprint(__('You do not have enough points'));
				}
				finergy.xcall('finergy.social.doctype.energy_point_log.energy_point_log.review', {
					doc: {
	...
	setup_detail_popover(el, data) {
		let subject = '';
		let fullname = finergy.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${finergy.datetime.comment_when(data.creation)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${finergy.utils.icon('review')}
							${subject}

							<p class="mt-1">
								<!-- ${finergy.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${finergy.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;
