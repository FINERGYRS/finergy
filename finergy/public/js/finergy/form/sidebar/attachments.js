...
finergy.ui.form.Attachments = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.make();
	},
	...
	add_attachment: function(attachment) {
		...
		if (finergy.model.can_write(this.frm.doctype, this.frm.name)) {
			remove_action = function(target_id) {
				finergy.confirm(__("Are you sure you want to delete the attachment?"),
					function() {
		...
		const icon = `<a href="/app/file/${fileid}">
				${finergy.utils.icon(attachment.is_private ? 'lock' : 'unlock', 'sm ml-0')}
			</a>`;

		$(`<li class="attachment-row">`)
			.append(finergy.get_data_pill(
				file_label,
				fileid,
				remove_action,
				icon
	...
	remove_attachment: function(fileid, callback) {
		...
		var me = this;
		return finergy.call({
			method: 'finergy.desk.form.utils.remove_attach',
			args: {
				fid: fileid,
				dt: me.frm.doctype,
				dn: me.frm.docname
			...
			callback: function(r,rt) {
				if(r.exc) {
					if(!r._server_messages)
						finergy.msgprint(__("There were errors"));
					return;
				}
