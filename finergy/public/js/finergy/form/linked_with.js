...
finergy.ui.form.LinkedWith = class LinkedWith {
	...
	load_doctypes() {
		...
		const promises = doctypes_to_load.map(dt => {
			return finergy.model.with_doctype(dt, () => {
				if(finergy.listview_settings[dt]) {
					// add additional fields to __linked_doctypes
					this.frm.__linked_doctypes[dt].add_fields =
						finergy.listview_settings[dt].add_fields;
				}
			});
		});
	...
	links_not_permitted_or_missing() {
		...
		if (this.frm.__linked_doctypes) {
			links =
				Object.keys(this.frm.__linked_doctypes)
					.filter(finergy.model.can_get_report);
		}
	...
	get_linked_docs() {
		return finergy.call({
			method: "finergy.desk.form.linked_with.get_linked_docs",
			args: {
				doctype: this.frm.doctype,
		...
		return `<div class="list-row-container">
			<div class="level list-row small">
				<div class="level-left bold">
					<a href="/app/${finergy.router.slug(doctype)}/${doc.name}">${doc.name}</a>
				</div>
			</div>
		</div>`;
	}
