...
export default class GridRow {
	constructor(opts) {
		...
		if (this.doc && this.parent_df.options) {
			finergy.meta.make_docfield_copy_for(this.parent_df.options, this.doc.name, this.docfields);
			const docfields = finergy.meta.get_docfields(this.parent_df.options, this.doc.name);
		...
		if(this.grid.is_editable()) {
			if(this.frm) {
				...
				finergy.run_serially([
					...
					() => {
						finergy.model.clear_doc(this.doc.doctype, this.doc.name);
	...
	move() {
		...
		finergy.prompt({
			...
			if (me.doc._sortable === false) {
				finergy.msgprint(__('Cannot move row'));
				return;
			}
	...
	render_template() {
		...
		this.row_display = $('<div class="row-data sortable-handle template-row"></div>').appendTo(this.row)
			.html(finergy.render(this.grid.template, {
				doc: this.doc ? finergy.get_format_helper(this.doc) : null,
				frm: this.frm,
				row: this
			}));
	}
				...
				if (!this.configure_columns) {
					this.open_form_button = $(`
						<div class="btn-open-row">
							<a>${finergy.utils.icon('edit', 'xs')}</a>
		...
		if (this.configure_columns && this.frm) {
			this.configure_columns_button = $(`
				<div class="col grid-static-col col-xs-1 d-flex justify-content-center" style="cursor: pointer;">
					<a>${finergy.utils.icon('setting-gear', 'sm', '', 'filter: opacity(0.5)')}</a>
				</div>
			`)
				.appendTo(this.row)
				.on('click', () => {
	...
	configure_dialog_for_columns_selector() {
		this.grid_settings_dialog = new finergy.ui.Dialog({
		...
		d.set_primary_action(__('Add'), () => {
			...
			if (selected_fields) {
				selected_fields.forEach(selected_column => {
					let docfield = finergy.meta.get_docfield(this.grid.doctype, selected_column);
	...
	prepare_columns_for_dialog(selected_fields) {
		...
		const blocked_fields = finergy.model.no_value_type;
	...
	render_selected_columns() {
		...
		if (this.selected_columns_for_grid) {
			this.selected_columns_for_grid.forEach(d => {
				let docfield = finergy.meta.get_docfield(this.grid.doctype, d.fieldname);
				...
				fields += `
					<div class='control-input flex align-center form-control fields_order sortable-handle sortable'
						...
						<div class='row'>
							<div class='col-md-1'>
								<a style='cursor: grabbing;'>${finergy.utils.icon('drag', 'xs')}</a>
							</div>
	...
	update_column_width() {
		$(this.fields_html_wrapper).find('.column-width').change((event) => {
			if (cint(event.target.value) === 0) {
				...
				finergy.throw(__('Column width cannot be zero.'));
			}
	...
	validate_columns_width() {
		...
		if (total_column_width && total_column_width > 10) {
			finergy.throw(__('The total column width cannot be more than 10.'));
		}
	}
	...
	remove_selected_column() {
		$(this.fields_html_wrapper).find('.remove-field').click((event) => {
			...
			if (selected_columns_for_grid && selected_columns_for_grid.length === 0) {
				finergy.throw(__('At least one column is required to show in the grid.'));
			}
	...
	update_user_settings_for_grid() {
		...
		finergy.model.user_settings.save(this.frm.doctype, 'GridView', value)
			.then((r) => {
				finergy.model.user_settings[this.frm.doctype] = r.message || r;
				this.grid.reset_grid();
			});
	}

	reset_user_settings_for_grid() {
		finergy.model.user_settings.save(this.frm.doctype, 'GridView', null)
			.then((r) => {
				finergy.model.user_settings[this.frm.doctype] = r.message || r;
				this.grid.reset_grid();
			});
	...
	setup_columns() {
		...
		this.grid.visible_columns.forEach((col, ci) => {
			...
			let colsize = col[1];
			let txt = this.doc ?
				finergy.format(this.doc[df.fieldname], df, null, this.doc) :
				__(df.label);
	...
	evaluate_depends_on_value(expression) {
		...
		if (typeof (expression) === 'boolean') {
			...
			try {
				out = finergy.utils.eval(expression.substr(5), { doc, parent });
				if (parent && parent.istable && expression.includes('is_submittable')) {
					out = true;
				}
			} catch (e) {
				finergy.throw(__('Invalid "depends_on" expression'));
			}
	...
	make_column(df, colsize, txt, ci) {
		...
		var $col = $('<div class="col grid-static-col col-xs-'+colsize+' '+add_class+'"></div>')
			...
			.appendTo(this.row)
			.on('click', function() {
				if(finergy.ui.form.editable_row===me) {
					return;
				}
	...
	toggle_editable_row(show) {
		...
		if(this.grid.allow_on_grid_editing() && this.grid.is_editable() && this.doc && show !== false) {
			...
			if(finergy.ui.form.editable_row
				&& finergy.ui.form.editable_row !== this) {
			...
			finergy.ui.form.editable_row = null;
		}
	}
	...
	make_control(column) {
		...
		var field = finergy.ui.form.make_control({
			df: df,
			parent: parent,
			only_input: true,
			with_link_btn: true,
	...
	set_arrow_keys(field) {
		...
		if (field.$input) {
			field.$input.on('keydown', function(e) {
				var { TAB, UP: UP_ARROW, DOWN: DOWN_ARROW } = finergy.ui.keyCode;
	...
	show_form() {
		...
		finergy.dom.freeze("", "dark");
		if (cur_frm) cur_frm.cur_grid = this;
		this.wrapper.addClass("grid-row-open");
		if (!finergy.dom.is_element_in_viewport(this.wrapper)
			&& !finergy.dom.is_element_in_modal(this.wrapper)) {
			// -15 offset to make form look visually centered
			finergy.utils.scroll_to(this.wrapper, true, -15);
		}
	...
	hide_form() {
		...
		if (!finergy.dom.is_element_in_modal(this.row)) {
			finergy.utils.scroll_to(this.row, true, 15);
		}
		this.refresh();
	...
	refresh_field(fieldname, txt) {
		...
		if (df && this.doc) {
			txt = finergy.format(this.doc[fieldname], df, null, this.doc);
		}

		if (!txt && this.frm) {
			txt = finergy.format(this.doc[fieldname], df, null, this.frm.doc);
		}
	...
	get_visible_columns(blacklist=[]) {
		...
		var visible_columns = $.map(this.docfields, function(df) {
			var visible = !df.hidden && df.in_list_view && me.grid.frm.get_perm(df.permlevel, "read")
				&& !in_list(finergy.model.layout_fields, df.fieldtype) && !in_list(blacklist, df.fieldname);
