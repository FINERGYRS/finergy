...
finergy.ui.form.save = function (frm, action, callback, btn) {
	...
	var save = function () {
		...
		if ((action !== 'Save' || frm.is_dirty()) && check_mandatory()) {
			_call({
				method: "finergy.desk.form.save.savedocs",
			...
			!frm.is_dirty() && finergy.show_alert({message: __("No changes in document"), indicator: "orange"});
	...
	var remove_empty_rows = function() {
		...
		*/
		const docs = finergy.model.get_all_docs(frm.doc);

		// we should only worry about table data
		const tables = docs.filter(d => {
			return finergy.model.is_table(d.doctype);
		});

		let modified_table_fields = [];

		tables.map(doc => {
			const cells = finergy.meta.docfield_list[doc.doctype] || [];
			...
			if (is_empty_row(in_list_view_cells)) {
				finergy.model.clear_doc(doc.doctype, doc.name);
	...
	var cancel = function () {
		...
		var workflow_state_fieldname = finergy.workflow.get_state_fieldname(frm.doctype);
		...
		_call({
			method: "finergy.desk.form.save.cancel",
			args: args,
			callback: function (r) {
				$(document).trigger("save", [frm.doc]);
				callback(r);
	...
	var check_mandatory = function () {
		...
		$.each(finergy.model.get_all_docs(frm.doc), function (i, doc) {
			var error_fields = [];
			var folded = false;

			$.each(finergy.meta.docfield_list[doc.doctype] || [], function (i, docfield) {
				if (docfield.fieldname) {
					const df = finergy.meta.get_docfield(doc.doctype,
					...
					if (is_docfield_mandatory(doc, df) &&
						!finergy.model.has_value(doc.doctype, doc.name, df.fieldname)) {
						has_errors = true;
			...
			if (error_fields.length) {
				let meta = finergy.get_meta(doc.doctype);
				if (meta.istable) {
					const table_label = __(finergy.meta.docfield_map[doc.parenttype][doc.parentfield].label).bold();
	...
	let is_docfield_mandatory = function(doc, df) {
		...
		let parent = finergy.get_meta(df.parent);
		...
		if (typeof (expression) === 'boolean') {
			...
			try {
				...
				finergy.throw(__('Invalid "mandatory_depends_on" expression'));
			}
	...
	var _call = function (opts) {
		...
		if ( frm.is_new() ) {
			finergy.ui.form.remove_old_form_route();
		}
		...
		return finergy.call({
			...
			always: function (r) {
				...
				if (r) {
					...
					if (doc) {
						finergy.ui.form.update_calling_link(doc);
					}
				}
			}
		})
...
finergy.ui.form.remove_old_form_route = () => {
	let current_route = finergy.get_route().join("/");
	finergy.route_history = finergy.route_history
		.filter((route) => route.join("/") !== current_route);
}

finergy.ui.form.update_calling_link = (newdoc) => {
	if (!finergy._from_link) return;
	var doc = finergy.get_doc(finergy._from_link.doctype, finergy._from_link.docname);

	let is_valid_doctype = () => {
		if (finergy._from_link.df.fieldtype==='Link') {
			return newdoc.doctype === finergy._from_link.df.options;
		} else {
			// dynamic link, type is dynamic
			return newdoc.doctype === doc[finergy._from_link.df.options];
		}
	};
	...
	if (is_valid_doctype()) {
		...
		if (doc && doc.parentfield) {
			...
			$.each(finergy._from_link.frm.fields_dict[doc.parentfield].grid.grid_rows, function (index, field) {
				if (field.doc && field.doc.name === finergy._from_link.docname) {
					finergy._from_link.set_value(newdoc.name);
				}
			});
		} else {
			finergy._from_link.set_value(newdoc.name);
		...
		// refresh field
		finergy._from_link.refresh();

		// if from form, switch
		if (finergy._from_link.frm) {
			finergy.set_route("Form",
				finergy._from_link.frm.doctype, finergy._from_link.frm.docname)
				.then(() => {
					finergy.utils.scroll_to(finergy._from_link_scrollY);
				});
		}

		finergy._from_link = null;
	}
}
