...
finergy.provide('finergy.success_action');

finergy.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = finergy.boot.success_action
	...
	show_alert() {
		finergy.db.get_list(this.form.doctype, {limit: 2})
			.then(result => {
	...
	get default_actions() {
		return {
			'new': {
				label: __('New'),
				action: (frm) => finergy.new_doc(frm.doctype)
			},
			'print': {
				label: __('Print'),
			...
			'list': {
				label: __('View All'),
				action: (frm) => {
					finergy.set_route('List', frm.doctype);
				}
			}
		};
	}
