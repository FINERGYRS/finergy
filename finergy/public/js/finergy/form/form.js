...
finergy.provide('finergy.model.docinfo');
...
finergy.ui.form.Controller = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	}
});

finergy.ui.form.Form = class FinergyForm {
	constructor(doctype, parent, in_form, doctype_layout_name) {
		...
		this.sections = [];
		this.grids = [];
		this.cscript = new finergy.ui.form.Controller({ frm: this });
		this.events = {};
		this.fetch_dict = {};
		this.parent = parent;
		this.doctype_layout = finergy.get_doc('DocType Layout', doctype_layout_name);
	...
	setup_meta() {
		this.meta = finergy.get_doc('DocType', this.doctype);

		if(this.meta.istable) {
			this.meta.in_dialog = 1;
		}

		this.perm = finergy.perm.get_perm(this.doctype); // for create
	...
	setup() {
		this.fields = [];
		this.fields_dict = {};
		this.state_fieldname = finergy.workflow.get_state_fieldname(this.doctype);
		...
		this.toolbar = new finergy.ui.form.Toolbar({
			frm: this,
			page: this.page
		});
		...
		this.script_manager = new finergy.ui.form.ScriptManager({
			frm: this
		});
		this.script_manager.setup();
		this.watch_model_updates();

		if (!this.meta.hide_toolbar && finergy.boot.desk_settings.timeline) {
			this.footer = new finergy.ui.form.Footer({
				frm: this,
	...
	setup_std_layout() {
		...
		// layout
		this.layout = new finergy.ui.form.Layout({
			parent: this.body,
		...
		this.dashboard = new finergy.ui.form.Dashboard({
			frm: this,
			parent: $('<div class="form-dashboard">').insertAfter(this.layout.wrapper.find('.form-message'))
		});

		// workflow state
		this.states = new finergy.ui.form.States({
			frm: this
		});
	}
	...
	watch_model_updates() {
		...
		// on main doc
		finergy.model.on(me.doctype, "*", function(fieldname, value, doc) {
			// set input
		...
		// on table fields
		var table_fields = finergy.get_children("DocType", me.doctype, "fields", {
			fieldtype: ["in", finergy.model.table_fields]
		});

		// using $.each to preserve df via closure
		$.each(table_fields, function(i, df) {
			finergy.model.on(df.options, "*", function(fieldname, value, doc) {
	...
	setup_file_drop() {
		...
		this.$wrapper.on('dragenter dragover', false)
			.on('drop', function(e) {
				...
				if(me.doc.__islocal) {
					finergy.msgprint(__("Please save before attaching."));
					throw "attach error";
				}
	...
	refresh(docname) {
		...
		if(this.docname) { // document to show
			...
			// set the doc
			this.doc = finergy.get_doc(this.doctype, this.docname);

			// check permissions
			if (!this.has_read_permission()) {
				finergy.show_not_permitted(__(this.doctype) + " " + __(this.docname));
				return;
			}

			// read only (workflow)
			this.read_only = finergy.workflow.is_read_only(this.doctype, this.docname);
	...
	setup_doctype_actions() {
		if (this.meta.actions) {
			for (let action of this.meta.actions) {
				finergy.ui.form.on(this.doctype, 'refresh', () => {
					if (!this.is_new()) {
						if (!action.hidden) {
	...
	execute_action(action) {
		if (typeof action === 'string') {
			...
			if (typeof action === 'string') {
				finergy.throw(`Action ${action} not found`);
			}
		}
		if (action.action_type==='Server Action') {
			finergy.xcall(action.action, {'doc': this.doc}).then((doc) => {
				if (doc.doctype) {
					// document is returned by the method,
					// apply the changes locally and refresh
					finergy.model.sync(doc);
					this.refresh();
				}
			...
			finergy.set_route(action.action);
		}
	}
	...
	initialize_new_doc() {
		...
		this.script_manager.trigger("before_load", this.doctype, this.docname)
			.then(() => {
				...
				finergy.after_ajax(function() {
					me.trigger_link_fields();
				});

				finergy.breadcrumbs.add(me.meta.module, me.doctype);
			});
	...
	render_form(switched) {
		if(!this.meta.istable) {
			...
			if (finergy.boot.desk_settings.form_sidebar) {
				this.sidebar = new finergy.ui.form.Sidebar({
					frm: this,
					page: this.page
				});
				this.sidebar.make();
	...
	run_after_load_hook() {
		if (finergy.route_hooks.after_load) {
			let route_callback = finergy.route_hooks.after_load;
			delete finergy.route_hooks.after_load;
	...
	refresh_header(switched) {
		...
		if(!this.meta.in_dialog || this.in_form) {
			finergy.utils.set_title(this.meta.issingle ? this.doctype : this.docname);
		}
	...
	validate_and_save(save_action, callback, btn, on_error, resolve, reject) {
		...
		var after_save = function(r) {
			if(!r.exc) {
				if (["Save", "Update", "Amend"].indexOf(save_action)!==-1) {
					finergy.utils.play_sound("click");
				}

				me.script_manager.trigger("after_save");

				if (finergy.route_hooks.after_save) {
					let route_callback = finergy.route_hooks.after_save;
					delete finergy.route_hooks.after_save;
		...
		if(save_action != "Update") {
			// validate
			finergy.validated = true;
			finergy.run_serially([
				() => this.script_manager.trigger("validate"),
				() => this.script_manager.trigger("before_save"),
				() => {
					if(!finergy.validated) {
						fail();
						return;
					}

					finergy.ui.form.save(me, save_action, after_save, btn);
				}
			]).catch(fail);
		} else {
			finergy.ui.form.save(me, save_action, after_save, btn);
	...
	savesubmit(btn, callback, on_error) {
		...
		return new Promise(resolve => {
			...
			finergy.confirm(__("Permanently Submit {0}?", [this.docname]), function() {
				finergy.validated = true;
				me.script_manager.trigger("before_submit").then(function() {
					...
					me.save('Submit', function(r) {
						if(r.exc) {
							...
							finergy.utils.play_sound("submit");
							callback && callback();
							me.script_manager.trigger("on_submit")
								.then(() => resolve(me))
								.then(() => {
									if (finergy.route_hooks.after_submit) {
										let route_callback = finergy.route_hooks.after_submit;
										delete finergy.route_hooks.after_submit;
										route_callback(me);
									}
								});
						}
	...
	savecancel(btn, callback, on_error) {
		...
		finergy.call({
			method: "finergy.desk.form.linked_with.get_submitted_linked_docs",
			args: {
	...
	_cancel_all(r, btn, callback, on_error) {
		...
		for (let doctype of doctypes) {
			if (!me.ignore_doctypes_on_cancel_all.includes(doctype)) {
				let docnames = links
					...
					.map((link) => finergy.utils.get_form_link(link.doctype, link.name, true))
					.join(", ");
		...
		let can_cancel = links.every((link) => finergy.model.can_cancel(link.doctype));
		if (can_cancel) {
		...
		let d = new finergy.ui.Dialog({
			...
			fields: [{
				fieldtype: "HTML",
				options: `<p class="finergy-confirm-message">${confirm_message}</p>`
			}]
		}, () => me.handle_save_fail(btn, on_error));
		...
		if (can_cancel) {
			d.set_primary_action("Cancel All", () => {
				d.hide();
				finergy.call({
					method: "finergy.desk.form.linked_with.cancel_all_linked_docs",
					args: {
						docs: links,
	...
	_cancel(btn, callback, on_error, skip_confirm) {
			...
			me.script_manager.trigger("before_cancel").then(() => {
				...
				var after_cancel = function(r) {
					if (r.exc) {
						...
						finergy.utils.play_sound("cancel");
						me.refresh();
						callback && callback();
						me.script_manager.trigger("after_cancel");
					}
				};
				finergy.ui.form.save(me, "cancel", after_cancel, btn);
			});
		}
		...
		if (skip_confirm) {
			...
			finergy.confirm(__("Permanently Cancel {0}?", [this.docname]), cancel_doc, me.handle_save_fail(btn, on_error));
		}
	};

	savetrash() {
		this.validate_form_action("Delete");
		finergy.model.delete_doc(this.doctype, this.docname, function() {
			window.history.back();
		});
	}
	...
	amend_doc() {
		if (!this.fields_dict['amended_from']) {
			finergy.msgprint(__('"amended_from" field must be present to do an amendment.'));
			return;
		}

		finergy.xcall('finergy.client.is_document_amended', {
			'doctype': this.doc.doctype,
			'docname': this.doc.name
		}).then(is_amended => {
			if (is_amended) {
				finergy.throw(__('This document is already amended, you cannot ammend it again'));
			}
			...
			var fn = function(newdoc) {
				...
				if (me.fields_dict && me.fields_dict['amendment_date'])
					newdoc.amendment_date = finergy.datetime.obj_to_str(new Date());
			};
			this.copy_doc(fn, 1);
			finergy.utils.play_sound("click");
		});
	...
	validate_form_action(action, resolve) {
		...
		var perms = finergy.perm.get_perm(this.doc.doctype)[0];
		...
		if ((finergy.workflow.is_read_only(this.doctype, this.docname) && (perms["write"] ||
			perms["create"] || perms["submit"] || perms["cancel"])) || !finergy.workflow.is_read_only(this.doctype, this.docname)) {
	...
	show_submit_message() {
		if(this.meta.is_submittable
			...
			&& !this.is_dirty()
			&& !this.is_new()
			&& !finergy.model.has_workflow(this.doctype) // show only if no workflow
	...
	has_read_permission() {
		...
		this.perm = finergy.perm.get_perm(dt, this.doc);
	...
	check_doctype_conflict(docname) {
		if(this.doctype=='DocType' && docname=='DocType') {
			finergy.msgprint(__('Allowing DocType, DocType. Be careful!'));
		} else if(this.doctype=='DocType') {
			if (finergy.views.formview[docname] || finergy.pages['List/'+docname]) {
				window.location.reload();
				//	finergy.msgprint(__("Cannot open {0} when its instance is open", ['DocType']))
				// throw 'doctype open conflict'
			}
		} else {
			if (finergy.views.formview.DocType && finergy.views.formview.DocType.frm.opendocs[this.doctype]) {
				window.location.reload();
				//	finergy.msgprint(__("Cannot open instance when its {0} is open", ['DocType']))
				// throw 'doctype open conflict'
			}
		}
	}
	...
	rename_notify(dt, old, name) {
		...
		// cleanup
		if(this && this.opendocs[old] && finergy.meta.docfield_copy[dt]) {
			// delete docfield copy
			finergy.meta.docfield_copy[dt][name] = finergy.meta.docfield_copy[dt][old];
			delete finergy.meta.docfield_copy[dt][old];
		}
		...
		finergy.re_route[finergy.router.get_sub_path()] = `${encodeURIComponent(finergy.router.slug(this.doctype))}/${encodeURIComponent(name)}`;
		!finergy._from_link && finergy.set_route('Form', this.doctype, name);
	}
	...
	print_doc() {
		...
		finergy.set_route('print', this.doctype, this.doc.name);
	}

	navigate_records(prev) {
		let filters, sort_field, sort_order;
		let list_view = finergy.get_list_view(this.doctype);
		if (list_view) {
			filters = list_view.get_filters_for_args();
			sort_field = list_view.sort_by;
			sort_order = list_view.sort_order;
		} else {
			let list_settings = finergy.get_user_settings(this.doctype)['List'];
			if (list_settings) {
		...
		finergy.call('finergy.desk.form.utils.get_next', args).then(r => {
			if (r.message) {
				finergy.set_route('Form', this.doctype, r.message);
	...
	rename_doc() {
		finergy.model.rename_doc(this.doctype, this.docname, () => this.refresh_header());
	}
	...
	email_doc(message) {
		new finergy.views.CommunicationComposer({
			doc: this.doc,
			frm: this,
	...
	copy_doc(onload, from_amend) {
		...
		var newdoc = finergy.model.copy_doc(this.doc, from_amend);
	...
	reload_doc() {
		...
		if(!this.doc.__islocal) {
			finergy.model.remove_from_locals(this.doctype, this.docname);
			return finergy.model.with_doc(this.doctype, this.docname, () => {
				this.refresh();
			});
		}
	}
	...
	has_perm(ptype) {
		return finergy.perm.has_perm(this.doctype, 0, ptype, this.doc);
	}
	...
	get_docinfo() {
		return finergy.model.docinfo[this.doctype][this.docname];
	}
	...
	set_footnote(txt) {
		this.footnote_area = finergy.utils.set_footnote(this.footnote_area, this.body, txt);
	}
	...
	scroll_to_element() {
		if (finergy.route_options && finergy.route_options.scroll_to) {
			var scroll_to = finergy.route_options.scroll_to;
			delete finergy.route_options.scroll_to;
			...
			if (selector.length) {
				finergy.utils.scroll_to(selector);
			}
		}
	}
	...
	show_success_action() {
		...
		const success_action = new finergy.ui.form.SuccessAction(this);
		success_action.show();
	}
	...
	set_currency_labels(fields_list, currency, parentfield) {
		...
		$.each(fields_list, function(i, fname) {
			var docfield = finergy.meta.docfield_map[doctype][fname];
			if(docfield) {
	...
	field_map(fnames, fn) {
		...
		for (var i=0, l=fnames.length; i<l; i++) {
			...
			var field = finergy.meta.get_docfield(this.doctype, fieldname, this.docname);
			if(field) {
				fn(field);
	...
	get_docfield(fieldname1, fieldname2) {
		if(fieldname2) {
			...
			return finergy.meta.get_docfield(doctype, fieldname2, this.docname);
		} else {
			// for parent
			return finergy.meta.get_docfield(this.doctype, fieldname1, this.docname);
		}
	}
	...
	set_df_property(fieldname, property, value, docname, table_field, table_row_name=null) {
		...
		if (!docname || !table_field) {
			...
			const filtered_fields = finergy.utils.filter_dict(grid.docfields, {'fieldname': table_field});
			if (filtered_fields.length) {
				df = finergy.meta.get_docfield(filtered_fields[0].parent, table_field, table_row_name);
			}
		}
	...
	get_files() {
		return this.attachments
			? finergy.utils.sort(this.attachments.get_attachments(), "file_name", "string")
			: [] ;
	}
	...
	clear_table(fieldname) {
		finergy.model.clear_table(this.doc, fieldname);
	}

	add_child(fieldname, values) {
		var doc = finergy.model.add_child(this.doc, finergy.meta.get_docfield(this.doctype, fieldname).options, fieldname);
		if(values) {
	...
	set_value(field, value, if_missing) {
		...
		var _set = function(f, v) {
			...
			if(fieldobj) {
				if(!if_missing || !finergy.model.has_value(me.doctype, me.doc.name, f)) {
					if(finergy.model.table_fields.includes(fieldobj.df.fieldtype) && $.isArray(v)) {

						finergy.model.clear_table(me.doc, fieldobj.df.fieldname);

						for (var i=0, j=v.length; i < j; i++) {
							var d = v[i];
							var child = finergy.model.add_child(me.doc, fieldobj.df.options,
						...
						return finergy.model.set_value(me.doctype, me.doc.name, f, v);
					}
				}
			} else {
				finergy.msgprint(__("Field {0} not found.",[f]));
				throw "frm.set_value";
			}
		};
	...
	call(opts, args, callback) {
		...
		if(!opts.doc) {
			if(opts.method.indexOf(".")===-1)
				opts.method = finergy.model.get_server_module_name(me.doctype) + "." + opts.method;
			...
			opts.callback = function(r) {
				if($.isPlainObject(r.message)) {
					if(opts.child) {
						...
						var std_field_list = ["doctype"].concat(finergy.model.std_fields_list);
	...
	set_read_only() {
		const docperms = finergy.perm.get_perm(this.doc.doctype);
	...
	get_formatted(fieldname) {
		return finergy.format(this.doc[fieldname],
			finergy.meta.get_docfield(this.doctype, fieldname, this.docname),
			{no_icon:true}, this.doc);
	}

	open_grid_row() {
		return finergy.ui.form.get_open_grid_form();
	}
	...
	get_selected() {
		...
		finergy.meta.get_table_fields(this.doctype).forEach(function(df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		var doctype;
		if (finergy.meta.docfield_map[this.doctype][fieldname]) {
			doctype = this.doctype;
		} else {
			finergy.meta.get_table_fields(this.doctype).every(function(df) {
				if (finergy.meta.docfield_map[df.options][fieldname]) {
					doctype = df.options;
					return false;
				} else {
					return true;
		...
		finergy.meta.docfield_map[doctype][fieldname].formatter =
			function(value, df, options, doc) {
				if (value) {
					var label;
					if (get_text) {
						label = get_text(doc);
					} else if (finergy.form.link_formatters[df.options]) {
						label = finergy.form.link_formatters[df.options](value, doc);
					} else {
						label = value;
					}
					...
					return `
						<a class="indicator ${get_color(doc || {})}"
							href="/app/${finergy.router.slug(df.options)}/${escaped_name}"
	...
	can_create(doctype) {
		...
		if(!finergy.model.can_create(doctype)) {
			return false;
		}
	...
	make_new(doctype) {
		...
		if(this.make_methods && this.make_methods[doctype]) {
			...
			finergy.model.with_doctype(doctype, function() {
				let new_doc = finergy.model.get_new_doc(doctype, null, null, true);

				// set link fields (if found)
				me.set_link_field(doctype, new_doc);

				finergy.ui.form.make_quick_entry(doctype, null, null, new_doc);
				// finergy.set_route('Form', doctype, new_doc.name);
			});
		}
	}
	...
	set_link_field(doctype, new_doc) {
		let me = this;
		finergy.get_meta(doctype).fields.forEach(function(df) {
	...
	scroll_to_field(fieldname) {
		...
		// scroll to input
		finergy.utils.scroll_to($el, true, 15);
	...
	show_tour(on_finish) {
		const tour_info = finergy.tour[this.doctype];

		if (!Array.isArray(tour_info)) {
			return;
		}

		const driver = new finergy.Driver({
			className: 'finergy-driver',
			allowClose: false,
			padding: 10,
		...
		let steps = tour_info.map(step => {
			...
			return {
				element: `.finergy-control[data-fieldname='${step.fieldname}']`,
				popover: {
		...
		finergy.router.on('change', () => driver.reset());
		driver.start();
	}
	...
	setup_docinfo_change_listener() {
		...
		// to avoid duplicates
		finergy.realtime.off(listener_name);
		finergy.realtime.on(listener_name, ({doc, key, action='update'}) => {
			let doc_list = (finergy.model.docinfo[doctype][docname][key] || []);
			if (action === 'add') {
				finergy.model.docinfo[doctype][docname][key].push(doc);
			}
			...
			if (docindex > -1) {
				if (action === 'update') {
					finergy.model.docinfo[doctype][docname][key].splice(docindex, 1, doc);
				}
				if (action === 'delete') {
					finergy.model.docinfo[doctype][docname][key].splice(docindex, 1);
				}
			}
			// no need to update timeline of owner of comment
			// gets handled via comment submit code
			if (!(['add', 'update'].includes(action) && doc.doctype === 'Comment' && doc.owner === finergy.session.user)) {
	...
	set_fields_as_options(fieldname, reference_doctype, filter_function, default_options=[], table_fieldname) {
		...
		return new Promise(resolve => {
			finergy.model.with_doctype(reference_doctype, () => {
				finergy.get_meta(reference_doctype).fields.map(df => {
...
finergy.validated = 0;
// Proxy for finergy.validated
Object.defineProperty(window, 'validated', {
	get: function() {
		console.warn('Please use `finergy.validated` instead of `validated`. It will be deprecated soon.'); // eslint-disable-line
		return finergy.validated;
	},
	set: function(value) {
		console.warn('Please use `finergy.validated` instead of `validated`. It will be deprecated soon.'); // eslint-disable-line
		finergy.validated = value;
		return finergy.validated;
	}
});
