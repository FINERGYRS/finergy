...
finergy.ui.form.get_open_grid_form = function () {
	return $(".grid-row-open").data("grid_row");
}

finergy.ui.form.close_grid_form = function () {
	var open_form = finergy.ui.form.get_open_grid_form();
	open_form && open_form.hide_form();

	// hide editable row too
	if (finergy.ui.form.editable_row) {
		finergy.ui.form.editable_row.toggle_editable_row(false);
	}
}
...
export default class Grid {
	constructor(opts) {
		...
		if (this.doctype) {
			this.meta = finergy.get_meta(this.doctype);
		}
		this.fields_map = {};
		this.template = null;
		...
		this.debounced_refresh = finergy.utils.debounce(this.debounced_refresh, 100);
	}

	allow_on_grid_editing() {
		if (finergy.utils.is_xs()) {
			return false;
	...
	make() {
			...
			<div class="form-grid">
				...
				<div class="grid-body">
					...
					<div class="grid-empty text-center">
						<img
							src="/assets/finergyrs/images/ui-states/grid-empty-state.svg"
		...
		finergy.utils.bind_actions_with_object(this.wrapper, this);
	...
	delete_rows() {
		...
		selected_children.forEach(doc => {
			...
			tasks.push(() => finergy.timeout(0.1));
		});
	...
	delete_all_rows() {
		finergy.confirm(__("Are you sure you want to delete all rows?"), () => {
	...
	scroll_to_top() {
		finergy.utils.scroll_to(this.wrapper);
	}
	...
	refresh(force) {
		...
		if (this.frm) {
			this.display_status = finergy.perm.get_field_display_status(this.df, this.frm.doc,
				this.perm);
	...
	setup_fields() {
		...
		if (this.frm && this.frm.docname) {
			...
			this.df = finergy.meta.get_docfield(this.frm.doctype, this.df.fieldname,
				this.frm.docname);
		} else {
			// use non-doc specific docfield
			if (this.df.options) {
				this.df = finergy.meta.get_docfield(this.df.options, this.df.fieldname) || this.df || null;
			}
		}

		if (this.doctype && this.frm) {
			this.docfields = finergy.meta.get_docfields(this.doctype, this.frm.docname);
		} else {
				...
				row.columns_list.forEach(column => {
					...
					if (column.df.fieldname == fieldname) {
						if (show) {
							...
							if (row != finergy.ui.form.editable_row) {
	...
	get_docfield(fieldname) {
		return finergy.meta.get_docfield(this.doctype, fieldname, this.frm ? this.frm.docname : null);
	}
	...
	add_new_row(idx, callback, show, copy_doc, go_to_last_page = false, go_to_first_page = false) {
		if (this.is_editable()) {
			...
			if (this.frm) {
				var d = finergy.model.add_child(this.frm.doc, this.df.options, this.df.fieldname, idx);
				if (copy_doc) {
	...
	setup_visible_columns() {
		...
		for (var ci in fields) {
			...
			if (df && !df.hidden
				...
				&& !in_list(finergy.model.layout_fields, df.fieldtype)) {
				...
				if (df.fieldtype == 'Link' && !df.formatter && df.parent && finergy.meta.docfield_map[df.parent]) {
					const docfield = finergy.meta.docfield_map[df.parent][df.fieldname];
		...
		while (total_colsize < 11 && passes < 12) {
			for (var i in this.visible_columns) {
				...
				if (colsize > 1 && colsize < 11
					&& !in_list(finergy.model.std_fields_list, df.fieldname)) {
	...
	setup_user_defined_columns() {
		if (this.frm) {
			let user_settings = finergy.get_user_settings(this.frm.doctype, 'GridView');
			if (user_settings && user_settings[this.doctype] && user_settings[this.doctype].length) {
				this.user_defined_columns = user_settings[this.doctype].map(row => {
					let column = finergy.meta.get_docfield(this.doctype, row.fieldname);
					if (column) {
	...
	set_multiple_add(link, qty) {
		...
		var link_field = finergy.meta.get_docfield(this.df.options, link);
	...
	setup_allow_bulk_edit() {
		...
		if (this.frm && this.frm.get_docfield(this.df.fieldname).allow_bulk_edit) {
			...
			const value_formatter_map = {
				"Date": val => val ? finergy.datetime.user_to_str(val) : val,
				"Int": val => cint(val),
				"Check": val => cint(val),
				"Float": val => flt(val),
			};
			...
			// upload
			finergy.flags.no_socketio = true;
			$(this.wrapper).find(".grid-upload").removeClass('hidden').on("click", () => {
				new finergy.ui.FileUploader({
					as_dataurl: true,
					allow_multiple: false,
					on_success(file) {
						var data = finergy.utils.csv_to_array(finergy.utils.get_decoded_string(file.dataurl));
							...
							if (i > 6) {
								...
								if (!blank_row) {
									...
									$.each(row, (ci, value) => {
										...
										var df = finergy.meta.get_docfield(me.df.options, fieldname);
										if (df) {
						...
						finergy.msgprint({ message: __('Table updated'), title: __('Success'), indicator: 'green' });
					}
				});
				return false;
			});
	...
	setup_download() {
		let title = this.df.label || finergy.model.unscrub(this.df.fieldname);
		$(this.wrapper).find(".grid-download").removeClass('hidden').on("click", () => {
			...
			data.push(["------"]);
			$.each(finergy.get_meta(this.df.options).fields, (i, df) => {
				// don't include the read-only field in the template
				if (finergy.model.is_value_type(df.fieldtype)) {
					data[1].push(df.label);
					data[2].push(df.fieldname);
					let description = (df.description || "") + ' ';
					if (df.fieldtype === "Date") {
						description += finergy.boot.sysdefaults.date_format;
					}
			...
			finergy.tools.downloadify(data, null, title);
			return false;
		});
	}
