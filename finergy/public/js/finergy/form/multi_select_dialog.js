finergy.ui.form.MultiSelectDialog = class MultiSelectDialog {
	constructor(opts) {
		...
		if (!this.for_select) {
			finergy.model.with_doctype(this.doctype, () => this.init());
		} else {
			this.init();
		}
	}
	...
	make_new_document(e) {
		...
		if (e) {
			...
			finergy.new_doc(this.doctype, true);
		}
	}
	...
	set_route_options() {
		...
		if (Array.isArray(this.setters)) {
			for (let df of this.setters) {
				finergy.route_options[df.fieldname] = this.dialog.fields_dict[df.fieldname].get_value() || undefined;
			}
		} else {
			Object.keys(this.setters).forEach(setter => {
				finergy.route_options[setter] = this.dialog.fields_dict[setter].get_value() || undefined;
			});
		}
	}
	...
	get_child_datatable_columns() {
		...
		return [parent, ...this.child_columns].map(d => ({ name: finergy.unscrub(d), editable: false }));
	}
	...
	setup_child_datatable() {
		...
		this.child_datatable = new finergy.DataTable(this.$child_wrapper.get(0), {
	...
	get_primary_filters() {
		...
		if ($.isArray(this.setters)) {
			...
			Object.keys(this.setters).forEach((setter, index) => {
				let df_prop = finergy.meta.docfield_map[this.doctype][setter];
		...
		if (this.allow_child_item_selection) {
			this.child_doctype = finergy.meta.get_docfield(this.doctype, this.child_fieldname).options;
			columns[0].push({
				fieldtype: "Check",
	...
	bind_events() {
		...
		this.$parent.find('.input-with-feedback').on('change', () => {
			finergy.flags.auto_scroll = false;
			this.get_results();
		});
	...
	make_list_row(result = {}) {
		...
		columns.forEach(function (column) {
			contents += `<div class="list-item__content ellipsis">
				${
	head ? `<span class="ellipsis text-muted" title="${__(finergy.model.unscrub(column))}">${__(finergy.model.unscrub(column))}</span>`
		: (column !== "name" ? `<span class="ellipsis result-row" title="${__(result[column] || '')}">${__(result[column] || '')}</span>`
			: `<a href="${"/app/" + finergy.router.slug(me.doctype) + "/" + result[column] || ''}" class="list-id ellipsis" title="${__(result[column] || '')}">
							${__(result[column] || '')}</a>`)}
			</div>`;
		});
	...
	render_result_list(results, more = 0, empty = true) {
		...
		if (!finergy.flags.auto_scroll && empty) {
			this.empty_list();
		}
		more_btn.hide();
	...
	async perform_search(args) {
		const res = await finergy.call({
			type: "GET",
			method: 'finergy.desk.search.search_widget',
			no_spinner: true,
			args: args,
		});
