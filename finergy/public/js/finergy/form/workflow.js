...
finergy.ui.form.States = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.state_fieldname = finergy.workflow.get_state_fieldname(this.frm.doctype);

		// no workflow?
		if(!this.state_fieldname)
			return;

		this.update_fields = finergy.workflow.get_update_fields(this.frm.doctype);
	...
	setup_help: function() {
		...
		this.frm.page.add_action_item(__("Help"), function() {
			finergy.workflow.setup(me.frm.doctype);
			var state = me.get_state();
			var d = new finergy.ui.Dialog({
				title: "Workflow: "
					+ finergy.workflow.workflows[me.frm.doctype].name
			});

			finergy.workflow.get_transitions(me.frm.doc).then((transitions) => {
				const next_actions = $.map(transitions, d => `${d.action.bold()} ${__("by Role")} ${d.allowed}`)
					.join(", ") || __("None: End of Workflow").bold();

				const document_editable_by = finergy.workflow.get_document_state(me.frm.doctype, state).allow_edit.bold();
	...
	show_actions: function() {
		...
		finergy.workflow.get_transitions(this.frm.doc).then(transitions => {
			this.frm.page.clear_actions_menu();
			transitions.forEach(d => {
				if (finergy.user_roles.includes(d.allowed) && has_approval_access(d)) {
					added = true;
					me.frm.page.add_action_item(__(d.action), function() {
						// set the workflow_action for use in form scripts
						me.frm.selected_workflow_action = d.action;
						me.frm.script_manager.trigger('before_workflow_action').then(() => {
							finergy.xcall('finergy.model.workflow.apply_workflow',
	...
	set_default_state: function() {
		var default_state = finergy.workflow.get_default_state(this.frm.doctype, this.frm.doc.docstatus);
		if(default_state) {
