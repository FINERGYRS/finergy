...
finergy.ui.form.recent_link_validations = {};

finergy.ui.form.ControlLink = finergy.ui.form.ControlData.extend({
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-open no-decoration" title="${__("Open Link")}">
					${finergy.utils.icon('arrow-right', 'xs')}
				</a>
			</span>
		...
		this.$input.on("focus", function() {
			setTimeout(function() {
				if (me.$input.val() && me.get_options()) {
					...
					me.$link.toggle(true);
					me.$link_open.attr('href', finergy.utils.get_form_link(doctype, name));
				}
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return finergy.get_route && finergy.get_route()[0] === 'List' ? finergy.get_route()[1] : null;
		}
	},
	...
	is_translatable() {
		return in_list(finergy.boot.translated_doctypes || [], this.get_options());
	},
	...
	new_doc: function() {
		...
		if (df && df.get_route_options_for_new_doc) {
			finergy.route_options = df.get_route_options_for_new_doc(this);
		} else {
			finergy.route_options = {};
		}

		// partially entered name field
		finergy.route_options.name_field = this.get_label_value();

		// reference to calling link
		finergy._from_link = this;
		finergy._from_link_scrollY = $(document).scrollTop();

		finergy.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete: function() {
		...
		this.$input.on("input", finergy.utils.debounce(function(e) {
		...
		this.$input.on("awesomplete-select", function(e) {
			...
			if (me.df.remember_last_selected_value) {
				finergy.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	toggle_href(doctype) {
		if (finergy.model.can_select(doctype) && !finergy.model.can_read(doctype)) {
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = finergy.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : finergy.model.unscrub(fieldname);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = finergy.meta.get_docfield(this.frm.doctype, key);
	...
	validate_link_and_fetch(df, options, docname, value) {
		...
		if (value) {
			return finergy.xcall("finergy.client.validate_link", {
				doctype: options,
				docname: value,
