finergy.ui.form.ControlMultiSelectList = finergy.ui.form.ControlData.extend({
	...
	make_input() {
		...
		this.$list_wrapper.on('input', 'input', finergy.utils.debounce((e) => {
			this.set_options()
				.then(() => {
	...
	set_selectable_items(options) {
		let html = options.map(option => {
			...
			return `<li class="selectable-item ${selected}" data-value="${encoded_value}">
				...
				<div class="multiselect-check">${finergy.utils.icon('tick', 'xs')}</div>
			</li>`;
		}).join('');
		if (!html) {
