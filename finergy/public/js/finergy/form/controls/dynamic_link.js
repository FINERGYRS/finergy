finergy.ui.form.ControlDynamicLink = finergy.ui.form.ControlLink.extend({
	get_options: function() {
		...
		if (this.df.get_options) {
			...
			options = finergy.model.get_value(this.df.parent, this.docname, this.df.options);
		}

		if (finergy.model.is_single(options)) {
			finergy.throw(__("{0} is not a valid DocType for Dynamic Link", [options.bold()]));
		}
