finergy.ui.form.ControlInput = finergy.ui.form.Control.extend({
	horizontal: true,
	make: function() {
		// parent element
		this._super();
	...
	make_wrapper: function() {
		if(this.only_input) {
			this.$wrapper = $('<div class="form-group finergy-control">').appendTo(this.parent);
		} else {
			this.$wrapper = $('<div class="finergy-control">\
	...
	refresh_input: function() {
		...
		if (me.disp_status != "None") {
			// refresh value
			if (me.frm) {
				me.value = finergy.model.get_value(me.doctype, me.docname, me.df.fieldname);
			} else if (me.doc) {
	...
	set_disp_area: function(value) {
		...
		if (this.df.fieldtype === 'Data') {
			value = finergy.utils.escape_html(value);
		}
		let doc = this.doc || (this.frm && this.frm.doc);
		let display_value = finergy.format(value, this.df, { no_icon: true, inline: true }, doc);
