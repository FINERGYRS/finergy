...
finergy.ui.form.ControlTable = finergy.ui.form.Control.extend({
	make: function() {
		this._super();
		...
		this.$wrapper.on('paste', ':text', e => {
			...
			let data = finergy.utils.csv_to_array(pasted_data, '\t');
					...
					if (!blank_row) {
						...
						row.forEach((value, data_index) => {
							if (fieldnames[data_index]) {
								finergy.model.set_value(doctype, row_name, fieldnames[data_index], value);
							}
						});
						row_idx++;
						if (data_length >= 10) {
							let progress = i + 1;
							finergy.show_progress(__('Processing'), progress, data_length, null, true);
						}
					}
				}, 0);
			});
	...
	get_field(field_name) {
		...
		this.grid.meta.fields.some(field => {
			if (finergy.model.no_value_type.includes(field.fieldtype)) {
				return false;
			}
