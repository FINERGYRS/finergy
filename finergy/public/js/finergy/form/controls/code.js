finergy.ui.form.ControlCode = finergy.ui.form.ControlText.extend({
	make_input() {
	...
	make_ace_editor() {
		...
		// events
		this.editor.session.on('change', finergy.utils.debounce(() => {
	...
	load_lib() {
		...
		if (finergy.boot.developer_mode) {
			this.root_lib_path = '/assets/finergyrs/node_modules/ace-builds/src-noconflict/';
		} else {
			this.root_lib_path = '/assets/finergyrs/node_modules/ace-builds/src-min-noconflict/';
		}

		this.library_loaded = new Promise(resolve => {
			finergy.require(this.root_lib_path + 'ace.js', () => {
