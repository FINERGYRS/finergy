...
finergy.ui.form.ControlGeolocation = finergy.ui.form.ControlData.extend({
	horizontal: false,
	...
	make_wrapper() {
		...
		if (this.frm) {
			...
			$(document).on('finergy.ui.Dialog:shown', () => {
				this.make_map();
			});
		}
	},
	...
	format_for_input(value) {
		...
		if(value) {
			...
			this.map.setView(finergy.utils.map_defaults.center, finergy.utils.map_defaults.zoom);
		}
	...
	bind_leaflet_draw_control() {
		...
		var options = {
			...
			draw: {
				polyline: {
					shapeOptions: {
						color: finergy.ui.color.get('blue'),
						weight: 10
					}
				},
				polygon: {
					...
					drawError: {
						color: finergy.ui.color.get('orange'), // Color the shape will turn when intersects
