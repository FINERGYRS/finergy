finergy.ui.form.Control = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.make();

		// if developer_mode=1, show fieldname as tooltip
		if(finergy.boot.user && finergy.boot.developer_mode===1 && this.$wrapper) {
	...
	make_wrapper: function() {
		this.$wrapper = $("<div class='finergy-control'></div>").appendTo(this.parent);
	...
	get_status: function(explain) {
		...
		var status = finergy.perm.get_field_display_status(this.df,
			finergy.model.get_doc(this.doctype, this.docname), this.perm || (this.frm && this.frm.perm), explain);
		...
		if (this.doctype && status==="Read" && !this.only_input
			&& is_null(finergy.model.get_value(this.doctype, this.docname, this.df.fieldname))
	...
	show_translatable_button(value) {
		...
		if (!finergy.model
			...
			|| !finergy.model.can_write('Translation')
			|| !value) return;

		// Disable translation in website
		if (!finergy.views || !finergy.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on('click', () => {
				if (!this.doc.__islocal) {
					new finergy.views.TranslationManager({
						'df': this.df,
	...
	set_model_value: function(value) {
		if (this.frm) {
			...
			return finergy.model.set_value(this.doctype, this.docname, this.df.fieldname,
