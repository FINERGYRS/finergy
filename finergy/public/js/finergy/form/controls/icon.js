...
finergy.ui.form.ControlIcon = class ControlIcon extends finergy.ui.form.ControlData {
	make_input() {
	...
	get_all_icons() {
		finergy.symbols = [];
		$("#finergy-symbols > symbol[id]").each(function() {
			finergy.symbols.push(this.id.replace('icon-', ''));
		});
	}
	...
	make_icon_input() {
		...
		if (!this.selected_icon) {
			this.selected_icon = $(`<div class="selected-icon">${finergy.utils.icon("folder-normal", "md")}</div>`);
