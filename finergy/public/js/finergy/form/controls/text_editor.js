...
finergy.ui.form.ControlTextEditor = finergy.ui.form.ControlCode.extend({
	make_wrapper() {
		this._super();
	},
	...
	bind_events() {
		this.quill.on('text-change', finergy.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on('keydown', (e) => {
			const key = finergy.ui.keys && finergy.ui.keys.get_key(e);
	...
	parse(value) {
		...
		return finergy.dom.remove_script_and_style(value);
	},
