finergy.ui.form.ControlDate = finergy.ui.form.ControlData.extend({
	...
	set_formatted_input: function(value) {
		...
		if(should_refresh) {
			this.datepicker.selectDate(finergy.datetime.str_to_obj(value));
		}
	},
	set_date_options: function() {
		// webformTODO:
		let sysdefaults = finergy.boot.sysdefaults;

		let lang = 'en';
		finergy.boot.user && (lang = finergy.boot.user.language);
		...
		this.date_format = finergy.defaultDateFormat;
		this.datepicker_options = {
			...
			startDate: now_date,
			keyboardNav: false,
			firstDay: finergy.datetime.get_first_day_of_the_week_index(),
			onSelect: () => {
	...
	get_now_date: function() {
		return finergy.datetime.now_date(true);
	},
		...
		this.$input.on("keydown", function(e) {
			if(e.which===84) { // 84 === t
				if(me.df.fieldtype=='Date') {
					me.set_value(finergy.datetime.nowdate());
				} if(me.df.fieldtype=='Datetime') {
					me.set_value(finergy.datetime.now_datetime());
				} if(me.df.fieldtype=='Time') {
					me.set_value(finergy.datetime.now_time());
				}
				return false;
			}
		});
	...
	parse: function(value) {
		if(value) {
			return finergy.datetime.user_to_str(value);
		}
	},
	format_for_input: function(value) {
		if(value) {
			return finergy.datetime.str_to_user(value);
		}
		return "";
	},
	validate: function(value) {
		if(value && !finergy.datetime.validate(value)) {
			let sysdefaults = finergy.sys_defaults;
			let date_format = sysdefaults && sysdefaults.date_format
				? sysdefaults.date_format : 'yyyy-mm-dd';
			finergy.msgprint(__("Date {0} must be in format: {1}", [value, date_format]));
			return '';
		}
		return value;
	}
