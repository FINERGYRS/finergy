finergy.ui.form.ControlTableMultiSelect = finergy.ui.form.ControlLink.extend({
	make_input() {
		this._super();
		...
		this.$input_area.on('click', '.btn-link-to-form', (e) => {
			...
			const value = decodeURIComponent($value.data().value);
			const link_field = this.get_link_field();
			finergy.set_route('Form', link_field.options, value);
		});
		this.$input.on('keydown', e => {
			// if backspace key pressed on empty input, delete last value
			if (e.keyCode == finergy.ui.keyCode.BACKSPACE && e.target.value === '') {
	...
	parse(value) {
		...
		if (value) {
			if (this.frm) {
				const new_row = finergy.model.add_child(this.frm.doc, this.df.options, this.df.fieldname);
	...
	get_pill_html(value) {
		...
		return `
			<button class="data-pill btn tb-selected-value" data-value="${encoded_value}">
				...
				<span class="btn-remove">${finergy.utils.icon('close')}</span>
			</button>
		`;
	},
	get_options() {
	...
	get_link_field() {
		if (!this._link_field) {
			const meta = finergy.get_meta(this.df.options);
