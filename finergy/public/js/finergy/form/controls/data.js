finergy.provide('finergy.phone_call');

finergy.ui.form.ControlData = finergy.ui.form.ControlInput.extend({
	html_element: "input",
	input_type: "text",
	...
	make_input: function() {
		...
		this.$input.on('paste', (e) => {
			...
			if (maxlength && pasted_data.length > maxlength) {
				...
				if (this.frm && finergy.model.can_write(this.frm.doctype)) {
					let doctype_edit_link = null;
					if (this.frm.meta.custom) {
						doctype_edit_link = finergy.utils.get_form_link(
							'DocType',
							this.frm.doctype, true,
							__('this form')
						);
					} else {
						doctype_edit_link = finergy.utils.get_form_link('Customize Form', 'Customize Form', true, null, {
		...
		this.$wrapper.find('.control-input').append(
			`<span class="link-btn">
				<a class="btn-open no-decoration" title="${__("Open Link")}" target="_blank">
					${finergy.utils.icon('link-url', 'sm')}
				</a>
			</span>`
		);
	...
	bind_change_event: function() {
		...
		if (this.trigger_change_on_input_event && !this.in_grid()) {
			...
			this.$input.on("input", finergy.utils.debounce(change_handler, 500));
		}
	},
	setup_autoname_check: function() {
		if (!this.df.parent) return;
		this.meta = finergy.get_meta(this.df.parent);
			...
			this.$input.on('keyup', () => {
				...
				if (this.doc && this.doc.__islocal) {
					...
					let timeout = setTimeout(() => {
						...
						finergy.db.get_value(this.doctype, this.$input.val(),
							'name', (val) => {
								if (val && val.name) {
	...
	validate: function(v) {
		...
		if(this.df.options == 'Phone') {
			...
			var email_list = finergy.utils.split_emails(v);
			if (!email_list) {
				return '';
			} else {
