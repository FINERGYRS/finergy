finergy.ui.form.ControlSelect = finergy.ui.form.ControlData.extend({
	html_element: 'select',
	make_input: function() {
		this._super();
	...
	set_icon: function(is_xs_input) {
		const select_icon_html =
			`<div class="select-icon ${is_xs_input ? 'xs' : ''}">
				${finergy.utils.icon('select', is_xs_input ? 'xs' : 'sm')}
			</div>`;
		if (this.only_input) {
	...
	get_file_attachment_list: function() {
		if(!this.frm) return;
		var fl = finergy.model.docinfo[this.frm.doctype][this.frm.docname];
