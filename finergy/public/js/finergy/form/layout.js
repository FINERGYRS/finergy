...
finergy.ui.form.Layout = Class.extend({
	init: function (opts) {
		this.views = {};
		this.pages = [];
		this.sections = [];
	...
	show_empty_form_message: function() {
		if (!(this.wrapper.find(".finergy-control:visible").length || this.wrapper.find(".section-head.collapsed").length)) {
	...
	get_doctype_fields: function() {
		...
		if (this.doctype_layout) {
			...
			fields = fields.concat(finergy.meta.sort_docfields(finergy.meta.docfield_map[this.doctype]));
		}
	...
	get_fields_from_layout() {
		...
		for (let f of this.doctype_layout.fields) {
			const docfield = copy_dict(finergy.meta.docfield_map[this.doctype][f.fieldname]);
	...
	init_field: function (df, render = false) {
		const fieldobj = finergy.ui.form.make_control({
			df: df,
	...
	make_page: function (df) { // eslint-disable-line no-unused-vars
		...
		this.fold_btn = head.find(".btn-fold").on("click", function () {
			...
			if (page.hasClass("hide")) {
				...
				finergy.utils.scroll_to($(this), true, 30);
				me.folded = false;
			} else {
	...
	make_section: function (df) {
		this.section = new finergy.ui.form.Section(this, df);
	...
	make_column: function (df) {
		this.column = new finergy.ui.form.Column(this.section, df);
		if (df && df.fieldname) {
	...
	is_numeric_field_active() {
		const control = $(document.activeElement).closest(".finergy-control");
	...
	refresh_sections: function() {
		...
		this.wrapper.find(".form-section:not(.hide-control)").each(function() {
			...
			if (section.find(".finergy-control:not(.hide-control)").length) {
	...
	attach_doc_and_docfields: function (refresh) {
		...
		for (var i = 0, l = this.fields_list.length; i < l; i++) {
			...
			if (me.doc) {
				...
				fieldobj.df = finergy.meta.get_docfield(me.doc.doctype,
	...
	focus_on_next_field: function (start_idx, fields) {
		...
		for (var i = start_idx + 1, len = fields.length; i < len; i++) {
			...
			if (this.is_visible(field)) {
				...
				} else if (!in_list(finergy.model.no_value_type, field.df.fieldtype)) {
...
finergy.ui.form.Section = Class.extend({
	...
	collapse: function (hide) {
		...
		this.indicator & this.indicator.html(finergy.utils.icon(indicator_icon, 'sm', 'mb-1'));
...
finergy.ui.form.Column = Class.extend({
