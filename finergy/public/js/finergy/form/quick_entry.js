...
finergy.quick_edit = function(doctype, name) {
	finergy.db.get_doc(doctype, name).then(doc => {
		finergy.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

finergy.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, '');
	var controller_name = "QuickEntryForm";

	if(finergy.ui.form[trimmed_doctype + "QuickEntryForm"]){
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	finergy.quick_entry = new finergy.ui.form[controller_name](doctype, after_insert, init_callback, doc, force);
	return finergy.quick_entry.setup();
};

finergy.ui.form.QuickEntryForm = Class.extend({
		...
		return new Promise(resolve => {
			finergy.model.with_doctype(this.doctype, () => {
				...
				if (this.is_quick_entry() || this.force) {
					...
					finergy.quick_entry = null;
					finergy.set_route('Form', this.doctype, this.doc.name)
						.then(() => resolve(this));
	...
	check_quick_entry_doc: function() {
		if (!this.doc) {
			this.doc = finergy.model.get_new_doc(this.doctype, null, null, true);
		}
	},
	...
	render_dialog: function() {
		...
		this.dialog.onhide = () => finergy.quick_entry = null;
		this.dialog.show();
	...
	insert: function() {
		...
		return new Promise(resolve => {
			me.update_doc();
			finergy.call({
				method: "finergy.client.save",
				args: {
					doc: me.dialog.doc
				},
				callback: function(r) {

					if (finergy.model.is_submittable(me.doctype)) {
						...
						me.dialog.hide();
						// delete the old doc
						finergy.model.clear_doc(me.dialog.doc.doctype, me.dialog.doc.name);
						me.dialog.doc = r.message;
						if(finergy._from_link) {
							finergy.ui.form.update_calling_link(me.dialog.doc);
						} else {
							if(me.after_insert) {
	...
	open_doc: function(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			finergy.route_options = finergy.route_options || {};
			finergy.route_options.after_save = (frm) => {
