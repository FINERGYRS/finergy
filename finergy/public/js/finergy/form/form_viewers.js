finergy.ui.form.FormViewers = class FormViewers {
	...
	refresh() {
		...
		let currently_viewing = users.current.filter(user => user != finergy.session.user);
		let avatar_group = finergy.avatar_group(currently_viewing, 5, {'align': 'left', 'overlap': true});
		this.parent.empty().append(avatar_group);
	}
};

finergy.ui.form.FormViewers.set_users = function(data, type) {
	const doctype = data.doctype;
	const docname = data.docname;
	const docinfo = finergy.model.get_docinfo(doctype, docname);

	const past_users = ((docinfo && docinfo[type]) || {}).past || [];
	const users = data.users || [];
	const new_users = users.filter(user => !past_users.includes(user));

	finergy.model.set_docinfo(doctype, docname, type, {
