...
finergy.provide("finergy.ui.form.handlers");

finergy.ui.form.get_event_handler_list = function(doctype, fieldname) {
	if(!finergy.ui.form.handlers[doctype]) {
		finergy.ui.form.handlers[doctype] = {};
	}
	if(!finergy.ui.form.handlers[doctype][fieldname]) {
		finergy.ui.form.handlers[doctype][fieldname] = [];
	}
	return finergy.ui.form.handlers[doctype][fieldname];
}

finergy.ui.form.on = finergy.ui.form.on_change = function(doctype, fieldname, handler) {
	var add_handler = function(fieldname, handler) {
		var handler_list = finergy.ui.form.get_event_handler_list(doctype, fieldname);
...
finergy.ui.form.off = function(doctype, fieldname, handler) {
	var handler_list = finergy.ui.form.get_event_handler_list(doctype, fieldname);
...
finergy.ui.form.trigger = function(doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
}

finergy.ui.form.ScriptManager = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	},
	...
	trigger: function(event_name, doctype, name) {
		...
		this.frm.selected_doc = finergy.get_doc(doctype, name);
	...
	get_handlers: function(event_name, doctype) {
		...
		if(finergy.ui.form.handlers[doctype] && finergy.ui.form.handlers[doctype][event_name]) {
			$.each(finergy.ui.form.handlers[doctype][event_name], function(i, fn) {
	...
	setup: function() {
		...
		$.each(this.frm.fields, function(i, field) {
			...
			if(finergy.model.table_fields.includes(field.df.fieldtype)) {
				$.each(finergy.meta.get_docfields(field.df.options, me.frm.docname), function(i, df) {
					setup_add_fetch(df);
				});
			}
		});
		...
		// css
		doctype.__css && finergy.dom.set_style(doctype.__css);

		this.trigger('setup');
	},
	log_error: function(caller, e) {
		finergy.show_alert({message: __("Error in Client Script."), indicator: "error"});
	...
	copy_from_first_row: function(parentfield, current_row, fieldnames) {
		...
		$.each(fieldnames, function(i, fieldname) {
			finergy.model.set_value(current_row.doctype, current_row.name, fieldname,
				data[0][fieldname]);
		});
	}
});
