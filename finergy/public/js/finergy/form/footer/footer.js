...
finergy.ui.form.Footer = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.make();
	...
	make: function() {
		this.wrapper = $(finergy.render_template("form_footer", {}))
	...
	make_comment_box: function() {
		this.frm.comment_box = finergy.ui.form.make_control({
			...
			on_submit: (comment) => {
				if (strip_html(comment).trim() != "" || comment.includes('img')) {
					...
					finergy.xcall("finergy.desk.form.utils.add_comment", {
						reference_doctype: this.frm.doctype,
						reference_name: this.frm.docname,
						content: comment,
						comment_email: finergy.session.user,
						comment_by: finergy.session.user_fullname
					}).then((comment) => {
