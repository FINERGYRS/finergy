...
class FormTimeline extends BaseTimeline {
		...
		if (this.frm.meta.allow_events_in_timeline) {
			let create_event = () => {
				const args = {
					...
					txt: finergy.markdown(this.frm.comment_box.get_value())
				};
				return new finergy.views.InteractionComposer(args);
			};
		...
		if (doc_info.document_email) {
			...
			this.document_email_link_wrapper
				...
				.on("click", e => {
					...
					finergy.utils.copy_to_clipboard(text);
				});
		}
	}
	...
	set_document_info() {
		...
		let creation_message =
			finergy.utils.is_current_user(this.frm.doc.owner)
		...
		let modified_message =
			finergy.utils.is_current_user(this.frm.doc.modified_by)
		...
		if (this.frm.doc.route && cint(finergy.boot.website_tracking_enabled)) {
			let route = this.frm.doc.route;
			finergy.utils.get_page_view_count(route).then((res) => {
	...
	get_user_link(user) {
		const user_display_text = (finergy.user_info(user).fullname || '').bold();
		return finergy.utils.get_form_link('User', user, true, user_display_text);
	}
	...
	get_view_timeline_contents() {
		...
		(this.doc_info.views || []).forEach(view => {
			...
			let view_message = finergy.utils.is_current_user(view.owner)
	...
	get_communication_timeline_content(doc, allow_reply=true) {
		doc._url = finergy.utils.get_form_link("Communication", doc.name);
		...
		doc.content = finergy.dom.remove_script_and_style(doc.content);
		let communication_content = $(finergy.render_template('timeline_message_box', { doc }));
		if (allow_reply) {
	...
	get_comment_timeline_content(doc) {
		doc.content = finergy.dom.remove_script_and_style(doc.content);
		const comment_content = $(finergy.render_template('timeline_message_box', { doc }));
		...
		(this.doc_info.milestones || []).forEach(milestone_log => {
			milestone_timeline_contents.push({
				...
				content: __('{0} changed {1} to {2}', [
					...
					finergy.meta.get_label(this.frm.doctype, milestone_log.track_field),
	...
	get_custom_timeline_contents() {
		...
		(this.doc_info.additional_timeline_content || []).forEach(custom_item => {
			custom_timeline_contents.push({
				...
				content: custom_item.content || finergy.render_template(custom_item.template, custom_item.template_data),
			});
		});
	...
	get_energy_point_timeline_contents() {
		...
		(this.doc_info.energy_point_logs || []).forEach(log => {
			...
			energy_point_timeline_contents.push({
				...
				content: finergy.energy_points.format_form_log(log)
			});
		});
	...
	setup_reply(communication_box, communication_doc) {
		...
		let reply = $(`<a class="action-btn reply">${finergy.utils.icon('reply', 'md')}</a>`).click(() => {
			this.compose_mail(communication_doc);
		});
		let reply_all = $(`<a class="action-btn reply-all">${finergy.utils.icon('reply-all', 'md')}</a>`).click(() => {
	...
	compose_mail(communication_doc=null, reply_all=false) {
		const args = {
			doc: this.frm.doc,
			frm: this.frm,
			recipients: communication_doc && communication_doc.sender != finergy.session.user_email ? communication_doc.sender : this.get_recipient(),
		...
		if (this.frm.doctype === "Communication") {
			...
			const comment_value = finergy.markdown(this.frm.comment_box.get_value());
			args.message = strip_html(comment_value) ? comment_value : '';
		}

		new finergy.views.CommunicationComposer(args);
	}
	...
	setup_comment_actions(comment_wrapper, doc) {
		...
		let delete_button = $();
		if (finergy.model.can_delete("Comment") && (finergy.session.user == doc.owner || finergy.user.has_role("System Manager"))) {
			delete_button = $(`
				<button class="btn btn-link action-btn">
					${finergy.utils.icon('close', 'sm')}
				</button>
		...
		let edit_button = $();
		let current_user = finergy.session.user;
	...
	make_editable(container) {
		return finergy.ui.form.make_control({
			parent: container,
			df: {
				fieldtype: 'Comment',
				fieldname: 'comment',
	...
	update_comment(name, content) {
		return finergy.xcall('finergy.desk.form.utils.update_comment', { name, content })
			.then(() => {
	...
	delete_comment(comment_name) {
		finergy.confirm(__('Delete comment?'), () => {
			return finergy.xcall("finergy.client.delete", {
				doctype: "Comment",
				name: comment_name
			}).then(() => {
