...
function get_version_timeline_content(version_doc, frm) {
	...
	if (!$.isEmptyObject(updater_reference)) {
		...
		if (doctype && docname) {
			updater_reference_link = finergy.utils.get_form_link(
	...
	if (data.changed && data.changed.length) {
		...
		data.changed.every(function(p) {
			if (p[0] === 'docstatus') {
				...
				const df = finergy.meta.get_docfield(frm.doctype, p[0], frm.docname);
				if (df && !df.hidden) {
					const field_display_status = finergy.perm.get_field_display_status(df, null,
						frm.perm);
		...
		data.row_changed.every(function(row) {
			row[3].every(function(p) {
				var df = frm.fields_dict[row[0]] &&
					finergy.meta.get_docfield(frm.fields_dict[row[0]].grid.doctype,
						p[0], frm.docname);

				if (df && !df.hidden) {
					var field_display_status = finergy.perm.get_field_display_status(df,
						null, frm.perm);

					if (field_display_status === 'Read' || field_display_status === 'Write') {
						parts.push(__('{0} from {1} to {2} in row #{3}', [
							finergy.meta.get_label(frm.fields_dict[row[0]].grid.doctype,
								p[0]),
							format_content_for_timeline(p[1]),
							format_content_for_timeline(p[2]),
							row[1]
	...
	['added', 'removed'].forEach(function(key) {
		if (data[key] && data[key].length) {
			let parts = (data[key] || []).map(function(p) {
				var df = finergy.meta.get_docfield(frm.doctype, p[0], frm.docname);
				if (df && !df.hidden) {
					var field_display_status = finergy.perm.get_field_display_status(df, null,
						frm.perm);

					if (field_display_status === 'Read' || field_display_status === 'Write') {
						return __(finergy.meta.get_label(frm.doctype, p[0]));
					}
				}
			});
...
function get_version_comment(version_doc, text) {
	...
	if (text.includes("<a")) {
		...
		try {
			...
			Array.from($(text)).forEach(element => {
				if ($(element).is('a')) {
					version_comment += unlinked_content ? finergy.utils.get_form_link('Version', version_doc.name, true, unlinked_content) : "";
					unlinked_content = "";
			...
			if (unlinked_content) {
				version_comment += finergy.utils.get_form_link('Version', version_doc.name, true, unlinked_content);
			}
	...
	return finergy.utils.get_form_link('Version', version_doc.name, true, text);
}
...
function format_content_for_timeline(content) {
	...
	// escapes HTML
	// and makes it bold
	content = finergy.utils.html2text(content);
	content = finergy.ellipsis(content, 40) || '""';
	content = finergy.utils.escape_html(content);
	return content.bold();
}
