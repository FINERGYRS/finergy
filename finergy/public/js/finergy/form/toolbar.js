...
finergy.ui.form.Toolbar = class Toolbar {
	constructor(opts) {
		$.extend(this, opts);
		this.refresh();
	...
	set_title() {
		if (this.frm.is_new()) {
			...
			if(this.frm.doc.__islocal || title === this.frm.docname || this.frm.meta.autoname==="hash") {
				...
				this.page.$sub_title_area.on('click', (event) => {
					...
					finergy.utils.copy_to_clipboard(this.frm.docname);
				});
			}
		} else {
		...
		if(this.frm.meta.title_field) {
			finergy.utils.set_title(title + " - " + this.frm.docname);
		}
	...
	rename_document_title(new_name, new_title, merge=false) {
		...
		let rename_document = () => {
			return finergy.xcall("finergy.model.rename_doc.update_document_title", {
				doctype,
				docname,
				new_name,
				title_field,
		...
		return new Promise((resolve, reject) => {
			if (new_title === this.frm.doc[title_field] && new_name === docname) {
				...
				finergy.confirm(confirm_message, () => {
	...
	set_indicator() {
		var indicator = finergy.get_indicator(this.frm.doc);
	...
	make_viewers() {
		...
		this.frm.viewers = new finergy.ui.form.FormViewers({
			frm: this.frm,
	...
	make_menu_items() {
		...
		const is_submittable = finergy.model.is_submittable(this.frm.doc.doctype);
		...
		if (!is_submittable || docstatus == 1  ||
			...
			if (finergy.model.can_print(null, me.frm) && !this.frm.meta.issingle) {
		...
		// email
		if(finergy.model.can_email(null, me.frm) && me.frm.doc.docstatus < 2) {
		...
		// duplicate
		if(in_list(finergy.boot.user.can_create, me.frm.doctype) && !me.frm.meta.allow_copy) {
		...
		this.page.add_menu_item(__("Copy to Clipboard"), function() {
			finergy.utils.copy_to_clipboard(JSON.stringify(me.frm.doc));
		}, true);
		...
		if((cint(me.frm.doc.docstatus) != 1) && !me.frm.doc.__islocal
			&& finergy.model.can_delete(me.frm.doctype)) {
		...
		if(this.can_repeat()) {
			this.page.add_menu_item(__("Repeat"), function(){
				finergy.utils.new_auto_repeat_prompt(me.frm);
			}, true);
		}
		...
		if(p[CREATE] && !this.frm.meta.issingle) {
			this.page.add_menu_item(__("New {0}", [__(me.frm.doctype)]), function() {
				finergy.new_doc(me.frm.doctype, true);
			}, true, {
				shortcut: 'Ctrl+B',
				condition: () => !this.frm.is_new()
			});
	...
	make_customize_buttons() {
		if (finergy.user_roles.includes("System Manager")) {
			...
			if (finergy.boot.developer_mode===1 && !is_doctype_form) {
				// edit doctype
				this.page.add_menu_item(__("Edit DocType"), () => {
					finergy.set_route('Form', 'DocType', this.frm.doctype);
				}, true);
			}
		}
	...
	has_workflow() {
		if(this._has_workflow === undefined)
			this._has_workflow = finergy.get_list("Workflow", {document_type: this.frm.doctype}).length;
	...
	show_linked_with() {
		if(!this.frm.linked_with) {
			this.frm.linked_with = new finergy.ui.form.LinkedWith({
				frm: this.frm
			});
		}
	...
	set_page_actions(status) {
		...
		if(status === "Edit") {
			...
			if (this.has_workflow()) {
				finergy.xcall('finergy.model.workflow.can_cancel_document', {
