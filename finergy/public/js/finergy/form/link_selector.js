...
finergy.ui.form.LinkSelector = Class.extend({
	init: function (opts) {
		...
		if (this.doctype != "[Select]") {
			finergy.model.with_doctype(this.doctype, function (r) {
				me.make();
			});
		} else {
			this.make();
	...
	search: function () {
		...
		finergy.link_search(this.doctype, args, function (r) {
			...
			if (r.values.length) {
				...
				$('<p><br><span class="text-muted">' + __("No Results") + '</span>'
					+ (finergy.model.can_create(me.doctype) ?
			...
			finergy.prompt({
				...
				$.each(me.target.frm.doc[me.target.df.fieldname] || [], function (i, d) {
					if (d[me.fieldname] === value) {
						finergy.model.set_value(d.doctype, d.name, me.qty_fieldname, data.qty);
						finergy.show_alert(__("Added {0} ({1})", [value, d[me.qty_fieldname]]));
						updated = true;
						return false;
					}
				});
				if (!updated) {
					finergy.run_serially([
						...
						() => {
							...
							return finergy.model.set_value(d.doctype, d.name, args);
						},
						() => finergy.show_alert(__("Added {0} ({1})", [value, data.qty]))
					]);
				}
			}, __("Set Quantity"), __("Set"));
		} else if (me.dynamic_link_field) {
			var d = me.target.add_new_row();
			finergy.model.set_value(d.doctype, d.name, me.dynamic_link_field, me.dynamic_link_reference);
			finergy.model.set_value(d.doctype, d.name, me.fieldname, value);
			finergy.show_alert(__("{0} {1} added", [me.dynamic_link_reference, value]));
		} else {
			var d = me.target.add_new_row();
			finergy.model.set_value(d.doctype, d.name, me.fieldname, value);
			finergy.show_alert(__("{0} added", [value]));
		}
	}
});

finergy.link_search = function (doctype, args, callback, btn) {
	if (!args) {
		args = {
			txt: ''
		}
