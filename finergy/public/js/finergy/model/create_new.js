...
$.extend(finergy.model, {
	...
	get_new_doc: function(
		...
		var doc = {
			docstatus: 0,
			doctype: doctype,
			name: finergy.model.get_new_name(doctype),
			__islocal: 1,
			__unsaved: 1,
			owner: finergy.session.user
		};
		finergy.model.set_default_values(doc, parent_doc);
		...
		if (parent_doc) {
			...
			finergy.provide("finergy.model.docinfo." + doctype + "." + doc.name);
		}

		finergy.model.add_to_locals(doc);

		if (with_mandatory_children) {
			finergy.model.create_mandatory_children(doc);
		}
		...
		if (finergy.route_options && finergy.route_options.name_field) {
			var meta = finergy.get_meta(doctype);
			// set title field / name as name
			if (meta.autoname && meta.autoname.indexOf("field:") !== -1) {
				doc[meta.autoname.substr(6)] = finergy.route_options.name_field;
			} else if (meta.title_field) {
				doc[meta.title_field] = finergy.route_options.name_field;
			}

			delete finergy.route_options.name_field;
		}

		// set route options
		if (finergy.route_options && !doc.parent) {
			$.each(finergy.route_options, function(fieldname, value) {
				var df = finergy.meta.has_field(doctype, fieldname);
				if (
					df &&
					in_list(
	...
	make_new_doc_and_get_name: function(doctype, with_mandatory_children) {
		return finergy.model.get_new_doc(
			doctype,
			null,
			null,
			with_mandatory_children
	...
	get_new_name: function(doctype) {
		var cnt = finergy.model.new_name_count;
		if (!cnt[doctype]) cnt[doctype] = 0;
		cnt[doctype]++;
		return finergy.router.slug(`new-${doctype}-${cnt[doctype]}`);
	},

	set_default_values: function(doc, parent_doc) {
		var doctype = doc.doctype;
		var docfields = finergy.meta.get_docfields(doctype);
		var updated = [];
		for (var fid = 0; fid < docfields.length; fid++) {
			var f = docfields[fid];
			if (
				!in_list(finergy.model.no_value_type, f.fieldtype) &&
				doc[f.fieldname] == null
			) {
				if (f.no_default) continue;
				var v = finergy.model.get_default_value(f, doc, parent_doc);
				if (v) {
	...
	create_mandatory_children: function(doc) {
		var meta = finergy.get_meta(doc.doctype);
		if (meta && meta.istable) return;

		// create empty rows for mandatory table fields
		finergy.meta.get_docfields(doc.doctype).forEach(function(df) {
			if (df.fieldtype === "Table" && df.reqd) {
				finergy.model.add_child(doc, df.fieldname);
			}
		});
	},
	...
	get_default_value: function(df, doc, parent_doc) {
		var user_default = "";
		var user_permissions = finergy.defaults.get_user_permissions();
		let allowed_records = [];
		...
		if (user_permissions) {
			({
				allowed_records,
				default_doc
			} = finergy.perm.filter_allowed_docs_for_doctype(
		...
		if (df.fieldtype === "Link" && df.options !== "User") {
			...
			if (has_user_permissions && default_doc) {
				...
				if (!df.ignore_user_permissions) {
					var user_defaults = finergy.defaults.get_user_defaults(df.options);
				...
				if (!user_default) {
					user_default = finergy.defaults.get_user_default(df.fieldname);
				} else if (
					!user_default &&
					df.remember_last_selected_value &&
					finergy.boot.user.last_selected_values
				) {
					user_default = finergy.boot.user.last_selected_values[df.options];
				}
		...
		if (!value || df["default"]) {
			...
			if (
				...
				value = finergy.session.user;
			} else if (default_val == "user_fullname") {
				value = finergy.session.user_fullname;
			} else if (default_val == "Today") {
				value = finergy.datetime.get_today();
			} else if ((default_val || "").toLowerCase() === "now") {
				value = finergy.datetime.now_datetime();
			} else if (default_val[0] === ":") {
				var boot_doc = finergy.model.get_default_from_boot_docs(
					df,
					doc,
					parent_doc
				);
			...
			value = finergy.datetime.now_time();
		}
	...
	get_default_from_boot_docs: function(df, doc, parent_doc) {
		...
		if (finergy.get_list(df["default"]).length > 0) {
			...
			var ref_value = parent_doc
				...
				: finergy.defaults.get_user_default(ref_fieldname);
			var ref_doc = ref_value
				? finergy.get_doc(df["default"], ref_value)
				: null;
	...
	add_child: function(parent_doc, doctype, parentfield, idx) {
		...
		if (arguments.length === 2) {
			parentfield = doctype;
			doctype = finergy.meta.get_field(parent_doc.doctype, parentfield)
				.options;
		}
		...
		var child = finergy.model.get_new_doc(doctype, parent_doc, parentfield);
		child.idx = idx;
	...
	copy_doc: function(doc, from_amend, parent_doc, parentfield) {
		...
		var newdoc = finergy.model.get_new_doc(
			doc.doctype,
			parent_doc,
			parentfield
		);
		...
		for (var key in doc) {
			...
			var df = finergy.meta.get_docfield(doc.doctype, key);
	...
	open_mapped_doc: function(opts) {
		...
		return finergy.call({
			type: "POST",
			method: "finergy.model.mapper.make_mapped_doc",
			args: {
				method: opts.method,
			...
			callback: function(r) {
				if (!r.exc) {
					...
					finergy.set_route("Form", r.message.doctype, r.message.name);
				}
			}
		});
	}
...
finergy.create_routes = {};
finergy.new_doc = function(doctype, opts, init_callback) {
	if (doctype === "File") {
	...
	return new Promise(resolve => {
		if (opts && $.isPlainObject(opts)) {
			finergy.route_options = opts;
		}
		finergy.model.with_doctype(doctype, function() {
			if (finergy.create_routes[doctype]) {
				finergy
					.set_route(finergy.create_routes[doctype])
					.then(() => resolve());
			} else {
				finergy.ui.form
