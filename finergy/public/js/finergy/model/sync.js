...
$.extend(finergy.model, {
	...
	sync: function(r) {
		/* docs:
			...
			from incoming request and set in `locals` and `finergy.model.docinfo`
		*/
		var isPlain;
		...
		if(r.docs) {
			...
			for(var i=0, l=r.docs.length; i<l; i++) {
				...
				if (locals[d.doctype] && locals[d.doctype][d.name]) {
					// update values
					finergy.model.update_in_locals(d);
				} else {
					finergy.model.add_to_locals(d);
				}
				...
				if(d.localname) {
					finergy.model.new_names[d.localname] = d.name;
					$(document).trigger('rename', [d.doctype, d.localname, d.name]);
					delete locals[d.doctype][d.localname];

					// update docinfo to new dict keys
					if(i===0) {
						finergy.model.docinfo[d.doctype][d.name] = finergy.model.docinfo[d.doctype][d.localname];
						finergy.model.docinfo[d.doctype][d.localname] = undefined;
					}
				}
			}
		...
		if(r.docinfo) {
			...
			if(doc) {
				if(!finergy.model.docinfo[doc.doctype])
					finergy.model.docinfo[doc.doctype] = {};
				finergy.model.docinfo[doc.doctype][doc.name] = r.docinfo;
			}
		}
	...
	add_to_locals: function(doc) {
		...
		if(!doc.name && doc.__islocal) { // get name (local if required)
			if(!doc.parentfield) finergy.model.clear_doc(doc);

			doc.name = finergy.model.get_new_name(doc.doctype);

			if(!doc.parentfield) finergy.provide("finergy.model.docinfo." + doc.doctype + "." + doc.name);
		}
	...
	update_in_locals: function(doc) {
		...
		for (let fieldname in doc) {
			let df = finergy.meta.get_field(doc.doctype, fieldname);
			if (df && finergy.model.table_fields.includes(df.fieldtype)) {
				// table
				...
				for (let i=0; i < doc[fieldname].length; i++ ) {
					...
					if (local_d) {
						...
						if (!d.name) {
							...
							d.name = finergy.model.get_new_name(doc.doctype);
						}
