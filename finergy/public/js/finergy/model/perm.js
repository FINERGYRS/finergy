...
$.extend(finergy.perm, {
	...
	has_perm: (doctype, permlevel, ptype, doc) => {
		...
		if (!finergy.perm.doctype_perm[doctype]) {
			finergy.perm.doctype_perm[doctype] = finergy.perm.get_perm(doctype);
		}

		let perms = finergy.perm.doctype_perm[doctype];
		...
		if (permlevel === 0 && perm && doc) {
			let docinfo = finergy.model.get_docinfo(doctype, doc.name);
	...
	get_perm: (doctype, doc) => {
		...
		let meta = finergy.get_doc("DocType", doctype);
		const user  = finergy.session.user;

		if (user === "Administrator" || finergy.user_roles.includes("Administrator")) {
			perm[0].read = 1;
		}

		if (!meta) return perm;

		perm = finergy.perm.get_role_permissions(meta);
				...
				for (let i = 0; i < docinfo.shared.length; i++) {
					...
					if (s.user === user) {
						...
						if (s.read) {
							...
							perm[0].email = finergy.boot.user.can_email.indexOf(doctype) !== -1 ? 1 : 0;
							perm[0].print = finergy.boot.user.can_print.indexOf(doctype) !== -1 ? 1 : 0;
						}
					}
				}
			}
		...
		if (finergy.model.can_read(doctype) && !perm[0].read) {
			// read via sharing
			perm[0].read = 1;
		}
	...
	get_role_permissions: (meta) => {
		...
		(meta.permissions || []).forEach(p => {
			...
			if (finergy.user_roles.includes(p.role)) {
				finergy.perm.rights.forEach(right => {
	...
	get_match_rules: (doctype, ptype) => {
		...
		let perm = finergy.perm.get_perm(doctype);

		let user_permissions = finergy.defaults.get_user_permissions();

		if (user_permissions && !$.isEmptyObject(user_permissions)) {
			let rules = {};
			let fields_to_check = finergy.meta.get_fields_to_check_permissions(doctype);
			$.each(fields_to_check, (i, df) => {
				const user_permissions_for_doctype = user_permissions[df.options] || [];
				const allowed_records = finergy.perm.get_allowed_docs_for_doctype(user_permissions_for_doctype, doctype);
		...
		if (perm[0].if_owner && perm[0].read) {
			match_rules.push({ "Owner": finergy.session.user });
		}
		return match_rules;
	},
	...
	get_field_display_status: (df, doc, perm, explain) => {
		...
		if (!perm && doc) {
			perm = finergy.perm.get_perm(doc.doctype, doc);
		}
	...
	is_visible: (df, doc, perm) => {
		if (typeof df === 'string') {
			// df is fieldname
			df = finergy.meta.get_docfield(doc.doctype, df, doc.parent || doc.name);
		}

		let status = finergy.perm.get_field_display_status(df, doc, perm);
	...
	get_allowed_docs_for_doctype: (user_permissions, doctype) => {
		...
		return finergy.perm.filter_allowed_docs_for_doctype(user_permissions, doctype, false);
	},
