...
$.extend(finergy.model, {
	...
	init: function() {
		...
		finergy.realtime.on("doc_update", function(data) {
			// set list dirty
			finergy.views.ListView.trigger_list_update(data);
			var doc = locals[data.doctype] && locals[data.doctype][data.name];

			if(doc) {
				// current document is dirty, show message if its not me
				if(finergy.get_route()[0]==="Form" && cur_frm.doc.doctype===doc.doctype && cur_frm.doc.name===doc.name) {
					if(!finergy.ui.form.is_saving && data.modified!=cur_frm.doc.modified) {
					...
					if(!doc.__unsaved) {
						...
						finergy.model.remove_from_locals(doc.doctype, doc.name);
					} else {
		...
		finergy.realtime.on("list_update", function(data) {
			finergy.views.ListView.trigger_list_update(data);
		});
	...
	is_value_type: function(fieldtype) {
		...
		return finergy.model.no_value_type.indexOf(fieldtype)===-1;
	},

	is_non_std_field: function(fieldname) {
		return !finergy.model.std_fields_list.includes(fieldname);
	},

	get_std_field: function(fieldname, ignore=false) {
		var docfield = $.map([].concat(finergy.model.std_fields).concat(finergy.model.std_fields_table),
			function(d) {
		...
		if (!docfield.length) {
			...
			if (ignore) {
				...
				finergy.msgprint(__("Unknown Column: {0}", [fieldname]));
			}
		}
		return docfield[0];
	},
	...
	set_in_localstorage: function(doctype, docs) {
		try {
			...
			finergy.model.clear_local_storage();
			localStorage["_doctype:" + doctype] = JSON.stringify(docs);
		}
	},
	...
	with_doctype: function(doctype, callback, async) {
		if(locals.DocType[doctype]) {
			...
			let cached_docs = finergy.model.get_from_localstorage(doctype);
			...
			return finergy.call({
				method:'finergy.desk.form.load.getdoctype',
				type: "GET",
				args: {
					doctype: doctype,
					with_parent: 1,
				...
				callback: function(r) {
					if(r.exc) {
						finergy.msgprint(__("Unable to load: {0}", [__(doctype)]));
						throw "No doctype";
					}
					if(r.message=="use_cache") {
						finergy.model.sync(cached_doc);
					} else {
						finergy.model.set_in_localstorage(doctype, r.docs)
					}
					finergy.model.init_doctype(doctype);

					if(r.user_settings) {
						// remember filters and other settings from last view
						finergy.model.user_settings[doctype] = JSON.parse(r.user_settings);
						finergy.model.user_settings[doctype].updated_on = moment().toString();
					}
					callback && callback(r);
				}
			});
	...
	init_doctype: function(doctype) {
		...
		if(meta.__templates) {
			$.extend(finergy.templates, meta.__templates);
		}
	},
	...
	with_doc: function(doctype, name, callback) {
		return new Promise(resolve => {
			...
			if(locals[doctype] && locals[doctype][name] && finergy.model.get_docinfo(doctype, name)) {
				callback && callback(name);
				resolve(finergy.get_doc(doctype, name));
			} else {
				return finergy.call({
					method: 'finergy.desk.form.load.getdoc',
					type: "GET",
					args: {
						doctype: doctype,
						name: name
	...
	get_docinfo: function(doctype, name) {
		return finergy.model.docinfo[doctype] && finergy.model.docinfo[doctype][name] || null;
	},

	set_docinfo: function(doctype, name, key, value) {
		if (finergy.model.docinfo[doctype] && finergy.model.docinfo[doctype][name]) {
			finergy.model.docinfo[doctype][name][key] = value;
		}
	},

	get_shared: function(doctype, name) {
		return finergy.model.get_docinfo(doctype, name).shared;
	},

	get_server_module_name: function(doctype) {
		var dt = finergy.model.scrub(doctype);
		var module = finergy.model.scrub(locals.DocType[doctype].module);
		var app = finergy.boot.module_app[module];
	...
	can_create: function(doctype) {
		return finergy.boot.user.can_create.indexOf(doctype)!==-1;
	},

	can_select: function(doctype) {
		if (finergy.boot.user) {
			return finergy.boot.user.can_select.indexOf(doctype)!==-1;
		}
	},

	can_read: function(doctype) {
		if (finergy.boot.user) {
			return finergy.boot.user.can_read.indexOf(doctype)!==-1;
		}
	},

	can_write: function(doctype) {
		return finergy.boot.user.can_write.indexOf(doctype)!==-1;
	},

	can_get_report: function(doctype) {
		return finergy.boot.user.can_get_report.indexOf(doctype)!==-1;
	},

	can_delete: function(doctype) {
		if(!doctype) return false;
		return finergy.boot.user.can_delete.indexOf(doctype)!==-1;
	},

	can_cancel: function(doctype) {
		if(!doctype) return false;
		return finergy.boot.user.can_cancel.indexOf(doctype)!==-1;
	},

	has_workflow: function(doctype) {
		return finergy.get_list('Workflow', {'document_type': doctype,
			'is_active': 1}).length;
	},
	...
	is_single: function(doctype) {
		if(!doctype) return false;
		return finergy.boot.single_types.indexOf(doctype) != -1;
	},

	is_tree: function(doctype) {
		if (!doctype) return false;
		return finergy.boot.treeviews.indexOf(doctype) != -1;
	},
	...
	can_import: function(doctype, frm) {
		...
		if(finergy.user_roles.includes("System Manager")) return true;

		if(frm) return frm.perm[0].import===1;
		return finergy.boot.user.can_import.indexOf(doctype)!==-1;
	},
	...
	can_export: function(doctype, frm) {
		...
		return finergy.boot.user.can_export.indexOf(doctype)!==-1;
	},

	can_print: function(doctype, frm) {
		if(frm) return frm.perm[0].print===1;
		return finergy.boot.user.can_print.indexOf(doctype)!==-1;
	},

	can_email: function(doctype, frm) {
		if(frm) return frm.perm[0].email===1;
		return finergy.boot.user.can_email.indexOf(doctype)!==-1;
	},
	...
	can_share: function(doctype, frm) {
		...
		return finergy.boot.user.can_share.indexOf(doctype)!==-1;
	},
	...
	can_set_user_permissions: function(doctype, frm) {
		...
		return finergy.boot.user.can_set_user_permissions.indexOf(doctype)!==-1;
	},

	has_value: function(dt, dn, fn) {
		// return true if property has value
		var val = locals[dt] && locals[dt][dn] && locals[dt][dn][fn];
		var df = finergy.meta.get_docfield(dt, fn, dn);

		if(finergy.model.table_fields.includes(df.fieldtype)) {
			var ret = false;
	...
	get_list: function(doctype, filters) {
		...
		if($.isEmptyObject(docsdict))
			return [];
		return finergy.utils.filter_dict(docsdict, filters);
	},
	...
	get_value: function(doctype, filters, fieldname, callback) {
		if(callback) {
			...
			if(typeof filters==="string" && locals[doctype] && locals[doctype][filters]) {
				...
				var l = finergy.get_list(doctype, filters);
	...
	set_value: function(doctype, docname, fieldname, value, fieldtype) {
		...
		$.each(to_update, (key, value) => {
			if (doc && doc[key] !== value) {
				...
				doc[key] = value;
				tasks.push(() => finergy.model.trigger(key, value, doc));
			} else {
	...
	on: function(doctype, fieldname, fn) {
		...
		*/
		/* example: finergy.model.on("Customer", "age", function(fieldname, value, doc) {
		  if(doc.age < 16) {
		   	finergy.msgprint("Warning, Customer must atleast be 16 years old.");
		    raise "CustomerAgeError";
		  }
		}) */
		finergy.provide("finergy.model.events." + doctype);
		if(!finergy.model.events[doctype][fieldname]) {
			finergy.model.events[doctype][fieldname] = [];
		}
		finergy.model.events[doctype][fieldname].push(fn);
	},
		...
		function enqueue_events(events) {
			...
			for (const fn of events) {
				...
				tasks.push(() => {
					...
					// or use the default promise finergy.after_ajax
					if (return_value && return_value.then) {
						return return_value;
					} else {
						return finergy.after_server_call();
					}
				});
			}
		};

		if(finergy.model.events[doc.doctype]) {
			enqueue_events(finergy.model.events[doc.doctype][fieldname]);
			enqueue_events(finergy.model.events[doc.doctype]['*']);
		}
	...
	get_doc: function(doctype, name) {
		...
		if($.isPlainObject(name)) {
			var doc = finergy.get_list(doctype, name);
	...
	get_children: function(doctype, parent, parentfield, filters) {
		if($.isPlainObject(doctype)) {
			...
			var doc = finergy.get_doc(doctype, parent);
		}

		var children = doc[parentfield] || [];
		if(filters) {
			return finergy.utils.filter_dict(children, filters);
		} else {
			return children;
		}
	},
	...
	remove_from_locals: function(doctype, name) {
		...
		if(finergy.views.formview[doctype]) {
			delete finergy.views.formview[doctype].frm.opendocs[name];
		}
	},
	...
	get_no_copy_list: function(doctype) {
		...
		var docfields = finergy.get_doc("DocType", doctype).fields || [];
	...
	delete_doc: function(doctype, docname, callback) {
		let title = docname;
		const title_field = finergy.get_meta(doctype).title_field;
		if (finergy.get_meta(doctype).autoname == "hash" && title_field) {
			const value = finergy.model.get_value(doctype, docname, title_field);
			if (value) {
				title = `${value} (${docname})`;
			}
		}
		finergy.confirm(__("Permanently delete {0}?", [title.bold()]), function() {
			return finergy.call({
				method: 'finergy.client.delete',
				args: {
					doctype: doctype,
					name: docname
				},
				callback: function(r, rt) {
					if(!r.exc) {
						finergy.utils.play_sound("delete");
						finergy.model.clear_doc(doctype, docname);
	...
	round_floats_in: function(doc, fieldnames) {
		if(!fieldnames) {
			fieldnames = finergy.meta.get_fieldnames(doc.doctype, doc.parent,
	...
	validate_missing: function(doc, fieldname) {
		if(!doc[fieldname]) {
			finergy.throw(__("Please specify") + ": " +
				__(finergy.meta.get_label(doc.doctype, fieldname, doc.parent || doc.name)));
		}
	},
	...
	is_numeric_field: function(fieldtype) {
		...
		return finergy.model.numeric_fieldtypes.includes(fieldtype);
	}
});

// legacy
finergy.get_doc = finergy.model.get_doc;
finergy.get_children = finergy.model.get_children;
finergy.get_list = finergy.model.get_list;
