...
export default {
	...
	methods: {
		...
		refresh: function() {
			finergy.call("finergy.recorder.get").then( r => this.requests = r.message);
		},
		update: function(message) {
			this.requests.push(JSON.parse(message));
		},
		clear: function() {
			finergy.call("finergy.recorder.delete").then(r => this.refresh());
		},
		start: function() {
			finergy.call("finergy.recorder.start").then(r => this.fetch_status());
		},
		stop: function() {
			finergy.call("finergy.recorder.stop").then(r => this.fetch_status());
		},
		fetch_status: function() {
			finergy.call("finergy.recorder.status").then(r => this.update_status(r.message));
		},
		update_status: function(result) {
			...
			if(this.status.status == "Active") {
				finergy.realtime.on("recorder-dump-event", this.update);
			} else {
				finergy.realtime.off("recorder-dump-event", this.update);
			}
		...
		export_data: function() {
			...
			finergy.call("finergy.recorder.export_data")
				.then((r) => {
