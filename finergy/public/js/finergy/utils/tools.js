...
finergy.provide("finergy.tools");

finergy.tools.downloadify = function(data, roles, title) {
	if(roles && roles.length && !has_common(roles, roles)) {
		finergy.msgprint(__("Export not allowed. You need {0} role to export.", [finergy.utils.comma_or(roles)]));
		return;
	}

	var filename = title + ".csv";
	var csv_data = finergy.tools.to_csv(data);
...
finergy.markdown = function(txt) {
	if(!finergy.md2html) {
		finergy.md2html = new showdown.Converter();
	}
	...
	return finergy.md2html.makeHtml(txt);
};

finergy.tools.to_csv = function(data) {
	var res = [];
