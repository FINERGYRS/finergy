function prettyDate(date, mini) {
	...
	if (typeof (date) == "string") {
		date = finergy.datetime.convert_to_user_tz(date);
...
window.comment_when = function(datetime, mini) {
	var timestamp = finergy.datetime.str_to_user ?
		finergy.datetime.str_to_user(datetime) : datetime;
	return '<span class="finergy-timestamp '
		+ (mini ? " mini" : "") + '" data-timestamp="' + datetime
		+ '" title="' + timestamp + '">'
		+ prettyDate(datetime, mini) + '</span>';
};
finergy.datetime.comment_when = comment_when;
finergy.datetime.prettyDate = prettyDate;

finergy.datetime.refresh_when = function() {
	if (jQuery) {
		$(".finergy-timestamp").each(function() {
