finergy.provide('finergy.contacts')

$.extend(finergy.contacts, {
	...
	render_address_and_contact: function(frm) {
		...
		if(frm.fields_dict['address_html'] && "addr_list" in frm.doc.__onload) {
			$(frm.fields_dict['address_html'].wrapper)
				.html(finergy.render_template("address_list",
					frm.doc.__onload))
				.find(".btn-address").on("click", function() {
					finergy.new_doc("Address");
				});
		}
		...
		if(frm.fields_dict['contact_html'] && "contact_list" in frm.doc.__onload) {
			$(frm.fields_dict['contact_html'].wrapper)
				.html(finergy.render_template("contact_list",
					frm.doc.__onload))
	...
	get_last_doc: function(frm) {
		const reverse_routes = finergy.route_history.slice().reverse();
