...
finergy.avatar = function (user, css_class, title, image_url=null, remove_color=false, filterable=false) {
	let user_info;
	if (user) {
		// desk
		user_info = finergy.user_info(user);
	} else {
		// website
		let full_name = title || finergy.get_cookie("full_name");
		user_info = {
			image: image_url === null ? finergy.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: finergy.get_abbr(full_name),
			color: finergy.get_palette(full_name)
		};
	}
	...
	return finergy.get_avatar(
		css_class, title, image_url || user_info.image, remove_color, data_attr
	);
};

finergy.get_avatar = function(css_class, title, image_url=null, remove_color, data_attributes) {
	if (!css_class) {
		css_class = "avatar-small";
	}

	if (image_url) {
		const image = (window.cordova && image_url.indexOf('http') === -1) ? finergy.base_url + image_url : image_url;
		return `<span class="avatar ${css_class}" title="${title}" ${data_attributes}>
				<span class="avatar-frame" style='background-image: url("${image}")'
					title="${title}"></span>
			</span>`;
	} else {
		let abbr =  finergy.get_abbr(title);
		let style = '';
		if (!remove_color) {
			let color = finergy.get_palette(title);
...
finergy.avatar_group = function (users, limit=4, options = {}) {
	...
	let html = display_users.map(user =>
		finergy.avatar(user, 'avatar-small ' + css_class, null, null, false, options.filterable)
	).join('');
	if (extra_users.length === 1) {
		html += finergy.avatar(extra_users[0], 'avatar-small ' + css_class, null, null, false, options.filterable);
	} else if (extra_users.length > 1) {
		html = `
			${html}
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map(u => finergy.user_info(u).fullname).join(', ')}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${finergy.utils.icon(options.action_icon, 'sm')}
				</div>
			</span>
		`;
	}
...
finergy.ui.scroll = function (element, animate, additional_offset) {
...
finergy.get_palette = function (txt) {
	var idx = cint((parseInt(md5(txt).substr(4, 2), 16) + 1) / 5.33);
	return finergy.palette[idx % 8];
}

finergy.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
finergy.gravatars = {};
finergy.get_gravatar = function (email_id, size = 0) {
	var param = size ? ('s=' + size) : 'd=retro';
	if (!finergy.gravatars[email_id]) {
		// TODO: check if gravatar exists
		finergy.gravatars[email_id] = "https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return finergy.gravatars[email_id];
}
...
finergy.get_cookie = function getCookie(name) {
	return finergy.get_cookies()[name];
}

finergy.get_cookies = function getCookies() {
...
finergy.is_mobile = function () {
	return $(document).width() < 768;
}

finergy.utils.xss_sanitise = function (string, options) {
...
finergy.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = finergy.utils.strip_url(url);

	return is_external(url) ? "" : sanitise_javascript(finergy.utils.xss_sanitise(url, { strategies: ["js"] }));
};

finergy.utils.strip_url = (url) => {
...
finergy.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			'fieldname': 'frequency',
			'fieldtype': 'Select',
	...
	finergy.prompt(fields, function (values) {
		finergy.call({
			method: "finergy.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
			args: {
...
finergy.utils.get_page_view_count = function (route) {
	return finergy.call("finergy.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route
	});
};