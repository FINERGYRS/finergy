...
Object.assign(finergy.energy_points, {
	get_points(points) {
	...
	format_history_log(log) {
		...
		const separator = `<span>&nbsp;-&nbsp;</span>`;
		const route = finergy.utils.get_form_link(log.reference_doctype, log.reference_name);
		const formatted_log = `<div class="flex">
			...
			${log.reason ? separator + log.reason : ''}
			${separator + finergy.datetime.comment_when(log.creation)}
		</div>`;
		return formatted_log;
	},
	get_history_log_message(log) {
		const owner_name = finergy.user.full_name(log.owner).bold();
	...
	get_form_log_message(log) {
		// redundant code to honor readability and to avoid confusion
		const owner_name = finergy.user.full_name(log.owner).bold();
		const user = finergy.user.full_name(log.user).bold();
		if (log.type === 'Appreciation') {
