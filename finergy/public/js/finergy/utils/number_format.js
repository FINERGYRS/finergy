...
if (!window.finergy) window.finergy = {};
...
window.format_number = function (v, format, decimals) {
	if (!format) {
		...
		if (decimals == null) decimals = cint(finergy.defaults.get_default("float_precision")) || 3;
	}
...
function format_currency(v, currency, decimals) {
	...
	if(decimals === undefined) {
		decimals = finergy.boot.sysdefaults.currency_precision || null;
	}
...
function get_currency_symbol(currency) {
	if (finergy.boot) {
		if (finergy.boot.sysdefaults && finergy.boot.sysdefaults.hide_currency_symbol == "Yes")
			return null;

		if (!currency)
			currency = finergy.boot.sysdefaults.currency;

		return finergy.model.get_value(":Currency", currency, "symbol") || currency;
	} else {
		// load in template
		return finergy.currency_symbols[currency];
	}
}

function get_number_format(currency) {
	return (finergy.boot && finergy.boot.sysdefaults && finergy.boot.sysdefaults.number_format) || "#,###.##";
}

function get_number_format_info(format) {
	var info = finergy.number_format_info[format];
...
function precision(fieldname, doc) {
	if (cur_frm) {
		...
		var df = finergy.meta.get_docfield(doc.doctype, fieldname, doc.parent || doc.name);
		if (!df) console.log(fieldname + ": could not find docfield in method precision()");
		return finergy.meta.get_field_precision(df, doc);
	} else {
		return finergy.boot.sysdefaults.float_precision
	}
}
...
function round_based_on_smallest_currency_fraction(value, currency, precision) {
	var smallest_currency_fraction_value = flt(finergy.model.get_value(":Currency",
