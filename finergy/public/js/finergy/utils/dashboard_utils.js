finergy.dashboard_utils = {
	render_chart_filters: function(filters, button_class, container, append) {
		filters.forEach(filter => {
			...
			if (filter.icon) {
				icon_html = finergy.utils.icon(filter.icon);
			}
	...
	get_filters_for_chart_type: function(chart) {
		if (chart.chart_type === 'Custom' && chart.source) {
			const method = 'finergy.desk.doctype.dashboard_chart_source.dashboard_chart_source.get_config';
			return finergy.xcall(method, {name: chart.source}).then(config => {
				finergy.dom.eval(config);
				return finergy.dashboards.chart_sources[chart.source].filters;
			});
		} else if (chart.chart_type === 'Report' && chart.report_name) {
			return finergy.report_utils.get_report_filters(chart.report_name).then(filters => {
				return filters;
			});
		} else {
	...
	get_dashboard_settings() {
		return finergy.db.get_list('Dashboard Settings', {
			filters: {
	...
	create_dashboard_settings() {
		return finergy.xcall(
			'finergy.desk.doctype.dashboard_settings.dashboard_settings.create_dashboard_settings',
			{
	...
	get_years_since_creation(creation) {
		...
		let current_year = this.get_year(finergy.datetime.now_date());
		let years_list = [];
		...
		let fields = [
			{
				...
				options:
					`<div>
						...
						<p>Ex:
							<code>finergy.defaults.get_user_default("Company")</code>
						</p>
					</div>`
			}
		];
		...
		if ($.isArray(dynamic_filters)) {
			dynamic_filters.forEach(f => {
				try {
					...
					finergy.throw(__("Invalid expression set in filter {0} ({1})", [f[1], f[0]]));
				}
			});
			filters = [...filters, ...dynamic_filters];
		} else {
			for (let key of Object.keys(dynamic_filters)) {
				try {
					...
					finergy.throw(__("Invalid expression set in filter {0}", [key]));
				}
			}
	...
	get_add_to_dashboard_dialog(docname, doctype, method) {
		...
		const dialog = new finergy.ui.Dialog({
			...
			primary_action: (values) => {
				values.name = docname;
				values.set_standard = finergy.boot.developer_mode;
				finergy.xcall(
					method,
					{args: values}
				).then(()=> {
