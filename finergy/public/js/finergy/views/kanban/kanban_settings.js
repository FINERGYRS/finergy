export default class KanbanSettings {
	constructor({ kanbanview, doctype, meta, settings }) {
		if (!doctype) {
			finergy.throw(__("DocType required"));
		}
	...
	make() {
		...
		this.dialog.set_primary_action(__("Save"), () => {
			...
			finergy.call({
				method:
					"finergy.desk.doctype.kanban_board.kanban_board.save_settings",
				args: {
		...
		for (let fieldname of this.fields) {
			...
			fields += `
				<div class="control-input flex align-center form-control fields_order sortable"
					...
					<div class="row">
						<div class="col-md-1">
							${finergy.utils.icon("drag", "xs", "", "", "sortable-handle")}
						</div>
	...
	get_docfield(field_name) {
		return (
			finergy.meta.get_docfield(this.doctype, field_name) ||
	...
	get_multiselect_fields() {
		...
		const ignore_fieldtypes = [
			...
			"Code",
			"Color",
			...finergy.model.no_value_type
		];

		return finergy.model.std_fields
