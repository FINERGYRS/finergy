...
(function() {
	...
	var method_prefix = 'finergy.desk.doctype.kanban_board.kanban_board.';
	...
	var store = fluxify.createStore({
		...
		actionCallbacks: {
			...
			add_column: function(updater, col) {
				if(finergy.model.can_create('Custom Field')) {
			...
			add_card: function(updater, card_title, column_title) {
				...
				if (field && !quick_entry) {
					...
					finergy.new_doc(this.doctype, doc);
				}
			},
	...
	finergy.views.KanbanBoard = function(opts) {
		...
		function prepare() {
			...
			if(self.$kanban_board.length === 0) {
				self.$kanban_board = $(finergy.render_template("kanban_board"));
		...
		function make_columns() {
			...
			columns.filter(is_active_column).map(function(col) {
				finergy.views.KanbanBoardColumn(col, self.$kanban_board);
			});
		}
	...
	finergy.views.KanbanBoardColumn = function(column, wrapper) {
		var self = {};
		var filtered_cards = [];
		...
		function make_dom() {
			self.$kanban_column = $(finergy.render_template(
				'kanban_column', {
					title: column.title,
			...
			if(order) {
				...
				filtered_cards.forEach(function(card) {
					if(order.indexOf(card.name) === -1) {
						finergy.views.KanbanBoardCard(card, self.$kanban_cards);
					}
				});
				order.forEach(function(name) {
					if (!filtered_cards_names.includes(name)) return;
					finergy.views.KanbanBoardCard(get_card(name), self.$kanban_cards);
				});
			} else {
	...
	finergy.views.KanbanBoardCard = function(card, wrapper) {
		var self = {};
		...
		function make_dom() {
			var opts = {
				name: card.name,
				title: finergy.utils.html2text(card.title),
				disable_click: card._disable_click ? 'disable-click' : '',
				creation: card.creation,
				doc_content: get_doc_content(card),
				image_url: cur_list.get_image_url(card),
				form_link: finergy.utils.get_form_link(card.doctype, card.name)
			};

			self.$card = $(finergy.render_template('kanban_card', opts))
				.appendTo(wrapper);
		}
		...
		function get_doc_content(card) {
			...
			for (let field_name of cur_list.board.fields) {
				let field = (
					finergy.meta.get_docfield(card.doctype, field_name, card.name)
					|| finergy.model.get_std_field(field_name)
				);
				let label = cur_list.board.show_labels ? `<span>${__(field.label)}: </span>` : '';
				let value = finergy.format(card.doc[field_name], field);
				fields.push(`
		...
		function render_card_meta() {
			...
			if (card.color && finergy.ui.color.validate_hex(card.color)) {
				const $div = $('<div>');
				$('<div></div>').css({
					width: '30px',
					height: '4px',
		...
		function get_assignees_group() {
			return finergy.avatar_group(card.assigned_list, 3, {
		...
		function show_assign_to_dialog(e) {
			e.preventDefault();
			e.stopPropagation();
			self.assign_to = new finergy.ui.form.AssignToDialog({
				obj: self,
				method: 'finergy.desk.form.assign_to.add',
	...
	function fetch_customization(doctype) {
		return new Promise(function(resolve) {
			finergy.model.with_doc("Customize Form", "Customize Form", function() {
				var doc = finergy.get_doc("Customize Form");
	...
	function insert_doc(doc) {
		return finergy.call({
			method: "finergy.client.insert",
			args: {
				doc: doc
			},
			callback: function() {
				finergy.model.clear_doc(doc.doctype, doc.name);
				finergy.show_alert({ message: __("Saved"), indicator: 'green' }, 1);
			}
		});
	}
	...
	function get_column_indicators(callback) {
		finergy.model.with_doctype('Kanban Board Column', function() {
			var meta = finergy.get_meta('Kanban Board Column');
			var indicators;
