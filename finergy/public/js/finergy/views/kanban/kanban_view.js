...
finergy.views.KanbanView = class KanbanView extends finergy.views.ListView {
	static load_last_view() {
		const route = finergy.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = finergy.get_user_settings(doctype)['Kanban'] || {};
			if (!user_settings.last_kanban_board) {
				...
				finergy.set_route('List', doctype, 'List');
				return true;
			}
	...
	setup_defaults() {
		return super.setup_defaults()
			.then(() => {
				this.board_name = finergy.get_route()[3];
	...
	get_board() {
		return finergy.db.get_doc('Kanban Board', this.board_name)
			.then(board => {
				this.board = board;
	...
	save_kanban_board_filters() {
		...
		finergy.call({
			method: 'finergy.desk.doctype.kanban_board.kanban_board.save_filters',
			args: {
	...
	render() {
		...
		this.kanban = new finergy.views.KanbanBoard({
	...
	get_card_meta() {
		var meta = finergy.get_meta(this.doctype);
		var doc = finergy.model.get_new_doc(this.doctype);
		var title_field = null;
		var quick_entry = false;

		if (this.meta.title_field) {
			title_field = finergy.meta.get_field(this.doctype, this.meta.title_field);
		}
		...
		if (mandatory.some(df => finergy.model.table_fields.includes(df.fieldtype)) || mandatory.length > 1) {
			quick_entry = true;
		}

		if (!title_field) {
			title_field = finergy.meta.get_field(this.doctype, 'name');
		}
	...
	get required_libs() {
		return [
			'assets/finergyrs/js/lib/fluxify.min.js',
			'assets/finergyrs/js/finergyrs/views/kanban/kanban_board.js'
		];
	}
};


finergy.views.KanbanView.get_kanbans = function (doctype) {
	let kanbans = [];
		...
		.then((kanban_boards) => {
			if (kanban_boards) {
				kanban_boards.forEach(board => {
					let route = `/app/${finergy.router.slug(board.reference_doctype)}/view/kanban/${board.name}`;
	...
	function get_kanban_boards() {
		return finergy.call('finergy.desk.doctype.kanban_board.kanban_board.get_kanban_boards', { doctype })
			.then(r => r.message);
	}
};


finergy.views.KanbanView.show_kanban_dialog = function (doctype, show_existing) {
	let dialog = null;

	finergy.views.KanbanView.get_kanbans(doctype).then(kanbans => {
	...
	function make_kanban_board(board_name, field_name, project) {
		return finergy.call({
			method: 'finergy.desk.doctype.kanban_board.kanban_board.quick_kanban_board',
			args: {
				doctype,
				board_name,
				field_name,
			...
			callback: function (r) {
				var kb = r.message;
				if (kb.filters) {
					finergy.provide('finergy.kanban_filters');
					finergy.kanban_filters[kb.kanban_board_name] = kb.filters;
				}
				finergy.set_route('List', doctype, 'Kanban', kb.kanban_board_name);
			}
		});
	}
	...
	function new_kanban_dialog(kanbans, show_existing) {
		...
		*/
		...
		const select_fields = finergy.get_meta(doctype).fields.filter(df => {
			...
			if (to_save) {
				...
				if (!values.selected_kanban || values.selected_kanban == 'Create New Board') {
					make_kanban_board(values.board_name, values.field_name, values.project).then(
						() => dialog.hide(),
						(err) => finergy.msgprint(err)
					);
				} else {
					finergy.set_route(kanbans.find(kanban => kanban.name == values.selected_kanban).route);
				}
			} else {
				finergy.set_route("Form", "Customize Form", {"doc_type": doctype});
			}
		};
