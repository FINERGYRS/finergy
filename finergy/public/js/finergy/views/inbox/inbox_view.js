/**
 * finergy.views.InboxView
 */

finergy.provide("finergy.views");

finergy.views.InboxView = class InboxView extends finergy.views.ListView {
	static load_last_view() {
		const route = finergy.get_route();
		if (!route[3] && finergy.boot.email_accounts.length) {
			let email_account;
			if (finergy.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = finergy.boot.email_accounts[0].email_account;
			}
			finergy.set_route("List", "Communication", "Inbox", email_account);
			return true;
		} else if (!route[3] || (route[3] !== "All Accounts" && !is_valid(route[3]))) {
			finergy.throw(__('No email account associated with the User. Please add an account under User > Email Inbox.'));
		}
		return false;

		function is_valid(email_account) {
			return finergy.boot.email_accounts.find(d => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = finergy.get_route()[3];
	...
	get_seen_class(doc) {
		const seen =
			Boolean(doc.seen) || JSON.parse(doc._seen || '[]').includes(finergy.session.user)
				? ''
				: 'bold';
		return seen;
	}
	...
	get_meta_html(email) {
		...
		if (email.reference_doctype && email.reference_doctype !== this.doctype) {
			link = `<a class="text-muted grey"
				href="${finergy.utils.get_form_link(email.reference_doctype, email.reference_name)}"
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			filters = default_filters.concat([
				...
				["Communication", "email_account", "in", finergy.boot.all_accounts, true]
			]);
		} else {
			var op = "=";
			if (email_account == "All Accounts") {
				op = "in";
				email_account = finergy.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		} else if (!email_account && !finergy.boot.email_accounts.length) {
		...
		const html = finergy.model.can_create(args.doctype) ?
			`<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !finergy.boot.email_accounts.length) {
			finergy.route_options = {
				'email_id': finergy.session.user_email
			};
			finergy.new_doc('Email Account');
		} else {
