...
finergy.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[finergy.breadcrumbs.current_page()] = obj;
		this.update();
	},

	current_page() {
		return finergy.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[finergy.breadcrumbs.current_page()];
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (breadcrumbs.workspace) {
			if (!breadcrumbs.module_info.blocked && finergy.visible_modules.includes(breadcrumbs.module_info.module)) {
				$(`<li><a href="/app/${finergy.router.slug(breadcrumbs.workspace)}">${__(breadcrumbs.workspace)}</a></li>`)
	...
	set_workspace(breadcrumbs) {
		...
		if (breadcrumbs.module) {
			...
			breadcrumbs.module_info = finergy.get_module(breadcrumbs.module);

			// set workspace
			if (breadcrumbs.module_info && finergy.boot.module_page_map[breadcrumbs.module]) {
				breadcrumbs.workspace = finergy.boot.module_page_map[breadcrumbs.module];
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = finergy.get_doc('DocType', doctype);
		if ((doctype==="User" && !finergy.user.has_role('System Manager'))
			|| (doctype_meta && doctype_meta.issingle)) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = finergy.router.slug(finergy.router.doctype_layout || doctype);
			if (finergy.boot.treeviews.indexOf(doctype) !== -1) {
				let view = finergy.model.user_settings[doctype].last_view || 'Tree';
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		const docname = finergy.get_route().slice(2).join("/");
		let form_route = `/app/${finergy.router.slug(doctype)}/${docname}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				finergy.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = finergy.get_route()[1];
		let dashboard_route = `/app/${finergy.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`)
			.appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!finergy.visible_modules) {
			finergy.visible_modules = $.map(finergy.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete finergy.breadcrumbs.all[old_route_str];
		this.update();
	},
