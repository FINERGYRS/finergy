...
finergy.views.DashboardView = class DashboardView extends finergy.views.ListView {
	get view_name() {
		return 'Dashboard';
	}
	...
	setup_defaults() {
		return super.setup_defaults()
			.then(() => {
				...
				this.dashboard_settings = finergy.get_user_settings(this.doctype)['dashboard_settings'] || null;
			});
	}
	...
	fetch_dashboard_items(doctype, filters, obj_name) {
		return finergy.db.get_list(doctype, {
			filters: filters,
			fields: ['*']
		}).then(items => {
			this[obj_name] = items;
	...
	render_empty_state() {
		...
		const empty_state_image = '/assets/finergyrs/images/ui-states/list-empty-state.svg';
	...
	save_dashboard_customization() {
		...
		finergy.model.user_settings.save(this.doctype, 'dashboard_settings', this.dashboard_settings);
		this.make_dashboard();
	}
	...
	reset_dashboard_customization() {
		finergy.confirm(__("Are you sure you want to reset all customizations?"), () => {
		...
		const dialog = new finergy.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					get_query: () => {
						return {
							'query': 'finergy.desk.doctype.dashboard_chart.dashboard_chart.get_charts_for_user',
							filters: {
								document_type: this.doctype,
							}
						};
			...
			primary_action: (values) => {
				...
				if (chart.new_or_existing == 'New Chart') {
					...
					chart.filters_json = '[]';
					finergy.xcall('finergy.desk.doctype.dashboard_chart.dashboard_chart.create_dashboard_chart', {'args': chart}).then((doc)=> {
	...
	get_field_options() {
		...
		finergy.get_meta(this.doctype).fields.map(df => {
