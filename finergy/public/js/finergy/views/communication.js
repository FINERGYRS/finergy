...
finergy.last_edited_communication = {};
const separator_element = '<div>---</div>';

finergy.views.CommunicationComposer = class {
	constructor(opts) {
		$.extend(this, opts);
		if (!this.doc) {
	...
	get_fields() {
		const fields = [
			...
			},
			{
				...
				fieldname: "content",
				onchange: finergy.utils.debounce(
			...
			{ fieldtype: "Section Break" },
			{
				...
				default: finergy.boot.user.send_me_a_copy
			},
			{
		...
		const email_accounts = finergy.boot.email_accounts.filter(account => {
			return (
				!in_list(
	...
	toggle_more_options(show_options) {
		...
		const label = finergy.utils.icon(show_options ? 'up-line': 'down');
	...
	setup_multiselect_queries() {
		['recipients', 'cc', 'bcc'].forEach(field => {
			this.dialog.fields_dict[field].get_data = () => {
				...
				finergy.call({
					method: "finergy.email.get_contact_list",
					args: {txt},
					callback: (r) => {
	...
	setup_email_template() {
		...
		this.dialog.fields_dict["email_template"].df.onchange = () => {
			...
			finergy.call({
				method: 'finergy.email.doctype.email_template.email_template.get_email_template',
				args: {
	...
	get_last_edited_communication(clear) {
		if (!finergy.last_edited_communication[this.doctype]) {
			finergy.last_edited_communication[this.doctype] = {};
		}

		if (clear || !finergy.last_edited_communication[this.doctype][this.key]) {
			finergy.last_edited_communication[this.doctype][this.key] = {};
		}

		return finergy.last_edited_communication[this.doctype][this.key];
	}
	...
	async set_values() {
		...
		const subject = finergy.utils.html2text(this.subject) || '';
	...
	setup_print_language() {
		...
		$(fields.language_sel.input)
			.empty()
			.add_options(finergy.get_languages());
	...
	setup_print() {
		...
		if (this.frm) {
			const print_formats = finergy.meta.get_print_formats(this.frm.meta.name);
	...
	setup_attach() {
		...
		$(`
			...
			<p class='add-more-attachments'>
				<button class='btn btn-xs btn-default'>
					${finergy.utils.icon('small-add', 'xs')}&nbsp;
					${__("Add Attachment")}
				</button>
			</p>
		...
		attach
			...
			.on('click', () => new finergy.ui.FileUploader(args));
			...
			if (files.length) {
				$.each(files, (i, f) => {
					...
					if (!attachment_rows.find(`[data-file-name="${f.name}"]`).length) {
						f.file_url = finergy.urllib.get_full_url(f.file_url);
	...
	setup_email() {
		...
		$(fields.send_me_a_copy.input).on('click', () => {
			...
			finergy.db.set_value('User', finergy.session.user, 'send_me_a_copy', val);
			finergy.boot.user.send_me_a_copy = val;
		});
	...
	send_email(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if (!form_values.recipients) {
			finergy.msgprint(__("Enter Email Recipient(s)"));
			return;
		}
		...
		if (this.frm && !finergy.model.can_email(this.doc.doctype, this.frm)) {
			finergy.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}


		return finergy.call({
			method:"finergy.core.doctype.communication.email.make",
			args: {
			...
			callback(r) {
				if (!r.exc) {
					finergy.utils.play_sound("email");

					if (r.message["emails_not_sent_to"]) {
						finergy.msgprint(__("Email not sent to {0} (unsubscribed / disabled)",
							[ finergy.utils.escape_html(r.message["emails_not_sent_to"]) ]) );
					}
					...
					finergy.msgprint(__("There were errors while sending email. Please try again."));
	...
	is_print_letterhead_checked() {
		if (this.frm && $(this.frm.wrapper).find('.form-print-wrapper').is(':visible')){
			...
			return (finergy.model.get_doc(":Print Settings", "Print Settings") ||
	...
	async get_signature(sender_email) {
		let signature = finergy.boot.user.email_signature;
		...
		if (!signature) {
			...
			const email_accounts = await finergy.db.get_list("Email Account", {
				filters: filters,
		...
		if (!finergy.utils.is_html(signature)) {
	...
	get_earlier_reply() {
		...
		const communication_date = finergy.datetime.global_date_format(
