finergy.standard_pages['Workspaces'] = function() {
	var wrapper = finergy.container.add_page('Workspaces');
	...
	finergy.workspace = new finergy.views.Workspace(wrapper);
	$(wrapper).bind('show', function () {
		finergy.workspace.show();
	});
};

finergy.views.Workspace = class Workspace {
	constructor(wrapper) {
	...
	get_page_to_show() {
		...
		} else if (finergy.boot.allowed_workspaces) {
			default_page = finergy.boot.allowed_workspaces[0].name;
		} else {
			default_page = "Build";
		}

		let page = finergy.get_route()[1] || default_page;
		return page;
	}
	...
	build_sidebar_section(title, items) {
		...
		const get_sidebar_item = function (item) {
			return $(`
				<a
					href="/app/${finergy.router.slug(item.name)}"
					class="desk-sidebar-item standard-sidebar-item ${item.selected ? "selected" : ""}"
				>
					<span>${finergy.utils.icon(item.icon || "folder-normal", "md")}</span>
		...
		if (this.current_page && this.current_page.allow_customization) {
			...
			this.page.set_secondary_action(
				...
				() => {
					...
					finergy.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
					this.page.clear_primary_action();
					this.page.clear_secondary_action();
					this.setup_dropdown();
				}
...
class DesktopPage {
	...
	make() {
		...
		this.page.append(finergy.render_template('workspace_loading_skeleton'));
		this.page.appendTo(this.container);

		this.get_data().then(() => {
			if (Object.keys(this.data).length == 0) {
				delete localStorage.current_workspace;
				finergy.set_route("workspace");
				return;
			}
			this.refresh();
	...
	get_data() {
		return finergy.xcall("finergy.desk.desktop.get_desktop_page", {
			page: this.page_name
		}).then(data => {
			this.data = data;
			if (Object.keys(this.data).length == 0) return;

			return finergy.dashboard_utils.get_dashboard_settings().then(settings => {
	...
	save_customization() {
		...
		finergy.call('finergy.desk.desktop.save_customization', {
			page: this.page_name,
			config: config
		}).then(res => {
			finergy.dom.unfreeze();
			if (res.message) {
				finergy.show_alert({ message: __("Customizations Saved Successfully"), indicator: "green" });
				this.reload();
			} else {
				finergy.throw({ message: __("Something went wrong while saving customizations"), indicator: "red" });
				this.reload();
			}
		});
	}
	...
	reset_customization() {
		finergy.call('finergy.desk.desktop.reset_customization', {
			page: this.page_name
		}).then(() => {
			finergy.show_alert({ message: __("Removed page customizations"), indicator: "green" });
			this.reload();
		});
	}

	make_onboarding() {
		this.onboarding_widget = finergy.widget.make_widget({
	...
	make_charts() {
		this.sections["charts"] = new finergy.widget.WidgetGroup({
			container: this.page,
			type: "chart",
			columns: 1,
	...
	make_shortcuts() {
		this.sections["shortcuts"] = new finergy.widget.WidgetGroup({
	...
	make_cards() {
		let cards = new finergy.widget.WidgetGroup({
