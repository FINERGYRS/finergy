...
finergy.provide("finergy.views.calendar");
finergy.provide("finergy.views.calendars");

finergy.views.CalendarView = class CalendarView extends finergy.views.ListView {
	static load_last_view() {
		const route = finergy.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = finergy.get_user_settings(doctype)['Calendar'] || {};
	...
	setup_defaults() {
		return super.setup_defaults()
			.then(() => {
				...
				this.calendar_settings = finergy.views.calendar[this.doctype] || {};
				this.calendar_name = finergy.get_route()[3];
			});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then(options => {
				this.calendar = new finergy.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise(resolve => {
			if (calendar_name === 'default') {
				Object.assign(options, finergy.views.calendar[this.doctype]);
				resolve(options);
			} else {
				finergy.model.with_doc('Calendar View', calendar_name, () => {
					const doc = finergy.get_doc('Calendar View', calendar_name);
					if (!doc) {
						finergy.show_alert(__("{0} is not a valid Calendar. Redirecting to default Calendar.", [calendar_name.bold()]));
						finergy.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
	...
	get required_libs() {
		let assets = [
			'assets/finergyrs/js/lib/fullcalendar/fullcalendar.min.css',
			'assets/finergyrs/js/lib/fullcalendar/fullcalendar.min.js',
		];
		let user_language = finergy.boot.user.language;
		if (user_language && user_language !== 'en') {
			assets.push('assets/finergyrs/js/lib/fullcalendar/locale-all.js');
		}
		return assets;
	}
};

finergy.views.Calendar = Class.extend({
	...
	make_page: function() {
		...
		$.each(finergy.boot.calendars, function(i, doctype) {
			if(finergy.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function() {
					finergy.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make: function() {
		...
		this.footnote_area = finergy.utils.set_footnote(this.footnote_area, this.$wrapper,
	...
	set_css: function() {
		...
		this.$wrapper.find('.fc-prev-button span')
			.attr('class', '').html(finergy.utils.icon('left'));
		this.$wrapper.find('.fc-next-button span')
			.attr('class', '').html(finergy.utils.icon('right'));

		this.$wrapper.find('.fc-today-button')
			.prepend(finergy.utils.icon('today'));
	...
	get_system_datetime: function(date) {
		date._offset = (moment(date).tz(finergy.sys_defaults.time_zone)._offset);
		return finergy.datetime.convert_to_system_tz(date);
	},
	setup_options: function(defaults) {
		var me = this;
		defaults.meridiem = 'false';
		this.cal_options = {
			locale: finergy.boot.user.language || "en",
			header: {
			...
			events: function(start, end, timezone, callback) {
				return finergy.call({
					method: me.get_events_method || "finergy.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function(event) {
				...
				if(finergy.model.can_read(doctype)) {
					finergy.set_route("Form", doctype, event.name);
				}
			},
			...
			select: function(startDate, endDate, jsEvent, view) {
				...
				var event = finergy.model.get_new_doc(me.doctype);
				...
				finergy.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events: function(events) {
		...
		return (events || []).map(d => {
			d.id = d.name;
			d.editable = finergy.model.can_write(d.doctype || me.doctype);
			...
			// convert to user tz
			d.start = finergy.datetime.convert_to_user_tz(d.start);
			d.end = finergy.datetime.convert_to_user_tz(d.end);

			// show event on single day if start or end date is invalid
			if (!finergy.datetime.validate(d.start) && d.end) {
				d.start = finergy.datetime.add_days(d.end, -1);
			}

			if (d.start && !finergy.datetime.validate(d.end)) {
				d.end = finergy.datetime.add_days(d.start, 1);
			}

			me.fix_end_date_for_event_render(d);
			me.prepare_colors(d);

			d.title = finergy.utils.html2text(d.title);
	...
	prepare_colors: function(d) {
		...
		if(this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = finergy.ui.color.validate_hex(color_name) ?
					color_name : 'blue';
			}

			d.backgroundColor = finergy.ui.color.get(color_name, 'extra-light');
			d.textColor = finergy.ui.color.get(color_name, 'dark');
		} else {
			color = d.color;
			if (!finergy.ui.color.validate_hex(color) || !color) {
				color = finergy.ui.color.get('blue', 'extra-light');
			}
			d.backgroundColor = color;
			d.textColor = finergy.ui.color.get_contrast_color(color);
		}
		return d;
	},
	update_event: function(event, revertFunc) {
		var me = this;
		finergy.model.remove_from_locals(me.doctype, event.name);
		return finergy.call({
			method: me.update_event_method || "finergy.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function(r) {
				if(r.exc) {
					finergy.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function() {
