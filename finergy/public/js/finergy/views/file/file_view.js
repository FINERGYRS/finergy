...
finergy.views.FileView = class FileView extends finergy.views.ListView {
	static load_last_view() {
		const route = finergy.get_route();
		if (route.length === 2) {
			const view_user_settings = finergy.get_user_settings("File", "File");
			finergy.set_route(
				"List",
				"File",
				view_user_settings.last_folder || finergy.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			finergy.views.FileView.grid_view = !finergy.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (finergy.views.FileView.grid_view) {
			Object.assign(args, {
		...
		const items = [
			{
				label: __("Home"),
				action: () => {
					finergy.set_route("List", "File", "Home");
				}
			},
			{
				label: __("New Folder"),
				action: () => {
					finergy.prompt(
						...
						values => {
							...
							finergy.call({
								method:
									"finergy.core.doctype.file.file.create_new_folder",
								args: data
							});
						},
			...
			},
			{
				...
				action: () => {
					new finergy.ui.FileUploader({
						...
						on_success: file => {
							finergy.show_alert(__("Unzipping files..."));
							finergy
								.call(
									"finergy.core.doctype.file.file.unzip_file",
									{
										name: file.name
									}
								)
	...
	add_file_action_buttons() {
		...
		this.$paste_btn = this.page
			.add_button(__("Paste"), () =>
				finergy.file_manager.paste(this.current_folder)
			)
			.hide();
	}
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter(
				df => finergy.model.is_value_type(df.fieldtype) && !df.hidden
			)
			.map(df => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (finergy.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
	...
	before_render() {
		...
		finergy.model.user_settings.save(
			"File",
			"grid_view",
			finergy.views.FileView.grid_view
		);
	...
	get_breadcrumbs_html() {
		const route = finergy.router.parse();
	...
	get_header_html() {
		...
		let header_selector_html = !finergy.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
				"Select All"
			)}">`
			: "";

		let header_columns_html = !finergy.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === finergy.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = finergy.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size
			? finergy.form.formatters.FileSize(file.file_size)
			: "";
		...
		this.$result.on("drop", e => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					finergy.file_manager.paste(folder_name);
					finergy.show_alert(
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!finergy.file_manager.can_paste ||
			finergy.file_manager.old_folder === this.current_folder;
...
finergy.views.FileView.grid_view =
	finergy.get_user_settings("File").grid_view || false;
