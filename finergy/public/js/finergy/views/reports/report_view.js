/**
 * finergy.views.ReportView
 */
import DataTable from 'finergy-datatable';

finergy.provide('finergy.views');

finergy.views.ReportView = class ReportView extends finergy.views.ListView {
	get view_name() {
		return 'Report';
	}
	...
	setup_sort_selector() {
		this.sort_selector = new finergy.ui.SortSelector({
		...
		this.group_by_control = new finergy.ui.GroupBy(this);
	...
	before_refresh() {
		if (this.report_doc) {
			// don't parse finergy.route_options if this is a Custom Report
	...
	set_dirty_state_for_custom_report() {
		...
		if (!finergy.utils.deep_equal(current_settings, report_settings)) {
	...
	save_report_settings() {
		finergy.model.user_settings.save(this.doctype, 'last_view', this.view_name);
	...
	prepare_data(r) {
		let data = r.message || {};
		data = finergy.utils.dict(data.keys, data.values);
	...
	on_update(data) {
		if (this.doctype === data.doctype && data.name) {
			...
			const flash_row = data.user !== finergy.session.user;
			if (this.data.find(d => d.name === data.name)) {
				// update existing
				finergy.db.get_doc(data.doctype, data.name)
					...
					const d = new finergy.ui.Dialog({
						...
						primary_action: ({ column, insert_before }) => {
							if (!columns_in_picker.map(col => col.value).includes(column)) {
								finergy.show_alert({message: __('Invalid column'), indicator: 'orange'});
								d.hide();
								return;
							}
	...
	setup_charts() {
		...
		for (let col of this.columns) {
			...
			// numeric values in y
			if (col.docfield && finergy.model.numeric_fieldtypes.includes(col.docfield.fieldtype)) {
				y_fields.push({
					label: col.content,
					fieldname: col.id,
					value: col.id
			...
			fields: [
				...
				},
				{
					...
					default: defaults.chart_type ?
						finergy.utils.to_title_case(defaults.chart_type) :
						'Bar'
				}
			],
			primary_action: (data) => {
	...
	make_chart() {
		...
		this.chart = new finergy.Chart(this.$charts_wrapper.find('.charts-inner-wrapper')[0], {
			...
			tooltipOptions: {
				formatTooltipY: value => finergy.format(value, get_df(this.chart_args.y_axes[0]), { always_show_decimals: true, inline: true }, get_doc(value.doc))
			}
		});
	}
	...
	set_control_value(doctype, docname, fieldname, value) {
		...
		return new Promise((resolve, reject) => {
			finergy.db.set_value(doctype, docname, {[fieldname]: value})
				.then(r => {
					if (r.message) {
						resolve(r.message);
					} else {
	...
	render_editing_input(colIndex, value, parent) {
		...
		if (col.docfield.fieldtype === 'Text Editor') {
			...
			// make control
			control = finergy.ui.form.make_control({
				df: col.docfield,
				parent: parent,
				render_input: true
			});
	...
	is_editable(df, data) {
		if (df
			&& finergy.model.can_write(this.doctype)
			...
			&& !finergy.model.std_fields_list.includes(df.fieldname))
			return true;
		return false;
	}
	...
	set_default_fields() {
		...
		const fields = this.meta.fields.filter(df => {
			return (df.in_list_view || df.in_standard_filter)
				&& finergy.perm.has_perm(this.doctype, df.permlevel, 'read')
				&& finergy.model.is_value_type(df.fieldtype)
				&& !df.report_hide;
		});
	...
	get_fields() {
		let fields = this.fields.map(f => {
			let column_name = finergy.model.get_full_column_name(f[0], f[1]);
			if (f[1] !== this.doctype) {
				// child table field
		...
		const cdt_name_fields =
			this.get_unique_cdt_in_view()
				.map(cdt => finergy.model.get_full_column_name('name', cdt) + ' as ' + `'${cdt}:name'`);
	...
	add_currency_column(fieldname, doctype, col_index) {
		...
		const df = finergy.meta.get_docfield(doctype, fieldname);
		if (df && df.fieldtype === 'Currency' && df.options &&
			!df.options.includes(':') && finergy.meta.has_field(doctype, df.options)
		) {
			...
			const field_label = finergy.meta.get_label(doctype, field[0]);
			finergy.show_alert(
	...
	remove_column_from_datatable(column) {
		...
		if (field[0] === 'name') {
			this.refresh();
			finergy.throw(__('Cannot remove ID field'));
		}
	...
	get_columns_for_picker() {
		...
		const standard_fields_filter = df => !in_list(finergy.model.no_value_type, df.fieldtype);

		let doctype_fields = finergy.meta.get_docfields(this.doctype).filter(standard_fields_filter);

		// filter out docstatus field from picker
		let std_fields = finergy.model.std_fields.filter( df => df.fieldname !== 'docstatus');

		// add status field derived from docstatus, if status is not a standard field
		let has_status_values = false;

		if (this.data) {
			has_status_values = finergy.get_indicator(this.data[0], this.doctype);
		}

		if (!finergy.meta.has_field(this.doctype, 'status') && has_status_values) {
			doctype_fields = [{
				label: __('Status'),
				fieldname: 'docstatus',
				fieldtype: 'Data'
		...
		const table_fields = finergy.meta.get_table_fields(this.doctype);

		table_fields.forEach(df => {
			const cdt = df.options;
			const child_table_fields = finergy.meta.get_docfields(cdt).filter(standard_fields_filter);
	...
	setup_columns() {
		...
		for (let f of this.fields) {
			...
			if (f[0]!=='docstatus') {
				...
				if (!this.fields.includes(['status', this.doctype]) && !finergy.meta.has_field(this.doctype, 'status')) {
	...
	build_column(c) {
		...
		let docfield = finergy.meta.docfield_map[doctype || this.doctype][fieldname];
		...
		if (!docfield) {
			docfield = finergy.model.get_std_field(fieldname, true);
			...
			if (docfield) {
				...
				if (fieldname == 'docstatus' && !finergy.meta.has_field(this.doctype, 'status')) {
		...
		const editable = finergy.model.is_non_std_field(fieldname) && !docfield.read_only;

		const align = (() => {
			const is_numeric = finergy.model.is_numeric_field(docfield);
			if (is_numeric) {
				return 'right';
			}
		...
		return {
			...
			format: (value, row, column, data) => {
				...
				return finergy.format(value, column.docfield, { always_show_decimals: true }, doc);
			}
		};
	}
		...
		if (this.add_totals_row) {
			...
			const totals_row = this.columns.map((col, i) => {
				return {
					...
					format: value => {
						let formatted_value = finergy.format(value, col.docfield, {
	...
	format_total_cell(formatted_value, df) {
		...
		if (finergy.model.is_numeric_field(df.docfield)) {
		...
		return this.columns.map(col => {
			if (col.docfield.parent !== this.doctype) {
				...
				return {
					...
					format: value => {
						return finergy.format(value, col.docfield, { always_show_decimals: true }, d);
					}
				};
			}
			if (col.field === 'docstatus' && !finergy.meta.has_field(this.doctype, 'status')) {
				// get status from docstatus
				let status = finergy.get_indicator(d, this.doctype);
				if (status) {
	...
	save_report(save_type) {
		const _save_report = (name) => {
			...
			return finergy.call({
				method: 'finergy.desk.reportview.save_report',
				args: {
					name: name,
					doctype: this.doctype,
				...
				callback:(r) => {
					if(r.exc) {
						finergy.msgprint(__("Report was not saved (there were errors)"));
						return;
					}
					if(r.message != this.report_name) {
						// Rerender the reports dropdown,
						// so that this report is included in the dropdown as well.
						finergy.boot.user.all_reports[r.message] = {
							ref_doctype: this.doctype,
							report_type: "Report Builder",
							title: r.message,
						};

						finergy.set_route('List', this.doctype, 'Report', r.message);
						return;
					}
		...
		if(this.report_name && save_type == "save") {
			...
			finergy.prompt({fieldname: 'name', label: __('New Report name'), reqd: 1, fieldtype: 'Data'}, (data) => {
	...
	delete_report() {
		return finergy.call({
			method: 'finergy.desk.reportview.delete_report',
	...
	get_report_doc() {
		return new Promise(resolve => {
			finergy.model.with_doc('Report', this.report_name, () => {
				resolve(finergy.get_doc('Report', this.report_name));
			});
		});
	}
	...
	get_filters_html_for_print() {
		...
		return filters.map(f => {
			...
			const label = finergy.meta.get_label(doctype, fieldname);
	...
	get_columns_totals(data) {
		...
		this.columns.forEach((col, i) => {
			const totals = data.reduce((totals, d) => {
				if (col.id in d && finergy.model.is_numeric_field(col.docfield)) {
					totals += flt(d[col.id]);
					return totals;
				}
			}, 0);
	...
	report_menu_items() {
		let items = [
			...
			},
			{
				...
				action: () => {
					...
					finergy.utils.setup_search(d.$body, '.unit-checkbox', '.label-area');
					d.show();
				}
			}
		];

		if (finergy.model.can_export(this.doctype)) {
			items.push({
				label: __('Export'),
				action: () => {
					...
					const d = new finergy.ui.Dialog({
						...
						primary_action: (data) => {
							args.cmd = 'finergy.desk.reportview.export_query';
		...
		items.push({
			...
			action: () => {
				if(this.report_name) {
					finergy.set_route('List', 'Auto Email Report', {'report' : this.report_name});
				} else {
					finergy.msgprint(__('Please save the report first'));
				}
			}
		});
		...
		const can_edit_or_delete = (action) => {
			...
			return (
				...
				&& (
					finergy.model[method]("Report")
					|| this.report_doc.owner === finergy.session.user
				)
			);
		};
		...
		// user permissions
		if(this.report_name && finergy.model.can_set_user_permissions("Report")) {
			items.push({
				...
				action: () => {
					...
					finergy.set_route('List', 'User Permission', args);
				}
			});
		}
