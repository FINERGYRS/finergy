finergy.provide('finergy.report_utils');

finergy.report_utils = {
	make_chart_options: function(columns, raw_data, { y_fields, x_field, chart_type, colors, height }) {
		...
		let datasets = y_fields.map(y_field => ({
			name: finergy.model.unscrub(y_field),
		...
		function get_options(fields) {
			return fields.map((field) => {
				if (field.fieldname) {
					...
					field = finergy.report_utils.prepare_field_from_column(field);
					return {label: field.label, value: field.fieldname};
				}
			});
		}
	...
	get_report_filters: function(report_name) {
		...
		if (finergy.query_reports[report_name]) {
			let filters = finergy.query_reports[report_name].filters;
			return Promise.resolve(filters);
		}

		return finergy.xcall(
			'finergy.desk.query_report.get_script',
			{
				report_name: report_name
			}
		).then(r => {
			finergy.dom.eval(r.script || '');
			return finergy.after_ajax(() => {
				return finergy.query_reports[report_name] && finergy.query_reports[report_name].filters;
			})
		});
	},
