...
import DataTable from 'finergy-datatable';

finergy.provide('finergy.widget.utils');
finergy.provide('finergy.views');
finergy.provide('finergy.query_reports');

finergy.standard_pages['query-report'] = function() {
	var wrapper = finergy.container.add_page('query-report');
	...
	finergy.query_report = new finergy.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind('show', function() {
		finergy.query_report.show();
	});
};

finergy.views.QueryReport = class QueryReport extends finergy.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = finergy.run_serially(tasks);
		return this.init_promise;
	}
	...
	setup_defaults() {
		...
		this.refresh = finergy.utils.throttle(this.refresh, 300);
	...
	get_no_result_message() {
		return `<div class="msg-box no-border">
			<div>
				<img src="/assets/finergyrs/images/ui-states/list-empty-state.svg" alt="Generic Empty State" class="null-state">
			</div>
	...
	setup_events() {
		finergy.realtime.on("report_generated", (data) => {
			...
			if(data.report_name) {
				...
				if(data.name == this.prepared_report_doc_name) {
					...
					finergy.show_alert({message: alert_message, indicator: 'orange'});
				}
			}
		});
	...
	load() {
		if (finergy.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, finergy.route_options);

		if (this.report_name !== finergy.get_route()[1]) {
			// different report
			this.load_report(route_options);
		}
		else if (finergy.has_route_options()) {
	...
	add_card_to_dashboard() {
		let field_options = finergy.report_utils.get_field_options_from_report(this.columns, this.raw_data);
		const dashboard_field = finergy.dashboard_utils.get_dashboard_link_field();
		const set_standard = finergy.boot.developer_mode;
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			finergy.msgprint(__('Please Set Chart'));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			'finergy.desk.doctype.number_card.number_card.create_report_number_card',
			args,
			dashboard_name,
			card_name,
			'Number Card'
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			'chart_type': 'Report',
			'report_name': this.report_name,
			'type': chart_args.chart_type || finergy.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			'finergy.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart',
			args,
			dashboard_name,
			chart_name,
			'Dashboard Chart'
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		finergy.xcall(
			...
			finergy.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	get_report_doc() {
		return finergy.model.with_doc('Report', this.report_name)
			.then(doc => {
				this.report_doc = doc;
			})
			.then(() => finergy.model.with_doctype(this.report_doc.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (finergy.query_reports[this.report_name]) {
				this.report_settings = finergy.query_reports[this.report_name];
				resolve();
			} else {
				finergy.xcall('finergy.desk.query_report.get_script', {
					report_name: this.report_name
				}).then(settings => {
					finergy.dom.eval(settings.script || '');
					finergy.after_ajax(() => {
						...
						finergy.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings() {
		let report_script_name = this.report_doc.report_type === 'Custom Report'
			...
			: this.report_name;
		return finergy.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function()  {
			seconds_elapsed += 1;
			finergy.show_progress(__('Preparing Report'), seconds_elapsed, execution_time);
		}, 1000);
	}
		...
		if (doc) {
			if (typeof expression === 'boolean') {
				...
				try {
					...
					finergy.throw(__('Invalid "depends_on" expression set in filter {0}', [filter_label]));
				}
			} else {
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = finergy.route_options;
		...
		if (route_options) {
			...
			return finergy.run_serially(promises);
		}
	}
	...
	refresh() {
		...
		return new Promise(resolve => {
			this.last_ajax = finergy.call({
				method: 'finergy.desk.query_report.run',
				type: 'GET',
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			if (data.result && data.result.length) {
				...
				if (this.chart_options) {
					...
					if (this.chart_fields) {
						this.chart_options =
							finergy.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			finergy.utils.build_summary_item(summary).appendTo(this.$summary);
		})
	...
	get_query_params() {
		const query_string = finergy.utils.get_query_string(finergy.get_route_str());
		const query_params = finergy.utils.get_query_params(query_string);
		return query_params;
	}
	...
	add_prepared_report_buttons(doc) {
			...
			this.page.add_inner_button(__("Download Report"), function (){
				window.open(
					finergy.urllib.get_full_url(
						"/api/method/finergy.core.doctype.prepared_report.prepared_report.download_attachment?"
						+"dn="+encodeURIComponent(doc.name)));
			});

			const part1 = __('This report was generated {0}.', [finergy.datetime.comment_when(doc.report_end_time)]);
		...
		this.primary_action_map = {
			...
			"Edit": {
				label: __("Edit"),
				click: () => {
					finergy.set_route(finergy.get_route());
				}
			},
			"Rebuild": {
				label:	__("Rebuild"),
	...
	show_warning_or_generate_report() {
		finergy.xcall(
			'finergy.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state',
			{
			...
			if (reports.length) {
				...
				this.prepared_report_dialog = finergy.warn(
				...
				finergy.utils.bind_actions_with_object(this.prepared_report_dialog.wrapper, this);
			} else {
				this.generate_background_report();
			}
		});
	...
	delete_old_queued_reports() {
		...
		finergy.xcall(
			'finergy.core.doctype.prepared_report.prepared_report.delete_prepared_reports',
			{
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			let filters = this.get_filter_values(true);
			return new Promise(resolve => finergy.call({
				method: 'finergy.desk.query_report.background_enqueue_run',
				type: 'GET',
				args: {
					report_name: this.report_name,
					filters: filters
				...
				finergy.show_alert({message: alert_message, indicator: 'orange'}, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (this.datatable && this.datatable.options
			...
			let datatable_options = {
				...
				data: data,
				inlineFilters: true,
				language: finergy.boot.lang,
				translations: finergy.utils.datatable.get_translations(),
				treeView: this.tree_report,
				layout: 'fixed',
				cellHeight: 33,
				showTotalRow: this.raw_data.add_total_row && !this.report_settings.tree,
				direction: finergy.utils.is_rtl() ? 'rtl' : 'ltr',
				hooks: {
					columnTotal: finergy.utils.report_column_total
				}
			};
	...
	get_chart_options(data) {
		...
		if (options.fieldtype) {
			options.tooltipOptions = {
				formatTooltipY: d => finergy.format(d, {
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new finergy.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = finergy.report_utils.make_chart_options(me.columns, me.raw_data, values);
				me.chart_fields = values;
				wrapper.empty();
				new finergy.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		return columns.map(column => {
			column = finergy.report_utils.prepare_field_from_column(column);
			...
			const format_cell = (value, row, column, data) => {
				...
				return finergy.format(value, column,
			...
			if (column.fieldtype === 'Date') {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = finergy.datetime.user_to_obj(keyword);
					const cellValue = finergy.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = finergy.get_meta(this.report_doc.ref_doctype);
		finergy.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log (method, file_format) {
		finergy.call("finergy.core.doctype.access_log.access_log.make_access_log",
			{
	...
	pdf_report(print_settings) {
		const base_url = finergy.urllib.get_base_url();
		const print_css = finergy.boot.print_css;
		const landscape = print_settings.orientation == 'Landscape';
		...
		const content = finergy.render_template(template, {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = finergy.render_template('print_template', {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: finergy.boot.lang,
			layout_direction: finergy.utils.is_rtl() ? "rtl" : "ltr"
		});

		finergy.render_pdf(html, print_settings);
	}
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map(fieldname => {
				const label = finergy.query_report.get_filter(fieldname).df.label;
	...
	export_report() {
		...
		this.export_dialog = finergy.prompt(export_dialog_fields, ({ file_format, include_indentation }) => {
			...
			if (file_format === 'CSV') {
				...
				finergy.tools.downloadify(out, null, this.report_name);
			} else {
				let filters = this.get_filter_values(true);
				if (finergy.urllib.get_dict("prepared_report_name")) {
					filters = Object.assign(finergy.urllib.get_dict("prepared_report_name"), filters);
				}
				...
				const args = {
					cmd: 'finergy.desk.query_report.export_query',
					report_name: this.report_name,
			...
			return row
				...
				.map((cell, i) => {
					if (cell.column.fieldtype === "Duration") {
						cell.content = finergy.utils.get_formatted_duration(cell.content);
					}
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __('Edit'),
				action: () => finergy.set_route('Form', 'Report', this.report_name),
				condition: () => finergy.user.is_report_manager(),
				standard: true
			},
			{
				label: __('Print'),
				action: () => {
					let dialog = finergy.ui.get_print_settings(
						false,
				...
				condition: () => finergy.model.can_print(this.report_doc.ref_doctype),
				standard: true
			},
			{
				label: __('PDF'),
			...
			},
			{
				...
				condition: () => finergy.model.can_export(this.report_doc.ref_doctype),
				standard: true
			},
			{
				label: __('Setup Auto Email'),
				action: () => finergy.set_route('List', 'Auto Email Report', {'report' : this.report_name}),
				standard: true
			},
			{
				label: __('Add Column'),
				action: () => {
					let d = new finergy.ui.Dialog({
						...
						fields: [
							{
								...
								change: () => {
									...
									finergy.model.with_doctype(doctype, () => {
										let options = finergy.meta.get_docfields(doctype)
											.filter(finergy.model.is_value_type)
						...
						primary_action: (values) => {
							...
							let df = finergy.meta.get_docfield(values.doctype, values.field);
							...
							finergy.call({
								method: 'finergy.desk.query_report.get_data_for_custom_field',
								args: {
									doctype: values.doctype,
									fieldname: fieldname,
									field: values.field
			...
			},
			{
				...
				action: () => finergy.set_route('List', 'User Permission', {
					doctype: 'Report',
					name: this.report_name
				}),
				condition: () => finergy.model.can_set_user_permissions('Report'),
				standard: true
			}
		];

		if (finergy.user.is_report_manager()) {
			items.push({
				label: __('Save'),
				action: () => {
					let d = new finergy.ui.Dialog({
						...
						primary_action: (values) => {
							finergy.call({
								method: "finergy.desk.query_report.save_report",
								args: {
								...
								callback: function(r) {
									this.show_save = false;
									d.hide();
									finergy.set_route('query-report', r.message);
								}
							});
						}
					});
	...
	get_fieldname_for_column(fieldname, doctype) {
		...
		if (existing_fieldnames.includes(fieldname)) {
			fieldname = finergy.model.scrub(fieldname + " " + doctype);
		}
...
Object.defineProperty(finergy, 'query_report_filters_by_name', {
	get() {
		console.warn('[Query Report] finergy.query_report_filters_by_name is deprecated. Please use the new api: finergy.query_report.get_filter_value(fieldname) and finergy.query_report.set_filter_value(fieldname, value)');
		return null;
	}
});
