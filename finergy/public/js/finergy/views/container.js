...
finergy.views.Container = Class.extend({
	...
	init: function() {
		...
		$(document).on("page-change", function() {
			...
			var route_str = finergy.get_route_str();
			$("body").attr("data-route", route_str);
			$("body").attr("data-sidebar", me.has_sidebar() ? 1 : 0);
		});

		$(document).bind('rename', function(event, dt, old_name, new_name) {
			finergy.breadcrumbs.rename(dt, old_name, new_name);
		});
	},
	...
	change_to: function(label) {
		...
		this.page._route = finergy.router.get_sub_path();
		$(this.page).trigger('show');
		!this.page.disable_scroll_to_top && finergy.utils.scroll_to(0);
	...
	has_sidebar: function() {
		var flag = 0;
		var route_str = finergy.get_route_str();
		// check in finergy.ui.pages
		flag = finergy.ui.pages[route_str] && !finergy.ui.pages[route_str].single_column;

		// sometimes finergy.ui.pages is updated later,
