...
finergy.provide("finergy.interaction_settings");

finergy.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		finergy.model.with_doctype('Event', () => {
			let categories = finergy.meta.get_docfield("Event", "event_category").options.split("\n");
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)){
			interaction_values["owner"] = finergy.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = finergy.session.user;
		}
		return finergy.call({
			...
			callback: function(r) {
				if(!r.exc) {
					...
					finergy.msgprint(__("There were errors while creating the document. Please try again."));
				}
			}
		});
	...
	add_attachments(doc, attachments) {
		finergy.call({
			method:'finergy.utils.file_manager.add_attachments',
			args: {
				doctype: doc.doctype,
				name: doc.name,
