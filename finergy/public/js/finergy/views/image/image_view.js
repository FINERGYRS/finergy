/**
 * finergy.views.ImageView
 */
finergy.provide("finergy.views");

finergy.views.ImageView = class ImageView extends finergy.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = finergy.utils.escape_html(title);
	...
	get_attached_images() {
		return finergy
			.call({
				method: "finergy.core.doctype.file.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new finergy.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
...
finergy.views.GalleryView = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		var me = this;
	...
	prepare: function() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = finergy.render_template("photoswipe_dom");
	...
	load_lib: function() {
		return new Promise(resolve => {
			var asset_dir = "assets/finergyrs/js/lib/photoswipe/";
			finergy.require(
				[
