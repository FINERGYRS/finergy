...
export default class ChartWidget extends Widget {
	...
	render_time_series_filters() {
		...
		finergy.dashboard_utils.render_chart_filters(
			filters,
			"chart-actions",
			this.action_area,
			0
	...
	render_date_range_field() {
		if (
			...
			this.date_range_field = finergy.ui.form.make_control({
				df: {
					fieldtype: 'DateRange',
					fieldname: 'from_date',
	...
	get_report_chart_data(result) {
		if (result.chart && this.chart_doc.use_report_chart) {
			...
			let columns = result.columns.map(col => {
				return finergy.report_utils.prepare_field_from_column(col);
			});

			let data = finergy.report_utils.make_chart_options(
				columns,
				result,
				chart_fields
			).data;
	...
	prepare_chart_actions() {
		...
		if (this.chart_doc.document_type) {
			actions.push({
				...
				action: "action-list",
				handler: () => {
					finergy.set_route("List", this.chart_doc.document_type);
				}
			});
	...
	setup_filter_dialog(fields) {
		...
		if (this.chart_doc.chart_type == 'Report') {
			...
			finergy.query_report = new finergy.views.QueryReport({'filters': dialog.fields_list});
			finergy.query_reports[this.chart_doc.report_name].onload
					&& finergy.query_reports[this.chart_doc.report_name].onload(finergy.query_report);
		}
	...
	save_chart_config_for_user(config, reset=0) {
		...
		finergy.xcall('finergy.desk.doctype.dashboard_settings.dashboard_settings.save_chart_config', {
			'reset': reset,
	...
	create_filter_group_and_add_filters() {
		this.filter_group = new finergy.ui.FilterGroup({
		...
		this.filters &&
			finergy.model.with_doctype(this.chart_doc.document_type, () => {
	...
	fetch(filters, refresh = false, args) {
		...
		return finergy.xcall(method, args);
	}
	...
	async get_source_doctype() {
		...
		if (this.chart_doc.chart_type == "Report" && this.chart_doc.report_name) {
			return await finergy.db.get_value("Report", this.chart_doc.report_name, "ref_doctype").then(r => r.message.ref_doctype);
		}
	}
	...
	async render() {
		let setup_dashboard_chart = () => {
			...
			if (!this.dashboard_chart) {
				this.dashboard_chart = finergy.utils.make_chart(this.chart_wrapper[0], chart_args);
			} else {
		...
		if (!this.data || !this.data.labels || !Object.keys(this.data).length) {
			...
			if (this.chart_doc.document_type) {
				finergy.model.with_doctype(this.chart_doc.document_type, setup_dashboard_chart);
			} else {
	...
	get_chart_args() {
		...
		if (this.chart_doc.document_type) {
			let doctype_meta = finergy.get_meta(this.chart_doc.document_type);
		...
		chart_args.tooltipOptions = {
			formatTooltipY: value => finergy.format(value, { fieldtype, options }, { always_show_decimals: true, inline: true })
		};
	...
	update_chart_object() {
		finergy.db.get_doc("Dashboard Chart", this.chart_doc.name).then(doc => {
			this.chart_doc = doc;
	...
	prepare_chart_object() {
		if (this.chart_doc.type == 'Heatmap' && !this.chart_doc.heatmap_year) {
			this.chart_doc.heatmap_year = finergy.dashboard_utils.get_year(finergy.datetime.now_date());
		}
	...
	set_chart_filters() {
		...
		let chart_saved_filters = finergy.dashboard_utils.get_all_filters(this.chart_doc);

		if (this.chart_doc.chart_type == 'Report') {
			return finergy.dashboard_utils
				.get_filters_for_chart_type(this.chart_doc).then(filters => {
					...
					this.filters =
					finergy.utils.parse_array(user_saved_filters) || finergy.utils.parse_array(this.filters)
						|| finergy.utils.parse_array(chart_saved_filters);
				});
		} else {
			this.filters =
				finergy.utils.parse_array(user_saved_filters) || finergy.utils.parse_array(this.filters)
			...
			.then(chart_doc => {
				if (chart_doc) {
					...
					if (this.chart_doc.chart_type == "Custom") {
						// custom source
						if (
							finergy.dashboards.chart_sources[this.chart_doc.source]
						) {
							this.settings =
								finergy.dashboards.chart_sources[
									this.chart_doc.source
								];
							return Promise.resolve();
						} else {
							const method =
								"finergy.desk.doctype.dashboard_chart_source.dashboard_chart_source.get_config";
							return finergy
								.xcall(method, { name: this.chart_doc.source })
								.then(config => {
									finergy.dom.eval(config);
									this.settings =
										finergy.dashboards.chart_sources[
											this.chart_doc.source
										];
						...
						this.settings = {
							method: "finergy.desk.doctype.dashboard_chart.dashboard_chart.get"
						};
						return Promise.resolve();
					}
				}
