class WidgetDialog {
		...
		this.dialog = new finergy.ui.Dialog({
			...
			primary_action: (data) => {
				...
				if (!this.editing && !data.name) {
					data.name = `${this.type}-${this.label}-${finergy.utils.get_random(20)}`;
				}
	...
	get_title() {
		...
		let label = action = action + " " + finergy.model.unscrub(this.type);
		return __(label);
	}
...
class ShortcutDialog extends WidgetDialog {
		...
		return [
			...
			},
			{
				...
				onchange: () => {
					if (this.dialog.get_value("type") == "DocType") {
						...
						if (doctype && finergy.boot.single_types.includes(doctype)) {
							this.hide_filters();
						} else if (doctype) {
						...
						if (finergy.boot.treeviews.includes(doctype)) views.push("Tree");
						if (finergy.boot.calendars.includes(doctype)) views.push("Calendar");
			...
			},
			{
				...
				depends_on: (state) => {
					if (this.dialog) {
						let doctype = this.dialog.get_value("link_to");
						let is_single = finergy.boot.single_types.includes(doctype);
	...
	process_data(data) {
		...
		data.label = data.label
			? data.label
			: finergy.model.unscrub(data.link_to);
...
class NumberCardDialog extends WidgetDialog {
	...
	get_fields() {
		...
		fields = [
			...
			},
			{
				...
				get_query: () => {
					return {
						'query': 'finergy.desk.doctype.number_card.number_card.get_cards_for_user',
						filters: {
	...
	set_aggregate_function_fields() {
		...
		if (this.document_type) {
			finergy.get_meta(this.document_type).fields.map(df => {
