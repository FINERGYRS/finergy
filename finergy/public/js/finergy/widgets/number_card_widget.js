...
export default class NumberCardWidget extends Widget {
	...
	make_card() {
		finergy.model.with_doc('Number Card', this.name).then(card => {
			if (!card) {
	...
	create_number_card() {
		...
		return finergy.xcall(
			'finergy.desk.doctype.number_card.number_card.create_number_card',
			{
				'args': this.card_doc
			}
		).then(doc => {
	...
	set_route() {
		...
		const route = finergy.utils.generate_route({
			name: name,
		...
		if (is_document_type) {
			...
			finergy.route_options = filters.reduce((acc, filter) => {
	...
	get_settings(type) {
		...
		const settings_map = {
			...
			'Document Type': {
				method: 'finergy.desk.doctype.number_card.number_card.get_result',
				args: {
					doc: this.card_doc,
					filters: this.filters,
				},
	...
	get_filters() {
		const filters = finergy.dashboard_utils.get_all_filters(this.card_doc);
		return filters;
	}
	...
	get_number() {
		return finergy.xcall(this.settings.method, this.settings.args).then(res => {
	...
	get_number_for_doctype_card(res) {
		...
		if (this.card_doc.function !== 'Count') {
			return finergy.model.with_doctype(this.card_doc.document_type, () => {
				const based_on_df =
					finergy.meta.get_docfield(this.card_doc.document_type, this.card_doc.aggregate_function_based_on);
	...
	get_number_for_report_card(res) {
		...
		this.number = finergy.report_utils.get_result_of_fn(this.card_doc.report_function, vals);
		this.set_formatted_number(col);
	}

	set_formatted_number(df) {
		const default_country = finergy.sys_defaults.country;
		const shortened_number = finergy.utils.shorten_number(this.number, default_country, 5);
		let number_parts = shortened_number.split(' ');

		const symbol = number_parts[1] || '';
		const formatted_number = $(finergy.format(number_parts[0], df)).text();
			...
			if (this.percentage_stat == 0 || this.percentage_stat == undefined) {
				...
				caret_html =
					`<span class="indicator-pill-round green">
						${finergy.utils.icon('arrow-up-right', 'xs')}
					</span>`;
				color_class = 'green-stat';
			} else {
				caret_html =
					`<span class="indicator-pill-round red">
						${finergy.utils.icon('arrow-down-left', 'xs')}
					</span>`;
	...
	get_percentage_stats() {
		return finergy.xcall('finergy.desk.doctype.number_card.number_card.get_percentage_difference', {
			doc: this.card_doc,
			filters: this.filters,
			result: this.number
		}).then(res => {
			if (res !== undefined) {
				this.percentage_stat = finergy.utils.shorten_number(res);
			}
		});
	}
