...
export default class OnboardingWidget extends Widget {
	...
	add_step(step, index) {
		...
		let $step = $(`<a class="onboarding-step ${status}">
				<div class="step-title">
					...
					<div class="step-index step-skipped">${finergy.utils.icon('tick', 'xs')}</div>
					<div class="step-index step-complete">${finergy.utils.icon('tick', 'xs')}</div>
	...
	show_step(step) {
		...
		let actions = {
			...
			"Create Entry": (step) => {
				if (step.is_complete) {
					finergy.set_route(`/app/List/${step.reference_document}`);
				} else {
		...
		const toggle_content = () => {
			...
			this.step_body.html(
				step.description ?
					finergy.markdown(step.description)
					: `<h1>${step.title}</h1>`
			);
	...
	go_to_page(step) {
		...
		finergy.set_route(step.path).then(() => {
			...
			let msg_dialog = finergy.msgprint({
				message: message,
				title: title,
				primary_action: {
					action: () => {
	...
	open_report(step) {
		let route = finergy.utils.generate_route({
		...
		let current_route = finergy.get_route();

		finergy.set_route(route).then(() => {
			let msg_dialog = finergy.msgprint({
				message: __(step.report_description),
				title: __(step.reference_report),
				primary_action: {
					action: () => {
						finergy.set_route(current_route).then(() => {
							this.mark_complete(step);
						});
						msg_dialog.hide();
					},
			...
			finergy.msg_dialog.custom_onhide = () => this.mark_complete(step);
		});
	}
	...
	show_form_tour(step) {
		let route;
		if (step.is_single) {
			route = finergy.router.slug(step.reference_document);
		} else {
			route = `${finergy.router.slug(step.reference_document)}/new`;
		}

		let current_route = finergy.get_route();

		finergy.route_hooks = {};
		finergy.route_hooks.after_load = (frm) => {
			frm.show_tour(() => {
				let msg_dialog = finergy.msgprint({
	...
	update_settings(step) {
		...
		finergy.route_hooks.after_save = (frm) => {
			let success = false;
			let args = {};
			...
			if (success) {
				...
				args.secondary_action = {
					action: () => finergy.set_route(current_route),
					label: __("Go Back"),
				};
			...
			finergy.msgprint(args);
			finergy.msg_dialog.custom_onhide = () => custom_onhide();
		};

		finergy.set_route("Form", step.reference_document);
	}
	...
	create_entry(step) {
		...
		if (step.is_submittable) {
			finergy.route_hooks.after_save = () => {
				finergy.msgprint({
					message: __("Submit this document to complete this step."),
					title: __("Great")
				});
			};
			finergy.route_hooks.after_submit = callback;
		} else {
			finergy.route_hooks.after_save = callback;
		}

		finergy.set_route('Form', step.reference_document, 'new');
	}

	show_quick_entry(step) {
		let current_route = finergy.get_route_str();
		finergy.ui.form.make_quick_entry(
			step.reference_document,
			() => {
				if (finergy.get_route_str() != current_route) {
					let success_dialog = finergy.msgprint({
						message: __("Let's take you back to onboarding"),
	...
	show_video(step) {
		finergy.help.show_video(step.video_url, step.title);
		this.mark_complete(step);
	}
	...
	update_step_status(step, status, value, callback) {
		...
		finergy
			.call("finergy.desk.desktop.update_onboarding_step", {
				name: step.name,
				field: status,
				value: value,
			})
	...
	show_success() {
		...
		let success_state_image =
			...
			"/assets/finergyrs/images/ui-states/success-color.png";
		let documentation = "";
		if (this.docs_url) {
			documentation = __(
	...
	is_dismissed() {
		...
		if (Object.keys(dismissed).includes(this.label)) {
			...
			let today = new Date();
			let diff = finergy.datetime.get_hour_diff(today, last_hidden);
			return diff < 24;
		}
		return false;
	}
	...
	set_actions() {
		...
		dismiss.on("click", () => {
			...
			dismissed[this.label] = finergy.datetime.now_datetime();
