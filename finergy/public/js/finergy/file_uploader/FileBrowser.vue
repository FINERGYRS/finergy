...
export default {
	...
	methods: {
		...
		get_files_in_folder(folder, start) {
			return finergy
				.call("finergy.core.doctype.file.file.get_files_in_folder", {
					folder,
					start,
		...
		search_by_name: finergy.utils.debounce(function() {
			if (this.search_text === "") {
			...
			finergy
				.call(
					"finergy.core.doctype.file.file.get_files_by_search_text",
					{
		...
		make_file_node(file) {
			...
			let label = finergy.utils.file_name_ellipsis(filename, 40);
			return {
				label: label,
				filename: filename,
