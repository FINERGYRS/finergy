				...
				<div class="mt-2 text-center">
					...
					<button v-if="google_drive_settings.enabled" class="btn btn-file-upload" @click="show_google_drive_picker">
						<svg width="30" height="30">
							<image href="/assets/finergyrs/icons/social/google_drive.svg" width="30" height="30"/>
						</svg>
...
export default {
	...
	created() {
		...
		if (finergy.user_id !== "Guest") {
			finergy.call({
				// method only available after login
				method: "finergy.integrations.doctype.google_settings.google_settings.get_file_picker_settings",
				callback: (resp) => {
					if (!resp.exc) {
	...
	methods: {
		...
		upload_via_file_browser() {
			...
			if (!selected_file.value) {
				finergy.msgprint(__('Click on a file to select it.'));
		...
		upload_via_web_link() {
			...
			if (!file_url) {
				finergy.msgprint(__('Invalid URL'));
				return Promise.reject();
			}
		...
		return_as_dataurl() {
			let promises = this.files.map(file =>
				finergy.dom.file_to_base64(file.file_obj)
					.then(dataurl => {
		...
		capture_image() {
			const capture = new finergy.ui.Capture({
				animate: false,
				error: true
			});
			capture.show();
			capture.submit(data_urls => {
				data_urls.forEach(data_url => {
					let filename = `capture_${finergy.datetime.now_datetime().replaceAll(/[: -]/g, '_')}.png`;
