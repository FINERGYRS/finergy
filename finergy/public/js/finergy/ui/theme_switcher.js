...
finergy.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
		...
		const preview = $(`<div class="${this.current_theme == theme.name ? "selected" : "" }">
			<div data-theme=${theme.name}>
				<div class="background">
					<div>
						<div class="preview-check">${finergy.utils.icon('tick', 'xs')}</div>
					</div>
	...
	toggle_theme(theme) {
		...
		finergy.show_alert(__("Theme Changed"), 3);

		finergy.xcall("finergy.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme)
		});
	}
	show() {
