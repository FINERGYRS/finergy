finergy.provide('finergy.ui.keys');

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
finergy.ui.keys.shortcut_groups = shortcut_groups;

finergy.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new finergy.ui.keys.AltShortcutGroup());
	}
...
finergy.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on('keydown', (e) => {
		let key = (finergy.ui.keys.key_map[e.which] || '').toLowerCase();
...
finergy.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = finergy.ui.keys.standard_shortcuts
			.filter(s => !s.page)
