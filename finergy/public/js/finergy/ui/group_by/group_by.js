...
finergy.ui.GroupBy = class {
	...
	init_group_by_popover() {
		...
		const group_by_template = $(
			finergy.render_template('group_by', {
			...
			if (!this.aggregate_on_html.length) {
				...
				for (let doctype in this.all_fields) {
					...
					doctype_fields.forEach((field) => {
						...
						if (finergy.model.is_numeric_field(field.fieldtype)) {
							let option_text =
								doctype == this.doctype
									? field.label
	...
	get_group_by_docfield() {
		...
		if (this.aggregate_function === 'count') {
			...
			docfield = Object.assign(
				{},
				finergy.meta.docfield_map[this.aggregate_on_doctype][
					this.aggregate_on_field
				]
			);
	...
	get_group_by_fields() {
		...
		const standard_fields_filter = (df) =>
			!in_list(finergy.model.no_value_type, df.fieldtype) && !df.report_hide;

		const table_fields = finergy.meta
