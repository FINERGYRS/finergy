finergy.ui.FilterGroup = class {
	...
	add_filters(filters) {
		...
		return finergy.run_serially(promises).then(() => this.update_filters());
	}
	...
	_push_new_filter(doctype, fieldname, condition, value, hidden = false) {
		...
		let filter = new finergy.ui.Filter(args);
	...
	filter_exists(filter_value) {
		...
		this.filters
			...
			.map((f) => {
				...
				let equal = finergy.utils.arrays_equal;
