finergy.ui.Filter = class {
	constructor(opts) {
		...
		this.utils = finergy.ui.filter_utils;
		this.set_conditions();
	...
	set_conditions_from_config() {
		if (finergy.boot.additional_filters_config) {
			this.filters_config = finergy.boot.additional_filters_config;
	...
	make() {
		this.filter_edit_area = $(
			finergy.render_template('edit_filter', {
	...
	make_select() {
		this.fieldselect = new finergy.ui.FieldSelect({
	...
	make_field(df, old_fieldtype) {
		...
		let f = finergy.ui.form.make_control({
			df: df,
			parent: field_area,
			only_input: true,
		});
	...
	toggle_nested_set_conditions(df) {
		let show_condition =
			df.fieldtype === 'Link' &&
			finergy.boot.nested_set_doctypes.includes(df.options);
...
finergy.ui.filter_utils = {
	get_formatted_value(field, value) {
		...
		return finergy.format(value, field.df, { only_value: 1 });
	},
