...
finergy.ui.FieldSelect = Class.extend({
	...
	build_options() {
		var me = this;
		me.table_fields = [];
		var std_filters = $.map(finergy.model.std_fields, function(d) {
		...
		// main table
		var main_table_fields = std_filters.concat(finergy.meta.docfield_list[me.doctype]);
		$.each(finergy.utils.sort(main_table_fields, "label", "string"), function(i, df) {
			let doctype = finergy.get_meta(me.doctype).istable && me.parent_doctype ?
				me.parent_doctype : me.doctype;

			// show fields where user has read access and if report hide flag is not set
			if (finergy.perm.has_perm(doctype, df.permlevel, "read"))
		...
		$.each(me.table_fields, function(i, table_df) {
			if(table_df.options) {
				let child_table_fields = [].concat(finergy.meta.docfield_list[table_df.options]);

				if (table_df.fieldtype === "Table MultiSelect") {
					const link_field = finergy.meta.get_docfields(table_df.options)
						.find(df => df.fieldtype === 'Link');
					child_table_fields = link_field ? [link_field] : [];
				}

				$.each(finergy.utils.sort(child_table_fields, "label", "string"), function(i, df) {
					let doctype = finergy.get_meta(me.doctype).istable && me.parent_doctype ?
						me.parent_doctype : me.doctype;
	...
	add_field_option(df) {
		...
		if (df.fieldname == 'docstatus' && !finergy.model.is_submittable(me.doctype))
			return;
		...
		if(finergy.model.no_value_type.indexOf(df.fieldtype) == -1 &&
