 ...
 *
 * @returns {finergy.ui.Page}
 */

/**
 * @typedef {Object} finergy.ui.Page
 */


finergy.ui.make_app_page = function(opts) {
	opts.parent.page = new finergy.ui.Page(opts);
	return opts.parent.page;
}

finergy.ui.pages = {};

finergy.ui.Page = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		...
		this.make();
		finergy.ui.pages[finergy.get_route_str()] = this;
	},
	...
	load_lib: function(callback) {
		finergy.require(this.required_libs, callback);
	},

	add_main_section: function() {
		$(finergy.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_page: function() {
		...
		this.card_layout && this.main.addClass('finergy-card');
	...
	setup_sidebar_toggle() {
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (finergy.utils.is_xs() || finergy.utils.is_sm()) {
	...
	update_sidebar_icon() {
		...
		sidebar_toggle_icon.html(finergy.utils.icon(is_sidebar_visible ? 'sidebar-collapse' : 'sidebar-expand', 'md'));
	},
	...
	add_action_icon: function(icon, click, css_class='', tooltip_label) {
		const button = $(`
			<button class="text-muted btn btn-default ${css_class} icon-btn">
				${finergy.utils.icon(icon)}
			</button>
		`);

		button.appendTo(this.icon_group.removeClass("hide"));
		button.click(click);
		button.attr("title", __(tooltip_label || finergy.unscrub(icon)))
	...
	get_icon_label: function(icon, label) {
		...
		return `${icon ? finergy.utils.icon(icon_name, size) : ''} <span class="hidden-xs"> ${__(label)} </span>`;
	},
	...
	*/
	add_dropdown_item: function({label, click, standard, parent, shortcut, show_parent=true, icon=null}) {
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${finergy.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			finergy.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
	...
	*/
	...
	set_title: function(title, icon=null, strip=true, tab_title="") {
		...
		this.title = title;
		finergy.utils.set_title(tab_title || title);
		if (icon) {
			title = `${finergy.utils.icon(icon)} ${title}`;
		}
	...
	add_button: function(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || 'btn-default'} ${opts.btn_size || 'btn-sm'} ellipsis">
				${opts.icon ? finergy.utils.icon(opts.icon): ''}
				${label}
		</button>`);
	...
	add_dropdown_button: function(parent, label, click, icon) {
		finergy.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	},
	...
	add_field: function(df, parent) {
		...
		var f = finergy.ui.form.make_control({
			df: df,
