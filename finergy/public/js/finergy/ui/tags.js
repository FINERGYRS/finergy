...
finergy.ui.Tags = class {
	...
	bind() {
		...
		const select_tag = function() {
			const tagValue = finergy.utils.xss_sanitise(me.$input.val());
			me.addTag(tagValue);
			me.$input.val('');
		};
	...
	removeTag(label) {
		label = finergy.utils.xss_sanitise(label);
	...
	get_tag(label) {
		let $tag = finergy.get_data_pill(label, label, (target, pill_wrapper) => {
