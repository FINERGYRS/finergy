finergy.ui.LinkPreview = class {
	...
	setup_popover_control(e) {
		if (!(finergy.boot.link_preview_doctypes || []).includes(this.doctype)) {
			return;
		}
	...
	get_preview_data() {
		return finergy.xcall('finergy.desk.link_preview.get_preview_data', {
	...
	get_content_html(preview_data) {
		...
		Object.keys(preview_data).forEach(key => {
			if (!['preview_image', 'preview_title', 'name'].includes(key)) {
				let value = finergy.ellipsis(preview_data[key], 280);
				let label = key;
				content_html += `
