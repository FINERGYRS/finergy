finergy.ui.TagEditor = Class.extend({
	init: function(opts) {
		/* docs:
		Arguments
	...
	setup_tags: function() {
		...
		this.tags = new finergy.ui.Tags({
			parent: this.wrapper,
			placeholder: __("Add a tag ..."),
			onTagAdd: (tag) => {
				if(me.initialized && !me.refreshing) {
					return finergy.call({
						method: "finergy.desk.doctype.tag.tag.add_tag",
			...
			onTagRemove: (tag) => {
				if(!me.refreshing) {
					return finergy.call({
						method: "finergy.desk.doctype.tag.tag.remove_tag",
						args: me.get_args(tag),
						callback: function(r) {
	...
	setup_awesomplete: function() {
		...
		$input.on("input", function(e) {
			...
			finergy.call({
				method: "finergy.desk.doctype.tag.tag.get_tags",
				args:{
