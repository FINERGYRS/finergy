...
finergy.ui.Notifications = class Notifications {
	constructor() {
		this.tabs = {};
		this.notification_settings = finergy.boot.notification_settings;
		this.make();
	}
	...
	make() {
		...
		this.user = finergy.session.user;
	...
	setup_headers() {
		...
		$(`<span class="notification-settings pull-right" data-action="go_to_settings">
			${finergy.utils.icon('setting-gear')}
		</span>`)
			.on('click', (e) => {
				e.stopImmediatePropagation();
				this.dropdown.dropdown('hide');
				finergy.set_route('Form', 'Notification Settings', finergy.session.user);
			}).appendTo(this.header_actions)
			.attr('title', __("Notification Settings"))
			.tooltip({ delay: { "show": 600, "hide": 100}, trigger: "hover"  });

		$(`<span class="mark-all-read pull-right" data-action="mark_all_as_read">
			${finergy.utils.icon('mark-as-read')}
		</span>`)
	...
	mark_all_as_read(e) {
		...
		finergy.call(
			'finergy.desk.doctype.notification_log.notification_log.mark_all_as_read',
		);
	}
...
finergy.ui.notifications = {
	get_notification_config() {
		return finergy.xcall('finergy.desk.notifications.get_notification_info').then(r => {
			finergy.ui.notifications.config = r;
			return r;
		});
	},

	show_open_count_list(doctype) {
		if (!finergy.ui.notifications.config) {
	...
	route_to_list_with_filters(doctype) {
		let filters = finergy.ui.notifications.config['conditions'][doctype];
		if (filters && $.isPlainObject(filters)) {
			if (!finergy.route_options) {
				finergy.route_options = {};
			}
			$.extend(finergy.route_options, filters);
		}
		finergy.set_route('List', doctype);
	}
};
...
class NotificationsView extends BaseNotificationsView {
	...
	mark_as_read(docname, $el) {
		finergy.call(
			'finergy.desk.doctype.notification_log.notification_log.mark_as_read',
			{ docname: docname }
		).then(() => {
	...
	get_dropdown_item_html(field) {
		...
		message = title ? message.replace(title[1], finergy.ellipsis(strip_html(title[1]), 100)) : message;

		let timestamp = finergy.datetime.comment_when(field.creation);
		...
		let user_avatar = finergy.avatar(user, 'avatar-medium user-avatar');
			...
			if (this.dropdown_items.length) {
				...
				this.container.append($(`<div class="notification-null-state">
					<div class="text-center">
						<img src="/assets/finergyrs/images/ui-states/notification-empty-state.svg" alt="Generic Empty State" class="null-state">
	...
	get_notifications_list(limit) {
		return finergy.db.get_list('Notification Log', {
			fields: ["*"],
			limit: limit,
	...
	get_item_link(notification_doc) {
		...
		return finergy.utils.get_form_link(
			link_doctype,
			link_docname
		);
	}
	...
	toggle_seen(flag) {
		finergy.call(
			'finergy.desk.doctype.notification_settings.notification_settings.set_seen_value',
			{
				value: cint(flag),
	...
	setup_notification_listeners() {
		finergy.realtime.on('notification', () => {
			this.toggle_notification_icon(false);
			this.update_dropdown();
		});

		finergy.realtime.on('indicator_hide', () => {
		...
		this.parent.on('show.bs.dropdown', () => {
			...
			if (this.notifications_icon.find('.notifications-unseen').is(':visible')) {
				this.toggle_notification_icon(true);
				finergy.call(
					'finergy.desk.doctype.notification_log.notification_log.trigger_indicator_hide'
				);
			}
		});
...
class EventsView extends BaseNotificationsView {
	make() {
		let today = finergy.datetime.get_today();
		finergy.xcall('finergy.desk.doctype.event.event.get_events', {
			start: today,
			end: today
		}).then(event_list => {
	...
	render_events_html(event_list) {
		...
		if (event_list.length) {
			let get_event_html = (event) => {
				let time = __("All Day");
				if (!event.all_day) {
					let start_time = finergy.datetime.get_time(event.starts_on);
					let days_diff = finergy.datetime.get_day_diff(event.ends_on, event.starts_on);
					let end_time = finergy.datetime.get_time(event.ends_on);
					if (days_diff > 1) {
				...
				if (event.particpants) {
					particpants = finergy.avatar_group(event.particpants, 3);
				}
			...
			html = `
				<div class="notification-null-state">
					<div class="text-center">
					<img src="/assets/finergyrs/images/ui-states/event-empty-state.svg" alt="Generic Empty State" class="null-state">
