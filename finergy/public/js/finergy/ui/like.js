...
finergy.ui.is_liked = function(doc) {
	var liked = finergy.ui.get_liked_by(doc);
	return liked.indexOf(finergy.session.user)===-1 ? false : true;
}

finergy.ui.get_liked_by = function(doc) {
...
finergy.ui.toggle_like = function($btn, doctype, name, callback) {
	var add = $btn.hasClass("not-liked") ? "Yes" : "No";
	// disable click
	$btn.css('pointer-events', 'none');

	finergy.call({
		method: "finergy.desk.like.toggle_like",
		quiet: true,
		args: {
			doctype: doctype,
			name: name,
			...
			if(!r.exc) {
				...
				if(doc) {
					var liked_by = JSON.parse(doc._liked_by || "[]"),
						idx = liked_by.indexOf(finergy.session.user);
					if(add==="Yes") {
						if(idx===-1)
							liked_by.push(finergy.session.user);
					} else {
						if(idx!==-1) {
...
finergy.ui.click_toggle_like = function() {
	var $btn = $(this);
	...
	finergy.ui.toggle_like($btn, doctype, name, function() {
		if (not_liked) {
...
finergy.ui.setup_like_popover = ($parent, selector, check_not_liked=true) => {
	if (finergy.dom.is_touchscreen()) {
		return;
	}
			...
			content: () => {
				...
				liked_by.forEach(user => {
					...
					liked_by_list.append(`
						<li data-user=${user}>${finergy.avatar(user, "avatar-xs")}
							<span>${finergy.user.full_name(user)}</span>
						</li>
					`);
				});
				...
				liked_by_list.children('li').click(ev => {
					...
					finergy.set_route(link_base + user);
				});
