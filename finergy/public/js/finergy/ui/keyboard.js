...
finergy.provide('finergy.ui.keys.handlers');

finergy.ui.keys.setup = function() {
	$(window).on('keydown', function(e) {
		var key = finergy.ui.keys.get_key(e);
		if(finergy.ui.keys.handlers[key]) {
			var out = null;
			for(var i=0, l = finergy.ui.keys.handlers[key].length; i<l; i++) {
				var handler = finergy.ui.keys.handlers[key][i];
...
finergy.ui.keys.standard_shortcuts = standard_shortcuts;
finergy.ui.keys.add_shortcut = ({shortcut, action, description, page, target, condition, ignore_inputs = false} = {}) => {
	...
	finergy.ui.keys.off(shortcut, page);
	// attach new handler
	finergy.ui.keys.on(shortcut, handler);
...
finergy.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (finergy.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map(shortcut => {
				let shortcut_label = shortcut.shortcut
					.split('+')
					.map(finergy.utils.to_title_case)
					.join('+');
				if (finergy.utils.is_mac()) {
	...
	let dialog = new finergy.ui.Dialog({
		...
		on_hide() {
			finergy.ui.keys.is_dialog_shown = false;
		}
	});
	...
	dialog.show();
	finergy.ui.keys.is_dialog_shown = true;
}

finergy.ui.keys.get_key = function(e) {
	var keycode = e.keyCode || e.which;
	var key = finergy.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
finergy.ui.keys.on = function(key, handler) {
	if(!finergy.ui.keys.handlers[key]) {
		finergy.ui.keys.handlers[key] = [];
	}
	finergy.ui.keys.handlers[key].push(handler);
}

finergy.ui.keys.off = function(key, page) {
	let handlers = finergy.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	finergy.ui.keys.handlers[key] = handlers.filter(h => {
...
finergy.ui.keys.add_shortcut({
	shortcut: 'ctrl+s',
	action: function(e) {
		finergy.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
finergy.ui.keys.on('escape', function(e) {
	close_grid_and_dialog();
});

finergy.ui.keys.on('esc', function(e) {
	close_grid_and_dialog();
});

finergy.ui.keys.on('enter', function(e) {
	if(window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger('click');
	}
});

finergy.ui.keys.on('ctrl+down', function(e) {
	var grid_row = finergy.ui.form.get_open_grid_form();
	grid_row && grid_row.toggle_view(false, function() { grid_row.open_next() });
});

finergy.ui.keys.on('ctrl+up', function(e) {
	var grid_row = finergy.ui.form.get_open_grid_form();
...
'abcdefghijklmnopqrstuvwxyz'.split('').forEach((letter, i) => {
	finergy.ui.keys.key_map[65 + i] = letter;
});
...
$(document).on('keydown', '.dropdown-toggle', (e) => {
	if (e.which === finergy.ui.keyCode.ESCAPE) {
