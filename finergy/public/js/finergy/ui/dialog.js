...
finergy.ui.open_dialogs = [];

finergy.ui.Dialog = class Dialog extends finergy.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		this.$wrapper = finergy.get_modal("", "");
		...
		this.$wrapper
			.on("hide.bs.modal", function() {
				...
				if(finergy.ui.open_dialogs[finergy.ui.open_dialogs.length-1]===me) {
					finergy.ui.open_dialogs.pop();
					if(finergy.ui.open_dialogs.length) {
						window.cur_dialog = finergy.ui.open_dialogs[finergy.ui.open_dialogs.length-1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function() {
				...
				finergy.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger('finergy.ui.Dialog:shown');
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(finergy.utils.icon(icon));
