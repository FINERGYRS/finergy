finergy.ui.SortSelector = Class.extend({
	// parent:
	// change:
	// args:
	...
	make: function() {
		...
		this.wrapper = $(finergy.render_template('sort_selector', this.args)).appendTo(this.parent);
		this.bind_events();
	},
	bind_events: function() {
		...
		this.wrapper.find('.btn-order').on('click', function() {
			...
			btn.find('.sort-order').html(finergy.utils.icon(icon_name, 'sm'));
		...
		if(!this.args.options) {
			...
			meta.fields.forEach(function(df) {
				if (
					...
					&& finergy.perm.has_perm(me.doctype, df.permlevel, "read")
				) {
	...
	get_label: function(fieldname) {
		if(fieldname==='idx') {
			...
			return this.labels[fieldname]
				|| finergy.meta.get_label(this.doctype, fieldname);
		}
	},
