...
finergy.ui.Tree = class {
	constructor({
		...
		if (!icon_set) {
			this.icon_set = {
				open: finergy.utils.icon('folder-open', 'md'),
				closed: finergy.utils.icon('folder-normal', 'md'),
				leaf: finergy.utils.icon('primitive-dot', 'xs')
			};
		}
	...
	get_all_nodes(value, is_root, label) {
		...
		return new Promise(resolve => {
			finergy.call({
				method: 'finergy.desk.treeview.get_all_nodes',
				args: args,
				callback: (r) => {
	...
	on_node_click(node) {
		this.expand_node(node);
		finergy.dom.activate(this.wrapper, node.$tree_link, 'tree-link');
