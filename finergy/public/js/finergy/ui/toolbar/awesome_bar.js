...
finergy.provide('finergy.tags');

finergy.search.AwesomeBar = Class.extend({
	setup: function(element) {
		...
		$input.on("input", finergy.utils.debounce(function(e) {
			...
			if (txt && txt.length > 1) {
				...
				me.options = me.options.concat(
					me.deduplicate(finergy.search.utils.get_recent_pages(txt || "")));
				me.options = me.options.concat(finergy.search.utils.get_frequent_links());
			}
			me.add_help();
		...
		$input.on("awesomplete-select", function(e) {
			...
			if(item.route_options) {
				finergy.route_options = item.route_options;
			}

			if(item.onclick) {
				item.onclick(item.match);
			} else {
				finergy.set_route(item.route);
			}
			$input.val("");
		});
		...
		finergy.search.utils.setup_recent();
		finergy.tags.utils.fetch_tags();
	},
	...
	add_help: function() {
		this.options.push({
			...
			onclick: function() {
				...
				finergy.msgprint(txt, __("Search Help"));
			}
		});
	},
	...
	build_options: function(txt) {
		var options = finergy.search.utils.get_creatables(txt).concat(
			finergy.search.utils.get_search_in_list(txt),
			finergy.search.utils.get_doctypes(txt),
			finergy.search.utils.get_reports(txt),
			finergy.search.utils.get_pages(txt),
			finergy.search.utils.get_workspaces(txt),
			finergy.search.utils.get_dashboards(txt),
			finergy.search.utils.get_recent_pages(txt || ""),
			finergy.search.utils.get_executables(txt)
		);
		if (txt.charAt(0) === "#") {
			options = finergy.tags.utils.get_tags(txt);
		}
	...
	make_global_search: function(txt) {
		...
		// 		<span class="ellipsis">Search for ${finergy.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	finergy.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${ __('Search for {0}', [finergy.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
	...
	make_search_in_current: function(txt) {
		...
		if(route[0]==="List" && txt.indexOf(" in") === -1) {
			...
			var meta = finergy.get_meta(finergy.container.page.list_view.doctype);
		...
		if(first==parseInt(first) || first==="(" || first==="=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function() {
						finergy.msgprint(formatted_value, "Result");
					}
				});
			} catch(e) {
				// pass
	...
	make_random: function(txt) {
		if(txt.toLowerCase().includes('random')) {
			this.options.push({
				...
				value: finergy.utils.get_random(16),
				onclick: function() {
					finergy.msgprint(finergy.utils.get_random(16), "Result");
				}
			})
		}
	}
