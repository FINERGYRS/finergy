...
finergy.search.SearchDialog = class {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		this.search_dialog = new finergy.ui.Dialog({
			minimizable: true,
			size: "large"
		});
		this.set_header();
		...
		this.searches = {
			global_search: {
				...
				get_results: (keywords, callback) => {
					let start = 0,
						limit = 1000;
					let results = finergy.search.utils.get_nav_results(keywords);
			...
			tags: {
				...
				no_results_status: keyword =>
					...
					"</div>",
				get_results: (keywords, callback) => {
					var results = finergy.search.utils.get_nav_results(keywords);
					finergy.tags.utils.get_tag_results(keywords).then(
						global_results => {
							results = results.concat(global_results);
							callback(results, keywords);
						},
	...
	render_data(result_sets) {
		let $search_results = $(finergy.render_template("search")).addClass(
			"hide"
		);
	...
	render_result(type, result) {
		...
		if (result.image !== undefined) {
			let avatar_html = finergy.get_avatar(
				"avatar-medium",
				result.label,
				result.image
			);
	...
	handle_result_click(result, $result) {
		if (result.route_options) {
			finergy.route_options = result.route_options;
		}
		$result.on("click", () => {
			...
			if (result.onclick) {
				...
				finergy.set_route(result.route);
