...
finergy.search.utils = {
	setup_recent: function() {
		this.recent = JSON.parse(finergy.boot.user.recent || "[]") || [];
	},
	...
	get_recent_pages: function(keywords) {
		...
		finergy.route_history.forEach(function(route, i) {
			if(route[0]==='Form') {
				...
				values.push([finergy.route_titles[route.join('/')] || route[0], route]);
			}
		});
	...
	get_frequent_links() {
		let options = [];
		finergy.boot.frequently_visited_links.forEach(link => {
			const label = finergy.utils.get_route_label(link.route);
			options.push({
				'route': link.route,
				'label': label,
				'value': label,
	...
	get_search_in_list: function(keywords) {
		...
		if(in_list(keywords.split(" "), "in") && (keywords.slice(-2) !== "in")) {
			...
			finergy.boot.user.can_read.forEach(function(item) {
				if(finergy.boot.user.can_search.includes(item)) {
	...
	get_creatables: function(keywords) {
		...
		if(firstKeyword.toLowerCase() === __("new")) {
			finergy.boot.user.can_create.forEach(function(item) {
	...
	get_doctypes: function(keywords) {
		...
		finergy.boot.user.can_read.forEach(function(item) {
			...
			if (level) {
				target = item;
				if (in_list(finergy.boot.single_types, item)) {
					out.push(option("", ["Form", item, item], 0.05));

				} else if (finergy.boot.user.can_search.includes(item)) {
					// include 'making new' option
					if (in_list(finergy.boot.user.can_create, item)) {
						var match = item;
						out.push({
							type: "New",
							...
							match: item,
							onclick: function() {
								finergy.new_doc(match, true);
							}
						});
					}
					if (in_list(finergy.boot.treeviews, item)) {
						out.push(option("Tree", ["Tree", item], 0.05));

					} else {
						out.push(option("List", ["List", item], 0.05));
						if (finergy.model.can_get_report(item)) {
	...
	get_reports: function(keywords) {
		...
		var out = [];
		var route;
		Object.keys(finergy.boot.user.all_reports).forEach(function(item) {
			var level = me.fuzzy_search(keywords, item);
			if(level > 0) {
				var report = finergy.boot.user.all_reports[item];
	...
	get_workspaces: function(keywords) {
		var me = this;
		var out = [];
		finergy.boot.allowed_workspaces.forEach(function(item) {
			...
			if (level > 0) {
				var ret = {
					...
					index: level,
					route: [finergy.router.slug(item.name)]
				};
	...
	get_dashboards: function(keywords) {
		var me = this;
		var out = [];
		finergy.boot.dashboards.forEach(function(item) {
			var level = me.fuzzy_search(keywords, item.name);
			if (level > 0) {
				var ret = {
					type: "Dashboard",
	...
	get_global_results: function(keywords, start, limit, doctype = "") {
		...
		return new Promise(function(resolve, reject) {
			finergy.call({
				method: "finergy.utils.global_search.search",
				args: {
					text: keywords,
					start: start,
					limit: limit,
