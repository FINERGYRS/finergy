...
finergy.tags.utils = {
	get_tags: function(txt) {
		...
		for (let i in finergy.tags.tags) {
			let tag = finergy.tags.tags[i];
			let level = finergy.search.utils.fuzzy_search(txt, tag);
			if (level) {
				out.push({
					type: "Tag",
					label: __("#{0}", [finergy.search.utils.bolden_match_part(__(tag), txt)]),
					value: __("#{0}", [__(tag)]),
					index: 1 + level,
					match: tag,
					onclick() {
						// Use Global Search Dialog for tag search too.
						finergy.searchdialog.search.init_search("#".concat(tag), "tags");
					}
				});
			}
		}
	...
	fetch_tags() {
		finergy.call({
			method: "finergy.desk.doctype.tag.tag.get_tags_list_for_awesomebar",
			callback: function(r) {
				if (r && r.message) {
					finergy.tags.tags = $.extend([], r.message);
				}
			}
		});
	},
	...
	get_tag_results: function(tag) {
		...
		return new Promise(function(resolve) {
			finergy.call({
				method: "finergy.desk.doctype.tag.tag.get_documents_for_tag",
				args: {
					tag: tag
				},
				callback: function(r) {
