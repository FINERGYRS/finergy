// finergy.ui.Capture
// Author - Achilles Rasquinha <achilles@finergy-rs.fr>
 ...
 *
 * @example
 * finergy._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
finergy._.get_data_uri = element => {
...
/**
 * @description Finergy's Capture object.
 *
 * @example
 * const capture = finergy.ui.Capture()
 * capture.show()
 *
 ...
 */
finergy.ui.Capture = class {
	constructor(options = {}) {
		this.options = finergy.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...finergy.ui.Capture.OPTIONS, ...options };
	...
	build_dialog() {
		...
		me.$template = $(finergy.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch(err => {
				if (me.options.error) {
					finergy.show_alert(finergy.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__('Take Photo'), () => {
			const data_url = finergy._.get_data_uri(me.video);
...
finergy.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false
};
finergy.ui.Capture.ERR_MESSAGE = __('Unable to load camera.');
finergy.ui.Capture.TEMPLATE = `
<div class="finergy-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${finergy.ui.Capture.ERR_MESSAGE}</video>
	</div>
