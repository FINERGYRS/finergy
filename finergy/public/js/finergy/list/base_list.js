...
finergy.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || finergy.router.doctype_layout || __(this.doctype);
		this.meta = finergy.get_meta(this.doctype);
		this.settings = finergy.listview_settings[this.doctype] || {};
		this.user_settings = finergy.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = 20;
		this.data = [];
		this.method = "finergy.desk.reportview.get";

		this.can_create = finergy.model.can_create(this.doctype);
	...
	get_list_view_settings() {
		return finergy
			.call("finergy.desk.listview.get_list_settings", {
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			finergy.model.std_fields_list.includes(fieldname) ||
			finergy.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = finergy.workflow.get_state_fieldname(
			this.doctype
		);
		if (this.workflow_state_fieldname) {
			if (!finergy.workflow.workflows[this.doctype]["override_status"]) {
	...
	setup_page() {
		...
		!this.hide_card_layout && this.page.main.addClass('finergy-card');
	...
	setup_view_menu() {
		...
		if (finergy.boot.desk_settings.view_switcher) {
			/* @preserve
			...
			this.views_list = new finergy.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		finergy.breadcrumbs.add(this.meta.module, this.doctype);
	}

	setup_side_bar() {
		if (this.hide_sidebar || !finergy.boot.desk_settings.list_sidebar) return;
		this.list_sidebar = new finergy.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_list_wrapper() {
		this.$finergy_list = $('<div class="finergy-list">').appendTo(
			this.page.main
		);
	}
	...
	setup_result_area() {
		...
		this.$finergy_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$finergy_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$finergy_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$finergy_list.append(this.$paging_area);
	...
	get_fields() {
		...
		return this.fields.map((f) =>
			finergy.model.get_full_column_name(f[0], f[1])
		);
	}
	...
	get_group_by() {
		...
		if (name_field) {
			return finergy.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	refresh() {
		...
		return finergy.call(this.get_call_args()).then((r) => {
			// render
			this.prepare_data(r);
	...
	prepare_data(r) {
		...
		data = !Array.isArray(data)
			? finergy.utils.dict(data.keys, data.values)
			: data;
...
class FilterArea {
	...
	clear(refresh = true) {
		...
		return finergy.run_serially(promises).then(() => {
			this.trigger_refresh = true;
		});
	}
		...
		fields = fields.concat(
			doctype_fields
				...
				.map((df) => {
					...
					let default_value =
						fieldtype === "Link"
							? finergy.defaults.get_user_default(options)
							: null;
	...
	make_filter_list() {
		...
		this.filter_list = new finergy.ui.FilterGroup({
...
finergy.views.is_valid = (view_mode) =>
	finergy.views.view_modes.includes(view_mode);
