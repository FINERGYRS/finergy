...
finergy.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = finergy.utils.to_title_case(route[2] || "");
		if (route.length > 2 && finergy.views.view_modes.includes(view_name)) {
	...
	setup_views() {
		const views = {
			...
			Calendar: {
				condition: finergy.views.calendar[this.doctype],
				...
				current_view_handler: () => {
					...
					if (
						...
						default_action = {
							...
							action: () => finergy.new_doc("Email Account")
						};
					}
			...
			Tree: {
				condition:
					finergy.treeview_settings[this.doctype] ||
					finergy.get_meta(this.doctype).is_tree,
			...
			Kanban: {
				...
				current_view_handler: () => {
					finergy.views.KanbanView.get_kanbans(this.doctype).then(
		...
		finergy.views.view_modes.forEach(view => {
	...
	setup_kanban_switcher(kanbans) {
		...
		this.page.add_custom_menu_item(
			...
			() => finergy.views.KanbanView.show_kanban_dialog(this.doctype),
			true
		);
	}

	get_page_name() {
		return finergy.utils.to_title_case(
			finergy.get_route().slice(-1)[0] || ""
		);
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(finergy.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
		const last_opened_kanban =
			finergy.model.user_settings[this.doctype]["Kanban"] &&
			finergy.model.user_settings[this.doctype]["Kanban"]
				.last_kanban_board;

		if (!last_opened_kanban) {
			return finergy.views.KanbanView.show_kanban_dialog(
				this.doctype,
				true
			);
		}
		finergy.db.exists("Kanban Board", last_opened_kanban).then(exists => {
			if (exists) {
				finergy.set_route(
					"list",
					this.doctype,
				...
				finergy.views.KanbanView.show_kanban_dialog(this.doctype, true);
			}
		});
	}
	...
	get_calendars() {
		...
		return finergy.db
			...
			.then(result => {
				...
				if (finergy.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = finergy.boot.email_accounts;
	...
	slug() {
		return finergy.router.slug(finergy.router.doctype_layout || this.doctype);
	}
};
