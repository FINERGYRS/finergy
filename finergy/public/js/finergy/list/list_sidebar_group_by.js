...
finergy.views.ListGroupBy = class ListGroupBy {
	constructor(opts) {
		$.extend(this, opts);
		this.make_wrapper();
	...
	render_group_by_items() {
		let get_item_html = (fieldname) => {
			...
			if (fieldname === 'assigned_to') {
				...
				label = finergy.meta.get_label(this.doctype, fieldname);
				let docfield = finergy.meta.get_docfield(this.doctype, fieldname);
				if (!docfield) {
					return;
				}
			...
			return `<li class="group-by-field list-link">
					...
					href="#" onclick="return false;">
						...
						<span>${finergy.utils.icon('select', 'xs')}</span>
					</a>
	...
	get_group_by_count(field) {
		...
		return finergy
			.call('finergy.desk.listview.get_group_by_count', args)
			.then((r) => {
				let field_counts = r.message || [];
				field_counts = field_counts.filter((f) => f.count !== 0);
				let current_user = field_counts.find(
					(f) => f.name === finergy.session.user
				);
				field_counts = field_counts.filter(
					(f) =>
						!['Guest', 'Administrator', finergy.session.user].includes(f.name)
				);
				// Set finergy.session.user on top of the list
	...
	get_dropdown_html(field, fieldtype, applied=false) {
		...
		let applied_html = applied ? `<span class="applied"> ${finergy.utils.icon('tick', 'xs')} </span>` : '';
