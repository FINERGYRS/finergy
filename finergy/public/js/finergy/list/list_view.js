...
finergy.views.ListView = class ListView extends finergy.views.BaseList {
	static load_last_view() {
		const route = finergy.get_route();
		const doctype = route[1];

		if (route.length === 2) {
			const user_settings = finergy.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			finergy.set_route(
				"list",
				finergy.router.doctype_layout || doctype,
				finergy.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	has_permissions() {
		const can_read = finergy.perm.has_perm(this.doctype, 0, "read");
		return can_read;
	}
	...
	show() {
		...
		if (!this.has_permissions()) {
			finergy.set_route('');
			finergy.msgprint(__("Not permitted to view {0}", [this.doctype]));
			return;
		}
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(finergy.model.std_fields_list);
		return filters
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = finergy.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${finergy.utils.icon('restriction', 'xs')}
				</button>`
	...
	show_restrictions(match_rules_list = []) {
		finergy.msgprint(
			finergy.render_template("list_view_permission_restrictions", {
	...
	set_fields() {
		...
		this.fields.forEach((f) => {
			const df = finergy.meta.get_docfield(f[1], f[0]);
			if (
				df &&
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = finergy.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				finergy.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
		...
		setInterval(() => {
			...
			if (finergy.get_route_str() === this.page_name) {
				this.refresh();
			}
		}, interval);
	}
	...
	set_primary_action() {
		if (this.can_create) {
			const doctype_name = __(finergy.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (f[2] === "=" && finergy.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		finergy.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = finergy.meta.get_docfield.bind(null, this.doctype);
		...
		if (finergy.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter((df) => {
					if (
						finergy.has_indicator(this.doctype) &&
						df.fieldname === "status"
					) {
						return false;
					}
	...
	get_no_result_message() {
		...
		let empty_state_image =
			...
			"/assets/finergyrs/images/ui-states/list-empty-state.svg";
	...
	before_refresh() {
		if (finergy.route_options && this.filter_area) {
	...
	before_render() {
		...
		finergy.model.user_settings.save(
			this.doctype,
			"last_view",
			this.view_name
		);
	...
	get_header_html() {
		...
		let subject_html = `
			...
			<span class="level-item list-liked-by-me hidden-xs">
				<span title="${__("Likes")}">${finergy.utils.icon('heart', 'sm', 'like-icon')}</span>
			</span>
		...
		const $columns = this.columns
			.map(col => {
				let classes = [
					...
					finergy.model.is_numeric_field(col.df) ? "text-right" : "",
				].join(" ");
	...
	get_column_html(col, doc) {
		...
		const format = () => {
			if (df.fieldtype === "Code") {
				...
				return finergy.format(value, df, null, doc);
			}
		};
		...
		const field_html = () => {
			...
			if (strip_html_required) {
				...
				_value =
					typeof value === "string"
						? finergy.utils.escape_html(value)
						: value;
			}
			...
			if (df.fieldtype === "Image") {
				html = df.options ? `<img src="${doc[df.options]}"
					...
					: `<div class="missing-image small">
						${finergy.utils.icon('restriction')}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="filterable indicator-pill ${finergy.utils.guess_colour(
					_value
				)} ellipsis"
				...
				html = `<a class="filterable ellipsis"
					data-filter="${fieldname},=,${finergy.utils.escape_html(value)}">
					${format()}
				</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${finergy.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		const css_class = [
			"list-row-col ellipsis",
			class_map[col.type],
			finergy.model.is_numeric_field(df) ? "text-right" : "",
		].join(" ");
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments">
					${finergy.avatar_group(assigned_users, 3, { filterable: true })[0].outerHTML}
				</div>`;
		}
		...
		const comment_count = `<span class="${
			!doc._comment_count ? "text-extra-muted" : ""
				...
				${finergy.utils.icon('small-message')}
	...
	get_count_str() {
		...
		return finergy.db.count(this.doctype, {
	...
	get_form_link(doc) {
		...
		return `/app/${finergy.router.slug(finergy.router.doctype_layout || this.doctype)}/${encodeURIComponent(doc.name)}`;
	}

	get_seen_class(doc) {
		return JSON.parse(doc._seen || '[]').includes(finergy.session.user)
			? ''
			: 'bold';
	}
	...
	get_like_html(doc) {
		...
		let heart_class = liked_by.includes(finergy.session.user)
			? "liked-by liked"
			: "not-liked";
		...
		return `<span
			...
			title="${liked_by.map(u => finergy.user_info(u).fullname).join(', ')}">
			${finergy.utils.icon('heart', 'sm', 'like-icon')}
		</span>
	...
	get_subject_html(doc) {
		...
		let escaped_subject = finergy.utils.escape_html(subject);
	...
	get_indicator_html(doc) {
		const indicator = finergy.get_indicator(doc, this.doctype);
		if (indicator) {
	...
	get_image_url(doc) {
		...
		if (window.cordova && !finergy.utils.is_url(url)) {
			url = finergy.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = finergy.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = finergy.session.user;
				}
	...
	setup_list_click() {
		this.$result.on("click", ".list-row, .image-view-header, .file-header", (e) => {
			...
			if (link) {
				finergy.set_route(link.pathname);
				return false;
			}
		});
	}
	...
	setup_like() {
		this.$result.on("click", ".like-action", finergy.ui.click_toggle_like);
	...
	setup_realtime_updates() {
		...
		finergy.realtime.on("list_update", (data) => {
			...
			finergy.call(call_args).then(({ message }) => {
				if (!message) return;
				const data = finergy.utils.dict(message.keys, message.values);
				if (!(data && data.length)) {
	...
	save_view_user_settings(obj) {
		return finergy.model.user_settings.save(
			this.doctype,
			this.view_name,
			obj
		);
	...
	share_url() {
		const d = new finergy.ui.Dialog({
			...
			primary_action: () => {
				finergy.utils.copy_to_clipboard(this.get_share_url());
				d.hide();
			},
		});
		d.show();
	...
	get_menu_items() {
		...
		if (finergy.model.can_import(doctype)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					finergy.set_route("list", "data-import", {
		...
		if (finergy.model.can_set_user_permissions(doctype)) {
			items.push({
		...
		if (finergy.user_roles.includes("System Manager")) {
			...
			items.push({
				...
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						finergy.set_route("form", "doctype", doctype);
		...
		if (
			...
			finergy.boot.developer_mode === 1
		) {
			// edit doctype
			items.push({
				label: __("Edit DocType", null, "Button in list view menu"),
				action: () => finergy.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	show_list_settings() {
		finergy.model.with_doctype(this.doctype, () => {
			new ListSettings({
				listview: this,
				doctype: this.doctype,
				settings: this.list_view_settings,
				meta: finergy.get_meta(this.doctype)
			});
		});
	}
	...
	get_workflow_action_menu_items() {
		...
		if (finergy.model.has_workflow(this.doctype)) {
			const actions = finergy.workflow.get_all_transition_actions(
				this.doctype
			);
			actions.forEach((action) => {
				workflow_actions.push({
					...
					action: () => {
						finergy.xcall(
							"finergy.model.workflow.bulk_workflow_approval",
							{
	...
	toggle_workflow_actions() {
		if (!finergy.model.has_workflow(this.doctype)) return;
		const checked_items = this.get_checked_items();
		finergy
			.xcall("finergy.model.workflow.get_common_transition_actions", {
				docs: checked_items,
				doctype: this.doctype,
			})
			.then((actions) => {
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				finergy.model.is_value_type(field_doc) &&
		...
		const has_submit_permission = (doctype) => {
			return finergy.perm.has_perm(doctype, 0, "submit");
		};
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					finergy.meta.get_docfields(doctype).forEach((field_doc) => {
		...
		// bulk printing
		if (finergy.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			finergy.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!finergy.model.has_workflow(doctype)
		) {
		...
		// bulk cancel
		if (
			finergy.model.can_cancel(doctype) &&
			!finergy.model.has_workflow(doctype)
		) {
		...
		// bulk delete
		if (finergy.model.can_delete(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		for (let field in finergy.route_options) {
			let doctype = null;
			let value = finergy.route_options[field];
			...
			if (!doctype) {
				doctype = finergy.meta.get_doctype_for_field(
					this.doctype,
					field
				);
			}
	...
	static trigger_list_update(data) {
		...
		const list_view = finergy.views.list_view[page_name];
		if (
...
$(document).on("save", (event, doc) => {
	finergy.views.ListView.trigger_list_update(doc);
});

finergy.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return finergy.views.list_view[route];
};
