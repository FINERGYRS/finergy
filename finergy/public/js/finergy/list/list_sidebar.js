...
finergy.views.ListSidebar = class ListSidebar {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		var sidebar_content = finergy.render_template("list_sidebar", { doctype: this.doctype });
	...
	setup_views() {
		...
		if (this.doctype === "Communication" && finergy.boot.email_accounts.length) {
			this.sidebar.find('.list-link[data-view="Inbox"]').removeClass('hide');
			show_list_link = true;
		}

		if (finergy.treeview_settings[this.doctype] || finergy.get_meta(this.doctype).is_tree) {
			this.sidebar.find(".tree-link").removeClass("hide");
		}

		this.current_view = 'List';
		var route = finergy.get_route();
		if (route.length > 2 && finergy.views.view_modes.includes(route[2])) {
	...
	setup_reports() {
		...
		var reports = Object.values(finergy.boot.user.all_reports).sort((a,b) => a.title.localeCompare(b.title)) || [];
	...
	setup_kanban_boards() {
		...
		finergy.views.KanbanView.setup_dropdown_in_sidebar(this.doctype, $dropdown);
	}
	...
	setup_list_group_by() {
		this.list_group_by = new finergy.views.ListGroupBy({
			doctype: this.doctype,
			sidebar: this,
			list_view: this.list_view,
			page: this.page
	...
	get_stats() {
		var me = this;
		finergy.call({
			method: 'finergy.desk.reportview.get_sidebar_stats',
			type: 'GET',
			args: {
				stats: me.stats,
				doctype: me.doctype,
			...
			callback: function(r) {
				...
				finergy.utils.setup_search(stats_dropdown, '.stat-link', '.stat-label');
			}
		});
	}
	...
	render_stat(stats) {
		...
		let tag_list = $(finergy.render_template("list_sidebar_stat", args)).on("click", ".stat-link", (e) => {
