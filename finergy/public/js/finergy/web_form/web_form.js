...
finergy.provide("finergy.web_form");
import EventEmitterMixin from '../../finergyrs/event_emitter';

export default class WebForm extends finergy.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		finergy.web_form = this;
		finergy.web_form.events = {};
		Object.assign(finergy.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		finergy.init_client_script && finergy.init_client_script();
		finergy.web_form.events.trigger('after_load');
	...
	set_default_values() {
		let values = finergy.utils.get_query_params();
		delete values.new;
	...
	setup_delete_button() {
		finergy.has_permission(this.doc_type, "", "delete", () => {
			this.add_button_to_header(
				finergy.utils.icon('delete'),
				"danger",
				() => this.delete()
			);
		});
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _section.find(".finergy-control:not(.hide-control)");
	...
	save() {
		...
		if (this.validate && !this.validate()) {
			finergy.throw(__("Couldn't save, please check the data you have entered"), __("Validation Error"));
		}
		...
		// Save
		window.saving = true;
		finergy.form_dirty = false;

		finergy.call({
			type: "POST",
			method: "finergy.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
			...
			callback: response => {
				...
				if (!response.exc) {
					...
					finergy.web_form.events.trigger('after_save');
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						finergy.call({
							type: 'POST',
							method: "finergy.handler.upload_file",
							args: {
	...
	delete() {
		finergy.call({
			type: "POST",
			method: "finergy.website.doctype.web_form.web_form.delete",
			args: {
	...
	handle_success(data) {
		...
		const success_dialog = new finergy.ui.Dialog({
