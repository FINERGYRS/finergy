...
finergy.provide("finergy.web_form_list");

export default class WebFormList {
	constructor(opts) {
		Object.assign(this, opts);
		finergy.web_form_list = this;
	...
	make_filters() {
		...
		finergy.call('finergy.website.doctype.web_form.web_form.get_web_form_filters', {
			...
			fields.forEach(field => {
				...
				let input = finergy.ui.form.make_control({
					df: {
	...
	get_list_view_fields() {
		return finergy
			.call({
				method:
					"finergy.website.doctype.web_form.web_form.get_in_list_view_fields",
	...
	fetch_data() {
		return finergy.call({
			method: "finergy.www.list.get_list_data",
			args: {
	...
	more() {
		...
		this.fetch_data().then((res) => {
			if (res.message.length === 0) {
				finergy.msgprint(__("No more items to display"));
			}
	...
	append_rows(row_data) {
		...
		row_data.forEach((data_item) => {
			...
			let row = new finergy.ui.WebFormListRow({
				row: row_element,
				doc: data_item,
	...
	make_actions() {
		...
		finergy.has_permission(this.doctype, "", "delete", () => {
	...
	delete_rows() {
		...
		finergy
			.call({
				type: "POST",
				method:
					"finergy.website.doctype.web_form.web_form.delete_multiple",
				args: {
					web_form_name: this.web_form_name,
...
finergy.ui.WebFormListRow = class WebFormListRow {
	...
	make_row() {
		...
		this.columns.forEach(field => {
			let cell = this.row.insertCell();
			let formatter = finergy.form.get_formatter(field.fieldtype);
