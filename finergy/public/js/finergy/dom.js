...
finergy.dom = {
	...
	get_unique_id: function() {
		const id = 'unique-' + finergy.dom.id_count;
		finergy.dom.id_count++;
		return id;
	},
	set_unique_id: function(ele) {
		...
		var id = 'unique-' + finergy.dom.id_count;
		$ele.attr('id', id);
		finergy.dom.id_count++;
		return id;
	},
	...
	add: function(parent, newtag, className, cs, innerHTML, onclick) {
		if(parent && parent.substr)parent = finergy.dom.by_id(parent);
	...
	freeze: function(msg, css_class) {
		...
		finergy.dom.freeze_count++;
	},
	unfreeze: function() {
		if(!finergy.dom.freeze_count) return; // anything open?
		finergy.dom.freeze_count--;
		if(!finergy.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				finergy.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
finergy.ellipsis = function(text, max) {
	if(!max) max = 20;
	text = cstr(text);
	if(text.length > max) {
...
finergy.run_serially = function(tasks) {
...
finergy.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
finergy.timeout = seconds => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

finergy.scrub = function(text, spacer='_') {
	return text.replace(/ /g, spacer).toLowerCase();
};

finergy.unscrub = function(txt) {
	return finergy.model.unscrub(txt);
};

finergy.get_data_pill = (label, target_id=null, remove_action=null, image=null) => {
...
finergy.get_modal = function(title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${finergy.utils.icon('collapse')}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${finergy.utils.icon('close-alt', 'sm', 'close-alt')}
						</button>
					</div>
				</div>
...
finergy.is_online = function() {
	if (finergy.boot.developer_mode == 1) {
