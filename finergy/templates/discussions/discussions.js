...
const setup_socket_io = () => {
	const assets = [
		"/assets/finergyrs/js/lib/socket.io.min.js",
		"/assets/finergyrs/js/finergyrs/socketio_client.js"
	];

	finergy.require(assets, () => {
		if (window.dev_server) {
			finergy.boot.socketio_port = "9000";
		}
		finergy.socketio.init(9000);
		finergy.socketio.socket.on("publish_message", (data) => {
...
const publish_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${data.topic_info.name}]`).length) {
		...
		if (data.topic_info.owner == finergy.session.user) {
			$(".discussion-on-page").collapse();
			$(".sidebar-topic").first().click();
		}
	} else if (data.topic_info.owner == finergy.session.user) {
		post_message_cleanup();
...
const submit_discussion = (e) => {
	...
	if (reply) {
		...
		finergy.call({
			method: "finergy.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = finergy.get_palette(element.attr("title"));
