...
login.bind_events = function () {
	...
	$(".form-login").on("submit", function (event) {
		...
		var args = {};
		args.cmd = "login";
		args.usr = finergy.utils.xss_sanitise(($("#login_email").val() || "").trim());
		args.pwd = $("#login_password").val();
		args.device = "desktop";
		if (!args.usr || !args.pwd) {
			finergy.msgprint('{{ _("Both login and password required") }}');
			return false;
		}
		login.call(args);
		return false;
	...
	$(".form-signup").on("submit", function (event) {
		event.preventDefault();
		var args = {};
		args.cmd = "finergy.core.doctype.user.user.sign_up";
		args.email = ($("#signup_email").val() || "").trim();
		args.redirect_to = finergy.utils.sanitise_redirect(finergy.utils.get_url_arg("redirect-to"));
		args.full_name = finergy.utils.xss_sanitise(($("#signup_fullname").val() || "").trim());
	...
	$(".form-forgot").on("submit", function (event) {
		event.preventDefault();
		var args = {};
		args.cmd = "finergy.core.doctype.user.user.reset_password";
	...
	var login_handlers = {
		200: function (data) {
			if (data.message == 'Logged In') {
				...
				window.location.href = finergy.utils.sanitise_redirect(finergy.utils.get_url_arg("redirect-to")) || data.home_page;
			} else if (data.message == 'Password Reset') {
				window.location.href = finergy.utils.sanitise_redirect(data.redirect_to);
			} else if (data.message == "No App") {
				login.set_status("{{ _('Success') }}", 'green');
				if (localStorage) {
					var last_visited =
						localStorage.getItem("last_visited")
						|| finergy.utils.sanitise_redirect(finergy.utils.get_url_arg("redirect-to"));
				...
				if (cint(data.message[0]) == 0) {
					...
					login.set_status('{{ _("Success") }}', 'green');
					finergy.msgprint(data.message[1])
				}
...
var verify_token = function (event) {
	$(".form-verify").on("submit", function (eventx) {
		...
		args.tmp_id = finergy.get_cookie('tmp_id');
		if (!args.otp) {
			finergy.msgprint('{{ _("Login token required") }}');
			return false;
		}
		login.call(args);
		return false;
