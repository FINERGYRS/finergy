...
import finergy
from finergy.core.doctype.user.user import generate_keys
from finergy.event_streaming.doctype.event_producer.event_producer import pull_from_node
...
class TestEventProducer(unittest.TestCase):
	# @classmethod
	# def setUpClass(cls):
	# 	finergy.print_sql(True)

	# @classmethod
	# def tearDownClass(cls):
	# 	finergy.print_sql(False)
	...
	def test_insert(self):
		...
		self.assertTrue(finergy.db.exists("ToDo", producer_doc.name))
	...
	def test_update(self):
		...
		self.pull_producer_data()
		local_doc = finergy.get_doc(producer_doc.doctype, producer_doc.name)
	...
	def test_delete(self):
		...
		self.pull_producer_data()
		self.assertFalse(finergy.db.exists("ToDo", producer_doc.name))
	...
	def test_multiple_doctypes_sync(self):
		...
		producer_note1 = finergy._dict(doctype="Note", title="test multiple doc sync 1")
		delete_on_remote_if_exists(producer, "Note", {"title": producer_note1["title"]})
		finergy.db.delete("Note", {"title": producer_note1["title"]})
		producer_note1 = producer.insert(producer_note1)
		producer_note2 = finergy._dict(doctype="Note", title="test multiple doc sync 2")
		delete_on_remote_if_exists(producer, "Note", {"title": producer_note2["title"]})
		finergy.db.delete("Note", {"title": producer_note2["title"]})
		...
		# check inserted
		self.assertTrue(finergy.db.exists("ToDo", producer_todo.name))

		# check update
		local_todo = finergy.get_doc("ToDo", producer_todo.name)
		self.assertEqual(local_todo.description, producer_todo.description)
		local_note1 = finergy.get_doc("Note", producer_note1.name)
		self.assertEqual(local_note1.content, producer_note1.content)

		# check delete
		self.assertFalse(finergy.db.exists("Note", producer_note2.name))

	def test_child_table_sync_with_dependencies(self):
		producer = get_remote_site()
		producer_user = finergy._dict(
			doctype="User",
		...
		finergy.db.delete("User", {"email": producer_user.email})
		producer_user = producer.insert(producer_user)

		producer_note = finergy._dict(
		...
		finergy.db.delete("Note", {"title": producer_note.title})
		producer_note = producer.insert(producer_note)

		self.pull_producer_data()
		self.assertTrue(finergy.db.exists("User", producer_user.name))
		if self.assertTrue(finergy.db.exists("Note", producer_note.name)):
			local_note = finergy.get_doc("Note", producer_note.name)
	...
	def test_dynamic_link_dependencies_synced(self):
		...
		event_producer = finergy.get_doc("Event Producer", producer_url, for_update=True)
		event_producer.producer_doctypes = []
		event_producer.append("producer_doctypes", {"ref_doctype": "ToDo", "use_same_name": 1})
		event_producer.save()

		producer_link_doc = finergy._dict(doctype="Note", title="Test Dynamic Link 1")

		delete_on_remote_if_exists(producer, "Note", {"title": producer_link_doc.title})
		finergy.db.delete("Note", {"title": producer_link_doc.title})
		producer_link_doc = producer.insert(producer_link_doc)
		producer_doc = finergy._dict(
			doctype="ToDo",
		...
		self.assertTrue(finergy.db.exists("Note", producer_link_doc.name))
		self.assertEqual(
			producer_link_doc.name, finergy.db.get_value("ToDo", producer_doc.name, "reference_name")
		)
	...
	def test_conditional_events(self):
		...
		# Add Condition
		event_producer = finergy.get_doc("Event Producer", producer_url)
		...
		# Make test doc
		producer_note1 = finergy._dict(doctype="Note", public=0, title="test conditional sync")
		delete_on_remote_if_exists(producer, "Note", {"title": producer_note1["title"]})
		producer_note1 = producer.insert(producer_note1)
		...
		self.assertFalse(finergy.db.exists("Note", producer_note1.name))
		...
		# it should sync now
		self.assertTrue(finergy.db.exists("Note", producer_note1.name))
		local_note = finergy.get_doc("Note", producer_note1.name)
	...
	def test_conditional_events_with_cmd(self):
		...
		note_producer_entry.condition = (
			"cmd: finergy.event_streaming.doctype.event_producer.test_event_producer.can_sync_note"
		)
		event_producer.save()

		# Make test doc
		producer_note1 = finergy._dict(doctype="Note", public=0, title="test conditional sync cmd")
		delete_on_remote_if_exists(producer, "Note", {"title": producer_note1["title"]})
		producer_note1 = producer.insert(producer_note1)
	...
	def test_event_sync_log(self):
		...
		self.pull_producer_data()
		self.assertTrue(finergy.db.exists("Event Sync Log", {"docname": producer_doc.name}))
	...
	def test_mapping(self):
		...
		producer_note = finergy._dict(doctype="Note", title="Test Mapping", content="Test Mapping")
		delete_on_remote_if_exists(producer, "Note", {"title": producer_note.title})
		producer_note = producer.insert(producer_note)
		self.pull_producer_data()
		# check inserted
		self.assertTrue(finergy.db.exists("ToDo", {"description": producer_note.content}))
		...
		self.pull_producer_data()
		# check delete
		self.assertFalse(finergy.db.exists("ToDo", {"description": producer_note.content}))
	...
	def test_inner_mapping(self):
		...
		self.assertTrue(finergy.db.exists("Role", {"role_name": producer_note.title}))
		# check doc inserted
		self.assertTrue(finergy.db.exists("ToDo", {"description": producer_note.content}))
...
def get_mapping(mapping_name, local, remote, field_map):
	name = finergy.db.exists("Document Type Mapping", mapping_name)
	if name:
		doc = finergy.get_doc("Document Type Mapping", name)
	else:
		doc = finergy.new_doc("Document Type Mapping")
...
def create_event_producer(producer_url):
	if finergy.db.exists("Event Producer", producer_url):
		event_producer = finergy.get_doc("Event Producer", producer_url)
	...
	response = producer_site.post_api(
		"finergy.core.doctype.user.user.generate_keys", params={"user": "Administrator"}
	)
	...
	event_producer = finergy.new_doc("Event Producer")
	event_producer.producer_doctypes = []
...
def get_remote_site():
	producer_doc = finergy.get_doc("Event Producer", producer_url)
	producer_site = FinergyClient(
...
def connect():
	def _connect():
		return FinergyClient(url=producer_url, username="Administrator", password="admin", verify=False)
