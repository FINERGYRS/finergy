...
class EventConsumer(Document):
	def validate(self):
		...
		# finergy.flags.in_test won't work here as tests are running on the consumer site
	...
	def on_update(self):
		if not self.incoming_change:
			...
			finergy.db.set_value(self.doctype, self.name, "incoming_change", 0)

		finergy.cache().delete_value("event_consumer_document_type_map")

	def on_trash(self):
		for i in finergy.get_all("Event Update Log Consumer", {"consumer": self.name}):
			finergy.delete_doc("Event Update Log Consumer", i.name)
		finergy.cache().delete_value("event_consumer_document_type_map")
	...
	def update_consumer_status(self):
		...
		event_producer = finergy._dict(event_producer)
		...
		for entry in config:
			...
			entry["status"] = finergy.db.get_value(
...
def register_consumer(data):
	...
	if finergy.db.exists("Event Consumer", data["event_consumer"]):
		return None

	user = data["user"]
	if not finergy.db.exists("User", user):
		finergy.throw(_("User {0} not found on the producer site").format(user))

	if "System Manager" not in finergy.get_roles(user):
		finergy.throw(_("Event Subscriber has to be a System Manager."))

	consumer = finergy.new_doc("Event Consumer")
...
def get_consumer_site(consumer_url):
	"""create a FinergyClient object for event consumer site"""
	consumer_doc = finergy.get_doc("Event Consumer", consumer_url)
	consumer_site = FinergyClient(
		url=consumer_url,
...
def notify(consumer):
	...
	if consumer_status == "online":
		try:
			...
			client.post_request(
				{
					"cmd": "finergy.event_streaming.doctype.event_producer.event_producer.new_event_notification",
	...
	if not consumer.flags.notified:
		enqueued_method = "finergy.event_streaming.doctype.event_consumer.event_consumer.notify"
		jobs = get_jobs()
		if not jobs or enqueued_method not in jobs[finergy.local.site] and not consumer.flags.notifed:
			finergy.enqueue(
...
def has_consumer_access(consumer, update_log):
	...
	if isinstance(consumer, str):
		consumer = finergy.get_doc("Event Consumer", consumer)

	if not finergy.db.exists(update_log.ref_doctype, update_log.docname):
		# Delete Log
		# Check if the last Update Log of this document was read by this consumer
		last_update_log = finergy.get_all(
			"Event Update Log",
			filters={
		...
		last_update_log = finergy.get_doc("Event Update Log", last_update_log[0].name)
		return len([x for x in last_update_log.consumers if x.consumer == consumer.name])

	doc = finergy.get_doc(update_log.ref_doctype, update_log.docname)
	try:
		for dt_entry in consumer.consumer_doctypes:
			...
			if condition.startswith("cmd:"):
				...
				return finergy.call(cmd, **args)
			else:
				return finergy.safe_eval(condition, finergy._dict(doc=doc))
	except Exception as e:
		finergy.log_error(title="has_consumer_access error", message=e)
	return False
