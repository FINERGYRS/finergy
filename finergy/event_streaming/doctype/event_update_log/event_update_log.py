...
import finergy
from finergy.model import no_value_fields, table_fields
...
class EventUpdateLog(Document):
	def after_insert(self):
		...
		enqueued_method = (
			"finergy.event_streaming.doctype.event_consumer.event_consumer.notify_event_consumers"
		)
		jobs = get_jobs()
...
def notify_consumers(doc, event):
	...
	if finergy.flags.in_install or finergy.flags.in_migrate:
		return
...
def get_update(old, new, for_child=False):
	...
	out = finergy._dict(changed={}, added={}, removed={}, row_changed={})
...
def make_event_update_log(doc, update_type):
	...
	if update_type != "Delete":
		...
		data = finergy.as_json(doc) if not doc.get("diff") else finergy.as_json(doc.diff)
	else:
		data = None
...
def is_consumer_uptodate(update_log, consumer):
	...
	prev_log_consumers = finergy.get_all(
...
def mark_consumer_read(update_log_name, consumer_name):
	...
	"""
	update_log = finergy.get_doc("Event Update Log", update_log_name)
...
def get_update_logs_for_consumer(event_consumer, doctypes, last_update):
	...
	if isinstance(doctypes, str):
		doctypes = finergy.parse_json(doctypes)

	from finergy.event_streaming.doctype.event_consumer.event_consumer import has_consumer_access

	consumer = finergy.get_doc("Event Consumer", event_consumer)
	docs = finergy.get_list(
		doctype="Event Update Log",
