"""
FinergyClient is a library that helps you connect with other finergy systems
"""
...
class FinergyException(Exception):
	pass


class FinergyClient(object):
	def __init__(
		self,
		url,
		username=None,
		...
		api_key=None,
		api_secret=None,
		finergy_authorization_source=None,
	):
		self.headers = {
		...
		self.finergy_authorization_source = finergy_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.finergy_authorization_source:
				auth_source = {"Finergy-Authorization-Source": self.finergy_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": finergy.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return finergy._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "finergy.client.insert_many", "docs": finergy.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": finergy.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return finergy._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "finergy.client.bulk_update", "docs": finergy.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "finergy.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "finergy.client.submit", "doc": finergy.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "finergy.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": finergy.as_json(filters),
			}
		)
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "finergy.client.cancel", "doctype": doctype, "name": name})
	...
	def migrate_doctype(
		...
		for doc in docs:
			...
			if doctype != "User" and not finergy.db.exists("User", doc.get("owner")):
				finergy.get_doc(
	...
	def migrate_single(self, doctype):
		...
		doc.modified = finergy.db.get_single_value(doctype, "modified")
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = "FinergyClient Request Failed\n\n" + exc
			except Exception:
				exc = rjson["exc"]

			raise FinergyException(exc)
		if "message" in rjson:
...
class FinergyOAuth2Client(FinergyClient):
