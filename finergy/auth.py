...
import finergy
import finergy.database
import finergy.utils
import finergy.utils.user
from finergy import _, conf
from finergy.core.doctype.activity_log.activity_log import add_authentication_log
from finergy.modules.patch_handler import check_session_stopped
from finergy.sessions import Session, clear_sessions, delete_session
from finergy.translate import get_language
from finergy.twofactor import (
	authenticate_for_2factor,
	confirm_otp_token,
	get_cached_user_pass,
	should_run_2fa,
)
from finergy.utils import cint, date_diff, datetime, get_datetime, today
from finergy.utils.password import check_password
from finergy.website.utils import get_home_page


class HTTPRequest:
	def __init__(self):
		# set finergy.local.request_ip
		self.set_request_ip()

		# load cookies
		self.set_cookies()

		# set finergy.local.db
		self.connect()
		...
		finergy.local.cookie_manager.init_cookies()
	...
	def domain(self):
		if not getattr(self, "_domain", None):
			self._domain = finergy.request.host
	...
	def set_request_ip(self):
		if finergy.get_request_header("X-Forwarded-For"):
			finergy.local.request_ip = (finergy.get_request_header("X-Forwarded-For").split(",")[0]).strip()

		elif finergy.get_request_header("REMOTE_ADDR"):
			finergy.local.request_ip = finergy.get_request_header("REMOTE_ADDR")
	...
	def validate_csrf_token(self):
		if finergy.local.request and finergy.local.request.method in ("POST", "PUT", "DELETE"):
			if not finergy.local.session:
				return
			if (
				not finergy.local.session.data.csrf_token
				or finergy.local.session.data.device == "mobile"
				or finergy.conf.get("ignore_csrf", None)
			):
				# not via boot
				return

			csrf_token = finergy.get_request_header("X-Finergy-CSRF-Token")
			if not csrf_token and "csrf_token" in finergy.local.form_dict:
				csrf_token = finergy.local.form_dict.csrf_token
				del finergy.local.form_dict["csrf_token"]

			if finergy.local.session.data.csrf_token != csrf_token:
				finergy.local.flags.disable_traceback = True
				finergy.throw(_("Invalid Request"), finergy.CSRFTokenError)

	def set_lang(self):
		finergy.local.lang = get_language()
	...
	def connect(self):
		...
		finergy.local.db = finergy.database.get_db(
...
class LoginManager:
	def __init__(self):
		...
		if (
			finergy.local.form_dict.get("cmd") == "login" or finergy.local.request.path == "/api/method/login"
		):
			if self.login() == False:
				return
			self.resume = False
	...
	def login(self):
		# clear cache
		finergy.clear_cache(user=finergy.form_dict.get("usr"))
		user, pwd = get_cached_user_pass()
		self.authenticate(user=user, pwd=pwd)
		if self.force_user_to_reset_password():
			doc = finergy.get_doc("User", self.user)
			finergy.local.response["redirect_to"] = doc.reset_password(
				send_email=False, password_expired=True
			)
			finergy.local.response["message"] = "Password Reset"
			return False
		...
		if should_run_2fa(self.user):
			...
			if not confirm_otp_token(self):
				return False
		finergy.form_dict.pop("pwd", None)
		self.post_login()
	...
	def get_user_info(self):
		self.info = finergy.db.get_value(
	...
	def setup_boot_cache(self):
		finergy.cache_manager.build_table_count_cache()
		finergy.cache_manager.build_domain_restriced_doctype_cache()
		finergy.cache_manager.build_domain_restriced_page_cache()
	...
	def set_user_info(self, resume=False):
		...
		if self.info.user_type == "Website User":
			finergy.local.cookie_manager.set_cookie("system_user", "no")
			if not resume:
				finergy.local.response["message"] = "No App"
				finergy.local.response["home_page"] = "/" + get_home_page()
		else:
			finergy.local.cookie_manager.set_cookie("system_user", "yes")
			if not resume:
				finergy.local.response["message"] = "Logged In"
				finergy.local.response["home_page"] = "/app"

		if not resume:
			finergy.response["full_name"] = self.full_name

		# redirect information
		redirect_to = finergy.cache().hget("redirect_after_login", self.user)
		if redirect_to:
			finergy.local.response["redirect_to"] = redirect_to
			finergy.cache().hdel("redirect_after_login", self.user)

		finergy.local.cookie_manager.set_cookie("full_name", self.full_name)
		finergy.local.cookie_manager.set_cookie("user_id", self.user)
		finergy.local.cookie_manager.set_cookie("user_image", self.info.user_image or "")

	def clear_preferred_language(self):
		finergy.local.cookie_manager.delete_cookie("preferred_language")

	def make_session(self, resume=False):
		# start session
		finergy.local.session_obj = Session(
			user=self.user, resume=resume, full_name=self.full_name, user_type=self.user_type
		)

		# reset user if changed to Guest
		self.user = finergy.local.session_obj.user
		finergy.local.session = finergy.local.session_obj.data
		self.clear_active_sessions()

	def clear_active_sessions(self):
		"""Clear other sessions of the current user if `deny_multiple_sessions` is not set"""
		if not (
			cint(finergy.conf.get("deny_multiple_sessions"))
			or cint(finergy.db.get_system_setting("deny_multiple_sessions"))
		):
			return

		if finergy.session.user != "Guest":
			clear_sessions(finergy.session.user, keep_current=True)

	def authenticate(self, user: str = None, pwd: str = None):
		from finergy.core.doctype.user.user import User

		if not (user and pwd):
			user, pwd = finergy.form_dict.get("usr"), finergy.form_dict.get("pwd")
		if not (user and pwd):
		...
		ignore_tracker = should_run_2fa(user.name) and ("otp" in finergy.form_dict)
	...
	def force_user_to_reset_password(self):
		...
		if reset_pwd_after_days:
			last_password_reset_date = (
				finergy.db.get_value("User", self.user, "last_password_reset_date") or today()
			)
	...
	def fail(self, message, user=None):
		if not user:
			user = _("Unknown User")
		finergy.local.response["message"] = message
		add_authentication_log(message, user, status="Failed")
		finergy.db.commit()
		raise finergy.AuthenticationError

	def run_trigger(self, event="on_login"):
		for method in finergy.get_hooks().get(event, []):
			finergy.call(finergy.get_attr(method), login_manager=self)

	def validate_hour(self):
		"""check if user is logging in during restricted hours"""
		login_before = int(finergy.db.get_value("User", self.user, "login_before", ignore=True) or 0)
		login_after = int(finergy.db.get_value("User", self.user, "login_after", ignore=True) or 0)
		...
		if login_before and current_hour > login_before:
			finergy.throw(_("Login not allowed at this time"), finergy.AuthenticationError)
	...
	def logout(self, arg="", user=None):
		...
		if user == finergy.session.user:
			delete_session(finergy.session.sid, user=user, reason="User Manually Logged Out")
			self.clear_cookies()
		else:
			clear_sessions(user)
...
class CookieManager:
	...
	def init_cookies(self):
		if not finergy.local.session.get("sid"):
			return

		# sid expires in 3 days
		expires = datetime.datetime.now() + datetime.timedelta(days=3)
		if finergy.session.sid:
			self.set_cookie("sid", finergy.session.sid, expires=expires, httponly=True)
		if finergy.session.session_country:
			self.set_cookie("country", finergy.session.session_country)

	def set_cookie(self, key, value, expires=None, secure=False, httponly=False, samesite="Lax"):
		if not secure and hasattr(finergy.local, "request"):
			secure = finergy.local.request.scheme == "https"

		# Cordova does not work with Lax
		if finergy.local.session.data.device == "mobile":
			samesite = None
...
def clear_cookies():
	if hasattr(finergy.local, "session"):
		finergy.session.sid = ""
	finergy.local.cookie_manager.delete_cookie(
...
def validate_ip_address(user):
	...
	user = (
		finergy.get_cached_doc("User", user) if not finergy.flags.in_test else finergy.get_doc("User", user)
	)
	...
	system_settings = (
		finergy.get_cached_doc("System Settings")
		if not finergy.flags.in_test
		else finergy.get_single("System Settings")
	)
	...
	for ip in ip_list:
		if finergy.local.request_ip.startswith(ip) or bypass_restrict_ip_check:
			return

	finergy.throw(_("Access not allowed from this IP Address"), finergy.AuthenticationError)
...
def get_login_attempt_tracker(user_name: str, raise_locked_exception: bool = True):
	...
	"""
	sys_settings = finergy.get_doc("System Settings")
...
class LoginAttemptTracker(object):
	...
	def login_failed_count(self):
		return finergy.cache().hget("login_failed_count", self.user_name)

	@login_failed_count.setter
	def login_failed_count(self, count):
		finergy.cache().hset("login_failed_count", self.user_name, count)

	@login_failed_count.deleter
	def login_failed_count(self):
		finergy.cache().hdel("login_failed_count", self.user_name)
	...
	def login_failed_time(self):
		...
		"""
		return finergy.cache().hget("login_failed_time", self.user_name)

	@login_failed_time.setter
	def login_failed_time(self, timestamp):
		finergy.cache().hset("login_failed_time", self.user_name, timestamp)

	@login_failed_time.deleter
	def login_failed_time(self):
		finergy.cache().hdel("login_failed_time", self.user_name)
