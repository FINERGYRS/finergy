...
import finergy
from finergy.automation.doctype.auto_repeat.auto_repeat import (
...
from finergy.utils import add_days, add_months, getdate, today
...
class TestAutoRepeat(unittest.TestCase):
	...
	def test_daily_auto_repeat(self):
		...
		todo = finergy.get_doc(doc.reference_doctype, doc.reference_document)
		self.assertEqual(todo.auto_repeat, doc.name)

		new_todo = finergy.db.get_value(
			"ToDo", {"auto_repeat": doc.name, "name": ("!=", todo.name)}, "name"
		)

		new_todo = finergy.get_doc("ToDo", new_todo)
	...
	def monthly_auto_repeat(self, doctype, docname, start_date, end_date=None):
		...
		data = get_auto_repeat_entries(getdate(today()))
		create_repeated_entries(data)
		docnames = finergy.get_all(doc.reference_doctype, {"auto_repeat": doc.name})
		self.assertEqual(len(docnames), 1)

		doc = finergy.get_doc("Auto Repeat", doc.name)
	...
	def test_submit_on_creation(self):
		...
		submittable_doc = finergy.get_doc(dict(doctype=doctype, test="test submit on creation")).insert()
	...
	doc = finergy.get_doc(
		{
			...
			"reference_document": args.reference_document or finergy.db.get_value("ToDo", "name"),
...
def create_submittable_doctype(doctype, submit_perms=1):
	if finergy.db.exists("DocType", doctype):
		return
	else:
		doc = finergy.get_doc(
			{
