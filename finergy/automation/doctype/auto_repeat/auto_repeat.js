...
finergy.provide("finergy.auto_repeat");

finergy.ui.form.on('Auto Repeat', {
	setup: function(frm) {
		frm.fields_dict['reference_doctype'].get_query = function() {
			return {
				query: "finergy.automation.doctype.auto_repeat.auto_repeat.get_auto_repeat_doctypes"
			};
		};
	...
	refresh: function(frm) {
		...
		if (!frm.is_dirty()) {
			...
			frm.add_custom_button(label, () =>
				finergy.set_route("List", frm.doc.reference_doctype, { auto_repeat: frm.doc.name })
			);
		}

		// auto repeat schedule
		finergy.auto_repeat.render_schedule(frm);
	...
	toggle_submit_on_creation: function(frm) {
		...
		if (frm.doc.reference_doctype) {
			finergy.model.with_doctype(frm.doc.reference_doctype, () => {
				let meta = finergy.get_meta(frm.doc.reference_doctype);
	...
	template: function(frm) {
		if (frm.doc.template) {
			finergy.model.with_doc("Email Template", frm.doc.template, () => {
				let email_template = finergy.get_doc("Email Template", frm.doc.template);
	...
	preview_message: function(frm) {
		if (frm.doc.message) {
			finergy.call({
				method: "finergy.automation.doctype.auto_repeat.auto_repeat.generate_message_preview",
				args: {
				...
				callback: function(r) {
					if (r.message) {
						finergy.msgprint(r.message.message, r.message.subject)
					}
				}
			});
		} else {
			finergy.msgprint(__("Please setup a message first"), __("Message not setup"))
		}
	}
});

finergy.auto_repeat.render_schedule = function(frm) {
	if (!frm.is_dirty() && frm.doc.status !== 'Disabled') {
		frm.call("get_auto_repeat_schedule").then(r => {
			frm.dashboard.wrapper.empty();
			frm.dashboard.add_section(
				finergy.render_template("auto_repeat_schedule", {
