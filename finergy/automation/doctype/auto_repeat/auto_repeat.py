...
from finergy.automation.doctype.assignment_rule.assignment_rule import get_repeated
from finergy.contacts.doctype.contact.contact import (
...
class AutoRepeat(Document):
	...
	def after_save(self):
		finergy.get_doc(self.reference_doctype, self.reference_document).notify_update()

	def on_trash(self):
		finergy.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", "")
		finergy.get_doc(self.reference_doctype, self.reference_document).notify_update()
	...
	def validate_reference_doctype(self):
		if finergy.flags.in_test or finergy.flags.in_patch:
			return
		if not finergy.get_meta(self.reference_doctype).allow_auto_repeat:
			finergy.throw(
	...
	def validate_submit_on_creation(self):
		if self.submit_on_creation and not finergy.get_meta(self.reference_doctype).is_submittable:
			finergy.throw(
				_("Cannot enable {0} for a non-submittable doctype").format(finergy.bold("Submit on Creation"))
			)
	...
	def validate_dates(self):
		...
		if self.end_date == self.start_date:
			finergy.throw(
				_("{0} should not be same as {1}").format(finergy.bold("End Date"), finergy.bold("Start Date"))
			)
			...
			if self.recipients:
				...
				for email in email_list:
					if not validate_email_address(email):
						finergy.throw(_("{0} is an invalid email address in 'Recipients'").format(email))
			else:
				finergy.throw(_("'Recipients' not specified"))
	...
	def validate_auto_repeat_days(self):
		...
		if not len(set(auto_repeat_days)) == len(auto_repeat_days):
			...
			finergy.throw(_("Auto Repeat Day {0} has been repeated.").format(finergy.bold(repeated_days)))

	def update_auto_repeat_id(self):
		# check if document is already on auto repeat
		auto_repeat = finergy.db.get_value(self.reference_doctype, self.reference_document, "auto_repeat")
		if auto_repeat and auto_repeat != self.name and not finergy.flags.in_patch:
			finergy.throw(
				_("The {0} is already on auto repeat {1}").format(self.reference_document, auto_repeat)
			)
		else:
			finergy.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", self.name)
	...
	def create_documents(self):
		try:
			...
			error_log = finergy.log_error(finergy.get_traceback(), _("Auto Repeat Document Creation Failure"))

			self.disable_auto_repeat()

			if self.reference_document and not finergy.flags.in_test:
				self.notify_error_to_user(error_log)

	def make_new_document(self):
		reference_doc = finergy.get_doc(self.reference_doctype, self.reference_document)
		new_doc = finergy.copy_doc(reference_doc, ignore_no_copy=False)
	...
	def update_doc(self, new_doc, reference_doc):
		...
		auto_repeat_doc = finergy.get_doc("Auto Repeat", self.name)
	...
	def set_auto_repeat_period(self, new_doc):
		...
		if mcount and new_doc.meta.get_field("from_date") and new_doc.meta.get_field("to_date"):
			last_ref_doc = finergy.db.get_all(
	...
	def send_notification(self, new_doc):
		...
		if not self.subject:
			...
			subject = finergy.render_template(self.subject, {"doc": new_doc})
		...
		try:
			...
			).format(finergy.bold(_("Note")), finergy.bold(_("Allow Print for Draft")))
			attachments = "[]"
		...
		if error_string:
			...
			message = finergy.render_template(self.message, {"doc": new_doc})
	...
	def fetch_linked_contacts(self):
		if self.reference_doctype and self.reference_document:
			...
			if not email_ids:
				finergy.msgprint(_("No contacts linked to document"), alert=True)
			else:
				self.recipients = ", ".join(email_ids)

	def disable_auto_repeat(self):
		finergy.db.set_value("Auto Repeat", self.name, "disabled", 1)
	...
	def notify_error_to_user(self, error_log):
		...
		form_link = finergy.utils.get_link_to_form(self.reference_doctype, self.reference_document)
		auto_repeat_failed_for = _("Auto Repeat failed for {0}").format(form_link)

		error_log_link = finergy.utils.get_link_to_form("Error Log", error_log.name)
...
def make_auto_repeat_entry():
	enqueued_method = "finergy.automation.doctype.auto_repeat.auto_repeat.create_repeated_entries"
	jobs = get_jobs()

	if not jobs or enqueued_method not in jobs[finergy.local.site]:
		date = getdate(today())
		data = get_auto_repeat_entries(date)
		finergy.enqueue(enqueued_method, data=data)


def create_repeated_entries(data):
	for d in data:
		doc = finergy.get_doc("Auto Repeat", d.name)
		...
		if schedule_date == current_date and not doc.disabled:
			...
			if schedule_date and not doc.disabled:
				finergy.db.set_value("Auto Repeat", doc.name, "next_schedule_date", schedule_date)
...
def set_auto_repeat_as_completed():
	auto_repeat = finergy.get_all("Auto Repeat", filters={"status": ["!=", "Disabled"]})
	for entry in auto_repeat:
		doc = finergy.get_doc("Auto Repeat", entry.name)
		if doc.is_completed():
...
def make_auto_repeat(doctype, docname, frequency="Daily", start_date=None, end_date=None):
	...
	doc = finergy.new_doc("Auto Repeat")
...
def update_reference(docname, reference):
	result = ""
	try:
		finergy.db.set_value("Auto Repeat", docname, "reference_document", reference)
		result = "success"
...
def generate_message_preview(reference_dt, reference_doc, message=None, subject=None):
	finergy.has_permission("Auto Repeat", "write", throw=True)
	doc = finergy.get_doc(reference_dt, reference_doc)
	subject_preview = _("Please add a subject to your email")
	msg_preview = finergy.render_template(message, {"doc": doc})
	if subject:
		subject_preview = finergy.render_template(subject, {"doc": doc})
