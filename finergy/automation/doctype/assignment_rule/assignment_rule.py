...
class AssignmentRule(Document):
	def validate(self):
		...
		if not len(set(assignment_days)) == len(assignment_days):
			...
			finergy.throw(_("Assignment Day {0} has been repeated.").format(finergy.bold(repeated_days)))
		if self.document_type == "ToDo":
			finergy.throw(
				_("Assignment Rule is not allowed on {0} document type").format(finergy.bold("ToDo"))
			)
		...
		if user:
			assign_to.add(
				dict(
					...
					name=doc.get("name"),
					description=finergy.render_template(self.description, doc),
	...
	def get_user_based_on_field(self, doc):
		...
		if finergy.db.exists("User", val):
			return val

	def safe_eval(self, fieldname, doc):
		try:
			if self.get(fieldname):
				return finergy.safe_eval(self.get(fieldname), None, doc)
		except Exception as e:
			# when assignment fails, don't block the document as it may be
			# a part of the email pulling
			finergy.msgprint(finergy._("Auto assignment failed: {0}").format(str(e)), indicator="orange")
	...
	def is_rule_not_applicable_today(self):
		today = finergy.flags.assignment_day or finergy.utils.get_weekday()
		assignment_days = self.get_assignment_days()
...
def apply(doc, method=None, doctype=None, name=None):
	...
	assignment_rules = finergy.cache_manager.get_doctype_map(
		"Assignment Rule",
		doc.doctype,
	...
	for d in assignment_rules:
		assignment_rule_docs.append(finergy.get_cached_doc("Assignment Rule", d.get("name")))
...
def update_due_date(doc, state=None):
	...
	for rule in assignment_rules:
		rule_doc = finergy.get_cached_doc("Assignment Rule", rule.get("name"))
		...
		if (
			...
			assignment_todos = finergy.get_all(
				"ToDo",
				{
...
def get_assignment_rules():
	return [
		d.document_type
		for d in finergy.db.get_all("Assignment Rule", fields=["document_type"], filters=dict(disabled=0))
	]
...
def clear_assignment_rule_cache(rule):
	finergy.cache_manager.clear_doctype_map("Assignment Rule", rule.document_type)
	finergy.cache_manager.clear_doctype_map(
