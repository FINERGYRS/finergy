...
def execute():
	...
	finergy.reload_doc("integrations", "doctype", "social_login_key", force=True)

	if not finergy.db.exists("DocType", "Social Login Keys"):
		return

	social_login_keys = finergy.get_doc("Social Login Keys", "Social Login Keys")
	if social_login_keys.get("facebook_client_id") or social_login_keys.get("facebook_client_secret"):
		facebook_login_key = finergy.new_doc("Social Login Key")
	...
	if social_login_keys.get("finergy_server_url"):
		finergy_login_key = finergy.new_doc("Social Login Key")
		finergy_login_key.get_social_login_provider("Finergy", initialize=True)
		finergy_login_key.social_login_provider = "Finergy"
		finergy_login_key.base_url = social_login_keys.get("finergy_server_url")
		finergy_login_key.client_id = social_login_keys.get("finergy_client_id")
		finergy_login_key.client_secret = social_login_keys.get("finergy_client_secret")
		if not (
			finergy_login_key.client_secret and finergy_login_key.client_id and finergy_login_key.base_url
		):
			finergy_login_key.enable_social_login = 0
		finergy_login_key.save()

	if social_login_keys.get("github_client_id") or social_login_keys.get("github_client_secret"):
		github_login_key = finergy.new_doc("Social Login Key")
	...
	if social_login_keys.get("google_client_id") or social_login_keys.get("google_client_secret"):
		google_login_key = finergy.new_doc("Social Login Key")
	...
	finergy.delete_doc("DocType", "Social Login Keys")


def run_patch():
	finergy.reload_doc("core", "doctype", "user", force=True)
	finergy.reload_doc("core", "doctype", "user_social_login", force=True)
	...
	for user in users:
		idx = 0
		if user.finergy_userid:
			insert_user_social_login(user.name, user.modified_by, "finergy", idx, userid=user.finergy_userid)
			idx += 1
...
def insert_user_social_login(user, modified_by, provider, idx, userid=None, username=None):
	...
	creation_time = finergy.utils.get_datetime_str(finergy.utils.get_datetime())
	values = [
		finergy.generate_hash(length=10),
		creation_time,
		creation_time,
		user,
		modified_by,
	...
	query = """INSERT INTO `tabUser Social Login` (`{source_cols}`)
		VALUES ({values})
	""".format(
		source_cols="`, `".join(source_cols), values=", ".join([finergy.db.escape(d) for d in values])
	)
