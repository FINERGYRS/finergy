...
def execute():
	...
	charts_to_modify = finergy.db.get_all(
		"Dashboard Chart",
	...
	for chart in charts_to_modify:
		old_filters = finergy.parse_json(chart.filters_json)
		...
		if chart.filters_json and isinstance(old_filters, dict):
			...
			finergy.db.set_value("Dashboard Chart", chart.name, "filters_json", new_filters_json)
