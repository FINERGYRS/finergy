...
def execute():
	...
	"""
	finergy.reload_doc("email", "doctype", "email_domain", force=True)
	finergy.reload_doc("email", "doctype", "email_account", force=True)
...
def setup_incoming_email_port_in_email_domains():
	email_domains = finergy.db.get_all(
...
def setup_incoming_email_port_in_email_accounts():
	email_accounts = finergy.db.get_all(
