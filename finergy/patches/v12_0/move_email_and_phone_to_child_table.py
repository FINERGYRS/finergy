...
def execute():
	...
	contact_details = finergy.db.sql(
		"""
		SELECT
	...
	for count, contact_detail in enumerate(contact_details):
		...
		if contact_detail.email_id:
			email_values.append(
				(
					1,
					finergy.generate_hash(contact_detail.email_id, 10),
	...
	finergy.db.add_index("Contact Phone", ["phone"])
	finergy.db.add_index("Contact Email", ["email_id"])
