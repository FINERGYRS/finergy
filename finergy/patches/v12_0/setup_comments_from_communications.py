...
def execute():
	finergy.reload_doctype("Comment")

	if finergy.db.count("Communication", filters=dict(communication_type="Comment")) > 20000:
		finergy.db.auto_commit_on_many_writes = True

	for comment in finergy.get_all(
		"Communication", fields=["*"], filters=dict(communication_type="Comment")
	):

		new_comment = finergy.new_doc("Comment")
	...
	# clean up
	finergy.db.sql("delete from `tabCommunication` where communication_type = 'Comment'")
