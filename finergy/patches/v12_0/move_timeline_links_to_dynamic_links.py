		...
		if communication.timeline_doctype and communication.timeline_name:
			...
			values.append(
				"""({0}, "{1}", "timeline_links", "Communication", "{2}", "{3}", "{4}", "{5}", "{6}", "{7}")""".format(
					counter,
					str(name),
					finergy.db.escape(communication.name),
					finergy.db.escape(communication.timeline_doctype),
					finergy.db.escape(communication.timeline_name),
		...
		if communication.link_doctype and communication.link_name:
			...
			values.append(
				"""({0}, "{1}", "timeline_links", "Communication", "{2}", "{3}", "{4}", "{5}", "{6}", "{7}")""".format(
					...
					str(name),
					finergy.db.escape(communication.name),
					finergy.db.escape(communication.link_doctype),
					finergy.db.escape(communication.link_name),
					communication.creation,
					communication.modified,
					communication.modified_by,
				)
