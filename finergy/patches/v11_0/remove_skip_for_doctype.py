...
from finergy.patches.v11_0.replicate_old_user_permissions import get_doctypes_to_skip
...
def execute():
	finergy.reload_doctype("User Permission")

	# to check if we need to migrate from skip_for_doctype
	has_skip_for_doctype = finergy.db.has_column("User Permission", "skip_for_doctype")
	...
	for user_permission in finergy.get_all("User Permission", fields=["*"]):
		skip_for_doctype = []
		...
		if skip_for_doctype:
			...
			finergy.db.set_value("User Permission", user_permission.name, "apply_to_all_doctypes", 1)
