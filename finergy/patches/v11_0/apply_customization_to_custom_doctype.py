...
def execute():
	custom_doctypes = finergy.get_all("DocType", filters={"custom": 1})

	for doctype in custom_doctypes:
		property_setters = finergy.get_all(
			"Property Setter",
			filters={"doc_type": doctype.name, "doctype_or_field": "DocField"},
			fields=["name", "property", "value", "property_type", "field_name"],
		)

		custom_fields = finergy.get_all("Custom Field", filters={"dt": doctype.name}, fields=["*"])

		property_setter_map = {}

		for prop in property_setters:
			property_setter_map[prop.field_name] = prop
			finergy.db.sql("DELETE FROM `tabProperty Setter` WHERE `name`=%s", prop.name)

		meta = finergy.get_meta(doctype.name)
		...
		for cf in custom_fields:
			...
			if field:
				...
				df = finergy.new_doc("DocField", meta, "fields")
				df.update(cf)
				meta.fields.append(df)
			finergy.db.sql("DELETE FROM `tabCustom Field` WHERE name=%s", cf.name)
