...
def execute():
	...
	"""
	finergy.reload_doctype("DocType Link")
	if finergy.db.has_table("Custom Link"):
		for custom_link in finergy.get_all("Custom Link", ["name", "document_type"]):
			finergy.db.sql(
				"update `tabDocType Link` set custom=1, parent=%s where parent=%s",
				(custom_link.document_type, custom_link.name),
			)

		finergy.delete_doc("DocType", "Custom Link")
