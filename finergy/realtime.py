...
def publish_realtime(
	...
	if event is None:
		if getattr(finergy.local, "task_id", None):
			event = "task_progress"
		else:
			event = "global"
	...
	if not room:
		if not task_id and hasattr(finergy.local, "task_id"):
			task_id = finergy.local.task_id
	...
	if after_commit:
		...
		if not params in finergy.local.realtime_log:
			finergy.local.realtime_log.append(params)
	else:
...
def emit_via_redis(event, message, room):
	...
	try:
		r.publish("events", finergy.as_json({"event": event, "message": message, "room": room}))
...
def get_redis_server():
	...
	if not redis_server:
		...
		redis_server = Redis.from_url(finergy.conf.redis_socketio or "redis://localhost:12311")
	return redis_server
...
def can_subscribe_doc(doctype, docname):
	...
	from finergy.exceptions import PermissionError
	from finergy.sessions import Session

	session = Session(None, resume=True).get_session_data()
	if not finergy.has_permission(user=session.user, doctype=doctype, doc=docname, ptype="read"):
...
def get_doc_room(doctype, docname):
	return "".join([finergy.local.site, ":doc:", doctype, "/", docname])


def get_user_room(user):
	return "".join([finergy.local.site, ":user:", user])


def get_site_room():
	return "".join([finergy.local.site, ":all"])


def get_task_progress_room(task_id):
	return "".join([finergy.local.site, ":task_progress:", task_id])


def get_chat_room(room):
	room = "".join([finergy.local.site, ":room:", room])
