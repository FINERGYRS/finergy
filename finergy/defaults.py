...
import finergy
from finergy.cache_manager import clear_defaults_cache, common_default_keys
...
def set_user_default(key, value, user=None, parenttype=None):
	set_default(key, value, user or finergy.session.user, parenttype)


def add_user_default(key, value, user=None, parenttype=None):
	add_default(key, value, user or finergy.session.user, parenttype)


def get_user_default(key, user=None):
	user_defaults = get_defaults(user or finergy.session.user)
	...
	if is_a_user_permission_key(key):
		if d and isinstance(d, (list, tuple)) and len(d) == 1:
			...
			d = user_defaults.get(finergy.scrub(key), None)
...
def is_a_user_permission_key(key):
	return ":" not in key and key != finergy.scrub(key)
...
def not_in_user_permission(key, value, user=None):
	...
	user_permission = get_user_permissions(user).get(finergy.unscrub(key)) or []
...
def get_user_permissions(user=None):
	...
	"""Return finergy.core.doctype.user_permissions.user_permissions._get_user_permissions (kept for backward compatibility)"""
...
def get_defaults(user=None):
	...
	if not user:
		user = finergy.session.user if finergy.session else "Guest"
...
def clear_user_default(key, user=None):
	clear_default(key, parent=user or finergy.session.user)
...
def get_defaults_for(parent="__default"):
	"""get all defaults"""
	defaults = finergy.cache().hget("defaults", parent)
	...
	if defaults == None:
		...
		finergy.cache().hset("defaults", parent, defaults)
...
def _clear_cache(parent):
	if parent in common_default_keys:
		...
		finergy.clear_cache(user=parent)
