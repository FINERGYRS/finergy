...
def get_context(context):
	doc = finergy.get_doc("Contact Us Settings", "Contact Us Settings")
...
def send_message(subject="Website Query", message="", sender=""):
	if not message:
		finergy.response["message"] = "Please write something"
		return

	if not sender:
		finergy.response["message"] = "Email Address Required"
		return
	...
	# send email
	forward_to_email = finergy.db.get_value("Contact Us Settings", None, "forward_to_email")
	if forward_to_email:
		finergy.sendmail(recipients=forward_to_email, sender=sender, content=message, subject=subject)
