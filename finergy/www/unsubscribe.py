...
import finergy
from finergy.email.doctype.newsletter.newsletter import confirmed_unsubscribe
...
def get_context(context):
	...
	if "email" in finergy.form_dict and finergy.request.method == "GET":
		if verify_request():
			user_email = finergy.form_dict["email"]
			context.email = user_email
			title = finergy.form_dict["name"]
	...
	elif "user_email" in finergy.form_dict and finergy.request.method == "POST":
		context.status = "unsubscribed"
		email = finergy.form_dict["user_email"]
		email_group = get_email_groups(email)
		for group in email_group:
			if group.email_group in finergy.form_dict:
