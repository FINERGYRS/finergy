...
from finergy.model.document import Document, get_controller
from finergy.utils import cint, quoted
from finergy.website.render import resolve_path
...
def get_context(context, **dict_params):
	...
	finergy.local.form_dict.update(dict_params)
	doctype = finergy.local.form_dict.doctype
	context.parents = [{"route": "me", "title": _("My Account")}]
	context.meta = finergy.get_meta(doctype)
	context.update(get_list_context(context, doctype) or {})
	context.doctype = doctype
	context.txt = finergy.local.form_dict.txt
	context.update(get(**finergy.local.form_dict))
...
def get(doctype, txt=None, limit_start=0, limit=20, pathname=None, **kwargs):
	...
	list_context = finergy.flags.list_context
	...
	for doc in raw_result:
		doc.doctype = doctype
		new_context = finergy._dict(doc=doc, meta=meta, list_view_fields=list_view_fields)

		if not list_context.get_list and not isinstance(new_context.doc, Document):
			new_context.doc = finergy.get_doc(doc.doctype, doc.name)
			new_context.update(new_context.doc.as_dict())

		if not finergy.flags.in_test:
			pathname = pathname or finergy.local.request.path
			new_context["pathname"] = pathname.strip("/ ")
		new_context.update(list_context)
		set_route(new_context)
		rendered_row = finergy.render_template(row_template, new_context, is_path=True)
...
def get_list_data(
	...
	if not txt and finergy.form_dict.search:
		txt = finergy.form_dict.search
		del finergy.form_dict["search"]

	controller = get_controller(doctype)
	meta = finergy.get_meta(doctype)

	filters = prepare_filters(doctype, controller, kwargs)
	list_context = get_list_context(finergy._dict(), doctype, web_form_name)
	...
	finergy.flags.list_context = list_context
...
def prepare_filters(doctype, controller, kwargs):
	...
	if filters.pathname:
		...
		for key, val in finergy.local.form_dict.items():
...
def get_list_context(context, doctype, web_form_name=None):
	...
	list_context = context or finergy._dict()
	meta = finergy.get_meta(doctype)
	...
	def update_context_from_module(module, list_context):
		...
		if hasattr(module, "get_list_context"):
			out = finergy._dict(module.get_list_context(list_context) or {})
			if out:
				list_context = out
		return list_context
	...
	if meta.custom and web_form_name:
		webform_list_contexts = finergy.get_hooks("webform_list_context")
		if webform_list_contexts:
			out = finergy._dict(finergy.get_attr(webform_list_contexts[0])(meta.module) or {})
			if out:
				list_context = out
