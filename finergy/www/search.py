...
from finergy.utils.global_search import web_search


def get_context(context):
	context.no_cache = 1
	if finergy.form_dict.q:
		query = str(utils.escape(sanitize_html(finergy.form_dict.q)))
		context.title = _("Search Results for ")
		context.query = query
		context.route = "/search"
		context.update(get_search_results(query, finergy.utils.sanitize_html(finergy.form_dict.scope)))
	else:
...
def get_search_results(text, scope=None, start=0, as_html=False):
	...
	if as_html:
		out.results = finergy.render_template("templates/includes/search_result.html", out)
