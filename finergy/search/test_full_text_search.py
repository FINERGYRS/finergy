...
class TestFullTextSearch(unittest.TestCase):
	...
	def test_search_term(self):
		...
		self.assertEqual(res[0], "sw/capkpi")
	...
	def test_update_index(self):
		...
		self.index.update_index({"name": "sw/capkpi", "content": """AwesomeCapKPI"""})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue("sw/capkpi" not in res)

		res = self.index.search("AwesomeCapKPI")
		self.assertEqual(res[0], "sw/capkpi")

		# Update new doc
		self.index.update_index({"name": "sw/finergybooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/finergybooks")
...
def get_index():
	return TestWrapper("test_finergy_index")
...
def get_documents():
	...
	docs.append(
		{
			"name": "sw/capkpi",
			"content": """CapKPI is a free and open-source integrated Enterprise Resource Planning software developed by
			Finergy Reporting Solutions SAS and is built on MariaDB database system using a Python based server-side framework.
			CapKPI is a generic ERP software used by manufacturers, distributors and services companies. CommonSearchTerm""",
		}
	)
	...
	docs.append(
		{
			"name": "sw/finergy",
			"content": """Finergy Framework is a full-stack web framework, that includes everything you need to build and
