...
from finergy.utils import set_request, update_progress_bar
...
class WebsiteSearch(FullTextSearch):
	...
	def get_document_to_index(self, route):
		...
		try:
			...
			return finergy._dict(title=title, content=text_content, path=route)
		except Exception:
			pass
		finally:
...
def slugs_with_web_view():
	...
	doctype_with_web_views = finergy.get_all("DocType", filters=filters, fields=fields)

	for doctype in doctype_with_web_views:
		if doctype.is_published_field:
			routes = finergy.get_all(doctype.name, filters={doctype.is_published_field: 1}, fields="route")
