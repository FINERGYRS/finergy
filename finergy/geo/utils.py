...
def merge_location_features_in_one(coords):
	...
	for element in coords:
		geojson_loc = finergy.parse_json(element["location"])
		if not geojson_loc:
			continue
...
def return_location(doctype, filters_sql):
	...
	if filters_sql:
		try:
			coords = finergy.db.sql(
				"""SELECT name, location FROM `tab{}`  WHERE {}""".format(doctype, filters_sql), as_dict=True
			)
		except InternalError:
			finergy.msgprint(finergy._("This Doctype does not contain location fields"), raise_exception=True)
			return
	else:
		coords = finergy.get_all(doctype, fields=["name", "location"])
	return coords
...
def return_coordinates(doctype, filters_sql):
	...
	if filters_sql:
		try:
			...
			finergy.msgprint(
				finergy._("This Doctype does not contain latitude and longitude fields"), raise_exception=True
			)
			return
	else:
		coords = finergy.get_all(doctype, fields=["name", "latitude", "longitude"])
	return coords
...
def get_coords_conditions(doctype, filters=None):
	...
	if not finergy.has_permission(doctype):
		finergy.throw(finergy._("Not Permitted"), finergy.PermissionError)
