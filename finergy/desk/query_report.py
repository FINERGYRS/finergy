...
from finergy.core.utils import ljust_list
from finergy.model.utils import render_include
from finergy.modules import get_module_path, scrub
from finergy.permissions import get_role_permissions
...
def get_report_doc(report_name):
	doc = finergy.get_doc("Report", report_name)
	doc.custom_columns = []

	if doc.report_type == "Custom Report":
		custom_report_doc = doc
		reference_report = custom_report_doc.reference_report
		doc = finergy.get_doc("Report", reference_report)
	...
	if not finergy.has_permission(doc.ref_doctype, "report"):
		finergy.throw(
	...
	if doc.disabled:
		finergy.throw(_("Report {0} is disabled").format(report_name))
...
def generate_report_result(
	...
	return {
		...
		"status": None,
		"execution_time": finergy.cache().hget("report_execution_time", report.name) or 0,
	}
...
def background_enqueue_run(report_name, filters=None, user=None):
	...
	track_instance = finergy.get_doc(
		{
...
def get_script(report_name):
	report = get_report_doc(report_name)
	module = report.module or finergy.db.get_value("DocType", report.ref_doctype, "module")

	is_custom_module = finergy.get_cached_value("Module Def", module, "custom")
	...
	if not script:
		script = "finergy.query_reports['%s']={}" % report_name

	# load translations
	if finergy.lang != "en":
		send_translations(finergy.get_lang_dict("report", report_name))

	return {
		"script": render_include(script),
		"html_format": html_format,
		"execution_time": finergy.cache().hget("report_execution_time", report_name) or 0,
	}
...
def run(
	...
	if not user:
		user = finergy.session.user
	if not finergy.has_permission(report.ref_doctype, "report"):
		finergy.msgprint(
...
def get_prepared_report_result(report, filters, dn="", user=None):
	...
	if dn:
		# Get specified dn
		doc = finergy.get_doc("Prepared Report", dn)
	else:
		...
		if doc_list:
			# Get latest
			doc = finergy.get_doc("Prepared Report", doc_list[0])

	if doc:
		try:
			# Prepared Report data is stored in a GZip compressed JSON file
			attached_file_name = finergy.db.get_value(
				"File",
				{"attached_to_doctype": doc.doctype, "attached_to_name": doc.name},
				"name",
			)
			attached_file = finergy.get_doc("File", attached_file_name)
			...
			finergy.delete_doc("Prepared Report", doc.name)
			finergy.db.commit()
			doc = None
...
def export_query():
	...
	if data.get("report_name"):
		...
		finergy.permissions.can_export(
			finergy.get_cached_value("Report", report_name, "ref_doctype"),
			raise_exception=True,
		)

	file_format_type = data.get("file_format_type")
	custom_columns = finergy.parse_json(data.get("custom_columns", "[]"))
	...
	if file_format_type == "Excel":
		...
		finergy.response["filename"] = report_name + ".xlsx"
...
def get_data_for_custom_field(doctype, fieldname, field=None):
	...
	if not finergy.has_permission(doctype, "read"):
	...
	value_map = finergy._dict(finergy.get_all(doctype, fields=["name", custom_field], as_list=1))
...
def save_report(reference_report, report_name, columns):
	...
	if docname:
		report = finergy.get_doc("Report", docname)
		...
		report.save()
		finergy.msgprint(_("Report updated successfully"))

		return docname
	else:
		new_report = finergy.get_doc(
			{
				"doctype": "Report",
				"report_name": report_name,
		...
		finergy.msgprint(_("{0} saved successfully").format(new_report.name))
...
def get_filtered_data(ref_doctype, columns, data, user):
	...
	shared = finergy.share.get_shared(ref_doctype, user)
	columns_dict = get_columns_dict(columns)

	role_permissions = get_role_permissions(finergy.get_meta(ref_doctype), user)
			...
			for match_filters in filter_list:
				...
				for dt, idx in linked_doctypes.items():
					...
					if (
						...
						and finergy.db.exists(dt, cell_value)
					):
						match = False
						break
...
def get_column_as_dict(col):
	...
	if isinstance(col, string_types):
		...
		col_dict["fieldname"] = finergy.scrub(col[0])

	# dict
	else:
		col_dict.update(col)
		if "fieldname" not in col_dict:
			col_dict["fieldname"] = finergy.scrub(col_dict["label"])
...
def get_user_match_filters(doctypes, user):
	...
	for dt in doctypes:
		filter_list = finergy.desk.reportview.build_match_conditions(dt, user, False)
		if filter_list:
