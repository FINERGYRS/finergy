...
# Author - Shivam Mishra <shivam@finergy-rs.fr>
...
import finergy
from finergy import DoesNotExistError, ValidationError, _, _dict
from finergy.boot import get_allowed_pages, get_allowed_reports
from finergy.cache_manager import (
...
class Workspace:
	def __init__(self, page_name, minimal=False):
		...
		self.restricted_doctypes = (
			finergy.cache().get_value("domain_restricted_doctypes") or build_domain_restriced_doctype_cache()
		)
		self.restricted_pages = (
			finergy.cache().get_value("domain_restricted_pages") or build_domain_restriced_page_cache()
		)
	...
	def is_page_allowed(self):
		...
		def _in_active_domains(item):
			if not item.restrict_to_domain:
				...
				return item.restrict_to_domain in finergy.get_active_domains()
	...
	def get_cached(self, cache_key, fallback_fn):
		...
		value = _cache.get_value(cache_key, user=finergy.session.user)
		if value:
			return value
		...
		_cache.set_value(cache_key, value, finergy.session.user, 21600)
		return value
	...
	def get_page_for_user(self):
		filters = {"extends": self.page_name, "for_user": finergy.session.user}
		user_pages = finergy.get_all("Workspace", filters=filters, limit=1)
		if user_pages:
			return finergy.get_cached_doc("Workspace", user_pages[0])

		filters = {"extends_another_page": 1, "extends": self.page_name, "is_default": 1}
		default_page = finergy.get_all("Workspace", filters=filters, limit=1)
		if default_page:
			return finergy.get_cached_doc("Workspace", default_page[0])

		self.get_pages_to_extend()
		return finergy.get_cached_doc("Workspace", self.page_name)

	def get_onboarding_doc(self):
		# Check if onboarding is enabled
		if not finergy.get_system_settings("enable_onboarding"):
			return None

		if not self.doc.onboarding:
			return None

		if finergy.db.get_value("Module Onboarding", self.doc.onboarding, "is_complete"):
			return None

		doc = finergy.get_doc("Module Onboarding", self.doc.onboarding)

		# Check if user is allowed
		allowed_roles = set(doc.get_allowed_roles())
		user_roles = set(finergy.get_roles())
	...
	def get_pages_to_extend(self):
		pages = finergy.get_all(
			...
			filters={
				...
				"restrict_to_domain": ["in", finergy.get_active_domains()],
				"for_user": "",
				"module": ["in", self.allowed_modules],
			},
		)

		pages = [finergy.get_cached_doc("Workspace", page["name"]) for page in pages]
	...
	def _doctype_contains_a_record(self, name):
		...
		if not exists and finergy.db.exists(name):
			if not finergy.db.get_value("DocType", name, "issingle"):
				exists = bool(finergy.db.get_all(name, limit=1))
			else:
				exists = True
	...
	def get_links(self):
		...
		default_country = finergy.db.get_default("country")
	...
	def get_charts(self):
		all_charts = []
		if finergy.has_permission("Dashboard Chart", throw=False):
			charts = self.doc.charts
			if len(self.extended_charts):
				charts = charts + self.extended_charts

			for chart in charts:
				if finergy.has_permission("Dashboard Chart", doc=chart.chart_name):
					# Translate label
	...
	def get_onboarding_steps(self):
		...
		for doc in self.onboarding_doc.get_steps():
			...
			if step.action == "Create Entry":
				step.is_submittable = finergy.db.get_value(
...
def get_desk_sidebar_items():
	...
	blocked_modules = finergy.get_doc("User", finergy.session.user).get_blocked_modules()
...
def get_table_with_counts():
	counts = finergy.cache().get_value("information_schema:counts")
	if not counts:
...
def get_custom_workspace_for_user(page):
	...
	"""
	filters = {"extends": page, "for_user": finergy.session.user}
	pages = finergy.get_list("Workspace", filters=filters)
	if pages:
		return finergy.get_doc("Workspace", pages[0])
	doc = finergy.new_doc("Workspace")
	doc.extends = page
	doc.for_user = finergy.session.user
	return doc
...
def save_customization(page, config):
	...
	"""
	original_page = finergy.get_doc("Workspace", page)
	...
	# Set label
	page_doc.label = page + "-" + finergy.session.user
	...
	try:
		...
		finergy.log_error(log, _("Could not save customization"))
		return False
...
def update_onboarding_step(name, field, value):
	...
	"""
	finergy.db.set_value("Onboarding Step", name, field, value)
