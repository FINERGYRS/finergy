...
from finergy.desk.doctype.desktop_icon.desktop_icon import clear_desktop_icons_cache, set_hidden
...
def hide_module(module):
	set_hidden(module, finergy.session.user, 1)
...
def filter_by_restrict_to_domain(data):
	...
	pages = finergy.cache().get_value("domain_restricted_pages") or build_domain_restriced_page_cache()
...
def build_standard_config(module, doctype_info):
	...
	if not finergy.db.get_value("Module Def", module):
		finergy.throw(_("Module Not Found"))
...
def get_disabled_reports():
	if not hasattr(finergy.local, "disabled_reports"):
		finergy.local.disabled_reports = set(r.name for r in finergy.get_all("Report", {"disabled": 1}))
	return finergy.local.disabled_reports


def get_config(app, module):
	"""Load module info from `[app].config.[module]`."""
	config = finergy.get_module("{app}.config.{module}".format(app=app, module=module))
...
def config_exists(app, module):
	try:
		finergy.get_module("{app}.config.{module}".format(app=app, module=module))
		return True
	except ImportError:
		return False
...
def get_links(app, module):
	try:
		sections = get_config(app, finergy.scrub(module))
	except ImportError:
		return []
...
def get_desktop_settings():
	...
	def apply_user_saved_links(module):
		...
		if module.module_name in user_saved_links_by_module:
			user_links = finergy.parse_json(user_saved_links_by_module[module.module_name])
...
def update_hidden_modules(category_map):
	category_map = finergy.parse_json(category_map)
	home_settings = get_home_settings()

	saved_hidden_modules = home_settings.hidden_modules or []

	for category in category_map:
		config = finergy._dict(category_map[category])
...
def update_global_hidden_modules(modules):
	modules = finergy.parse_json(modules)
	finergy.only_for("System Manager")

	doc = finergy.get_doc("User", "Administrator")
...
def update_links_for_module(module_name, links):
	links = finergy.parse_json(links)
	home_settings = get_home_settings()
...
def get_options_for_global_modules():
	...
	blocked_modules = finergy.get_doc("User", "Administrator").get_blocked_modules()
...
def set_home_settings(home_settings):
	finergy.cache().hset("home_settings", finergy.session.user, home_settings)
	finergy.db.set_value("User", finergy.session.user, "home_settings", json.dumps(home_settings))


@finergy.whitelist()
def get_home_settings():
	def get_from_db():
		settings = finergy.db.get_value("User", finergy.session.user, "home_settings")
		return finergy.parse_json(settings or "{}")

	home_settings = finergy.cache().hget("home_settings", finergy.session.user, get_from_db)
...
def get_last_modified(doctype):
	...
	last_modified = finergy.cache().hget("last_modified", doctype, _get)
