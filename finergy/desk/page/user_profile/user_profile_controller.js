import BaseTimeline from "../../../public/js/finergyrs/form/footer/base_timeline";
finergy.provide('finergy.energy_points');

class UserProfile {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = finergy.ui.make_app_page({
			parent: wrapper,
		});
	...
	show() {
		...
		this.user_id = route[1] || finergy.session.user;
		finergy.dom.freeze(__('Loading user profile') + '...');
		finergy.db.exists('User', this.user_id).then(exists => {
			finergy.dom.unfreeze();
			if (exists) {
				this.make_user_profile();
			} else {
				finergy.msgprint(__('User does not exist'));
			}
		});
	}

	make_user_profile() {
		this.user = finergy.user_info(this.user_id);
		this.page.set_title(this.user.fullname);
		this.setup_user_search();
		this.main_section.empty().append(finergy.render_template('user_profile'));
	...
	show_user_search_dialog() {
		let dialog = new finergy.ui.Dialog({
			...
			primary_action: ({ user }) => {
				dialog.hide();
				finergy.set_route('user-profile', user);
			}
		});
		dialog.show();
	}
	...
	render_heatmap() {
		this.heatmap = new finergy.Chart('.performance-heatmap', {
			type: 'heatmap',
	...
	update_heatmap_data(date_from) {
		finergy.xcall('finergy.desk.page.user_profile.user_profile.get_energy_points_heatmap_data', {
			user: this.user_id,
			date: date_from || finergy.datetime.year_start(),
		}).then((r) => {
	...
	render_line_chart() {
		...
		this.line_chart = new finergy.Chart('.performance-line-chart', {
			type: 'line',
			height: 200,
			data: {
				labels: [],
	...
	update_line_chart_data() {
		...
		finergy.xcall('finergy.desk.doctype.dashboard_chart.dashboard_chart.get', {
	...
	render_percentage_chart(field, title) {
		finergy.xcall('finergy.desk.page.user_profile.user_profile.get_energy_points_percentage_chart_data', {
			user: this.user_id,
			field: field
		}).then(chart => {
			if (chart.labels.length) {
				this.percentage_chart = new finergy.Chart('.performance-percentage-chart', {
					type: 'percentage',
					data: {
						labels: chart.labels,
	...
	create_line_chart_filters() {
		...
		finergy.dashboard_utils.render_chart_filters(filters, 'chart-filter', '.line-chart-options', 1);
	}
	...
	create_percentage_chart_filters() {
		...
		finergy.dashboard_utils.render_chart_filters(filters, 'chart-filter', '.percentage-chart-options');
	}

	create_heatmap_chart_filters() {
		let filters = [
			{
				label: finergy.dashboard_utils.get_year(finergy.datetime.now_date()),
				options: finergy.dashboard_utils.get_years_since_creation(finergy.boot.user.creation),
				action: (selected_item) => {
					this.update_heatmap_data(finergy.datetime.obj_to_str(selected_item));
				}
			},
		];
		finergy.dashboard_utils.render_chart_filters(filters, 'chart-filter', '.heatmap-options');
	}


	edit_profile() {
		let edit_profile_dialog = new finergy.ui.Dialog({
			...
			primary_action: values => {
				...
				finergy.xcall('finergy.desk.page.user_profile.user_profile.update_profile_info', {
					profile_info: values
				}).then(user => {
	...
	render_user_details() {
		this.sidebar.empty().append(finergy.render_template('user_profile_sidebar', {
			user_image: this.user.image,
	...
	setup_user_profile_links() {
		if (this.user_id !== finergy.session.user) {
	...
	get_user_rank() {
		return finergy.xcall('finergy.desk.page.user_profile.user_profile.get_user_rank', {
			user: this.user_id,
		}).then(r => {
	...
	get_user_points() {
		return finergy.xcall(
			'finergy.social.doctype.energy_point_log.energy_point_log.get_user_energy_and_review_points',
			{
				user: this.user_id,
			}
		).then(r => {
	...
	render_points_and_rank() {
		...
		const _get_stat_dom = (value, label, icon) => {
			return `<div class="user-stats-item mt-4">
				${finergy.utils.icon(icon, "lg", "no-stroke")}
				<div>
	...
	go_to_user_settings() {
		finergy.set_route('Form', 'User', this.user_id);
	}
...
class UserProfileTimeline extends BaseTimeline {
	...
	get_user_activity_data() {
		return finergy.xcall('finergy.desk.page.user_profile.user_profile.get_energy_points_list', {
	...
	get_activity_timeline_item(data) {
		...
		return {
			...
			is_card: true,
			content: finergy.energy_points.format_history_log(data),
		};
	}
...
finergy.ui.UserProfile = UserProfile;
