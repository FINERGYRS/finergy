...
from finergy.utils import cint, get_site_path, get_url
from finergy.utils.data import convert_utc_to_user_timezone
...
def get_scheduled_backup_limit():
	backup_limit = finergy.db.get_singles_value("System Settings", "backup_limit")
...
def schedule_files_backup(user_email):
	...
	queued_jobs = get_jobs(site=finergy.local.site, queue="long")
	method = "finergy.desk.page.backups.backups.backup_files_and_notify_user"

	if method not in queued_jobs[finergy.local.site]:
		enqueue(
			"finergy.desk.page.backups.backups.backup_files_and_notify_user",
			queue="long",
			user_email=user_email,
		)
		finergy.msgprint(_("Queued for backup. You will receive an email with the download link"))
	else:
		finergy.msgprint(
...
def backup_files_and_notify_user(user_email=None):
	from finergy.utils.backups import backup
