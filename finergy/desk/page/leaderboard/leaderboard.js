finergy.pages["leaderboard"].on_page_load = (wrapper) => {
	finergy.leaderboard = new Leaderboard(wrapper);

	$(wrapper).bind('show', ()=> {
		// Get which leaderboard to show
		let doctype = finergy.get_route()[1];
		finergy.leaderboard.show_leaderboard(doctype);
	});
};
...
class Leaderboard {
	...
	get_leaderboard_config() {
		...
		finergy.xcall("finergy.desk.page.leaderboard.leaderboard.get_leaderboard_config").then(config => {
			...
			const _initial_doctype = finergy.get_route()[1] || this.doctypes[0];
	...
	setup_leaderboard_fields() {
		...
		this.type_select = this.page.add_select(__("Field"),
			this.options.selected_filter.map(d => {
				return {"label": __(finergy.model.unscrub(d)), value: d };
			})
		);
	...
	create_date_range_field() {
		let timespan_field = $(this.parent).find(`.finergy-control[data-original-title="${__('Timespan')}"]`);
		this.date_range_field = $(`<div class="from-date-field"></div>`).insertAfter(timespan_field).hide();

		let date_field = finergy.ui.form.make_control({
			df: {
				fieldtype: 'DateRange',
				fieldname: 'selected_date_range',
				placeholder: __("Date Range"),
				default: [finergy.datetime.month_start(), finergy.datetime.now_date()],
	...
	render_selected_doctype() {
		...
		this.$sidebar_list.on("click", "li", (e)=> {
			...
			this.options.selected_company = finergy.defaults.get_default("company");
			...
			finergy.set_route("leaderboard", this.options.selected_doctype);
			this.make_request();
		});
	}
	...
	make_request() {
		...
		finergy.model.with_doctype(this.options.selected_doctype, ()=> {
	...
	get_leaderboard(notify) {
		if (!this.options.selected_company) {
			finergy.throw(__("Please select Company"));
		}
		finergy.call(
			...
			finergy.utils.make_chart('.leaderboard-graph', custom_options);
	...
	get_leaderboard_data(me, res) {
		if (res && res.message.length) {
			...
			finergy.utils.setup_search($(me.parent), ".list-item-container", ".list-id");
		} else {
			me.$graph_area.hide();
	...
	render_message() {
		...
		let html = `<div class="leaderboard-empty-state ${display_class}">
			<div class="no-result text-center">
				<img src="/assets/finergyrs/images/ui-states/search-empty-state.svg"
					alt="Empty State"
					class="null-state"
				>
	...
	get_item_html(item, index) {
		...
		const value = finergy.format(item.value, fields.find(field => {
			let fieldname = field.fieldname || field;
			return fieldname === this.options.selected_filter_item;
		}));

		const link = `/app/${finergy.router.slug(this.options.selected_doctype)}/${item.name}`;
	...
	get_sidebar_item(item, icon) {
		let icon_html = icon ? finergy.utils.icon(icon, 'md') : '';
	...
	get_date_range() {
		...
		let current_date = finergy.datetime.now_date();
		let date_range_map = {
			"this week": [finergy.datetime.week_start(), finergy.datetime.week_end()],
			"this month": [finergy.datetime.month_start(), finergy.datetime.month_end()],
			"this quarter": [finergy.datetime.quarter_start(), finergy.datetime.quarter_end()],
			"this year": [finergy.datetime.year_start(), finergy.datetime.year_end()],
			"last week": [finergy.datetime.add_days(current_date, -7), current_date],
			"last month": [finergy.datetime.add_months(current_date, -1), current_date],
			"last quarter": [finergy.datetime.add_months(current_date, -3), current_date],
			"last year": [finergy.datetime.add_months(current_date, -12), current_date],
			"all time": null,
			"select date range": this.selected_date_range || [finergy.datetime.month_start(), current_date]
		}
