finergy.pages['translation-tool'].on_page_load = function(wrapper) {
	...
	finergy.translation_tool = new TranslationTool(page);
};
...
class TranslationTool {
	constructor(page) {
		...
		this.wrapper.append(finergy.render_template('translation_tool'));
	...
	setup_language_filter() {
		let languages = Object.keys(finergy.boot.lang_dict).map(language_label => {
			let value = finergy.boot.lang_dict[language_label];
			return {
		...
		if (translation_language || finergy.boot.lang !== 'en') {
			language_selector.set_value(translation_language || finergy.boot.lang);
		} else {
			finergy.prompt(
				{
	...
	fetch_messages() {
		finergy.dom.freeze(__('Fetching...'));
		return finergy
			.xcall('finergy.translate.get_messages', {
			...
			<div
				...
				<div class="bold ellipsis">
					<span class="indicator ${this.get_indicator_color(message)}">
						<span>${finergy.utils.escape_html(message.source_text)}</span>
					</span>
				</div>
			</div>
		`;
	...
	get_additional_info(source_id) {
		finergy.dom.freeze('Fetching...');
		return finergy.xcall('finergy.translate.get_source_additional_info', {
			source: source_id,
			language: this.page.fields_dict['language'].get_value()
		}).finally(finergy.dom.unfreeze);
	}

	make_edit_form(translation, { contributions, positions }) {
		if (!this.form) {
			this.form = new finergy.ui.FieldGroup({
				fields: [
					{
						fieldtype: 'HTML',
						fieldname: 'header',
	...
	show_confirmation_dialog() {
		this.confirmation_dialog = new finergy.ui.Dialog({
			fields: [
				{
					label: __('Language'),
					fieldname: 'language',
	...
	create_translations() {
		finergy.dom.freeze(__('Submitting...'));
		return finergy
			.xcall(
				'finergy.core.doctype.translation.translation.create_translations',
				{
			...
			.then(() => {
				finergy.dom.unfreeze();
				finergy.show_alert({ message: __('Successfully Submitted!'), indicator: 'success'});
				this.edited_translations = {};
				this.update_header();
				this.fetch_messages_then_render();
			})
	...
	setup_local_contributions() {
		// TODO: Refactor
		finergy
			.xcall('finergy.translate.get_contributions', {
				language: this.language
			})
			.then(messages => {
				let template = message => `
	...
	show_translation_status_modal(e, $el) {
		...
		finergy.xcall('finergy.translate.get_contribution_status', { message_id })
			.then(doc => {
	...
	get_code_url(path, line_no, app) {
		...
		return `https://github.com/finergyrs/${app}/blob/develop/${code_path}#L${line_no}`;
	}
}
