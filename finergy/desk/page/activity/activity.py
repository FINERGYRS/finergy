...
import finergy
from finergy.core.doctype.activity_log.feed import get_feed_match_conditions
...
def get_feed(start, page_length):
	"""get feed"""
	match_conditions_communication = get_feed_match_conditions(finergy.session.user, "Communication")
	match_conditions_comment = get_feed_match_conditions(finergy.session.user, "Comment")
	...
	result = finergy.db.sql(
		...
		{"user": finergy.session.user, "start": cint(start), "page_length": cint(page_length)},
		as_dict=True,
	)
