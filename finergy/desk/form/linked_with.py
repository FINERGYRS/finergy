...
from finergy.model.meta import is_single
...
def cancel_all_linked_docs(docs, ignore_doctypes_on_cancel_all=None):
	...
	for i, doc in enumerate(docs, 1):
		if validate_linked_doc(doc, ignore_doctypes_on_cancel_all):
			linked_doc = finergy.get_doc(doc.get("doctype"), doc.get("name"))
			linked_doc.cancel()
		finergy.publish_progress(percent=i / len(docs) * 100, title=_("Cancelling documents"))
...
def validate_linked_doc(docinfo, ignore_doctypes_on_cancel_all=None):
	...
	if not finergy.get_meta(docinfo.get("doctype")).is_submittable:
		return False
...
def get_exempted_doctypes():
	...
	for doctypes in finergy.get_hooks("auto_cancel_exempted_doctypes"):
...
def get_linked_docs(doctype, name, linkinfo=None, for_doctype=None):
	...
	if for_doctype:
		links = finergy.get_doc(doctype, name).get_link_filters(for_doctype)
	...
	me = finergy.db.get_value(doctype, name, ["parenttype", "parent"], as_dict=True)

	for dt, link in linkinfo.items():
		filters = []
		link["doctype"] = dt
		link_meta_bundle = finergy.desk.form.load.get_meta_bundle(dt)
		...
		if not linkmeta.get("issingle"):
			fields = [
				...
				for d in linkmeta.get(
					...
					{
						"in_list_view": 1,
						"fieldtype": ["not in", ("Image", "HTML", "Button") + finergy.model.table_fields],
					},
				)
			...
			try:
				if link.get("filters"):
					ret = finergy.get_all(doctype=dt, fields=fields, filters=link.get("filters"))

				elif link.get("get_parent"):
					if me and me.parent and me.parenttype == dt:
						ret = finergy.get_all(doctype=dt, fields=fields, filters=[[dt, "name", "=", me.parent]])
					else:
						ret = None
					...
					if link_fieldnames:
						...
						ret = finergy.get_all(doctype=dt, fields=fields, filters=filters, or_filters=or_filters)
...
def get_linked_doctypes(doctype, without_ignore_user_permissions_enabled=False):
	...
	if without_ignore_user_permissions_enabled:
		...
		return finergy.cache().hget("linked_doctypes", doctype, lambda: _get_linked_doctypes(doctype))
...
def _get_linked_doctypes(doctype, without_ignore_user_permissions_enabled=False):
	...
	links = finergy.get_all("DocField", fields=["parent as dt"], filters=filters)
	links += finergy.get_all("Custom Field", fields=["dt"], filters=filters)
...
def get_linked_fields(doctype, without_ignore_user_permissions_enabled=False):
	...
	# find links of parents
	links = finergy.get_all("DocField", fields=["parent", "fieldname"], filters=filters, as_list=1)
	links += finergy.get_all(
	...
	table_doctypes = finergy.get_all(
	...
	for parent, options in finergy.get_all(
