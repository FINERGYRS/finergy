...
import finergy
from finergy.build import scrub_html_template
from finergy.model.meta import Meta
from finergy.model.utils import render_include
from finergy.modules import get_module_path, load_doctype_module, scrub
from finergy.translate import extract_messages_from_code, make_dict_from_messages
from finergy.utils import get_html_format
...
def get_meta(doctype, cached=True):
	...
	if cached and not finergy.conf.developer_mode:
		meta = finergy.cache().hget("form_meta", doctype)
		if meta:
			meta = FormMeta(meta)
		else:
			meta = FormMeta(doctype)
			finergy.cache().hset("form_meta", doctype, meta.as_dict())
	else:
		meta = FormMeta(doctype)

	if finergy.local.lang != "en":
		meta.set_translations(finergy.local.lang)
...
class FormMeta(Meta):
	...
	def add_code(self):
		...
		system_country = finergy.get_system_settings("country")
	...
	def add_search_fields(self):
		...
		for df in self.get("fields", {"fieldtype": "Link", "options": ["!=", "[Select]"]}):
			if df.options:
				search_fields = finergy.get_meta(df.options).search_fields
				if search_fields:
	...
	def add_linked_document_type(self):
		for df in self.get("fields", {"fieldtype": "Link"}):
			if df.options:
				try:
					df.linked_document_type = finergy.get_meta(df.options).document_type
				except finergy.DoesNotExistError:
					# edge case where options="[Select]"
					pass

	def load_print_formats(self):
		print_formats = finergy.db.sql(
	...
	def load_workflows(self):
		...
		if workflow_name and finergy.db.exists("Workflow", workflow_name):
			workflow = finergy.get_doc("Workflow", workflow_name)
			workflow_docs.append(workflow)

			for d in workflow.get("states"):
				workflow_docs.append(finergy.get_doc("Workflow State", d.state))
		...
		if not self.custom:
			...
			if hasattr(module, "form_grid_templates"):
				for key, path in iteritems(module.form_grid_templates):
					templates[key] = get_html_format(finergy.get_app_path(app, path))

				self.set("__form_grid_templates", templates)

	def set_translations(self, lang):
		self.set("__messages", finergy.get_lang_dict("doctype", self.name))
...
def get_code_files_via_hooks(hook, name):
	...
	for app_name in finergy.get_installed_apps():
		code_hook = finergy.get_hooks(hook, default={}, app_name=app_name)
		if not code_hook:
			continue
		...
		for file in files:
			path = finergy.get_app_path(app_name, *file.strip("/").split("/"))
