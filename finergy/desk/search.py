...
from finergy.translate import get_translated_doctypes
from finergy.utils import cint, cstr, unique
...
def sanitize_searchfield(searchfield):
	...
	def _raise_exception(searchfield):
		finergy.throw(_("Invalid Search Field {0}").format(searchfield), finergy.DataError)
...
def search_link(
	...
	finergy.response["results"] = build_for_autosuggest(finergy.response["values"])
	del finergy.response["values"]
...
def search_widget(
	...
	standard_queries = finergy.get_hooks().standard_queries or {}

	if query and query.split()[0].lower() != "select":
		# by method
		try:
			is_whitelisted(finergy.get_attr(query))
			finergy.response["values"] = finergy.call(
				query, doctype, txt, searchfield, start, page_length, filters, as_dict=as_dict
			)
		except finergy.exceptions.PermissionError as e:
			if finergy.local.conf.developer_mode:
				raise e
			else:
		...
		if query:
			finergy.throw(_("This query style is discontinued"))
			# custom query
			# finergy.response["values"] = finergy.db.sql(scrub_custom_query(query, searchfield, txt))
		else:
			...
			translated_doctypes = finergy.cache().hget(
			...
			formatted_fields.append(
				"""locate({_txt}, `tab{doctype}`.`name`) as `_relevance`""".format(
					_txt=finergy.db.escape((txt or "").replace("%", "").replace("@", "")), doctype=doctype
				)
			)

			# In order_by, `idx` gets second priority, because it stores link count
			from finergy.model.db_query import get_order_by
			...
			if as_dict:
				for r in values:
					r.pop("_relevance")
				finergy.response["values"] = values
			else:
				finergy.response["values"] = [r[:-1] for r in values]
...
def validate_and_sanitize_search_inputs(fn, instance, args, kwargs):
	...
	if kwargs["doctype"] and not finergy.db.exists("DocType", kwargs["doctype"]):
		return []
...
def get_names_for_mentions(search_term):
	users_for_mentions = finergy.cache().get_value("users_for_mentions", get_users_for_mentions)
	user_groups = finergy.cache().get_value("user_groups", get_user_groups)
	...
	for mention_data in users_for_mentions + user_groups:
		...
		mention_data["link"] = finergy.utils.get_url_to_form(
