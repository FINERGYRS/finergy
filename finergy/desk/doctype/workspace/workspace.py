...
class Workspace(Document):
	def validate(self):
		if self.is_standard and not finergy.conf.developer_mode and not disable_saving_as_standard():
			finergy.throw(_("You need to be in developer mode to edit this document"))
		validate_route_conflict(self.doctype, self.name)

		duplicate_exists = finergy.db.exists(
			"Workspace", {"name": ["!=", self.name], "is_default": 1, "extends": self.extends}
		)

		if self.is_default and self.name and duplicate_exists:
			finergy.throw(_("You can only have one default page that extends a particular standard page."))
	...
	def get_module_page_map():
		...
		pages = finergy.get_all("Workspace", fields=["name", "module"], filters=filters, as_list=1)

		return {page[1]: page[0] for page in pages if page[1]}

	def get_link_groups(self):
		cards = []
		current_card = finergy._dict(
			{
				"label": "Link",
				"type": "Card Break",
				"icon": None,
		...
		for link in self.links:
			...
			if link.type == "Card Break":
				if card_links and (
					not current_card.only_for or current_card.only_for == finergy.get_system_settings("country")
				):
...
def get_report_type(report):
	report_type = finergy.get_value("Report", report, "report_type")
