...
class Dashboard(Document):
	...
	def validate(self):
		if not finergy.conf.developer_mode and self.is_standard:
			finergy.throw(_("Cannot edit Standard Dashboards"))
		...
		if self.is_standard:
			...
			if non_standard_docs_map["Dashboard Chart"] or non_standard_docs_map["Number Card"]:
				...
				finergy.throw(message, title=_("Standard Not Set"), is_minimizable=True)
...
def get_permitted_charts(dashboard_name):
	permitted_charts = []
	dashboard = finergy.get_doc("Dashboard", dashboard_name)
	for chart in dashboard.charts:
		if finergy.has_permission("Dashboard Chart", doc=chart.chart):
			chart_dict = finergy._dict()
...
def get_permitted_cards(dashboard_name):
	...
	for card in dashboard.cards:
		if finergy.has_permission("Number Card", doc=card.card):
			permitted_cards.append(card)
	return permitted_cards


def get_non_standard_charts_in_dashboard(dashboard):
	non_standard_charts = [doc.name for doc in finergy.get_list("Dashboard Chart", {"is_standard": 0})]
	return [
...
def get_non_standard_cards_in_dashboard(dashboard):
	non_standard_cards = [doc.name for doc in finergy.get_list("Number Card", {"is_standard": 0})]
...
def get_non_standard_warning_message(non_standard_docs_map):
	...
	def get_html(docs, doctype):
		html = "<p>{}</p>".format(finergy.bold(doctype))
		for doc in docs:
