...
class Event(Document):
	def validate(self):
		...
		if (
			...
			finergy.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			finergy.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		if communications:
			for communication in communications:
				finergy.delete_doc_if_exists("Communication", communication.name)
	...
	def sync_communication(self):
		if self.event_participants:
			for participant in self.event_participants:
				...
				comms = finergy.get_all("Communication", filters=filters, fields=["name"])

				if comms:
					for comm in comms:
						communication = finergy.get_doc("Communication", comm.name)
						self.update_communication(participant, communication)
				else:
					meta = finergy.get_meta(participant.reference_doctype)
	...
	def update_communication(self, participant, communication):
		...
		communication.sender_full_name = finergy.utils.get_fullname(self.owner)
		communication.reference_doctype = self.doctype
		communication.reference_name = self.name
...
def delete_communication(event, reference_doctype, reference_docname):
	deleted_participant = finergy.get_doc(reference_doctype, reference_docname)
	...
	comms = finergy.get_list("Communication", filters=filters, fields=["name"])

	if comms:
		deletion = []
		for comm in comms:
			delete = finergy.get_doc("Communication", comm.name).delete()
...
def send_event_digest():
	...
	for user in get_enabled_system_users():
		...
		if events:
			finergy.set_user_lang(user.name, user.language)
			...
			finergy.sendmail(
				...
				subject=finergy._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[finergy._("Events in Today's Calendar"), "blue"],
			)
...
def delete_events(ref_type, ref_name, delete_event=False):
	participations = finergy.get_all(
		"Event Participants",
	...
	if participations:
		for participation in participations:
			if delete_event:
				finergy.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = finergy.get_all(
...
def set_status_of_events():
	...
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			finergy.db.set_value("Event", event.name, "status", "Closed")
