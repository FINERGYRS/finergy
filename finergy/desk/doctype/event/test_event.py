...
from finergy.test_runner import make_test_objects

test_records = finergy.get_test_records("Event")
...
class TestEvent(unittest.TestCase):
	def setUp(self):
		...
		self.test_records = finergy.get_test_records("Event")
	...
	def test_allowed_public(self):
		finergy.set_user(self.test_user)
		doc = finergy.get_doc("Event", finergy.db.get_value("Event", {"subject": "_Test Event 1"}))
		self.assertTrue(finergy.has_permission("Event", doc=doc))

	def test_not_allowed_private(self):
		finergy.set_user(self.test_user)
		doc = finergy.get_doc("Event", finergy.db.get_value("Event", {"subject": "_Test Event 2"}))
		self.assertFalse(finergy.has_permission("Event", doc=doc))

	def test_allowed_private_if_in_event_user(self):
		name = finergy.db.get_value("Event", {"subject": "_Test Event 3"})
		finergy.share.add("Event", name, self.test_user, "read")
		finergy.set_user(self.test_user)
		doc = finergy.get_doc("Event", name)
		self.assertTrue(finergy.has_permission("Event", doc=doc))
		finergy.set_user("Administrator")
		finergy.share.remove("Event", name, self.test_user)
	...
	def test_revert_logic(self):
		ev = finergy.get_doc(self.test_records[0]).insert()
		name = ev.name

		finergy.delete_doc("Event", ev.name)
	...
	def test_assign(self):
		from finergy.desk.form.assign_to import add
