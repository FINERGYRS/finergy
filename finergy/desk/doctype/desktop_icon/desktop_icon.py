...
def after_doctype_insert():
	finergy.db.add_unique("Desktop Icon", ("module_name", "owner", "standard"))
...
def get_desktop_icons(user=None):
	...
	user_icons = finergy.cache().hget("desktop_icons", user)
	...
	if not user_icons:
		...
		blocked_doctypes = finergy.get_all(
			"DocType",
		...
		standard_icons = finergy.db.get_all("Desktop Icon", fields=fields, filters={"standard": 1})
		...
		user_blocked_modules = finergy.get_doc("User", user).get_blocked_modules()
		for icon in user_icons:
		...
		finergy.cache().hset("desktop_icons", user, user_icons)
...
def add_user_icon(_doctype, _report=None, label=None, link=None, type="link", standard=0):
	...
	icon_name = finergy.db.exists(
		"Desktop Icon", {"standard": standard, "link": link, "owner": finergy.session.user}
	)

	if icon_name:
		if finergy.db.get_value("Desktop Icon", icon_name, "hidden"):
			# if it is hidden, unhide it
			finergy.db.set_value("Desktop Icon", icon_name, "hidden", 0)
			clear_desktop_icons_cache()

	else:
		idx = (
			finergy.db.sql("select max(idx) from `tabDesktop Icon` where owner=%s", finergy.session.user)[0][
				0
			]
			or finergy.db.sql("select count(*) from `tabDesktop Icon` where standard=1")[0][0]
		)

		if not finergy.db.get_value("Report", _report):
			_report = None
			userdefined_icon = finergy.db.get_value(
		...
		except finergy.UniqueValidationError as e:
			finergy.throw(_("Desktop Icon already exists"))
	...
	for i, module_name in enumerate(new_order):
		if module_name not in ("Explore",):
			if user:
				...
				name = finergy.db.get_value("Desktop Icon", {"standard": 1, "module_name": module_name})
				if name:
					icon = finergy.get_doc("Desktop Icon", name)
				else:
...
def set_desktop_icons(visible_list, ignore_duplicate=True):
	...
	if not int(finergy.defaults.get_defaults().setup_complete or 0):
		finergy.db.sql("delete from `tabDesktop Icon` where standard=0")

	# set standard as blocked and hidden if setting first active domain
	if not finergy.flags.keep_desktop_icons:
		finergy.db.sql("update `tabDesktop Icon` set blocked=0, hidden=1 where standard=1")

	# set as visible if present, or add icon
	for module_name in visible_list:
		name = finergy.db.get_value("Desktop Icon", {"module_name": module_name})
		if name:
			finergy.db.set_value("Desktop Icon", name, "hidden", 0)
		else:
			if finergy.db.exists("DocType", module_name):
				try:
...
def set_hidden(module_name, user=None, hidden=1):
	...
	if user:
		...
		if hidden and icon.custom:
			finergy.delete_doc(icon.doctype, icon.name, ignore_permissions=True)
			return

		# hidden by user
		icon.db_set("hidden", hidden)
	else:
		icon = finergy.get_doc("Desktop Icon", {"standard": 1, "module_name": module_name})
...
def get_all_icons():
	return [
		d.module_name
		for d in finergy.get_all("Desktop Icon", filters={"standard": 1}, fields=["module_name"])
	]


def clear_desktop_icons_cache(user=None):
	finergy.cache().hdel("desktop_icons", user or finergy.session.user)
	finergy.cache().hdel("bootinfo", user or finergy.session.user)
...
def get_user_copy(module_name, user=None):
	...
	desktop_icon_name = finergy.db.get_value(
		"Desktop Icon", {"module_name": module_name, "owner": user, "standard": 0}
	)

	if desktop_icon_name:
		return finergy.get_doc("Desktop Icon", desktop_icon_name)
	else:
...
def make_user_copy(module_name, user):
	...
	standard_name = finergy.db.get_value("Desktop Icon", {"module_name": module_name, "standard": 1})

	if not standard_name:
		finergy.throw(_("{0} not found").format(module_name), finergy.DoesNotExistError)

	original = finergy.get_doc("Desktop Icon", standard_name)

	desktop_icon = finergy.get_doc(
...
def sync_from_app(app):
	...
	try:
		modules = finergy.get_attr(app + ".config.desktop.get_data")() or {}
	except ImportError:
		return []
	...
	for i, m in enumerate(modules_list):
		...
		if desktop_icon_name:
			desktop_icon = finergy.get_doc("Desktop Icon", desktop_icon_name)
		else:
			# new icon
			desktop_icon = finergy.get_doc(
		...
		try:
			desktop_icon.save()
		except finergy.exceptions.UniqueValidationError:
			pass
...
def update_icons(hidden_list, user=None):
	...
	finergy.msgprint(finergy._("Updated"), indicator="green", title=_("Success"), alert=True)


def get_context(context):
	context.icons = get_user_icons(finergy.session.user)
	context.user = finergy.session.user

	if "System Manager" in finergy.get_roles():
		context.users = finergy.db.get_all(
			"User",
...
def get_module_icons(user=None):
	...
	if not user:
		icons = finergy.db.get_all("Desktop Icon", fields="*", filters={"standard": 1}, order_by="idx")
	else:
		finergy.cache().hdel("desktop_icons", user)
		icons = get_user_icons(user)

	for icon in icons:
		icon.value = finergy.db.escape(_(icon.label or icon.module_name))
...
def get_user_icons(user):
	...
	from finergy.boot import get_allowed_pages
