...
from finergy.boot import get_allowed_report_names
...
from finergy.utils import cint, get_datetime, getdate, has_common, now_datetime, nowdate
from finergy.utils.dashboard import cache_source
from finergy.utils.data import format_date
from finergy.utils.dateutils import (
...
def get_permission_query_conditions(user):
	...
	allowed_doctypes = [
		finergy.db.escape(doctype) for doctype in finergy.permissions.get_doctypes_with_read()
	]
	allowed_reports = [finergy.db.escape(report) for report in get_allowed_report_names()]
	allowed_modules = [
		finergy.db.escape(module.get("module_name")) for module in get_modules_from_all_apps_for_user()
	]
...
def has_permission(doc, ptype, user):
	...
	if doc.chart_type == "Report":
		...
		allowed_doctypes = finergy.permissions.get_doctypes_with_read()
...
def get(
	...
	filters = finergy.parse_json(filters) or finergy.parse_json(chart.filters_json)
	if not filters:
		filters = []
...
def create_dashboard_chart(args):
	args = finergy.parse_json(args)
	doc = finergy.new_doc("Dashboard Chart")
	...
	if finergy.db.exists("Dashboard Chart", args.chart_name):
...
def add_chart_to_dashboard(args):
	...
	dashboard = finergy.get_doc("Dashboard", args.dashboard)
	dashboard_link = finergy.new_doc("Dashboard Chart Link")
	dashboard_link.chart = args.chart_name or args.name

	if args.set_standard and dashboard.is_standard:
		chart = finergy.get_doc("Dashboard Chart", dashboard_link.chart)
		chart.is_standard = 1
...
def get_charts_for_user(doctype, txt, searchfield, start, page_len, filters):
	or_filters = {"owner": finergy.session.user, "is_public": 1}
...
class DashboardChart(Document):
	def on_update(self):
		finergy.cache().delete_key("chart-data:{}".format(self.name))
		if finergy.conf.developer_mode and self.is_standard:
			export_to_files(record_list=[["Dashboard Chart", self.name]], record_module=self.module)

	def validate(self):
		if not finergy.conf.developer_mode and self.is_standard:
			finergy.throw(_("Cannot edit Standard charts"))
	...
	def check_required_field(self):
		if not self.document_type:
			finergy.throw(_("Document type is required to create a dashboard chart"))

		if (
			self.document_type
			and finergy.get_meta(self.document_type).istable
			and not self.parent_document_type
		):
			finergy.throw(_("Parent document type is required to create a dashboard chart"))

		if self.chart_type == "Group By":
			if not self.group_by_based_on:
				finergy.throw(_("Group By field is required to create a dashboard chart"))
			if self.group_by_type in ["Sum", "Average"] and not self.aggregate_function_based_on:
				finergy.throw(_("Aggregate Function field is required to create a dashboard chart"))
		else:
			if not self.based_on:
				finergy.throw(_("Time series based on is required to create a dashboard chart"))

	def check_document_type(self):
		if finergy.get_meta(self.document_type).issingle:
			finergy.throw(_("You cannot create a dashboard chart from single DocTypes"))
	...
	def validate_custom_options(self):
		if self.custom_options:
			try:
				...
				finergy.throw(_("Invalid json added in the custom options: {0}").format(error))
