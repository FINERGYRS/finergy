...
finergy.ui.form.on('Dashboard Chart', {
	setup: function(frm) {
	...
	before_save: function(frm) {
		...
		static_filters =
			finergy.dashboard_utils.remove_common_static_filter_values(static_filters, dynamic_filters);
	...
	refresh: function(frm) {
		...
		frm.is_disabled = !finergy.boot.developer_mode && frm.doc.is_standard;
		...
		frm.add_custom_button('Add Chart to Dashboard', () => {
			const dialog = finergy.dashboard_utils.get_add_to_dashboard_dialog(
				frm.doc.name,
				'Dashboard Chart',
				'finergy.desk.doctype.dashboard_chart.dashboard_chart.add_chart_to_dashboard'
			);

			if (!frm.doc.chart_name) {
				finergy.msgprint(__('Please create chart first'));
			} else {
				dialog.show();
			}
		});
	...
	set_heatmap_year_options: function(frm) {
		if (frm.doc.type == 'Heatmap') {
			finergy.db.get_doc('System Settings').then(doc => {
				const creation_date = doc.creation;
				frm.set_df_property('heatmap_year', 'options', finergy.dashboard_utils.get_years_since_creation(creation_date));
			});
		}
	},
	...
	set_chart_report_filters: function(frm) {
		...
		if (report_name) {
			if (frm.doc.filters_json.length > 2) {
				...
				finergy.report_utils.get_report_filters(report_name).then(filters => {
					if (filters) {
						frm.chart_filters = filters;
						let filter_values = finergy.report_utils.get_filter_values(filters);
	...
	set_chart_field_options: function(frm) {
		...
		if (frm.doc.dynamic_filters_json && frm.doc.dynamic_filters_json.length > 2) {
			filters = finergy.dashboard_utils.get_all_filters(frm.doc);
		}
		finergy.xcall(
			'finergy.desk.query_report.run',
			{
			...
			if (!frm.doc.use_report_chart) {
				if (data.result.length) {
					frm.field_options = finergy.report_utils.get_field_options_from_report(data.columns, data);
					frm.set_df_property('x_field', 'options', frm.field_options.non_numeric_fields);
					if (!frm.field_options.numeric_fields.length) {
						finergy.msgprint(__("Report has no numeric fields, please change the Report Name"));
					} else {
						let y_field_df = finergy.meta.get_docfield('Dashboard Chart Field', 'y_field', frm.doc.name);
						y_field_df.options = frm.field_options.numeric_fields;
					}
				} else {
					finergy.msgprint(__('Report has no data, please modify the filters or change the Report Name'));
				}
			} else {
	...
	update_options: function(frm) {
		...
		if (doctype) {
			finergy.model.with_doctype(doctype, () => {
				...
				finergy.get_meta(doctype).fields.map(df => {
	...
	show_filters: function(frm) {
		frm.chart_filters = [];
		finergy.dashboard_utils.get_filters_for_chart_type(frm.doc).then(filters => {
			if (filters) {
				frm.chart_filters = filters;
			}
	...
	render_filters_table: function(frm) {
		...
		table.on('click', () => {
			frm.is_disabled && finergy.throw(__('Cannot edit filters for standard charts'));
			...
			finergy.dashboards.filters_dialog = dialog;

			if (is_document_type) {
				frm.filter_group = new finergy.ui.FilterGroup({
			...
			if (frm.doc.chart_type == 'Report') {
				...
				finergy.query_reports[frm.doc.report_name]
					&& finergy.query_reports[frm.doc.report_name].onload
						&& finergy.query_reports[frm.doc.report_name].onload(finergy.query_report);
			}
	...
	render_dynamic_filters_table(frm) {
		...
		let fields = finergy.dashboard_utils.get_fields_for_dynamic_filter_dialog(
	...
	set_parent_document_type: async function(frm) {
		...
		let doc_is_table = document_type &&
			(await finergy.db.get_value('DocType', document_type, 'istable')).message.istable;

		frm.set_df_property('parent_document_type', 'hidden', !doc_is_table);

		if (document_type && doc_is_table) {
			let parent = await finergy.db.get_list('DocField', {
				filters: {
					'fieldtype': 'Table',
