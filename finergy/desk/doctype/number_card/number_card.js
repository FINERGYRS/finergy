...
finergy.ui.form.on('Number Card', {
	...
	create_add_to_dashboard_button: function(frm) {
		frm.add_custom_button('Add Card to Dashboard', () => {
			const dialog = finergy.dashboard_utils.get_add_to_dashboard_dialog(
				frm.doc.name,
				'Number Card',
				'finergy.desk.doctype.number_card.number_card.add_card_to_dashboard'
			);

			if (!frm.doc.name) {
				finergy.msgprint(__('Please create Card first'));
			} else {
				dialog.show();
			}
		});
...
<code>
{
	...
	filters_config: function(frm) {
		frm.filters = eval(frm.doc.filters_config);
		const filter_values = finergy.report_utils.get_filter_values(frm.filters);
		...
		if (doctype) {
			finergy.model.with_doctype(doctype, () => {
				finergy.get_meta(doctype).fields.map(df => {
					if (finergy.model.numeric_fieldtypes.includes(df.fieldtype)) {
	...
	set_report_filters: function(frm) {
		...
		if (report_name) {
			finergy.report_utils.get_report_filters(report_name).then(filters => {
				if (filters) {
					frm.filters = filters;
					const filter_values = finergy.report_utils.get_filter_values(filters);
	...
	render_dynamic_filters_table(frm) {
		if (!finergy.boot.developer_mode || !frm.doc.is_standard || frm.doc.type == 'Custom') {
			return;
		}
