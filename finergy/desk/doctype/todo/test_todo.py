...
import finergy
from finergy.core.doctype.doctype.doctype import clear_permissions_cache
from finergy.model.db_query import DatabaseQuery
from finergy.permissions import add_permission, reset_perms
...
class TestToDo(unittest.TestCase):
	def test_delete(self):
		...
		finergy.db.sql("delete from `tabDeleted Document`")
		todo.delete()

		deleted = finergy.get_doc(
	...
	def test_fetch(self):
		...
		self.assertEqual(
			todo.assigned_by_full_name, finergy.db.get_value("User", todo.assigned_by, "full_name")
		)

	def test_fetch_setup(self):
		finergy.db.sql("delete from tabToDo")

		todo_meta = finergy.get_doc("DocType", "ToDo")
	...
	def test_doc_read_access(self):
		...
		test_user = finergy.get_doc("User", "test4@example.com")
...
def create_new_todo(description, assigned_by):
	...
	return finergy.get_doc(todo).insert()
