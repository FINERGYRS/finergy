...
from finergy.model.db_query import DatabaseQuery
from finergy.utils import cstr, format_duration
...
def get_list():
	...
	if is_virtual_doctype(args.doctype):
		...
		# uncompressed (refactored from finergy.model.db_query.get_list)
		data = execute(**args)
...
def get_form_params():
	...
	data = finergy._dict(finergy.local.form_dict)
	clean_params(data)
	validate_args(data)
	return data
	...
	if isinstance(filters, list):
		...
		for condition in filters:
			if len(condition) == 3:
				...
				if is_standard(fieldname):
					continue
				meta = finergy.get_meta(condition[0])
...
def setup_group_by(data):
	...
	if data.group_by and data.aggregate_function:
		if data.aggregate_function.lower() not in ("count", "sum", "avg"):
			finergy.throw(_("Invalid aggregate function"))

		if finergy.db.has_column(data.aggregate_on_doctype, data.aggregate_on_field):
			data.fields.append(
...
def raise_invalid_field(fieldname):
	finergy.throw(_("Field not permitted in query") + ": {0}".format(fieldname), finergy.DataError)
...
def get_meta_and_docfield(fieldname, data):
	...
	meta = finergy.get_meta(parenttype)
...
def update_wildcard_field_param(data):
	if (isinstance(data.fields, string_types) and data.fields == "*") or (
		...
		data.fields = finergy.db.get_table_columns(data.doctype)
		return True
...
def compress(data, args=None):
	...
	from finergy.desk.query_report import add_total_row
	...
	if args.get("add_total_row"):
		meta = finergy.get_meta(args.doctype)
...
def save_report(name, doctype, report_settings):
	...
	if finergy.db.exists("Report", name):
		report = finergy.get_doc("Report", name)
		if report.is_standard == "Yes":
			finergy.throw(_("Standard Reports cannot be edited"))

		if report.report_type != "Report Builder":
			finergy.throw(_("Only reports of type Report Builder can be edited"))

		if report.owner != finergy.session.user and not finergy.has_permission("Report", "write"):
			finergy.throw(_("Insufficient Permissions for editing Report"), finergy.PermissionError)
	else:
		report = finergy.new_doc("Report")
	...
	finergy.msgprint(
		_("Report {0} saved").format(finergy.bold(report.name)),
		indicator="green",
		alert=True,
	)
	return report.name
...
def delete_report(name):
	...
	report = finergy.get_doc("Report", name)
	if report.is_standard == "Yes":
		finergy.throw(_("Standard Reports cannot be deleted"))

	if report.report_type != "Report Builder":
		finergy.throw(_("Only reports of type Report Builder can be deleted"))

	if report.owner != finergy.session.user and not finergy.has_permission("Report", "delete"):
		finergy.throw(_("Insufficient Permissions for deleting Report"), finergy.PermissionError)

	report.delete(ignore_permissions=True)
	finergy.msgprint(
		_("Report {0} deleted").format(finergy.bold(report.name)),
		indicator="green",
		alert=True,
	)
...
def export_query():
	...
	title = finergy.form_dict.title
	finergy.form_dict.pop("title", None)
	...
	finergy.permissions.can_export(doctype, raise_exception=True)

	if "selected_items" in form_params:
		si = json.loads(finergy.form_dict.get("selected_items"))
	...
	if file_format_type == "CSV":
		...
		from finergy.utils.xlsxutils import handle_html
		...
		for r in data:
			...
			writer.writerow(
				[handle_html(finergy.as_unicode(v)) if isinstance(v, string_types) else v for v in r]
			)

		f.seek(0)
		finergy.response["result"] = cstr(f.read())
		finergy.response["type"] = "csv"
		finergy.response["doctype"] = title

	elif file_format_type == "Excel":

		from finergy.utils.xlsxutils import make_xlsx

		xlsx_file = make_xlsx(data, doctype)

		finergy.response["filename"] = title + ".xlsx"
		finergy.response["filecontent"] = xlsx_file.getvalue()
		finergy.response["type"] = "binary"
...
def get_labels(fields, doctype):
	...
	for key in fields:
		...
		df = finergy.get_meta(parenttype).get_field(fieldname)
...
def delete_items():
	...
	items = sorted(json.loads(finergy.form_dict.get("items")), reverse=True)
	doctype = finergy.form_dict.get("doctype")

	if len(items) > 10:
		finergy.enqueue("finergy.desk.reportview.delete_bulk", doctype=doctype, items=items)
	else:
...
def get_stats(stats, doctype, filters=None):
	...
	except (finergy.db.InternalError, finergy.db.ProgrammingError):
	...
	for tag in tags:
		...
		except finergy.db.SQLError:
			pass
		except finergy.db.InternalError as e:
			# raised when _user_tags column is added on the fly
			pass
...
def is_virtual_doctype(doctype):
	return finergy.db.get_value("DocType", doctype, "is_virtual")
