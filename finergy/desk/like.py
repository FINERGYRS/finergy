...
def _toggle_like(doctype, name, add, user=None):
	...
	try:
		liked_by = finergy.db.get_value(doctype, name, "_liked_by")
		...
		finergy.db.set_value(doctype, name, "_liked_by", json.dumps(liked_by), update_modified=False)
