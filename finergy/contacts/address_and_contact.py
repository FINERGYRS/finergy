...
def load_address_and_contact(doc, key=None):
	...
	from finergy.contacts.doctype.address.address import get_address_display, get_condensed_address
	...
	address_list = finergy.get_list("Address", filters=filters, fields=["*"], order_by="creation asc")
	...
	contact_list = finergy.get_all("Contact", filters=filters, fields=["*"])

	for contact in contact_list:
		contact["email_ids"] = finergy.get_list(
			"Contact Email",
			filters={"parenttype": "Contact", "parent": contact.name, "is_primary": 0},
			fields=["email_id"],
		)

		contact["phone_nos"] = finergy.get_list(
			"Contact Phone",
			filters={
				"parenttype": "Contact",
				"parent": contact.name,
		...
		if contact.address:
			address = finergy.get_doc("Address", contact.address)
...
def has_permission(doc, ptype, user):
	...
	for df in links.get("permitted_links") + links.get("not_permitted_links"):
		...
		if name and finergy.has_permission(doctype, ptype, doc=name):
			return True
...
def delete_contact_and_address(doctype, docname):
	for parenttype in ("Contact", "Address"):
		...
		for name in items:
			doc = finergy.get_doc(parenttype, name)
			if len(doc.links) == 1:
				doc.delete()
...
def filter_dynamic_link_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	_doctypes = finergy.db.get_all("Custom Field", filters=filters, fields=["dt"], as_list=True)
...
def set_link_title(doc):
	...
	for link in doc.links:
		if not link.link_title:
			linked_doc = finergy.get_doc(link.link_doctype, link.link_name)
