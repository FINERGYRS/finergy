...
from finergy.utils import cint, cstr, has_gravatar
...
class Contact(Document):
	def autoname(self):
		...
		if finergy.db.exists("Contact", self.name):
	...
	def validate(self):
		...
		if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
			finergy.throw(_("Select Google Contacts to which contact should be synced."))

		deduplicate_dynamic_links(self)

	def set_user(self):
		if not self.user and self.email_id:
			self.user = finergy.db.get_value("User", {"email": self.email_id})
	...
	def add_email(self, email_id, is_primary=0, autosave=False):
		if not finergy.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
	...
	def add_phone(self, phone, is_primary_phone=0, is_primary_mobile_no=0, autosave=False):
		if not finergy.db.exists("Contact Phone", {"phone": phone, "parent": self.name}):
			self.append(
				"phone_nos",
				{
					"phone": phone,
	...
	def set_primary_email(self):
		...
		if len([email.email_id for email in self.email_ids if email.is_primary]) > 1:
			finergy.throw(_("Only one {0} can be set as primary.").format(finergy.bold("Email ID")))
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			finergy.throw(
				_("Only one {0} can be set as primary.").format(finergy.bold(finergy.unscrub(fieldname)))
			)
...
def invite_user(contact):
	contact = finergy.get_doc("Contact", contact)
...
def update_contact(doc, method):
	...
	contact_name = finergy.db.get_value("Contact", {"email_id": doc.name})
	if contact_name:
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if (
		not finergy.get_meta("Contact").get_field(searchfield)
		and searchfield not in finergy.db.DEFAULT_COLUMNS
	):
		return []
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = finergy.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "Contact"})
	if not link_fields:
		return []

	contact_names = finergy.get_value(doctype, docname, fieldname=[f.fieldname for f in link_fields])
	if not contact_names:
		return []

	return finergy.get_list("Contact", fields=fields, filters={"name": ("in", contact_names)})
