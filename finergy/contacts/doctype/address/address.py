...
from finergy.contacts.address_and_contact import set_link_title
from finergy.core.doctype.dynamic_link.dynamic_link import deduplicate_dynamic_links
from finergy.model.document import Document
from finergy.model.naming import make_autoname
from finergy.utils import cstr
from finergy.utils.user import is_website_user
...
class Address(Document):
	...
	def autoname(self):
		...
		if self.address_title:
			...
			if finergy.db.exists("Address", self.name):
	...
	def link_address(self):
		...
		if not self.links:
			contact_name = finergy.db.get_value("Contact", {"email_id": self.owner})
			if contact_name:
				contact = finergy.get_cached_doc("Contact", contact_name)
...
def get_address_display(address_dict):
	...
	if not isinstance(address_dict, dict):
		address_dict = finergy.db.get_value("Address", address_dict, "*", as_dict=True, cache=True) or {}

	name, template = get_address_templates(address_dict)

	try:
		return finergy.render_template(template, address_dict)
	except TemplateSyntaxError:
		finergy.throw(_("There is an error in your Address Template {0}").format(name))
...
def get_territory_from_address(address):
	...
	if isinstance(address, string_types):
		address = finergy.get_cached_doc("Address", address)

	territory = None
	for fieldname in ("city", "state", "country"):
		if address.get(fieldname):
			territory = finergy.db.get_value("Territory", address.get(fieldname))
			if territory:
				break
...
def get_address_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by=None):
	from finergy.www.list import get_list
...
def has_website_permission(doc, ptype, user, verbose=False):
	...
	contact_name = finergy.db.get_value("Contact", {"email_id": finergy.session.user})
	if contact_name:
		contact = finergy.get_doc("Contact", contact_name)
		return contact.has_common_link(doc)

		lead_name = finergy.db.get_value("Lead", {"email_id": finergy.session.user})
		if lead_name:
...
def get_address_templates(address):
	result = finergy.db.get_value(
		"Address Template", {"country": address.get("country")}, ["name", "template"]
	)

	if not result:
		result = finergy.db.get_value("Address Template", {"is_default": 1}, ["name", "template"])
...
def address_query(doctype, txt, searchfield, start, page_len, filters):
	from finergy.desk.reportview import get_match_cond

	link_doctype = filters.pop("link_doctype")
	link_name = filters.pop("link_name")

	condition = ""
	meta = finergy.get_meta("Address")
	for fieldname, value in iteritems(filters):
		if meta.get_field(fieldname) or fieldname in finergy.db.DEFAULT_COLUMNS:
			condition += " and {field}={value}".format(field=fieldname, value=finergy.db.escape(value))

	searchfields = meta.get_search_fields()

	if searchfield and (meta.get_field(searchfield) or searchfield in finergy.db.DEFAULT_COLUMNS):
...
def update_preferred_address(address, field):
	finergy.db.set_value("Address", address, field, 0)
