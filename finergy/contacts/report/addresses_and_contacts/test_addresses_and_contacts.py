...
from finergy.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data


def get_custom_linked_doctype():
	if bool(finergy.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if finergy.flags.test_address_created:
		return

	address = finergy.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	finergy.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if finergy.flags.test_contact_created:
		return
	...
	finergy.flags.test_contact_created = True
