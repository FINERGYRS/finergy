...
def get_pdf(html, options=None, output=None):
	...
	try:
		...
		if any([error in str(e) for error in PDF_CONTENT_ERRORS]):
			if not filedata:
				finergy.throw(_("PDF generation failed because of broken image links"))
	...
	if "password" in options:
		...
		if six.PY2:
			password = finergy.safe_encode(password)
...
def prepare_options(html, options):
	...
	# page size
	pdf_page_size = (
		options.get("page-size") or finergy.db.get_single_value("Print Settings", "pdf_page_size") or "A4"
	)

	if pdf_page_size == "Custom":
		options["page-height"] = options.get("page-height") or finergy.db.get_single_value(
			"Print Settings", "pdf_page_height"
		)
		options["page-width"] = options.get("page-width") or finergy.db.get_single_value(
...
def prepare_header_footer(soup):
	...
	css = finergy.read_file(os.path.join(finergy.local.sites_path, "assets/css/printview.css"))
	...
	for html_id in ("header-html", "footer-html"):
		...
		if content:
			...
			# create temp file
			fname = os.path.join("/tmp", "finergy-pdf-{0}.html".format(finergy.generate_hash()))
			with open(fname, "wb") as f:
				f.write(html.encode("utf-8"))

			# {"header-html": "/tmp/finergy-pdf-random.html"}
...
def get_wkhtmltopdf_version():
	wkhtmltopdf_version = finergy.cache().hget("wkhtmltopdf_version", None)

	if not wkhtmltopdf_version:
		try:
			res = subprocess.check_output(["wkhtmltopdf", "--version"])
			wkhtmltopdf_version = res.decode("utf-8").split(" ")[1]
			finergy.cache().hset("wkhtmltopdf_version", None, wkhtmltopdf_version)
		except Exception:
			pass
