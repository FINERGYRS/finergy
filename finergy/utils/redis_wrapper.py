...
class RedisWrapper(redis.Redis):
	...
	def make_key(self, key, user=None, shared=False):
		...
		return "{0}|{1}".format(finergy.conf.db_name, key).encode("utf-8")
	...
	def get_value(self, key, generator=None, user=None, expires=False):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in finergy.local
		"""
		original_key = key
		key = self.make_key(key, user)

		if key in finergy.local.cache:
			val = finergy.local.cache[key]
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(finergy.local.cache) if regex.match(cstr(k))]
	...
	def hset(self, name, key, value, shared=False):
		...
		# set in local
		if not _name in finergy.local.cache:
			finergy.local.cache[_name] = {}
		finergy.local.cache[_name][key] = value
	...
	def hget(self, name, key, generator=None, shared=False):
		...
		if key in finergy.local.cache[_name]:
			return finergy.local.cache[_name][key]
	...
	def hdel(self, name, key, shared=False):
		...
		if _name in finergy.local.cache:
			if key in finergy.local.cache[_name]:
				del finergy.local.cache[_name][key]
		try:
	...
	def hdel_keys(self, name_starts_with, key):
		...
		for name in finergy.cache().get_keys(name_starts_with):
			name = name.split("|", 1)[1]
			self.hdel(name, key)
