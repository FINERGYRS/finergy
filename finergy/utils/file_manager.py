...
def get_file_url(file_data_name):
	data = finergy.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = finergy.form_dict.doctype
	dn = finergy.form_dict.docname
	file_url = finergy.form_dict.file_url
	filename = finergy.form_dict.filename
	finergy.form_dict.is_private = cint(finergy.form_dict.is_private)

	if not filename and not file_url:
		finergy.msgprint(_("Please select a file or url"), raise_exception=True)

	file_doc = get_file_doc()

	comment = {}
	if dt and dn:
		comment = finergy.get_doc(dt, dn).add_comment(
			"Attachment",
			_("added {0}").format(
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	finergy.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = finergy.form_dict.file_size
	...
	try:
		...
		return finergy.get_doc("File", f.duplicate_entry)
	return f


def get_uploaded_content():
	# should not be unicode when reading a file, hence using finergy.form
	if "filedata" in finergy.form_dict:
		if "," in finergy.form_dict.filedata:
			finergy.form_dict.filedata = finergy.form_dict.filedata.rsplit(",", 1)[1]
		finergy.uploaded_content = base64.b64decode(finergy.form_dict.filedata)
		finergy.uploaded_filename = finergy.form_dict.filename
		return finergy.uploaded_filename, finergy.uploaded_content
	else:
		finergy.msgprint(_("No file attached"))
		return None, None
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in finergy.db.sql_list(
		...
		b = finergy.get_doc("File", name)
		return {k: b.get(k) for k in finergy.get_hooks()["write_file_keys"]}
	return False
...
def remove_all(dt, dn, from_delete=False, delete_permanently=False):
	...
	try:
		for fid in finergy.db.sql_list(
			...
			if from_delete:
				...
				finergy.delete_doc("File", fid, ignore_permissions=True, delete_permanently=delete_permanently)
			else:
...
def remove_file(
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = finergy.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = doc.has_permission("write") or False
		if finergy.flags.in_web_form:
			ignore_permissions = True
		if not file_name:
			file_name = finergy.db.get_value("File", fid, "file_name")
...
def get_file_name(fname, optional_suffix):
	...
	n_records = finergy.db.sql("select name from `tabFile` where file_name=%s", fname)
...
def download_file(file_url):
	...
	Method : GET
	Endpoint : finergy.utils.file_manager.download_file
	...
	finergy.local.response.filecontent = filedata
...
def extract_images_from_html(doc, content):
	...
	def _save_file(match):
		...
		if not finergy.flags.has_dataurl:
