...
import finergy
from finergy.utils import get_sites
...
def get_logger(
	...
	if allow_site is True:
		site = getattr(finergy.local, "site", None)
	...
	try:
		return finergy.loggers[logger_name]
	except KeyError:
		pass
	...
	logger.setLevel(finergy.log_level or default_log_level)
	...
	finergy.loggers[logger_name] = logger
...
class SiteContextFilter(logging.Filter):
	...
	def filter(self, record):
		if "Form Dict" not in text_type(record.msg):
			site = getattr(finergy.local, "site", None)
			form_dict = getattr(finergy.local, "form_dict", None)
...
def set_log_level(level):
	...
	finergy.log_level = getattr(logging, (level or "").upper(), None) or default_log_level
	finergy.loggers = {}
