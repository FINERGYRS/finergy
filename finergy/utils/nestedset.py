...
class NestedSetRecursionError(finergy.ValidationError):
	pass


class NestedSetMultipleRootsError(finergy.ValidationError):
	pass


class NestedSetChildExistsError(finergy.ValidationError):
	pass


class NestedSetInvalidMergeError(finergy.ValidationError):
	pass
...
def update_nsm(doc):
	...
	parent_field = "parent_" + finergy.scrub(doc.doctype)
	...
	finergy.db.set_value(doc.doctype, doc.name, old_parent_field, parent or "", update_modified=False)
...
def update_add_node(doc, parent, parent_field):
	...
	finergy.db.sql("update `tab{0}` set rgt = rgt+2 where rgt >= %s".format(doctype), (right,))
	finergy.db.sql("update `tab{0}` set lft = lft+2 where lft >= %s".format(doctype), (right,))

	# update index of new node
	if finergy.db.sql(
		"select * from `tab{0}` where lft=%s or rgt=%s".format(doctype), (right, right + 1)
	):
		finergy.msgprint(_("Nested set error. Please contact the Administrator."))
		raise Exception
...
def update_move_node(doc, parent_field):
	...
	if parent:
		...
		# new root
		max_rgt = finergy.db.sql("""select max(rgt) from `tab{0}`""".format(doc.doctype))[0][0]
...
def rebuild_tree(doctype, parent_field):
	...
	if finergy.request and finergy.local.form_dict.cmd == "rebuild_tree":
		finergy.only_for("System Manager")

	meta = finergy.get_meta(doctype)
	if not meta.has_field("lft") or not meta.has_field("rgt"):
		finergy.throw(
			_("Rebuilding of tree is not supported for {}").format(finergy.bold(doctype)),
			title=_("Invalid Action"),
		)

	# get all roots
	finergy.db.auto_commit_on_many_writes = 1

	right = 1
	result = finergy.db.sql(
	...
	finergy.db.auto_commit_on_many_writes = 0
...
def validate_loop(doctype, name, lft, rgt):
	...
	if name in finergy.db.sql_list(
		"""select name from `tab{0}` where lft <= %s and rgt >= %s""".format(doctype), (lft, rgt)
	):
		finergy.throw(_("Item cannot be added to its own descendents"), NestedSetRecursionError)
...
class NestedSet(Document):
	...
	def on_trash(self, allow_root_deletion=False):
		if not getattr(self, "nsm_parent_field", None):
			self.nsm_parent_field = finergy.scrub(self.doctype) + "_parent"

		parent = self.get(self.nsm_parent_field)
		if not parent and not allow_root_deletion:
			finergy.throw(_("Root {0} cannot be deleted").format(_(self.doctype)))
	...
	def before_rename(self, olddn, newdn, merge=False, group_fname="is_group"):
		if merge and hasattr(self, group_fname):
			is_group = finergy.db.get_value(self.doctype, newdn, group_fname)
	...
	def validate_one_root(self):
		if not self.get(self.nsm_parent_field):
			if self.get_root_node_count() > 1:
				finergy.throw(_("""Multiple root nodes not allowed."""), NestedSetMultipleRootsError)

	def get_root_node_count(self):
		return finergy.db.count(self.doctype, {self.nsm_parent_field: ""})
...
def get_ancestors_of(doctype, name, order_by="lft desc", limit=None):
	...
	lft, rgt = finergy.db.get_value(doctype, name, ["lft", "rgt"])
