...
def main():
	...
	commands.update({"get-finergy-commands": get_finergy_commands, "get-finergy-help": get_finergy_help})
	click.Group(commands=commands)(prog_name="bench")


def get_app_groups():
	"""Get all app groups, put them in main group "finergy" since bench is
	...
	ret = dict(finergy=click.group(name="finergy", commands=commands)(app_group))
	return ret
...
def app_group(ctx, site=False, force=False, verbose=False, profile=False):
	...
	if ctx.info_name == "finergy":
		ctx.info_name = ""


def get_sites(site_arg):
	if site_arg == "all":
		return finergy.utils.get_sites()
	elif site_arg:
		return [site_arg]
	elif os.environ.get("FINERGY_SITE"):
		return [os.environ.get("FINERGY_SITE")]
...
@click.command("get-finergy-commands")
def get_finergy_commands():
	commands = list(get_app_commands("finergy"))
...
@click.command("get-finergy-help")
def get_finergy_help():
	print(click.Context(get_app_groups()["finergy"]).get_help())


def get_apps():
	return finergy.get_all_apps(with_internal_apps=False, sites_path=".")


if __name__ == "__main__":
	if not finergy._dev_server:
