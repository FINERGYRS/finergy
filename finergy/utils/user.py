...
from finergy.boot import get_allowed_reports
from finergy.core.doctype.domain_settings.domain_settings import get_active_modules
from finergy.permissions import get_roles, get_valid_perms
from finergy.utils import cint


class UserPermissions:
	"""
	A user permission object can be accessed as `finergy.get_user()`
	"""

	def __init__(self, name=""):
		self.defaults = None
		self.name = name or finergy.session.get("user")
		self.roles = []
	...
	def setup_user(self):
		def get_user_doc():
			user = None
			try:
				user = finergy.get_doc("User", self.name).as_dict()
			except finergy.DoesNotExistError:
				pass
			except Exception as e:
				# install boo-boo
				if not finergy.db.is_table_missing(e):
					raise

			return user

		if not finergy.flags.in_install_db and not finergy.flags.in_test:
			user_doc = finergy.cache().hget("user_doc", self.name, get_user_doc)
			if user_doc:
				self.doc = finergy.get_doc(user_doc)
	...
	def build_perm_map(self):
		...
		for r in get_valid_perms():
			...
			for k in finergy.permissions.rights:
	...
	def build_permissions(self):
		...
		self.build_perm_map()
		user_shared = finergy.share.get_shared_doctypes()
		no_list_view_link = []
		...
		self.shared = finergy.db.sql_list(
		...
		if "System Manager" in self.get_roles():
			docs = finergy.get_all("DocType", {"allow_import": 1})
		...
		finergy.cache().hset("can_import", finergy.session.user, self.can_import)

	def get_defaults(self):
		import finergy.defaults

		self.defaults = finergy.defaults.get_defaults(self.name)
...
def get_fullname_and_avatar(user):
	first_name, last_name, avatar, name = finergy.db.get_value(
...
def get_system_managers(only_name=False):
	...
	system_managers = finergy.db.sql(
...
def add_role(user, role):
	finergy.get_doc("User", user).add_roles(role)
...
def is_website_user():
	return finergy.db.get_value("User", finergy.session.user, "user_type") == "Website User"


def is_system_user(username):
	return finergy.db.get_value("User", {"email": username, "enabled": 1, "user_type": "System User"})
...
def get_users():
	...
	users = []
	system_managers = finergy.utils.user.get_system_managers(only_name=True)
	for user in get_system_users():
		users.append(
			{
				"full_name": finergy.utils.user.get_user_fullname(user),
				"email": user,
...
def set_last_active_to_now(user):
	...
	finergy.db.set_value("User", user, "last_active", now_datetime())


def reset_simultaneous_sessions(user_limit):
	for user in finergy.db.sql(
		...
		if user.simultaneous_sessions < user_limit:
			...
			finergy.db.set_value("User", user.name, "simultaneous_sessions", 1)
