...
EXCLUDE_EXCEPTIONS = (
	finergy.AuthenticationError,
	finergy.CSRFTokenError,  # CSRF covers OAuth too
...
def make_error_snapshot(exception):
	if finergy.conf.disable_error_snapshot:
		return

	if isinstance(exception, EXCLUDE_EXCEPTIONS):
		return

	logger = finergy.logger(with_more_info=True)

	try:
		error_id = "{timestamp:s}-{ip:s}-{hash:s}".format(
			timestamp=cstr(datetime.datetime.now()),
			ip=finergy.local.request_ip or "127.0.0.1",
			hash=finergy.generate_hash(length=3),
		)
		snapshot_folder = get_error_snapshot_path()
		finergy.create_folder(snapshot_folder)
		...
		with open(encode(snapshot_file_path), "wb") as error_file:
			error_file.write(encode(finergy.as_json(snapshot)))
	...
	try:
		...
		for fname in os.listdir(path):
			...
			for field in ["locals", "exception", "frames"]:
				data[field] = finergy.as_json(data[field])

			doc = finergy.new_doc("Error Snapshot")
			doc.update(data)
			doc.save()
...
def get_error_snapshot_path():
	return finergy.get_site_path("error-snapshots")
