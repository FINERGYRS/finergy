...
import finergy
from finergy.core.page.permission_manager.permission_manager import get_standard_permissions
from finergy.custom.doctype.custom_field.custom_field import create_custom_field
from finergy.custom.doctype.customize_form.customize_form import (
	docfield_properties,
	doctype_properties,
)
from finergy.custom.doctype.property_setter.property_setter import make_property_setter
from finergy.modules import get_module_path, scrub, utils
from finergy.permissions import setup_custom_perms

branch = "develop"


def reset_all():
	for doctype in finergy.db.get_all("DocType", dict(custom=0)):
		print(doctype.name)
...
def reset_doc(doctype):
	...
	try:
		git_link = "/".join(
			["https://raw.githubusercontent.com/finergy", app, branch, doc_path.split("apps/" + app)[1]]
		)
...
def make_custom_fields(doctype, local_doc, original_doc):
	...
	doctype_doc = finergy.get_doc("DocType", doctype)
...
def get_custom_docfield_properties():
	fields_meta = finergy.get_meta("Custom Field").fields
	fields = {}
...
def setup_perms_for(doctype):
	perms = finergy.get_all("DocPerm", fields="*", filters=dict(parent=doctype), order_by="idx asc")
	# get default perms
	try:
	...
	if len(standard_perms) != len(perms):
		...
		for i, p in enumerate(perms):
			...
			for fieldname in finergy.get_meta("DocPerm").get_fieldnames_with_value():
