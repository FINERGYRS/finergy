...
def get_secret():
	return finergy.local.conf.get("secret") or str(
		finergy.db.get_value("User", "Administrator", "creation")
	)


def verify_request():
	"""Verify if the incoming signed request if it is correct."""
	query_string = finergy.safe_decode(
		finergy.local.flags.signed_query_string or getattr(finergy.request, "query_string", None)
	)
...
def get_url(cmd, params, nonce=None, secret=None):
	...
	return finergy.utils.get_url("".join(["api/method/", cmd, "?", urlencode(params)]))


def get_signature(params, nonce, secret=None):
	params = "".join((finergy.utils.cstr(p) for p in params.values()))
	if not secret:
		secret = finergy.local.conf.get("secret") or "secret"
