...
from finergy.utils.data import *
from finergy.utils.html_utils import sanitize_html
...
def get_fullname(user=None):
	...
	if not hasattr(finergy.local, "fullnames"):
		finergy.local.fullnames = {}

	if not finergy.local.fullnames.get(user):
		p = finergy.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			finergy.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			finergy.local.fullnames[user] = user

	return finergy.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return finergy.db.get_value("User", user, "email")
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		finergy.throw(
			finergy._("{0} is not a valid Phone Number").format(phone_number), finergy.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	if not match and throw:
		finergy.throw(finergy._("{0} is not a valid Name").format(name), finergy.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = finergy.utils.escape_html(e)
				finergy.throw(
					finergy._("{0} is not a valid Email Address").format(invalid_email),
					finergy.InvalidEmailAddressError,
				)
			return None
		else:
			return matched
...
def validate_url(txt, throw=False, valid_schemes=None):
	...
	if not is_valid and throw:
		finergy.throw(finergy._("'{0}' is not a valid URL").format(finergy.bold(txt)))
...
def has_gravatar(email):
	...
	if finergy.flags.in_import or finergy.flags.in_install or finergy.flags.in_test:
		# no gravatar if via upload
		# since querying gravatar for every item will be slow
		return ""

	hexdigest = hashlib.md5(finergy.as_unicode(email).encode("utf-8")).hexdigest()
...
def get_gravatar(email):
	from finergy.utils.identicon import Identicon
...
def get_defaults(key=None):
	...
	"""
	return finergy.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return finergy.db.set_default(key, val)
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = finergy.local.site_path
	return os.path.join(base, *path)


def get_site_base_path(sites_dir=None, hostname=None):
	return finergy.local.site_path
...
def get_bench_path():
	return os.path.realpath(os.path.join(os.path.dirname(finergy.__file__), "..", "..", ".."))
...
def get_site_url(site):
	return "http://{site}:{port}".format(site=site, port=finergy.get_conf(site).webserver_port)
...
def get_test_client():
	from finergy.app import application

	return Client(application)


def get_hook_method(hook_name, fallback=None):
	method = finergy.get_hooks().get(hook_name)
	if method:
		method = finergy.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in finergy.get_hooks(hook):
		out = out or finergy.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l):
	...
	if not getattr(finergy.local, "request", None) or is_cli():
		lt = len(txt)
		try:
...
def get_html_format(print_path):
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in re.findall("""({% include ['"]([^'"]*)['"] %})""", html_format):
			for app_name in finergy.get_installed_apps():
				include_path = finergy.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(finergy.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else finergy.utils.md_to_html(text)
...
def get_site_info():
	...
	from finergy.email.queue import get_emails_sent_this_month
	...
	system_settings = finergy.db.get_singles_dict("System Settings")
	space_usage = finergy._dict((finergy.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": finergy.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in finergy.get_hooks("get_site_info"):
		site_info.update(finergy.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return json.loads(finergy.as_json(site_info))
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                bench --site capkpi.local execute finergy.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
		db_count[doctype] = finergy.db.count(doctype)

	return json.loads(finergy.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: finergy function to be called
	...
	Example:
	        via terminal:
	                bench --site capkpi.local execute finergy.utils.call --args '''["finergy.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
	return json.loads(finergy.as_json(finergy.call(fn, *args, **kwargs)))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, (int, float)):
			filters = finergy.as_unicode(filters)
...
def set_request(**kwargs):
	...
	finergy.local.request = Request(builder.get_environ())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(finergy.local.sites_path, ".build")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return finergy.utils.random_string(8)
