...
def get_monthly_goal_graph_data(
	...
	if not finergy.has_permission(goal_doctype):
		return None
	...
	from finergy.utils import add_months, formatdate, getdate, today
	...
	if month_to_value_dict is None:
		doc_filter = (
			(goal_doctype_link + " = " + finergy.db.escape(docname)) if doctype != goal_doctype else ""
		)
		if filter_str:
