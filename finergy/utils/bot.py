...
from finergy.desk.notifications import get_notifications
	...
	def get_reply(self):
		if self.startswith("open", "show open", "list open", "get open"):
			if self.tables:
				...
				from finergy.desk.notifications import get_notification_config

				filters = get_notification_config().get("for_doctype").get(doctype, None)
				if filters:
					if isinstance(filters, dict):
						data = finergy.get_list(doctype, filters=filters)
					else:
						data = [{"name": d[0], "title": d[1]} for d in finergy.get_attr(filters)(as_list=True)]
	...
	def get_reply(self):
		...
		if self.startswith(_("list"), _("show")):
			...
			if self.tables:
				...
				return self.format_list(finergy.get_list(self.get_doctype(), fields=fields, filters=filters))
...
class CountBot(BotParser):
	def get_reply(self):
		if self.startswith("how many"):
			self.tables = self.reply.identify_tables(self.query.split(None, 1)[1])
			if self.tables:
				return str(finergy.db.sql("select count(*) from `tab{0}`".format(self.get_doctype()))[0][0])
		...
		if self.startswith("find", "search"):
			...
			if table:
				...
				if self.tables:
					...
					title_field = finergy.get_meta(self.get_doctype()).title_field
					if title_field and title_field != "name":
						or_filters = {"title": ("like", "%{0}%".format(text))}

					data = finergy.get_list(self.get_doctype(), filters=filters, or_filters=or_filters)
					if data:
...
class BotReply(object):
	...
	def get_reply(self, query):
		...
		if self.query.split()[0] in ("hello", "hi"):
			return _("Hello {0}").format(finergy.utils.get_fullname())

		if self.query == "help":
			return help_text.format(finergy.utils.get_fullname())

		# build using parsers
		replies = []
		for parser in finergy.get_hooks("bot_parsers"):
			reply = None
			try:
				reply = finergy.get_attr(parser)(self, query).get_reply()
	...
	def setup_tables(self):
		tables = finergy.get_all("DocType", {"istable": 0})
