...
def get_queues_timeout():
	common_site_config = finergy.get_conf()
...
def enqueue(
	...
	**kwargs
	...
	:param now: if now=True, the method is executed via finergy.call
	:param kwargs: keyword arguments to be passed to the method
	"""
	# To handle older implementations
	is_async = kwargs.pop("async", is_async)

	if now or finergy.flags.in_migrate:
		return finergy.call(method, **kwargs)
	...
	if enqueue_after_commit:
		if not finergy.flags.enqueue_after_commit:
			finergy.flags.enqueue_after_commit = []

		finergy.flags.enqueue_after_commit.append(
			{"queue": queue, "is_async": is_async, "timeout": timeout, "queue_args": queue_args}
		)
		return finergy.flags.enqueue_after_commit
	else:
...
def enqueue_doc(
	...
	return enqueue(
		"finergy.utils.background_jobs.run_doc_method",
		doctype=doctype,
		name=name,
		doc_method=method,
		queue=queue,
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(finergy.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	finergy.monitor.start("job", method_name, kwargs)
	try:
		method(**kwargs)

	except (finergy.db.InternalError, finergy.RetryBackgroundJobError) as e:
		finergy.db.rollback()

		if retry < 5 and (
			isinstance(e, finergy.RetryBackgroundJobError)
			or (finergy.db.is_deadlocked(e) or finergy.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			...
			finergy.log_error(title=method_name)
			raise
...
def validate_queue(queue, default_queue_list=None):
	...
	if queue not in default_queue_list:
		finergy.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn():
	if not hasattr(finergy.local, "conf"):
		raise Exception("You need to call finergy.init")

	elif not finergy.local.conf.redis_queue:
		raise Exception("redis_queue missing in common_site_config.json")

	global redis_connection

	if not redis_connection:
		redis_connection = redis.from_url(finergy.local.conf.redis_queue)

	return redis_connection


def enqueue_test_job():
	enqueue("finergy.utils.background_jobs.test_job", s=100)
