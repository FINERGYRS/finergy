...
from finergy.utils import cint, get_file_size, get_url, now, now_datetime
...
class BackupGenerator:
	...
	def __init__(
		...
		site = finergy.local.site or finergy.generate_hash(length=8)
	...
	def setup_backup_tables(self):
		...
		existing_doctypes = set([x.name for x in finergy.get_all("DocType")])
		...
		specified_tables = get_tables(finergy.conf.get("backup", {}).get("includes", []))
		include_tables = (specified_tables + base_tables) if specified_tables else []

		conf_tables = {
			"include": include_tables,
			"exclude": get_tables(finergy.conf.get("backup", {}).get("excludes", [])),
		}
	...
	def print_summary(self):
		...
		print("Backup Summary for {0} at {1}".format(finergy.local.site, now()))
	...
	def backup_files(self):
		for folder in ("public", "private"):
			files_path = finergy.get_site_path(folder, "files")
			...
			finergy.utils.execute_in_shell(
	...
	def copy_site_config(self):
		...
		site_config_path = os.path.join(finergy.get_site_path(), "site_config.json")
	...
	def take_dump(self):
		...
		if not (gzip_exc and db_exc[1]):
			...
			finergy.throw(
				f"{_exc} not found in PATH! This is required to take a backup.", exc=finergy.ExecutableNotFound
			)
		db_exc = db_exc[0]

		database_header_content = [
			f"Backup generated by Finergy {finergy.__version__} on branch {get_app_branch('finergy') or 'N/A'}",
			"",
		]

		# escape reserved characters
		args = finergy._dict(
			[item[0], finergy.utils.esc(str(item[1]), "$ ")] for item in self.__dict__.copy().items()
		)
		...
		if self.partial:
			...
			database_header_content.extend(
				[
					f"Partial Backup of Finergy Site {finergy.local.site}",
		...
		if self.db_type == "postgres":
			...
			if self.backup_includes:
				...
				args["exclude"] = " ".join(
					[
						"--ignore-table='{0}.{1}'".format(finergy.conf.db_name, table)
		...
		finergy.utils.execute_in_shell(command, low_priority=True)

	def send_email(self):
		"""
		Sends the link to backup file located at capkpi/backups
		"""
		from finergy.email import get_system_managers
...
download only after 24 hours.""" % {
		...
		finergy.sendmail(recipients=recipient_list, msg=msg, subject=subject)
...
def fetch_latest_backups(partial=False):
	...
	odb = BackupGenerator(
		...
		finergy.conf.db_name,
		finergy.conf.db_password,
		db_host=finergy.db.host,
		db_type=finergy.conf.db_type,
		db_port=finergy.conf.db_port,
	)
...
def delete_temp_backups(older_than=24):
	...
	"""
	older_than = cint(finergy.conf.keep_backups_for_hours) or older_than
	backup_path = get_backup_path()
...
def get_backup_path():
	backup_path = finergy.utils.get_site_path(conf.get("backup_path", "private/backups"))
	return backup_path
