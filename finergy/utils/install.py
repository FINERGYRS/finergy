...
def before_install():
	...
	finergy.reload_doc("core", "doctype", "docperm")
...
def after_install():
	...
	finergy.db.set_global("installed_apps", '["finergy"]')

	create_user_type()
	install_basic_docs()

	from finergy.core.doctype.file.file import make_home_folder
	...
	# all roles to admin
	finergy.get_doc("User", "Administrator").add_roles(
		*finergy.db.sql_list("""select name from tabRole""")
	)

	# update admin password
	update_password("Administrator", get_admin_password())

	if not finergy.conf.skip_setup_wizard:
		finergy.db.set_default("desktop:home_page", "setup-wizard")
...
def create_user_type():
	for user_type in ["System User", "Website User"]:
		if not finergy.db.exists("User Type", user_type):
			finergy.get_doc({"doctype": "User Type", "name": user_type, "is_standard": 1}).insert(
...
def get_admin_password():
	...
	admin_password = finergy.conf.get("admin_password")
...
def before_tests():
	if len(finergy.get_installed_apps()) > 1:
		# don't run before tests if any other app is installed
		return

	finergy.db.sql("delete from `tabCustom Field`")
	finergy.db.sql("delete from `tabEvent`")
	finergy.db.commit()
	finergy.clear_cache()

	# complete setup if missing
	if not int(finergy.db.get_single_value("System Settings", "setup_complete") or 0):
...
def complete_setup_wizard():
	from finergy.desk.page.setup_wizard.setup_wizard import setup_complete
...
def import_country_and_currency():
	from finergy.geo.country_info import get_all
	from finergy.utils import update_progress_bar

	data = get_all()

	for i, name in enumerate(data):
		update_progress_bar("Updating country info", i, len(data))
		country = finergy._dict(data[name])
	...
	for currency in ("INR", "USD", "GBP", "EUR", "AED", "AUD", "JPY", "CNY", "CHF"):
		finergy.db.set_value("Currency", currency, "enabled", 1)


def add_country_and_currency(name, country):
	if not finergy.db.exists("Country", name):
		finergy.get_doc(
			{
				"doctype": "Country",
				"country_name": name,
	...
	if country.currency and not finergy.db.exists("Currency", country.currency):
		finergy.get_doc(
			{
				"doctype": "Currency",
...
def add_standard_navbar_items():
	...
	standard_navbar_items = [
		...
		},
		{
			...
			"item_type": "Action",
			"action": "finergy.ui.toolbar.route_to_user()",
			"is_standard": 1,
		},
		{
			"item_label": "Session Defaults",
			"item_type": "Action",
			"action": "finergy.ui.toolbar.setup_session_defaults()",
			"is_standard": 1,
		},
		{
			"item_label": "Reload",
			"item_type": "Action",
			"action": "finergy.ui.toolbar.clear_cache()",
			"is_standard": 1,
		},
		{
			"item_label": "View Website",
			"item_type": "Action",
			"action": "finergy.ui.toolbar.view_website()",
			"is_standard": 1,
		},
		{
			"item_label": "Toggle Full Width",
			"item_type": "Action",
			"action": "finergy.ui.toolbar.toggle_full_width()",
			"is_standard": 1,
		},
		{
	...
	standard_help_items = [
		{
			"item_label": "About",
			"item_type": "Action",
			"action": "finergy.ui.toolbar.show_about()",
			"is_standard": 1,
		},
		{
			"item_label": "Keyboard Shortcuts",
			"item_type": "Action",
			"action": "finergy.ui.toolbar.show_shortcuts(event)",
			"is_standard": 1,
		},
	]
