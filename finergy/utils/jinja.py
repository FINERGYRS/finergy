...
def get_jenv():
	...
	if not getattr(finergy.local, "jenv", None):
		from jinja2 import DebugUndefined
		from jinja2.sandbox import SandboxedEnvironment

		# finergy will be loaded last, so app templates will get precedence
		...
		finergy.local.jenv = jenv

	return finergy.local.jenv
...
def validate_template(html):
	...
	try:
		...
		finergy.msgprint("Line {}: {}".format(e.lineno, e.message))
		finergy.throw(finergy._("Syntax error in template"))
...
def render_template(template, context, is_path=None, safe_render=True):
	...
	from finergy import _, get_traceback, throw
...
def get_jloader():
	...
	if not getattr(finergy.local, "jloader", None):
		from jinja2 import ChoiceLoader, PackageLoader, PrefixLoader

		if finergy.local.flags.in_setup_help:
			apps = ["finergy"]
		else:
			apps = finergy.get_hooks("template_apps")
			if not apps:
				apps = finergy.local.flags.web_pages_apps or finergy.get_installed_apps(sort=True)
				apps.reverse()

		if "finergy" not in apps:
			apps.append("finergy")

		finergy.local.jloader = ChoiceLoader(
	...
	return finergy.local.jloader


def set_filters(jenv):
	import finergy
	from finergy.utils import cint, cstr, flt, global_date_format, markdown
	from finergy.website.utils import abs_url, get_shade

	jenv.filters["global_date_format"] = global_date_format
	jenv.filters["markdown"] = markdown
	jenv.filters["json"] = finergy.as_json
	...
	if finergy.flags.in_setup_help:
		return
...
def get_jenv_customization(customization_type):
	...
	out = {}
	if not getattr(finergy.local, "site", None):
		return out

	values = finergy.get_hooks("jenv", {}).get(customization_type)
	if not values:
		return out

	for value in values:
		fn_name, fn_string = value.split(":")
		out[fn_name] = finergy.get_attr(fn_string)
...
def resolve_class(classes):
	...
	if isinstance(classes, finergy.string_types):
		return classes
...
def web_blocks(blocks):
	from finergy import _, _dict, throw
	from finergy.website.doctype.web_page.web_page import get_web_blocks_html
