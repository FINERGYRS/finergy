...
from finergy.utils import cint, comma_or, cstr, encode, flt


def read_csv_content_from_attached_file(doc):
	fileid = finergy.get_all(
		"File",
		fields=["name"],
	...
	try:
		_file = finergy.get_doc("File", fileid)
		fcontent = _file.get_content()
		return read_csv_content(fcontent, finergy.form_dict.get("ignore_encoding_errors"))
	except Exception:
		finergy.throw(
...
def read_csv_content(fcontent, ignore_encoding=False):
	...
	for line in fcontent.splitlines(True):
		if six.PY2:
			...
			content.append(finergy.safe_decode(line))
	...
	try:
		...
		finergy.msgprint(_("Not a valid Comma Separated Value (CSV File)"))
		raise
...
def send_csv_to_client(args):
	...
	finergy.response["result"] = cstr(to_csv(args.data))
	finergy.response["doctype"] = args.filename
...
def build_csv_response(data, filename):
	finergy.response["result"] = cstr(to_csv(data))
	finergy.response["doctype"] = filename
	finergy.response["type"] = "csv"
	...
	for key in d:
		...
		if docfield:
			if docfield.reqd and (val == "" or val == None):
				finergy.msgprint(_("{0} is required").format(docfield.label), raise_exception=1)
...
def import_doc(d, doctype, overwrite, row_idx, submit=False, ignore_links=False):
	...
	if d.get("name") and finergy.db.exists(doctype, d["name"]):
		if overwrite:
			doc = finergy.get_doc(doctype, d["name"])
