...
from finergy.utils.pdf import cleanup, get_pdf
from finergy.www.printview import validate_print_permission
...
def download_multi_pdf(doctype, name, format=None, no_letterhead=False, options=None):
	...
	if not isinstance(doctype, dict):
		...
		finergy.local.response.filename = "{doctype}.pdf".format(
		...
		for doctype_name in doctype:
			for doc_name in doctype[doctype_name]:
				try:
					...
					finergy.log_error("Permission Error on doc {} of doctype {}".format(doc_name, doctype_name))
		finergy.local.response.filename = "{}.pdf".format(name)

	finergy.local.response.filecontent = read_multi_pdf(output)
	finergy.local.response.type = "download"


def read_multi_pdf(output):
	# Get the content of the merged pdf files
	fname = os.path.join("/tmp", "finergy-pdf-{0}.pdf".format(finergy.generate_hash()))
...
def download_pdf(doctype, name, format=None, doc=None, no_letterhead=0):
	doc = doc or finergy.get_doc(doctype, name)
	try:
		validate_print_permission(doc)
	except finergy.exceptions.LinkExpired:
		finergy.local.response.http_status_code = 410
		finergy.local.response.message = _("Link Expired")
		return
	except finergy.exceptions.InvalidKeyError:
		finergy.local.response.http_status_code = 401
		finergy.local.response.message = _("Invalid Key")
		return

	html = finergy.get_print(doctype, name, format, doc=doc, no_letterhead=no_letterhead)
	finergy.local.response.filename = "{name}.pdf".format(
		name=name.replace(" ", "-").replace("/", "-")
	)
	finergy.local.response.filecontent = get_pdf(html)
	finergy.local.response.type = "pdf"


@finergy.whitelist()
def report_to_pdf(html, orientation="Landscape"):
	make_access_log(file_type="PDF", method="PDF", page=html)
	finergy.local.response.filename = "report.pdf"
	finergy.local.response.filecontent = get_pdf(html, {"orientation": orientation})
	finergy.local.response.type = "pdf"
...
def print_by_server(
	...
	print_settings = finergy.get_doc("Network Printer Settings", printer_setting)
	try:
		import cups
	except ImportError:
		finergy.throw(_("You need to install pycups to use this feature!"))
	...
	try:
		...
		if not file_path:
			file_path = os.path.join("/", "tmp", "finergy-pdf-{0}.pdf".format(finergy.generate_hash()))
		...
		if (
			...
			finergy.throw(_("PDF generation failed"))
	except cups.IPPError:
		finergy.throw(_("Printing failed"))
	finally:
		return
