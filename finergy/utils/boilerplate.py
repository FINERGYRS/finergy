...
import finergy
from finergy.utils import cstr, touch_file
...
def make_boilerplate(dest, app_name):
	...
	app_name = finergy.scrub(app_name)
	...
	finergy.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, finergy.scrub(hooks.app_title)), with_init=True
	)
	finergy.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True
	)
	finergy.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	finergy.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "pages"), with_init=True
	)
	finergy.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "includes"))
	finergy.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	finergy.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "css"))
	finergy.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "js"))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "__init__.py"), "w") as f:
		f.write(finergy.as_unicode(init_template))

	with open(os.path.join(dest, hooks.app_name, "MANIFEST.in"), "w") as f:
		f.write(finergy.as_unicode(manifest_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
		f.write(finergy.as_unicode(gitignore_template.format(app_name=hooks.app_name)))

	with open(os.path.join(dest, hooks.app_name, "requirements.txt"), "w") as f:
		f.write("# finergy -- https://github.com/finergyrs/finergy is installed via 'bench init'")
	...
	with open(os.path.join(dest, hooks.app_name, "license.txt"), "w") as f:
		f.write(finergy.as_unicode("License: " + hooks.app_license))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "modules.txt"), "w") as f:
		f.write(finergy.as_unicode(hooks.app_title))
	...
	with open(os.path.join(dest, hooks.app_name, "setup.py"), "w") as f:
		f.write(finergy.as_unicode(setup_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "hooks.py"), "w") as f:
		f.write(finergy.as_unicode(hooks_template.format(**hooks)))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "config", "desktop.py"), "w") as f:
		f.write(finergy.as_unicode(desktop_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "config", "docs.py"), "w") as f:
		f.write(finergy.as_unicode(docs_template.format(**hooks)))
...
# Desk Notifications
# ------------------
# See finergy.core.notifications.get_notification_config
...
#	"finergy.desk.doctype.event.event.get_events": "{app_name}.event.get_events"
# }}
#
# each overriding function accepts a `data` argument;
# generated from the base implementation of the doctype dashboard,
# along with any modifications made in other Finergy apps
...
desktop_template = """from finergy import _
