...
def get_first_day_of_the_week():
	return finergy.get_system_settings("first_day_of_the_week") or "Sunday"
...
def getdate(string_date: Optional[str] = None):
	...
	try:
		...
		finergy.throw(
			finergy._("{} is not a valid date string.").format(finergy.bold(string_date)),
			title=finergy._("Invalid Date"),
		)
...
def add_to_date(
	...
	if isinstance(date, str):
		...
		try:
			...
			finergy.throw(finergy._("Please select a valid date filter"), title=finergy._("Invalid Date"))
...
def _get_time_zone():
	return finergy.db.get_system_setting("time_zone") or "Asia/Kolkata"  # Default to India ?!
...
def get_time_zone():
	...
	return finergy.cache().get_value("time_zone", _get_time_zone)
...
def now():
	...
	if finergy.flags.current_date:
		return (
			getdate(finergy.flags.current_date).strftime(DATE_FORMAT)
			+ " "
...
def get_user_date_format():
	...
	if getattr(finergy.local, "user_date_format", None) is None:
		finergy.local.user_date_format = finergy.db.get_default("date_format")

	return finergy.local.user_date_format or "yyyy-mm-dd"
...
def get_user_time_format():
	...
	if getattr(finergy.local, "user_time_format", None) is None:
		finergy.local.user_time_format = finergy.db.get_default("time_format")

	return finergy.local.user_time_format or "HH:mm:ss"
	...
	* dd/mm/yyyy
	...
	try:
		formatted_date = babel.dates.format_date(
			date, format_string, locale=(finergy.local.lang or "").replace("-", "_")
		)
	...
	* HH:mm
	...
	try:
		formatted_time = babel.dates.format_time(
			time_, format_string, locale=(finergy.local.lang or "").replace("-", "_")
		)
	except UnknownLocaleError:
	...
	* mm-dd-yyyy HH:mm
	...
	try:
		formatted_datetime = babel.dates.format_datetime(
			datetime, format_string, locale=(finergy.local.lang or "").replace("-", "_")
		)
	except UnknownLocaleError:
...
def validate_duration_format(duration):
	...
	if not is_valid_duration:
		finergy.throw(
			finergy._("Value {0} must be in the valid duration format: d h m s").format(
				finergy.bold(duration)
			)
		)
...
def global_date_format(date, format="long"):
	...
	date = getdate(date)
	formatted_date = babel.dates.format_date(
		date, locale=(finergy.local.lang or "en").replace("-", "_"), format=format
	)
	return formatted_date
...
def cast_fieldtype(fieldtype, value, show_warning=True):
	if show_warning:
		message = (
			"Function `finergy.utils.data.cast_fieldtype` has been deprecated in favour"
			" of `finergy.utils.data.cast`. Use the newer util for safer type casting."
		)
...
def cast(fieldtype, value=None):
	"""Cast the value to the Python native object of the Finergy fieldtype provided.
	...
	Mapping of Python types => Finergy types:
...
def cstr(s, encoding="utf-8"):
	return finergy.as_unicode(s, encoding)
...
def round_based_on_smallest_currency_fraction(value, currency, precision=2):
	smallest_currency_fraction_value = flt(
		finergy.db.get_value("Currency", currency, "smallest_currency_fraction_value", cache=True)
	)
...
def fmt_money(amount, precision=None, currency=None, format=None):
	...
	"""
	number_format = format or finergy.db.get_default("number_format") or "#,###.##"
	if precision is None:
		precision = cint(finergy.db.get_default("currency_precision")) or None
	...
	if decimal_str:
		...
		if precision > 2:
			if len(decimals) < 3:
				if currency:
					fraction = finergy.db.get_value("Currency", currency, "fraction_units", cache=True) or 100
	...
	if currency and finergy.defaults.get_global_default("hide_currency_symbol") != "Yes":
		symbol = finergy.db.get_value("Currency", currency, "symbol", cache=True) or currency
...
def money_in_words(
	...
	"""
	from finergy.utils import get_defaults
	...
	if not fraction_currency:
		fraction_currency = finergy.db.get_value("Currency", main_currency, "fraction", cache=True) or _(
			"Cent"
		)

	number_format = (
		finergy.db.get_value("Currency", main_currency, "number_format", cache=True)
		or finergy.db.get_default("number_format")
		or "#,###.##"
	)
...
def in_words(integer, in_million=True):
	...
	locale = "en_IN" if not in_million else finergy.local.lang
	integer = int(integer)
	try:
...
def get_thumbnail_base64_for_image(src):
	...
	from finergy import cache, safe_decode
	from finergy.core.doctype.file.file import get_local_image

	if not src:
		finergy.throw("Invalid source for image: {0}".format(src))
	...
	def _get_base64():
		file_path = finergy.get_site_path("public", src.lstrip("/"))
...
def comma_or(some_list, add_quotes=True):
	return comma_sep(some_list, finergy._("{0} or {1}"), add_quotes)


def comma_and(some_list, add_quotes=True):
	return comma_sep(some_list, finergy._("{0} and {1}"), add_quotes)
...
def comma_sep(some_list, pattern, add_quotes=True):
	if isinstance(some_list, (list, tuple)):
		...
		if not some_list:
			...
			return pattern.format(", ".join(finergy._(s) for s in some_list[:-1]), some_list[-1])
	else:
		return some_list
...
def get_url(uri=None, full_address=False):
	...
	host_name = finergy.local.conf.host_name or finergy.local.conf.hostname
	...
	if not host_name:
		...
		elif finergy.local.site:
			protocol = "http://"

			if finergy.local.conf.ssl_certificate:
				protocol = "https://"

			elif finergy.local.conf.wildcard:
				domain = finergy.local.conf.wildcard.get("domain")
				if (
					domain
					and finergy.local.site.endswith(domain)
					and finergy.local.conf.wildcard.get("ssl_certificate")
				):
					protocol = "https://"

			host_name = protocol + finergy.local.site

		else:
			host_name = finergy.db.get_value("Website Settings", "Website Settings", "subdomain")
	...
	if not uri and full_address:
		uri = finergy.get_request_header("REQUEST_URI", "")

	port = finergy.conf.http_port or finergy.conf.webserver_port

	if (
		not (finergy.conf.restart_supervisor_on_update or finergy.conf.restart_systemd_on_update)
		and host_name
...
def get_host_name_from_request():
	if hasattr(finergy.local, "request") and finergy.local.request and finergy.local.request.host:
		protocol = (
			"https://" if "https" == finergy.get_request_header("X-Forwarded-Proto", "") else "http://"
		)
		return protocol + finergy.local.request.host
...
def get_filter(doctype: str, f: Union[Dict, List, Tuple], filters_config=None) -> "finergy._dict":
	...
	"""
	from finergy.model import default_fields, optional_fields
	...
	if not isinstance(f, (list, tuple)):
		finergy.throw(finergy._("Filter must be a tuple or list (in a list)"))
	...
	if len(f) == 3:
		...
		finergy.throw(
			finergy._("Filter must have 4 values (doctype, fieldname, operator, value): {0}").format(str(f))
		)

	f = finergy._dict(doctype=f[0], fieldname=f[1], operator=f[2], value=f[3])
	...
	if f.operator.lower() not in valid_operators:
		finergy.throw(finergy._("Operator must be one of {0}").format(", ".join(valid_operators)))

	if f.doctype and (f.fieldname not in default_fields + optional_fields):
		# verify fieldname belongs to the doctype
		meta = finergy.get_meta(f.doctype)
		if not meta.has_field(f.fieldname):

			# try and match the doctype name from child tables
			for df in meta.get_table_fields():
				if finergy.get_meta(df.options).has_field(f.fieldname):
					f.doctype = df.options
					break

	try:
		df = finergy.get_meta(f.doctype).get_field(f.fieldname)
	except finergy.exceptions.DoesNotExistError:
		df = None
...
def sanitize_column(column_name):
	...
	def _raise_exception():
		finergy.throw(_("Invalid field name {0}").format(column_name), finergy.DataError)
...
def generate_hash(*args, **kwargs) -> str:
	return finergy.generate_hash(*args, **kwargs)
...
def get_user_info_for_avatar(user_id: str) -> Dict:
	...
	try:
		user_info["email"] = finergy.get_cached_value("User", user_id, "email")
		user_info["name"] = finergy.get_cached_value("User", user_id, "full_name")
		user_info["image"] = finergy.get_cached_value("User", user_id, "user_image")
	except Exception:
		finergy.local.message_log = []
	return user_info
...
def validate_python_code(string: str, fieldname=None, is_expression: bool = True) -> None:
	...
	try:
		...
		msg = finergy._("{} Invalid python code on line {}").format(
			fieldname + ":" if fieldname else "", line_no + 1
		)
		msg += f"<br><pre>{error_line}</pre>"
		msg += f"<pre>{' ' * offset}^</pre>"

		finergy.throw(msg, title=finergy._("Syntax Error"))
	except Exception as e:
		finergy.msgprint(
			finergy._("{} Possibly invalid python code. <br>{}").format(fieldname + ": " or "", str(e)),
			indicator="orange",
		)
...
def format_timedelta(o: datetime.timedelta) -> str:
	...
	# but finergy doesnt - i think via babel : only allows 0..23 range for hour
