...
import finergy
from finergy import _, safe_decode
...
def get_change_log(user=None):
	...
	last_known_versions = finergy._dict(
		json.loads(finergy.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "finergy" in current_versions:
		set_in_change_log("finergy", current_versions["finergy"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(finergy.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = finergy.read_file(file_path)
...
def get_versions():
	...
	Example:
	        ...
	        {
	                "finergy": {
	                        "title": "Finergy Framework",
	                        "version": "5.0.0"
	                }
	        }"""
	versions = {}
	for app in finergy.get_installed_apps(sort=True):
		app_hooks = finergy.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = finergy.get_attr(app + ".__version__")
...
def check_for_update():
	updates = finergy._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def check_release_on_github(app: str):
	"""
	Check the latest release for a given Finergy application hosted on Github.

	Args:
	        app (str): The name of the Finergy application.
...
def add_message_to_redis(update_json):
	...
	user_list = [x.name for x in finergy.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in finergy.get_roles(user)]
...
def show_update_popup():
	...
	if update_message:
		finergy.msgprint(update_message, title=_("New updates are available"), indicator="green")
