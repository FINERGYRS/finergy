...
from finergy.utils import cint, strip_html_tags
...
def reset():
	...
	:return:
	"""
	finergy.db.sql("DELETE FROM `__global_search`")
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in finergy.get_all("DocType", fields=["name", "module"], filters=filters):
			meta = finergy.get_meta(d.name)
			if len(meta.get_global_search_fields()) > 0:
				global_search_doctypes.append(d)

		installed_apps = finergy.get_installed_apps()
		module_app = finergy.local.module_app

		doctypes = [
			d.name
			for d in global_search_doctypes
			if module_app.get(finergy.scrub(d.module))
			and module_app[finergy.scrub(d.module)] in installed_apps
		]

		return doctypes

	return finergy.cache().get_value("doctypes_with_global_search", _get)
...
def rebuild_for_doctype(doctype):
	...
	"""
	if finergy.local.conf.get("disable_global_search"):
		return

	def _get_filters():
		filters = finergy._dict({"docstatus": ["!=", 2]})
	...
	if cint(meta.istable) == 1:
		parent_doctypes = finergy.get_all(
			"DocField",
			fields="parent",
			filters={"fieldtype": ["in", finergy.model.table_fields], "options": doctype},
		)
	...
	all_records = finergy.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = finergy.get_doc(doctype, doc.name)
			...
			all_contents.append(
				{
					"doctype": finergy.db.escape(doctype),
					"name": finergy.db.escape(doc.name),
					"content": finergy.db.escape(" ||| ".join(content or "")),
					"published": published,
					"title": finergy.db.escape((title or "")[: int(finergy.db.VARCHAR_LEN)]),
					"route": finergy.db.escape((route or "")[: int(finergy.db.VARCHAR_LEN)]),
				}
			)
	if all_contents:
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = finergy._dict()
	child_search_fields = finergy._dict()

	for child in meta.get_table_fields():
		child_meta = finergy.get_meta(child.options)
		...
		if search_fields:
			...
			child_records = finergy.get_all(
				child.options, fields=child_fieldnames, filters={"docstatus": ["!=", 1], "parenttype": doctype}
			)

			for record in child_records:
				all_children.setdefault(record.parent, finergy._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	for field in doc.meta.get_global_search_fields():
		if doc.get(field.fieldname) and field.fieldtype not in finergy.model.table_fields:
	...
	if content:
		...
		title = (doc.get_title() or "")[: int(finergy.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if finergy.conf.get("disable_global_search"):
		return
...
def get_routes_to_index():
	...
	routes_to_index = []
	for app in apps:
		base = finergy.get_app_path(app, "www")
		path_to_index = finergy.get_app_path(app, "www")
...
def add_route_to_global_search(route):
	...
	try:
		...
		sync_value_in_queue(value)
	except (finergy.PermissionError, finergy.DoesNotExistError, finergy.ValidationError, Exception):
		pass
...
def get_formatted_value(value, field):
	...
	if getattr(field, "fieldtype", None) in ["Text", "Text Editor"]:
		value = unescape_html(finergy.safe_decode(value))
...
def sync_global_search():
	...
	:return:
	"""
	while finergy.cache().llen("global_search_queue") > 0:
		# rpop to follow FIFO
		# Last one should override all previous contents of same document
		value = json.loads(finergy.cache().rpop("global_search_queue").decode("utf-8"))
		sync_value(value)
...
def sync_value_in_queue(value):
	try:
		...
		finergy.cache().lpush("global_search_queue", json.dumps(value))
...
def search(text, start=0, limit=20, doctype=""):
	...
	"""
	from finergy.desk.doctype.global_search_settings.global_search_settings import (
		get_doctypes_for_global_search,
	)
	from finergy.query_builder.functions import Match
	...
	for doctype in allowed_doctypes:
		for index, r in enumerate(results):
			if r.doctype == doctype and r.rank > 0.0:
				try:
					meta = finergy.get_meta(r.doctype)
					if meta.image_field:
						r.image = finergy.db.get_value(r.doctype, r.name, meta.image_field)
				except Exception:
