...
from finergy.query_builder import Table
from finergy.utils import cstr, encode
...
def check_password(user, pwd, doctype="User", fieldname="password", delete_tracker_cache=True):
	"""Checks if user and password are correct, else raises finergy.AuthenticationError"""
	...
	if not result or not passlibctx.verify(pwd, result[0].password):
		raise finergy.AuthenticationError(_("Incorrect User or Password"))
...
def delete_login_failed_cache(user):
	finergy.cache().hdel("last_login_tried", user)
	finergy.cache().hdel("login_failed_count", user)
	finergy.cache().hdel("locked_account_time", user)
...
def rename_password(doctype, old_name, new_name):
	...
	finergy.qb.update(Auth).set(Auth.name, new_name).where(
		(Auth.doctype == doctype) & (Auth.name == old_name)
	).run()


def rename_password_field(doctype, old_fieldname, new_fieldname):
	finergy.qb.update(Auth).set(Auth.fieldname, new_fieldname).where(
...
def decrypt(pwd):
	try:
		...
		finergy.throw(_("Encryption key is invalid, Please check site_config.json"))


def get_encryption_key():
	from finergy.installer import update_site_config

	if "encryption_key" not in finergy.local.conf:
		encryption_key = Fernet.generate_key().decode()
		update_site_config("encryption_key", encryption_key)
		finergy.local.conf.encryption_key = encryption_key

	return finergy.local.conf.encryption_key


def get_password_reset_limit():
	return finergy.db.get_single_value("System Settings", "password_reset_limit") or 0
