...
from finergy.utils import cint, cstr, flt
...
class DBTable:
	def __init__(self, doctype, meta=None):
		...
		self.meta = meta or finergy.get_meta(doctype, False)
		self.columns = {}
	...
	def sync(self):
		...
		if self.is_new():
			...
			finergy.cache().hdel("table_columns", self.table_name)
			self.alter()
	...
	def get_column_definitions(self):
		column_list = [] + finergy.db.DEFAULT_COLUMNS
		ret = []
	...
	def get_index_definitions(self):
		...
		for key, col in self.columns.items():
			if (
				col.set_index
				and not col.unique
				and col.fieldtype in finergy.db.type_map
				and finergy.db.type_map.get(col.fieldtype)[0] not in ("text", "longtext")
			):
	...
	def get_columns_from_docfields(self):
		...
		if not self.meta.get("istable"):
			for fieldname in finergy.db.OPTIONAL_COLUMNS:
	...
	def validate(self):
		...
		columns = [
			finergy._dict({"fieldname": f, "fieldtype": "Data"}) for f in finergy.db.STANDARD_VARCHAR_COLUMNS
		]
		...
		for col in columns:
			if len(col.fieldname) >= 64:
				finergy.throw(
					_("Fieldname is limited to 64 characters ({0})").format(finergy.bold(col.fieldname))
				)

			if "varchar" in finergy.db.type_map.get(col.fieldtype, ()):

				# validate length range
				new_length = cint(col.length) or cint(finergy.db.VARCHAR_LEN)
				if not (1 <= new_length <= 1000):
					finergy.throw(_("Length of {0} should be between 1 and 1000").format(col.fieldname))
				...
				if cint(current_length) != cint(new_length):
					try:
						...
						if finergy.db.is_missing_column(e):
					...
					if max_length and max_length[0][0] and max_length[0][0] > new_length:
						...
						finergy.msgprint(info_message)

	def is_new(self):
		return self.table_name not in finergy.db.get_tables()

	def setup_table_columns(self):
		# TODO: figure out a way to get key data
		for c in finergy.db.get_table_columns_description(self.table_name):
...
class DbColumn:
	...
	def get_definition(self, with_default=1):
		...
		if self.fieldtype in ("Check", "Int"):
			...
			self.default
			and (self.default not in finergy.db.DEFAULT_SHORTCUTS)
			and not cstr(self.default).startswith(":")
			and column_def not in ("text", "longtext")
		):
			column_def += " default {}".format(finergy.db.escape(self.default))
	...
	if special_characters:
		...
		finergy.throw(
			_("Fieldname {0} cannot have special characters like {1}").format(
				finergy.bold(cstr(n)), special_characters
			),
			finergy.db.InvalidColumnName,
		)
	return n


def validate_column_length(fieldname):
	if len(fieldname) > finergy.db.MAX_COLUMN_LENGTH:
		finergy.throw(_("Fieldname is limited to 64 characters ({0})").format(fieldname))


def get_definition(fieldtype, precision=None, length=None):
	d = finergy.db.type_map.get(fieldtype)

	# convert int to long int if the length of the int is greater than 11
	if fieldtype == "Int" and length and length > 11:
		d = finergy.db.type_map.get("Long Int")
...
def add_column(doctype, column_name, fieldtype, precision=None):
	if column_name in finergy.db.get_table_columns(doctype):
		# already exists
		return
