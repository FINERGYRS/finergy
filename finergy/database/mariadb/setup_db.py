...
import finergy
from finergy.database.db_manager import DbManager
...
def get_mariadb_versions():
	...
	mariadb_variables = finergy._dict(finergy.db.sql("""show variables"""))
...
def setup_database(force, source_sql, verbose, no_mariadb_socket=False):
	finergy.local.session = finergy._dict({"user": "Administrator"})

	db_name = finergy.local.conf.db_name
	...
	dbman.create_user(db_name, finergy.conf.db_password)
	if no_mariadb_socket:
		dbman.create_user(db_name, finergy.conf.db_password, host="%")
	if verbose:
...
def setup_help_database(help_db_name):
	dbman = DbManager(get_root_connection(finergy.flags.root_login, finergy.flags.root_password))
...
def drop_user_and_database(db_name, root_login, root_password):
	finergy.local.db = get_root_connection(root_login, root_password)
	dbman = DbManager(finergy.local.db)
	dbman.delete_user(db_name, host="%")
	dbman.delete_user(db_name)
	dbman.drop_database(db_name)
...
def bootstrap_database(db_name, verbose, source_sql=None):
	...
	finergy.connect(db_name=db_name)
	if "tabDefaultValue" not in finergy.db.get_tables(cached=False):
...
def import_db_from_sql(source_sql=None, verbose=False):
	...
	db_name = finergy.conf.db_name
	if not source_sql:
		source_sql = os.path.join(os.path.dirname(__file__), "framework_mariadb.sql")
	DbManager(finergy.local.db).restore_database(db_name, source_sql, db_name, finergy.conf.db_password)
	if verbose:
