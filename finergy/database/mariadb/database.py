...
from finergy.database.mariadb.schema import MariaDBTable
from finergy.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBDatabase(Database):
	...
	def get_connection(self):
		usessl = 0
		if finergy.conf.db_ssl_ca and finergy.conf.db_ssl_cert and finergy.conf.db_ssl_key:
			usessl = 1
			ssl_params = {
				"ca": finergy.conf.db_ssl_ca,
				"cert": finergy.conf.db_ssl_cert,
				"key": finergy.conf.db_ssl_key,
			}
		...
		conn = pymysql.connect(
			...
			conv=conversions,
			local_infile=finergy.conf.local_infile,
		)
	...
	def escape(s, percent=True):
		...
		s = finergy.as_unicode(escape_string(finergy.as_unicode(s)), "utf-8").replace("`", "\\`")
