...
from finergy.database.schema import DBTable
...
class MariaDBTable(DBTable):
	def create(self):
		...
		finergy.db.sql(
			...
			COLLATE=utf8mb4_unicode_ci""".format(
				varchar_len=finergy.db.VARCHAR_LEN, engine=self.meta.get("engine") or "InnoDB"
			)
	...
	def alter(self):
		...
		for col in self.add_index:
			...
			if not finergy.db.has_index(self.table_name, col.fieldname + "_index"):
		...
		for col in self.drop_index + self.drop_unique:
			if col.fieldname != "name":  # primary key
				...
				if unique_constraint_changed and not col.unique:
					# nosemgrep
					unique_index_record = finergy.db.sql(
						"""
						SHOW INDEX FROM `{0}`
						WHERE Key_name=%s
						AND Non_unique=0
