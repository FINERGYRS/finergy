...
from finergy.database.schema import DBTable, get_definition
...
class PostgresTable(DBTable):
	def create(self):
		...
		finergy.db.sql(
			...
			%s)""".format(
				varchar_len=finergy.db.VARCHAR_LEN
			)
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			finergy.db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = "{}".format(finergy.db.escape(col.default))
		...
		try:
			if query:
				...
				# nosemgrep
				finergy.db.sql(final_alter_query)
			if create_contraint_query:
				# nosemgrep
				finergy.db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				finergy.db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if finergy.db.is_duplicate_fieldname(e):
				finergy.throw(str(e))
			elif finergy.db.is_duplicate_entry(e):
