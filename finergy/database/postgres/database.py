...
from finergy.database.postgres.schema import PostgresTable
from finergy.utils import cstr, get_table_name
...
class PostgresDatabase(Database):
		...
		return [
			...
			where table_catalog='{0}'
				...
				and table_schema='{1}'""".format(
					finergy.conf.db_name, finergy.conf.get("db_schema", "public")
				)
			)
		]
	...
	def format_date(self, date):
		...
		if not isinstance(date, finergy.string_types):
