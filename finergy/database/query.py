...
from finergy.query_builder import Criterion, Field, Order


def like(key: str, value: str) -> finergy.qb:
	...
	Returns:
	        finergy.qb: `finergy.qb object with `LIKE`
	"""
	return Field(key).like(value)


def func_in(key: str, value: Union[List, Tuple]) -> finergy.qb:
	...
	Returns:
	        finergy.qb: `finergy.qb object with `IN`
	"""
	return Field(key).isin(value)


def not_like(key: str, value: str) -> finergy.qb:
	...
	Returns:
	        finergy.qb: `finergy.qb object with `NOT LIKE`
	"""
...
def func_not_in(key: str, value: Union[List, Tuple]):
	...
	Returns:
	        finergy.qb: `finergy.qb object with `NOT IN`
	"""
	return Field(key).notin(value)


def func_regex(key: str, value: str) -> finergy.qb:
	...
	Returns:
	        finergy.qb: `finergy.qb object with `REGEX`
	"""
	return Field(key).regex(value)


def func_between(key: str, value: Union[List, Tuple]) -> finergy.qb:
	...
	Returns:
	        finergy.qb: `finergy.qb object with `BETWEEN`
	"""
...
class Query:
	def get_condition(self, table: str, **kwargs) -> finergy.qb:
		...
		Returns:
		        finergy.qb: DocType with initial condition
		"""
		if kwargs.get("update"):
			return finergy.qb.update(table)
		if kwargs.get("into"):
			return finergy.qb.into(table)
		return finergy.qb.from_(table)

	def criterion_query(self, table: str, criterion: Criterion, **kwargs) -> finergy.qb:
		...
		Returns:
		        finergy.qb: condition object
		"""
		condition = self.add_conditions(self.get_condition(table, **kwargs), **kwargs)
		return condition.where(criterion)

	def add_conditions(self, conditions: finergy.qb, **kwargs):
		"""Adding additional conditions

		Args:
		        conditions (finergy.qb): built conditions

		Returns:
		        conditions (finergy.qb): finergy.qb object
		"""
	...
	def build_conditions(
		...
		Returns:
		        finergy.qb: finergy.qb conditions object
		"""
...
class Permission:
	...
	def check_permissions(cls, query, **kwargs):
		...
		for dt in doctype:
			...
			) and not finergy.has_permission(
				dt,
				"read",
				user=kwargs.get("user"),
				parent_doctype=kwargs.get("parent_doctype"),
			):
				finergy.throw(_("Insufficient Permission for {0}").format(finergy.bold(dt)))
