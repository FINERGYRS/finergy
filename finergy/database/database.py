...
from finergy.database.query import Query
from finergy.model.utils.link_count import flush_local_link_count
from finergy.query_builder.utils import DocType
from finergy.utils import cast, get_datetime, get_table_name, getdate, now, sbool
...
class Database(object):
	...
	class InvalidColumnName(finergy.ValidationError):
		pass

	def __init__(self, host=None, user=None, password=None, ac_name=None, use_default=0, port=None):
		self.setup_type_map()
		self.host = host or finergy.conf.db_host or "127.0.0.1"
		self.port = port or finergy.conf.db_port or ""
		self.user = user or finergy.conf.db_name
		self.db_name = finergy.conf.db_name
		self._conn = None

		if ac_name:
			self.user = ac_name or finergy.conf.db_name

		if use_default:
			self.user = finergy.conf.db_name

		self.transaction_writes = 0
		self.auto_commit_on_many_writes = 0

		self.password = password or finergy.conf.db_password
		self.value_cache = {}
		self.query = Query()
	...
	def connect(self):
		...
		finergy.local.rollback_observers = []
	...
	def sql(
		...
		Examples:
		        ...
		        finergy.db.sql("select name from tabCustomer", as_dict=True)

		        # return names beginning with a
		        finergy.db.sql("select name from tabCustomer where name like %s", "a%")

		        # values as dict
		        finergy.db.sql("select name from tabCustomer where name like %(name)s and owner=%(owner)s",
		...
		try:
			...
			if debug:
				time_end = time()
				finergy.errprint(("Execution time: {0} sec").format(round(time_end - time_start, 2)))
			...
			if finergy.conf.db_type == "postgres":
				...
				# only for mariadb
				finergy.errprint("Syntax error in query:")
				finergy.errprint(query)

			elif self.is_deadlocked(e):
				raise finergy.QueryDeadlockError(e)

			elif self.is_timedout(e):
				raise finergy.QueryTimeoutError(e)
	...
	def log_query(self, query, values, debug, explain):
		...
		if finergy.conf.get("allow_tests") and finergy.cache().get_value("flag_print_sql"):
		...
		if debug:
			...
			finergy.errprint(self.mogrify(query, values))

		# info
		if (finergy.conf.get("logging") or False) == 2:
			finergy.log("<<<< query")
			finergy.log(self.mogrify(query, values))
			finergy.log(">>>>")
	...
	def explain_query(self, query, values=None):
		...
		try:
			finergy.errprint("--- query explain ---")
			if values is None:
			...
			finergy.errprint(json.dumps(self.fetch_as_dict(), indent=1))
			finergy.errprint("--- query explain end ---")
		except Exception:
			finergy.errprint("error in query explain")
	...
	def sql_list(self, query, values=(), debug=False):
		...
		Example:
		        ...
		        doctypes = finergy.db.sql_list("select name from DocType")
		"""
		...
		if query[:6].lower() in ("update", "insert", "delete"):
			...
			if self.transaction_writes > self.MAX_WRITES_PER_TRANSACTION:
				if self.auto_commit_on_many_writes:
					...
					raise finergy.TooManyWritesError(msg)
	...
	def fetch_as_dict(self, formatted=0, as_utf8=0):
		...
		for r in result:
			...
			ret.append(finergy._dict(zip(keys, values)))
		return ret

	@staticmethod
	def clear_db_table_cache(query):
		if query and query.strip().split()[0].lower() in {"drop", "create"}:
			finergy.cache().delete_key("db_tables")
	...
	def get_value(
		...
		Example:
		        ...
		        finergy.db.get_value("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        finergy.db.get_value("User", "test@example.com", "last_login")

		        last_login, last_ip = finergy.db.get_value("User", "test@example.com",
		                ["last_login", "last_ip"])

		        # returns default date_format
		        finergy.db.get_value("System Settings", None, "date_format")
		"""
	...
	def get_values(
		...
		*,
		...
		Example:
		        ...
		        # return first customer starting with a
		        customers = finergy.db.get_values("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        user = finergy.db.get_values("User", "test@example.com", "*")[0]
		"""
		out = None
		if (
			cache
		...
		if isinstance(filters, list):
			...
			if (filters is not None) and (filters != doctype or doctype == "DocType"):
				try:
					...
					if ignore and (finergy.db.is_missing_column(e) or finergy.db.is_table_missing(e)):
						# table or column not found, return None
						out = None
					elif (not ignore) and finergy.db.is_table_missing(e):
	...
	def get_values_from_single(
		...
		"""
		# TODO
		# if not finergy.model.meta.is_single(doctype):
		# 	raise finergy.DoesNotExistError("DocType", doctype)
	...
	def get_singles_dict(self, doctype, debug=False):
		...
		Example:
		        ...
		        account_settings = finergy.db.get_singles_dict("Accounts Settings")
		"""
		result = self.sql(
			"""
			SELECT field, value
		...
		dict_ = finergy._dict(result)

		return dict_

	@staticmethod
	def get_all(*args, **kwargs):
		return finergy.get_all(*args, **kwargs)

	@staticmethod
	def get_list(*args, **kwargs):
		return finergy.get_list(*args, **kwargs)
	...
	def set_single_value(self, doctype, fieldname, value, *args, **kwargs):
		...
		Example:
		        ...
		        company = finergy.db.set_single_value("System Settings", "deny_multiple_sessions", True)
		"""
	...
	def get_single_value(self, doctype, fieldname, cache=True):
		...
		df = finergy.get_meta(doctype).get_field(fieldname)

		if not df:
			finergy.throw(
				_("Invalid field name: {0}").format(finergy.bold(fieldname)), self.InvalidColumnName
			)
	...
	def set_value(
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		if update_modified:
			...
			modified_by = modified_by or finergy.session.user
		...
		if is_single_doctype:
			...
			query = (
				finergy.qb.into("Singles").columns("doctype", "field", "value").insert(*singles_data)
			).run(debug=debug)
			finergy.clear_document_cache(dt, dt)
			...
			if for_update:
				...
				query = finergy.qb.update(table).where(table.name.isin(docnames))

				for docname in docnames:
					finergy.clear_document_cache(dt, docname)

			else:
				query = self.query.build_conditions(table=dt, filters=dn, update=True)
				# TODO: Fix this; doesn't work rn - gavin@finergy-rs.fr
				# finergy.cache().hdel_keys(dt, "document_cache")
				# Workaround: clear all document caches
				finergy.cache().delete_value("document_cache")
	...
	def set_temp(value):
		...
		key = finergy.generate_hash()
		finergy.cache().hset("temp", key, value)
		return key

	@staticmethod
	def get_temp(key):
		"""Return the temperory value and delete it."""
		return finergy.cache().hget("temp", key)
	...
	def set_default(key, val, parent="__default", parenttype=None):
		...
		finergy.defaults.set_default(key, val, parent, parenttype)
	...
	def add_default(key, val, parent="__default", parenttype=None):
		...
		finergy.defaults.add_default(key, val, parent, parenttype)

	@staticmethod
	def get_defaults(key=None, parent="__default"):
		"""Get all defaults"""
		if key:
			defaults = finergy.defaults.get_defaults(parent)
			d = defaults.get(key, None)
			if not d and key != finergy.scrub(key):
				d = defaults.get(finergy.scrub(key), None)
			return d
		else:
			return finergy.defaults.get_defaults(parent)
	...
	def commit(self):
		...
		for method in finergy.local.before_commit:
			finergy.call(method[0], *(method[1] or []), **(method[2] or {}))
	...
	def add_before_commit(self, method, args=None, kwargs=None):
		finergy.local.before_commit.append([method, args, kwargs])

	@staticmethod
	def flush_realtime_log():
		for args in finergy.local.realtime_log:
			finergy.realtime.emit_via_redis(*args)

		finergy.local.realtime_log = []

	def savepoint(self, save_point):
		"""Savepoints work as a nested transaction.

		Changes can be undone to a save point by doing finergy.db.rollback(save_point)
	...
	def rollback(self, *, save_point=None):
		...
		if save_point:
			...
			self.sql("rollback")
			self.begin()
			for obj in finergy.local.rollback_observers:
	...
	def get_tables(self, cached=True):
		tables = finergy.cache().get_value("db_tables")
		if not tables or not cached:
			...
			finergy.cache().set_value("db_tables", tables)
		return tables
	...
	def count(self, dt, filters=None, debug=False, cache=False):
		...
		if cache and not filters:
			cache_count = finergy.cache().get_value("doctype:count:{}".format(dt))
		...
		if filters:
			...
			if cache:
				finergy.cache().set_value("doctype:count:{}".format(dt), count, expires_in_sec=86400)
	...
	def format_datetime(datetime):
		...
		if isinstance(datetime, finergy.string_types):
			if ":" not in datetime:
	...
	def get_db_table_columns(self, table):
		...
		columns = finergy.cache().hget("table_columns", table)
		if columns is None:
			columns = [
				r[0]
				for r in self.sql(
			...
			if columns:
				finergy.cache().hset("table_columns", table, columns)
	...
	def get_system_setting(self, key):
		...
		return finergy.cache().get_value("system_settings", _load_system_settings).get(key)
	...
	def is_column_missing(e):
		return finergy.db.is_missing_column(e)
	...
	def multisql(self, sql_dict, values=(), **kwargs):
		current_dialect = finergy.db.db_type or "mariadb"
	...
	def log_touched_tables(self, query, values=None):
		if values:
			query = finergy.safe_decode(self._cursor.mogrify(query, values))
		if query.strip().lower().split()[0] in ("insert", "delete", "update", "alter", "drop", "rename"):
			...
			if finergy.flags.touched_tables is None:
				finergy.flags.touched_tables = set()
			finergy.flags.touched_tables.update(tables)
...
def enqueue_jobs_after_commit():
	from finergy.utils.background_jobs import execute_job, get_queue

	if finergy.flags.enqueue_after_commit and len(finergy.flags.enqueue_after_commit) > 0:
		for job in finergy.flags.enqueue_after_commit:
		...
		finergy.flags.enqueue_after_commit = []
