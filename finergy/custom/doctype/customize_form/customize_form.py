...
from finergy.custom.doctype.property_setter.property_setter import delete_property_setter
from finergy.model import core_doctypes_list, no_value_fields
...
class CustomizeForm(Document):
	def on_update(self):
		finergy.db.sql("delete from tabSingles where doctype='Customize Form'")
		finergy.db.sql("delete from `tabCustomize Form Field`")
	...
	def validate_doctype(self, meta):
		...
		if self.doc_type in core_doctypes_list:
			finergy.throw(_("Core DocTypes cannot be customized."))

		if meta.issingle:
			finergy.throw(_("Single DocTypes cannot be customized."))

		if meta.custom:
			finergy.throw(_("Only standard DocTypes are allowed to be customized from Customize Form."))
	...
	def get_name_translation(self):
		...
		return finergy.get_value(
			"Translation",
			{"source_text": self.doc_type, "language": finergy.local.lang or "en"},
	...
	def set_name_translation(self):
		...
		if not self.label:
			if current:
				# clear translation
				finergy.delete_doc("Translation", current.name)
			return
			...
			finergy.get_doc(
				{
					...
					"language_code": finergy.local.lang or "en",
				}
			).insert()
			return

		if self.label != current.translated_text:
			finergy.db.set_value("Translation", current.name, "translated_text", self.label)
	...
	def save_customization(self):
		...
		if self.flags.update_db:
			finergy.db.updatedb(self.doc_type)

		if not hasattr(self, "hide_success") or not self.hide_success:
			finergy.msgprint(_("{0} updated").format(_(self.doc_type)), alert=True)
		finergy.clear_cache(doctype=self.doc_type)
		self.fetch_to_customize()

		if self.flags.rebuild_doctype_for_global_search:
			finergy.enqueue(
				"finergy.utils.global_search.rebuild_for_doctype", now=True, doctype=self.doc_type
			)
	...
	def allow_property_change(self, prop, meta_df, df):
		if prop == "fieldtype":
			...
			(
				finergy.db.get_value("DocField", {"parent": self.doc_type, "fieldname": df.fieldname}, "reqd")
				== 1
			)
			and (df.get(prop) == 0)
		):
			...
			finergy.msgprint(_("You cannot unset 'Read Only' for field {0}").format(df.label))
			return False

		elif prop == "options" and df.get("fieldtype") not in ALLOWED_OPTIONS_CHANGE:
			finergy.msgprint(_("You can't set 'Options' for field {0}").format(df.label))
			return False

		elif prop == "translatable" and not supports_translation(df.get("fieldtype")):
			finergy.msgprint(_("You can't set 'Translatable' for field {0}").format(df.label))
			return False
	...
	def set_property_setters_for_actions_and_links(self, meta):
		...
		for doctype, fieldname, field_map in (
			...
			for i, d in enumerate(self.get(fieldname) or []):
				d.idx = i
				if finergy.db.exists(doctype, d.name) and not d.custom:
					# check property and apply property setter
					original = finergy.get_doc(doctype, d.name)
	...
	def update_order_property_setter(self, has_custom, fieldname):
		...
		if has_custom:
			...
			finergy.db.delete("Property Setter", dict(property=property_name, doc_type=self.doc_type))
	...
	def clear_removed_items(self, doctype, items):
		...
		"""
		if items:
			finergy.db.delete(doctype, dict(parent=self.doc_type, custom=1, name=("not in", items)))
		else:
			finergy.db.delete(doctype, dict(parent=self.doc_type, custom=1))

	def update_custom_fields(self):
		for i, df in enumerate(self.get("fields")):
			if df.get("is_custom_field"):
				if not finergy.db.exists("Custom Field", {"dt": self.doc_type, "fieldname": df.fieldname}):
	...
	def add_custom_field(self, df, i):
		d = finergy.new_doc("Custom Field")
	...
	def update_in_custom_field(self, df, i):
		...
		custom_field = finergy.get_doc("Custom Field", meta_df[0].name)
		changed = False
		for prop in docfield_properties:
	...
	def delete_custom_fields(self):
		...
		for fieldname in fields_to_remove:
			...
			if df.get("is_custom_field"):
				finergy.delete_doc("Custom Field", df.name)
	...
	def get_existing_property_value(self, property_name, fieldname=None):
		...
		if fieldname:
			property_value = finergy.db.get_value(
				"DocField", {"parent": self.doc_type, "fieldname": fieldname}, property_name
			)
		else:
			if finergy.db.has_column("DocType", property_name):
				property_value = finergy.db.get_value("DocType", self.doc_type, property_name)
			else:
				property_value = None
	...
	def validate_fieldtype_change(self, df, old_value, new_value):
		...
		if allowed:
			old_value_length = cint(finergy.db.type_map.get(old_value)[1])
			new_value_length = cint(finergy.db.type_map.get(new_value)[1])
	...
	def validate_fieldtype_length(self):
		for field in self.check_length_for_fieldtypes:
			df = field.get("df")
			max_length = cint(finergy.db.type_map.get(df.fieldtype)[1])
			...
			label = df.label
			for doc in docs:
				links.append(finergy.utils.get_link_to_form(self.doc_type, doc.name))
			...
			if docs:
				finergy.throw(
					...
					).format(finergy.bold(label), links_str, finergy.bold(max_length)),
...
def reset_customization(doctype):
	...
	for setter in setters:
		finergy.delete_doc("Property Setter", setter)
