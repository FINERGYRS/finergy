...
import finergy
from finergy.core.doctype.doctype.doctype import InvalidFieldNameError
...
class TestCustomizeForm(unittest.TestCase):
	def insert_custom_field(self):
		finergy.delete_doc_if_exists("Custom Field", "Event-test_custom_field")
		finergy.get_doc(
			{
				...
				"options": "\nCustom 1\nCustom 2\nCustom 3",
				"default": "Custom 3",
				"insert_after": finergy.get_meta("Event").fields[-1].fieldname,
			}
		).insert()

	def setUp(self):
		self.insert_custom_field()
		finergy.db.delete("Property Setter", dict(doc_type="Event"))
		finergy.db.commit()
		finergy.clear_cache(doctype="Event")

	def tearDown(self):
		finergy.delete_doc("Custom Field", "Event-test_custom_field")
		finergy.db.commit()
		finergy.clear_cache(doctype="Event")

	def get_customize_form(self, doctype=None):
		d = finergy.get_doc("Customize Form")
		if doctype:
			d.doc_type = doctype
	...
	def test_fetch_to_customize(self):
		...
		self.assertEqual(len(d.get("fields")), len(finergy.get_doc("DocType", d.doc_type).fields) + 1)
	...
	def test_save_customization_custom_field_property(self):
		d = self.get_customize_form("Event")
		self.assertEquals(finergy.db.get_value("Custom Field", "Event-test_custom_field", "reqd"), 0)

		custom_field = d.get("fields", {"fieldname": "test_custom_field"})[0]
		custom_field.reqd = 1
		custom_field.no_copy = 1
		d.run_method("save_customization")
		self.assertEqual(finergy.db.get_value("Custom Field", "Event-test_custom_field", "reqd"), 1)
		self.assertEqual(finergy.db.get_value("Custom Field", "Event-test_custom_field", "no_copy"), 1)

		custom_field = d.get("fields", {"is_custom_field": True})[0]
		custom_field.reqd = 0
		custom_field.no_copy = 0
		d.run_method("save_customization")
		self.assertEqual(finergy.db.get_value("Custom Field", "Event-test_custom_field", "reqd"), 0)
		self.assertEqual(finergy.db.get_value("Custom Field", "Event-test_custom_field", "no_copy"), 0)
	...
	def test_save_customization_new_field(self):
		...
		finergy.delete_doc("Custom Field", "Event-test_add_custom_field_via_customize_form")
		self.assertEquals(
			finergy.db.get_value("Custom Field", "Event-test_add_custom_field_via_customize_form"), None
		)
	...
	def test_save_customization_remove_field(self):
		...
		self.assertEqual(finergy.db.get_value("Custom Field", custom_field.name), None)

		finergy.local.test_objects["Custom Field"] = []
	...
	def test_reset_to_defaults(self):
		...
		finergy.local.test_objects["Property Setter"] = []
	...
	def test_core_doctype_customization(self):
		self.assertRaises(finergy.ValidationError, self.get_customize_form, "User")
	...
	def test_custom_internal_links(self):
		...
		finergy.clear_cache()
		user_group = finergy.get_meta("User Group")
		...
		finergy.clear_cache()
		user_group = finergy.get_meta("Event")
		self.assertFalse(
