...
finergy.ui.form.on('Custom Field', {
	setup: function(frm) {
		frm.set_query('dt', function(doc) {
			var filters = [
				...
				['DocType', 'restrict_to_domain', 'in', finergy.boot.active_domains]
			];
	...
	dt: function(frm) {
		...
		return finergy.call({
			method: 'finergy.custom.doctype.custom_field.custom_field.get_fields_label',
	...
	label: function(frm) {
		if(frm.doc.label && finergy.utils.has_special_chars(frm.doc.label)) {
...
finergy.utils.has_special_chars = function(t) {
	var iChars = "!@#$%^&*()+=-[]\\\';,./{}|\":<>?";
