...
class PropertySetter(Document):
	...
	def validate_fieldtype_change(self):
		if self.field_name in not_allowed_fieldtype_change and self.property == "fieldtype":
			finergy.throw(_("Field type cannot be changed for {0}").format(self.field_name))
	...
	def get_property_list(self, dt):
		return finergy.db.get_all(
			...
			filters={
				...
				"fieldtype": [
					"not in",
					("Section Break", "Column Break", "HTML", "Read Only", "Fold") + finergy.model.table_fields,
				],
				"fieldname": ["!=", ""],
			},
			order_by="label asc",
	...
	def get_setup_data(self):
		return {
			"doctypes": [d[0] for d in finergy.db.sql("select name from tabDocType")],
	...
	def get_defaults(self):
		if not self.field_name:
			return finergy.db.sql("select * from `tabDocType` where name=%s", self.doc_type, as_dict=1)[0]
		else:
			return finergy.db.sql(
...
def make_property_setter(
	...
	property_setter = finergy.get_doc(
		{
...
def delete_property_setter(doc_type, property, field_name=None, row_name=None):
	...
	finergy.db.delete("Property Setter", filters)
