...
class ParallelTestRunner:
	def __init__(self, app, site, build_number=1, total_builds=1, with_coverage=False):
		...
		self.build_number = finergy.utils.cint(build_number) or 1
		self.total_builds = finergy.utils.cint(total_builds)
		self.setup_test_site()
		self.run_tests()

	def setup_test_site(self):
		finergy.init(site=self.site)
		if not finergy.db:
			finergy.connect()
	...
	def before_test_setup(self):
		...
		for fn in finergy.get_hooks("before_tests", app_name=self.app):
			finergy.get_attr(fn)()

		test_module = finergy.get_module(f"{self.app}.tests")
	...
	def get_module(self, path, filename):
		app_path = finergy.get_pymodule_path(self.app)
	...
	def start_coverage(self):
		if self.with_coverage:
			...
			from finergy.utils import get_bench_path
	...
	def get_test_file_list(self):
		...
		split_size = finergy.utils.ceil(len(test_list) / self.total_builds)
...
class ParallelTestWithOrchestrator(ParallelTestRunner):
	...
	def __init__(self, app, site, with_coverage=False):
		...
		if not self.orchestrator_url:
			...
			click.echo("Pass public URL after hosting https://github.com/finergyrs/test-orchestrator")
			sys.exit(1)

		self.ci_build_id = os.environ.get("CI_BUILD_ID")
		self.ci_instance_id = os.environ.get("CI_INSTANCE_ID") or finergy.generate_hash(length=10)
