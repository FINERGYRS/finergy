...
class WorkflowStateError(finergy.ValidationError):
	pass


class WorkflowTransitionError(finergy.ValidationError):
	pass


class WorkflowPermissionError(finergy.ValidationError):
	pass


def get_workflow_name(doctype):
	workflow_name = finergy.cache().hget("workflow", doctype)
	if workflow_name is None:
		workflow_name = finergy.db.get_value(
			"Workflow", {"document_type": doctype, "is_active": 1}, "name"
		)
		finergy.cache().hset("workflow", doctype, workflow_name or "")
...
def get_transitions(doc, workflow=None, raise_exception=False):
	...
	doc = finergy.get_doc(finergy.parse_json(doc))
	...
	finergy.has_permission(doc, "read", throw=True)
	...
	if not current_state:
		if raise_exception:
			...
			finergy.throw(_("Workflow State not set"), WorkflowStateError)
...
def get_workflow_safe_globals():
	# access to finergy.db.get_value, finergy.db.get_list, and date time utils.
	return dict(
		finergy=finergy._dict(
			db=finergy._dict(get_value=finergy.db.get_value, get_list=finergy.db.get_list),
			session=finergy.session,
			utils=finergy._dict(
				now_datetime=finergy.utils.now_datetime,
				add_to_date=finergy.utils.add_to_date,
				get_datetime=finergy.utils.get_datetime,
...
def apply_workflow(doc, action):
	...
	if not transition:
		finergy.throw(_("Not a valid Workflow Action"), WorkflowTransitionError)

	if not has_approval_access(user, doc, transition):
		finergy.throw(_("Self approval is not allowed"))
	...
	if doc.docstatus == 0 and new_docstatus == 0:
		...
		finergy.throw(_("Illegal Document Status for {0}").format(next_state.state))
...
def validate_workflow(doc):
	...
	if current_state != next_state:
		bold_current = finergy.bold(current_state)
		bold_next = finergy.bold(next_state)
...
def get_workflow(doctype):
	return finergy.get_doc("Workflow", get_workflow_name(doctype))
...
def get_workflow_field_value(workflow_name, field):
	value = finergy.cache().hget("workflow_" + workflow_name, field)
	if value is None:
		value = finergy.db.get_value("Workflow", workflow_name, field)
		finergy.cache().hset("workflow_" + workflow_name, field, value)
	return value
...
def bulk_workflow_approval(docnames, doctype, action):
	...
	print("Clearing finergy.message_log...")
	...
	for (idx, docname) in enumerate(docnames, 1):
		...
		try:
			...
			apply_workflow(finergy.get_doc(doctype, docname), action)
			finergy.db.commit()
		except Exception as e:
			if not finergy.message_log:
			...
			if not message_dict:
				if finergy.message_log:
					messages = finergy.get_message_log()
...
def print_workflow_log(messages, title, doctype, indicator):
	if messages.keys():
		...
		for doc in messages.keys():
			if len(messages[doc]):
				html = "<details><summary>{0}</summary>".format(finergy.utils.get_link_to_form(doctype, doc))
		...
		finergy.msgprint(msg, title=_("Workflow Status"), indicator=indicator, is_minimizable=True)
	...
	try:
		for (i, doc) in enumerate(docs, 1):
			...
			actions = [
				...
				if has_approval_access(finergy.session.user, doc, t)
			]
			if not actions:
				return []
...
def set_workflow_state_on_action(doc, workflow_name, action):
	workflow = finergy.get_doc("Workflow", workflow_name)
