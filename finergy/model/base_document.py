...
from finergy.model import default_fields, display_fieldtypes, table_fields
from finergy.model.naming import set_new_name
from finergy.model.utils.link_count import notify_link_count
...
from finergy.utils.html_utils import unescape_html
...
def get_controller(doctype):
	...
	For `custom` type, returns `finergy.model.document.Document`.
	...
	def _get_controller():
		...
		module_name, custom = finergy.db.get_value(
			"DocType", doctype, ("module", "custom"), cache=True
		) or ["Core", False]

		if custom:
			if finergy.db.field_exists("DocType", "is_tree"):
				is_tree = finergy.db.get_value("DocType", doctype, "is_tree", cache=True)
			else:
				is_tree = False
			_class = NestedSet if is_tree else Document
		else:
			class_overrides = finergy.get_hooks("override_doctype_class")
			if class_overrides and class_overrides.get(doctype):
				import_path = class_overrides[doctype][-1]
				module_path, classname = import_path.rsplit(".", 1)
				module = finergy.get_module(module_path)
	...
	if finergy.local.dev_server:
		return _get_controller()

	site_controllers = finergy.controllers.setdefault(finergy.local.site, {})
...
class BaseDocument(object):
	...
	def meta(self):
		if not hasattr(self, "_meta"):
			self._meta = finergy.get_meta(self.doctype)
	...
	def get_db_value(self, key):
		return finergy.db.get_value(self.doctype, self.name, key)
		...
		for fieldname in self.meta.get_valid_columns():
			...
			if df:
				...
				if isinstance(d[fieldname], list) and df.fieldtype not in table_fields:
					finergy.throw(_("Value for {0} cannot be a list").format(_(df.label)))
	...
	def get_valid_columns(self):
		if self.doctype not in finergy.local.valid_columns:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
				from finergy.model.meta import get_table_columns

				valid = get_table_columns(self.doctype)
			else:
				valid = self.meta.get_valid_columns()

			finergy.local.valid_columns[self.doctype] = valid

		return finergy.local.valid_columns[self.doctype]
	...
	def as_json(self):
		return finergy.as_json(self.as_dict())
	...
	def db_insert(self):
		...
		if not self.creation:
			...
			self.created_by = self.modified_by = finergy.session.user
		...
		try:
			...
			if finergy.db.is_primary_key_violation(e):
				if self.meta.autoname == "hash":
					# hash collision? try again
					finergy.flags.retry_count = (finergy.flags.retry_count or 0) + 1
					if finergy.flags.retry_count > 5 and not finergy.flags.in_test:
						raise
					self.name = None
					self.db_insert()
					return
				...
				finergy.msgprint(
					_("{0} {1} already exists").format(self.doctype, finergy.bold(self.name)),
					title=_("Duplicate Name"),
					indicator="red",
				)
				raise finergy.DuplicateEntryError(self.doctype, self.name, e)

			elif finergy.db.is_unique_key_violation(e):
				# unique constraint
	...
	def db_update(self):
		...
		try:
			...
			if finergy.db.is_unique_key_violation(e):
				self.show_unique_validation_message(e)
			else:
				raise
	...
	def show_unique_validation_message(self, e):
		if finergy.db.db_type != "postgres":
			...
			finergy.msgprint(_("{0} must be unique").format(label or fieldname))

		# this is used to preserve traceback
		raise finergy.UniqueValidationError(self.doctype, self.name, e)
	...
	def update_modified(self):
		...
		finergy.db.set_value(self.doctype, self.name, "modified", self.modified, update_modified=False)
		...
		if self.meta.istable:
			for fieldname in ("parent", "parenttype"):
				if not self.get(fieldname):
					missing.append((fieldname, get_msg(finergy._dict(label=fieldname))))
		...
		for df in self.meta.get_link_fields() + self.meta.get(
			...
			if docname:
				if df.fieldtype == "Link":
					doctype = df.options
					if not doctype:
						finergy.throw(_("Options not set for link field {0}").format(df.fieldname))
				else:
					doctype = self.get(df.options)
					if not doctype:
						finergy.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
				...
				if not finergy.get_meta(doctype).get("is_virtual"):
					if not fields_to_fetch:
						# cache a single value type
						values = finergy._dict(name=finergy.db.get_value(doctype, docname, "name", cache=True))
					else:
						values_to_fetch = ["name"] + [_df.fetch_from.split(".")[-1] for _df in fields_to_fetch]

						# don't cache if fetching other values too
						values = finergy.db.get_value(doctype, docname, values_to_fetch, as_dict=True)

				if finergy.get_meta(doctype).issingle:
					values.name = doctype

				if finergy.get_meta(doctype).get("is_virtual"):
					values = finergy.get_doc(doctype, docname)
				...
				if values:
					...
					if not values.name:
						...
						and finergy.get_meta(doctype).is_submittable
						and cint(finergy.db.get_value(doctype, docname, "docstatus")) == 2
					):
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ["Small Text", "Text", "Data"]:
			if fetch_from_fieldname in default_fields:
				from finergy.model.meta import get_default_df

				fetch_from_df = get_default_df(fetch_from_fieldname)
			else:
				fetch_from_df = finergy.get_meta(doctype).get_field(fetch_from_fieldname)

			if not fetch_from_df:
				finergy.throw(
					_('Please check the value of "Fetch From" set for field {0}').format(finergy.bold(df.label)),
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (finergy.flags.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		...
		# data_field options defined in finergy.model.data_field_options
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in STANDARD_USERS) and (data in STANDARD_USERS):
					continue
				for email_address in finergy.utils.split_emails(data):
					finergy.utils.validate_email_address(email_address, throw=True)

			if data_field_options == "Name":
				finergy.utils.validate_name(data, throw=True)

			if data_field_options == "Phone":
				finergy.utils.validate_phone_number(data, throw=True)

			if data_field_options == "URL":
				if not data:
					continue

				finergy.utils.validate_url(data, throw=True)

	def _validate_constants(self):
		if finergy.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ("=", 1)})]
		if constants:
			values = finergy.db.get_value(self.doctype, self.name, constants, as_dict=True)
	...
	def _validate_code_fields(self):
		for field in self.meta.get_code_fields():
			...
			if language == "Python":
				finergy.utils.validate_python_code(code_string, fieldname=field.label, is_expression=False)

			elif language == "PythonExpression":
				finergy.utils.validate_python_code(code_string, fieldname=field.label)
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		finergy.throw(
			...
			finergy.CharacterLengthExceededError,
			title=_("Value too big"),
		)

	def _validate_update_after_submit(self):
		# get the full doc with children
		db_values = finergy.get_doc(self.doctype, self.name).as_dict()
			...
			if df and not df.allow_on_submit and (self.get(key) or db_value):
				...
				if self_value != db_value:
					finergy.throw(
						...
						finergy.UpdateAfterSubmitError,
					)
	...
	def _sanitize_content(self):
		...
		for fieldname, value in self.get_valid_dict().items():
			...
			value = finergy.as_unicode(value)
	...
	def _save_passwords(self):
		...
		from finergy.utils.password import remove_encrypted_password, set_encrypted_password
	...
	def precision(self, fieldname, parentfield=None):
		...
		if not hasattr(self, "_precision"):
			self._precision = finergy._dict()

		if cache_key not in self._precision:
			self._precision[cache_key] = finergy._dict()
	...
	def get_formatted(
		...
		if df and not currency:
			...
			if not finergy.db.exists("Currency", currency, cache=True):
				currency = None
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields):
		...
		if to_reset:
			if self.is_new():
				...
				ref_doc = finergy.new_doc(self.doctype)
			else:
	...
	def _extract_images_from_text_editor(self):
		from finergy.core.doctype.file.file import extract_images_from_doc
...
def _filter(data, filters, limit=None):
	...
	for d in data:
		for f, fval in _filters.items():
			if not finergy.compare(getattr(d, f, None), fval[0], fval[1]):
				break
		else:
			out.append(d)
