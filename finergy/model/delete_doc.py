...
from finergy import _, get_module_path
from finergy.desk.doctype.tag.tag import delete_tags_for_document
from finergy.exceptions import FileNotFoundError
from finergy.model.dynamic_links import get_dynamic_link_map
from finergy.model.naming import revert_series_if_last
from finergy.utils.file_manager import remove_all
from finergy.utils.global_search import delete_for_document
from finergy.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	# get from form
	if not doctype:
		doctype = finergy.form_dict.get("dt")
		name = finergy.form_dict.get("dn")
	...
	for name in names or []:
		...
		if doctype == "DocType":
			if for_reload:
				...
				finergy.db.sql("delete from `tabCustom Field` where dt = %s", name)
				finergy.db.sql("delete from `tabClient Script` where dt = %s", name)
				finergy.db.sql("delete from `tabProperty Setter` where doc_type = %s", name)
				finergy.db.sql("delete from `tabReport` where ref_doctype=%s", name)
				finergy.db.sql("delete from `tabCustom DocPerm` where parent=%s", name)
				finergy.db.sql("delete from `__global_search` where doctype=%s", name)

			delete_from_table(doctype, name, ignore_doctypes, None)

			if (
				finergy.conf.developer_mode
				and not doc.custom
				and not (
					for_reload or finergy.flags.in_migrate or finergy.flags.in_install or finergy.flags.in_uninstall
				)
			):
				try:
			...
			if not for_reload:
				...
				finergy.enqueue(
					"finergy.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
		...
		if doc and not for_reload:
			...
			finergy.defaults.clear_default(parenttype="User Permission", key=doctype, value=name)


def add_to_deleted_document(doc):
	"""Add this document to Deleted Document table. Called after delete"""
	if doc.doctype != "Deleted Document" and finergy.flags.in_install != "finergy":
		finergy.get_doc(
			dict(
...
def delete_from_table(doctype, name, ignore_doctypes, doc):
	if doctype != "DocType" and doctype == name:
		finergy.db.sql("delete from `tabSingles` where `doctype`=%s", name)
	else:
		finergy.db.sql("delete from `tab{0}` where `name`=%s".format(doctype), name)
	...
	if doc:
		...
		def get_table_fields(field_doctype):
			...
			return [
				r[0]
				for r in finergy.get_all(
					field_doctype,
					fields="options",
					filters={"fieldtype": ["in", finergy.model.table_fields], "parent": doctype},
					as_list=1,
				)
			]

		tables = get_table_fields("DocField")
		if not finergy.flags.in_install == "finergy":
...
def check_permission_and_not_submitted(doc):
	...
	if (
		...
		and finergy.session.user != "Administrator"
		...
		finergy.msgprint(
			...
			raise_exception=finergy.PermissionError,
		)
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from finergy.model.rename_doc import get_link_fields
	...
	for link_dt, link_field, issingle in link_fields:
		if not issingle:
			for item in finergy.db.get_values(
			...
			if finergy.db.get_value(link_dt, None, link_field) == doc.name:
				raise_link_exists_exception(doc, link_dt, link_dt)


def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	"""Raise `finergy.LinkExistsError` if the document is dynamically linked"""
	for df in get_dynamic_link_map().get(doc.doctype, []):
		...
		meta = finergy.get_meta(df.parent)
		if meta.issingle:
...
def insert_feed(doc):
	if (
		...
		or finergy.flags.in_uninstall
		or finergy.flags.in_import
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", finergy.scrub(doctype))
