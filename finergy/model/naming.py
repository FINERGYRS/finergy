...
from finergy.utils import cint, cstr, now_datetime
...
def set_new_name(doc):
	...
	autoname = finergy.get_meta(doc.doctype).autoname or ""

	if autoname.lower() != "prompt" and not finergy.flags.in_import:
		doc.name = None
	...
	if not doc.name and autoname.startswith("field:"):
		...
		finergy.throw(_("{0} is required").format(doc.meta.get_label(fieldname)))
	...
	doc.name = validate_name(
		doc.doctype, doc.name, finergy.get_meta(doc.doctype).get_field("name_case")
	)
...
def set_naming_from_document_naming_rule(doc):
	...
	for d in finergy.get_all(
		...
		finergy.get_cached_doc("Document Naming Rule", d.name).apply(doc)
		if doc.name:
			break
...
def set_name_by_naming_series(doc):
	...
	if not doc.naming_series:
		finergy.throw(finergy._("Naming Series mandatory"))
...
def make_autoname(key="", doctype="", doc=""):
	...
	*Example:*
	...
	"""
	if key == "hash":
		return finergy.generate_hash(doctype, 10)
...
def parse_naming_series(parts, doctype="", doc=""):
	...
	for e in parts:
		...
		if e.startswith("#"):
			...
			part = finergy.defaults.get_user_default("fiscal_year")
...
def getseries(key, digits):
	# series created ?
	current = finergy.db.sql("SELECT `current` FROM `tabSeries` WHERE `name`=%s FOR UPDATE", (key,))
	if current and current[0][0] is not None:
		current = current[0][0]
		# yes, update it
		finergy.db.sql("UPDATE `tabSeries` SET `current` = `current` + 1 WHERE `name`=%s", (key,))
		current = cint(current) + 1
	else:
		# no, create it
		finergy.db.sql("INSERT INTO `tabSeries` (`name`, `current`) VALUES (%s, 1)", (key,))
		current = 1
...
def revert_series_if_last(key, name, doc=None):
	...
	current = finergy.db.sql("SELECT `current` FROM `tabSeries` WHERE `name`=%s FOR UPDATE", (prefix,))

	if current and current[0][0] == count:
		finergy.db.sql("UPDATE `tabSeries` SET `current` = `current` - 1 WHERE `name`=%s", prefix)


def get_default_naming_series(doctype):
	"""get default value for `naming_series` property"""
	naming_series = finergy.get_meta(doctype).get_field("naming_series").options or ""
	if naming_series:
...
def validate_name(doctype, name, case=None, merge=False):
	if not name:
		finergy.throw(_("No Name Specified for {0}").format(doctype))
	if name.startswith("New " + doctype):
		finergy.throw(
			_("There were some errors setting the name, please contact the administrator"), finergy.NameError
		)
	if case == "Title Case":
		name = name.title()
	if case == "UPPER CASE":
	...
	if not finergy.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
		finergy.throw(_("Name of {0} cannot be {1}").format(doctype, name), finergy.NameError)

	special_characters = "<>"
	if re.findall("[{0}]+".format(special_characters), name):
		message = ", ".join("'{0}'".format(c) for c in special_characters)
		finergy.throw(
			_("Name cannot contain special characters like {0}").format(message), finergy.NameError
		)
...
def append_number_if_name_exists(doctype, value, fieldname="name", separator="-", filters=None):
	...
	exists = finergy.db.exists(doctype, filters)
	...
	if exists:
		last = finergy.db.sql(
			...
			`{fieldname}` DESC LIMIT 1""".format(
				doctype=doctype, fieldname=fieldname, regex_character=finergy.db.REGEX_CHARACTER
			),
			regex,
		)
...
def _set_amended_name(doc):
	...
	if finergy.db.get_value(doc.doctype, doc.amended_from, "amended_from"):
...
def _prompt_autoname(autoname, doc):
	...
	if not doc.name:
		finergy.throw(_("Name not set via prompt"))
