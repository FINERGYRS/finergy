...
def get_dynamic_link_map(for_delete=False):
	...
	"""
	if getattr(finergy.local, "dynamic_link_map", None) == None or finergy.flags.in_test:
		# Build from scratch
		dynamic_link_map = {}
		for df in get_dynamic_links():
			...
			if meta.issingle:
				...
				try:
					links = finergy.db.sql_list("""select distinct {options} from `tab{parent}`""".format(**df))
					for doctype in links:
						dynamic_link_map.setdefault(doctype, []).append(df)
				except finergy.db.TableMissingError:  # noqa: E722
					pass

		finergy.local.dynamic_link_map = dynamic_link_map
	return finergy.local.dynamic_link_map
...
def get_dynamic_links():
	...
	for query in dynamic_link_queries:
		df += finergy.db.sql(query, as_dict=True)
	return df
