...
from finergy.model import optional_fields
from finergy.model.meta import get_table_columns
from finergy.model.utils.user_settings import get_user_settings, update_user_settings
...
class DatabaseQuery(object):
	def __init__(self, doctype, user=None):
		...
		self.or_conditions = []
		self.fields = None
		self.user = user or finergy.session.user
	...
	def execute(
		...
		if (
			...
			and not finergy.has_permission(self.doctype, "select", user=user)
			and not finergy.has_permission(self.doctype, "read", user=user)
		):

			finergy.flags.error_message = _("Insufficient Permission for {0}").format(
				finergy.bold(self.doctype)
			)
			raise finergy.PermissionError(self.doctype)
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			finergy.throw(_("Use of sub-query or function is restricted"), finergy.DataError)
		...
		for field in self.fields:
			...
			if self.strict:
				if STRICT_FIELD_PATTERN.match(field):
					finergy.throw(_("Illegal SQL Query"))
	...
	def append_table(self, table_name):
		...
		ptype = "select" if finergy.only_has_select_perm(doctype) else "read"

		if (not self.flags.ignore_permissions) and (not finergy.has_permission(doctype, ptype=ptype)):
			finergy.flags.error_message = _("Insufficient Permission for {0}").format(finergy.bold(doctype))
			raise finergy.PermissionError(doctype)
	...
	def get_table_columns(self):
		...
		except finergy.db.TableMissingError:
			if self.ignore_ddl:
				return None
			else:
				raise
	...
	def prepare_filter_condition(self, f):
		...
		from finergy.boot import get_additional_filters_from_hooks
		...
		if f.operator.lower() in (
			...
			if finergy.get_meta(f.doctype).get_field(f.fieldname) is not None:
				ref_doctype = finergy.get_meta(f.doctype).get_field(f.fieldname).options

			result = []

			lft, rgt = "", ""
			if f.value:
				lft, rgt = finergy.db.get_value(ref_doctype, f.value, ["lft", "rgt"])

			# Get descendants elements of a DocType with a tree structure
			if f.operator.lower() in ("descendants of", "not descendants of"):
				result = finergy.get_all(
			...
			fallback = "''"
			value = [finergy.db.escape((v.name or "").strip(), percent=False) for v in result]
			if len(value):
			...
			values = f.value or ""
			if isinstance(values, finergy.string_types):
				values = values.split(",")

			fallback = "''"
			value = [finergy.db.escape((v or "").strip(), percent=False) for v in values]
			if len(value):
				value = "({0})".format(", ".join(value))
			else:
				value = "('')"
		else:
			df = finergy.get_meta(f.doctype).get("fields", {"fieldname": f.fieldname})
			...
			if f.operator in (">", "<") and (f.fieldname in ("creation", "modified")):
				...
				value = finergy.db.format_date(f.value)
				fallback = "'0001-01-01'"

			elif (df and df.fieldtype == "Datetime") or isinstance(f.value, datetime):
				value = finergy.db.format_datetime(f.value)
				fallback = "'0001-01-01 00:00:00'"
			...
			if isinstance(value, string_types) and not f.operator.lower() == "between":
				value = "{0}".format(finergy.db.escape(value, percent=False))
		...
		if (
			...
			if f.operator.lower() == "like" and finergy.conf.get("db_type") == "postgres":
				f.operator = "ilike"
	...
	def build_match_conditions(self, as_condition=True):
		...
		if not self.user:
			self.user = finergy.session.user

		if not self.tables:
			self.extract_tables()

		meta = finergy.get_meta(self.doctype)
		role_permissions = finergy.permissions.get_role_permissions(meta, user=self.user)
		self.shared = finergy.share.get_shared(self.doctype, self.user)
		...
		if (
			...
			only_if_shared = True
			if not self.shared:
				finergy.throw(_("No permission to read {0}").format(_(self.doctype)), finergy.PermissionError)
			else:
			...
			if requires_owner_constraint(role_permissions):
				self.match_conditions.append(
					f"`tab{self.doctype}`.`owner` = {finergy.db.escape(self.user, percent=False)}"
				)

			# add user permission only if role has read perm
			elif role_permissions.get("read") or role_permissions.get("select"):
				# get user permissions
				user_permissions = finergy.permissions.get_user_permissions(self.user)
	...
	def get_share_condition(self):
		...
		) % tuple([finergy.db.escape(s, percent=False) for s in self.shared])
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if finergy.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					condition += "`tab{doctype}`.`{fieldname}` in ({values})".format(
						...
						values=", ".join([(finergy.db.escape(doc, percent=False)) for doc in docs]),
					)
	...
	def get_permission_query_conditions(self):
		conditions = []
		condition_methods = finergy.get_hooks("permission_query_conditions", {}).get(self.doctype, [])
		if condition_methods:
			for method in condition_methods:
				c = finergy.call(finergy.get_attr(method), self.user)
				if c:
					conditions.append(c)

		permision_script_name = get_server_script_map().get("permission_query", {}).get(self.doctype)
		if permision_script_name:
			script = finergy.get_doc("Server Script", permision_script_name)
	...
	def validate_order_by_and_group_by(self, parameters):
		...
		if "select" in _lower and "from" in _lower:
			finergy.throw(_("Cannot use sub-query in order by"))
		...
		for field in parameters.split(","):
			if "." in field and field.strip().startswith("`tab"):
				...
				if tbl not in self.tables:
					if tbl.startswith("`"):
						tbl = tbl[4:-1]
					finergy.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not finergy.db.exists(
			"DocField", {"parent": parent, "options": child_doctype}
		):
			raise finergy.PermissionError

		if finergy.permissions.has_permission(parent):
			return
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = finergy.permissions.get_user_permissions(user=user)
...
def get_between_date_filter(value, df=None):
	...
	"""
	from_date = finergy.utils.nowdate()
	to_date = finergy.utils.nowdate()
	...
	if df and df.fieldtype == "Datetime":
		data = "'%s' AND '%s'" % (
			finergy.db.format_datetime(from_date),
			finergy.db.format_datetime(to_date),
		)
	else:
		data = "'%s' AND '%s'" % (finergy.db.format_date(from_date), finergy.db.format_date(to_date))
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = finergy._dict(finergy.get_attr(additional_filter["get_field"])())
	if f.query_value:
		for option in f.options:
			option = finergy._dict(option)
