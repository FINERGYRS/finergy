...
from finergy.utils.password import rename_password_field
...
def rename_field(doctype, old_fieldname, new_fieldname):
	...
	if not meta.issingle and not finergy.db.has_column(doctype, old_fieldname):
	...
	if new_field.fieldtype in table_fields:
		...
		if meta.issingle:
			...
			# copy field value
			finergy.db.sql("""update `tab%s` set `%s`=`%s`""" % (doctype, new_fieldname, old_fieldname))
...
def update_reports(doctype, old_fieldname, new_fieldname):
	...
	for r in reports:
		...
		if report_dict.get("updated"):
			...
			finergy.db.sql("""update `tabReport` set `json`=%s where name=%s""", (new_val, r.name))


def update_users_report_view_settings(doctype, ref_fieldname, new_fieldname):
	user_report_cols = finergy.db.sql(
