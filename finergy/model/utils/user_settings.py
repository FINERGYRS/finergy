...
def get_user_settings(doctype, for_update=False):
	user_settings = finergy.cache().hget(
		"_user_settings", "{0}::{1}".format(doctype, finergy.session.user)
	)
...
def update_user_settings(doctype, user_settings, for_update=False):
	...
	finergy.cache().hset(
		"_user_settings", "{0}::{1}".format(doctype, finergy.session.user), json.dumps(current)
	)


def sync_user_settings():
	"""Sync from cache to database (called asynchronously via the browser)"""
	for key, data in iteritems(finergy.cache().hgetall("_user_settings")):
		key = safe_decode(key)
