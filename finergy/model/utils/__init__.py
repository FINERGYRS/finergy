...
def set_default(doc, key):
	...
	if not doc.is_default:
		finergy.db.set(doc, "is_default", 1)
...
def set_field_property(filters, key, value):
	...
	docs = [
		finergy.get_doc("DocType", d.parent)
		for d in finergy.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(finergy.ValidationError):
	pass
...
def render_include(content):
	...
	for i in range(5):
		if "{% include" in content:
			...
			if not paths:
				finergy.throw(_("Invalid include path"), InvalidIncludePath)

			for path in paths:
				app, app_path = path.split("/", 1)
				with io.open(finergy.get_app_path(app, app_path), "r", encoding="utf-8") as f:
					include = f.read()
...
def get_fetch_values(doctype, fieldname, value):
	...
	db_values = finergy.db.get_value(
