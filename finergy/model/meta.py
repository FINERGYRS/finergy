...
from finergy.model.base_document import BaseDocument
from finergy.model.document import Document
from finergy.model.workflow import get_workflow_name
from finergy.modules import load_doctype_module
from finergy.utils import cast, cint, cstr


def get_meta(doctype, cached=True):
	if cached:
		if not finergy.local.meta_cache.get(doctype):
			meta = finergy.cache().hget("meta", doctype)
			if meta:
				meta = Meta(meta)
			else:
				meta = Meta(doctype)
				finergy.cache().hset("meta", doctype, meta.as_dict())
			finergy.local.meta_cache[doctype] = meta

		return finergy.local.meta_cache[doctype]
	else:
...
def get_table_columns(doctype):
	return finergy.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = finergy.scrub(doctype)
	with open(finergy.get_app_path("finergy", "core", "doctype", fname, fname + ".json"), "r") as f:
...
class Meta(Document):
	...
	def as_dict(self, no_nulls=False):
		def serialize(doc):
			...
			for key in doc.__dict__:
				...
				if isinstance(value, (finergy.text_type, int, float, datetime, list, tuple)) or (
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(finergy._dict(fieldtype="Data", fieldname="name", label="Name"))

		return fields

	def get_valid_columns(self):
		if not hasattr(self, "_valid_columns"):
			table_exists = finergy.db.table_exists(self.name)
	...
	def add_custom_fields(self):
		if not finergy.db.table_exists("Custom Field"):
			return

		custom_fields = finergy.db.sql(
			"""
	...
	def apply_property_setters(self):
		...
		"""
		if not finergy.db.table_exists("Property Setter"):
			return

		property_setters = finergy.db.sql(
	...
	def set_custom_permissions(self):
		...
		if finergy.flags.in_patch or finergy.flags.in_install:
			return

		if not self.istable and self.name not in ("DocType", "DocField", "DocPerm", "Custom DocPerm"):
			custom_perms = finergy.get_all(
				"Custom DocPerm",
				fields="*",
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(finergy._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = finergy.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other Finergy applications via hooks.
		"""
		data = finergy._dict()
		if not self.custom:
			try:
				...
				if hasattr(module, "get_data"):
					data = finergy._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in finergy.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = finergy._dict(finergy.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = finergy.scrub(self.module)
		doctype = finergy.scrub(self.name)
		template_path = finergy.get_module_path(
...
DOCTYPE_TABLE_FIELDS = [
	finergy._dict({"fieldname": "fields", "options": "DocField"}),
	finergy._dict({"fieldname": "permissions", "options": "DocPerm"}),
	finergy._dict({"fieldname": "actions", "options": "DocType Action"}),
	finergy._dict({"fieldname": "links", "options": "DocType Link"}),
]
...
def is_single(doctype):
	try:
		return finergy.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception("Cannot determine whether %s is single" % doctype)


def get_parent_dt(dt):
	parent_dt = finergy.db.get_all(
		"DocField", "parent", dict(fieldtype=["in", finergy.model.table_fields], options=dt), limit=1
	)
	return parent_dt and parent_dt[0].parent or ""


def set_fieldname(field_id, fieldname):
	finergy.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(finergy.local, "field_currency", None):
		finergy.local.field_currency = finergy._dict()

	if not (
		finergy.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.parent and finergy.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = finergy.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.parent:
				if currency:
					...
					if finergy.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = finergy.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			finergy.local.field_currency.setdefault((doc.doctype, ref_docname), finergy._dict()).setdefault(
				df.fieldname, currency
			)

	return finergy.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.parent and finergy.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)
...
def get_field_precision(df, doc=None, currency=None):
	...
	from finergy.utils import get_number_format_info

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(finergy.db.get_default("currency_precision"))
		if not precision:
			number_format = finergy.db.get_default("number_format") or "#,###.##"
			decimal_str, comma_str, precision = get_number_format_info(number_format)
	else:
		precision = cint(finergy.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in default_fields:
		if fieldname in ("creation", "modified"):
			return finergy._dict(fieldname=fieldname, fieldtype="Datetime")

		else:
			return finergy._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None):
	...
	for doctype in finergy.db.get_all("DocType", filters=filters):
		doctype = doctype.name
		columns = finergy.db.get_table_columns(doctype)
		fields = finergy.get_meta(doctype).get_fieldnames_with_value()
		columns_to_remove = [
