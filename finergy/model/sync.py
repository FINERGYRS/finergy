...
from finergy.modules.patch_handler import block_user
...
def sync_for(app_name, force=0, sync_everything=False, verbose=False, reset_permissions=False):
	...
	if app_name == "finergy":
		...
		for d in (
			...
			files.append(os.path.join(finergy.get_app_path("finergy"), d[0], "doctype", d[1], d[1] + ".json"))

	for module_name in finergy.local.app_modules.get(app_name) or []:
		folder = os.path.dirname(finergy.get_module(app_name + "." + module_name).__file__)
