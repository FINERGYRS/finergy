...
from finergy.model import default_fields, table_fields
...
def make_mapped_doc(method, source_name, selected_children=None, args=None):
	...
	for hook in finergy.get_hooks("override_whitelisted_methods", {}).get(method, []):
	...
	if args:
		finergy.flags.args = finergy._dict(json.loads(args))

	finergy.flags.selected_children = selected_children or None
...
def get_mapped_doc(
	...
	# main
	if not target_doc:
		target_doc = finergy.new_doc(table_maps[from_doctype]["doctype"])
	elif isinstance(target_doc, string_types):
		target_doc = finergy.get_doc(json.loads(target_doc))
	...
	source_doc = finergy.get_doc(from_doctype, from_docname)
		...
		for df in source_doc.meta.get_table_fields():
			...
			if table_map:
				for source_d in source_doc.get(df.fieldname):
					...
					if (
						finergy.flags.selected_children
						and (df.fieldname in finergy.flags.selected_children)
						and source_d.name not in finergy.flags.selected_children[df.fieldname]
					):
						continue
...
def map_fetch_fields(target_doc, df, no_copy_fields):
	...
	for fetch_df in target_doc.meta.get("fields", {"fetch_from": "^{0}.".format(df.fieldname)}):
		...
		if (
			...
			if not linked_doc:
				try:
					linked_doc = finergy.get_doc(df.options, target_doc.get(df.fieldname))
				except Exception:
					return
...
def map_child_doc(source_d, target_parent, table_map, source_parent=None):
	...
	target_d = finergy.new_doc(target_child_doctype, target_parent, target_parentfield)
