...
from finergy.permissions import filter_allowed_docs_for_doctype
from finergy.utils import cstr, now_datetime, nowdate, nowtime


def get_new_doc(doctype, parent_doc=None, parentfield=None, as_dict=False):
	if doctype not in finergy.local.new_doc_templates:
		# cache a copy of new doc as it is called
		# frequently for inserts
		finergy.local.new_doc_templates[doctype] = make_new_doc(doctype)

	doc = copy.deepcopy(finergy.local.new_doc_templates[doctype])
	...
	if as_dict:
		...
		return finergy.get_doc(doc)


def make_new_doc(doctype):
	doc = finergy.get_doc(
		{"doctype": doctype, "__islocal": 1, "owner": finergy.session.user, "docstatus": 0}
	)
...
def set_user_and_static_default_values(doc):
	...
	defaults = finergy.defaults.get_defaults()
	...
	for df in doc.meta.get("fields"):
		if df.fieldtype in data_fieldtypes:
			...
			if user_default_value is not None:
				...
				if not df.fieldtype == "Link" or finergy.db.exists(df.options, user_default_value):
...
def set_dynamic_default_values(doc, parent_doc, parentfield):
	...
	for df in finergy.get_meta(doc["doctype"]).get("fields"):
		if df.get("default"):
...
def get_default_based_on_another_field(df, user_permissions, parent_doc):
	...
	from finergy.permissions import get_allowed_docs_for_doctype

	ref_doctype = df.default[1:]
	ref_fieldname = ref_doctype.lower().replace(" ", "_")
	reference_name = (
		parent_doc.get(ref_fieldname) if parent_doc else finergy.db.get_default(ref_fieldname)
	)
	default_value = finergy.db.get_value(ref_doctype, reference_name, df.fieldname)
