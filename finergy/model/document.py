...
import finergy
from finergy import _, is_whitelisted, msgprint
from finergy.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from finergy.desk.form.document_follow import follow_document
from finergy.integrations.doctype.webhook import run_webhooks
from finergy.model import optional_fields, table_fields
from finergy.model.base_document import BaseDocument, get_controller
from finergy.model.naming import set_new_name, validate_name
from finergy.model.workflow import set_workflow_state_on_action, validate_workflow
from finergy.utils import cstr, date_diff, file_lock, flt, get_datetime_str, now
from finergy.utils.data import get_absolute_url
from finergy.utils.global_search import update_global_search
...
def get_doc(*args, **kwargs):
	"""returns a finergy.model.Document object.
...
class Document(BaseDocument):
		...
		if args and args[0] and isinstance(args[0], string_types):
			...
			if len(args) == 1:
				...
				if isinstance(args[1], dict):
					# filter
					self.name = finergy.db.get_value(args[0], args[1], "name")
					if self.name is None:
						finergy.throw(_("{0} {1} not found").format(_(args[0]), args[1]), finergy.DoesNotExistError)
				else:
					self.name = args[1]
	...
	def load_from_db(self):
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = finergy.db.get_singles_dict(self.doctype)
			if not single_doc:
				single_doc = finergy.new_doc(self.doctype).as_dict()
			...
			d = finergy.db.get_value(
				self.doctype, self.name, "*", as_dict=1, for_update=self.flags.for_update
			)
			if not d:
				finergy.throw(
					_("{0} {1} not found").format(_(self.doctype), self.name), finergy.DoesNotExistError
				)

			super(Document, self).__init__(d)

		if self.name == "DocType" and self.doctype == "DocType":
			from finergy.model.meta import DOCTYPE_TABLE_FIELDS
		...
		for df in table_fields:
			children = finergy.db.get_values(
				df.options,
	...
	def get_latest(self):
		if not getattr(self, "latest", None):
			self.latest = finergy.get_doc(self.doctype, self.name)
		return self.latest

	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `finergy.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self.raise_no_permission_to(permlevel or permtype)

	def has_permission(self, permtype="read", verbose=False):
		"""Call `finergy.has_permission` if `self.flags.ignore_permissions`
		is not set.
		...
		if self.flags.ignore_permissions:
			return True
		return finergy.permissions.has_permission(self.doctype, permtype, self, verbose=verbose)

	def raise_no_permission_to(self, perm_type):
		"""Raise `finergy.PermissionError`."""
		finergy.flags.error_message = _("Insufficient Permission for {0}").format(self.doctype)
		raise finergy.PermissionError
	...
	def insert(
		...
		if not (
			finergy.flags.in_migrate or finergy.local.flags.in_install or finergy.flags.in_setup_wizard
		):
			follow_document(self.doctype, self.name, finergy.session.user)
		return self
	...
	def _save(self, ignore_permissions=None, ignore_version=None):
		...
		self.flags.ignore_version = finergy.flags.in_test if ignore_version is None else ignore_version
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `finergy.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		finergy.db.sql("""delete from `tabSingles` where doctype=%s""", self.doctype)
		...
		if self.doctype in finergy.db.value_cache:
			del finergy.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = finergy.session.user
		...
		finergy.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.parentfield:
				return "{} {} #{}: {} {}".format(
					finergy.bold(_(self.doctype)),
					_("Row"),
					self.idx,
					_("Value cannot be negative for"),
					finergy.bold(_(df.label)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), finergy.bold(_(df.label))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				finergy.throw(msg, finergy.NonNegativeError, title=_("Negative Value"))
			...
			for field in set_only_once_fields:
				...
				if fail:
					finergy.throw(
						...
						finergy.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += finergy.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in finergy.get_meta(table_field.options).fields or []:
				if df.permlevel and not df.permlevel in has_access_to:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or finergy.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = finergy.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
				for d in self.get(df.fieldname):
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = finergy.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		new_doc = finergy.new_doc(self.doctype, as_dict=True)
		self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = finergy.new_doc(df.options, as_dict=True)
	...
	def check_if_latest(self):
		...
		if not self.get("__islocal") and not self.meta.get("is_virtual"):
			if self.meta.issingle:
				...
				if not tmp:
					finergy.throw(_("Record does not exist"))
				else:
					tmp = tmp[0]
			...
			if conflict:
				finergy.msgprint(
					...
					raise_exception=finergy.TimestampMismatchError,
				)
		else:
	...
	def check_docstatus_transition(self, docstatus):
		...
		if docstatus == 0:
			if self.docstatus == 0:
				...
				raise finergy.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise finergy.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise finergy.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if finergy.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise finergy.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			finergy.throw(_("Could not find {0}").format(msg), finergy.LinkValidationError)

		if cancelled_links:
			msg = ", ".join((each[2] for each in cancelled_links))
			finergy.throw(_("Cannot link cancelled document: {0}").format(msg), finergy.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			(finergy.flags.in_import and finergy.flags.mute_emails)
		...
		from finergy.email.doctype.notification.notification import evaluate_alert

		if self.flags.notifications == None:
			alerts = finergy.cache().hget("notifications", self.doctype)
			if alerts == None:
				alerts = finergy.get_all(
					"Notification",
					fields=["name", "event", "method"],
					filters={"enabled": 1, "document_type": self.doctype},
				)
				finergy.cache().hset("notifications", self.doctype, alerts)
	...
	def load_doc_before_save(self):
		...
		if not self.is_new():
			try:
				self._doc_before_save = finergy.get_doc(self.doctype, self.name)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in finergy.flags.currently_saving:
			finergy.flags.currently_saving.remove((self.doctype, self.name))
	...
	def reset_seen(self):
		...
		if getattr(self.meta, "track_seen", False):
			finergy.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([finergy.session.user]), update_modified=False
			)
	...
	def notify_update(self):
		...
		if (
			...
			data = {"doctype": self.doctype, "name": self.name, "user": finergy.session.user}
			finergy.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run finergy.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in finergy.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", finergy.session.user)
	...
	def db_get(self, fieldname):
		...
		return finergy.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from finergy.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def save_version(self):
		...
		if (
			...
			or finergy.flags.in_install
			or (not self._doc_before_save and finergy.flags.in_patch)
		):
			return

		version = finergy.new_doc("Version")

		if version.update_version_info(self._doc_before_save, self):
			version.insert(ignore_permissions=True)

			if not finergy.flags.in_migrate:
	...
	def validate_value(self, fieldname, condition, val2, doc=None, raise_exception=None):
		...
		if not finergy.compare(val1, condition, val2):
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			finergy.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or finergy.EmptyTableError
			)
		...
		out = finergy.get_doc(
			{
				...
				"comment_email": comment_email or finergy.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen:
			...
			_seen = finergy.parse_json(_seen)

			if user not in _seen:
				_seen.append(user)
				finergy.db.set_value(self.doctype, self.name, "_seen", json.dumps(_seen), update_modified=False)
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		existing_key = finergy.db.exists(
			"Document Share Key",
			{
				"reference_doctype": self.doctype,
		...
		if existing_key:
			doc = finergy.get_doc("Document Share Key", existing_key)
		else:
			doc = finergy.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", finergy._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", finergy._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		self.lock()
		enqueue(
			"finergy.model.document.execute_action",
			doctype=self.doctype,
			name=self.name,
			action=action,
			**kwargs,
			...
			if timeout:
				for i in range(timeout):
					...
					if not file_lock.lock_exists(signature):
						...
						break
			if lock_exists:
				raise finergy.DocumentLockedError
		...
		if date_diff(self.get(to_date_field), self.get(from_date_field)) < 0:
			finergy.throw(
				_("{0} must be after {1}").format(
					finergy.bold(self.meta.get_label(to_date_field)),
					finergy.bold(self.meta.get_label(from_date_field)),
				),
				finergy.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assignments = finergy.get_all(
			"ToDo",
			fields=["owner"],
			filters={
	...
	def add_tag(self, tag):
		...
		from finergy.desk.doctype.tag.tag import DocTags
	...
	def _rename_doc_on_cancel(self):
		if finergy.get_system_settings(
			"use_original_name_for_amended_document", ignore_if_not_exists=True
		):
			new_name = gen_new_name_for_cancelled_doc(self)
			finergy.rename_doc(self.doctype, self.name, new_name, force=True, show_alert=False)
			self.name = new_name
...
def execute_action(doctype, name, action, **kwargs):
	...
	try:
		...
		if finergy.local.message_log:
			msg = json.loads(finergy.local.message_log[-1]).get("message")
		else:
			msg = "<pre><code>" + finergy.get_traceback() + "</pre></code>"
