...
from finergy.model.dynamic_links import get_dynamic_link_map
from finergy.model.naming import validate_name
from finergy.model.utils.user_settings import sync_user_settings, update_user_settings_data
from finergy.utils import cint
from finergy.utils.password import rename_password
...
def update_document_title(
	...
	for key, val in [("docname", docname), ("new_title", new_title), ("new_name", new_name)]:
		if not isinstance(val, (str, type(None))):
			finergy.throw("{0}={1} must be of type str or None".format(key, val))
	...
	if title_updated:
		try:
			finergy.db.set_value(doctype, docname, title_field, new_title)
			finergy.msgprint(_("Saved"), alert=True, indicator="green")
		except Exception as e:
			if finergy.db.is_duplicate_entry(e):
				finergy.throw(
					_("{0} {1} already exists").format(doctype, finergy.bold(docname)),
					title=_("Duplicate Name"),
					exc=finergy.DuplicateEntryError,
				)
			raise
...
def rename_doc(
	...
	if not finergy.db.exists(doctype, old):
		return

	if ignore_if_exists and finergy.db.exists(doctype, new):
		return

	if old == new:
		finergy.msgprint(_("Please select a new name to rename"))
		return
	...
	# call before_rename
	old_doc = finergy.get_doc(doctype, old)
	...
	# call after_rename
	new_doc = finergy.get_doc(doctype, new)
	...
	if merge:
		new_doc.add_comment("Edit", _("merged {0} into {1}").format(finergy.bold(old), finergy.bold(new)))
	else:
		new_doc.add_comment(
			"Edit", _("renamed from {0} to {1}").format(finergy.bold(old), finergy.bold(new))
		)

	if merge:
		finergy.delete_doc(doctype, old)

	finergy.clear_cache()
	if rebuild_search:
		finergy.enqueue("finergy.utils.global_search.rebuild_for_doctype", doctype=doctype)
...
def update_assignments(old, new, doctype):
	old_assignments = finergy.parse_json(finergy.db.get_value(doctype, old, "_assign")) or []
	new_assignments = finergy.parse_json(finergy.db.get_value(doctype, new, "_assign")) or []
	...
	for user in common_assignments:
		...
		for todo in todos:
			finergy.delete_doc("ToDo", todo.name)

	unique_assignments = list(set(old_assignments + new_assignments))
	finergy.db.set_value(doctype, new, "_assign", finergy.as_json(unique_assignments, indent=0))
...
def update_user_settings(old, new, link_fields):
	...
	user_settings_details = finergy.db.sql(
...
def update_customizations(old: str, new: str) -> None:
	finergy.db.set_value("Custom DocPerm", {"parent": old}, "parent", new, update_modified=False)
...
def update_attachments(doctype, old, new):
	try:
		...
		if not finergy.db.is_column_missing(e):
			raise
...
def rename_eps_records(doctype, old, new):
	epl = finergy.qb.DocType("Energy Point Log")
	(
		finergy.qb.update(epl)
...
def rename_parent_and_child(doctype, old, new, meta):
	# rename the doc
	finergy.db.sql("UPDATE `tab{0}` SET `name`={1} WHERE `name`={1}".format(doctype, "%s"), (new, old))
...
def validate_rename(doctype, new, meta, merge, force, ignore_permissions):
	...
	if not (
		ignore_permissions or finergy.permissions.has_permission(doctype, "write", raise_exception=False)
	):
		finergy.msgprint(_("You need write permission to rename"), raise_exception=1)

	if not (force or ignore_permissions) and not meta.allow_rename:
		finergy.msgprint(_("{0} not allowed to be renamed").format(_(doctype)), raise_exception=1)
...
def rename_doctype(doctype, old, new, force=False):
	...
	fields_with_options = ("Link",) + finergy.model.table_fields
...
def update_link_field_values(link_fields, old, new, doctype):
	for field in link_fields:
		if field["issingle"]:
			try:
				single_doc = finergy.get_doc(field["parent"])
			...
			finergy.db.set_value(parent, {docfield: old}, docfield, new, update_modified=False)
...
def get_link_fields(doctype):
	...
	if not finergy.flags.link_fields:
		finergy.flags.link_fields = {}

	if not doctype in finergy.flags.link_fields:
		link_fields = finergy.db.sql(
			"""\
		...
		custom_link_fields = finergy.db.sql(
			"""\
		...
		property_setter_link_fields = finergy.db.sql(
			"""\
		...
		finergy.flags.link_fields[doctype] = link_fields

	return finergy.flags.link_fields[doctype]


def update_options_for_fieldtype(fieldtype, old, new):
	if finergy.conf.developer_mode:
		for name in finergy.get_all("DocField", filters={"options": old}, pluck="parent"):
			doctype = finergy.get_doc("DocType", name)
			save = False
...
def get_select_fields(old, new):
	...
	# get link fields from tabCustom Field
	custom_select_fields = finergy.db.sql(
		"""
		select dt as parent, fieldname,
			(select issingle from tabDocType dt
			where dt.name = df.dt) as issingle
	...
	# remove fields whose options have been changed using property setter
	property_setter_select_fields = finergy.db.sql(
		"""
		select ps.doc_type as parent, ps.field_name as fieldname,
			(select issingle from tabDocType dt
			where dt.name = ps.doc_type) as issingle
...
def update_parenttype_values(old, new):
	child_doctypes = finergy.db.get_all(
		"DocField",
		fields=["options", "fieldname"],
		filters={"parent": new, "fieldtype": ["in", finergy.model.table_fields]},
	)

	custom_child_doctypes = finergy.db.get_all(
		"Custom Field",
		fields=["options", "fieldname"],
		filters={"dt": new, "fieldtype": ["in", finergy.model.table_fields]},
	)

	child_doctypes += custom_child_doctypes
	fields = [d["fieldname"] for d in child_doctypes]

	property_setter_child_doctypes = finergy.get_all(
		"Property Setter",
	...
	for doctype in child_doctypes:
		finergy.db.sql(f"update `tab{doctype}` set parenttype=%s where parenttype=%s", (new, old))


def rename_dynamic_links(doctype, old, new):
	for df in get_dynamic_link_map().get(doctype, []):
		# dynamic link in single, just one value to check
		if finergy.get_meta(df.parent).issingle:
			refdoc = finergy.db.get_singles_dict(df.parent)
...
def bulk_rename(doctype, rows=None, via_console=False):
	...
	if not rows:
		finergy.throw(_("Please select a valid csv file with data"))

	if not via_console:
		max_rows = 500
		if len(rows) > max_rows:
			finergy.throw(_("Maximum {0} rows allowed").format(max_rows))
...
def update_linked_doctypes(doctype, docname, linked_to, value, ignore_doctypes=None):
	from finergy.model.utils.rename_doc import update_linked_doctypes
...
def get_fetch_fields(doctype, linked_to, ignore_doctypes=None):
	from finergy.model.utils.rename_doc import get_fetch_fields
...
def show_deprecation_warning(funct):
	...
	message = (
		f"Function finergy.model.rename_doc.{funct} has been deprecated and "
		"moved to the finergy.model.utils.rename_doc"
	)
