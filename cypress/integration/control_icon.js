context('Control Icon', () => {
	...
	it('should set icon', () => {
		...
		cy.get('.finergy-control[data-fieldname=icon] input').first().click();

		cy.get('.icon-picker .icon-wrapper[id=active]').first().click();
		cy.get('.finergy-control[data-fieldname=icon] input').first().should('have.value', 'active');
		...
		cy.get('.finergy-control[data-fieldname=icon] input').first().should('have.value', 'resting');
		cy.get('@dialog').then(dialog => {
			let value = dialog.get_value('icon');
