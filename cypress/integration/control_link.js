context('Control Link', () => {
	...
	it('should set the valid value', () => {
		...
		cy.intercept('POST', '/api/method/finergy.desk.search.search_link').as('search_link');

		cy.get('.finergy-control[data-fieldname=link] input').focus().as('input');
		cy.wait('@search_link');
		cy.get('@input').type('todo for link', { delay: 200 });
		cy.wait('@search_link');
		cy.get('.finergy-control[data-fieldname=link]').findByRole('listbox').should('be.visible');
		cy.get('.finergy-control[data-fieldname=link] input').type('{enter}', { delay: 100 });
		cy.get('.finergy-control[data-fieldname=link] input').blur();
	...
	it('should unset invalid value', () => {
		...
		cy.intercept('POST', '/api/method/finergy.client.validate_link').as('validate_link');

		cy.get('.finergy-control[data-fieldname=link] input')
			.type('invalid value', { delay: 100 })
			.blur();
		cy.wait('@validate_link');
		cy.get('.finergy-control[data-fieldname=link] input').should('have.value', '');
	});
	...
	it('should route to form on arrow click', () => {
		...
		cy.get('@todos').then(todos => {
			cy.get('.finergy-control[data-fieldname=link] input').as('input');
			cy.get('@input').focus();
			cy.wait('@search_link');
	...
	it('should update dependant fields (via fetch_from)', () => {
		cy.get('@todos').then(todos => {
			...
			cy.get('.finergy-control[data-fieldname=assigned_by] input').focus().as('input');
			cy.get('@input').type('Administrator', {delay: 100}).blur();
			cy.wait('@validate_link');
			cy.get('.finergy-control[data-fieldname=assigned_by_full_name] .control-value').should(
	...
	it("should set default values", () => {
		...
		cy.intercept("POST", "/api/method/finergy.desk.form.save.savedocs").as("save_form");
	...
	it("show translated text for Gender link field with language de with input in de", () => {
		cy.call("finergy.tests.ui_test_helpers.insert_translations").then(() => {
			cy.window()
				.its("finergy")
				.then((finergy) => {
					cy.set_value("User", finergy.user.name, { language: "de" });
				});
			...
			cy.wait("@search_link");
			cy.get(".finergy-control[data-fieldname=link] ul").should("be.visible");
			cy.get(".finergy-control[data-fieldname=link] input").type("{enter}", { delay: 100 });
			cy.get(".finergy-control[data-fieldname=link] input").blur();
			cy.get("@dialog").then((dialog) => {
	...
	it("show text for Gender link field with language en", () => {
		cy.window()
			.its("finergy")
			.then((finergy) => {
				cy.set_value("User", finergy.user.name, { language: "en" });
			});
