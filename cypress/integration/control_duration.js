context('Control Duration', () => {
	...
	it('should set duration', () => {
		...
		cy.get('.finergy-control[data-fieldname=duration] input')
			.first()
			.click();
		...
		cy.get('.duration-input[data-duration=minutes]')
			.type(30)
			.blur({ force: true });
		cy.get('.finergy-control[data-fieldname=duration] input').first().should('have.value', '45d 30m');
		cy.get('.finergy-control[data-fieldname=duration] input').first().blur();
	...
	it('should hide days or seconds according to duration options', () => {
		...
		cy.get('.finergy-control[data-fieldname=duration] input').first();
