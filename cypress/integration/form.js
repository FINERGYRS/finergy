context('Form', () => {
	before(() => {
		...
		return cy.window().its('finergy').then(finergy => {
			return finergy.call("finergy.tests.ui_test_helpers.create_contact_records");
		});
	});
	it('create a new form', () => {
		...
		cy.intercept({
			method: 'POST',
			url: 'api/method/finergy.desk.form.save.savedocs'
		}).as('form_save');
	...
	it('validates behaviour of Data options validations in child table', () => {
		...
		cy.get('.finergy-control[data-fieldname="email_ids"]').as('table');
