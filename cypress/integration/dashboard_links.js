...
context('Dashboard links', () => {
	before(() => {
		...
		return cy.window().its('finergy').then(finergy => {
			return finergy.xcall("finergy.tests.ui_test_helpers.update_child_table", {
	...
	it('check if child table is populated with linked field on creation from dashboard link', () => {
		...
		cy.get('.finergy-control[data-fieldname="child_table"] .rows .data-row .col[data-fieldname="doctype_to_link"]')
