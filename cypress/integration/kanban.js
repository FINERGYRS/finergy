context('Kanban Board', () => {
	...
	it('Create ToDo from kanban', () => {
		cy.intercept({
			method: 'POST',
			url: 'api/method/finergy.client.save'
		}).as('save-todo');
	...
	it('Add and Remove fields', () => {
		...
		cy.intercept('POST', '/api/method/finergy.desk.doctype.kanban_board.kanban_board.save_settings').as('save-kanban');
		cy.intercept('POST', '/api/method/finergy.desk.doctype.kanban_board.kanban_board.update_order').as('update-order');
		...
		cy.get('.finergy-control .label-area').contains('Show Labels').click();
		cy.click_modal_primary_button('Save');
		...
		cy.get('.page-actions .menu-btn-group > .btn').click();
		cy.get('.page-actions .menu-btn-group .dropdown-menu li').contains('Kanban Settings').click();
		cy.get_open_dialog().find('.finergy-control[data-fieldname="fields_html"] div[data-label="ID"] .remove-field').click();

		cy.wait('@update-order');
		cy.get_open_dialog().find('.finergy-control .label-area').contains('Show Labels').click();
		cy.get('.modal-footer .btn-primary').last().click();
	...
	// 	cy.intercept({
	// 		method: 'POST',
	// 		url: 'api/method/finergy.desk.doctype.kanban_board.kanban_board.update_order_for_single_card'
	// 	}).as('drag-completed');
