context('Grid', () => {
	...
	before(() => {
		...
		return cy.window().its('finergy').then(finergy => {
			return finergy.call("finergy.tests.ui_test_helpers.create_contact_phone_nos_records");
		});
	});
	it('update docfield property using update_docfield_property', () => {
		cy.visit('/app/contact/Test Contact');
		cy.window().its("cur_frm").then(frm => {
			cy.get('.finergy-control[data-fieldname="phone_nos"]').as('table');
			...
			cy.get('@table-form').find('.finergy-control[data-fieldname="is_primary_phone"]').should("be.hidden");
	...
	it('update docfield property using toggle_display', () => {
		...
		cy.window().its("cur_frm").then(frm => {
			...
			cy.get('@table').find('[data-idx="1"] .edit-grid-row').click();
			cy.get('.grid-row-open').as('table-form');
			cy.get('@table-form').find('.finergy-control[data-fieldname="is_primary_mobile_no"]').should("be.hidden");
			cy.get('@table-form').find('.grid-footer-toolbar').click();
	...
	it('update docfield property using toggle_enable', () => {
		...
		cy.window().its("cur_frm").then(frm => {
			...
			cy.get('@table').find('[data-idx="1"] .edit-grid-row').click();
			cy.get('.grid-row-open').as('table-form');
			cy.get('@table-form').find('.finergy-control[data-fieldname="phone"] .control-value').should('have.class', 'like-disabled-input');
			cy.get('@table-form').find('.grid-footer-toolbar').click();
