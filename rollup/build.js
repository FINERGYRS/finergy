...
const skip_finergy = process.argv.includes("--skip_finergy")

if (skip_finergy) {
	let idx = apps_list.indexOf("finergy");
	if (idx > -1) {
...
function build_files(files, app="finergy") {
	let ret;
...
function concatenate_files() {
	...
	const files_to_concat = Object.keys(get_build_json('finergy'))
		.filter(filename => filename.startsWith('concat:'));

	files_to_concat.forEach(output_file => {
		const input_files = get_build_json('finergy')[output_file];

		const file_content = input_files.map(file_name => {
			let prefix = get_app_path('finergy');
			if (file_name.startsWith('node_modules/')) {
				prefix = path.resolve(get_app_path('finergy'), '..');
			}
...
function show_production_message() {
	const production = process.env.FINERGY_ENV === 'production';
